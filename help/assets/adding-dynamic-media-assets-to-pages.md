...
要将Dynamic Media功能添加到您在网站上使用的资产中，您可以将 **Dynamic Media** 或 **交互式媒体** 组件。 为此，可进入布局模式并启用Dynamic Media组件。 然后，您可以将这些组件添加到页面，并将资产添加到该组件。 Dynamic Media和交互式媒体组件是智能的 — 它们知道您添加的是图像还是视频，可用选项会相应地发生更改。
...
Dynamic Media和交互式媒体可在 [!UICONTROL Dynamic Media] 选项卡 [!UICONTROL 组件]. 您使用 [!UICONTROL 交互式媒体] 组件。 对于所有其他Dynamic Media组件，请使用Dynamic Media组件。
...
>[!NOTE]
>
>添加Dynamic Media组件时，以及 **[!UICONTROL Dynamic Media设置]** 为空，或者您无法正确添加资产，请勾选以下内容：
>
>* 您拥有 [启用Dynamic Media](config-dynamic.md). Dynamic Media默认处于禁用状态。
