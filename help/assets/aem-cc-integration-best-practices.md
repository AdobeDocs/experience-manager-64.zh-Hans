...
|---|---|---|---|
| 与Creative用户共享较少数量(1)的DAM资源 | ✔✔ | ✔ |  |
| 与Creative用户共享更多(2)个DAM资源 | ✔✔ | ✘ | [Brand Portal](https://docs.adobe.com/content/help/en/experience-manager-brand-portal/using/home.html) <br> [资产共享](assets-finder-editor.md) |
| 与有权访问DAM的用户共享DAM资产 | ✔✔ | ✔ | [链接共享](link-sharing.md) |
| 与无权访问DAM的用户共享DAM资产 | ✘ | ✔✔ | [Brand Portal](https://docs.adobe.com/content/help/en/experience-manager-brand-portal/using/home.html) <br> [资产共享](assets-finder-editor.md) |
| 将较少的资源数／数量保存到DAM | ✔✔ | ✔ | [Web UI上传](managing-assets-touch-ui.md) |
| 将更多资源保存到DAM(3) | ✔✔ | ✘ | [Web UI上传自定义脚本](managing-assets-touch-ui.md) /工具 <br> （仅提供Web UI） |
...
大多数组织选择仅存储与下游营销/LOB流程相关的资产（通过AEM Sites发布到Web渠道等渠道或Adobe Experience Cloud提供的其他渠道- Marketing Cloud、Advertising cloud以及Analytics cloud衡量的、向用户／合作伙伴提供的资产等）。 此外，组织会存储在DAM中可能要经过审核／批准流程的资产。 这样，DAM会存储大部分具有高杠杆率的资产，并避免存储闲置资产。
...
通常与资产生命周期中的主要里程碑相关的更改。 例如，初始的创意就绪资源或创意团队提供的基于请求／审阅的正式更新应存储在DAM中并进行版本控制。
...
* 在创作团队确定资产准备就绪之前，在进行中工作阶段频繁对资产进行创意更改
...
通常，内部创意团队或已加入内部网络的代理／创意专业人士可以访问DAM实例，包括AEM登录名。

在这种情况下，AEM桌面应用程序可帮助您轻松访问最终／批准的资产，并可让您将创意就绪资产保存到DAM。

#### 无权访问DAM的创意用户 {#creative-users-without-access-to-dam}

没有直接访问DAM实例的外部机构和自由职业者可能需要访问已批准的资产或希望将其新设计添加到DAM。

在这种情况下，您可以利用AEM/Creative cloud集成来改进工作流。 先决条件是创意用户必须拥有Adobe ID，并拥有包含存储服务的Creative cloud帐户。

使用以下策略提供对最终／批准资产的访问：

* 要提供对大量资产的访问，请执行以下操作：使 [用AEM Assets Brand Portal](https://helpx.adobe.com/experience-manager/brand-portal/user-guide.html)，或客户在AEM发布基础结构上实 [施资源共享](assets-finder-editor.md) 。

* 要提供对几个资源的访问，请执行以下操作：除AEM Assets Brand Portal或Asset Share外，还可以使用与Adobe Creative Cloud共享的AEM文件夹。 请注意，此集成存在某些限制，本文将详细介绍这些限制。

### Use Cases {#use-cases}

以下用例描述了DAM与设计人员桌面之间的各种工作流程类型。

#### 使用DAM中的资源创建新设计 {#creating-new-designs-using-assets-from-dam}

下图说明了数字资产的生命周期。 它描述了创意用户和DAM用户（营销人员、LOB用户）如何利用现有资产并使用它们创建更多资产，并将其发送以供审批。

![chlimage_1-301](assets/chlimage_1-301.png)

资产生命周期包括以下阶段：

1. 将批准的资产共享到创意桌面：DAM中的最终资源可供创意用户使用（在桌面上）
1. 创建新设计（创意数字资产）:新文件存储在进行中(WIP)区域。
1. 在新设计中使用（放置）已批准的资产：创意用户使用Creative cloud应用程序中现有的已批准资产生成新资产
1. 频繁保存WIP更新：创意用户快速迭代并频繁保存文件。 在此阶段，创意用户可能与他人协作，但经常保存的更新通常对DAM用户不感兴趣。
1. 资产会达到创意就绪状态，并保存到Creative Ready文件夹
1. 资产更新：DAM中的用户可以使用资产更新或新文件
1. 资产投入生产：这是一个DAM流程，具体取决于组织，它可能包括标记、批准和更改访问控制。 在此阶段，资产被视为最终资产，而且能被更多利用DAM的团队使用吗？ 创意用户还可以使用它创建其他资源。

以下是关于如何通过此流程管理资产的一些一般建议：

* 为WIP文件使用专用存储区／系统，如Adobe Creative Cloud Assets已同步文件夹：与DAM用户无关的频繁更新最好由专用系统处理，而不是从AEM资产中处理。 WIP资产可以使用Adobe Creative cloud桌面应用程序同步到本地磁盘，并保存到本地存储中，等等。
...
* **** 1:将资源从DAM共享到桌面，或直接在桌面上选择的应用程序（例如，Adobe Photoshop等）中打开它。 建议注销以锁定文件。
* **** 2:次要更新：编辑文件并保存更改。
* 步骤2的替代流程

   * **** 答：主要更新：如果文件需要详细的更改集，则应间歇性保存并复制到WIP文件夹／区域。
   * **** B:继续处理WIP文件夹中的文件。 保存的更改不会同步到DAM中的版本
   * **** C:更新完成后，文件将复制回或保存到映射的文件夹

* **** 3:资产更新会反映在DAM中。 签入资产以将其解锁。
* **** 4:资产投入生产。

以下是关于如何在整个流程中管理资产的一些一般建议：

* 避免直接保存您从AEM桌面应用程序映射的网络共享中打开的文件，除非您对文件所做的更改很小。
* 如果要对文件进行其他更改、间歇性保存或与Creative团队协作，请将文件复制到单独的WIP文件夹。
...
* 上传照片或大型项目的结果
* 上传由创意机构提供的资产
* 如果选择是在DAM之外完成的，则从更大的集中上传选定的资产

请注意，此描述是指将文件作为桌面用户工作流程的常规部分进行操作（例如，每周或每张照片等）上传。 此处未涵盖大型资产迁移。

如果要批量上传资产，您可以利用以下功能：

* 要上传大型／分层文件夹，请使用AEM桌面应用程序，该应用程序提供“文件 [夹上传](https://helpx.adobe.com/experience-manager/desktop-app/aem-desktop-app.html#bulkupload) ”功能。 您还可以上传分层文件夹结构。 资产在后台上传，因此不会绑定到Web浏览器会话
* 如果要从单个文件夹上传几个文件，请将它们从桌面直接拖动到Web UI中，或使用AEM资产Web UI中的创建选项。
...
如果您使用网络文件共享管理数字资产，则仅使用由AEM桌面应用程序映射的网络共享可被视为便捷的替代方式。 从网络文件共享转换时，请记住，AEM Web UI提供了丰富的数字资产管理功能集，这些功能远远超出了网络共享（搜索、集合、元数据、协作、预览等）的可能范围，而AEM桌面应用程序提供了将服务器端DAM存储库与桌面工作相连的便捷链接。
