---
title: Assets 监控最佳实践
description: 监控部署后AEM实例的环境和性能的最佳实践。
contentOwner: AG
feature: 资产管理
role: 管理员，架构师
...
从Adobe Experience Manager(AEM)资产的观点来看，监测应包括观察和报告以下过程和技术：
...
通常，AEM Assets可以通过两种方式进行监控，即实时监控和长期监控。
...
* [首页](http://man7.org/linux/man-pages/man1/top.1.html):Top是打开仪表板的Linux命令，该命令显示使用情况统计信息，包括CPU、内存和IO使用情况。它提供了实例中所发生情况的高级概述。
...
* [Iotop](http://guichaz.free.fr/iotop/):Iotop是有关磁盘IO使用情况的详细仪表板。它显示描述使用磁盘IO的进程及其使用量的栏和仪表。 Iotop可以安装在使用`yum install iotop`或`apt-get install iotop`的大多数Linux系统上。

* [Iftop](http://www.ex-parrot.com/pdw/iftop/):Iftop显示有关以太网/网络使用的详细信息。Iftop显示使用以太网的实体的每个通信渠道统计信息以及它们使用的带宽量。 Iftop可安装在使用`yum install iftop`或`apt-get install iftop`的大多数Linux系统上。
...
* AEM error.log文件：您可以调查AEM error.log文件，了解已记录到系统中的错误的详细信息。 使用命令`tail -F quickstart/logs/error.log`识别您应调查的错误。
* [工作流控制台](../sites-administering/workflows.md):利用工作流控制台来监控滞后或卡住的工作流。

通常，您一起使用这些工具可以全面了解AEM实例的性能。
...
对AEM实例的长期监视包括对实时监视的相同部分进行较长的监视。 还包括定义特定于您的环境的警报。

### 日志聚合和报告{#log-aggregation-and-reporting}

聚合日志有多种可用工具，例如Splunk(TM)和Elastic Search/Logstash/Kabana(ELK)。 要评估AEM实例的正常运行时间，您必须了解特定于系统的日志事件并基于它们创建警报。 深入了解您的开发和操作实践可以帮助您更好地了解如何调整日志聚合过程以生成关键警报。
...
内部应用程序监视包括通过构建在平台上的自定义应用程序代码监视组成AEM堆栈的应用程序组件，包括JVM、内容存储库。 一般来说，它是通过JMX Mbeans执行的，这些解决方案可以由许多流行的监视解决方案直接进行监视，如SolarWinds(TM),HP OpenView(TM),Hyperic(TM),Zabix(TM)等。 对于不支持直接连接到JMX的系统，您可以编写shell脚本以提取JMX数据并以它们本身理解的格式向这些系统公开它。
...
与任何基于Java的应用程序堆栈一样，AEM取决于通过基础Java虚拟机提供给它的资源。 您可以通过JVM公开的平台MXBean来监视其中许多资源的状态。 有关MXBean的详细信息，请参阅[使用平台MBean服务器和平台MXBeans](https://docs.oracle.com/javase/7/docs/technotes/guides/management/mxbeans.html)。
...
AEM还通过JMX公布一组统计和操作。 这些功能有助于评估系统运行状况并在潜在问题影响用户之前找出它们。 有关详细信息，请参阅AEM JMX MBean上的[documentation](/help/sites-administering/jmx-console.md)。
...
**注意**:对于MBean和URL参数，请 `<AGENT_NAME>` 替换为要监视的复制代理的名称。
...
## 常见问题与决议{#common-issues-and-resolutions}

在监控过程中，如果您遇到问题，可以执行以下一些疑难解答任务，以解决AEM实例的常见问题：
...
* 调查来自客户端网络的出口点和到AEM实例网络的入口点，包括调度程序。 这些往往是瓶颈领域。 有关详细信息，请参阅[资产网络注意事项](assets-network-considerations.md)。
