---
title: 资产监控最佳实践
description: 在部署AEM实例后监控其环境和性能的最佳实践。
contentOwner: AG
...
* [可视化虚拟机](https://visualvm.github.io/):Visual VM允许您查看详细的Java VM信息，包括CPU使用情况、Java内存使用情况。 此外，它还允许您对运行于实例上的代码进行采样和评估。
* [顶部](http://man7.org/linux/man-pages/man1/top.1.html):顶部是打开功能板的Linux命令，该功能板显示使用情况统计信息，包括CPU、内存和IO使用情况。 它提供了实例中所发生情况的高级概述。
...
* [Iotop](http://guichaz.free.fr/iotop/):Iotop是有关磁盘IO使用情况的详细仪表板。 它显示描述使用磁盘IO的进程及其使用量的栏和表。 Iotop可以使用或安装在大多数Linux系 `yum install iotop` 统上 `apt-get install iotop`。

* [Iftop](http://www.ex-parrot.com/pdw/iftop/):Iftop显示有关以太网／网络使用的详细信息。 Iftop显示使用以太网的实体的每个通信通道的统计信息以及它们使用的带宽量。 Iftop可以使用或安装在大多数Linux系 `yum install iftop` 统上 `apt-get install iftop`。
...
* [工作流控制台](../sites-administering/workflows.md):利用工作流控制台来监视滞后或卡住的工作流。
...
* 警报阈值：当的值 `QueueBlocked` 为true或值大 `QueueNumEntries` 于基线的150%时。
...
* 调查来自客户端网络的出口点和指向AEM实例网络（包括调度程序）的入口点。 这些往往是瓶颈区。 有关详细信息，请参阅资 [产网络注意事项](assets-network-considerations.md)。
* 升级AEM服务器。 您的AEM实例可能大小不足。 Adobe支持可帮助您识别服务器是否大小不足。
