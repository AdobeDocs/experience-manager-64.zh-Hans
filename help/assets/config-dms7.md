...
## Dynamic Media-Scene7模式{#architecture-diagram-of-dynamic-media-scene-mode}的架构图

以下架构图描述了Dynamic Media-Scene7模式的工作方式。

借助新的体系结构，AEM负责主控资产并与Dynamic Media进行同步，以处理和发布资产：

1. 当主控资产上传到AEM时，会将其复制到Dynamic Media。 此时，Dynamic Media将处理所有资产处理和再现生成，如图像的视频编码和动态变型。
...
>[!NOTE]
>
>Dynamic Media-Scene7模式仅用于AEM作者实例。 因此，必须在AEM作者实例上配置`runmode=dynamicmedia_scene7`，而不是在AEM发布实例上配置。
...
如果要将AEMDynamic Media从6.3升级到6.4（现在包括零停机时间部署功能），您需要运行以下curl命令，以CRXDE Lite将所有预设和配置从`/etc`迁移到`/conf`。
...
功能包18912允许您通过FTP批量摄取资产，或在Dynamic Media上从AEM-混合模式或Dynamic Media经典迁移到Dynamic Media-Scene7模式。 客人可从Adobe Professional Services购买。

有关详细信息，请参见[安装功能包18912进行批量资产迁移](bulk-ingest-migrate.md)。
...
>[!NOTE]
>
>默认情况下，Cloud Services的配置路径为`/content/dam`。 任何其他配置路径都不受Dynamic Media-Scene7模式支持。
...
1. 如果连接成功，您还可以设置以下内容：
   ...
   * **[!UICONTROL 公司]** -Dynamic Media帐户的名称。您可能为不同子品牌、部门或不同的阶段／生产环境拥有多个Dynamic Media帐户。
   * **[!UICONTROL 公司根文件夹路径]**
   * **[!UICONTROL 发布资产]** -选 **** 项“立即”表示上传资产时，系统会收录资产并立即提供URL/嵌入。发布资产不需要用户干预。 选项&#x200B;**[!UICONTROL 在激活]**&#x200B;时，表示您需要先显式发布资产，然后才能提供URL/嵌入链接。
   * **[!UICONTROL 安全预览服务器]** -允许您指定到安全再现预览服务器的URL路径。也就是说，在生成再现后，AEM可以安全访问和预览远程Dynamic Media再现(不会将二进制文件发送回AEM实例)。
...
配置发布设置：在Dynamic Media经典中，点按&#x200B;**[!UICONTROL 设置>应用程序设置>发布设置>图像服务器]**。
...
要打开“[!UICONTROL 应用程序常规设置]”页，请在“Dynamic Media经典全局导航”栏中，点按&#x200B;**[!UICONTROL 设置>应用程序设置>常规设置]**。
...
>[!NOTE]
>
>要保持与AEM的一致性，请选择&#x200B;**[!UICONTROL 覆盖当前文件夹中的基本映像名称／扩展名]**。

* **[!UICONTROL 在任意文件夹中覆盖相同的基本资产名称]** /扩展名——要求替换图像的文件扩展名与原始图像相同(例如，替 `chair.jpg` 换 `chair.jpg` 但不替换 `chair.tif`)。但是，您可以将替换图像上传到与原始图像不同的文件夹。 更新后的图像驻留在新文件夹中；在文件的原始位置找不到该文件。
...
Dynamic Media颜色管理允许您对资产进行颜色校正。 通过颜色校正，摄取的资源保留其色彩空间（RGB、CMYK、灰色）和嵌入的颜色用户档案。 当您请求动态再现时，图像颜色会使用CMYK、RGB或灰色输出校正为目标色彩空间。 请参阅[配置图像预设](managing-image-presets.md)。
...
1. 滚动到需要更改的属性，例如&#x200B;**[!UICONTROL 颜色管理属性]**&#x200B;区域中的属性。
   ...
   * [!UICONTROL 颜色转换渲染方法] -指定渲染方法。可接受的值为`perceptual`、`relative`、`colometric`、`saturation`和`absolute colometric`。 Adobe建议将`relative`作为默认值。
...
1. 在AEM中，点按AEM徽标以访问全局导航控制台，然后点按&#x200B;**[!UICONTROL 工具]**（锤子）图标并导航到&#x200B;**[!UICONTROL 常规>CRXDE Lite]**。
1. 在左边栏中，导航到以下内容：
...
1. 在CRXDE Lite页的右侧，在下部分：
   ...
   * 多次-单击&#x200B;**[!UICONTROL jobParam]**&#x200B;以打开其关联的文本字段。 有关允许的处理参数值的列表，请参阅[支持的Mime类型](assets-formats.md#supported-mime-types)，这些参数值可用于给定的mime类型。
...
您可以为 AEM Assets 中不支持的格式添加自定义 MIME 类型。要确保在CRXDE Lite中添加的任何新节点不会被AEM删除，您必须确保在&#x200B;**[!UICONTROL image_]**&#x200B;之前移动MIME类型，并且其启用值设置为&#x200B;**[!UICONTROL false]**。

**为不支持的格式添加自定义MIME类型**

1. 从AEM中，单击&#x200B;**[!UICONTROL 工具>操作> Web Console]**。
...
1. 在页面上，向下滚动到名称&#x200B;**[!UICONTROL Adobe CQScene7资产MIME类型服务]**。 在名称右侧，点按&#x200B;**[!UICONTROL 编辑配置值]**（铅笔图标）。

   ![编辑](assets/2019-08-02_16-44-56.png)

1. 在&#x200B;**[!UICONTROL Adobe CQScene7资产MIME类型服务]**&#x200B;页面上，单击加号图标`+`。 在表中单击加号以添加新MIME类型的位置很琐碎。
...
1. 从AEM中，单击&#x200B;**[!UICONTROL 工具>常规>CRXDE Lite]**。
...
1. 在树中仍选择MIME类型`image_vnd.dwg`的&#x200B;**[!UICONTROL 属性]**&#x200B;选项卡中，在&#x200B;**[!UICONTROL enabled]**&#x200B;行的&#x200B;**[!UICONTROL 值]**&#x200B;列标题下，多次单击该值以打开&#x200B;**[!UICONTROL 值]**&#x200B;下拉列表。

1. 在字段中键入`false`(或从下拉列表中选择`false`)。

   ![虚假价值](assets/2019-08-02_16_60_30.png)
...
有两个元素可供定义：**[!UICONTROL Match]**&#x200B;和&#x200B;**[!UICONTROL Base Name]**。 这些字段允许您定义命名约定的所有元素，并标识用于命名包含这些元素的集合的约定部分。 公司的单个命名约定可对这些元素使用一行或多行定义。 您可以为您的唯一定义使用多行，并将它们分组为不同的元素，如主图像、颜色元素、替代视图元素和色板元素。
...
Dynamic Media使用批量集预设将资产组织为一组图像（替代图像、颜色选项、360旋转），以便在查看器中显示。 批量集预设会在Dynamic Media的资产上传流程旁边自动运行。

您可以创建、编辑和管理批集预设。 有两种形式的批集预设定义：一个用于您可能已设置的默认命名约定，另一个用于您动态创建的自定义命名约定。

您可以使用表单字段方法来定义批集预设或代码方法，它允许您使用常规表达式。 与默认命名一样，您可以在[!UICONTROL 表单视图]中定义的同时选择[!UICONTROL 视图代码]，并使用常规表达式构建您的定义。 或者，您也可以取消选中视图以使用其中一种或只使用另一种。
...
1. 在页面顶部附近的导航栏上，点按&#x200B;**[!UICONTROL 设置>应用程序设置>批集预设>批集预设]。**
   ...
   请注意，[!UICONTROL 视图表单]（如[!UICONTROL 详细信息]页面右上角所设置）是默认视图。

1. 在“预设”列表面板中，点按&#x200B;**[!UICONTROL 添加]**&#x200B;以激活屏幕右侧&#x200B;**[!UICONTROL 详细信息]**&#x200B;面板中的定义字段。
1. 在&#x200B;**[!UICONTROL 详细信息]**&#x200B;面板中的&#x200B;**[!UICONTROL 预设名称]**&#x200B;字段中，键入预设的名称。
...
1. 对于[!UICONTROL 序列顺序]，定义在Dynamic Media将图像集组合在一起后显示图像的顺序。

   默认情况下，资产按字母数字顺序排序。 但是，您可以使用逗号分隔的常规列表来定义顺序。

1. 对于&#x200B;**[!UICONTROL 设置命名]**&#x200B;和&#x200B;**[!UICONTROL 创建约定]**，请指定您在&#x200B;**[!UICONTROL 资产命名约定]**&#x200B;中定义的基本名称的后缀或前缀。 此外，定义在Dynamic Media文件夹结构中创建集的位置。
...
根据此信息，您的[!UICONTROL 批集类型]菜谱可能会创建如下：

![chlimage_1-1](assets/chlimage_1-1.png)

旋转集的共享资产名称部分的分组将添加到“匹 **[!UICONTROL 配]** ”字段（高亮显示）。 资产名称中包含行和列的变量部分将分别添加到 **[!UICONTROL 行]** 和 **[!UICONTROL 列字段]** 。

上传和发布旋转集后，您可以激活2D旋转集菜谱的名称，该菜谱列在&#x200B;**[!UICONTROL 上传作业选项]**&#x200B;对话框的&#x200B;**[!UICONTROL 批量集预设]**&#x200B;下。
...
1. 对于&#x200B;**[!UICONTROL 设置命名]**&#x200B;和&#x200B;**[!UICONTROL 创建约定]**，请指定您在&#x200B;**[!UICONTROL 资产命名约定]**&#x200B;中定义的基本名称的后缀或前缀。
...
1. 导航到`https://<server>/system/console/configMgr`并搜索&#x200B;**[!UICONTROL 队列：Granite Workflow Queue]**。
...
在非Dynamic Media部署中，您将AEM作者环境的*所有*资产（图像和视频）复制到AEM发布节点。 此工作流是必需的，因为AEM发布服务器也会传送资产。
...
1. 在AEM中，点按AEM徽标以访问全局导航控制台，然后点按&#x200B;**[!UICONTROL 工具]**&#x200B;图标，并导航到&#x200B;**[!UICONTROL 常规>CRXDE Lite]**。
