...
Dynamic Media-需要启用并配置混合功能以供使用。 根据您的用例，Dynamic Media有几个[支持的配置](#supported-dynamic-media-configurations)。

>[!NOTE]
>
>如果要在Scene7运行模式下配置和运行Dynamic Media，请参阅[配置Dynamic Media-Scene7模式](config-dms7.md)。
>
>如果要在混合运行模式下配置和运行Dynamic Media，请按照本页中的说明操作。

了解有关在Dynamic Media使用[视频](video.md)的更多信息。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     <li>在AEM <strong>author</strong>节点上，<a href="#enabling-dynamic-media">启用动态媒体</a>。</li> 
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     <li>在AEM <strong>author</strong>节点上，<a href="#enabling-dynamic-media">启用动态媒体</a>。</li> 
     <li>在AEM <strong>publish</strong>节点上，<a href="#enabling-dynamic-media">启用Dynamic Media</a>（发布实例为视频海报图像提供服务并为视频回放提供元数据）。</li> 
     <li>在<a href="#configuring-dynamic-media-cloud-services">Dynamic MediaCloud Services中配置视频。</a></li> 
     <li><a href="#replicating-viewer-presets">复制查看器预设</a>。</li> 
     <li>为“仅视频”</a>设置<a href="#setting-up-asset-filters-for-video-only-deployments">资产过滤器。</a></li> 
     <li><a href="#delivering-assets">交付资产。</a></li> 
    </ol> </td> 
  </tr> 
  <tr> 
...
[默认](https://www.adobe.com/solutions/web-experience-management/dynamic-media.html) 情况下禁用动态媒体。要利用Dynamic Media功能，您需要像使用&#x200B;**[!UICONTROL dynamicmedia]**&#x200B;运行模式一样使用&#x200B;**[!UICONTROL publish]**&#x200B;运行模式来启用Dynamic Media。 启用前，请确保查看[技术要求](/help/sites-deploying/technical-requirements.md#requirements-for-aem-dynamic-media-add-on)。

>[!NOTE]
>
>通过运行模式启用Dynamic Media将取代AEM 6.1和AEM 6.0中的功能，在这两个模式中，您通过将&#x200B;**[!UICONTROL dynamicMediaEnabled]**&#x200B;标志设置为&#x200B;**[!UICONTROL true]**&#x200B;来启用Dynamic Media。 此标志在AEM 6.2和更高版本中没有功能。 此外，您无需重新启动快速启动即可启用Dynamic Media。
...
1. 请求`http://localhost:4502/is/image`并确保图像服务器现在正在运行。
   ...
   >[!NOTE]
   >
   >要对Dynamic Media问题进行疑难解答，请参阅&#x200B;**[!UICONTROL crx-quickstart/logs/]**&#x200B;目录中的以下日志：
   >
   ...
   * s7access-&lt;yyyy>&lt;mm>&lt;dd>.log - s7access日志记录通过`/is/image`和`/is/content`向Dynamic Media发出的每个请求。
   这些日志仅在启用Dynamic Media时使用。 它们不包含在从&#x200B;**[!UICONTROL system/console/status-Bundlelist]**&#x200B;页面生成的&#x200B;**下载完整**&#x200B;包中；如果您遇到Dynamic Media问题，请致电客户支持，在问题后附加这两个日志。
...
* 如果未配置&#x200B;**自**&#x200B;域，则从Jetty HTTP服务检索端口号和上下文路径。

在AEM QuickStart WAR部署中，无法派生端口号和上下文路径，因此必须配置&#x200B;**self**&#x200B;域。 有关如何配置&#x200B;**self**&#x200B;域的信息，请参见[externalizer文档](/help/sites-developing/externalizer.md)。
...
1. 请求`http://localhost:4502/is/image`。 您会收到一条消息，说Dynamic Media被禁用。

   >[!NOTE]
   禁用“Dynamic Media”运行模式后，将自动跳过生成`qdam.pyramid.tiff`再现的工作流步骤。 这也会禁用动态再现支持和其他Dynamic Media功能。
...
如果您将AEMDynamic Media从6.3升级到6.4(现在包括零停机时间（也称为“选择加入”）部署)，则需要运行以下curl命令，以CRXDE Lite将所有预设和配置从`/etc`迁移到`/conf`。
...
配置复制代理后，您需要[验证并测试它是否已成功设置](#validating-the-replication-agent-for-dynamic-media)。 本节介绍这些过程。

>[!NOTE]
创建PTIFF的默认内存限制为所有工作流的3 GB。 例如，您可以处理一个需要3 GB内存的图像，而其他工作流则暂停，或者可以并行处理10个图像，每个图像需要300 MB内存。
内存限制是可配置的，并且应符合系统资源可用性和正在处理的图像内容类型。 如果您拥有许多超大资源并且系统内存充足，您可以增加此限制以确保并行处理图像。
需要超过最大内存限制的图像将被拒绝。
要更改创建PTIFF的内存限制，请导航到&#x200B;**[!UICONTROL 工具>操作> Web控制台>Adobe CQScene7PTiffManager]**&#x200B;并更改`maxMemory`值。
...
1. 在“用户管理”页上，导航到&#x200B;**[!UICONTROL dynamic-media-replication]**&#x200B;用户，然后点按打开。

   ![dm复制](assets/dm-replication.png)

1. 在“Edit User Settings For dynamic-media-replication”页中，点按&#x200B;**[!UICONTROL Keystore]**&#x200B;选项卡，然后点按&#x200B;**[!UICONTROL 创建KeyStore]**。
...
1. 在&#x200B;**[!UICONTROL 设置KeyStore访问密码]**&#x200B;对话框中输入密码并确认密码。
   ...
   >[!NOTE]
   请记住您输入的密码。 以后配置&#x200B;**[!UICONTROL 复制代理]**&#x200B;时，您需要再次输入它。

   ![chlimage_1-508](assets/chlimage_1-508.png)

1. 在&#x200B;**[!UICONTROL 编辑Dynamic-media-replication的用户设置]**&#x200B;页面上，展开KeyStore文件&#x200B;]**中的**[!UICONTROL &#x200B;添加私钥并添加以下内容（请参阅下面的图像）:

   * 在&#x200B;**[!UICONTROL 新建别名]**&#x200B;字段中，输入您以后在复制配置中使用的别名的名称；例如，**replication**。
   * 点按&#x200B;**[!UICONTROL 密钥存储文件]**。 导航到通过Adobe提供给您的KeyStore文件，选择它，然后点按&#x200B;**[!UICONTROL 打开]**。
   * 在&#x200B;**[!UICONTROL KeyStore File Password]**&#x200B;字段中，输入KeyStore File密码。 这是您在步骤5中创建的KeyStore密码&#x200B;_，但是是KeyStore文件密码Adobe在配置过程中发送给您的欢迎电子邮件中提供的。_&#x200B;如果您未收到KeyStore文件密码，请与Adobe客户服务部门联系。
   * 在&#x200B;**[!UICONTROL 私钥密码]**&#x200B;字段中，输入私钥密码（可能与上一步中提供的私钥密码相同）。 Adobe在配置过程中向您发送的欢迎电子邮件中提供私钥密码。 如果您未收到私钥密码，请与Adobe客户服务联系。
   * 在&#x200B;**[!UICONTROL 私钥别名]**&#x200B;字段中，输入私钥别名。 例如，`companyname-alias`。 Adobe在设置过程中向您发送的欢迎电子邮件中提供私钥别名。 如果您未收到私钥别名，请与Adobe客户服务联系。
...
1. 点按&#x200B;**[!UICONTROL 设置]**&#x200B;选项卡，然后输入：
   ...
   * **[!UICONTROL 密钥存储别名]** -此值与在设置身份验证中生成密钥时设置的**新别名**值 [相同](#setting-up-authentication);例如 `replication`,（请参见[设置身份验证](#setting-up-authentication)中的步骤7。）
   * **[!UICONTROL 密钥存储密码]** -这是您在点击创建密钥存储时创建的 **[!UICONTROL KeyStore密码]**。Adobe不提供此密码。 请参阅[设置身份验证](#setting-up-authentication)的步骤5。
...
此问题可能是由于未为`dynamic-media-replication`用户设置KeyStore而引起的。
...
**解决方案**:检查是否 `KeyStore` 已保存 **[!UICONTROL 到Dynamic-]** Media-Replication用户并提供了正确的密码。
...
此问题由AEM作者实例中的配置错误引起。 作者上的java进程未获得正确的`javax.net.ssl.trustStore`。 复制日志中显示此错误：
...
1. 点按&#x200B;**[!UICONTROL KeyStore]**&#x200B;选项卡。 如果出现&#x200B;**[!UICONTROL 创建KeyStore]**&#x200B;按钮，则需要重做[之前设置身份验证](#setting-up-authentication)下的步骤。
...
* 添加的图像预设。
* 在&#x200B;**[!UICONTROL Cloud Services]**&#x200B;下配置&#x200B;**Dynamic Media配置（6.3之前版本）**。 此测试需要图像服务URL
...
>[!NOTE]
在设置Dynamic MediaCloud Services之前，请务必设置发布实例。 配置Dynamic MediaCloud Services之前，还必须设置复制。
...
**何时使用：** 在配置Dynamic Media **[!UICONTROL 配置（6.3之前版本）时]**，将启动包括视频报告在内的许多功能。该配置在区域Analytics公司中创建报表包。 如果配置多个作者节点，则为每个节点创建一个单独的报表包。 因此，报告数据在安装之间不一致。 此外，如果每个作者节点引用同一混合发布服务器，则上次作者安装会更改所有视频报告的目标报表包。 此问题会使Analytics系统的报表包过多。

**开始：通** 过完成以下三个报告配置视频任务。

1. 在第一个作者节点上配置&#x200B;**[!UICONTROL Dynamic Media配置（6.3之前版本）]**&#x200B;后，创建[!DNL Video Analytics]预设包。 此初始任务很重要，因为它允许新配置继续使用同一报表包。
1. 将[!DNL Video Analytics]预设包安装到配置Dynamic Media配置（6.3之前）之前的任何&#x200B;***新***&#x200B;作者节点&#x200B;***。***
...
1. 如果尚未配置&#x200B;**[!UICONTROL Dynamic Media配置（6.3之前版本）]**。
1. （可选）视图并复制&#x200B;**[!UICONTROL 报表包ID]**（您必须有权访问JCR）。 虽然不需要使用&#x200B;**[!UICONTROL 报表包ID]**，但它使验证更容易。
...
1. 构建包。
1. 下载或共享[!DNL Video Analytics]预设包，以便与后续新的作者节点共享。

### 在配置其他作者节点{#installing-the-video-analytics-preset-package-before-you-configure-additional-author-nodes}之前，请先安装[!DNL Video Analytics]预设包

确保在&#x200B;_之前完成此任务_，配置&#x200B;**[!UICONTROL Dynamic Media配置（6.3之前）]**。 否则，将创建另一个未使用的报表包。 此外，即使视频报告仍能正常工作，数据收集也未得到优化。

确保第一个作者节点中的[!DNL Video Analytics]预设包可在新的作者节点上访问。

1. 将您创建的[!DNL Video Analytics]预设包上传到&#x200B;**[!UICONTROL 包管理器]**。
...
设，通 [!DNL Video Analytics] 过JCR检查预设，您必须有权访 **[!UICONTROL 问CRXDE Lite]**。
...
* 不首先安装[!DNL Video Analytics]预设包会导致创建新报告包。
* 从AEM 6.3升级到AEM 6.4或AEM 6.4.1，然后配置&#x200B;**[!UICONTROL Dynamic Media配置（6.3之前）]**&#x200B;仍然会创建报表包。 此问题已知并将在AEM 6.4.2中修复。

### 关于[!DNL Video Analytics]预设{#about-the-video-analytics-preset}

[!DNL Video Analytics]预设（有时也称为分析预设）存储在Dynamic Media的查看器预设旁边。 它基本上与查看器预设相同，但包含用于配置AppMeasurement和视频心跳报告的信息。
...
这些过滤器为您提供了一种将&#x200B;_排除_&#x200B;资产复制到AEM发布节点的方法。

### 使用默认资产过滤器进行复制{#using-default-asset-filters-for-replication}

如果您正在将Dynamic Media用于生产&#x200B;_或_ 2)成像和视频中的1)成像，则可以使用我们按原样提供的默认过滤器。 默认情况下，以下过滤器处于活动状态：
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>现成的“过滤器——视频”将： 
    <ul> 
     <li>包括用于复制的代理视频演绎版、视频缩略图／海报图像、元数据（父视频和视频演绎版中均有）（任何以<strong>cqdam</strong>开头的演绎版）。</li> 
...
>[!NOTE]
如果作者中有许多不同的过滤器，则每个代理都需要分配给它一个不同的用户。 花岗岩代码强制实施每用户一个过滤器模型。 对于设置的每个过滤器，始终具有不同的用户。
如果您在服务器上使用多个筛选器(例如，一个要发布的复制筛选器和s7投放的第二个筛选器)，则需要确保这两个过滤器在&#x200B;**[!UICONTROL jcr:content]**&#x200B;节点中分配了不同的&#x200B;**userId**。 请参阅下图：
...
使用上面的示例，如果您只想复制PTIFF（金字塔TIFF），则应输入`+cqdam,*`，其中包括与`cqdam`开始的所有再现。 在示例中，该再现为`cqdam.pyramid.tiff`。
...
**入门项目**: _在_ 配置Dynamic Media映像服务器之前，请确保您的Windows虚拟机包括Microsoft Visual C++库的安装。运行Dynamic Media图像服务器时必须使用这些库。 您可以[在此下载Microsoft Visual C++ 2010 Redistributable Package(x64)](https://www.microsoft.com/en-us/download/details.aspx?id=14632)。

**配置Dynamic Media图像服务器设置**:

1. 在AEM的左上角，点按&#x200B;**[!UICONTROL Adobe Experience Manager]**&#x200B;以访问全局导航控制台，然后点按&#x200B;**[!UICONTROL 工具>操作> Web Console]**。
1. 在&#x200B;**[!UICONTROL Adobe Experience ManagerWeb控制台配置]**&#x200B;页面上，点按&#x200B;**[!UICONTROL OSGi >配置]**&#x200B;以列表AEM中当前运行的所有捆绑包。
...
1. 在捆绑包的列表中，点按&#x200B;**[!UICONTROL Adobe CQScene7图像服务器]**&#x200B;的右侧，点按&#x200B;**[!UICONTROL 编辑]**&#x200B;图标。
1. 在&#x200B;**[!UICONTROL Adobe CQScene7ImageServer]**&#x200B;对话框中，设置以下配置值：
...
1. 点按&#x200B;**[!UICONTROL 保存]**。
1. 在包的列表中，在&#x200B;**[!UICONTROL Adobe CQScene7平台服务器]**&#x200B;的右侧，点按&#x200B;**[!UICONTROL 编辑]**&#x200B;图标。
1. 在&#x200B;**[!UICONTROL Adobe CQScene7平台服务器]**&#x200B;对话框中，设置以下默认值选项：
   ...
   | **[!UICONTROL 高速缓存最大大小]** | 200000000 | 响应缓存的最大大小（字节）。 |
...
默认清单配置的位置来自&#x200B;**[!UICONTROL Adobe CQScene7平台服务器]**&#x200B;捆绑包的&#x200B;**[!UICONTROL 目录根]**&#x200B;默认值。 默认情况下，此值位于&#x200B;**[!UICONTROL 工具>常规>CRXDE Lite]**&#x200B;中的以下路径：
...
请确保点按&#x200B;**[!UICONTROL 访问控制]**&#x200B;选项卡（**[!UICONTROL 属性]**&#x200B;选项卡的右侧），然后将每个用户和动态媒体复制用户的访问控制权限设置为`jcr:read`。
...
1. 导航到[软件分发](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)并下载`cq-6.3.0-featurepack-12445`。
   ...
   有关在[!DNL Adobe Experience Manager]中使用包的详细信息，请参见[如何使用包](/help/sites-administering/package-manager.md)。
...
1. 在&#x200B;**[!UICONTROL 工具>常规>CRXDE Lite]**&#x200B;中，导航到`/conf/global/settings/dam/dm/imageserver/configuration/settings`，其中包含默认的Adobe Color用户档案。

   ![chlimage_1-514](assets/chlimage_1-514.png)

1. 通过滚动到&#x200B;**[!UICONTROL 属性]**&#x200B;选项卡的底部并手动输入属性名称、类型和值，添加颜色校正属性，如下表所述。 输入值后，点按&#x200B;**[!UICONTROL 添加]**，然后点按&#x200B;**[!UICONTROL 保存全部]**&#x200B;以保存值。

   颜色校正属性在&#x200B;**[!UICONTROL 颜色校正属性]**&#x200B;表中进行了说明。 可分配给颜色校正属性的值位于&#x200B;**[!UICONTROL 颜色用户档案]**&#x200B;表中。

   例如，在&#x200B;**[!UICONTROL 名称]**&#x200B;中，添加`iccprofilecmyk`，选择&#x200B;**[!UICONTROL 类型]** `String`，将`WebCoated`添加为&#x200B;**[!UICONTROL 值]**。 点按&#x200B;**[!UICONTROL 添加]**，然后点按&#x200B;**[!UICONTROL 保存全部]**&#x200B;以保存您的值。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>复制图像URL</td> 
   <td><p>“复制URL”对话框显示与以下内容类似的URL（URL仅供演示之用）:</p> <p><code>https://IMAGESERVICEPUBLISHNODE/is/image/content/dam/path/to/Image.jpg?$preset$</code></p> <p>其中<code>IMAGESERVICEPUBLISHNODE</code>指的是图像服务URL。</p> <p>另请参阅<a href="/help/assets/delivering-dynamic-media-assets.md">传送Dynamic Media资产</a>。</p> </td> 
  </tr> 
  <tr> 
   <td>复制查看器URL</td> 
