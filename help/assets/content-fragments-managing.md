...
内容片段存储为&#x200B;**[!UICONTROL 资产]**，因此主要从&#x200B;**[!UICONTROL 资产]**&#x200B;控制台进行管理。
...
[在使用结](content-fragments-models.md) 构化内容创建内容片段之前，可以启用和创建内容片段模型。
...
1. 向导的第一步要求您指定新片段的基础。
   ...
   * 这可以是：
      ...
      * [模型](content-fragments-models.md)  — 用于创建需要结构化内容的片段；例如， **** Airportmodel
   * 将显示所有可用的模板和模型。
...
在&#x200B;**[!UICONTROL 资产]**&#x200B;控制台中，您的内容片段可以执行一系列操作，其中之一是：
...
* **[!UICONTROL 编辑]**
   ...
   * 允许您[打开片段以编辑内容](content-fragments-variations.md)及其元素、变量、关联内容和元数据。

* **[!UICONTROL 管理标记]**
* **[!UICONTROL 目标收藏集]**

   * 将片段添加到集合。
   * 当[将集合与片段](content-fragments-assoc-content.md#adding-associated-content)关联时，也可以执行此操作。
...
>[!NOTE]
>
>其中许多是[对Assets](managing-assets-touch-ui.md)和/或[桌面应用程序](https://helpx.adobe.com/cn/experience-manager/desktop-app/aem-desktop-app.html)的标准操作。
...
>[!NOTE]
>
>也可以从时间轴](https://helpx.adobe.com/experience-manager/6-3/assets/using/content-fragments-managing.html#timeline-for-content-fragments)创建、比较和还原版本。[
...
* **[!UICONTROL 保存]**
   ...
   >[!NOTE]
   >
   >在选择&#x200B;**[!UICONTROL 保存]**&#x200B;之前，可以保留在编辑器中并进行一系列更改。

   >[!CAUTION]
   >
   >除了简单地保存更改外，**[!UICONTROL Save]**&#x200B;还会更新任何引用并确保调度程序根据需要刷新。 这些更改可能需要时间才能处理。 因此，对于大型/复杂/重负载的系统，可能会产生性能影响。
   >
   >
   >在使用&#x200B;**[!UICONTROL 保存]**&#x200B;时请牢记这一点，然后快速重新输入片段编辑器以进行和保存进一步的更改。

* **[!UICONTROL 取消]**

   将退出编辑器，而不保存最新更改。

编辑内容片段时，AEM会自动创建版本，以确保在您&#x200B;**[!UICONTROL 取消]**&#x200B;更改时，先前的内容可以恢复：

1. 打开内容片段以编辑AEM时，将检查是否存在基于cookie的令牌，该令牌指示是否存在&#x200B;*编辑会话*:

   1. 如果找到令牌，则片段将被视为现有编辑会话的一部分。
   1. 如果令牌&#x200B;*不*&#x200B;可用，且用户开始编辑内容，则会创建一个版本，并将此新编辑会话的令牌发送到客户端，该令牌保存在Cookie中。

1. 当存在&#x200B;*active*&#x200B;编辑会话时，每600秒（默认）自动保存一次正在编辑的内容。
...
1. 如果用户选择&#x200B;**[!UICONTROL 取消]**&#x200B;编辑，则恢复在编辑会话开始创建的版本，并删除令牌以结束编辑会话。
1. 如果用户选择&#x200B;**[!UICONTROL 保存]**&#x200B;编辑，则会保留更新的元素/变量，并删除令牌以结束编辑会话。

## 编辑片段{#editing-the-content-of-your-fragment}的内容

打开片段后，您可以使用[变量](content-fragments-variations.md)选项卡创作内容。

## 在片段{#creating-and-managing-variations-within-your-fragment}中创建和管理变量

创建主控内容后，即可创建和管理该内容的[变量](content-fragments-variations.md)。

## 将内容与片段{#associating-content-with-your-fragment}关联

您还可以[将内容](content-fragments-assoc-content.md)与片段关联。 这提供了一种连接，以便在将片段添加到内容页面时，可以（可选）将资产（即图像）与片段一起使用。

## 查看和编辑片段{#viewing-and-editing-the-metadata-properties-of-your-fragment}的元数据（属性）

您可以使用[[!UICONTROL 元数据]](content-fragments-metadata.md)选项卡视图和编辑片段的属性。
...
* 版本操作
   ...
   * **[[!UICONTROL 与当前比较]](#comparing-fragment-versions)** （先选择一个现有片段，然后选择特定版本）
   * 添加&#x200B;**[!UICONTROL Label]**&#x200B;和/或&#x200B;**[!UICONTROL Comment]**（先选择一个现有片段，然后选择特定版本）
   * **[!UICONTROL 另存为版本]** （选择一个现有片段，然后选择时间轴底部的向上箭头）
...
选择特定版本后，可从[[!UICONTROL 时间轴]](https://helpx.adobe.com/experience-manager/6-3/assets/using/content-fragments-managing.html#timeline-for-content-fragments)中执行&#x200B;**[!UICONTROL 与当前版本比较]**&#x200B;操作。
...
* 直接从[[!UICONTROL 时间轴]](content-fragments-managing.md#timeline-for-content-fragments)。

   选择所需的版本，然后执行&#x200B;**[!UICONTROL 还原到此版本]**&#x200B;操作。

* 当[将某个版本与当前版本](content-fragments-managing.md#comparing-fragment-versions)进行比较时，您可以&#x200B;**[!UICONTROL 将]**&#x200B;还原到选定版本。

## 发布和引用片段{#publishing-and-referencing-a-fragment}

>[!CAUTION]
>
>如果片段基于模型，则应确保已发布[模型](content-fragments-models.md#publishing-a-content-fragment-model)。
>
