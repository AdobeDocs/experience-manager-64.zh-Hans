...
Adobe Experience Manager(AEM)内容片段允许您设计、创建、策划和发 [布独立于页面的内容](/help/sites-authoring/content-fragments.md)。 它们允许您准备内容，准备好在多个位置／通过多个渠道使用。

使用AEM核心组件的Sling Model(JSON)导出功能，内容片段也可以以JSON格式交付。 这种交付形式：

* 允许您使用组件管理片段中要传送的元素
* 允许批量交付，方法是在用于API交付的页面上添加多个内容片段核心组件

本页和以下页面涵盖创建、配置和维护内容片段的任务：

* [管理内容片段](content-fragments-managing.md) -创建您的内容片段；然后编辑、发布和引用

* [内容片段模型](content-fragments-models.md) -启用、创建和定义模型

* [变量——创作片段内容](content-fragments-variations.md) -创作片段内容并创建主节点的变量
...
>[!NOTE]
>
>这些页面应结合使用内容片段 [进行页面创作来读取](/help/sites-authoring/content-fragments.md)。

通信渠道的数量每年都在增加。 通常，渠道指的是交付机制，如：

* 物理渠道；例如，桌面、移动设备。
* 实体渠道中的交付形式；例如，“产品详细信息页面”、“产品类别页面”（适用于桌面）或“移动Web”（适用于移动设备的移动应用程序）。
...
此结构化内容与AEM核心组件的JSON导出功能一起可用于将AEM内容交付到AEM页面以外的渠道。
...
>[!NOTE]
>
>AEM还支持片段内容的翻译。 有关更 [多信息，请参阅为内容片段创建翻译项目](creating-translation-projects-for-content-fragments.md) 。
...
* 简单片段
   ...
   * 它们没有预定义的结构。 它们仅包含文本和图像。
   * 这些模板基于简单片段模板。

* 包含结构化内容的片段

   * 这些代码基于内容 [片段模型](content-fragments-models.md)，该模型为生成的片段预定义了结构。
...
* 在页面编 [辑器中通过内容片段组件](/help/sites-authoring/content-fragments.md) （引用组件）使用：

   * 内容 **片段组件** 可供页面作者使用。 它允许他们以HTML或JSON格式引用和交付所需的内容片段。

内容片段是一种内容结构，它：

* 没有布局或设计（在富文本模式下，可以设置一些文本格式）。
...
* **将资产插入** （混合媒体片段）
   ...
   * 定义资产的位置。
   * 有关 [详细信息，请参阅在片段编辑器中将资产插入片段](content-fragments-variations.md#inserting-assets-into-your-fragment) 。
   >[!NOTE]
   >
...
* **关联的内容**
   ...
   * 有关详 [细信息](content-fragments-assoc-content.md) ，请参阅关联内容。
...
* 页面编辑器的&#x200B;**资产浏览器**&#x200B;中的可用资产
   ...
   * 为定位提供了一定的灵活性。
   * 不提供特定片段的批准概念。
   * 有关更 [多信息，请参阅](/help/sites-authoring/author-environment-tools.md#assets-browser) “资产浏览器”。
...
* **片段元素**
   ...
   * 对于简单的片段：
      ...
      * 元素在片段模板中定义(创作片段时无法定义，请参阅内容片段 [模板](/help/sites-developing/content-fragment-templates.md))。
...
* **插入到片段（混合媒体片段）中的资产**
   ...
   * 只能使用片段编辑器中的富文本格式添加到片段、从片段中删除 [片段或在片段中移动](content-fragments-variations.md#inserting-assets-into-your-fragment)。
   * 只能添加到多行文本元素（任何片段类型）。
   * 附加到前面的文本（段落）。
   >[!CAUTION]
   >
   ...
   >[!NOTE]
   >
   >在页面上使用片 [段时，还可以将资产添加为其他](/help/sites-authoring/content-fragments.md#using-associated-content) （中间）内容；使用关联内容或资产浏览器中的资产。

* **关联的内容**

   * 这是片段外部的内容，但与片段的编辑相关。 通常是图像、视频或其他片段。
   * 将集合中的单个资产添加到页面时，这些资产可用于页面编辑器中的片段。 这意味着，这些渠道是可选的，具体取决于特定渠道的要求。
   * 资产通过集 [合与片段关联](content-fragments-assoc-content.md);关联的集合允许作者决定在创作页面时要使用哪些资产。

      * 集合可以通过模板、默认内容或作者在片段创作过程中与片段关联。
      * [资产(DAM)集合](managing-collections-touch-ui.md) 是片段关联内容的基础。
   * （可选）您还可以将片段本身添加到集合以帮助跟踪。


* **片段元数据**

   * 使用资产 [元数据架构](metadata.md)。
   * 在以下情况下可以创建标记：
...
* **母版**
   ...
   * 碎片的一个完整部分
      ...
      * 每个内容片段都有一个主实例。
      * 无法删除主视图。
   * 在片段编辑器中的“变量”下可访问主 **[页](content-fragments-variations.md)**。
   * “主”不是变体，而是所有变体的基础。


* **变量**

   * 特定于编辑目的的片段文本的再现；可以与频道相关，但不是强制的，也可以是临时本地修改。
   * 创建为主页的副 **本**，但可以根据需要进行编辑；这些变体之间通常存在内容重叠。
   * 可以在片段创作过程中定义，也可以在片段模板中预定义。
   * 存储在片段中，以帮助避免内容副本的散布。
   * 如果主内容 [已更新](content-fragments-variations.md#synchronizing-with-master) ，则可以与主内容同步变量。
   * 可以进行 [汇总](content-fragments-variations.md#summarizing-text) ，以将文本快速截断到预定义的长度。
   * 可在片段编辑 [器的](content-fragments-variations.md) “变量”选项卡下使用。
...
* 在 [页面上使用／引用片段后](/help/sites-authoring/content-fragments.md#adding-in-between-content) ，在片段流中添加的其他内容是否。
...
* **内容模型**
   ...
   * 启 [用，然后使用工具创建](content-fragments-models.md)。
   * 创建结 [构化片段必需](content-fragments-managing.md#creating-content-fragments)。
   * 定义片段的结构（标题、内容元素、标记定义）。
   * 内容模型定义需要一个标题和一个数据元素；其他一切都是可选的。 模型定义片段和默认内容的最小范围（如果适用）。 创作片段内容时，作者无法更改定义的结构。

* **片段模板**

   * 创建简 [单片段所需](content-fragments-managing.md#creating-content-fragments)。
