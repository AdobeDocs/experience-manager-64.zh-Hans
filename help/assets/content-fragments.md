...
Adobe Experience Manager(AEM)内容片段允许您设计、创建、管理和[发布与页面无关的内容](/help/sites-authoring/content-fragments.md)。 它们允许您准备内容，准备好在多个位置／多个渠道使用。
...
* [管理内容片段](content-fragments-managing.md) -创建您的内容片段；然后，编辑、发布和引用
...
>[!NOTE]
>
>这些页面应结合[使用内容片段创作页面](/help/sites-authoring/content-fragments.md)进行读取。
...
* 包含结构化内容的片段
   ...
   * 这些代码基于[内容片段模型](content-fragments-models.md)，该模型预定义所生成片段的结构。
...
* **插入到片段（混合媒体片段）中的资产**
   ...
   * 只能在片段编辑器](content-fragments-variations.md#inserting-assets-into-your-fragment)中使用[富文本格式的片段中添加、删除或移动该片段。
   * 只能添加到多行文本元素（任何片段类型）。
   * 附加到前面的文本（段落）。
   ...
   >[!NOTE]
   >
   >在页面上使用片段时，资产也可以添加为[附加（中间）内容](/help/sites-authoring/content-fragments.md#using-associated-content);使用资产浏览器中的关联内容或资产。
...
* **变量**
   ...
   * 存储在片段中，以帮助避免内容副本的散布。
   * 如果主控内容已更新，变量可以为[synchronized](content-fragments-variations.md#synchronizing-with-master)且主控。
   * 可以是[Asgumated](content-fragments-variations.md#summarizing-text)，以快速将文本截断为预定义的长度。
   * 位于片段编辑器的[变量](content-fragments-variations.md)选项卡下。
...
* 是在片段](/help/sites-authoring/content-fragments.md#adding-in-between-content)的流中添加的[附加内容，一旦在页面上使用／引用它。
