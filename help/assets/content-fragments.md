...
Adobe Experience Manager(AEM)内容片段允许您设计、创建、策划和 [发布独立于页面的内容](/help/sites-authoring/content-fragments.md). 利用这些功能，可准备内容以准备在多个位置/多个渠道中使用。
...
>[!NOTE]
>
>这些页面应结合 [使用内容片段进行页面创作](/help/sites-authoring/content-fragments.md).
...
>
>有关详细信息，另请参见[了解 AEM 中的内容片段和体验片段](https://helpx.adobe.com/experience-manager/kt/platform-repository/using/content-fragments-experience-fragments-article-understand.html)。
...
>[!NOTE]
>
>AEM 还支持片段内容的翻译。 请参阅 [为内容片段创建翻译项目](creating-translation-projects-for-content-fragments.md) 以了解更多信息。
...
* 包含结构化内容的片段
   ...
   * 这些值基于 [内容片段模型](content-fragments-models.md)，用于为生成片段预定义结构。
...
* **插入资产**&#x200B;到片段（混合媒体片段）
   ...
   * 定义资产的位置。
   * 请参阅[将资产插入片段](content-fragments-variations.md#inserting-assets-into-your-fragment)（在片段编辑器中）以了解更多信息。
...
* **关联的内容**
   ...
   * 请参阅[关联内容](content-fragments-assoc-content.md)以了解更多信息。
...
* 页面编辑器的&#x200B;**资产浏览器**&#x200B;中的可用资产
   ...
   * 具有一定的定位灵活性。
   * 不提供为特定片段批准的概念。
   * 请参阅[资产浏览器](/help/sites-authoring/author-environment-tools.md#assets-browser)以了解更多信息。
...
* **片段段落**
   ...
   * 文本块，即：
      ...
      * 由垂直空格（回车）分隔
      * 在多行文本元素中；在简单或结构化片段中
   * 在[富文本](content-fragments-variations.md#rich-text)和 [Markdown](content-fragments-variations.md#markdown) 模式中，一个段落可以被格式化为一个标题，在这种情况下它和后面的段落属于一个单元。
   * 在页面创作期间启用内容控制。
...
* **插入到片段中的资产（混合媒体片段）**
   ...
   * 只能在片段编辑器中使用[富文本格式](content-fragments-variations.md#inserting-assets-into-your-fragment)添加、删除或移动片段。
   * 只能添加到多行文本元素（任何片段类型）。
   * 附于前文（段落）。
...
* **关联的内容**
   ...
   * 资产包括[通过收藏集关联到片段](content-fragments-assoc-content.md)；关联的收藏集允许作者决定在创作页面时要使用的资产。

      * 收藏集可以通过模板、默认内容或在片段创作期间由作者与片段关联。
      * [资产 (DAM) 收藏集](managing-collections-touch-ui.md)是片段关联内容的基础。
...
* **主控**
   ...
   * 片段的一个组成部分
      ...
      * 每个内容片段都有一个主控实例。
      * 无法删除主控。
   * 主控可以在&#x200B;**[变体](content-fragments-variations.md)**&#x200B;下的片段编辑器中访问。
   * 主控不是此变体，而是所有变体的基础。
...
* **变体**
   ...
   * 存储在片段中，以帮助避免内容副本的散布。
   * 如果主控内容已更新，则变体可以与主控[同步](content-fragments-variations.md#synchronizing-with-master)。
   * 可以[概述](content-fragments-variations.md#summarizing-text)以快速将文本截断为预定义的长度。
   * 在片段编辑器的[变体](content-fragments-variations.md)选项卡下可用。
...
* **内容模型**
   ...
   * 是 [启用，然后使用工具创建](content-fragments-models.md).
   * 需要 [创建结构化片段](content-fragments-managing.md#creating-content-fragments).
   * 定义片段的结构（标题、内容元素、标记定义）。
   * 内容模型定义需要一个标题和一个数据元素；其他内容都是可选的。 模型定义片段和默认内容的最小范围（如果适用）。 创作片段内容时，作者无法更改定义的结构。

* **片段模板**

   * 需要 [创建简单片段](content-fragments-managing.md#creating-content-fragments).
   * 通常 [在项目实施过程中开发](/help/sites-developing/content-fragment-templates.md);创作时无法创建。
