...
智能内容服务是托管在AdobeI/O上的云服务。 要在Adobe Experience Manager(AEM)中使用它，系统管理员必须将AEM实例与AdobeIO集成。
...
管理员可以点击链接将智能内容服务与AEM集成。 要将服务与AEM Assets集成，请参 [阅配置智能标记](config-smart-tagging.md)。

管理员配置服务并在AEM中添加用户时，入门过程即完成。

>[!NOTE]
>
>如果您使用AEM 6.3或更早版本并且需要为资产自动添加标签服务，请参阅智 [能标签](https://helpx.adobe.com/experience-manager/6-3/assets/using/touch-ui-smart-tags.html)。 智能标记不使用AI功能，其准确性低于增强的智能标记功能。
...
您可以启用智能内容服务，以定期培训文件夹中的资产和相关标记。 打开资产文件夹的属性页面，在详细 **[!UICONTROL 信息选项卡下]** ，选 **[!UICONTROL 择启]** 用智能标记，然后保存更改。
...
1. 要下载报告，请从列表中选择它，然后点按／单击工 **[!UICONTROL 具栏]** 中的下载图标。 报告以Excel文件的形式下载。
