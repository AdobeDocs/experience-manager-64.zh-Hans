---
title: 文件夹元数据架构
description: 本文介绍如何为AEM Assets的资产文件夹创建元数据模式
...
本文介绍如何为AEM Assets的资产文件夹创建元数据模式。

Adobe Experience Manager(AEM)资产允许您为资产文件夹创建元数据模式，定义在文件夹属性页面中显示的布局和元数据。

>[!NOTE]
>
>此功能要求AEM 6.4（至少部署了Service Pack 2）。 有关AEM 6.4 Service Pack的详细信息，请参阅这 [些发行说明](/help/release-notes/sp-release-notes.md)。
...
1. 在文件夹元数据模式Forms页面中，点按／单 **[!UICONTROL 击创建]**。
1. 指定表单的名称，然后点按／单击创 **[!UICONTROL 建]**。 新的模式表单列在模式Forms页面。
...
1. 在“模式Forms”页面中，选择您创建的表单，然后点按／单击工 **[!UICONTROL 具栏]** 中的编辑图标。
...
1. 在AEM Assets用户界面中，点按／单 **[!UICONTROL 击工]** 具栏中的创建。
1. 指定文件夹的标题和名称。
