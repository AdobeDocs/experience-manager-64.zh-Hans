...
# Dynamic Media图像用户档案{#image-profiles}

上传图像时，您可以通过将图像用户档案应用到文件夹，在上传时自动裁剪图像。

>[!NOTE]
>
>Smart Crop仅在Dynamic Media-Scene7模式下可用。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>像素裁剪</td> 
   <td>仅根据尺寸批量裁切图像。</td> 
   <td><p>要使用此选项，请从“裁剪选项”下拉列表中选择<strong>像素裁剪</strong>。</p> <p>要从图像的侧边进行裁剪，请输入要从图像的任一侧边或各个侧边裁剪的像素数。裁剪的图像多少取决于图像文件中的 ppi（每英寸像素数）设置。</p> <p>图像用户档案像素裁剪采用以下方式：<br /> </p> 
    <ul> 
  ...
  <tr> 
   ...
   <td><p>Smart Crop利用Adobe Sensei人工智能的强大功能快速实现批量图像裁剪自动化。 Smart Crop可自动检测并裁切到任何图像的焦点，以捕获预期的兴趣点，而不管屏幕大小。</p> <p>要使用智能裁剪，请从“裁剪选项”下拉列表中选择<strong>智能裁剪</strong>，然后在响应式图像裁剪的右侧，启用（打开）该功能。</p> <p>大、中和小的默认断点大小通常涵盖大多数图像在移动和平板电脑设备、桌面和横幅上使用的所有大小。 如果需要，可以编辑“大”、“中”和“小”的默认名称。</p> <p>要添加更多断点，请单击<strong>添加裁剪</strong>;要删除裁剪，请单击垃圾桶图标。</p> </td> 
  </tr> 
  <tr> 
   <td>颜色和图像样本</td> 
...
有关“锐化”的信息，请参阅[锐化图像](/help/assets/assets/s7_sharpening_images.pdf)。
...
1. 点按AEM徽标，然后导航到&#x200B;**[!UICONTROL 工具>资产>图像用户档案]**。
1. 选择要编辑或删除的图像配置文件。要编辑图像配置文件，请选择&#x200B;**[!UICONTROL 编辑图像处理配置文件]**。要删除该用户档案，请选择&#x200B;**[!UICONTROL 删除图像处理]**。
...
在向现有图像用户档案添加智能裁剪时，如果要为资产存储库中的现有资产生成裁剪，则需要重新触发[DAM更新资产工作流](assets-workflow.md)。
...
1. 执行下列操作之一：
   ...
   * 导航到&#x200B;**https://&lt;AEM服务器>/mnt/overlay/dam/gui/content/assets/foldersharewizard.html/content/dam**&#x200B;并应用相应的用户档案，然后点按&#x200B;**保存**。
...
1. 点按AEM徽标并导航到&#x200B;**[!UICONTROL 资产]**，然后导航到应用了智能裁剪或智能样本图像用户档案的文件夹。
...
1. 在页面的右上角附近，点按&#x200B;**[!UICONTROL 保存]**。然后点按&#x200B;**[!UICONTROL 关闭]**&#x200B;以返回资产文件夹。
