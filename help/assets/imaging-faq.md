...
智能成像技术使用Adobe Sensei AI功能，并与现有的“图像预设”配合使用，通过基于客户端浏览器功能自动优化图像格式、大小和质量，从而提高图像交付性能。
...
| [图2](https://techsupport.scene7.com/is/image/TechSupport/SmartImaging_3?hei=500&amp;fmt=jpg&amp;qlt=85&amp;resmode=bisharp&amp;op_usm=5,0.125,5,0) | ![picture2](assets-dm/picture2.png) | 191 KB | 70.66 KB | 63% |
| [图像3](https://techsupport.scene7.com/is/image/TechSupport/SmartImaging_2?hei=500&amp;fmt=jpg&amp;qlt=85&amp;resmode=bisharp&amp;op_usm=5,0.125,5,0) | ![图片3](assets-dm/picture3.png) | 96.64 KB | 39.44 KB | 59% |
...
图像构成页面大部分加载时间。 因此，任何性能改进都可能对提高转化率、在网站逗留时间和降低网站跳出率产生深远影响。
...
否. “智能成像”功能包含在您现有的Dynamic Media Classic或Adobe Experience Manager - Dynamic Media许可证(内部部署、AMS和Experience Manageras a Cloud Service)中。
...
智能成像可与您现有的“图像预设”配合使用，如果请求的文件格式为JPEG或PNG，则它会观察除质量(`qlt`)和格式(`fmt`)以外的所有图像设置。 对于格式转换，Adobe会根据图像预设设置的定义，保持完整的视觉保真度，但文件大小较小。 如果原始图像大小小于智能成像生成的图像大小，则会提供原始图像。
...
点按&#x200B;**[!UICONTROL 设置]** > **[!UICONTROL 应用程序设置]** > **[!UICONTROL 常规设置]**。 查找标有&#x200B;**[!UICONTROL Published Server Name]**&#x200B;的字段。 如果您当前使用的是通用域，则在提交技术支持票证时，可以请求转移到您自己的自定义域作为此过渡的一部分。
...
客户网页不存在风险。 但是，过渡到智能成像会清除CDN中的缓存，因为它涉及在Experience Manager上转移到Dynamic Media Classic或Dynamic Media的新配置。
...
1. 使用智能成像配置帐户后，在浏览器上加载Dynamic Media Classic/Dynamic Media图像URL。
...
>[!NOTE]
>
>并非所有图像都会转换。 “智能成像”功能可决定转换是否提高性能。 有时，如果没有预期的性能增益或格式不JPEG或PNG，则不会转换图像。
