...
否. Smart Imaging包含在您现有的Dynamic Media Classic或Adobe Experience Manager - Dynamic Media(内部部署、AMS和Experience Manager作为Cloud Service)许可证中。
...
“智能成像”可与您现有的“图像预设”配合使用，如果请求的文件格式为JPEG或PNG，则“智能成像”会检查除质量(`qlt`)和格式(`fmt`)以外的所有图像设置。 对于格式转换，Adobe会根据图像预设设置的定义，保持完整的视觉保真度，但文件大小较小。 如果原始图像大小小于智能成像生成的图像大小，则会提供原始图像。
...
>[!NOTE]
>
>并非所有图像都会转换。 “智能成像”功能可决定转换是否提高性能。 有时，如果没有预期的性能增益或格式不是JPEG或PNG，则不会转换图像。
