...
1. 要了解`com.day.cq.dam.cq-dam-switchengine`的捆绑id，请登录Web控制台，然后点按&#x200B;**[!UICONTROL OSGi >捆绑]**。 或者，要打开捆绑包控制台，请访问`https://[aem_server:[port]/system/console/bundles/` URL。 找到`com.day.cq.dam.cq-dam-switchengine`捆绑包及其ID。

1. 通过使用命令`ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/`检查文件夹，确保解压所有所需的库，其中文件夹名称是使用捆绑ID构建的。 例如，如果bundle id为`588`，则命令为`ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle588/data/binaries/`。
...
1. 在用于开始AEM的帐户中，编辑`.bash_profile`文件。 添加`LD_LIBRARY_PATH`。
...
验证配置、上传TIFF图像并监视error.log文件。 您会注意到`INFO`消息中提及`SwitchEngineHandlingProcess execute: executing command line`。 日志中提到生成的演绎版。 工作流完成后，您可以在AEM中视图新再现。
