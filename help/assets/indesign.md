...
这些任务可以涵盖各种任务；例如，使用Adobe inDesign server处理文件。
...
InDesign server可以与AEM资产集成，以便上传使用InDesign( `.indd`)创建的文件、生成再现、提取所有媒体 ** （例如，视频）并存储为资产：
...
1. 框架通过SOAP（简单对象访问协议）将命令脚本发送到InDesign Server。
   ...
   * 将生成的文件发布回AEM资产。
   ...
   >
   >有关 [详细信息，请参阅Adobe inDesign Interchange Formats INX和IDML](http://www.peachpit.com/articles/article.aspx?p=1381880&seqNum=8) 。

   >[!CAUTION]
   >
   >如果InDesign server未安装或未配置，则仍可将文件上传 `.indd` 到AEM中。 但是，生成的演绎版将限 `png` 制为 `jpeg`并且，您将无法生成或 `html`生成 `idml` 页面演绎版。
...
默认情况下，IDML导出提取处理程序可用。 它对在“媒体提取” `IDML` 步骤中生成的再现进行操作。

* **页面名称**:指定要分配给结果页面的名称。 如果留空，则名称为“page”（如果“page”已存在，则为派生名称）。

* **页面标题**:指定要分配给结果页面的标题。

* **页面根路径**:生成页面的根位置的路径。 如果保留为空，则使用包含资产演绎版的节点。
...
1. 打开Felix **[!UICONTROL Console的]** “配置”选项卡；例如：
...
1. 保存这些更改。
1. 要启用对Adobe CS6和更新版本的多会话支持，请选中下方的 `enable.multisession.name` 复选框 `com.day.cq.dam.ids.impl.IDSJobProcessor.name configuration`。
1. 通过向IDS worker [配置添 `*x*>` 加SOAP端点，创建&lt; IDS Worker的池](#configuring-the-proxy-worker-for-indesign-server)。
   ...
   >
   >此外，在配置 `com.day.cq.dam.ids.impl.IDSPoolImpl.name` 下，为max.errors.to.blacklist参数设置一个正值，该值确定在将IDS从作业处理程序列表中禁止之前的作业检索数
   >
