---
title: 资产 HTTP API
description: 了解Assets HTTP API的实施、数据模型和功能。 使用资产HTTP API执行资产周围的各种任务
contentOwner: AG
...
资产HTTP API允许对资产执行创建——读取——更新——删除(CRUD)操作，包括二进制、元数据、演绎版和注释，以及使用AEM内容片段的结构化内容。 它在上公开， `/api/assets` 并作为REST API实现。

访问API:

1. 打开位于的API服务文档 `http://[hostname]:[port]/api.json`。
1. 请按照以下链接的“资产”服务链 `http://[hostname]:[server]/api/assets.json`接：

API响应是某些MIME类型的JSON，是所有MIME类型的响应代码。 JSON响应是可选的，可能不可用，例如PDF文件。 依赖响应代码进行进一步分析或操作。

结束 [!UICONTROL 时间后]，资产及其演绎版不能通过资产Web界面或通过HTTP API使用。 如果开始时间是将来的， [!UICONTROL 或结束时间是过去的] ，则API会返回404 [!UICONTROL 错误消息] 。

## Data model {#data-model}

资产HTTP API公开两个主要元素，文件夹和资产。
...
>[!NOTE]
>
>文件夹或资产的某些属性会映射到其他前缀。 JCR前缀 `jcr:title`、 `jcr:description`和 `jcr:language` 替换为前 `dc` 缀。 因此，在返回的JSON中， `dc:title` 并 `dc:description` 分别包含 `jcr:title` 和的值 `jcr:description`。
...
* 资产的属性和元数据。
* 多个演绎版，如原始演绎版（最初上传的资产）、缩略图和各种其他演绎版。 其他再现可能是不同大小的图像、不同的视频编码或从PDF或Adobe inDesign中提取的页面。
* 可选注释。
...
>[!NOTE]
>
>为了便于读取，以下示例省略了完整的cURL记号。 事实上，该记号确实与 [Resty](https://github.com/micha/resty) （cURL的脚本包装器）相关。

**前提条件**

* 转到 `https://[AEM_server]:[port]/system/console/configMgr`.
* 导航到 **[!UICONTROL Adobe Granite CSRF滤镜]**。
* 确保属性“过滤 **[!UICONTROL 器方法]** ”包括： `POST`, `PUT`, `DELETE`。
...
创建新 `sling`:在给 `OrderedFolder` 定路径。 如果是a&amp;ast;给出的是节点名，而不是节点名，则servlet将使用参数名作为节点名。 作为请求数据接受是新文件夹的Siren表示形式或一组名称——值对，编码为或 `application/www-form-urlencoded` / `multipart``form``data`- ，对于直接从HTML表单创建文件夹很有用。 此外，文件夹的属性可以指定为URL查询参数。
...
在给定路径上使用给定文件创建DAM资产。 如果是a&amp;ast;给定的是节点名，而不是节点名，则servlet将使用参数名或文件名作为节点名。
