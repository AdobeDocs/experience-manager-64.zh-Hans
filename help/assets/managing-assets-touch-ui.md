...
|  | 阈值 | 阈值是应用USM锐化滤镜时忽略的对比度范围。 这很重要，因此在使用此滤镜时，不会给图像引入“杂色”。 值范围是0-255，它是灰度图像中的亮度步骤数。 0=黑色，128=50%灰色，255=白色。<br> 例如，阈值为12时会忽略肤色亮度的细微变化，以避免添加杂色，但仍会为对比区域添加边缘对比度，如睫毛与皮肤相遇的地方。<br> 例如，如果您有一张某人脸部的照片，USM锐化会影响图像的对比部分，如睫毛和皮肤相遇以创建明显的对比度区域以及平滑的皮肤本身。 即使最平滑的外观也会显示亮度值的细微变化。 如果不使用阈值，滤镜会突出外观像素的这些细微更改。 反之，在增加睫毛的对比度的同时产生噪声和不期望的效果，从而增强锐度。<br> 为避免此问题，引入了一个阈值，告知滤镜忽略不会显着改变对比度的像素，如平滑外观。<br> 在前面显示的拉链图形中，请注意拉链旁边的纹理。 由于阈值过低而无法抑制噪声，因此出现图像噪声。 |
