...
本节介绍如何创建、修改和一般管理图像预设。 无论您何时预览图像，都可以对图像应用图像预设。See [Applying Image Presets](image-presets.md).
...
要在AEM中管理图像预设，请点按AEM徽标以访问全局导航控制台，然后点按工具图标并导航到资产>图像预 **[!UICONTROL 设]**。
...
>[!NOTE]
>
>当您在资产的详细信息视图中选择 **[!UICONTROL 演绎版]** ，系统会显示各种 **[!UICONTROL 演绎版]** 。 您可以增加或减少显示的图像预设数。 See [Increasing the number of image presets that display](#increasing-or-decreasing-the-number-of-image-presets-that-display).
...
* PDF文档由一个或多个页面组成。 每页都会作为单页PDF子资产提取到主多页PDF文档下。
...
在打开资产时，您可以查看子资产或页面，点按内容菜单，然后选择子 **[!UICONTROL 资产]** 或 **[!UICONTROL 页面]**。 子资产是真实资产。 即，PDF页面由工作流组件 `Create Sub Asset` 提取。 然后，它们会作 `page1.pdf`为、 `page2.pdf`等存储在主资产下方。 存储资产后， **[!UICONTROL DAM更新资产工作流会处理]** 这些资产。

要使用Dynamic media预览和生成AI、EPS或PDF文件的动态演绎版，需要执行以下处理步骤：

1. 在 **[!UICONTROL DAM更新资产工作流程中]** ,“栅格化 **[!UICONTROL PDF/AI图像预览演绎版”流程组件使用配置的分辨率将原始资产的第一页栅格化为演绎]**`cqdam.preview.png` 版。

1. 然后 `cqdam.preview.png` ，再现会通过工作流中的 **[!UICONTROL Dynamic Media Process图像资产流程组件优化为PTIFF]** 。
...
您可以通 **[!UICONTROL 过“DAM更新资产”工作流访问“栅格化PDF/AI图像预览演绎版]** ”流程组 **[!UICONTROL 件选项]** 。

点按左上角的Adobe Experience Manager，导航到工具>工 **[!UICONTROL 作流>模型]**。 在“工 **[!UICONTROL 作流模型]** ”页面上，选择 **[!UICONTROL DAM更新资产]**，然后在工具栏中点按编 **[!UICONTROL 辑]**。 在“ **[!UICONTROL DAM更新资产”工作流程页]** ，双击“栅格化PDF/AI图像预览演绎版”流程组件以打开其“步骤属性 ******** ”对话框。
...
**[!UICONTROL “最大宽度]** ”和“最 **[!UICONTROL 大高度]** ”限制栅格化时的分辨率。 例如，如果最大值保持不变，且“分辨率”设置为300 ppi，则“美国字母”文档将以186 ppi栅格化。 即，文档为1581 x 2046像素。

栅格 **[!UICONTROL 化PDF/AI图像预览演绎版流程组件定义了最大值]** ，以确保它不会在内存中创建过大的图像。 这样的大映像可能会溢出提供给JVM（Java虚拟机）的内存。 必须注意向JVM提供足够的内存来管理所配置的并行工作流数，每个工作流都有可能以最大配置大小创建映像。
...
**[!UICONTROL DAM更新资产工作流中的媒体提取流程组件运行多个预配置]** 的扩展脚本 **** ，以处理InDesign文件。

![媒体提取过程参数中的扩展脚本路径](assets/media_extraction_arguments.png)

在 **[!UICONTROL DAM]** 更新资产工作流中媒体提取流程组件的参数中，扩展脚本路径 ******** 。
...
缩览图大小按以下格式定义： **width:height:center**，例如 *80:80:false*。 宽度和高度决定缩览图的像素大小；中心值为false或true，如果设置为true，则表示缩略图的大小与配置中给定的大小完全相同。 如果调整后的图像较小，则图像将居中在缩略图中。
...
1. 点按Dynamic Media Process图 **[!UICONTROL 像资产步骤]** ，然后点按缩略图 **[!UICONTROL 选项卡]** 。 根据需要更改缩略图大小，然后点按&#x200B;**[!UICONTROL 确定]**。
...
在预览资产时，您创建的图像预设可以作为动态演绎版使用。 从详细信息视图>演绎版查看资产时，AEM会显示 **[!UICONTROL 各种动态演绎版]**。 您可以增加或减少显示的演绎版限制。
...
>[!NOTE]
>
>如果使用Internet Explorer 9，则创建预设不会在保存后立即显示在预设列表中。 要解决此问题，请禁用 IE9 的缓存。
...
1. 在“编 **[!UICONTROL 辑图像预设]********** ”页面上，根据需要在“基本”和“高级”选项卡中输入值，包括名称。 图像预设选项中概 [述了这些选项](#image-preset-options)。 预设显示在左窗格中，并可以与其他资产一起动态使用。
...
在创建或编辑图像预设时，您可以使用本节介绍的几种选项。此外，Adobe建议从以下三个最佳 *实践选* 项开始：

* **[!UICONTROL 格式]** (**[!UICONTROL 基本选项卡]** )-选择 **[!UICONTROL JPEG]** 或其他符合您要求的格式。所有Web浏览器都支持JPEG图像格式；它可以在小文件大小和图像质量之间实现良好的平衡。但是，JPEG格式图像使用有损压缩方案，如果压缩设置太低，则会引入不需要的图像伪影。 因此，Adobe 建议将压缩质量设置为 75。此设置在图像质量和小文件大小之间提供了良好的平衡。
* **[!UICONTROL 启用简单锐化]** -请勿选择启 **[!UICONTROL 用简单锐化]** （此锐化滤镜提供的控制比USM锐化设置少）。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>您可以覆盖默认的渲染方法。 渲染方法确定目标颜色配置文件中无法再现的颜色（溢色）会发生什么情况。 如果“渲染方法”与ICC配置文件不兼容，则会忽略它。 
    <ul> 
     <li>选择 <strong>“可感知</strong> ”，当原始图像中的一种或多种颜色超出目标色彩空间的色域时，将一个色彩空间的总色域压缩为另一个色彩空间。</li> 
     <li>当当前 <strong>颜色空间中的颜色超出目标颜色空间中的色域时，选择“相对比色”</strong> ，并且您希望将其映射到目标颜色空间的色域中最接近的颜色，而不影响任何其他颜色。 </li> 
  ...
  <tr> 
   ...
   <td>如果输出配置文件支持此功能，请选择此选项。 如果黑点补偿与指定的ICC配置文件不兼容，则忽略黑点补偿。</td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><p>Select <strong>None</strong>, <strong>Sharpen</strong>, or <strong>Unsharp Mask</strong>. </p> 
    ...
    <div>
      有关“锐化”的信息，请参阅<a href="https://marketing.adobe.com/resources/help/en_US/s7/sharpening/s7_sharpening_images.pdf">锐化图像</a>。 
    </div> </td> 
  </tr> 
  <tr> 
...
* [op_invert](https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/http_ref/r_op_invert.html) —— 反转每个颜色分量以获得负片图像效果。
...
* [op_blur](https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/http_ref/r_op_blur.html) - 向图像应用模糊滤镜。
...
* [op_brightness](https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/http_ref/r_op_brightness.html) —— 降低或增加亮度。
...
* [opac](https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/http_ref/r_opac.html) - 调整图像不透明度。可用于降低前景不透明度。
...
1. 在AEM中，点按AEM徽标以访问全局导航控制台。
1. 点按工 **[!UICONTROL 具图标]** ，然后导航到资 **[!UICONTROL 产>图像预设]**。
1. 从图像预设列表中选择一个或多个图像预设，然后点按发 **[!UICONTROL 布]**。
...
1. 在AEM中，点按AEM徽标以访问全局导航控制台。
1. 点按工 **[!UICONTROL 具图标]** ，然后导航到资 **[!UICONTROL 产>图像预设]**。
1. 选择预设，然后点按删 **[!UICONTROL 除]**。 Dynamic media会确认您是否要删除它。 点按 **[!UICONTROL 删除]**。
