...
可以定义一个工作流，当特定类型或格式的资产上传到服务器时，该工作流会自动执行。 这些处理步骤定义为一系列AEM Assets媒体处理程序。 AEM提供一些内置的处理函数[,](#default-media-handlers)和其他处理函数可以是[自定义开发的](#creating-a-new-media-handler)，或者通过将进程委派给[命令行工具](#command-line-based-media-handler)来定义。
...
>[!NOTE]
>
>请参阅[资产支持的格式](assets-formats.md)页面，了解AEM Assets支持的所有格式以及每种格式支持的功能。
...
AEM具有一些处理资产的默认工作流。 要视图它们，请打开工作流控制台，然后单击&#x200B;**[!UICONTROL 模型]**&#x200B;选项卡：与AEM Assets开始的工作流标题是特定资产的工作流标题。
...
请参阅[开发工具](../sites-developing/dev-tools.md)以安装Eclipse并使用Maven插件设置Eclipse，以及设置Maven项目所需的依赖项。
...
1. 在Eclipse中，创建`myBundle` Maven项目：
   ...
   1. 在对话框中，展开Maven文件夹，选择Maven Project，然后单击&#x200B;**[!UICONTROL Next]**。
...
1. 创建包`com.day.cq5.myhandler`，它包含`myBundle/src/main/java`下的Java类：
...
1. 编译Java类并创建包：
   ...
   1. 右键单击myBundle项目，选择&#x200B;**[!UICONTROL 运行方式]**，然后选择&#x200B;**[!UICONTROL 启动安装]**。
   1. 在`myBundle/target`下创建捆绑`myBundle-0.0.1-SNAPSHOT.jar`（包含编译类）。

1. 在CRX资源管理器中，在`/apps/myApp`下创建一个新节点。 名称= `install`，类型= `nt:folder`。
1. 复制捆绑包`myBundle-0.0.1-SNAPSHOT.jar`并将其存储在`/apps/myApp/install`下（例如WebDAV）。 新的文本处理函数现在在AEM中处于活动状态。
...
AEM使您能够在工作流中运行任何命令行工具来转换资产（如ImageMagick）并将新再现添加到资产。 您只需在托管AEM服务器的磁盘上安装命令行工具，并向工作流中添加和配置一个流程步骤。 调用的进程（称为`CommandLineProcess`）还能够根据特定MIME类型进行筛选，并基于新再现创建多个缩略图。
...
为此，请使用ImageMagick。 在承载AEM服务器的磁盘上安装ImageMagick:
...
1. 要查看该工具是否安装正确，请在命令行中运行以下命令`convert -h`。
   ...
   >[!NOTE]
   >
   >在某些版本的Windows（例如Windows SE）中，转换命令可能无法运行，因为它与作为Windows安装一部分的本机转换实用程序相冲突。 在这种情况下，请提及用于将图像文件转换为缩略图的ImageMagick实用程序的完整路径。 例如，`"C:\Program Files\ImageMagick-6.8.9-Q16\convert.exe" -define jpeg:size=319x319 ${filename} -thumbnail 319x319 cq5dam.thumbnail.319.319.png`。

1. 要查看该工具是否运行正常，请向工作目录中添加JPG图像，并在命令行中运行命令`convert <image-name>.jpg -flip <image-name>-flipped.jpg`。
...
1. 按如下方式更改&#x200B;**[!UICONTROL 启用Web的再现]**&#x200B;步骤的设置：
...
1. 打开资产`myImage.tiff`，验证已翻转的图像和三个缩略图是否已创建。
...
例如，如果在承载AEM服务器的磁盘上安装了ImageMagick，并且如果您使用&#x200B;**CommandLineProcess**&#x200B;作为实现创建了一个进程步骤，并使用以下值作为&#x200B;**Process Arguments**:

`mime:image/gif,mime:image/tiff,tn:140:100,tn:48:48,tn:10:250,cmd:convert ${directory}/${filename} -flip ${directory}/${basename}.flipped.jpg`

然后，在运行工作流时，该步骤仅应用于具有image/gif或mime:image/tiff作为mime类型的资产，它会创建原始图像的翻转图像，将其转换为。jpg并创建三个具有尺寸的缩略图：140x100、48x48和10x250。
