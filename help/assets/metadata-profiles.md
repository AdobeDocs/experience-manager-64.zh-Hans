...
1. 输入元数据用户档案的标题（例如“示例元数据”），然后单击&#x200B;**[!UICONTROL 提交]**。 将显示“元数据”用户档案的&#x200B;**[!UICONTROL 编辑表单]**。
...
1. 单击某个组件，然后在&#x200B;**[!UICONTROL 设置]**&#x200B;选项卡中配置其属性。例如，单击&#x200B;**[!UICONTROL 说明]**&#x200B;组件并编辑其属性。
   ...
   * **[!UICONTROL 映射到属性]**:此属性的值提供资产节点在存储库中保存的相对路径/名称。该值应始终与开始 `./` ，因为它指示该路径位于资产的节点下。
   ...
   您为&#x200B;**[!UICONTROL 映射到属性]**&#x200B;指定的值会作为属性存储在资产的元数据节点下。例如，如果您指定`/jcr:content/metadata/dc:desc` 作为映射到 **[!UICONTROL 属性的名称]**,AEM Assets会将该值 `dc:desc` 存储在资产的元数据节点。
   ...
   * **[!UICONTROL 默认值]**：使用此属性可为元数据组件添加默认值。例如，如果您指定“我的说明”，则会将此值分配给资产元数据节点的属性`dc:desc`。
   ...
   >[!NOTE]
   >
   >向新元数据属性添加默认值（在中尚不存在）。 `/jcr:content/metadata` 节点)默认情况下不在资产的“属性”页面上显示该属 **** 性及其值。要视图资产[!UICONTROL 属性]页面上的新属性，请修改相应的模式表单。

1. （可选）从&#x200B;**[!UICONTROL 构建表单]**&#x200B;选项卡将更多组件添加到&#x200B;**[!UICONTROL 编辑表单]**&#x200B;中，并在&#x200B;**[!UICONTROL 设置]**&#x200B;选项卡中配置其属性。从&#x200B;**[!UICONTROL 构建表单]**&#x200B;选项卡中可以使用以下属性：
...
1. 单击&#x200B;**[!UICONTROL 完成]**。元数据用户档案将添加到&#x200B;**[!UICONTROL 元数据用户档案]**&#x200B;页面中的用户档案列表。

   ![chlimage_1-485](assets/chlimage_1-485.png)

## 复制元数据用户档案{#copying-a-metadata-profile}

1. 从&#x200B;**[!UICONTROL 元用户档案]**&#x200B;页面中，选择要制作其副本的用户档案。

   ![chlimage_1-486](assets/chlimage_1-486.png)

1. 单击工具栏中的&#x200B;**[!UICONTROL 复制]**。
1. 在&#x200B;**[!UICONTROL 复制元数据用户档案]**&#x200B;对话框中，输入用户档案新副本的标题。
1. 单击&#x200B;**[!UICONTROL 复制]**。在&#x200B;**[!UICONTROL 元数据用户档案]**&#x200B;页面的用户档案列表中，将显示该用户档案的副本。
...
1. 在左边栏中，点按&#x200B;**[!UICONTROL 资产]**，然后导航到您要对其应用元数据用户档案的文件夹。
...
除了将用户档案应用到文件夹之外，您还可以全局应用一个用户档案，以便上传到AEM资产的任何内容都会应用选定的。 要全局应用元数据用户档案，请执行以下步骤：
...
要通过“用户档案”用户界面将元数据用户档案从文件夹删除，请执行以下步骤：

1. 点按AEM徽标，然后导航到&#x200B;**[!UICONTROL 工具>资产>元数据用户档案]**。
1. 选择您要从一个或多个文件夹删除的元数据配置文件。
1. 点按&#x200B;**[!UICONTROL 从文件夹]**&#x200B;中删除元数据用户档案，选择一个或多个要用于从中删除用户档案的文件夹，然后点按&#x200B;**[!UICONTROL 完成]**。

   如果元数据配置文件的名称不再出现在文件夹名称的下方，则可以确定该元数据配置文件不再应用于该文件夹。

### 通过属性{#removing-metadata-profiles-from-folders-via-properties}从文件夹删除元用户档案
