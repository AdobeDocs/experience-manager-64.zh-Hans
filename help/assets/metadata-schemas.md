...
1. 要视图资产的属性页面，请在卡片中，单击或点按 **[!UICONTROL 视图中资产拼贴]****** 上的快速操作中的视图属性。
...
1. 编辑各个选项卡下的可编辑元数据属性。
   ...
   要修改资产的MIME类型，请使用自定义元数据模式表单或修改现有表单。 有关更 [多信息，请参阅编辑元数据模式表](metadata-schemas.md#editing-metadata-schema-forms) 单。 如果您修改了某些MIME类型的元数据模式，则将修改当前MIME类型和所有资产子类型的资产的属性页面布局。 例如，在下修改 `jpeg` 模式 `default/image` 时，只会修改MIME类型资产的元数据布局（资产属性） `IMAGE/JPEG`。 但是，如果您编辑默认模式，则所做的更改将修改所有类型资产的元数据布局。
...
要在属性页面中除应用上下文元数据组件的选项卡之外，还包括任何选项卡，请从列表中选择该选项卡。 该选项卡会添加到属性页面。
...
元数据模式功能仅对管理员可用。 但是，管理员可以通过为文件夹提供“创建”、“修改 **[!UICONTROL ”和“删除]**”权限 **[!UICONTROL ，为非管理员用户提供]******`/conf` 访问权限。
...
1. 在元数 **[!UICONTROL 据模式表单页]** ，选择表 `my_default` 单，然后单击编 **[!UICONTROL 辑]**。
...
1. 如果文件夹已应用其他元数据模式，则会显示一条消息，警告您将覆盖现有元数据模式。 单击“ **[!UICONTROL 覆盖]**”。
...
1. 编辑自定义表单。 添加必填字段。 例如，添加一个 **类别** ，并将该字段设为必填字段。

   ![chlimage_1-190](assets/chlimage_1-190.png)

1. 单击&#x200B;**[!UICONTROL 保存]**。修改后的表单列在元数据模式表 **[!UICONTROL 单页面中]** 。 要将自定义元数据应用到文件夹，请选择表单，然后单击／点 **[!UICONTROL 按工具栏中的应用到文件夹]** 。
...
>[!C动作]
>
>元数据验证检查会占用大量资源，并可能会影响系统性能。 计划相应的检查。 如果AEM部署存在性能问题，请尝试禁用此作业。
