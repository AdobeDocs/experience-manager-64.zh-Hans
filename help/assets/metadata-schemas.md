...
1. 要查看资产的属性页面，请在卡片视图中单击或点按 **[!UICONTROL 资产拼贴]****[!UICONTROL 上的快速操作中的查]** 看属性。

   ![chlimage_1-170](assets/chlimage_1-170.png)

   或者，选择资产，然后单击或点按工具栏 **[!UICONTROL 中的]** “属性”图标。
...
1. 编辑各个选项卡下的可编辑元数据属性。
   ...
   要修改资产的MIME类型，请使用自定义元数据架构表单或修改现有表单。 有关详 [细信息，请参阅编辑元数据架构表单](metadata-schemas.md#editing-metadata-schema-forms) 。 如果修改了某些MIME类型的元数据架构，则将修改当前MIME类型和所有资产子类型的资产的属性页面布局。 例如，在下修改架 `jpeg` 构只会 `default/image` 修改MIME类型资产的元数据布局（资产属性） `IMAGE/JPEG`。 但是，如果您编辑默认架构，则所做的更改将修改所有类型资产的元数据布局。
...
1. 要查看表单/模板列表，请单击 AEM 徽标，然后导航到&#x200B;**[!UICONTROL 工具 > Assets > 元数据架构]**。
   ...
   * **default**:资产的基本元数据架构表单。
   ...
   ii. **应用程序**:此架构表单用于MIME类型应用程序 `application/pdf`等的 `application/zip`资产。

   a. **pdf**: Schema form for assets with sub type PDF.

   iii. **视频**:此架构表单用于MIME类型为视频(如 `video/avi`等) `video/mp4` 的资产。
...
1. 要将自定义模板添加到列表，请单击工 **[!UICONTROL 具栏中]** 的创建。
   ...
   >[!NOTE]
   >
   >未编辑的模板前面会显示一个锁图标。 如果自定义任何模板，则模板前面的锁图标会消失。
...
>[!NOTE]
>
>删除对默认表单的自定义更改后，元数据架构界面上会重新显示锁图标，以指示表单已恢复为默认状态。
...
在相应的表单类型下创建新表单。For example, to add a new template for the `image/png` subtype, create the form under the `image` forms. 架构表单的标题是子类型名称。In this case, the title is `png`.
...
元数据架构功能仅对管理员可用。 但是，管理员可以通过为文件夹提供“创建”、“修改 **[!UICONTROL ”和“删除]**”权限 **[!UICONTROL ，为非管理员用户提供]******`/conf` 访问权限。
...
1. 选中表单前面的复选框（例如默认元数据表单），然后单击或点按复制图标 **** ，并将其另存为自定义表单。 例如，指定表单的自定义名称 `my_default`。 或者，您也可以创建自定义表单。

   ![chlimage_1-184](assets/chlimage_1-184.png)

1. 在“元数 **[!UICONTROL 据架构表单]** ”页中，选择表 `my_default` 单，然后单击 **[!UICONTROL 编辑]**。
...
1. 如果文件夹应用了其他元数据架构，则会显示一条消息，警告您将覆盖现有元数据架构。 单击“ **[!UICONTROL 覆盖]**”。
...
1. 编辑自定义表单。 添加必填字段。 例如，添加“ **类别** ”字段并将该字段设为必填字段。

   ![chlimage_1-190](assets/chlimage_1-190.png)

1. 单击&#x200B;**[!UICONTROL 保存]**。修改后的表单列在“元数据架构表 **[!UICONTROL 单”页面中]** 。 要将自定义元数据应用到文件夹，请选择表单，然后单击／点 **[!UICONTROL 按工具栏中的应用到文件夹]** 。

1. 导航到文件夹，然后上传一些资产，其中缺少您添加到自定义表单的必填字段的元数据。 资产的卡片视图会显示一条消息，指示必填字段中缺少的元数据。

   ![chlimage_1-192](assets/chlimage_1-192.png)

1. （可选）访问 `http://[server]:[port]/system/console/components/`。 配置并启 `com.day.cq.dam.core.impl.MissingMetadataNotificationJob` 用默认禁用的组件。 设置AEM检查资产中元数据有效性的频率。
此配置将属性添 `hasValidMetadata` 加到资产的jcr:content。 使用此属性，AEM可以筛选搜索结果。
...
>[!C动作]
>
>元数据验证检查会占用大量资源，并可能会影响系统性能。 相应地安排检查。 如果AEM部署存在性能问题，请尝试禁用此作业。
