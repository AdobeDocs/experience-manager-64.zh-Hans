---
title: 在 [!DNL Adobe Experience Manager]中管理您的数字资产的元数据。
...
Exif定义的元数据字段通常是技术性的，在描述性元数据管理中的用途有限。 因此，[!DNL Experience Manager Assets]将Exif属性映射到[常见元数据架构](metadata-schemas.md)和[XMP](xmp-writeback.md)。
...
* PRISM - [行业标准元数据发布要求](https://www.prismstandard.org)。
* XMP - [!DNL Extensible Metadata Platform]。
* `xmpPG`  — 分页文本的XMP元数据。
