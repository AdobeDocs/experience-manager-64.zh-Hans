...
上面是[!DNL Experience Manager]可为资产管理的基本元数据属性，该属性允许用户查看所有资产。 例如，在尝试发现最近添加的资产时，按上次修改日期对资产进行排序非常有用。
...
较多的元数据可以帮助您进一步对资产分类，随着数字信息量的增多，元数据会非常有用。可以仅根据文件名管理几百个文件。 但是，此方法无法扩展。 当相关人员数量和管理资产数量增加时，这个数字就不够了。
...
* XMP:[!DNL Assets]用于在存储库中存储提取的元数据。
* ID3：适用于音频和视频文件。
* Exif:，用于图像文件。
...
[!DNL Extensible Metadata Platform] (XMP)是用于所有元数据管理 [!DNL Experience Manager Assets] 的开放标准。该标准提供了可嵌入所有文件格式的通用元数据编码。 Adobe和其他公司支持XMP standard，因为它提供了丰富的内容模型。 XMP standard和[!DNL Experience Manager Assets]的用户拥有强大的基础平台。 有关更多信息，请参阅[XMP](https://www.adobe.com/products/xmp.html)。
...
可交换图像文件格式(Exif)是数字摄影中最常用的元数据格式。 它提供了一种在多种文件格式（如JPEG、TIFF、RIFF和WAV）中嵌入固定的元数据属性词汇的方法。 Exif将元数据存储为元数据名称和元数据值的对。这些元数据名称 — 值对也称为标记，请不要将其与[!DNL Experience Manager]中的标记混淆。 现代数码相机创建Exif元数据，现代图形软件支持它。 Exif格式是元数据管理（尤其是对于图像）的最低常用分母。
...
* ACDSee — 由[!DNL ACDSee]程序管理的元数据。 请参阅[www.acdsee.com/](https://www.acdsee.com/)。
...
* [!DNL XMPRights]。
* PLUS - [图片许可通用系统](https://www.useplus.com)。
...
* PRISM - [发布行业标准元数据的要求](http://www.prismstandard.org/specifications/)。
* XMP - [!DNL Extensible Metadata Platform]。
* `xmpPG`  — 分页文本的XMP元数据。
...
* 工作流可以检查没有预定义必需元数据的资产，或检查具有&#x200B;*invalid*&#x200B;元数据的资产。
