...
要缩短资产上传时间，请对Java临时目录使用高性能存储。 在Linux和Windows上，可以使用RAM驱动器或SSD。 在基于云的环境中，可以使用等效的高速存储类型。 例如，在Amazon EC2中，临时 [驱动器可用](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/InstanceStorage.html) 于临时文件夹。
...
实施S3或共享文件数据存储有助于在大规模实施中节省磁盘空间和增加网络吞吐量。 有关使用共享数据存储的利弊的详细信息，请参阅资产 [大小调整指南](assets-sizing-guide.md)。
...
Adobe建议启用HTTPS，因为许多公司都有防火墙来监听HTTP通信，这会对上传和损坏文件产生不利影响。 对于大型文件上传，请确保用户已有连接到网络的有线连接，因为WiFi网络会迅速饱和。 有关确定网络瓶颈的准则，请参阅资产 [规模调整指南](assets-sizing-guide.md)。 要通过分析网络拓扑来评估网络性能，请参 [阅资产网络注意事项](assets-network-considerations.md)。
...
1. 在AEM作者上，转到 [http://localhost:4502/system/console/slingevent](http://localhost:4702/system/console/slingevent)。
...
>[!CAUTION]
>
>如果ImageMagick使用了所有可用磁盘空间，则错误配置可能会使服务器不稳定。 使用ImageMagick处理大型文件所需的策略更改可能会影响AEM性能。 有关详细信息，请参 [阅安装和配置ImageMagick](best-practices-for-imagemagick.md)。
...
1. 在每个复制代理上，在“触发器” **[!UICONTROL 选项卡上启用]** “接收 **[!UICONTROL 时]** ”
...
1. 在 **[!UICONTROL AEM 6.2之前的版本中启用CopyOnRead、CopyOnWrite和Prefetch索引文件]** 。这些值在AEM 6.2及更高版本中默认处于启用状态。

更新索引配置以缩短重新索引时间：

1. 打开CRXDe /crx/de/index.jsp并以管理用户身份登录
1. 浏览至/oak:index/lucene
1. 添加一个名为[] excludedPaths的String属 **[!UICONTROL 性]** ，其值为“/var”、“/etc/workflow/instances”和“/etc/replication”
1. 浏览至/oak:index/damAssetLucene
1. 添加一个名为[] includedPaths的String **[!UICONTROL 属性]** ，其中一个值为“/content/dam”
1. 保存

（仅限AEM6.1和6.2）更新ntBaseLucene索引以改进资产删除和移动性能：

1. 浏览 *到/oak:index/ntBaseLucene/indexRules/nt:base/properties*
1. 在 **[!UICONTROL /oak:index/ntBaseLucene/indexRules/nt:base/properties下添加两个nt:unstructured]** 节点slingResource **[!UICONTROL and]** damResolvedPath **
...
1. 完成索引后，返回CRXDe，并在这两个索引上将 **[!UICONTROL type]** 属性设置为disabled
...
同样，当使用共享S3数据存储时，文件大小达到2GB时，文件从缓存完全保留到文件系统可能需要一些时间。 因此，在使用无二进制复制时，可能在复制完成之前没有保留二进制数据。 这种情况可能会导致问题，尤其是当数据可用性很重要时（例如在卸载场景中）。
