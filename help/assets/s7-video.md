---
title: 视频
description: 了解集中式视频资产管理，您可以在其中将视频上传到Experience Manager资产，以便自动编码到Dynamic Media经典。 您还可以直接从“Dynamic Media经典”Experience Manager资产访问视频。 Dynamic Media经典视频集成通过自动设备和自动带宽检测将优化视频的范围扩展到所有屏幕。
...
资产提供集中式视频资产管理，您可以将视频直接上传到资产，以便自动编码到Dynamic Media经典。 您还可以直接从“资产”中访问Dynamic Media经典视频以进行页面创作。

Dynamic Media经典视频集成将优化视频的范围扩展到所有屏幕（自动设备和带宽检测）。

**[!UICONTROL Scene7视频]**&#x200B;组件可自动执行设备和带宽检测，以在台式机、平板电脑和移动设备上播放正确的格式和质量的视频。

您可以包含自适应视频集，而不是只包含单个视频资产。 自适应视频集是所有视频演绎版的容器，这些视频演绎版需要在多个屏幕上无缝播放视频。 自适应视频集为同一视频的一组版本，这些版本以不同的比特率和格式进行编码。 例如，400 kbps、800 kbps和1000 kbps。 您可以将自适应视频集与S7视频组件结合使用，以跨多个屏幕类型实现自适应视频流化。 例如，桌面、iOS、Android、BlackBerry和Windows移动设备。

有关详细信息，请参阅[关于自适应视频集的Dynamic Media经典文档](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/assets/dynamicmedia/video-profiles.html#dynamicmedia)。
...
启用和配置Dynamic Media经典集成不会自动从现成的DAM摄取工作流中删除或取消激活这两个工作流步骤。 如果您已经在Experience Manager中使用基于FFMPEG的视频编码，则创作环境可能已安装FFMPEG。 在这种情况下，使用DAM摄取的新视频将进行两次编码：一次来自FFMPEG编码器，另一次来自Dynamic Media经典集成。
...
如果上述任一问题的答案为“是”或两个问题的答案都为“是”，请将您的视频直接上传到 Adobe DAM。如果两个问题的答案都是“否”，则直接将视频上传到Dynamic Media经典。 下几节将介绍每个方案的工作流。

### 如果您正在将视频直接上传到AdobeDAM {#if-you-are-uploading-your-video-directly-to-adobe-dam}

如果您需要资产的工作流或版本控制，请先上传到AdobeDAM。 下面是建议的工作流：

1. 将视频资产上传到AdobeDAM，并自动编码并发布到Dynamic Media经典。
1. 在Experience Manager中，在内容查找器的&#x200B;**[!UICONTROL 电影]**&#x200B;选项卡的WCM中访问视频资产。
...
如果Experience Manager中的视频预设已在Scene7更新，则必须更新查看器和编码预设。 在这种情况下，请导航到云配置中的Scene7配置，然后单击&#x200B;**[!UICONTROL 更新查看器和编码预设]**。
...
>[!NOTE]
>
>开箱即用的S7视频组件使用通用视频用户档案。 但是，您可以以Experience Manager获取基于HTML5的视频播放器。 只需复制现成HTML5视频播放器的嵌入代码，并将其放入Experience Manager页。

## Experience Manager视频组件{#aem-video-component}

即使建议使用Scene7视频组件观看Scene7视频，也要为了完整起见，将Scene7视频与基金会视频组件结合使用。

### Experience Manager视频与Scene7视频比较{#aem-video-and-scene-video-comparison}

下表提供了Experience Manager基础视频组件与Scene7视频组件之间受支持功能的高级比较：
...
根据在您的Scene7云配置中选择的Scene7编码预设创建各种视频编码。 要使用基础视频组件，必须为选定的每个Scene7编码预设创建视频用户档案。 此方法允许视频组件相应地选择DAM演绎版。
...
1. 从&#x200B;**[!UICONTROL 配置控制台]**&#x200B;导航树中导航到&#x200B;**[!UICONTROL 工具> DAM >视频用户档案]**。
1. 创建Scene7视频用户档案。 在&#x200B;**[!UICONTROL 新建……]**&#x200B;下拉列表，选择&#x200B;**[!UICONTROL 创建页面]**，然后选择“Scene7视频用户档案”模板。 为新的视频配置文件页面指定一个名称，然后单击&#x200B;**[!UICONTROL 创建]**。
...
1. 编辑新的视频配置文件。首先选择云配置。然后，选择在云配置中选定的相同编码预设。
   ...
   | HTML5 视频类型 | 通过此属性可设置HTML5视频源元素的type属性值。 此信息不是由 S7 编码预设提供，但却是使用 HTML5 视频元素正确渲染视频所必需的信息。提供了通用格式列表，但是通用格式可被其他格式覆盖。 |

   对要在视频组件中使用的云配置中选定的所有编码预设重复此步骤。

#### 配置设计{#configuring-design}

**[!UICONTROL 基础视频]**&#x200B;组件必须了解使用哪些视频用户档案来构建视频源列表。 打开视频组件设计对话框并配置组件设计以使用新的视频用户档案。

>[!NOTE]
>
>如果您在移动页面上使用&#x200B;**[!UICONTROL 基础视频]**&#x200B;组件，请在移动页面设计中重复这些步骤。
...
1. 对于不支持HTML5的浏览器，视频组件允许您配置Flash回退。 打开视频组件设计对话框并更改为&#x200B;**[!UICONTROL Flash]**&#x200B;选项卡。 配置Flash Player设置并为Flash Player分配回退用户档案。

#### 核对清单 {#checklist}

1. 创建S7云配置。 确保视频编码预设已设置且导入程序正在运行。
