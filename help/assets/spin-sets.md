---
title: 旋转集
description: 了解如何使用Dynamic Media中的旋转集。 旋转集模拟旋转对象的真实动作，以便从任何角度检查它。
...
旋转集由带有&#x200B;**[!UICONTROL SPINSET]**&#x200B;字样的横幅来指定。此外，如果旋转集已发布，则横幅上将显示由&#x200B;**[!UICONTROL World]**&#x200B;图标指示的发布日期，并显示由&#x200B;**[!UICONTROL 铅笔]**&#x200B;图标指示的上次修改日期。
...
1. [上传多个视图的图像。](#uploading-assets-for-spin-sets)
   ...
   对于一维旋转集，一个项目至少需要8-12张照片，对于二维旋转集，需要16-24张照片。拍照必须以固定的间隔拍摄，以给人以项目正在旋转和翻动的印象。例如，如果一维旋转集包含12个镜头，则对每个镜头将项目旋转30度(360/12)。
...
1. [将 URL 关联到您的 Web 应用程序](linking-urls-to-yourwebapplication.md)或者[嵌入视频查看器或图像查看器](embed-code.md)。
   ...
   AEM Assets会在您发布旋转集后为旋转集创建URL调用并将其激活。您可以在预览资产时复制这些URL。您也可以将它们嵌入到您的网站上。

   选择旋转集，然后在左边栏下拉菜单中选择&#x200B;**[!UICONTROL 查看器]**。

   请参 [阅将旋转集关联到网页](linking-urls-to-yourwebapplication.md)[和嵌入视频查看器或图像查看器](embed-code.md)。

如果需要，您可以[编辑旋转集](#editing-spin-sets)。 此外，您还可以视图和编辑[旋转集属性](managing-assets-touch-ui.md#editing-properties)。
...
* 在一维旋转集中至少使用8-12幅图像，在二维旋转集中至少使用16-24幅图像。要能够旋转360度，至少需要8幅图像。一维旋转集比较常见，因为创建二维旋转集非常繁琐。
...
1. 在&#x200B;**[!UICONTROL 旋转集编辑器]**&#x200B;页面的&#x200B;**[!UICONTROL 标题]**&#x200B;字段中，输入旋转集的名称。 该名称会显示在旋转集的横幅中。（可选）输入说明。
   ...
   在创建旋转集时，您可以更改旋转集缩略图，或允许AEM根据旋转集中的资产自动选择缩略图。 要选择缩略图，请点按&#x200B;**[!UICONTROL 更改缩略图]**。选择任意图像（您也可以导航到其他文件夹以查找图像）。 如果您选择了缩略图，然后决定要AEM从旋转集中生成缩略图，请选择&#x200B;**[!UICONTROL 切换到自动缩略图]**。
...
1. 当您将资产添加到资产集时，资产会按字母数字顺序自动添加。 在添加资产后，您可以手动对资产重新排序或排序。 如有必要，请将资产的&#x200B;**[!UICONTROL 重新排序]**&#x200B;图标拖动到资产文件名右侧，以在设置的列表上下重新排序图像。
...
1. 点按&#x200B;**[!UICONTROL 保存]**。您新创建的旋转集将显示在创建时所在的文件夹中。

## 查看旋转集{#viewing-spin-sets}

您可以在用户界面中创建旋转集，也可以使用[批集预设](/help/assets/config-dms7.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets)自动创建旋转集。 但是，使用批集预设创建的集，请执行&#x200B;*not*&#x200B;操作，以在用户界面中显示。 您可以通过三种不同方式访问通过批集预设方式创建的集。 （即使您在用户界面中创建了旋转集，这些方法也可用。）

您还可以通过用户界面视图集，如[编辑旋转集](#editing-spin-sets)中所述。

**要视图旋转集：**

1. 打开单个资产的属性时。 属性指示所选资产是其成员（位于“集成成员”]**下）。**[!UICONTROL &#x200B;点按集的名称可查看整个集。

   ![chlimage_1-384](assets/chlimage_1-384.png)

1. 来自任何集的成员图像。选择&#x200B;**[!UICONTROL 设置]**&#x200B;菜单以显示资产所属的设置。

   ![chlimage_1-385](assets/chlimage_1-385.png)

1. 在搜索中，您可以选择&#x200B;**[!UICONTROL 过滤器]**，然后展开&#x200B;**[!UICONTROL Dynamic Media]**&#x200B;并选择&#x200B;**[!UICONTROL 设置]**。

   搜索会返回在用户界面中手动创建或通过批集预设自动创建的匹配集。 对于自动集，搜索查询使用具有&#x200B;]**搜索标准的**[!UICONTROL &#x200B;开始进行，该搜索标准不同于基于使用&#x200B;**[!UICONTROL Contains]**&#x200B;搜索标准的AEM搜索。 将筛选器设置为&#x200B;**[!UICONTROL Sets]**&#x200B;是搜索自动集的唯一方法。
...
1. 执行下列任一操作：
   ...
   * 将鼠标悬停在旋转集资产上，点按&#x200B;**[!UICONTROL 选择]**（复选标记图标），然后点按工具栏上的&#x200B;**[!UICONTROL 编辑]**。
