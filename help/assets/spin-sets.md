...
1. [创建旋转集。](#creating-spin-sets)
   ...
   要创建旋转集，请选择“创 **[!UICONTROL 建”>“旋转集]** ”，然后命名旋转集，选择资产，然后按图像的显示顺序对图像进行排序。
...
1. [发布旋转集。](publishing-dynamicmedia-assets.md)
   ...
   发布旋转集时，将激活图像在旋转集中的显示顺序。 请务必对它们进行排序，以使旋转为360度的平滑视图。**[!UICONTROL URL]** 和 **[!UICONTROL Embed]** 字符串。 此外，您必须发 [布查看器预设](managing-viewer-presets.md)。

1. [将URL关联到Web应用程序](linking-urls-to-yourwebapplication.md) ，或 [嵌入视频查看器或图像查看器](embed-code.md)。

   在您发布旋转集后，AEM资产会为旋转集创建URL调用并将其激活。您可以在预览资产时复制这些URL。或者，您也可以将它们嵌入到您的网站中。

   选择旋转集，然后在左边栏下拉菜单中选择&#x200B;**[!UICONTROL 查看器]**。

   请参 [阅将旋转集关联到网页](linking-urls-to-yourwebapplication.md)[和嵌入视频查看器或图像查看器](embed-code.md)。

If you need to, you can [edit Spin Sets](#editing-spin-sets). 此外，您还可以查看和编辑旋 [转集属性](managing-assets-touch-ui.md#editing-properties)。
...
>[!NOTE]
>
>您还可以通过[批量集预设](/help/assets/config-dms7.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets)自动创建旋转集。批量集由IPS（图像生产系统）创建，作为资产摄取的一部分，并且仅在Dynamic Media - Scene7模式中可用。
>
>请参阅配置Dynamic Media - Scene7模式中的“创建批量集预设以自动生 [成图像集和旋转集”](/help/assets/config-dms7.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets)。
...
1. 在“旋 **[!UICONTROL 转集编辑器]** ”页面的“标 **[!UICONTROL 题”字段中]** ，输入旋转集的名称。 该名称会显示在旋转集的横幅中。（可选）输入说明。
...
1. 执行以下操作之一：
   ...
   * 在旋转集编辑器页面的左上 **[!UICONTROL 角附近]** ，点按添 **[!UICONTROL 加资产]**。
   * 在“旋转集编辑器”页 **[!UICONTROL 面的中间附近]** ，点按 **[!UICONTROL 点按以打开资产选择器]**。
   点按以选择要包含在旋转集中的资产。 选定资产上有一个复选标记图标。When you are finished, near the upper-right corner of the page, tap **[!UICONTROL Select]**.

   借助资产选择器，您可以通过键入关键字并点按&#x200B;**[!UICONTROL 返回]**&#x200B;来搜索资产。您还可以应用过滤器来优化搜索结果。您可以按路径、收藏集、文件类型和标记进行过滤。选择过滤器，然后点按工具栏上的&#x200B;**[!UICONTROL 过滤器]**&#x200B;图标。要更改视图，请在页面的右上角附近，点按视图图标，然后点按 **[!UICONTROL 列视图]** 、卡 **[!UICONTROL 片视图]**&#x200B;或列表 ********&#x200B;视图。

   See [Working with Selectors](working-with-selectors.md).

   ![chlimage_1-383](assets/chlimage_1-383.png)

1. 将资产添加到资产集时，资产会按字母数字顺序自动添加。 在添加资产后，您可以手动对资产重新排序或排序。 如有必要，可将资产的“重 **[!UICONTROL 新排序]** ”图标拖动到资产文件名右侧，以在设置列表中向上或向下重新排序图像。
...
您可以在用户界面中创建旋转集，也可以使用批量集预 [设自动创建旋转集](/help/assets/config-dms7.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets)。 但是，使用批量集预设创建的集 *不会* 显示在用户界面中。 您可以通过三种不同方式访问通过批量集预设创建的集。 （即使您在用户界面中创建了旋转集，这些方法也可用）。

您还可以通过用户界面查看集，如编辑旋转集 [中所述](#editing-spin-sets)。

**要查看旋转集，请执行以下操作：**

1. 打开单个资产的属性时。 属性指示选定资产是其成员(位于“集成 **[!UICONTROL 员”下]**)的集。 点按集的名称可查看整个集。
...
1. From search, you can select **[!UICONTROL Filters]**, then expand **[!UICONTROL Dynamic Media]** and select **[!UICONTROL Sets]**.
   ...
   搜索会返回在用户界面中手动创建或通过批量集预设自动创建的匹配集。 对于自动集，搜索查询是使用搜索条件 **[!UICONTROL 开始]** （与基于使用“包含”搜索条件的AEM搜索不同） **** 执行的。 将过滤器设置 **[!UICONTROL 为]** “集”是搜索自动集的唯一方法。
...
1. 要编辑旋转集，请执行以下任一操作：
   ...
   * 要按升序或降序对项目排序，请点按列标题。
   * To add an asset or update an existing asset, tap **[!UICONTROL Add Asset]**. 导航到资产，选择它，然后点 **[!UICONTROL 按右上角]** 附近的选择。
如果通过将AEM用作缩略图的图像替换为其他图像来删除该图像，则仍会显示原始资产。
   * 要删除资产，请选择该资产，然后点按删 **[!UICONTROL 除资产]**。
   * 要应用预设，请点按预 **[!UICONTROL 设图标]** ，然后选择预设。
