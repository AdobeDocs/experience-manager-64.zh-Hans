...
seo-description: 对Scene7运行模式中的Dynamic media进行疑难解答。
...
* 确保已 **[!UICONTROL 启用Dynamic Media Asset Activation(scene7)]** Replication Agent。

   此复制代理位于“作者”上 **[!UICONTROL 的“代理]** ”下。
...
同步错误和问题记录在 `error.log` (AEM服务器目 `/crx-quickstart/logs/`录)中。 可以使用足够的日志记录来确定大多数问题的根本原因，但您可以通过Sling Console( `com.adobe.cq.dam.ips` http://localhost:4502/system/console/slinglog[](http://localhost:4502/system/console/slinglog))将日志记录增加到包上的DEBUG，以收集更多信息。
...
替换现有Dynamic media资产（同名和位置）时，您可以选择保留两个资产或替换或创建版本：

* 同时保留这两个属性将为已发布的资产URL创建一个具有唯一名称的新资产。 例如， **[!UICONTROL image.jpg是原始资产]** , **** image1.jpg是新上传的资产。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    <ul> 
     <li>检查运行模式是 <span class="kbd">否为dynamicmedia_scene7</span>。</li> 
     <li>检查是否配置了Dynamic Media云服务。</li> 
     <li>检查视频配置文件是否与上传文件夹相关联。</li> 
    </ul> </td> 
   <td> 
    <ol> 
     <li>使用 <span class="kbd">-r dynamicmedia_scene7检查AEM实例</span></li> 
  ...
  <tr> 
   ...
   <td><p>要确定视频编码是否仍在进行中或是否已进入失败状态，请执行以下操作：</p> 
    <ul> 
     ...
     <li>从工作流控制台&gt;实例、存档、 <code>http://localhost:4502/libs/cq/workflow/content/console.html</code> 失败选项卡监视视频。</li> 
    </ul> </td> 
   <td> </td> 
  </tr> 
  <tr> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>查看器预设未发布</td> 
   <td><p>继续执行示例管理器诊断页面： <code>http://localhost:4502/libs/dam/gui/content/s7dam/samplemanager/samplemanager.html</code></p> <p>观察计算值。 正确操作时，您应当看到：</p> <p><code class="code">_DMSAMPLE status: 0 unsyced assets - activation not necessary
   ...
   <td><p>如果示例资产或查看器预设图稿尚未同步或发布，请重新启动整个复制／同步过程：</p> 
    <ol> 
     ...
     <li>导航到CRX包管理器： <code>http://localhost:4502/crx/packmgr/</code><a href="http://localhost:4502/crx/packmgr/"></a> 
      <ol> 
       <li>在列表中搜索查看器包(它以 <span class="kbd">cq-dam-scene7-viewers-content开头</span>)</li> 
       <li>单击“ <strong>重新安装</strong>”。</li> 
      </ol> </li> 
     <li>在云服务下，导航到Dynamic media配置页面，然后打开Dynamic Media - S7配置的配置对话框。 
      <ul> 
       <li>不进行更改，单击“保 <strong>存”</strong>。 <br /> 这会再次触发逻辑以创建和同步示例资源、查看器预设CSS和图稿。 <br /> </li> 
      </ul> </li> 
    </ol> </td> 
  </tr> 
 </tbody> 
