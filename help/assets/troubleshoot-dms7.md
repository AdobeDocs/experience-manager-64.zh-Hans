...
description: Dynamic Media故障诊断 — Scene7运行模式。
...
* 首先，安装了任何AEM 6.4累积修补程序包(CFP) *之前* 任何可用的Dynamic Media功能包。
* 已安装可选功能包18912。

   此可选功能包适用于FTP支持，或者如果您要将资产从Dynamic Media Classic迁移到Dynamic Media。

* 导航到Cloud Services用户界面，并确认已配置的帐户显示在 **[!UICONTROL 可用配置]**.
* 确保 **[!UICONTROL Dynamic Media Asset Activation(scene7)]** 复制代理已启用。

   此复制代理位于 **[!UICONTROL 代理]** 作者。
...
已登录同步错误和问题 `error.log` (AEM服务器目录) `/crx-quickstart/logs/`)。 有足够的日志记录来确定导致大多数问题的根本原因，但您可以将 `com.adobe.cq.dam.ips` 包([http://localhost:4502/system/console/slinglog](http://localhost:4502/system/console/slinglog))以收集更多信息。

### 移动、复制或删除 {#move-copy-delete}

在执行移动、复制或删除操作之前，请执行以下操作：

* 对于图像和视频，请确认 `<object_node>/jcr:content/metadata/dam:scene7ID` 执行移动、复制或删除操作之前存在值。
* 对于图像预设和查看器预设，请确认 `https://<server>/crx/de/index.jsp#/etc/dam/presets/viewer/testpreset/jcr%3Acontent/metadata` 执行移动、复制或删除操作之前存在值。
* 如果缺少上述元数据值，则需要在移动、复制或删除操作之前重新上传资产。

### 版本控制 {#version-control}

替换现有Dynamic Media资产（名称和位置相同）时，您可以选择保留两个资产，也可以替换或创建版本：

* 同时保留这两个属性将为已发布的资产URL创建一个具有唯一名称的新资产。 例如， **[!UICONTROL image.jpg]** 是原始资产和 **[!UICONTROL image1.jpg]** 是新上传的资产。
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     <li><p>转到CRX/DE:</p> 
      <ul> 
       <li>检查JCR中的预设是否 <code>/etc/dam/presets/viewer/&lt;preset&gt; has lastReplicationAction</code> 定义。 请注意，如果您从AEM 6.x升级到6.4并选择退出迁移，则此位置适用。 否则，位置是 <code>/conf/global/settings/dam/dm/presets/viewer</code>.</li> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    <ul> 
     <li>检查运行模式是否为 <span class="kbd">dynamicmedia_scene7</span>.</li> 
     <li>检查是否配置了Dynamic Media云服务。</li> 
     <li>检查视频配置文件是否与上传文件夹关联。</li> 
    </ul> </td> 
   <td> 
    <ol> 
     <li>检查AEM实例 <span class="kbd">-r dynamicmedia_scene7</span></li> 
  ...
  <tr> 
   ...
   <td><p>要确定视频编码是否仍在进行中或是否已进入失败状态，请执行以下操作：</p> 
    <ul> 
     <li>检查视频状态 <code>http://localhost:4502/crx/de/index.jsp#/content/dam/folder/videomp4/jcr%3Acontent</code> &gt; <span class="kbd">dam:assetState</span></li> 
     <li>从工作流控制台中监控视频 <code>http://localhost:4502/libs/cq/workflow/content/console.html</code> &gt;实例、存档、失败选项卡。</li> 
    </ul> </td> 
   <td> </td> 
  </tr> 
  <tr> 
...
1. 重新创建 [Dynamic Media在Cloud Services中的配置](/help/assets/config-dms7.md#configuring-dynamic-media-cloud-services).
