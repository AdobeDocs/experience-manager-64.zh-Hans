...
# 社区功能{#analytics-configuration-for-communities-features}的分析配置
...
* 在作者环境中：
   ...
   * 在[成员管理控制台](members.md)(视图、帖子、关注、点赞)中显示参与数据
   * 支持资源[的趋势摘要、视频心率和视频设备报告](reports.md)
...
1. 创建AEM Analytics [云服务](#aem-analytics-cloud-service-configuration)和[framework](#aem-analytics-framework-configuration)
1. [为社](#enable-analytics-for-a-community-site) 区站点启用分析
1. [验](#verify-analytics-to-aem-variable-mapping) 证分析到AEM变量映射
1. 标识[主发布者](#primary-publisher)
...
使用Adobe Marketing Cloud的[报表包管理器](https://docs.adobe.com/content/help/en/analytics/admin/manage-report-suites/new-report-suite/new-report-suite.html)，可以配置Analytics报表包，以便允许社区站点提供社区功能的报表。
...
如果使用&#x200B;**现有报表包**，则可能需要在激活社区站点的Analytics框架之前，修改变量映射[。 ](#modifying-analytics-variable-mapping)有关专用于Communities的变量的任何疑虑，请与您的客户代表联系。
...
>
>要恢复预先存在的映射并将AEM变量移动到其他Analytics变量，请参阅[修改分析变量映射](#modifying-analytics-variable-mapping)一节。
>
>否则可能导致无法恢复的数据丢失。

### 视频心率分析{#video-heartbeat-analytics}

获得视频心率分析许可后，将分配`Marketing Cloud Org Id`。

要在[配置视频报告](#adobe-analytics-report-suite-for-video-reporting)的Analytics报表包后启用视频心率报告，请执行以下操作：
...
`Marketing Cloud Org Id`可在[社区站点创建](sites-console.md#enablement)或更高版本时通过[修改](sites-console.md#modifying-site-properties)社区站点属性输入。[](#aem-analytics-cloud-service-configuration)

![chlimage_1-264](assets/chlimage_1-264.png)

启用“视频心率分析”后，视频播放器的Javascript(JS)代码会实例化视频心率库代码（也包含JS），该代码会处理每10秒（不可配置）向Analytics视频跟踪服务器发送视频状态更新的所有逻辑，并最终向主Analytics服务器发送视频会话的累积报告。
...
初始创建新的Analytics配置会显示配置，并显示一个新对话框以输入Analytics设置。 此对话框要求从帐户代表处获得[必备帐户信息](#prerequisites)。
...
该框架的目的是将AEM变量映射到Analytics变量(evar和事件)。 可用于映射的Analytics变量在报表包](#adobe-analytics-report-suite-for-video-reporting)中定义为[。
...
要使Analytics跟踪和导入对社区站点正常工作，必须存在具有`scf-js-site-title`类和href属性的页面元素。 页面上只应存在一个此类元素，如社区站点的未修改`sitepage.hbs`脚本中的元素。 将提取`siteUrl`的值并作为&#x200B;*站点路径*&#x200B;发送到Adobe Analytics。
...
对于覆盖`sitepage.hbs`脚本的&#x200B;**自定义社区站点**，请确保元素存在。 当在服务到客户端之前在服务器上呈现时，将设置`siteUrl`变量。

对于包含Communities组件的&#x200B;**通用AEM站点**，但不是使用[站点创建向导](sites-console.md)创建的，必须添加元素。 href值应为站点的路径。 例如，如果站点路径为`/content/my/company/en`，则使用：
...
作者环境的[OSGi配置](../../help/sites-deploying/configuring-osgi.md)、`AEM Communities Analytics Component Configuration`提供了已为Analytics指令的组件列表。 变量的自动映射由列出的组件决定。
...
在启用Analytics并选择云配置框架的情况下保存社区站点后，AEM变量将自动映射到分别从evar1和事件1开始的Analytics evar和事件，并递增1。

如果使用现有的报表包将evar1到evar11以及事件1到事件7中的任何变量映射，则需要[重新映射AEM变量](#modifying-analytics-variable-mapping)并恢复原始映射。
...
|  | 启用资源类型 | 网站标题 | 函数类型 | 组标题 | 组路径 | UGC类型 | UGC标题 | 用户（会员） | UGC路径 | 站点路径 |
|------------------------|------------------------|-----------|--------------|------------|-----------|---------|----------|--------------|---------|----------|
|  | **eVar1** | **eVar2** | **eVar3** | **eVar4** | **eVar5** | **eVar6** | **eVar7** | **eVar8** | **eVar9** | **eVar10** |
| 事件1资源播放 | (某个) | - | - | - | - | - | - | - | (i) | - |
| 事件2SCFView | (某个) | (b) | (c) | (d) | (e) | (f) | (g) | (h) | (i) | (j) |
| 事件3SCFCreate（帖子） | - | (b) | (c) | (d) | (e) | (f) | (g) | (h) | (i) | (j) |
...
* UGC的SRP路径：/content/usergenerated/asi/.../forum/jmtz-topic3或&#x200B;*要遵循的组件路径*:/content/sites/communities/cn/jcr:content/content/primary/forum
...
如果Communities evar和事件覆盖报表包中预先存在的映射，则为避免数据丢失，请将Communities功能的AEM变量分配给其他Analytics evar和/或事件，并恢复原始映射。

>[!CAUTION]
>
>在启用了Analytics的社区站点[已发布](#publishing-the-community-site)之前重新映射很重要，否则可能会丢失数据。
...
如果选择的部署是[发布场](topologies.md#tarmk-publish-farm)，则必须将一个AEM发布实例标识为轮询Adobe Analytics以将报告数据写入[ SRP](working-with-srp.md)的主发布者。
...
有关具体说明，请参阅[部署社区](deploy-communities.md#primary-publisher)的主发布者部分。
...
要执行此操作，请按照[复制加密密钥](deploy-communities.md#replicate-the-crypto-key)中的说明操作。

### 发布社区站点和Analytics Cloud服务{#publish-community-site-and-analytics-cloud-service}

为社区站点启用Analytics云服务后，如果需要，将Analytics[映射到AEM变量](#mapped-analytics-to-aem-variables)进行调整后，必须通过[(re)发布社区站点](sites-console.md#publishing-the-site)将配置复制到发布环境。
...
发布启用Analytics的社区站点后，可以配置主发行商的[OSGi配置](../../help/sites-deploying/configuring-osgi.md)、`AEM Communities Analytics Report Importer`来设置那些未在CRXDE中单独配置的配置的默认轮询间隔。

轮询间隔控制向Adobe Analytics请求要提取并保存到[SRP](working-with-srp.md)中的数据的频率。
...
Adobe Analytics提供了允许您访问、导出和删除用户数据的API。 有关详细信息，请参阅[提交访问和删除请求](https://docs.adobe.com/content/help/en/analytics/admin/data-governance/gdpr-submit-access-delete.html)。

## 资源 {#resources}

* Adobe Marketing Cloud:[分析帮助和参考](https://docs.adobe.com/content/help/en/analytics/landing/home.html)
* AEM:[集成Adobe Analytics](../../help/sites-administering/adobeanalytics.md)
