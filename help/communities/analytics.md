...
通过使用 [公司](https://docs.adobe.com/content/help/en/analytics/analyze/analysis-workspace/home.html) 名和用户名登录 [](analytics.md#prerequisites)到Adobe Marketing Cloud，可以配置新的或现有的报表包：
...
要在创建新社区站点时添 [加Analytics云服务](sites-console.md):


* 步骤3
* 在“分析 [”选项卡下](sites-console.md#analytics):
...
要将Analytics云服务添加到现有社 [区站点](sites-console.md#modifying-site-properties):
...
对于包 **含Communities组件** 、但不是使用站点创建向导 [创建的通用AEM站点](sites-console.md)，必须添加元素。 href值应为站点的路径。 例如，如果站点路径为， `/content/my/company/en`则使用：
...
当选择的部署是发 [布场](topologies.md#tarmk-publish-farm)，则必须将一个AEM发布实例标识为轮询Adobe Analytics以将报告数据写入SRP的主发 [布者](working-with-srp.md)。
...
为社区站点启用Analytics云服务后，并在必要时调整 [了Analytics到AEM变量的映射](#mapped-analytics-to-aem-variables)，有必要通过（重新）发布社区站点将配 [置复制到发布环境](sites-console.md#publishing-the-site)。
...
* Adobe Marketing Cloud: [分析帮助与参考](https://docs.adobe.com/content/help/en/analytics/landing/home.html)
