...
* 在创作环境中：
   ...
   * 在[成员管理控制台](members.md)中显示参与数据（视图、帖子、关注、称赞）
   * 用于启用资源[报表的趋势摘要、视频心率和视频设备](reports.md)
...
通过使用[公司名称和用户名](analytics.md#prerequisites)登录到[Adobe Marketing Cloud](https://docs.adobe.com/content/help/en/analytics/analyze/analysis-workspace/home.html)，可以将新的或现有的报表包配置为：
...
如果使用&#x200B;**现有报表包**，则在为社区站点激活Analytics框架之前，可能需要[修改变量映射](#modifying-analytics-variable-mapping)。 有关专用于社区的变量的任何问题，请联系您的客户代表。
...
1. 在打开的对话框中选择&#x200B;**[!UICONTROL Yes]**:```Load default server settings? Do you want to load the default server settings and overwrite current values in the Server section?```
1. 选择`Run Mode`\
...
* 导航到&#x200B;**[!UICONTROL Communities > Sites]**&#x200B;控制台
* 选择社区站点的编辑站点图标
* 选择设置
* 在Analytics部分中：
...
对于包含Communities组件的&#x200B;**通用AEM站点**，但未使用[站点创建向导](sites-console.md)创建，则需要添加该元素。 href的值应该是网站的路径。 例如，如果站点路径为`/content/my/company/en`，则使用：
...
以下是[入门教程](getting-started-enablement.md)之后的默认映射示例：
...
| event1资源播放 | (某个) | - | - | - | - | - | - | - | (i) | - |
| event2SCFView | (某个) | (b) | (c) | (d) | (e) | (f) | (g) | (h) | (i) | (j) |
...
有关具体说明，请参阅[Deploying Communities](deploy-communities.md#primary-publisher)中的主发布者部分。
...
轮询间隔控制向Adobe Analytics请求数据提取并保存到[SRP](working-with-srp.md)的频率。
...
* Adobe Marketing Cloud:[Analytics帮助和引用](https://docs.adobe.com/content/help/en/analytics/landing/home.html)
