...
>[!CAUTION]
>
>前缀为&#x200B;**scf-js-&amp;ast;**&#x200B;的任何CSS类名称在Javascript代码中具有特定用途。 这些类会影响组件的状态（例如，从隐藏切换到可见），因此不应覆盖也不应删除。
>
...
覆盖或覆盖组件时，不应删除SCF脚本中的脚本标记。 为在HTML中插入JSON而自动创建的SCF脚本标记将使用属性`data-scf-json=`true进行标识。
...
* /libs/social/&lt;功能>/components/&lt;组件名称>/clientlibs
