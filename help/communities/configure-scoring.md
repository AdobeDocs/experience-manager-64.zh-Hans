...
例如，在中 `this.isAssigned` 搜索 `/libs/social/forum/components/hbs/topic/list-item.hbs`:
...
1. 访问 **[!UICONTROL Adobe Experience Manager Web Console日志支持]**，例如
...
Adobe存储库中的开发人员可 [以使用](deploy-communities.md#LatestReleases) 已安装版本的最新Javadoc。 请参 [阅使用Maven for Communities:Javadocs](maven.md#javadocs)。
...
1. 具有唯一ID的AEM站点（使用向导创建的社区站点）:
   ...
   * 使用在入门教程中创建的入门教程(参 [与)站点](getting-started.md)
   * 找到论坛页面节点
...
1. 没有唯一 *ID* 的AEM站点：
   ...
   * 使用“社 [区组件”指南](components-guide.md)
   * 找到论坛页面节点
