...
在“站 [点模板”步骤中](sites-console.md#step2013asitetemplate)，输入标题、说明、URL的名称，然后选择社区站点模板，例如：

* **[!UICONTROL 社区站点标题]**: `Getting Started Tutorial`

* **[!UICONTROL 社区站点描述]**: `A site for engaging with the community.`

* **[!UICONTROL 社区站点根目录]**:(默认根目录留空 `/content/sites`)

* **[!UICONTROL 云配置]**:（如果未指定云配置，则留空）提供指向指定云配置的路径。
...
在设置步骤中，在选择之前 `Next`，请注意有七个部分提供对涉及用户管理、标记、审核、组管理、分析、翻译和启用的配置的访问。

请访问 [AEM Communities的Enablement](getting-started-enablement.md) 入门教程，以体验使用启用功能的经验。
...
可以通过选择之前通过标记控制台定义的AEM命名空间(如教程命名空间 [](../../help/sites-administering/tags.md#tagging-console) )来控制应用于社区内容 [的标记](setup.md#create-tutorial-tags)。
...
“翻 [译设置](sites-console.md#translation) ”指定站点的基本语言，以及UGC是否可以翻译成哪种语言（如果可以）。
...
有关快速创建启用社区的类似教 [程](overview.md#enablement-community)，请参 [阅AEM Communities for Enablement入门](getting-started-enablement.md)。
...
* **发布站点**&#x200B;选择全球图标以发布社区站点（例如，如果发布服务器在本地计算机上运行，则默认情况下为localhost:4503）
...
>1. 在“设置” **[!UICONTROL 旁边]** ，选择“编 **[!UICONTROL 辑”]**
...
>
>1. 选择确 **[!UICONTROL 定]**
>1. （可选）选择或 `Clear` 重置 `Force Retry` 复制队列
...
对于此新社区站点，如果在步骤1中将站点名称设为“参与”，则四个新用户组可能会从“组”控制台中看 [到](members.md) (全局导航：社区、组):
...
注意， [Aaron mcDonald](tutorials.md#demo-users) 是
...
为此，需要使 [用CRXDE](http://localhost:4503/crx/de) Lite在发布时 [编辑资源映射](../../help/sites-deploying/resource-mapping.md) 表。

开始：

1. 在发布时，使用管理员权限登录
1. 浏览到http://localhost:4503/crx/de [目录](http://localhost:4503/crx/de)
