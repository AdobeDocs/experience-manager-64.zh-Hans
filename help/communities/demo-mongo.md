...
通过此设置，可从创作和发布环境访问社区内容，无需前进或反向复制用户生成的内容(UGC)。
...
* MongoDB可以与AEM运行在同一主机上或远程运行

### 开始 MongoDB {#start-mongodb}

* &lt;mongo-install>/bin/mongod  &lt;mongo-dbpath>
...
* 宾/蒙戈
   ...
   * cfg = {&quot;_id&quot;:&quot;rs0&quot;,&quot;version&quot;:1,&quot;members&quot;:[{&quot;id&quot;:0,&quot;host&quot;:&quot;127.0.0.1:27017&quot;}]}
   * rs.initiate(cfg)
...
### 为AEM Communities {#configure-solr-for-aem-communities}配置Solr

要为MSRP配置用于演示的Solr集合，需要做出两个决定（选择指向主文档的链接以了解详细信息）：
...
为了简单起见，以版本4.10为例，在独立模式下开始Solr:

* cd到&lt;solrinstall>/example
* java -jar 开始.jar

这将使用默认端口8983开始Solr HTTP服务器。 您可以浏览到Solr控制台，以获取要测试的Solr控制台。
...
如果是，则使用此主机名（如127.0.0.1 user-macbook-pro）的新条目更新etc/hosts文件，Solr将正确开始。

### SolrCloud {#solrcloud}

要运行非常基本（而非生产）的solrCloud设置，请使用以下开始解决器：

* java -Dbootstrap_confdir=。/solr/collection1/conf -Dbootstrap_conf=true -DzkRun -jar 开始.jar
...
按照主文档页上的说明操作：[MSRP - MongoDB Common Store](msrp.md)

## 测试 {#test}

要测试和验证MongoDB公用存储，请发布发布实例的注释并将其视图到创作实例，并在MongoDB和Solr中视图UGC:

1. 在发布实例上，浏览至[社区组件指南](http://localhost:4503/content/community-components/en/comments.html)页，然后选择评论组件。
1. 登录以发布评论：
1. 在注释文本输入框中输入文本，然后单击&#x200B;**[!UICONTROL 发布]**

   ![chlimage_1-111](assets/chlimage_1-191.png)

1. 只需视图[作者实例](http://localhost:4502/content/community-components/en/comments.html)上的评论（可能仍以admin/admin身份登录）。

   ![chlimage_1-112](assets/chlimage_1-192.png)

   注意：尽管作者上的&#x200B;*asipath*&#x200B;下有JCR节点，但这些节点用于SCF框架。 实际的UGC不在JCR中，而在MongoDB中。

1. 视图mongodb **[!UICONTROL “社区”>“集合”>“内容]**”中的UGC
...
1. 视图Solr中的UGC:
   ...
   * 要选择`collection1`的用户`core selector`
   * 选择 `Query`
   * 选择 `Execute Query`
