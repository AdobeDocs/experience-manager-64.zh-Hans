...
本教程介绍如何为一 [个作者](msrp.md) 实 *例和一个发* 布实例设置 *MSRP* 。

通过此设置，可以从创作和发布环境访问社区内容，无需前进或反向复制用户生成的内容(UGC)。

此配置适用于 *非生产环境* ，如开发和／或演示。
...
1. 在独立或SolrCloud模式下运 [行Solr](msrp.md#solrcloudmode)
1. 安装 [标准](msrp.md#installingstandardmls)[或高级多语](msrp.md#installingadvancedmls) 种搜索(MLS)
...
* java -Dbootstrap_confdir=。/solr/collection1/conf -Dbootstrap_conf=true -DzkRun -jar开始.jar
...
1. 在发布实例上，浏览到“社区组 [件指南”页](http://localhost:4503/content/community-components/en/comments.html) ，然后选择“注释”组件。
1. 登录以发布评论：
