...
[应](https://dev.mysql.com/downloads/mysql/) 按照目标 OS的说明下载并安装MySQL。
...
* 默认端口为`3306`
* 所选的连接名称将作为[JDBC OSGi configuration](#configurejdbcconnections)中的数据源名称输入
...
如果脚本中的数据库名称已更改，请务必在[JDBC config](#configurejdbcconnections)中也更改它。
...
在以下映像中，`init_schema.sql`文件已准备好执行：
...
* 输入以下值：
   ...
   * **[!UICONTROL JDBC连接URI]**:  `jdbc:mysql://localhost:3306/communities?characterEncoding=UTF-8`
      ...
      如果MySQL服务器与“this” AEM服务器不相同，则指定服务器代替localhost

      *通* 信默认数据库(模式)名称

   * **[!UICONTROL 用户名]**:  `root`

      或输入MySQL服务器的配置用户名（如果不是“root”）
   ...
   * **[!UICONTROL 数据源名称]**:为MySQL连 [接输入的名](#new-connection-settings)称，例如“communities”
