...
* **[!UICONTROL 电子邮件“发件人”地址]**: *（必需）* 将从中发送通知电子邮件的地址。可能为[默认邮件服务](#configuredefaultmailservice)指定的相同&#x200B;**SMTP用户**。 默认值为`no-reply@example.com`。

* **[!UICONTROL Reply-to-Delimiter]**:如果将跟踪器ID添加到回复标头，则将使用此分隔符。默认值为`+`（加号）。

* **[!UICONTROL 主题中的跟踪器ID前缀]**:如果将跟踪器ID添加到主题行，则将使用此前缀。默认值为`post#`。
...
* **[!UICONTROL 类型]**: *（必需）* 下拉菜单以选择  `POP3 (over SSL).`

* **[!UICONTROL URL]**: *（必需）* 出站邮件服务器。例如，`pop.gmail.com:995/INBOX?username=community-emailgmail.com&password=****`

* **[!UICONTROL 导入到路径]**(&amp;A);: *（必需）* 通过浏 `/content/usergenerated/mailFolder/postEmails`
览到 
`postEmails`文件夹，然后选择 **确定**

* **[!UICONTROL 更新间隔（以秒为单位）]**: *（可选）* 为默认邮件服务配置的邮件服务器可能需要更新间隔值。例如，Gmail可能要求间隔为`300`。
