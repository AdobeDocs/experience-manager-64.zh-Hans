...
[社区站](sites-console.md) 点创建采用向导，可指导您完成创建社区站点的各个步骤。在最后一步中提交站点之前，可以前进到`Next`步骤或`Back`到上一步。

要开始创建新社区站点，请执行以下操作：

使用[作者实例](http://localhost:4502/)
...
* **社区站点基础语言**:(对于单种语言，请保持不变：英语)使用下拉菜单从可用 *语* 言(德语、意大利语、法语、日语、西班牙语、葡萄牙语（巴西）、繁体中文和简体中文)中选择一种或多种基本语言。将根据[多语言站点翻译内容](../../help/sites-administering/translation.md)中介绍的最佳实践，为添加的每种语言创建一个社区站点，并且该站点将位于同一站点文件夹中。 每个站点的根页面将包含一个由所选语言之一的语言代码命名的子页面，如英语为“en”或法语为“fr”。
...
建议[启用社区](overview.md#enablement-community)为私有。

当拒绝匿名站点访客访问、不能自行注册或不能使用社交登录时，社区站点是私有的。

确保[用户管理](sites-console.md#user-management)中未选中大多数复选框：
...
可应用于社区内容的标记通过选择先前通过[标记控制台](../../help/sites-administering/tags.md#tagging-console)定义的AEM命名空间(如[教程命名空间](enablement-setup.md#create-tutorial-tags))进行控制。

此外，为社区站点选择标记命名空间会限制在定义目录和启用资源时显示的选择。 有关重要信息，请参阅[标记Enablement Resources](tag-resources.md)。
...
>[!NOTE]
>
>[隧道](deploy-communities.md#tunnel-service-on-author) 服务允许选择仅存在于发布环境中的成员和组。
...
[转换设置](sites-console.md#translation)指定UGC是否可以转换为哪种语言（如果可以）。
...
* **[!UICONTROL Marketing Cloud组织]**
Id（可选）在启用报告中包含视频心跳分析时必 [需的](analytics.md#video-heartbeat-analytics) Adobe Analytics帐户ID。
...
* **发布**
站点选择全球图标以发布社区站点（默认为localhost:4503）
...
* **删除**
站点要删除社区站点，请选择将鼠标悬停在社区站点控制台中的站点上时显示的删除站点图标。此操作将删除与站点关联的所有项目，如UGC、用户组、资产和数据库记录。
...
在创作时，启用隧道服务后，可以将在初始设置中创建的[用户分配给新创建的社区站点的社区成员组。](enablement-setup.md#publishcreateenablementmembers)
...
* 在&#x200B;**[!UICONTROL 将成员添加到组]**&#x200B;搜索框中输入`ski`
...
1. 在发布时，访问CRXDE并使用管理员权限登录
   ...
   * 例如，浏览至[http://localhost:4503/crx/de](http://localhost:4503/crx/de)并使用`admin/admin`登录
...
要修改站点的结构，请导航到&#x200B;**[!UICONTROL 社区，站点]**&#x200B;控制台，打开`enable`文件夹，然后选择&#x200B;**编辑站点**&#x200B;图标以访问`Enablement Tutorial`的属性。
...
选择右上角的&#x200B;**[!UICONTROL 保存]**&#x200B;以将更改保存到社区站点。
