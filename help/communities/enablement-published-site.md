...
现在，由两个学员（`Riley Taylor`和`Sidney Croft. By s`）选择`Administration`链接以访问协调控制台，Quinn可以使用[批量协调控制台](moderation.md)审核他们的帖子。
