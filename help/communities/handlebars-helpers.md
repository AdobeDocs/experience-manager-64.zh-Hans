...
Handlebars Helpers（帮助器）是从Handlebars脚本调用的方法，以便于与SCF组件一起使用。

该实现包括客户端和服务器端定义。 开发人员也可以创建自定义帮助程序。
...
返回符合maxWords和maxLength属性的缩写字符串的助手。

要缩写的字符串作为上下文提供。 如果未提供上下文，则返回空字符串。

首先，将上下文裁切为maxLength，然后将上下文分割为单词并缩小为maxWords。
...
* **椭圆文本**:字符串
   ...
   （可选）要显示的文本，指示存在隐藏文本。 默认为“...”。

* **safeString**:Boolean

   （可选）指示是否在返回结果之前应用Handlebars.SafeString()的布尔值。 默认值为false。
...
一个块帮助程序，它根据字符串分隔的 [模式列表测试](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) WCM模式的当前值。

### 参数 {#parameters-4}

* **上下文**:字符串

   （可选）要翻译的字符串。 如果未提供默认值，则为必需。

* **mode**:字符串

   （可选）要测试的 [WCM模式列表](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) （以逗号分隔）。
...
这允许以编程方式自定义资源，比作为JCR节点添加的资源更容易。 请参 [阅添加或包含社区组件](scf.md#add-or-include-a-communities-component)。

只包含少数几个“社区”组件。 对于AEM 6.1，包含的评论、评级 [、评论](essentials-comments.md)、评论和投 [票等内容](rating-basics.md)[](reviews-basics.md)[](essentials-voting.md)。

此帮助程序仅在服务器端适用，它提供的功能与JSP脚本的 [cq:include](../../help/sites-developing/taglib.md) 类似。
...
包含AEM html客户端库的帮助程序，可以是js、css或主题库。 对于不同类型的多个包含项（例如js和css），此标记需要在Handlebars脚本中多次使用。

此帮助程序仅在服务器端适用，它提供的功能与JSP脚 [本的ui:includeClientLib](../../help/sites-developing/taglib.md) 类似。
...
一个帮助程序，用于显示已经过多少时间到截止点，之后将显示常规日期格式。
...
* **上下文**:数字
   ...
   过去与“现在”比较的时间。 时间表示为从1970年1月1日（纪元）起的毫秒值偏移量。
...
一种帮助程序，它编码源字符串以编写到JavaScript字符串内容以帮助防止XSS。

注意：这不是验证程序，不用于写入任意JavaScript。
...
* 块帮助器是可以从模板中的任何位置调用的功能。 他们每次都可以调用模板块零次或多次，并具有不同的上下文。 它们包含{{#*name*}}和{{/*name*}}之间的上下文。

* Handlebars为名为“options”的帮助者提供一个最终参数。 特殊对象“选项”包括
...
必须在服务器端和客户端上实施自定义帮助器，尤其是在传递数据时。 对于SCF，当服务器在请求页面时为给定组件生成HTML时，大多数模板都会在服务器端编译和呈现。

### 服务器端自定义帮助器 {#server-side-custom-helpers}

要在服务器端实现和注册自定义SCF帮助程序，只需实现Java接口 [TemplateHelper](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/cq/social/handlebars/api/TemplateHelper.html)，将它作为 [OSGi服务](../../help/sites-developing/the-basics.md#osgi) ，并作为OSGi捆绑的一部分进行安装。
