...
* [分配徽章](#assign-and-revoke-badges) ，以标识成员在社区中的角色

* [为鼓励会员参与](#enable-scoring) （创建的内容数量）向会员授予基本徽章
* [高级徽章授予](advanced.md) ，将会员标识为专家（创建的内容质量）

**请注意** ，默认情况下 [不启用标记授予](implementing-scoring.md#main-pars-text-237875536)。
...
社区徽 [章控制台](badges.md) ，可添加自定义徽章，当会员获得（奖励）或在社区中承担特定角色（分配）时，这些徽章可显示给会员。

### 分配徽章 {#assigned-badges}

基于角色的徽章由管理员根据社区成员在社区中的角色分配给社区成员。

已分配（和已通知）标记存储在选定 [SRP](srp.md) 中，无法直接访问。 在GUI可用之前，分配基于角色的标记的唯一方法是使用代码或cURL进行分配。 有关cURL说明，请参阅“分配 [和撤销标记”部分](#assign-and-revoke-badges)。
...
* 评分和标记规则必 [须应](#apply-rules-to-content) 用于放置组件的页面（或上级）
...
可以使用标记控制台安装自 [定义标记](badges.md) ，并在标记规则中分配或指定。
...
* 对于每个得分规则累积的积分，分配 [徽章](#badges) (徽[章规则](#badging-rules))
...
要启用评分和标记，请添 `scoringRules` 加属 `badgingRules`性并添加到站点内容树中的任意节点。

如果站点已发布，则在应用所有规则和启用组件后，重新发布该站点。

应用于启用徽章的组件的规则是当前节点或其祖先的规则。

如果节点的类型 `cq:Page` 为（推荐），则使用CRXDE|Lite将属性添加到其 `jcr:content`节点。
...
评分和徽章规则仅适用于在创作模式下通过编辑组件配置启用徽章的组件 [实例](author-communities.md)。

布尔属性 `allowBadges`启用／禁用组件实例的标记显示。 可在论坛、问题与解 [答的组件编辑](author-communities.md) 对话框中配置该组件，并通过标记为显示标记的复 **选框对组件进行注释**。
...
评分规则是指其节点上 `cq:Page` 具有属性的 `jcr:content`节点，这些属性指定了定义它的子规则的列表。
...
该版本包含两个论坛功能的评 [分规则](functions.md#forum-function) （每个规则分别用于论坛和论坛功能的评论组件）:
...
徽章规则是其节点上具有属 `cq:Page` 性的类型的节点， `jcr:content`这些属性将评分规则与得分和徽章相关联。

徽章规则由强制属性 `thresholds`组成，它是映射到徽章的分数的有序列表。 得分必须按递增值排序。 例如：
...
徽 `scoringRules`章规则的属性只限制哪些评分规则可以与该特定徽章规则配对。
...
标记可以使用成员控制台或使用cURL [命令以编程方](members.md#badges-tab) 式分配给成员。
...
对于交付的社区组件，下表描述了为 `verbs`可用于评 `topic`分子规则 [的每个组件定义的](#scoring-sub-rules)。

>[!NOTE]
>
>新的布尔属性 `allowBadges`启用／禁用组件实例的标记显示。 在更新的组件编辑对话 [框中，可通过标有](author-communities.md) “显示标记”的复 **选框进行配置**。

**[日历组](calendar.md)**件`topic`社交活动= com/adobe/cq/social/calendar
...
**[评论组](comments.md)**件`topic`SocialEvent= com/adobe/cq/social/comment
...
**[文件库组](file-library.md)**件`topic`SocialEvent= com/adobe/cq/social/fileLibrary
...
**[论坛组](forum.md)**件社`topic`交活动= com/adobe/cq/social/forum
...
**[日志组](blog-feature.md)**件社`topic`交活动= com/adobe/cq/social/日志
...
**[问题与](working-with-qna.md)**组`topic`件社交事件= com/adobe/cq/social/qna
...
**[审阅组](reviews.md)**件`topic`SocialEvent= com/adobe/cq/social/review
...
**[评级组](rating.md)**件SocialEvent`topic`= com/adobe/cq/social/tally/rating
...
**[投票组](voting.md)**件`topic`SocialEvent= com/adobe/cq/social/tally/poting
...
对于自定义组件，将实例化一个SocialEvent以将组件的事件记 `actions`录为某个组件 `topic`。
...
然后， [重新发](sites-console.md#publishing-the-site) 布社区站点。
...
有关更多信息，请参阅开发 [人员的“评分和徽章](configure-scoring.md) ”页面。

有关高级评分引擎的信息，请参阅 [高级评分和标记](advanced.md)。

可配置的Lenerboard [组件](enabling-leaderboard.md)[和功能](functions.md#leaderboard-function) ，简化了社区站点上成员的显示及其得分。
