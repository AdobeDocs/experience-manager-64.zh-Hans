...
* [向成员授予基](#enable-scoring) 本奖励以鼓励其参与（创建的内容数量）
* [高级授予徽](advanced.md) 章，以将成员标识为专家（创建内容的质量）

**请注** 意，默认情况下 [不启用授予标记](implementing-scoring.md#main-pars-text-237875536)。
...
社区[标记控制台](badges.md)提供了添加自定义标记的功能，当会员获得（授予）或在社区中承担特定角色（分配）时，可以为其显示自定义标记。

### 分配的徽章{#assigned-badges}

基于角色的徽章由管理员根据社区成员在社区中的角色分配给社区成员。

已分配（和已获知）的标记存储在所选[SRP](srp.md)中，并且无法直接访问。 在GUI可用之前，分配基于角色的标记的唯一方法是使用代码或cURL进行分配。 有关cURL说明，请参阅标题为[分配和撤销标记](#assign-and-revoke-badges)的部分。
...
* 对于功能组件，标记必须[enabled](#enable-badges-for-component)
* 评分和标记规则必须[](#apply-rules-to-content)应用于放置组件的页面（或祖代）
...
可以使用[标记控制台](badges.md)安装自定义标记，并在标记规则中分配或指定这些标记。
...
| **属性** | **类型** | **描述** |
|---|---|---|
| badgingRules | String[] | [标记规则](#badging-rules)的数组列表 |
...
布尔属性`allowBadges`启用/禁用组件实例的标记显示。 可在论坛、QnA的[组件编辑对话框](author-communities.md)中配置此组件，并通过标有&#x200B;**显示标记**&#x200B;的复选框对组件进行注释。
...
此版本中包含[论坛功能](functions.md#forum-function)的两个评分规则（论坛功能的论坛和评论组件各有一个规则）：
...
1. /etc/community/scoring/rules/forums-scorning
...
标记规则是类型`cq:Page`的节点，其`jcr:content`节点上具有属性，这些属性将得分规则与得分和标记关联。
...
可以使用[members console](members.md#badges-tab)或使用cURL命令以编程方式将标记分配给成员。
...
>*member-用户档案-url*
>
>* 启用[隧道服务](users.md#tunnel-service)时，可引用作者实例
...
使用[入门教程](getting-started.md)（参与）站点可以快速尝试得分和标记：
...
有关更多信息，请参阅开发人员的[Scoring and Badges Essentials](configure-scoring.md)页面。
