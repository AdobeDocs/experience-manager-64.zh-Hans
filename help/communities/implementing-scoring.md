...
* [分配标记](#assign-and-revoke-badges) ，以标识成员在社区中的角色

* [向会员授予基本徽章](#enable-scoring) ，以鼓励其参与（创建的内容数量）
* [高级徽章奖励](advanced.md) ，将会员标识为专家（创建的内容质量）

**请注意** ，默认情况下不 [会启用标记授予](implementing-scoring.md#main-pars-text-237875536)。
...
徽章放置在会员的姓名下，以指示其在社区中的角色或地位。 标记可以显示为图像或名称。 当显示为图像时，名称将作为辅助功能的替代文本包含在内。
...
“社区 [标记](badges.md) ”控制台提供添加自定义标记的功能，这些标记可在会员获得（奖励）或在社区中承担特定角色（分配）时显示。

### 分配的徽章 {#assigned-badges}

基于角色的徽章由管理员根据社区成员在社区中的角色分配给社区成员。

分配的（和已唤醒的）标记存储在选定 [SRP中](srp.md) ，并且不能直接访问。 在GUI可用之前，分配基于角色的标记的唯一方法是使用代码或cURL进行分配。 有关cURL说明，请参阅标题为“分配 [和撤销标记”的部分](#assign-and-revoke-badges)。
...
评分服务会根据社区成员在社区活动中适用的规则，将基于奖励的徽章授予社区成员。

为了让徽章成为活动的奖励，必须做两件事：

* 必须为功 [能组件](#enable-badges-for-component) 启用标记
* 得分和标记规则必 [须应用](#apply-rules-to-content) 于放置组件的页面（或上级）
...
可以使用标记控制台安装自定 [义标记](badges.md) ，并在标记规则中分配或指定标记。
...
要启用评分和标记，请将属 `scoringRules` 性添 `badgingRules`加到站点内容树中的任意节点。
...
| scoringRules | String[] | 评分规则的数 [组列表](#scoring-rules) |
...
评分和徽章规则仅适用于通过在创作模式下编辑组件配置而启用徽章的组件 [实例](author-communities.md)。

布尔属性启 `allowBadges`用／禁用组件实例的标记显示。 可在论坛、问题与解 [答的组件编辑对话框中](author-communities.md) ，通过标有显示标记的复选框对组件进行 **配置和注释**。
...
很简单，每个得分规则都是一个或多个子规则的列表。 将评分规则应用于社区站点内容，以标识在启用标记时应用的规则。

评分规则是继承的，但不是加性的。 例如：

* 如果page2包含评分规则2，且其上级page1包含评分规则1
...
评分规则是类型的节 `cq:Page` 点，其节点上 `jcr:content`具有指定定义它的子规则列表的属性。
  ...
  <tr> 
   ...
   <td> 
    <ul> 
     ...
     <li>必须至少有一个动词属性</li> 
     <li>动词必须输入全部大写</li> 
     <li>可以有多个动词属性，但没有重复项</li> 
     <li>该值是该活动应用的得分</li> 
     <li>值可以是正数或负数</li> 
     <li>版本中支持的动词列表位于“主题”和“ <a href="#topics-and-verbs">动词”部分</a> 。</li> 
    </ul> </td> 
  </tr> 
  <tr> 
   ...
   <td> 
    <ul> 
     <li>可选；将子规则限制为由活动主题标识的社区组件</li> 
     <li>if specified:value是事件主题的多值字符串</li> 
     <li>“主题和动词”部分中的 <a href="#topics-and-verbs">主题列表</a></li> 
   ...
   <td> 
    <ul> 
     ...
     <li>如果为“高级”，则根据质量和数量指定评分引擎 
      <ul> 
       <li>需要其 <a href="advanced.md">他包</a></li> 
      </ul> </li> 
...
该版本包含两个论坛功能评分规则 [](functions.md#forum-function) （每个规则对应论坛功能的“论坛”和“评论”组件）:
...
在创作环境中对评分规则或子规则所做的任何更改或添加内容都需要在发布时安装。
...
徽章规则由必填属性组成， `thresholds`该属性是映射到徽章的分数的有序列表。 得分必须按递增值排序。 例如：
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><em>（必需）</em> “number|path”形式的多值字符串 
    ...
    </ul> 必须对字符串进行排序，以使数字在值中增加，并且数字和路径之间不应显示空格。<br /><br /> 示例条目： <code>80|/etc/community/badging/images/gold-badge/jcr:content/gold.png</code></td> 
  </tr> 
  <tr> 
   <td>badgingType</td> 
   <td>字符串</td> 
   <td><em>（可选）</em> 将评分引擎标识为“基本”或“高级”。 如果需要高级评分引擎，请参阅高 <a href="advanced.md">级评分和标记</a>。 默认值为“basic”。</td> 
  </tr> 
  <tr> 
   <td> 
    <code>scoringRules </code></td> 
   <td>String[]</td> 
   <td>(可<em>选</em>)用于将标记规则限制为由评分规则标识的评分事件的多值字符串</td> 
  </tr> 
 </tbody> 
</table>
...
可以使用成员控制台或使用cURL命令以编 [程方式将标记](members.md#badges-tab) 分配给成员。

以下cURL命令显示分配和撤销标记的HTTP请求所需的内容。 基本格式为：

cURL -i -X POST -H头-u ** *signin * -F * operation * -F * badge * *member-profile-url*
...
例如：/etc/community/badging/images/droducator/jcr:content/moderator.png
...
>成 *员-profile-url*
>
>* 如果启用了隧道服务 [](users.md#tunnel-service) ，则可引用作者实例
...
可以通过将为该组件创建的事件主题与动词相关联来为自定义组件创建评分和标记规则。

## 主题与动词 {#topics-and-verbs}

当成员与社区功能交互时，会发送可触发异步监听器的事件，如通知和评分。

组件的SocialEvent实例将事件记录 `actions`为发生的事件 `topic`。 SocialEvent包括一种方法，用于返回与 `verb`操作关联的内容。 和之间 *有n-1* 关 `actions`系 `verbs`。

对于交付的社区组件，下表描述了为可 `verbs`用于评 `topic`分子规则的每 [个组件定义的](#scoring-sub-rules)。

>[!NOTE]
>
>新的布尔属性 `allowBadges`启用／禁用组件实例的标记显示。 在更新的组件编辑对话 [框中，可通过标有“显示标记](author-communities.md) ”的复选框 **对其进行配置**。

**[日历组件](calendar.md)**SocialEvent`topic`= com/adobe/cq/social/calendar
...
**[评论组](comments.md)**件SocialEvent`topic`= com/adobe/cq/social/comment
...
**[文件库组件](file-library.md)**SocialEvent`topic`= com/adobe/cq/social/fileLibrary
...
**[论坛组](forum.md)**件SocialEvent`topic`= com/adobe/cq/social/forum
...
**[日记帐组](blog-feature.md)**件SocialEvent`topic`= com/adobe/cq/social/journal
...
**[问题与组](working-with-qna.md)**件SocialEvent`topic`= com/adobe/cq/social/qna
...
**[审阅组](reviews.md)**件SocialEvent`topic`= com/adobe/cq/social/review
...
**[评级组件](rating.md)**SocialEvent`topic`= com/adobe/cq/social/tally/rating
...
**[投票组件](voting.md)**SocialEvent`topic`= com/adobe/cq/social/tally/porting
...
对于自定义组件，将实例化SocialEvent，以将组件的事件记录 `actions`为某个组件的事件 `topic`。

要支持评分，SocialEvent需要覆盖该方法， `getVerb()` 以便为每 `verb`个事件返回相应的值 `action`。 为 `verb` 操作返回的可能是常用的(如 `POST`)或专用于组件的(如 `ADD RATING`)。 和之间 *有n-1* 关 `actions`系 `verbs`。

## 疑难解答 {#troubleshooting}

### 徽章不显示 {#badges-are-not-appearing}

如果对网站内容应用了评分和徽章规则，但没有为任何活动提醒标记，请确保为该组件的实例启用了标记。
...
使用入门教程（参与）站点可以快速尝试 [评分和徽章](getting-started.md) :
...
然后，重 [新发布](sites-console.md#publishing-the-site) 社区站点。
...
有关更多信息，请参阅适用于开 [发人员的Scoring和Badges Essentials](configure-scoring.md) （评分和标记基本信息）页面。

有关高级评分引擎的信息，请参阅高 [级评分和标记](advanced.md)。

可配置的Leorboard组 [件](enabling-leaderboard.md)[](functions.md#leaderboard-function) 和功能可简化成员的显示，并简化其在社区站点上的得分。
