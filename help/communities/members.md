...
使用“社区成员”和“组”控制台，可以从作者环境创建 *和管理* “发布”环境中注册的成员和 *成员组* 。 只有启用隧道服务 [时，才可](deploy-communities.md#tunnel-service-on-author) 以执行此操作。
...
当用户是启 `Views`用了 `Posts`Adobe Analytics `Follows`的一个或多个社区 `Likes` 站点的成员时，将更新显示、 [和列](sites-console.md#analytics)。
...
* **[!UICONTROL 贡献限]**制如果选中，成员发布内容的能力将受到限制。
默认值取决于贡献限制的配置。
请参 [阅成员贡献限制](limits.md)。

* **[!UICONTROL 更改口]**&#x200B;令修改现有成员时存在的链接。 提供社区管理员重置成员口令的功能。
