...
在AEM Communities中，可以通过作者和社区版主（指定为版主的受信任社区成员）的发布环境和发布批量[审核社区内容。](moderate-ugc.md)

管理员和社区审核者还可以在发布环境中执行[in-context moderation](in-context.md)。
...
仅当以管理员身份登录或以` [moderator permissions](in-context.md#identifyingtrustedmembers)`成员身份登录时，才可执行审核任务。 显示的唯一社区内容是允许登录成员审核的社区内容。

>[!NOTE]
>
>只有在所选SRP实现公共存储时，发布环境中的UGC才在作者中可见。 例如，默认情况下，存储为JSRP，它不是创作和发布的常用存储。 请参阅[社区内容存储](working-with-srp.md)。
...
* [筛](moderation.md#filter-rail) 选栏会打开边栏，允许选择要筛选内容的属性。

工具栏位于顶部导航栏的下方，在左侧提供以下切换开关：

![雪巫](assets/toggleswitch.png)

[滤镜边栏](moderation.md#filter-rail)\
在选择“搜索”时打开边栏，通过该边栏可选择要筛选内容的属性。
...
* 发布时
* 对该员额的答复数
* [与帖](moderate-ugc.md#sentiment) 子相关的情感
* 如果批准，则显示复选标记
* 如果存在附件，则显示平装夹
...
每个类别内的过滤器是&#x200B;**OR**&#x200B;的，不同类别内的过滤器是&#x200B;**AND**&#x200B;的。
...
标记某条内容后，它将保持标记状态，直到您再次选择&#x200B;**[!UICONTROL 标记]**&#x200B;按钮取消该条内容的标记。 请注意，没有标记级别，如重要或后续操作。
...
[Sentition](moderate-ugc.md#sentiment) 将引用的UGC限制为情绪值为正、负或中性的帖子。

![chlimage_1-479](assets/chlimage_1-479.png)

## 审核操作{#moderation-actions}

[对在](moderate-ugc.md#moderation-actions) 内容区域中或查看内容详细信息时所做的一个或多个选择执行审核操作扫描。
