...
对于生产环境,[SolrCloud模式](solr.md#solrcloud-mode)比独立模式（单个本地Solr设置）提供了更高的性能。
...
如果从现有AEM Communities 6.0站点升级，则任何预先存在的UGC必须在升级到AEM Communities 6.3后转换为符合[ SRP](srp.md) API所需的结构。
...
要解决此错误，请按照[安装标准MLS](solr.md#installing-standard-mls)的说明操作，确保
...
如果由于缺少类定义而尝试与MongoDB服务器建立安全连接失败，则必须更新公共主存储库中提供的MongoDB驱动程序包`mongo-java-driver`。
