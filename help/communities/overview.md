...
AEM Communities支持与网站访客建立关系，通过博客、问题与答案和活动日历提供信息，同时通过论坛、评论和其他社区内容(通常称为用户生成内容(UGC))获得洞察。

此外，AEM Communities还允许发布环境中受信任成员进行审核，使用Twitter和Facebook进行社交登录，内联社区内容翻译，从已发布的社区站点创建社区组，得分到奖章、文件共享、通知和活动流。
...
* 提供基本功能：
   ...
   * 登录：自助注册、社交 [登录](social-login.md)、用户配置文件
   * 通知：会员可以看到与他们相关的事件
   * 消息传递：会员可以在社区站点内发送或接收消息
   * 搜索：能够在社区站点内搜索
   * 语言切换：为多语言站点选择语言 [的能力](../../help/sites-administering/translation.md)
   * 管理：授权会员有权审核和管理社区站点内的用户
* 消除了许多页面级创作步骤：
   * 品牌：可选上传横幅图像，以在社区站点的所有页面上显示
   * 导航菜单：为社区站点模板中包含的功能提供了导航链接

要体验快速创建新社区站点的轻松性，请访 [问AEM Communities快速入门](getting-started.md)。
...
* [升级到AEM 6.3 Communities](upgrade.md):提供有关UGC的有用信息（当移动到AEM 6.3时）。
...
* [站点模板](tools.md#sitetemplatesconsole) “控制台
...
* [存储配置控制台](tools.md#storageconfiguratonconsole) ,

   * 选择并配置站 [点的公用](working-with-srp.md) store

* [组件指南](components-guide.md)

   * 示例站点“社 [区组件”](http://localhost:4502/editor.html/content/community-components/en.html)，它提供所有社区组件的示例，包括它们的默认配置以及试验这些组件的能力
...
请参阅“站 [点模板”控制台](sites.md)。

## 社区功能 {#community-functions}

社区体验预期的功能众所周知。 在AEM Communities中，这些功能可以作为构件块提供，称为社区功能。

社区功能是由连接到功能中的组件组成的常规AEM页面，该功能可轻松地融入社区站点模板中。

请参阅“社 [区功能”控制台](functions.md)。

## 社区组和组模板 {#community-groups-and-group-templates}

社区组功能是让来自创作和发布环境的授权用户和社区成员在社区站点内动态创建子社区的能力。

从创作环境中，当模板的结构包含“组”功能时，可以在现有社区站点内创建社区组（子社区）或嵌套在现有组 [内](functions.md#groups-function)。
...
* [AEM Communities快速入门](getting-started.md) -快速创建包含嵌套组的社区站点的教程

## 社区组件 {#community-components}

构 [建社区站点的社区组件](author-communities.md) ，可用于向任何AEM站点添加社区功能。

社区 [组件指南](components-guide.md) ，可用于交互式探索组件。
...
要体验快速创建新参与社区的轻松性，请访 [问AEM Communities快速入门](getting-started.md)。
...
在配置Enablement Add-on时，可以创建 [Enablement Community](enablement.md)，这需要额外的许可才能在生产环境中使用。 支持社区站点将包含任务 [功能](#community-functions)。

要体验创建新启用社区的轻松性，请访 [问AEM Communities for Enablement快速入门](getting-started-enablement.md)。
