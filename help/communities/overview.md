...
* 提供基本功能：
   ...
   * 登录：自助注册，[社交登录](social-login.md)，用户用户档案
   * 通知：会员可以看到与他们相关的事件
...
要体验快速创建新社区站点的便利性，请访问[AEM Communities入门](getting-started.md)。
...
在创作环境中，全局导航控制台提供对[社区控制台](consoles.md)的访问，该控制台包含：
...
* [Moderationconsole(](moderation.md) 审核控制台)
...
* [存储配](tools.md#storageconfiguratonconsole) 置控制台
   ...
   * 选择并配置站点的[公用存储](working-with-srp.md)

* [组件指南](components-guide.md)

   * 示例站点[社区组件](http://localhost:4502/editor.html/content/community-components/en.html)，它提供所有社区组件的示例及其默认配置和试验能力
...
当模板的结构包含[组函数](functions.md#groups-function)时，可以从作者环境在现有社区站点内或嵌套在现有组内创建社区组（子社区）。
...
要体验快速创建新参与社区的便利性，请访问[AEM Communities入门](getting-started.md)。
...
当配置[Enablement Add-on](enablement.md)时，可以创建启用社区，这需要在生产环境中使用额外的许可。 启用社区站点将包括[赋值函数](#community-functions)。

要体验创建新启用社区的轻松性，请访问[Enablement](getting-started-enablement.md)AEM Communities快速入门。
...
* 要安装的包
在Windows、MacOS和Linux上，AEM Demo Machine已通过CQ 5.5、CQ 5.6.1、AEM 6.0、AEM 6.1、AEM 6.2和AEM 6.3成功测试了Demo Machine。
