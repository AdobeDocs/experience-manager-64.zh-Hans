...
* **[!UICONTROL 资源名称(&amp;A);]**
   ...
   (*required*)资源的显示名称。 通过显示名称创建有效的节点名称。
...
* **[!UICONTROL 从我的本地文]**
件上传从本地文件系统上传将使用本机文件浏览器选择并上传文件。支持的文件类型包括SCORM.zip（HTML5或SWF）、MP4视频、SWF、PDF和图像类型(JPG、PNG、GIF、BMP)。 文件名将变为添加到资源库的资产名称。

* **[!UICONTROL 浏览资源]**
库从资源库中选择。选择仅限于社区站点中可见的选项。
...
* **[!UICONTROL 添]**
加受分派者使用下拉菜单从 [成](#members-group) 员（以粗体列出）)中选择要作为学员登记的用户和用户组。成员登录社区站点后，其登记的启用资源（和学习路径）将显示在其[分配](functions.md#assignments-function)页面上。
...
* **[!UICONTROL 学习路径名称]**
   ...
   (*required*)学习路径的显示名称。 通过显示名称创建有效的节点名称。
...
* **[!UICONTROL 必要的学习路径]**
(
*可选*)选择其他已发布的学习路径后，学员必须先完成这些路径，然后才能选择此学习路径。
...
* **[!UICONTROL 强制执行学习路径中的排序]**
   ...
   （*可选*），如果设置为“On”，则添加启用资源的顺序是学员完成学习路径的顺序。 默认为关闭。

* **[!UICONTROL 资源]**

   从为当前社区站点创建的*已发布*支持资源中选择一个或多个资源。
...
要临时使启用资源在发布环境中对成员不可访问而不将其删除，请使用`Unpublish`资源的全球图标。
