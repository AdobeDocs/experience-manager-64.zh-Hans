...
组 `Reviews`件是可供使用的组 [ 件 `Comments`](comments.md)[`Rating`](rating.md) 和组件的组合。

该组 `Reviews Summary (Display)` 件提供组件的活动或关闭实例的摘要，以 `Reviews` 便在站点的其他位置显示。

>[!NOTE]
>
>不支持匿名发布审阅。 站点访问者必须注册（成为会员）并登录才能参加。 已登录的访客可随时更新其审阅。
...
在“必 **[!UICONTROL 需评级]** ”选项卡下，从“允许评级 **** ”列表中重新输入需要评级的项目。 如果某个项目仅在“允许的评级”选项卡上指定，则该项目在由成员提交时可能会保持无标记状态。
...
* **[!UICONTROL 允许回复]**&#x200B;如果选中此项，则允许回复审阅。 默认为未选中。

* **[!UICONTROL 已关闭]**&#x200B;如果选中此项，则审阅将关闭给新审阅和回复。 默认为未选中。

* **[!UICONTROL 允许上传文]**&#x200B;件如果选中此项，则允许上传文件附件以供审阅。 默认为未选中。

* **[!UICONTROL 最大文件大小]**&#x200B;仅在选中“允许 **[!UICONTROL 文件上传]** ”时相关。 此字段限制已上载文件的大小（以字节为单位）。 默认为10 MB。

* **[!UICONTROL 最大消息长]**&#x200B;度可输入到文本框中的最大字符数。 默认为4096个字符。

* **[!UICONTROL 仅当选中“允]**&#x200B;许文件上传”时， **[!UICONTROL 允许的文件类型]** ，才相关。 以逗号分隔的文件扩展名列表，带有“点”分隔符。 例如：.jpg、.jpeg、.png、.doc、.docx、.pdf。 如果指定了任何文件类型，则不允许指定那些未指定的文件类型。 默认值未指定，因此允许所有文件类型。

* **[!UICONTROL 富文本编辑器]**&#x200B;如果选中此选项，则可以使用标记输入帖子。 默认为未选中。
...
* **[!UICONTROL 预审核]**&#x200B;如果选中此项，审阅必须经过批准，才能显示在发布站点上。 默认为未选中。

* **[!UICONTROL 删除审]**&#x200B;阅如果选中此项，则发布审阅的成员可以删除该审阅。 默认为未选中。

* **[!UICONTROL 拒绝审阅]**&#x200B;如果选中此项，则允许审核者拒绝审阅。 默认为未选中。

* **[!UICONTROL 关闭／重新打开审]**&#x200B;阅如果选中，允许审核者关闭并重新打开审阅。 默认为未选中。

* **[!UICONTROL 标记审阅]**&#x200B;如果选中此项，允许成员将审阅标记为不适当。 默认为未选中。

* **[!UICONTROL 标记原因列]**&#x200B;表如果选中此项，则允许成员从下拉列表中选择将审阅标记为不适当的原因。 默认为未选中。

* **[!UICONTROL 自定义标记原]**&#x200B;因如果选中此项，则允许成员输入自己将审阅标记为不适当的原因。 默认为未选中。

* **[!UICONTROL 审核阈]**&#x200B;值输入在通知审核者之前必须由成员标记审核的次数。 默认值是一次(1)。

* **[!UICONTROL 标记限]**&#x200B;制输入在审阅隐藏于公共视图之前必须标记的次数。 此数字必须大于或等于审核 **[!UICONTROL 阈值]**。 默认为5。
...
* `Review Path`
   ...
   /content/sites/engage/cn/page/jcr:content/content/primary/reviews
...
未登录的站点访问者只能阅读已发布的评论，翻译这些评论（如果支持），但不能添加评级或评论，也不能标记他人的评论评论。
