...
请参阅[AEM Communities](getting-started.md)入门，了解在创作环境中创建社区站点的速度，以及如何从创作和发布环境创建社区组。

>[!NOTE]
>
>用于创建[社区站点](sites-console.md)、[社区站点模板](sites.md)、[社区组模板](tools-groups.md)和[社区功能](functions.md)的主要“社区”菜单仅用于创作环境。
...
* 标识[主发布者](deploy-communities.md#primary-publisher)
...
* [为社交登](social-login.md#adobe-granite-oauth-authentication-handler) 录启用OAuth处理器
...
站点创建控制台提供了根据选定的[社区站点模板](sites.md)和设置来组合站点功能的分步方法。
...
* **[!UICONTROL 社区站点根]**:站点的根路径。

   默认根目录为`/content/sites`，但根目录可移至网站中的任意位置。

* **[!UICONTROL 社区站点基本语言]**:(对于单一语言，请保持原样：英语)使用下拉菜单从可用语 *言(* 德语、意大利语、法语、日语、西班牙语、葡萄牙语（巴西）、中文（繁体）和简体中文)中选择一种或多种基本语言。将根据[多语言站点翻译内容](../../help/sites-administering/translation.md)中介绍的最佳实践，为添加的每种语言创建一个社区站点，并且该站点将存在于同一站点文件夹中。 每个站点的根页面将包含一个子页面，该子页面由所选语言之一的语言代码命名，如“en”表示英语，“fr”表示法语。

* **[!UICONTROL 社区站点名称]**:显示在URL中的站点根页面的名称

   * 多次 — 检查名称，因为创建站点后，该名称不易更改
...
>
>约定适用于发布端[用户和用户组](users.md)（成员和成员组）在创作环境中不重复。
>
...
* **[!UICONTROL 允许社交登录: Facebook]**
   ...
   如果选中此项，允许网站访客使用其Facebook帐户凭据登录。 应将选定的[Facebook云配置](social-login.md#create-a-facebook-connect-cloud-service)配置为在创建社区站点后将用户添加到社区站点的成员组。
...
* **[!UICONTROL 允许社交登录: Twitter]**
   ...
   如果选中此项，允许站点访客使用其Twitter帐户凭据登录。 应将选定的[Twitter云配置](social-login.md#create-a-twitter-connect-cloud-service)配置为在创建社区站点后将用户添加到社区站点的成员组。
...
>**[!UICONTROL 允许社交登录]**
>
>尽管示例Facebook和Twitter配置可以存在并可供选择，但对于[生产环境](../../help/sites-administering/production-ready.md)，有必要创建自定义Facebook和Twitter应用程序。 请参阅[使用Facebook和Twitter进行社交登录](social-login.md)。
...
* **[!UICONTROL 拥有权限的社区成员]**
   ...
   开始键入以选择一个或多个社区成员或成员组，以便在为[社区功能](functions.md)选择了`Allow Privileged Member`时，能够创建新内容。
...
* **[!UICONTROL 内容已通过预审]**
   ...
   如果选中此项，则在审查方批准之前，不会显示已发布的社区内容。 默认为未选中。 有关详细信息，请参阅[协调社区内容](moderate-ugc.md#premoderation)。
...
* **[!UICONTROL 启用 Analytics]**
   ...
   仅当Adobe Analytics已[配置](analytics.md)以用于Communities功能时可用。
...
* **[!UICONTROL 云配置框架引用]**
   ...
   `Communities`是Analytics Configuration for Communities Features文 [档中的框](analytics.md#aem-analytics-framework-configuration) 架示例。
...
* **[!UICONTROL 允许机]**
器翻译选中（默认为未选中）后，站点内的UGC将启用机器翻译。这不会影响任何其他内容，如页面内容，即使该站点设置为多语言站点也是如此。 有关为AEM Communities配置授权翻译服务的信息，请参阅[翻译用户生成的内容](translate-ugc.md)。 有关完整概述，请参阅[多语言站点的翻译内容](../../help/sites-administering/translation.md)。

![chlimage_1-456](assets/chlimage_1-456.png)

* **[!UICONTROL 为选定的语言启用机器翻译]**

   为机器翻译启用的语言默认为由[翻译集成配置](translate-ugc.md#translation-integration-configuration)指定的系统设置。 通过删除默认值和/或从下拉菜单中选择其他语言，可能会覆盖此站点的这些默认设置。
...
* **选择持久性选项**
   ...
   * **[!UICONTROL 依据用户的请求翻译贡献的文稿，并在此后持续]**
      ...
      如果选择此项，则在发出请求之前不翻译内容。 翻译后，翻译会存储在存储库中。
...
当所选社区站点模板包含[赋值函数](functions.md#assignments-function)时，`ENABLEMENT`设置适用，当启用功能获得许可和[已配置](enablement.md)时，该函数可用。 包含赋值函数的引用站点模板为`Reference Structured Learning Site Template.`

* **[!UICONTROL 启用管理者]**

   （必需）只有`Community Enablementmanagers`组的成员可供选择以管理此启用社区。 Enablement Managers负责将成员分配给资源。 另请参阅[管理用户和用户组](users.md)。
...
“结构”面板允许修改最初从选定社区站点模板创建的结构。 从面板中，
...
例如，如果标题为“入门教程”的站点的名称为“参与”，则版主的用户组为：
...
创建后，应将已应用的[Facebook云服务](social-login.md#createafacebookcloudservice)配置为将用户添加到此组。
