...
对于生产环境,[SolrCloud模式](#solrcloud-mode)比独立模式（单个本地Solr设置）提供了更高的性能。
...
[建](https://cwiki.apache.org/confluence/display/solr/SolrCloud) 议对生产环境使用SolrCloudmode。在SolrCloud模式下运行时，必须先安装并配置SolrCloud，然后再安装多语言搜索(MLS)。
...
#### 1.将配置上载到ZooKeeper {#upload-a-configuration-to-zookeeper}
...
### 标准与高级MLS {#comparison-of-standard-and-advanced-mls}的比较
...
1. 准备新配置：
   ...
   1. 将现有Solr配置目录的内容复制到&#x200B;*new-config-dir*
      ...
      * 对于Solr5:copy *solr-install-dir*/server/solr/configsets/data_driven_模式-configs/&amp;ast;
...
1. 备份Solr配置目录中的&#x200B;**模式.xml**&#x200B;和&#x200B;**solrconfig.xml**，例如：
...
1. 准备新配置：
   ...
   1. 将提取的&#x200B;**模式.xml**&#x200B;和&#x200B;**solrconfig.xml**&#x200B;复制到&#x200B;*new-config-dir*&#x200B;以覆盖现有文件
   1. 对于Solr5:将&#x200B;*solr_install_dir*/server/solr/configsts/sample_techproducts_configs/lang/&amp;ast;.txt&quot;复制到&#x200B;*new-config-dir*/lang/
...
1. [创建一](#create-a-collection) 个集合，指定必要的参数，如分片数、副本数和配置名称。
1. 如果在创建集合时未提供配置名&#x200B;**，则[将此新创建的集合与上传到ZooKeeper的配置链接到](#link-a-collection-to-a-configuration-set)
