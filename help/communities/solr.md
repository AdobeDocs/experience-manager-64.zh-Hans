...
* [4.10版或](https://archive.apache.org/dist/lucene/solr/4.10.4/) 5 [版](https://archive.apache.org/dist/lucene/solr/5.5.3/)
...
[建议在生产环境中使用SolrCloud](https://cwiki.apache.org/confluence/display/solr/SolrCloud) 模式。 在SolrCloud模式下运行时，必须先安装并配置SolrCloud，然后再安装多语言搜索(MLS)。

建议按照SolrCloud说明安装：

* 3个SolrCloud节点位于同一台服务器上
* 外部Apache ZooKeeper
...
1. 准备新配置：
   ...
   1. 将现有Solr配置目录的内容复 *制到new-config-dir*
      ...
      * 对于Solr5:copy *solr-install-dir*/server/solr/configsets/data_driven_schema_configs/&amp;ast;
   1. 将下载的 **schema.xml和solrconfig** .xml复制到new-config-dir **,**** 以覆盖现有文件
...
为了支持高级MLS，除了自定义架构和Solr配置外，还需要新的Solr插件才能使用SRP集合（MSRP或DSRP）。 所有必需项都打包到一个可下载的zip文件中。 此外，安装脚本包含在独立模式部署Solr时使用。
...
1. 准备新配置：
   ...
   1. 创建 *new-config-dir*
      ...
      * 创建子文件夹stopwords/和lang/
   1. 将现有Solr配置目录的内容复制 *到new-config-dir*

      * 对于Solr4:复 *制solr-install-dir*/example/solr/collection1/conf/&amp;ast;
      * 对于Solr5:复 *制solr-install-dir*/server/solr/configsets/data_driven_schema_configs/&amp;ast;
   1. 将提取的 **schema.xml和solrconfig** .xml复制 **到**** new-config-dir以覆盖现有文件
   1. 对于Solr5:将 *solr_install_dir*/server/solr/configs/sample_techproducts_configs/conf/lang/&amp;ast;.txt”复制到 *new-config-dir*/lang/
   1. 将提取的 **stopwords/** folder复制到 *new-config-dir* ，从而生成 *new-config-dir*/stopwords/&amp;ast;.txt
...
* 安装脚本将在安装新版本之前先备份schema.xml和solrconfig.xml，然后附加“.orig”
...
要将搜索更改为使用更新后反映由提交引起的更改的索引，请将包含的&lt;openSearcher>更改为true。

&lt;autoSoftCommit>:“软”提交可确保更改可见（索引已更新），但不确保更改同步到稳定存储（硬提交）。 结果是性能的提高。 默认情况下，&lt;autoSoftCommit>处于禁用状态，包含的&lt;maxTime>设置为-1。
