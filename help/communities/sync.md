...
在AEM Communities中，从发布环境（取决于配置的权限）中，站点访客可 *能成为成员* ，创建用户组 *，并编*&#x200B;辑其成 ****&#x200B;员配置文件。

*用户数据* ，是指用户、用户配置 *文*&#x200B;件和用户 *组的术语***。

*Members* (成员 *)是指在发布环境中注册* 的用户，而不是指在创作环境中注册的用户。

有关用户数据的详细信息，请访 [问管理用户和用户组](users.md)。
...
当拓扑 [是发布场](topologies.md) ，在一个发 [](../../help/sites-deploying/recommended-deploys.md#tarmk-farm)布实例上进行的注册和修改需要与其他发布实例同步。 成员需要能够登录并查看任何发布节点上的数据。
...
* **VLT包**:是一个zip文件，其中包含在发布者上完成的所有更改，这些更改需要在发布者之间分发。 发布者上的更改将生成由更改事件监听器选取的事件。 这将创建一个包含所有更改的vlt包。
...
在创作时，当从 [Communities Sites控制台发布社区站点时](sites-console.md)，其效果是复制关联的页面， [](../../help/sites-deploying/configuring.md#replication-reverse-replication-and-replication-agents) Sling分发动态创建的社区用户组，包括其成员资格。

### 用户已创建或在发布时编辑配置文件 {#user-is-created-or-edits-profile-on-publish}

根据设计，在发布环境中创建的用户和配置文件（如通过自注册、社交登录、LDAP身份验证）不会显示在创作环境中。

当拓扑为发布场 [且用户同步已正确配置](topologies.md) ，则使用Sling分发在发布场 *内同步用户* 和用户配置文件 ** 。
...
作者实例ping发布者的导出器端点。 每当在特定发布者(n)上创建或更新用户时，作者就会从其导出者端点获取内容并将内容推送到其他发布者 [](sync.md#main-pars-image-1413756164) （n-1，即从中获取内容的发布者除外）。
...
1. 找到 **[!UICONTROL Apache Sling Distribution Agent - Sync Agents Factory]**。
   ...
   * 选择要打开进行编辑的现有配置（铅笔图标）。
   * 验证名称： **`socialpubsync`.**
   * 选中“已 **[!UICONTROL 启用]** ”复选框。
   * 选择 **[!UICONTROL 使用多个队列]**。
   * 指定导 **[!UICONTROL 出器端点]** 和导 **[!UICONTROL 入器端点]** （您可以添加更多导出器和导入器端点）。
...
1. 访问 [Web控制台](../../help/sites-deploying/configuring-osgi.md)。
   ...
   例如， [http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr)。
1. 找到 **[!UICONTROL Adobe Granite分发——加密密码传输机密提供商]**。
...
1. 选中“已 **[!UICONTROL 启用]** ”复选框并保存。
1. 指定要在允许的根中复制的节 **[!UICONTROL 点路径]**。
1. 对每个实例重复 `publish` 上述步骤。
...
此配置可同步发布者之间的组成员关系。\
如果更改某个发布者中某个用户组的成员关系不会更新其他发布者的成员资格，则请确保将 **ref:members** 添加到查 **看的属性名称**。
...
1. 选中“已 **[!UICONTROL 启用]** ”复选框。
1. 在查 **找的`:members`**属性名`description`称中，将rep指&#x200B;**[!UICONTROL 定为propertyName]**，然后指定为“保存”。
...
作者每30秒对发布者进行投票（默认）。 如果文件夹 */var/sling/distribution/packages/socialpubsync - vlt /shared*，则它将获取这些包并将其安装到其他发布者上。
...
对于订阅和后续订阅中存在差异的Sling分发问题，请检查 **[!UICONTROL AEM Communities用户同步监听器配置中是否设置了以下属性]** :
...
1. 将设置 **`ignorablenodes`** 为：
   ...
   rep( `:cache` 因为我们使用粘性会话，所以不需要将此节点同步到不同的发布者)
...
1. 在目录 `crx-quickstart/launchpad/felix` 中，搜索并删除名为_sling.id.file的文件。
...
1. 启动发布实例。 启动时，将为其分配一个新的Sling ID。
1. 验证 **[!UICONTROL Sling ID现在是]** 唯一的。
...
1. 策略处理
   * 要用新的表示节 `:policy` 点覆盖现有的表示节点，请添加第三个包过滤器：
...
1. **检查添加的[配置是否不正确](../../help/sites-administering/sync.md#improperconfig)。** 请确保不添加或编辑多个配置，而应编辑现有的默认配置。
1. **检查配置**。 确保在AEM作 [者实例中](sync.md#bestpractices) ，正确设置所有配置，如最佳实践 [中所述](sync.md#main-pars-header-863110628)。
1. **检查授权用户权限**。 如果包安装不正确，请检查在第一个Publish实例中 [创建的授权用户](../../help/sites-administering/sync.md#createauthuser) ，是否具有正确的ACL。
...
1. **检查Diff Observer Factory配置**。 如果仅特定节点未在发布场之间同步（例如，组成员未同步），则确保启用 [Adobe Granite Distribution - Diff Observer Factory](../../help/sites-administering/sync.md#diffobserver) （差异观察器工厂）配置，并在查看的属性名称中设置 **rep:members******。
1. **检查AEM Communities用户同步监听器配置。** 如果已创建的用户已同步，但订阅和跟踪操作不起作用，则请确保AEM Communities用户同步监听器配置具有：

   * 节点类型——设置为 **rep:nt:unstructured**, **nt:resource,** rep:ACL **,********sling:FolderFolder, andSling:OrderedFolder**
   * 可忽略节点——设置为 **.tokens****、** system **和rep:cache**
   * 已分发文件夹——设置为要分发的文件夹

1. **检查在发布实例上创建用户时生成的日志**。 如果上述配置设置得当，但用户同步仍不工作，请检查用户创建时生成的日志。
   ...
   1. 在AEM作者实例上，使用管理员权限登录。
      ...
      1. 访问 [Web控制台](../../help/sites-deploying/configuring-osgi.md)。 例如， [http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr)。
      1. 找到配置 **[!UICONTROL Apache Sling Distribution Agent - Sync Agents Factory]**。

      1. 取消选中“已 **[!UICONTROL 启用]** ”复选框。
      在作者实例上禁用用户同步时，将禁用（导出器和导入器）端点，并且作者实例是静态的。 作 **[!UICONTROL 者不]** ping或获取vlt包。

      现在，如果用户是在发布实例上创建的， **[!UICONTROL vlt]** package将在 */var/sling/distribution/packages/ socialpubsync - vlt /data节点中创建* 。 如果作者将这些包推送到其他服务。 您可以下载并提取此数据以检查将哪些属性推送到其他服务。

   1. 转到发布者，然后在发布者上创建用户。 因此，会创建事件。
   1. 检查在用 [户创建时创建](sync.md#troubleshoot-sling-distribution-in-aem-communities)的日志的顺序。
   1. 检查是否 **[!UICONTROL 在上]** 创建vlt包 `/var/sling/distribution/packages/socialpubsync-vlt/data`。
   1. 现在，在AEM作者实例上启用用户同步。
   1. 在发布者上，在 **[!UICONTROL Apache Sling Distribution Agent - Sync Agents Factory中更改导出器或导入程序端点]**。
