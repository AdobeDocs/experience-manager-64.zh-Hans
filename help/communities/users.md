...
seo-description: AEM Communities的用户可以自行注册和编辑其配置文件
...
在AEM Communities中，在发布环境中，用户可以自行注册和编辑其配置文件。 如果拥有相应的权限，则还可能

* 在社区站点内创建子社区(请参阅社 [区组](creating-groups.md))
* [审核用户生成的内容](moderation.md) (UGC)
* 成为启 [用资源联系人](resources.md)
* 有权 [为博客](#privileged-members-group) 、日历、问题与解答和论坛创建条目

在发布环境中注册的用户通常称为社区成 *员（成员）* ，以区分他们与创作环境中的*用户*。

通过将成员分配给在从创作环境创建或修 [改社区站点时动态创建的成员（用户）组之一](#publish-group-roles) ，来授予权 [限](sites-console.md)[,](sites-console.md#modifying-site-properties) 即可授予权限。 从创作环境中工作时，成员可通过隧道服务从发布环境中 [看到](#tunnel-service)。

根据设计，在发布环境中创建的成员和成员组不应显示在创作环境中。 在创作环境中创建的用户和用户组同样希望保留在创作环境中。

当创作用户和发布成员来自同一用户列表（如从同一LDAP目录同步）时，在创作和发布环境中，他们不被视为具有相同权限和用户组成员关系的同一用户。 成员和用户的角色必须根据需要在发布和创作时分别建立。

对于发 [布场](topologies.md)，在一个发布实例上进行的注册和修改需要与其他发布实例同步，以便它们能够访问相同的用户数据。 [有关详细信息， ](sync.md)请参阅用户同步[，该同步包含一个描述 ](sync.md#what-happens-when)何时发生……的部分.
...
创建新社区站点时，将使用唯一的id(uid)和权限动态创建新用户组，这些权限适用于在创作环境(请参阅 [Author Group Roles](#author-group-roles))或发布环境(请参阅 [Publish Group Roles](#publish-group-roles))中管理社区站点所需的各种管理功能。
...
使用创作环境创建站 [点](sites-console.md)、修改站点属性 [、管](sites-console.md#modifying-site-properties)[](members.md)理社区成员和成员组时，必须访问在发布环境中注册的用户和用户组。

隧道服务使用作者上的复制代理提供此访问。

* 有关详细信息，请参 [阅部署页面上的](deploy-communities.md#tunnel-service-on-author) “配置说明”
...
>[!NOTE]
>
>如果站点访问者注册为一个开放社区站点的成员，则他们会自动成为同一发布环境中其他开放社区站点的成员。

### 发布组角色 {#publish-group-roles}

| 如果组成员…… | 主要角色 |
|---|---|
| 社区&lt;*站点名称*>成员 | 社区站点成员是注册用户。 他们可以登录、修改个人资料、加入一个开放的社区组、向社区发布内容、向其他成员发送消息以及关注网站活动。 |
| 社区&lt;*站点名称*>版主 | 社区站点审查方是可信的社区成员，可以在发布内容的页面上批量审核UGC，也可以使用审核控制台或上下文审核UGC。 |
...
| 无 | 发现网站的匿名网站访客可以查看和搜索允许匿名访问的社区站点。 要参与和发布内容，用户必须自行注册（如果允许）并成为社区成员。 |
...
* [日历功能](functions.md#calendar-function) -限制新事件的创建
* [论坛功能](functions.md#forum-function) -限制新主题的创建
* [问题与解答函数](functions.md#qna-function) -用于限制新问题的创建

当社区功能未得到保护（未分配特权成员组）时，所有社区站点成员都可以创建功能内容（文章、活动、主题、问题）。
...
在中创建成员后， `/home/users/community`这些成员将继承为成员的配置文件授予读取权限的正确ACL。
...
这会导致用户被自动分配到社区站点的成员组以及存储库位置 `/home/users/community` 和 `/home/groups/community`，以便他们继承相应的权限以查看彼此的配置文件。

* 值 `User auto membership` 应该是属性， `rep:authorizableId` 而不是配置 `givenName` 文件中的（显示名称）。
...
虽然用户组通过用户同步的同步将包含在将来的版本中，但当前只有用户组的*成员*会在用户同步运行时同步。

## 关于社区组 {#about-community-groups}

讨论组时，有两个不同的主题：

* **[社区组](overview.md#communitygroups)**社区组是在发布环境中为支持创建社区组的社区站点创建的子社区。 创建社区组会导致向网站添加更多页面，并以与其父社区站点类似的方式进行管理。 有关详细信息，请[访问Community Group Essentials](essentials-groups.md)for developers和[Community Group](creating-groups.md)for authors。

* **[成员组](../../help/sites-administering/security.md)**成员组是成员可能属于的组，并通过“组”控制台进行管理。 本页讨论的大部分内容都集中在成员组中。 为社区站点自动创建的成员组(前缀为&#x200B;*`Community`*)可称为社区组，因此必须考虑讨论的上下文。
