...
如果输入的PDF文档基于Acrobat表单或静态XFA表单，则Assembler服务可以组合非交互式PDF文档，而不将输出服务作为AEM表单安装的一部分。 但是，如果输入PDF文档是动态XFA表单，则输出服务必须是AEM表单安装的一部分。 如果组装动态XFA表单时，输出服务不是AEM表单安装的一部分，则会引发异常。 请参 [阅创建文档输出流](/help/forms/developing/creating-document-output-streams.md)。
