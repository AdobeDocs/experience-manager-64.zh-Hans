...
您可以使用Assembler Java和Web服务API组合PDFPortfolio。 一个包可以组合多种文档的不同类型，包括word文件、图像文件（例如jpeg文件）和PDF文档。 作品集的布局可以设置为不同的样式，如 *带预览的Grid*、 *在图像上* 或甚至 *Revolve*。
...
必须引用DDX文档才能组合PDFPortfolio。 此DDX文档必须包 `Portfolio`含 `Navigator` 元素和 `PackageFiles` 元素。
...
汇编PDFPortfolio时，将NAV文件（导航器文件）传递给Assembler服务。 传递给Assembler服务的NAV文件取决于要创建的PDFPortfolio类型。 例如，要创建“ *在图像上* ”布局，请传递AdobeOnImage.nav文件。 可以在以下文件夹中找到NAV文件：
...
要组合PDFPortfolio，您需要调用该操 `invokeDDX` 作。 Assembler服务在集合对象中返回PDFPortfolio。
...
1. 组合个人信息。
   ...
   * 包 `java.util.Map` 含构建PDFPortfolio所需文件的对象。
   * 指定 `com.adobe.livecycle.assembler.client.AssemblerOptionSpec` 运行时选项（包括默认字体和作业日志级别）的对象

   该方 `invokeDDX` 法返回一个 `com.adobe.livecycle.assembler.client.AssemblerResult` 对象，该对象包含已装配的PDFPortfolio和发生的任何异常。
...
1. 保存组合的包。
   ...
   * 调用对 `com.adobe.idp.Document` 象的方 `copyToFile` 法以提取PDFPortfolio。
...
1. 引用所需文档。
   ...
   * 创建对 `MyMapOf_xsd_string_To_xsd_anyType` 象。 此集合对象用于存储创建PDFPortfolio所需的输入文件。
