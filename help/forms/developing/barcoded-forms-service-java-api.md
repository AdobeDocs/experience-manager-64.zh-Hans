...
description: 使用Barcoded Forms服务，使用Java API快速开始对Barcoded表单数据进行解码。
seo-description: 使用Barcoded Forms服务，使用Java API快速开始对Barcoded表单数据进行解码。
...
以下Java代码对保存为Loan.pdf的PDF表单中的表单数据进行解码。 解码后的数据将保存为名为extratedData.xml的XML文件。 此代码示例将`org.w3c.dom.Document`对象转换为`com.adobe.idp.Document`对象。 （请参阅[解码条形码表单数据](/help/forms/developing/barcoded-forms.md#decoding-barcoded-form-data)。）
...
>[!NOTE]
>
>在同一应用程序逻辑中同时使用`org.w3c.dom.Document`对象和`com.adobe.idp.Document`对象时，最好完全限定这两个对象。
