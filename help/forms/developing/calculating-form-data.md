...
通常，作为PDF内容提交的表单包含在客户端执行的脚本。 但是，也可以执行服务器端计算。 “提交”按钮不能用于计算脚本。 在这种情况下，不执行计算，因为Forms服务认为交互是完成的。

为了说明表单设计脚本的用法，本节将检查一个简单的交互式表单，该表单包含一个配置为在服务器上运行的脚本。 下图显示了一个包含脚本的表单设计，该脚本将用户在前两个字段中输入的值相加，并在第三个字段中显示结果。
...
>[!NOTE]
>
>有关创建表单设计脚本的信息，请参阅 [表单设计器](https://www.adobe.com/go/learn_aemforms_designer_63)。
...
在验证与已提交表单关联的处理状态是否为 `1`后，您必须将结果写回客户端Web浏览器。 显示表单时，计算值将显示在相应的字段中。
...
1. 检索包含计算脚本的表单
   ...
   * 要检索包含计算脚本的表单数据，请使用对象的构造 `com.adobe.idp.Document` 函数并从构造函数中调用该对 `javax.servlet.http.HttpServletResponse` 象的 `getInputStream` 方法来创建对象。
   ...
   * 通过调用对象的方法，验证与已提交表单关 `1` 联的处理状 `FormsResult` 态是否处于 `getAction` 状态。 如果此方法返回值， `1`则执行计算并将数据写回客户端Web浏览器。
...
1. 检索包含计算脚本的表单
   ...
   * 调用对 `FormsServiceClient` 象的方 `processFormSubmission` 法并传递以下值：
      ...
      * 包 `BLOB` 含表单数据的对象。
      * 指定环境变量的字符串值包括所有相关的HTTP头。 例如，可以指定以下字符串值： `HTTP_REFERER=referrer&HTTP_CONNECTION=keep-alive&CONTENT_TYPE=application/xml`
      * 指定标题值的字 `HTTP_USER_AGENT` 符串值；例如， `Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1; .NET CLR 1.1.4322)`。
      ...
      * 由方 `FormsResultHolder` 法填充的空对象，其中包含提交的表单。
      该方 `processFormSubmission` 法使用表 `FormsResultHolder` 单提交的结果填充该参数。 该方 `processFormSubmission` 法返回一个 `FormsResult` 包含表单提交结果的对象。
