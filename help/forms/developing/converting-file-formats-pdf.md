...
* appmon。*[appname]*.additional.*[语言]*.xml
* appmon。*[appname].[语言]*.xml（请勿修改此文件。）
* appmon.global.*[语言]*.xml（请勿修改此文件。）
...
* adobe-livecycle-native-jboss-x86_win32.ear > adobe-Native2PDFSvc.war\WEB-INF\lib > adobe-native.jar > Native2PDFSvc-native.jar\bin > adobe-appmondata.jar\com\adobe\appmon。 adobe-livecycle-native-jboss-x86_win32.ear文件位于*的导出文件夹中[AEM forms安装目录]\*configurationManager。 (如果AEM Forms部署在另一个J2EE应用程序服务器上，请将adobe-livecycle-native-jboss-x86_win32.ear文件替换为与您的J2EE应用程序服务器相对应的EAR文件。)
* adobe-generatepdf-dsc.jar > adobe-appmondata.jar\com\adobe\appmon （adobe-appmondata.jar文件位于adobe-generatepdf-dsc.jar文件内）。 adobe-generatepdf-dsc.jar文件位于 *[AEM forms安装目录]*\部署文件夹。
...
脚本包含在本机应用程序的窗口元素中导航的说明，以及对这些元素提供特定响应的说明。 包含此信息的文件是appmon。*[appname]*.script.*[语言]*.xml. 示例为appmon.notepad.script.en_US.xml。
...
>[!NOTE]
>
>在此上下文中，“附加”一词表示应用程序的内容。[应用程序名称].addition.[语言].xml文件。 此类文件指定对话XML文件的覆盖和添加。
...
标识其他dialogXML文件的文件名为appmon。*[appname]*.addition.*[语言]*.xml. 例如appmon.excel.addition.en_US.xml。

其他对话框XML文件的名称必须使用appmon格式。*[应用程序名称]*.addition.*[语言]*.xml，其中 *应用程序名称* 必须与XML配置文件和脚本中使用的应用程序名称完全匹配。
...
创建一个环境变量，以指定本机应用程序可执行文件的位置。 变量必须使用 *[应用程序名称]*&#x200B;路径(_P)，其中 *应用程序名称* 必须完全匹配XML配置文件和脚本中使用的应用程序名称，其中路径包含可执行文件的双引号路径。 此类环境变量的示例为 `Photoshop_PATH`.
...
1. 在“新建系统变量”对话框中， **变量名称** 框中，键入使用格式的名称 *[应用程序名称]*&#x200B;路径(_P)。
