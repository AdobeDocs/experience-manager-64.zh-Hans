...
该脚本包含在本机应用程序的窗口元素之间导航的指令，这些指令为这些元素提供特定的响应。 包含此信息的文件为appmon。*[appname]*.script。*[locale]*.xml。一个示例是appmon.notepad.script.en_US.xml。
...
>[!NOTE]
>
>在这方面，“附加”一词是指应用程序的内容。[applicationname].addition.[locale].xml文件。这样的文件会指定对话框XML文件的覆盖和附加。
...
标识其他dialogXML文件的文件名为appmon。*[appname]*.addition。*[locale]*.xml。一个示例是appmon.excel.addition.en_US.xml。

附加对话框XML文件的名称必须使用格式appmon。*[applicationname]*.addition.*[locale]*.xml，其中 ** applicationname必须与XML配置文件和脚本中使用的应用程序名称完全匹配。
