...
1. 设置打印运行时选项
   ...
   >[!NOTE]
   >
   >如果生成的是ZPL打印流，则不能 `PrintedOutputOptionsSpec` 使用对象的 `setPagination` 方法设置分页值。 同样，您无法为ZPL打印流设置以下选项：OutputJog、PageOffset和Stapler。 该 `setPagination`*方法对PostScript生成无效。 它仅适用于PCL生成。*
