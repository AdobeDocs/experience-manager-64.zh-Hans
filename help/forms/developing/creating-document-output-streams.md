...
1. 设置PDF运行时选项。
   ...
   * 通过调用对象的方法来设 `PDFOutputOptionsSpec` 置文件URI `setFileURI` 选项。 传递一个字符串值，它指定输出服务生成的PDF文件的位置。 “文件URI”选项与承载AEM Forms的J2EE应用程序服务器（而非客户端计算机）相关。
...
1. 设置PDF运行时选项
   ...
   * 使用对 `PDFOutputOptionsSpec` 象的构造函数创建对象。
   * 通过指定一个字符串值来设置“文件URI”选项，该字符串值指定输出服务生成的PDF文件到对 `PDFOutputOptionsSpec` 象数据成员 `fileURI` 的位置。 “文件URI”选项与承载AEM Forms的J2EE应用程序服务器（而非客户端计算机）相关。
...
1. 设置渲染运行时选项。
   ...
   * 通过调 `PDFARevisionNumber` 用对象的方 `RenderOptionsSpec` 法并传递 `setPDFARevisionNumber` 来设置值 `PDFARevisionNumber.Revision_1`。
   ...
   >[!NOTE]
   >
   >PDF/A文档的PDF版本是1.4，无论您为对象的方法指 `RenderOptionsSpec` 定哪个 `setPdfVersion`*值。*
...
1. 生成PDF/A文档。
   ...
   * 包 `com.adobe.idp.Document` 含XML数据源的对象，该数据源包含要与表单设计合并的数据。
   ...
   >[!NOTE]
   >
   >对 `OutputResult` 象的方 `getRecordLevelMetaDataList` 法返回 `null`*. *
...
1. 检索操作结果。
   ...
   * 调用 `com.adobe.idp.Document` 对象的方 `copyToFile` 法，将对象的内容复制到文件 `com.adobe.idp.Document` 中(确保使用该方 `com.adobe.idp.Document` 法返回的 `getStatusDoc` 对象)。
   ...
   >[!NOTE]
   >
   >尽管“输出”服务将PDF/A文档写入由传递给对象方法的 `PDFOutputOptionsSpec` 参数指定的 `setFileURI` 位置，但您可以通过调用对象的*方法以编程方 `OutputResult` 式检索PDF/A `getGeneratedDoc`文档。*
...
1. 创建一个Output和Assembler客户端对象。
   ...
   * 为字段 `BasicHttpSecurityMode.TransportCredentialOnly` 指定常 `BasicHttpBindingSecurity.Security.Mode`数值。
   ...
   >[!NOTE]
   >
   >为*对象重复 `AssemblerServiceClient`这些步骤。 *
...
1. 设置打印到文件所需的打印运行时选项。
   ...
   * 通过调用PrintedOutputOptionsSpec对象的方法并传 `setFileURI` 递一个字符串值来指定文件，该字符串值表示文件的名称和位置。 例如，如果希望输出服务打印到位于C:\Adobe的名为* MortgageForm.ps*的PostScript文件，请指定C:\\Adobe\MortgageForm.ps。
...
1. 设置打印运行时选项
   ...
   >[!NOTE]
   >
   >如果生成的是ZPL打印流， `PrintedOutputOptionsSpec` 则不能使 `setPagination` 用对象的方法设置分页值。 同样，您无法为ZPL打印流设置以下选项： OutputJog、PageOffset和Stapler。 *方 `setPagination`法对PostScript生成无效。 它仅对PCL生成有效。 *
...
1. 设置PDF运行时选项
   ...
   * 通过调用对象的方法并传 `PDFOutputOptionsSpec` 递一个字符串 `setFileUri` 值来设置“文件URI”选项，该字符串值指定输出服务生成的文件的位置。 “文件URI”选项与承载AEM Forms的J2EE应用程序服务器（而非客户端计算机）相关。
...
1. 设置PDF运行时选项。
   ...
   * 通过指定一个字符串值来设置文件URI选项，该字符串值指定输出服务生成的文件到对象数 `OutputOptionsSpec` 据成员的 `fileURI` 位置。 “文件URI”选项与承载AEM Forms的J2EE应用程序服务器（而非客户端计算机）相关。
...
1. 设置PDF运行时选项。
   ...
   * 使用对 `PDFOutputOptionsSpec` 象的构造函数创建对象。
   * 指定输出服务通过调用对象的方法生成的PDF文件 `PDFOutputOptionsSpec` 的名称和位 `setFileURI` 置。 传递一个指定PDF文件位置的字符串值。 “文件URI”选项与承载AEM Forms的J2EE应用程序服务器（而非客户端计算机）相关。
...
1. 设置PDF运行时选项
   ...
   * 使用对 `PDFOutputOptionsSpec` 象的构造函数创建对象。
   * 通过指定一个字符串值来设置文件URI选项，该字符串值指定输出服务生成给对象数据成 `PDFOutputOptionsSpec` 员的PDF文件 `fileURI` 的位置。 “文件URI”选项与承载AEM Forms的J2EE应用程序服务器（而非客户端计算机）相关。
