...
>[!NOTE]
>
>“使用v进行编程”中的快速开始基于您正在使用其他操作系统（如Unix）的表单服务器，将特定于窗口的路径替换为适用操作系统支持的路径。 同样，如果您使用的是另一台J2EE应用程序服务器，请确保指定有效的连接属性。 请参 [阅设置连接属性](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)。
...
以下代码示例将名为Loan.xdp的交互式 *PDF表单渲染* 到客户端Web浏览器。 文件将附加到表单。 请注意，表单设计是应用程序的一部分，它使用内容根URI值进行引用 `repository:///`。 (请参阅 [渲染交互式PDF forms](/help/forms/developing/rendering-forms.md#rendering-interactive-pdf-forms)。)
...
以下代码示例使用Forms服务Java API在客户端呈现名为*Loan.xdp *的表单。 请注意，表单设计是应用程序的一部分，它使用内容根URI值进行引用 `repository:///`。 (请参 [阅在客户端渲染Forms](/help/forms/developing/rendering-forms.md#rendering-forms-at-the-client)。)
...
以下代码示例将名为TLALifeClaim.xdp的指 *南（已弃用）呈现* 到客户端Web浏览器。
...
下面的代码示例渲染基于片段的表单。 表单设计的名称为 *PurchaseOrderDynamic.xdp* ，它位于AEM Forms存储库中（XDP文件存储在存储库中名为FormsFolder的文件夹中）。 此外，POFragment表单引用的片段还必须位于存储库中。 (请参阅 [渲染基于片段的Forms](/help/forms/developing/rendering-forms.md#rendering-forms-based-on-fragments)。)
...
下面的代码示例将启用权限的表单呈现给客户端Web浏览器。 通过在此代码示例中设置的使用权限，用户可以在表单中添加注释并保存表单数据。 (请参 [阅渲染启用权限的Forms](/help/forms/developing/rendering-forms.md#rendering-rights-enabled-forms)。)
...
以下代码示例使用Forms服务Java API呈现HTML表单。 工具栏会添加到HTML表单以及两个文件附件。 此外，从对象获得用户代理 `HttpServletRequest` 值。 (请参阅 [将Forms渲染为HTML](/help/forms/developing/rendering-forms.md#rendering-forms-as-html)。)
...
以下代码示例使用Forms服务客户端API呈现HTML表单。 引用的自定义CSS文件的名称为 *custom.css*。 (请参 [阅使用自定义CSS文件渲染HTMLForms](/help/forms/developing/rendering-forms.md#rendering-html-forms-using-custom-css-files)。)
...
下面的代码示例呈现一个HTML表单，其中工具栏以法语显示。 fscmenu.xml的位置为C:\Adobe (此文件夹必须位于承载AEM Forms的服务器上)。 请注意，区域设置值为 `fr_FR`。 讨论如何使用自定义工具栏呈现HTML表单的部分显示了此快速开始中使用的fscmenu.xml文件的语法。 (请参 [阅使用自定义工具栏渲染HTMLForms](/help/forms/developing/rendering-forms.md#rendering-html-forms-with-custom-toolbars)。)
...
## 快速开始（SOAP模式）: 使用Java API处理作为XML提交的PDF forms {#quick-start-soap-mode-handling-pdf-forms-submitted-as-xml-using-the-java-api}

以下代码示例处理作为XML提交的表单。 传递给方法的内容类 `processFormSubmission` 型值是 `CONTENT_TYPE=text/xml`。 将显示与名为、和的 `mortgageAmount`字段 `lastName`对应 `firstName` 的值。 此快速开始中使 `getNodeText` 用名为的用户定义方法。 它接受一个 `org.w3c.dom.Document` 实例和一个指定节点名称的字符串值。 此方法返回一个表示节点值的字符串值。 (请参阅处 [理提交的Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms)。)
...
>[!NOTE]
>
>在同一应 `com.adobe.idp.Document` 用程序中 `org.w3c.dom.Document` 使用对象和对象时，完全限定 `org.w3c.dom.Document`。

## 快速开始（SOAP模式）: 使用Java API处理以PDF形式提交的PDF forms {#quick-start-soap-mode-handling-pdf-forms-submitted-as-pdf-using-the-java-api}

以下代码示例处理作为PDF数据提交的表单。 传递给方法的内容类 `processFormSubmission` 型值是 `CONTENT_TYPE=application/pdf`。 提交的表单将保存为名为tempPDF. *pdf的PDF文件*。 此外，由于表单是以PDF形式提交的，因此可以检索文件附件。 任何文件附件均保存为JPEG文件。 (请参阅处 [理提交的Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms)。)
...
以下代码示例处理作为XML数据提交的HTML表单。 传递给方法的内容类 `processFormSubmission` 型值 `CONTENT_TYPE=application/x-www-form-urlencoded`为。将显示与名为、和的字 `mortgageAmount`段对应 `lastName`的 `firstName` 值。 此快速开始中使 `getNodeText` 用名为的用户定义方法。 它接受一个 `org.w3c.dom.Document` 实例和一个指定节点名称的字符串值。 此方法返回一个表示节点值的字符串值。 (请参阅处 [理提交的Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms)。)
...
以下Java代码示例处理作为XML提交的表单数据。 表单数据是使用FormsAPI从表单提交中检索并发送到输出服务的。 表单数据和表单设计用于创建非交互式PDF文档。 非交互式PDF文档存储在名为的Content Services（已弃用）节点中 `/Company Home/Test Directory`。 表单的名称将动态创建。 即，用户的名和姓用于命名PDF文件。 将新内容的资源标识符写出到客户端Web浏览器。 (请参 [阅使用提交的XML数据创建PDF文档](/help/forms/developing/rendering-forms.md#creating-pdf-documents-with-submitted-xml-data)。)
...
## 快速开始（SOAP模式）: 使用Java API使用可流式布局预填充Forms {#quick-start-soap-mode-prepopulating-forms-with-flowable-layouts-using-the-java-api}
...
* `convertDataSource`: 将对象 `org.w3c.dom.Document` 转换为对 `com.adobe.idp.Document` 象。 此方法将对象 `org.w3c.dom.Document` 接受为输入参数并返回一个 `com.adobe.idp.Document` 对象。
* `renderPOForm`: 使用Forms服务Java API呈现动态采购订单表。 方 `com.adobe.idp.Document` 法返回的对象 `convertDataSource` 用于预填充表单。

   从Java servlet的方法中调用所有这些方 `doPost` 法。 (请参阅 [使用可流动布局预填充Forms](/help/forms/developing/rendering-forms.md#prepopulating-forms-with-flowable-layouts)。)
...
下面的代码示例处理一个包含计算脚本的表单并将结果写回客户端Web浏览器。 (请参 [阅计算表单数据](/help/forms/developing/rendering-forms.md#calculating-form-data)。)
...
下面的代码示例通过设置缓存、独立和线性化选项来优化性能。 线性化文件针对Web上的投放进行优化。 (请参 [阅优化Forms服务的性能](/help/forms/developing/rendering-forms.md#optimizing-the-performance-of-the-forms-service)。)
...
以下Java快速开始渲染交互式PDF表单，该表单基于名为Loan.xdp *的表单设计* （按值）。 请注意，表单设计用于填充名 `com.adobe.idp.Document` 为* inputXDP*的对象。 (请参 [阅按值呈现Forms](/help/forms/developing/rendering-forms.md#rendering-forms-by-value)。)
...
以下Java快速开始从Content Services检索文件Loan.xdp（已弃用）。 此XDP文件位于空间中 `/Company Home/Form Designs`。 在实例中返回XDP文 `com.adobe.idp.Document` 件。 实 `com.adobe.idp.Document` 例传给Forms。 交互式表单被写入客户端Web浏览器。 (请参 [阅将文档传递给Forms服务](/help/forms/developing/passing-documents-forms-service.md)。)
