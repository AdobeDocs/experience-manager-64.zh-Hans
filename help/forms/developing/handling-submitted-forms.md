...
表单数据以XML或PDF数据的形式提交到Forms服务，XML或PDF数据是在Designer中设置的选项。 作为XML提交的表单使您能够提取单个字段数据值。 即，您可以提取用户在表单中输入的每个表单字段的值。 作为PDF数据提交的表单是二进制数据，而不是XML数据。 您可以将表单另存为PDF文件，或将表单发送到其他服务。 如果要从以XML形式提交的表单中提取数据，然后使用表单数据创建PDF文档，请调用另一个AEM Forms操作。 (请参 [阅使用提交的XML数据创建PDF文档](/help/forms/developing/creating-pdf-documents-submitted-xml.md))

下图显示了从Web浏览器中显示的交互式表 `HandleData` 单中命名的Java Servlet提交的数据。
...
<table>
 ...
 <tbody>
  ...
  <tr>
   <td><p>2</p></td>
   <td><p>数据作为XML数 <code>HandleData</code> 据提交到Java Servlet。</p></td>
  </tr>
  <tr>
   <td><p>3</p></td>
...
如果表单数据以URL UTF-16数据提交，客户端计算机需要Adobe Reader或Acrobat 8.1或更高版本。 此外，如果表单设计包含一个具有URL编码数据(HTTP Post)的提交按钮，且数据编码选项为UTF-16，则必须在文本编辑器（如记事本）中修改表单设计。 您可以将编码选项设置为 `UTF-16LE` 提交 `UTF-16BE` 按钮或提交按钮。 设计人员不提供此功能。
...
要检索提交的表单数据，请调 `FormsServiceClient` 用对象的方 `processFormSubmission` 法。 调用此方法时，必须指定已提交表单的内容类型。 当数据从客户端Web浏览器提交到Forms服务时，它可以作为XML或PDF数据提交。 要检索输入到表单字段中的数据，可以将数据作为XML数据提交。

您还可以通过设置以下运行时选项从作为PDF数据提交的表单检索表单字段：

* 将以下值作为内容 `processFormSubmission` 类型参数传递给方法： `CONTENT_TYPE=application/pdf`.
* 将对 `RenderOptionsSpec` 象的值 `PDFToXDP` 设置为 `true`
* 将对 `RenderOptionsSpec` 象的值 `ExportDataFormat` 设置为 `XMLData`

调用方法时，可指定已提交表单的内容 `processFormSubmission` 类型。 以下列表指定适用的内容类型值：
...
* **0（提交）:** 已提交的数据已准备好进行处理。
* **1（计算）:** Forms服务对数据执行计算操作，结果必须呈现给用户。
...
>[!NOTE]
>
>必须将计算和验证返回给用户。 (请参 [阅计算表单数据](/help/forms/developing/calculating-form-data.md#calculating-form-data)。)
...
1. 检索表单数据
   ...
   * 要检索发布到Java Servlet的表单数据，请使 `com.adobe.idp.Document` 用对象的构造函数并从构造函数中调 `javax.servlet.http.HttpServletResponse` 用该对 `getInputStream` 象的方法来创建对象。
   * 使用对 `RenderOptionsSpec` 象的构造函数创建对象。 通过调用对象的方法并 `RenderOptionsSpec` 传递指定 `setLocale` 区域设置值的字符串值来设置区域设置值。

   >[!NOTE]
   >
   >您可以通过调用对象的方法、传递以及调用和传递，指示Forms服务从已提交 `RenderOptionsSpec` 的PDF内容 `setPDF2XDP` 创建XDP `true` 或XML `setXMLData` 数据 `true`。 然后，可以调 `FormsResult` 用对象的 `getOutputXML` 方法来检索与XDP/XML数据对应的XML数据。 (对 `FormsResult` 象由*方 `processFormSubmission`法返回，将在下一个子步骤中说明。)*
...
1. 确定表单提交是否包含文件附件
   ...
   * 调用 `FormsResult` 对象的方 `getAttachments` 法。 此方法返回 `java.util.List` 一个对象，其中包含随表单提交的文件。
   * 对对象进 `java.util.List` 行迭代以确定是否有文件附件。 如果存在文件附件，则每个元素都是一个 `com.adobe.idp.Document` 实例。 可以通过调用对象的方法并 `com.adobe.idp.Document` 传递对象 `copyToFile` 来保存文件附 `java.io.File` 件。
...
1. 处理提交的数据
   ...
   * 如果数据内容类型为或， `application/vnd.adobe.xdp+xml` 请创 `text/xml`建应用程序逻辑以检索XML数据值。
   ...
   * 如果数据内容类型为， `application/pdf`则创建应用程序逻辑以将提交的PDF数据另存为PDF文件。

      * 通过 `com.adobe.idp.Document` 调用对象的 `FormsResult` 方法创建 `getOutputContent` 对象。
      * 使用其 `java.io.File` 公共构造函数创建对象。 请务必指定PDF作为文件扩展名。
      * 通过调用对象的方法 `com.adobe.idp.Document` 并传递对 `copyToFile` 象来填充PDF `java.io.File` 文件。
...
1. 检索表单数据
   ...
   * 通过 `java.io.InputStream` 调用对象的 `javax.servlet.http.HttpServletResponse` 方法创建 `getInputStream` 对象。
   ...
   * 调用对 `FormsService` 象的方 `processFormSubmission` 法并传递以下值：
      ...
      * 由方 `FormsResultHolder` 法填充的空对象，其中包含提交的表单。
      ...
      该方 `processFormSubmission` 法使用 `FormsResultHolder` 表单提交的结果填充参数。

   * 确定Forms服务是否已通过调用对象的方法处理 `FormsResult` 表单数 `getAction` 据。 如果此方法返回 `0`值，表单数据即可处理。 您可以通过 `FormsResult` 获取对象数据成员 `FormsResultHolder` 的值来获 `value` 取对象。


1. 确定表单提交是否包含文件附件

   获取对象数 `MyArrayOf_xsd_anyTypeHolder` 据成 `value` 员的值( `MyArrayOf_xsd_anyTypeHolder` 对象已传递到方 `processFormSubmission` 法)。 此数据成员返回一组 `Objects`。 数组中的每 `Object` 个元素 `Object`都与随表单一起提交的文件相对应。 您可以获取数组中的每个元素并将其转换为 `BLOB` 对象。

1. 处理提交的数据

   * 如果数据内容类型为或， `application/vnd.adobe.xdp+xml` 请创 `text/xml`建应用程序逻辑以检索XML数据值。

      * 通过 `BLOB` 调用对象的 `FormsResult` 方法创建 `getOutputContent` 对象。
      * 通过调用对象的方法 `BLOB` 创建字节 `getBinaryData` 数组。
      * 通过调 `java.io.InputStream` 用构造函数并传 `java.io.ByteArrayInputStream` 递字节数组来创建对象。
      * 通过调 `org.w3c.dom.DocumentBuilderFactory` 用静态对象的方 `org.w3c.dom.DocumentBuilderFactory` 法创建 `newInstance` 对象。
      * 通过调 `org.w3c.dom.DocumentBuilder` 用对象的方 `org.w3c.dom.DocumentBuilderFactory` 法创建 `newDocumentBuilder` 对象。
      * 通过调 `org.w3c.dom.Document` 用对象的方 `org.w3c.dom.DocumentBuilder` 法并传递 `parse` 对象来创建 `java.io.InputStream` 对象。
      * 检索XML文档中每个节点的值。 实现此任务的一种方法是创建接受两个参数的自定义方法： 要 `org.w3c.dom.Document` 检索其值的对象和节点的名称。 此方法返回表示节点值的字符串值。 在此过程后面的代码示例中，调用此自定义方法 `getNodeText`。 本文给出了该方法的主体。
