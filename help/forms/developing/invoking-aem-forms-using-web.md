...
以下C#代码示例对由Forms服务呈现的交互式表单进行签名。 客户端应用程序有两个服务引用。 与 `BLOB` Forms服务关联的实例属于命名空间 `SignInteractiveForm.ServiceReference2` 。 同样，与 `BLOB` 签名服务关联的实例也属于命名空 `SignInteractiveForm.ServiceReference1` 间。 签名的交互式表单将保存为名为 *LoanXFASpided.pdf的PDF文件*。
...
使用Microsoft .Net 3.5和WCF时，某些AEM Forms生成的代理类的名称不正确。 当为IBMFilenetContentRepositoryConnector、IDPSdechulerService或名称以字母I开头的任何其他服务创建代理类时，会发生此问题。例如，对于IBMFileNetContentRepositoryConnector，生成的客户端的名称是 `BMFileNetContentRepositoryConnectorClient`。 生成的代理类中缺少字母I。
