...
Java API(SOAP)快速入门适用于流程。 A *过程实例* 是由调用方法（如调用API）或从工作区中启动的特定进程的发生情况。
...
以下Java代码从名为的流程中清除数据 *SecureDocument*. 过滤器用于指定清除那些进程实例(其中进程变量名为 *inValue* 大于200。
