...
description: 使用输出服务Java API快速启动(SOAP)创建PDF文档、基于应用程序XDP文件创建PDF文档、创建PDF/A文档、将文档传递到输出服务、将位于AEM Forms存储库中的文档传递到输出服务、创建基于片段的PDF文档、打印到文件、将打印流发送到网络打印机、创建多个PDF文件、创建搜索规则、转换PDF文档。
...
以下Java代码示例将创建一个名为的PDF文档 *Loan.pdf*. 此PDF文档基于名为的表单设计 *Loan.xdp* 和名为 *Loan.xml*. 的 *Loan.pdf* 写入到C:\Adobe folder located on the J2EE application server hosting AEM Forms，而不是客户端计算机。 (请参阅 [创建PDF文档](/help/forms/developing/creating-document-output-streams.md#creating-pdf-documents).)
...
## 快速入门（SOAP模式）：使用Java API根据应用程序XDP文件创建PDF文档 {#quick-start-soap-mode-creating-a-pdf-document-based-on-an-application-xdp-file-using-the-java-api}

以下Java代码示例将创建一个名为的PDF文档 *Loan.pdf*. 此PDF文档基于名为的表单设计 *Loan.xdp* 和名为 *Loan.xml*. XDP文件将作为名为的AEM Forms应用程序的一部分部署 `Applications/FormsApplication`. 请注意，URI路径为 `repository:///Applications/FormsApplication/1.0/FormsFolder/`. 的 *Loan.pdf* 写入到C:\Adobe folder located on the J2EE application server hosting AEM Forms，而不是客户端计算机。 (请参阅 [创建PDF文档](/help/forms/developing/creating-document-output-streams.md#creating-pdf-documents).)
...
## 快速入门（SOAP模式）：使用Java API将位于存储库中的文档传递到输出服务 {#quick-start-soap-mode-passing-a-document-located-in-the-repository-to-the-output-service-using-the-java-api}

以下Java代码从存储库中检索XDP文件，并将其传递到 `com.adobe.idp.Document` 实例。 XDP文件将作为名为的AEM Forms应用程序的一部分部署 `Applications/FormsApplication`. 请注意，URI路径为 `repository:///Applications/FormsApplication/1.0/FormsFolder/`.

>[!NOTE]
>
>存储库API用于从此位置检索XDP文件。 (请参阅 [读取资源](/help/forms/developing/aem-forms-repository.md#reading-resources).)

另请注意内容根值 `repository:///Applications/FormsApplication/1.0/FormsFolder/` 被传递到 `OutputClient` 对象 `generatePDFOutput2` 方法（第二个参数）。 此值将传递到输出服务，以通知输出服务表单辅助资料（如图像）存储在此位置。

>[!NOTE]
>
>在调用 `generatePrintedOutput2` 方法。

的 *Loan.pdf* 将写入C:\Adobe folder located on the J2EE application server hosting AEM Forms。 (请参阅 [将存储库中的文档传递到输出服务](/help/forms/developing/creating-document-output-streams.md#passing-documents-located-in-the-repository-to-the-output-service).)
...
以下Java代码示例将创建一个名为的PDF/A文档 *LoanArchive.pdf*. 此PDF文档基于名为的表单设计 *Loan.xdp* 和名为 *Loan.xml*. 的 *LoanArchive.pdf* 写入到C:\Adobe folder located on the J2EE application server hosting AEM Forms，而不是客户端计算机。 (请参阅 [创建PDF/A文档](/help/forms/developing/creating-document-output-streams.md#creating-pdf-a-documents).)
...
以下Java快速入门可检索文件 *Loan.xdp* 从内容服务。 此XDP文件位于 `space /Company Home/Form Designs`. 在 `com.adobe.idp.Document` 实例。 的 `com.adobe.idp.Document` 实例会传递到输出服务。 非交互式表单在客户端计算机上另存为名为*Loan.pdf *的PDF文件。 由于已设置“文件URI”选项，因此PDF文件*Loan.pdf *也会保存在托管AEM Forms的J2EE应用程序服务器上。 (请参阅 [将位于Content Services ES2中的文档传递到输出服务](/help/forms/developing/creating-document-output-streams.md#passing-documents-located-in-content-services-deprecated-to-the-output-service).)
...
以下Java代码示例创建了一个基于由汇编程序服务组装的表单设计的PDF文档。 汇编程序服务将位于多个XDP文件中的片段组合为单个表单设计。 调用汇编程序服务的应用程序逻辑位于名为的用户定义的方法中 `GetFormDesign`. 非交互式表单在客户端计算机上另存为名为*Loan.pdf *的PDF文件。 (请参阅 [使用片段创建PDF文档](/help/forms/developing/creating-document-output-streams.md#creating-pdf-documents-using-fragments).)
...
## 快速入门（SOAP模式）：使用Java API向网络打印机发送打印流 {#quick-start-soap-mode-sending-a-print-stream-to-a-network-printer-using-the-java-api}

以下Java代码示例将PostScript打印流发送到名为 *\\打印机1\打印机*. 将两份副本发送到打印机。 (请参阅 [将打印流发送到打印机](/help/forms/developing/creating-document-output-streams.md#sending-print-streams-to-printers).)
...
以下Java代码会为位于名为的XML数据文件中的每个数据记录创建多个PDF文件 *Loan_data_batch.xml*. 文件将写入C:\Adobe directory目录。 PDF文件将写入C:\Adobe folder located on the J2EE application server hosting AEM Forms文件，而不是客户端计算机。 (请参阅 [创建多个输出文件](/help/forms/developing/creating-document-output-streams.md#creating-multiple-output-files).)
...
以下Java代码示例创建了输出服务搜索的两个文本模式。 第一种文本模式是Mortgage。 如果找到，输出服务将使用名为的表单设计 *Mortgage.xdp*. 第二种文本模式是“汽车”。 如果找到，输出服务将使用名为的表单设计 *AutomilyLoan.xdp*. 如果两个文本模式均未找到，输出服务将使用名为* Loan.xdp的默认表单设计。 *(请参阅 [创建搜索规则](/help/forms/developing/creating-document-output-streams.md#creating-search-rules).)
...
以下Java代码示例将转换名为的交互式PDF文档 *Loan.pdf* 到名为的非交互式PDF文档 *NonInteractiveLoan.pdf*. (请参阅 [拼合PDF文档](/help/forms/developing/creating-document-output-streams.md#flattening-pdf-documents).)
