...
* 任务管理器
   ...
   您可以将TaskManager端点组织到名为 *类别*. 然后，这些类别会通过TaskManager公开到工作区，最终用户在分类时会看到TaskManager端点。 在工作区中，最终用户会在导航窗格中看到这些类别。 每个类别中的端点在工作区的“开始进程”页面上显示为进程卡。
...
必须先创建EJB端点，然后才能以编程方式添加EJB端点 `EndpointRegistryClient` 对象。
...
1. 创建EndpointRegistry客户端对象。
   ...
   * 创建 `EndpointRegistryClient` 对象，并使用其构造函数进行传递 `ServiceClientFactory` 对象。

1. 设置EJB端点属性。

   * 创建 `CreateEndpointInfo` 对象。
   * 通过调用 `CreateEndpointInfo` 对象 `setConnectorId` 方法和传递字符串值 `EJB`.
   * 通过调用 `CreateEndpointInfo` 对象 `setDescription` 方法和传递描述端点的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setName` 方法和传递指定名称的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setServiceId` 方法和传递指定服务名称的字符串值。
   * 指定通过调用 `CreateEndpointInfo` 对象 `setOperationName` 方法，并传递指定操作名称的字符串值。 对于SOAP和EJB端点，指定通配符( `*`)，表示所有操作。

1. 创建EJB端点。

   通过调用 `EndpointRegistryClient` 对象 `createEndpoint` 方法和通过 `CreateEndpointInfo` 对象。 此方法将返回 `Endpoint` 表示新EJB端点的对象。

1. 启用端点。

   通过调用 `EndpointRegistryClient` 对象的启用方法和传递 `Endpoint` 由返回的对象 `createEndpoint` 方法。
...
要以编程方式向服务添加SOAP端点，您必须创建 `EndpointRegistryClient` 对象。
...
1. 设置SOAP端点属性。
   ...
   * 创建 `CreateEndpointInfo` 对象。
   * 通过调用 `CreateEndpointInfo` 对象 `setConnectorId` 方法和传递字符串值 `SOAP`.
   * 通过调用 `CreateEndpointInfo` 对象 `setDescription` 方法和传递描述端点的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setName` 方法和传递指定名称的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setServiceId` 方法和传递指定服务名称的字符串值。
   * 指定通过调用 `CreateEndpointInfo` 对象 `setOperationName` 方法和传递指定操作名称的字符串值。 对于SOAP和EJB端点，指定通配符( `*`)，表示所有操作。

1. 创建SOAP端点。

   通过调用 `EndpointRegistryClient` 对象 `createEndpoint` 方法和通过 `CreateEndpointInfo` 对象。 此方法将返回 `Endpoint` 表示新SOAP端点的对象。

1. 启用端点。

   通过调用 `EndpointRegistryClient` 对象的启用方法并传递 `Endpoint` 由返回的对象 `createEndpoint` 方法。
...
为了以编程方式向服务添加已监视文件夹端点，请考虑以下名为的短暂进程 *EncryptDocument*. (请参阅 [了解AEM Forms流程](/help/forms/developing/aem-forms-processes.md#understanding-aem-forms-processes).)

![aw_aw_encryptdocumentprocess](assets/aw_aw_encryptdocumentprocess.png)

此过程接受不安全的PDF文档作为输入值，然后将不安全的PDF文档传递到加密服务的 `EncryptPDFUsingPassword` 操作。 PDF文档使用密码进行加密，密码加密的PDF文档是此过程的输出值。 输入值的名称(不安全的PDF文档)为 `InDoc` 数据类型为 `com.adobe.idp.Document`. 输出值(密码加密的PDF文档)的名称为 `SecuredDoc` 数据类型为 `com.adobe.idp.Document`.
...
要以编程方式添加监视文件夹端点，您必须创建 `EndpointRegistryClient` 对象。
...
* **waitTime**:在创建文件夹或文件后扫描之前等待的时间（以毫秒为单位）。 例如，如果等待时间为36,000,000毫秒（一小时），而文件是在一分钟前创建的，则此文件将在59分钟或更久之后被提取。 此属性有助于确保将文件或文件夹完全复制到输入文件夹。 例如，如果要处理大文件，并且文件需要10分钟才能下载，请将等待时间设置为10&amp;ast;60 &amp;ast;1000毫秒。 如果已观看文件夹等待了10分钟，则此设置会阻止其扫描文件。 默认值为 0。
* **excludeFilePattern**:已监视文件夹用来确定要扫描和选取的文件和文件夹的模式。 将不会扫描任何具有此模式的文件或文件夹以进行处理。 当输入的文件夹包含多个文件时，此设置非常有用。 文件夹的内容可以复制到一个文件夹中，该文件夹的名称将由监视文件夹选取。 此步骤可阻止监视文件夹在文件夹完全复制到输入文件夹之前提取待处理的文件夹。 例如，如果excludeFilePattern值为 `data*`，所有与 `data*` 没有被接收。 其中包括名为 `data1`, `data2`，等等。 此外，可以用通配符模式来补充该模式以指定文件模式。 监视文件夹修改正则表达式以支持通配符模式，例如 `*.*` 和 `*.pdf`. 正则表达式不支持这些通配符模式。
* **includeFilePattern**:监视文件夹用来确定要扫描和选取的文件夹和文件的模式。 例如，如果此值为 `*`，所有与 `input*` 被接走。 其中包括名为 `input1`, `input2`，等等。 默认值为 `*`。此值指示所有文件和文件夹。 此外，可以用通配符模式来补充该模式以指定文件模式。 监视文件夹修改正则表达式以支持通配符模式，例如 `*.*` 和 `*.pdf`. 正则表达式不支持这些通配符模式。 此值是必填项。
...
创建监视文件夹端点时，必须定义输入参数值。 也就是说，您必须描述传递到监视文件夹所调用操作的输入值。 例如，请考虑本主题中介绍的流程。 它有一个名为 `InDoc` 其数据类型为 `com.adobe.idp.Document`. 为此进程创建“监视文件夹”端点（在激活某个进程后，该进程将成为服务）时，必须定义输入参数值。

要定义监视文件夹端点所需的输入参数值，请指定以下值：

**输入参数名称**:输入参数的名称。 在Workbench中为流程指定输入值的名称。 如果输入值属于服务操作（不是在Workbench中创建的进程的服务），则输入名称在component.xml文件中指定。 例如，本节介绍的流程的输入参数名称为 `InDoc`.
...
创建监视文件夹端点时，必须定义输出参数值。 也就是说，您必须描述由监视文件夹端点调用的服务返回的输出值。 例如，请考虑本主题中介绍的流程。 其输出值名为 `SecuredDoc` 其数据类型为 `com.adobe.idp.Document`. 为此进程创建“监视文件夹”端点时（在激活某个进程后，该进程将成为服务），必须定义输出参数值。
...
* 如果服务返回列表，则模式为 `Result\%F\`，且源目标为Result\sourcefilename\source1(output 1)和Result\sourcefilename\source2(output 2)。
* 如果服务返回映射，则模式为 `Result\%F\`，且源目标为Result\sourcefilename\file1和Result\sourcefilename\file2。 如果映射包含多个对象，则模式为 `Result\%F.pdf` 源目标为Result\sourcefilename1.pdf(output 1)、Result\sourcefilenam2.pdf(output 2)，等等。

**数据类型**:指定返回值的数据类型。 例如，本节介绍的流程返回值的数据类型为 `com.adobe.idp.Document`.
...
1. 设置监视文件夹端点属性。
   ...
   * 创建 `CreateEndpointInfo` 对象。
   * 通过调用 `CreateEndpointInfo` 对象 `setConnectorId` 方法和传递字符串值 `WatchedFolder`.
   * 通过调用 `CreateEndpointInfo` 对象 `setDescription` 方法和传递描述端点的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setName` 方法和传递指定名称的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setServiceId` 方法和传递指定服务名称的字符串值。
   * 指定通过调用 `CreateEndpointInfo` 对象 `setOperationName` 方法和传递指定操作名称的字符串值。 通常，在为源自在Workbench中创建的进程的服务创建“监视文件夹”端点时，会调用操作的名称。

1. 指定配置值。

   对于要为监视文件夹端点设置的每个配置值，必须调用 `CreateEndpointInfo` 对象 `setConfigParameterAsText` 方法。 例如，要设置 `url` 配置值，调用 `CreateEndpointInfo` 对象 `setConfigParameterAsText` 方法并传递以下字符串值：

   * 一个字符串值，用于指定配置值的名称。 设置 `url` 配置值，指定 `url`.
   * 一个字符串值，用于指定配置值的值。 设置 `url` 配置值，指定监视的文件夹位置。

   >[!NOTE]
   >
   >要查看为EncryptDocument服务设置的所有配置值，请参阅位于 [快速入门：使用Java API添加监视文件夹端点](/help/forms/developing/endpoint-registry-java-api-quick.md#quickstart-adding-a-watched-folder-endpoint-using-the-java-api).

1. 定义输入参数值。

   通过调用 `CreateEndpointInfo` 对象 `setInputParameterMapping` 方法并传递以下值：

   * 一个字符串值，用于指定输入参数的名称。 例如，EncryptDocument服务的输入参数的名称为 `InDoc`.
   * 一个字符串值，用于指定输入参数的数据类型。 例如， `InDoc` 输入参数为 `com.adobe.idp.Document`.
   * 指定映射类型的字符串值。 例如，您可以指定 `variable`.
   * 指定映射类型值的字符串值。 例如，您可以指定&amp;ast;.pdf作为文件模式。

   >[!NOTE]
   >
   >调用 `setInputParameterMapping` 方法来定义每个输入参数值。 由于EncryptDocument进程只有一个输入参数，因此需要调用此方法一次。

1. 定义输出参数值。

   通过调用 `CreateEndpointInfo` 对象 `setOutputParameterMapping` 方法并传递以下值：

   * 一个字符串值，用于指定输出参数的名称。 例如，EncryptDocument服务的输出参数的名称为 `SecuredDoc`.
   * 一个字符串值，用于指定输出参数的数据类型。 例如， `SecuredDoc` 输出参数为 `com.adobe.idp.Document`.
   * 指定映射类型的字符串值。 例如，您可以指定 `%F.pdf`.

1. 创建监视文件夹端点。

   通过调用 `EndpointRegistryClient` 对象 `createEndpoint` 方法和通过 `CreateEndpointInfo` 对象。 此方法将返回 `Endpoint` 表示“监视文件夹”端点的对象。

1. 启用端点。

   通过调用 `EndpointRegistryClient` 对象 `enable` 方法和通过 `Endpoint` 由返回的对象 `createEndpoint` 方法。
...
的 [快速入门：使用Java API添加监视文件夹端点](/help/forms/developing/endpoint-registry-java-api-quick.md#quickstart-adding-a-watched-folder-endpoint-using-the-java-api) 使用必须包含在Java项目中的常量文件来编译快速入门。 此常量文件表示在添加监视文件夹端点时必须设置的配置值。 以下Java代码表示常量文件。
...
为了以编程方式向服务添加电子邮件端点，请考虑以下名为 *MyApplication\EncryptDocument*. 有关短时间流程的信息，请参阅 [了解AEM Forms流程](/help/forms/developing/aem-forms-processes.md#understanding-aem-forms-processes).
...
您必须先创建一个 `EndpointRegistryClient` 对象。
...
* **domainPattern**:指定提供商接受的传入电子邮件的域模式。 例如，如果 `adobe.com` 使用时，只会处理adobe.com的电子邮件，而忽略其他域的电子邮件。
* **filePattern**:指定提供程序接受的传入文件附件模式。 这包括具有特定文件扩展名(&amp;ast;.dat、&amp;ast;.xml)的文件、具有特定名称（数据）的文件，以及具有名称和扩展名(&amp;ast;)的复合表达式的文件。[dD][aA][Tt])。 默认值为 `*`。
* **recipientSuccessfulJob**:将邮件发送到的电子邮件地址，用于指示作业成功。 默认情况下，成功的作业消息始终会发送给发件人。 如果您键入 `sender`，则会向发件人发送电子邮件结果。 最多支持100个收件人。 使用电子邮件地址指定其他收件人，每个收件人以逗号分隔。 要关闭此选项，请将此值留空。 在某些情况下，您可能希望触发某个流程，并且不希望收到有关结果的电子邮件通知。 默认值为 `sender`。
* **recipientFailedJob**:发送消息以指示作业失败的电子邮件地址。 默认情况下，失败的作业消息始终会发送给发件人。 如果您键入 `sender`，则会向发件人发送电子邮件结果。 最多支持100个收件人。 使用电子邮件地址指定其他收件人，每个收件人以逗号分隔。 要关闭此选项，请将此值留空。 默认值为 `sender`。
...
* **异步**:设置为同步时，将处理所有输入文档并返回单个响应。 设置为异步时，将为处理的每个输入文档发送响应。 例如，将为本主题中引入的流程创建电子邮件端点，并向端点的收件箱发送一封电子邮件，其中包含多个不安全的PDF文档。 当所有PDF文档都使用密码进行加密，并且如果端点配置为同步，则会发送单个响应电子邮件消息，并附加所有安全PDF文档。 如果端点配置为异步，则会为每个安全PDF文档发送单独的响应电子邮件消息。 每封电子邮件都包含一个PDF文档作为附件。 默认值为异步。

**定义输入参数值**

创建电子邮件端点时，必须定义输入参数值。 也就是说，您必须描述传递到电子邮件端点所调用操作的输入值。 例如，请考虑本主题中介绍的流程。 它有一个名为 `InDoc` 其数据类型为 `com.adobe.idp.Document`. 为此进程创建电子邮件端点（在激活某个进程后，该进程将变为服务）时，必须定义输入参数值。

要定义电子邮件端点所需的输入参数值，请指定以下值：

**输入参数名称**:输入参数的名称。 在Workbench中为流程指定输入值的名称。 如果输入值属于服务操作(不是在Workbench中创建的进程的Forms服务)，则输入名称在component.xml文件中指定。 例如，本节介绍的流程的输入参数名称为 `InDoc`.
...
创建电子邮件端点时，必须定义输出参数值。 也就是说，您必须描述由电子邮件端点调用的服务返回的输出值。 例如，请考虑本主题中介绍的流程。 其输出值名为 `SecuredDoc` 其数据类型为 `com.adobe.idp.Document`. 为此进程创建电子邮件端点（在激活某个进程后，该进程将变为服务）时，必须定义输出参数值。
...
1. 设置电子邮件端点属性。
   ...
   * 创建 `CreateEndpointInfo` 对象。
   * 通过调用 `CreateEndpointInfo` 对象 `setConnectorId` 方法和传递字符串值 `Email`.
   * 通过调用 `CreateEndpointInfo` 对象 `setDescription` 方法和传递描述端点的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setName` 方法和传递指定名称的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setServiceId` 方法和传递指定服务名称的字符串值。
   * 指定通过调用 `CreateEndpointInfo` 对象 `setOperationName` 方法和传递指定操作名称的字符串值。 通常，在为源自在Workbench中创建的进程的服务创建电子邮件端点时，会调用操作的名称。

1. 指定配置值。

   对于要为电子邮件端点设置的每个配置值，必须调用 `CreateEndpointInfo` 对象 `setConfigParameterAsText` 方法。 例如，要设置 `smtpHost` 配置值，调用 `CreateEndpointInfo` 对象 `setConfigParameterAsText` 方法并传递以下值：

   * 一个字符串值，用于指定配置值的名称。 设置 `smtpHost` 配置值，指定 `smtpHost`.
   * 一个字符串值，用于指定配置值的值。 设置 `smtpHost` 配置值，请指定指定SMTP服务器名称的字符串值。

   >[!NOTE]
   >
   >要查看本节中介绍的EncryptDocument服务的所有配置值集，请参阅位于 [快速入门：使用Java API添加电子邮件端点](/help/forms/developing/endpoint-registry-java-api-quick.md#quickstart-adding-an-email-endpoint-using-the-java-api).
...
1. 定义输出参数值。
   ...
   通过调用 `CreateEndpointInfo` 对象 `setOutputParameterMapping` 方法和传递以下值：
...
1. 创建电子邮件端点。
   ...
   通过调用 `EndpointRegistryClient` 对象 `createEndpoint` 方法和通过 `CreateEndpointInfo` 对象。 此方法将返回 `Endpoint` 表示电子邮件端点的对象。
...
的 [快速入门：使用Java API添加电子邮件端点](/help/forms/developing/endpoint-registry-java-api-quick.md#quickstart-adding-an-email-endpoint-using-the-java-api) 使用必须包含在Java项目中的常量文件来编译快速入门。 此常量文件表示添加电子邮件端点时必须设置的配置值。 以下Java代码表示常量文件。
...
您可以使用AEM Forms Java API以编程方式向服务添加远程处理端点。 通过添加远程处理端点，您使Flex应用程序能够使用远程处理来调用服务。 (请参阅 [使用调用AEM Forms(AEM表单已弃用)AEM Forms远程处理](/help/forms/developing/invoking-aem-forms-using-remoting.md#invoking-aem-forms-using-remoting).)

为了以编程方式向服务添加远程处理端点，请考虑以下名为 *EncryptDocument*.
...
要以编程方式添加远程处理端点，必须创建 `EndpointRegistryClient` 对象。
...
1. 设置远程处理端点属性。
   ...
   * 创建 `CreateEndpointInfo` 对象。
   * 通过调用 `CreateEndpointInfo` 对象 `setConnectorId` 方法和传递字符串值 `Remoting`.
   * 通过调用 `CreateEndpointInfo` 对象 `setDescription` 方法和传递描述端点的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setName` 方法和传递指定名称的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setServiceId` 方法和传递指定服务名称的字符串值。
   * 指定由 `CreateEndpointInfo` 对象 `setOperationName` 方法和传递指定操作名称的字符串值。 对于远程处理端点，指定通配符(&amp;ast;)。

1. 创建远程处理端点。

   通过调用 `EndpointRegistryClient` 对象 `createEndpoint` 方法和通过 `CreateEndpointInfo` 对象。 此方法将返回 `Endpoint` 表示新远程处理端点的对象。
...
在以编程方式添加TaskManager端点之前，必须先创建 `EndpointRegistryClient` 对象。
...
1. 为端点创建类别。
   ...
   * 创建 `CreateEndpointCategoryInfo` 对象，并传递以下值：

      * 指定类别标识符值的字符串值
      * 一个字符串值，用于指定类别的描述
   * 通过调用 `EndpointRegistryClient` 对象 `createEndpointCategory` 方法和通过 `CreateEndpointCategoryInfo` 对象。 此方法将返回 `EndpointCategory` 表示新类别的对象。


1. 设置TaskManager端点属性。

   * 创建 `CreateEndpointInfo` 对象。
   * 通过调用 `CreateEndpointInfo` 对象 `setConnectorId` 方法和传递字符串值 `TaskManagerConnector`.
   * 通过调用 `CreateEndpointInfo` 对象 `setDescription` 方法和传递描述端点的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setName` 方法和传递指定名称的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setServiceId` 方法和传递指定服务名称的字符串值。
   * 通过调用 `CreateEndpointInfo` 对象 `setCategoryId` 方法和传递指定类别标识符值的字符串值。 您可以调用 `EndpointCategory` 对象 `getId` 方法获取此类别的标识符值。
   * 指定通过调用 `CreateEndpointInfo` 对象 `setOperationName` 方法和传递指定操作名称的字符串值。 通常，在创建 `TaskManager` 源自在Workbench中创建的进程的服务的端点，操作的名称为 `invoke`.

1. 创建TaskManager端点。

   通过调用 `EndpointRegistryClient` 对象 `createEndpoint` 方法和通过 `CreateEndpointInfo` 对象。 此方法将返回 `Endpoint` 表示新TaskManager端点的对象。
...
要以编程方式修改端点，必须创建 `EndpointRegistryClient` 对象。
...
1. 检索要修改的端点。
   ...
   * 通过调用 `EndpointRegistryClient` 对象 `getEndpoints` 方法和通过 `PagingFilter` 用作过滤器的对象。 您可以通过 `(PagingFilter)null` 值来返回所有端点。 此方法将返回 `java.util.List` 每个元素为 `Endpoint` 对象。 有关 `PagingFilter` 对象，请参阅 [AEM Forms API参考](https://www.adobe.com/go/learn_aemforms_javadocs_63_en).
   * 循环访问 `java.util.List` 对象来确定其是否具有端点。 如果存在端点，则每个元素都是 `EndPoint` 实例。
   * 通过调用 `EndPoint` 对象 `getServiceId` 方法。 此方法将返回一个指定服务名称的字符串值。
   * 通过调用 `EndPoint` 对象 `getConnectorId` 方法。 此方法会返回一个指定端点类型的字符串值。 例如，如果端点是“监视文件夹”端点，则此方法将返回 `WatchedFolder`.

1. 指定新配置值。

   * 创建 `ModifyEndpointInfo` 对象。
   * 对于要设置的每个配置值，调用 `ModifyEndpointInfo` 对象 `setConfigParameterAsText` 方法。 例如，要设置URL配置值，请调用 `ModifyEndpointInfo` 对象 `setConfigParameterAsText` 方法并传递以下值：

      * 一个字符串值，用于指定配置值的名称。 例如，要设置 `url` 配置值，指定 `url`.
      * 一个字符串值，用于指定配置值的值。 为 `url` 配置值，指定监视的文件夹位置。
   * 调用 `EndpointRegistryClient` 对象 `modifyEndpoint` 方法和通过 `ModifyEndpointInfo` 对象。
...
为了演示如何从服务中删除端点，本节将从名为 *EncryptDocument*.
...
1. 检索要删除的端点。
   ...
   * 通过调用 `EndpointRegistryClient` 对象 `getEndpoints` 方法和通过 `PagingFilter` 用作过滤器的对象。 您可以通过 `(PagingFilter)null` 返回所有端点。 此方法将返回 `java.util.List` 每个元素为 `Endpoint` 对象。
   * 循环访问 `java.util.List` 对象来确定其是否具有端点。 如果存在端点，则每个元素都是 `EndPoint` 实例。
   * 通过调用 `EndPoint` 对象 `getServiceId` 方法。 此方法将返回一个指定服务名称的字符串值。
   * 通过调用 `EndPoint` 对象 `getConnectorId` 方法。 此方法会返回一个指定端点类型的字符串值。 例如，如果端点是EJB端点，则此方法将返回 `EJB`.

1. 删除终结点。

   通过调用 `EndpointRegistryClient` 对象 `remove` 方法和通过 `EndPoint` 表示要删除的端点的对象。
...
>[!NOTE]
>
>本主题使用 `ConnectorRegistryClient` 用于检索有关Endpoint Connectors的信息的API。 (请参阅 [AEM Forms API参考](https://www.adobe.com/go/learn_aemforms_javadocs_63_en).)
...
如果AEM Forms部署在非JBoss的受支持J2EE应用程序服务器上，则请将adobe-utilities.jar和jbossall-client.jar替换为特定于部署了AEM Forms的J2EE应用程序服务器的JAR文件。 有关所有AEM Forms JAR文件的位置的信息，请参阅 [包括AEM Forms Java库文件](/help/forms/developing/invoking-aem-forms-using-java.md#including-aem-forms-java-library-files).
...
1. 创建ConnectorRegistry客户端对象。
   ...
   * 创建 `ServiceClientFactory` 包含连接属性的对象。
   * 创建 `ConnectorRegistryClient` 对象，并使用其构造函数进行传递 `ServiceClientFactory` 对象。

1. 指定连接器类型。

   通过调用 `ConnectorRegistryClient` 对象 `getEndpointDefinition` 方法和传递指定连接器类型的字符串值。 例如，要指定“监视文件夹”连接器类型，请传递字符串值 `WatchedFolder`. 此方法将返回 `Endpoint` 对应于连接器类型的对象。

1. 检索配置值。

   * 通过调用 `Endpoint` 对象 `getConfigParameters` 方法。 此方法会返回 `ConfigParameter` 对象。
   * 通过检索数组中的每个元素，检索有关每个配置值的信息。 每个元素都是 `ConfigParameter` 对象。 例如，您可以通过调用 `ConfigParameter` 对象 `isRequired` 方法。 如果需要配置值，则此方法将返回 `true`.
