...
表单作者指定脚本是在服务器还是客户端上执行。 Forms服务创建一个分布式事件处理环境，用于执行表单智能，该表单智能可通过使用 `runAt` 属性。 有关此属性或在表单设计中创建脚本的信息，请参阅 [Forms Designer](https://www.adobe.com/go/learn_aemforms_designer_63_cn)
...
在客户端上运行或在客户端和服务器上同时运行的脚本必须写入XFA子集中。 在服务器上运行的脚本可以使用完整的XFA脚本模型，也可以使用FormCalc。 有关使用JavaScript的信息，请参阅 [Forms Designer](https://www.adobe.com/go/learn_aemforms_designer_63_cn).
...
有关空测试的更多信息，请参阅 [Forms Designer](https://www.adobe.com/go/learn_aemforms_designer_63_cn).

## 表单按钮 {#form-buttons}

单击提交按钮会将表单数据发送到Forms服务，并表示表单处理结束。 的 `preSubmit` 事件可设置为在客户端或服务器上运行。 的 `preSubmit` 如果将事件配置为在客户端上运行，则在表单提交之前运行。 否则， `preSubmit` 在表单提交期间，事件在服务器上运行。 有关 `preSubmit` 事件，请参阅 [Forms Designer](https://www.adobe.com/go/learn_aemforms_designer_63_cn).
