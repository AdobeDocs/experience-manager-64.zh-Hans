...
确保fscmenu.xml文件中引用了上述所有文件。 即，在fscmenu.xml文件中，指定URI位置以指向这些文件，以便Forms服务能够找到它们。 默认情况下，这些文件位于以内部关键字或开头的URI `FSWebRoot` 位置 `ApplicationWebRoot`。
...
此外，通过调用对象的方法并传递 `HTMLRenderSpec` 指定区 `setLocale` 域设置值的字符串值来指定有效的区域设置值。 例如，传递 `fr_FR` 以指定法语。 Forms服务与本地化的工具栏捆绑在一起。
