...
Forms服务向客户端设备（通常是Web浏览器）提供交互式PDF forms，以从用户收集信息。 呈现交互式表单后，用户可以将数据输入表单字段，并单击表单上的提交按钮，将信息发送回Forms服务。 Adobe Reader或Acrobat必须安装在承载客户端Web浏览器的计算机上，这样交互式PDF表单才可见。

>[!NOTE]
>
>在使用Forms服务渲染表单之前，请创建表单设计。 通常，表单设计在Designer中创建并保存为XDP文件。 有关创建表单设计的信息，请参阅 [Forms设计](https://www.adobe.com/go/learn_aemforms_designer_63)。

**贷款申请范例**

引入了一个贷款申请范例，以演示Forms服务如何使用交互式表单从用户处收集信息。 此应用程序允许用户填写表格，其中包含确保贷款安全所需的数据，然后将数据提交到Forms服务。 下图显示了贷款应用程序的逻辑流。
...
<table> 
 ...
 <tbody>
  ...
  <tr> 
   <td><p>2</p></td> 
   <td><p>Java <code>GetLoanForm</code> Servlet使用Forms服务客户端API向客户端Web浏览器呈现贷款表。 (请参 <a href="#render-an-interactive-pdf-form-using-the-java-api">阅使用Java API渲染交互式PDF表单</a>。)</p></td> 
  </tr> 
  <tr> 
   <td><p>3</p></td> 
  ...
  <tr> 
   <td><p>4</p></td> 
   <td><p>Java <code>HandleData</code> Servlet使用Forms服务客户端API处理表单提交和检索表单数据。 然后，数据存储在企业数据库中。 (请参阅处 <a href="/help/forms/developing/handling-submitted-forms.md#handling-submitted-forms">理提交的Forms</a>。)</p></td> 
  </tr> 
  <tr> 
   <td><p>5</p></td> 
...
Java `HandleData` Servlet使用用户的名和姓以及金额预填充此表单。 预填充表单后，表单将发送到客户端Web浏览器。 (请参阅 [使用可流动布局预填充Forms](/help/forms/developing/prepopulating-forms-flowable-layouts.md))
...
通常，您不会将Forms服务客户端API代码放在Java Servlet或 `doGet` 方法 `doPost` 中。 最好将此代码放在单独的类中，从方法（或方法）中实例化 `doPost` 该类并调 `doGet` 用相应的方法。 但是，对于代码简单性，本节中的代码示例将保持为最小值，并且代码示例将放在方法 `doPost` 中。
...
在以编程方式执行Forms服务客户端API操作之前，必须创建一个Forms客户端API对象。 如果您使用Java API，请创建一个 `FormsServiceClient` 对象。 如果您使用的是FormsWeb服务API，请创建一个 `FormsService` 对象。

**指定URI值**

您可以指定Forms服务渲染表单所需的URI值。 作为Forms应用程序的一部分保存的表单设计可以使用内容根URI值进行引用 `repository:///`。 例如，请考虑位于名为FormsApplication的 *Forms应用程序中* ，名为Loan.xdp的以 *下表单设计*:
...
如果目标URL是在表单设计中定义的，请勿使用Forms服务客户端API覆盖它。 即，使用FormsAPI设置目标URL会将表单设计中指定的URL重置为使用API指定的URL。 如果要将PDF表单提交到在表单设计中指定的目标URL，请以编程方式将目标URL设置为空字符串。
...
>[!NOTE]
>
>除了指定URL值以引用XDP文件外，您还可以将实例 `com.adobe.idp.Document` 传递给Forms服务。 实 `com.adobe.idp.Document` 例包含表单设计。 (请参 [阅将文档传递给Forms服务](/help/forms/developing/passing-documents-forms-service.md)。)

**将文件附加到表单**

可以将文件附加到表单。 当您渲染带有文件附件的PDF表单时，用户可以使用文件附件窗格在Acrobat检索文件附件。 可以将不同的文件类型附加到表单（如文本文件）或二进制文件（如JPG文件）。
...
使 `renderPDFForm` 用对 `URLSpec` 象。 XDP文件的内容根将使用对象的方法传递 `URLSpec` 给Forms服 `setContentRootURI` 务。 表单设计名称( `formQuery`)作为单独的参数值传递。 将这两个值连接起来，以获得对表单设计的绝对引用。
...
1. 指定URI值
   ...
   * 调用对 `URLSpec` 象的方 `setContentRootURI` 法并传递一个指定内容根URI值的字符串值。 确保表单设计位于内容根URI中。 如果没有，Forms服务会引发异常。 要引用存储库，请指定 `repository:///`。
...
1. 将文件附加到表单
   ...
   * 包 `com.adobe.idp.Document` 含文件附件的对象。
   ...
   >[!NOTE]
   >
   >对要附加到表单的每个文件重复此步骤。 此步骤是可选的，如 `null`果您不想发送文件附件，可以通过*。*
