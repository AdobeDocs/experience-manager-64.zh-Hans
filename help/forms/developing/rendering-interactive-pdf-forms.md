...
>[!NOTE]
>
>在使用Forms服务渲染表单之前，请创建表单设计。 通常，表单设计在Designer中创建并保存为XDP文件。 有关创建表单设计的信息，请参阅[Forms设计器](https://www.adobe.com/go/learn_aemforms_designer_63)。
...
`HandleData` Java Servlet预填充此表单时使用用户的名和姓以及金额。 预填充表单后，表单将发送到客户端Web浏览器。 (请参阅[使用可流动布局预填充Forms](/help/forms/developing/prepopulating-forms-flowable-layouts.md))
...
通常，您不会将Forms服务客户端API代码放在Java Servlet的`doGet`或`doPost`方法中。 最好将此代码放在单独的类中，从`doPost`方法（或`doGet`方法）实例化该类，并调用相应的方法。 但是，对于代码简短性，本节中的代码示例将保持为最小值，代码示例将放在`doPost`方法中。
...
在以编程方式执行Forms服务客户端API操作之前，必须创建一个Forms客户端API对象。 如果您使用Java API，请创建`FormsServiceClient`对象。 如果您使用的是FormsWeb服务API，请创建一个`FormsService`对象。

**指定URI值**

您可以指定Forms服务渲染表单所需的URI值。 作为Forms应用程序的一部分保存的表单设计可以使用内容根URI值`repository:///`进行引用。 例如，请考虑以下名为&#x200B;*Loan.xdp*&#x200B;的表单设计，它位于名为&#x200B;*FormsApplication*&#x200B;的Forms应用程序中：
...
如果您的表单包含提交按钮和计算按钮（带有在服务器上运行的相应脚本），则可以通过编程方式定义要将表单发送到的执行脚本的URL。 使用表单设计上的提交按钮指定发布表单数据的URL。 （请参阅[计算表单数据](/help/forms/developing/calculating-form-data.md)。）

>[!NOTE]
>
>您还可以将`com.adobe.idp.Document`实例传递给Forms服务，而不是指定URL值来引用XDP文件。 `com.adobe.idp.Document`实例包含表单设计。 (请参阅[将文档传递给Forms服务](/help/forms/developing/passing-documents-forms-service.md)。)
...
`renderPDFForm`使用`URLSpec`对象。 XDP文件的内容根将使用`URLSpec`对象的`setContentRootURI`方法传递到Forms服务。 表单设计名称(`formQuery`)作为单独的参数值传递。 将这两个值连接起来，以获得对表单设计的绝对引用。
...
1. 将文件附加到表单
   ...
   * 为每个文件调用`java.util.HashMap`对象的`put`方法以附加到呈现的表单。 将以下值传递给此方法：
   ...
   * 包含文件附件的`com.adobe.idp.Document`对象。
   ...
   >[!NOTE]
   >
   >对要附加到表单的每个文件重复此步骤。 此步骤是可选的，如果您不想发送文件附件，可以通过`null`*。*
