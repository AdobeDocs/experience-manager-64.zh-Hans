...
以下Java代码示例修改名为SignatureField1的签名字段，方法是在将签名应用到签名字段时锁定表单中的所有字段，并确保不允许更改。 在签名服务返回包含已修改的签名字段的PDF文档后，PDF文档将保存为名为LoanSig.pdf的PDF文件。 （此示例覆盖传递给签名服务的PDF文件。） （请参阅[修改签名字段](/help/forms/developing/digitally-signing-certifying-documents.md#modifying-signature-fields)。）
...
以下Java代码示例对由Forms服务呈现的交互式表单进行签名。 由Forms服务返回的`com.adobe.idp.Document`实例将传递给签名服务。 签名的交互式表单将保存为名为&#x200B;*LoanXFASpid.pdf*&#x200B;的PDF文件。
...
以下Java代码示例验证位于基于名为LoanSigned.pdf的PDF文件的已签名PDF文档中的数字签名。 验证时间被设置为当前时间，吊销检查选项被设置为尽最大努力。 （请参阅[验证数字签名](#unresolvedlink-lc-si)。）
...
以下Java代码示例验证位于基于名为LoanAllSigs.pdf的PDF文件的已签名PDF文档中的多个数字签名。 验证时间被设置为当前时间，吊销检查选项被设置为尽最大努力。 （请参阅[验证多个数字签名](#unresolvedlink-lc-si)。）
