...
在用户管理导出的配置XML中，属性值为`Domains`的根节点包含为用户管理定义的每个域的XML元素。 这些元素中的每一个都包含其他元素，这些元素定义了与特定服务提供商关联的域的各个方面。

**了解objectSID值**

使用Active Directory时，请务必了解`objectSID`值不是跨多个域的唯一属性。 此值存储对象的安全标识符。 在多域环境（例如，域树）中，`objectSID`值可以不同。

如果对象从一个Active Directory域移动到另一个域，则`objectSID`值会发生变化。 某些对象在域中的任何位置具有相同的`objectSID`值。 例如，诸如BUILTIN\Administrators、BUILTIN\Power Users等组的值与`objectSID`值相同，而不管域。 这些`objectSID`值是众所周知的。
...
1. 定义用户信息。
   ...
   * 通过调用`UserImpl`对象的`setGivenName`方法来设置给定名称。 传递指定用户给定名称的字符串值。 例如，可以指定`Wendy`。
   * 通过调用`UserImpl`对象的`setFamilyName`方法来设置系列名称。 传递一个指定用户姓氏的字符串值。 例如，可以指定`Blue`。
...
1. 创建DirectoryManagerService客户端。
   ...
   * 使用`System.ServiceModel.EndpointAddress`构造函数创建`DirectoryManagerServiceClient.Endpoint.Address`对象。 将指定WSDL的字符串值传递给AEM Forms服务（例如`http://localhost:8080/soap/services/DirectoryManagerService?blob=mtom`）。 您无需使用`lc_version`属性。 此属性在您创建服务引用时使用。 确保指定`?blob=mtom`。
   * 通过获取`DirectoryManagerServiceClient.Endpoint.Binding`字段的值创建`System.ServiceModel.BasicHttpBinding`对象。 将返回值转换为`BasicHttpBinding`。
   ...
   * 通过执行以下任务启用基本HTTP身份验证：
      ...
      * 为字段`DirectoryManagerServiceClient.ClientCredentials.UserName.Password`分配相应的口令值。
...
1. 从AEM Forms删除用户。
   ...
   将`User`对象的`oid`字段值传递给`DirectoryManagerServiceClient`对象的`deleteLocalUser`方法，以删除用户。
...
您可以使用目录管理器服务API（Java和Web服务）以编程方式创建AEM Forms组。 创建组后，您可以使用该组执行需要组的服务操作。 例如，可以将用户分配给新用户组。 （请参阅[管理用户和用户组](users.md#managing-users-and-groups)。）
...
创建组时，请确保该组不存在于同一域中。 也就是说，两个组不能在同一域中具有相同的名称。 要执行此任务，请执行搜索并根据两个值筛选搜索结果。 将主体类型设置为`com.adobe.idp.um.api.infomodel.Principal.PRINCIPALTYPE_GROUP`以确保只返回组。 另外，请确保指定域名。
...
1. 确定组是否存在。
   ...
   * 在`java.util.List`实例中迭代。 对于每个元素，检索组名称。 确保用户组名称不等于新用户组名称。
...
## 管理用户和用户组{#managing-users-and-groups}

本主题介绍如何使用(Java)以编程方式分配、删除和查询域、用户和用户组。

>[!NOTE]
>
>配置域时，必须为组和用户设置唯一标识符。 所选属性不仅必须在LDAP环境中唯一，而且必须不变，并且不会在目录中更改。 此属性还必须是简单的字符串数据类型（Active Directory 2000/2003当前允许的唯一异常是`"objectsid"`，它是二进制值）。 例如，Novell eDirectory属性`"GUID"`不是简单的字符串数据类型，因此将无法工作。
...
在以编程方式执行目录管理器服务操作之前，必须创建目录管理器服务客户端。 使用Java API，可通过创建`DirectoryManagerServiceClient`对象来完成此操作。 借助Web服务API，可通过创建`DirectoryManagerServiceService`对象来完成此操作。
...
1. 包括项目文件。
   ...
   在Java项目的类路径中包含客户端JAR文件，如adobe-usermanager-client.jar。 有关这些文件的位置的信息，请参见[包括AEM FormsJava库文件](/help/forms/developing/invoking-aem-forms-using-java.md#including-aem-forms-java-library-files)。
...
1. 调用相应的用户或组操作。
   ...
   使用生成的`User`或`Group`对象（两者都继承自`Principal`接口），在工作流中检索所需信息。 例如，域名和规范名称值组合使用来唯一标识主体。 通过分别调用`Principal`对象的`getDomainName`和`getCanonicalName`方法来检索这些属性。
...
1. 调用相应的用户或组操作。
   ...
   要查找用户或组，请调用`DirectoryManagerServiceService`对象查找主体的方法之一（因为主体可以是用户或组）。 在以下示例中，使用搜索筛选器（`PrincipalSearchFilter`对象）调用`findPrincipalsWithFilter`方法。 使用`PrincipalSearchFilter`对象时，仅当`isLocal`属性设置为`true`时，才返回本地承担者。 此行为与Java API的行为不同。

   >[!NOTE]
   >
   >如果未在搜索筛选器（通过`PrincipalSearchFilter.resultsMax`字段）中指定最大结果数，则最多将返回1000个结果。 这与使用Java API时的行为不同，在Java API中，默认最大值为10个结果。 此外，搜索方法（如`findGroupMembers`）不会生成任何结果，除非在搜索筛选器（例如，通过`GroupMembershipSearchFilter.resultsMax`字段）中指定最大结果数。 这适用于继承`GenericSearchFilter`类的所有搜索过滤器。 有关详细信息，请参阅[AEM FormsAPI参考](https://www.adobe.com/go/learn_aemforms_javadocs_63_en)。

   由于此情况下的返回值是包含`Principal`对象的`object[]`，因此对结果进行迭代，并将`Principal`对象转换为`User`或`Group`对象。

   使用生成的`User`或`Group`对象（两者都继承自`Principal`接口），在工作流中检索所需信息。 例如，域名和规范名称值组合使用来唯一标识主体。 通过分别调用`Principal`对象的`domainName`和`canonicalName`字段来检索这些字段。
...
在AEM Forms,*角色*&#x200B;是一组访问一个或多个系统级资源的权限。 这些权限是通过用户管理创建的，并由服务组件实施。 例如，管理员可以向一组用户分配“策略集作者”角色。 Rights Management随后将允许具有该角色的组的用户通过管理控制台创建策略集。

有两种角色：*默认角色*&#x200B;和&#x200B;*自定义角色*。 默认角色（*系统角色）*&#x200B;已驻留在AEM Forms。 假定默认角色不能由管理员删除或修改，因此不可变。 因此，管理员创建的自定义角色可变，管理员随后可以修改或删除这些角色。
...
在以编程方式执行User Management AuthorizationManagerService操作之前，必须创建AuthorizationManagerService客户端。 使用Java API，可通过创建`AuthorizationManagerServiceClient`对象来完成此操作。
...
1. 创建AuthorizationManagerService客户端。
   ...
   * 使用其默认构造函数创建`AuthorizationManagerServiceClient`对象。
   * 使用`System.ServiceModel.EndpointAddress`构造函数创建`AuthorizationManagerServiceClient.Endpoint.Address`对象。 将指定WSDL的字符串值传递给AEM Forms服务（例如，`http://localhost:8080/soap/services/AuthorizationManagerService?blob=mtom`）。 您无需使用`lc_version`属性。 此属性在您创建服务引用时使用。
   * 通过获取`AuthorizationManagerServiceClient.Endpoint.Binding`字段的值创建`System.ServiceModel.BasicHttpBinding`对象。 将返回值转换为`BasicHttpBinding`。
   * 将`System.ServiceModel.BasicHttpBinding`对象的`MessageEncoding`字段设置为`WSMessageEncoding.Mtom`。 此值确保使用MTOM。
   * 通过执行以下任务启用基本HTTP身份验证：

      * 为字段`AuthorizationManagerServiceClient.ClientCredentials.UserName.UserName`指定AEM表单用户名。
      * 为字段`AuthorizationManagerServiceClient.ClientCredentials.UserName.Password`分配相应的口令值。
      * 将常量值`HttpClientCredentialType.Basic`指定到字段`BasicHttpBindingSecurity.Transport.ClientCredentialType`。
      * 将常量值`BasicHttpSecurityMode.TransportCredentialOnly`指定到字段`BasicHttpBindingSecurity.Security.Mode`。
...
对用户进行身份验证后，可以在经过身份验证的用户中创建上下文。 然后，您可以使用该内容调用其他AEM Forms服务。 例如，您可以使用上下文创建`EncryptionServiceClient`并使用口令加密PDF文档。 确保已通过身份验证的用户具有调用AEM Forms服务所需的名为`Services User`的角色。
...
1. 调用身份验证操作。
   ...
   * 包含用户口令的字节数组（`byte[]`对象）。 通过调用`java.lang.String`对象的`getBytes`方法，可以获得`byte[]`对象。

   authenticate方法返回`AuthResult`对象，该对象包含有关已验证用户的信息。
...
1. 调用身份验证操作。
   ...
   * 包含用户名的`string`对象
   * 包含用户口令的字节数组（`byte[]`对象）。 您可以使用以下示例中显示的逻辑将包含密码的`string`对象转换为`byte[]`数组，从而获得`byte[]`对象。
   * 返回的值将是`AuthResult`对象，可用于检索有关用户的信息。 在以下示例中，首先获取`AuthResult`对象的`authenticatedUser`字段，然后获取生成的`User`对象的`canonicalName`和`domainName`字段，以检索用户的信息。
