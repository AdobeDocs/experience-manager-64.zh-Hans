...
**示例**: 要根据其他字段的值填充字段的值，请提供一个简单的选项表达式。 例如，要填充一个字段“ **子女数量** ”，根据在另 **一个字段中表** 示的“婚姻状态”,表达式为：

**`marital_status.value == "married" ? ["1=One", "2=two"] : ["0=Zero"]`.**

只要frine_status字 **段的值发生** ，就会触发表达式。 您还可以从REST服务填充下拉列表。 有关详细信息，请参 [阅动态填充下拉列表](/help/forms/using/dynamically-populate-dropdowns.md)。
...
要强制使组件成为必需组件，请 **[!UICONTROL 在组件的]** “编辑”对话框中，选择“标题 **[!UICONTROL 和文本”>“必需”选项]**。 您还可以添加相应 **的必需消** 息（可选）。 .

### 验证模式 {#validation-patterns}

有多个可用于字段的现成验证模式。 要选择验证模式，请在组件的 **[!UICONTROL “编辑]** ”对话框中，找到 **[!UICONTROL “模式]** ”部分并选 **[!UICONTROL 择模式]**。 您可以在“模式”文本框中创建您自己的自 **定义** 验证模式。 仅当填充的数 **据符** 合验证模式时，验证状态才返回 **True** ，否则返回False。 要编写您自己的自定义验证模式，请参 [阅HTML5表单的Picture子句支持](/help/forms/using/picture-clause-support.md)。

### 验证表达式 {#validation-expressions}

也可使用不同字段上的表达式计算字段的验证。 这些表达式编 **[!UICONTROL 写在组件]** “编辑” **[!UICONTROL 对话框]** “脚本 **[!UICONTROL ”选项卡]** 的“验证脚本”字段中。 字段的验证状态取决于表达式返回的值。 有关如何编写此类表达式的信息，请参阅验 [证表达式](/help/forms/using/adaptive-form-expressions.md#p-validate-expression-p)。

## 附加信息 {#additional-information}

### 使用字段显示格式 {#using-field-display-format}

“显示格式”可用于以不同格式显示数据。 例如，可以使用显示格式显示带连字符的电话号码、设置ZIP代码格式或日期选取器格式。 可以从组件的“编辑” **[!UICONTROL 对话框]** 的“图 **[!UICONTROL 案”部]** 分选择这些显示图案。 您可以编写与上述验证模式类似的自定义显示模式。
