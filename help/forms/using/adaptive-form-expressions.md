...
* **[单击表达式](/help/forms/using/adaptive-form-expressions.md#p-click-expression-p)**:可以处理按钮的单击事件。
...
* **[值提交](/help/forms/using/adaptive-form-expressions.md#p-value-commit-script-p)脚** 本：更改字段值后更改表单的组件。
...
**示例**:要在按钮的单 **击操** 作中填充包含值AEM Forms的文 **本框** 1，按钮的单击表达式为 `textbox1.value="AEM Forms"` &quot;
...
**示例**:要根据另一个字段的值填充字段的值，请提供一个简单的选项表达式。例如，要根据另一个字段中表达的&#x200B;**婚姻状态**&#x200B;填充字段&#x200B;**子数**,表达式为：

**`marital_status.value == "married" ? ["1=One", "2=two"] : ["0=Zero"]`.**

只要&#x200B;**frimery_status**&#x200B;字段的值发生更改，将重新触发表达式。 您还可以从REST服务填充下拉列表。 有关详细信息，请参阅[动态填充下拉列表](/help/forms/using/dynamically-populate-dropdowns.md)。
...
>[!NOTE]
>
>当以编程方式更改字段的值时，可以禁用执行值提交脚本。 要执行此操作，请转至`https://[server]:[port]/system/console/configMgr and change` **兼容性自适应Forms版本**&#x200B;到&#x200B;**AEM Forms 6.1**。 此后，仅当用户从UI更改字段值时，才执行值提交脚本。
...
一个字段有多种现成的验证模式。 要选择验证模式，请在组件的&#x200B;**[!UICONTROL 编辑]**&#x200B;对话框中，找到&#x200B;**[!UICONTROL 图案]**&#x200B;部分并选择&#x200B;**[!UICONTROL 图案]**。 您可以在&#x200B;**Pattern**&#x200B;文本框中创建您自己的自定义验证模式。 仅当填充的数据符合验证模式时，才返回&#x200B;**True**&#x200B;验证状态，否则返回&#x200B;**False**。 要编写您自己的自定义验证模式，请参阅[支持HTML5表单的Picture子句](/help/forms/using/picture-clause-support.md)。

### 验证表达式{#validation-expressions}

也可以使用不同字段上的表达式计算字段的验证。 这些表达式写入到组件&#x200B;**[!UICONTROL 编辑]**&#x200B;对话框的&#x200B;**[!UICONTROL 脚本]**&#x200B;选项卡的&#x200B;**[!UICONTROL 验证脚本]**&#x200B;字段中。 字段的验证状态取决于表达式返回的值。 有关如何编写此类表达式的信息，请参阅[验证表达式](/help/forms/using/adaptive-form-expressions.md#p-validate-expression-p)。

## 附加信息 {#additional-information}

### 使用字段显示格式{#using-field-display-format}

“显示格式”可用于显示不同格式的数据。 例如，可以使用显示格式显示带连字符的电话号码、设置ZIP代码格式或日期选取器格式。 这些显示模式可以从组件的&#x200B;**[!UICONTROL 编辑]**&#x200B;对话框的&#x200B;**[!UICONTROL 模式]**&#x200B;部分中选择。 您可以编写类似于上述验证模式的自定义显示模式。
