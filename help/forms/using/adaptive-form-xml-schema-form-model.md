...
description: 自适应表单可以使用XML模式作为表单模型，从而允许您利用现有的XSD模板创建自适应表单。 您可以将模式元素从XSD拖放到自适应表单上。
seo-description: 自适应表单可以使用XML模式作为表单模型，从而允许您利用现有的XSD模板创建自适应表单。 您可以将模式元素从XSD拖放到自适应表单上。
...
使用XML模式作为表单模型来创作自适应表单需要对XML模式有基本的了解。 此外，建议在本文之前阅读以下内容。
...
AEM Forms支持使用现有XML模式作为表单模型创建自适应表单。 此XML模式表示组织中的后端系统生成或使用数据的结构。

使用XML模式的主要功能有：

* 在自适应表单的创作模式中，XSD的结构在“内容查找器”选项卡中显示为树。 您可以将XSD层次结构中的元素拖放到自适应表单中。
* 您可以使用符合关联模式的XML预填充表单。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>指定最小值</p> <p>(对于可重复的子表单（复杂类型）)</p> </td> 
   <td>元素（复杂类型）</td> 
  </tr> 
  <tr> 
...
>
>
例如，如果添加`userFirstName`模式元素，则自适应表单中生成的题注为`User First Name`。

## 限制自适应表单组件{#limit-acceptable-values-for-an-adaptive-form-component}的可接受值

您可以向XML模式元素添加以下限制以限制自适应表单组件可接受的值：
...
在内容查找器中按住多次单击某个元素时，将显示一个字段名称和一个名为`bindRef`的属性。 此属性将树元素映射到模式中的元素或属性。

![XML模式元素的bindref字段](assets/dblclick.png)

bindRef</code>字段显示树元素与模式中的元素或属性之间的关联。

>[!NOTE]
>
>属性的`bindRef`值中有`@`符号，以区分它们与元素。 例如，`/config/projectDetails/@duration`。
...
`bindRef`是自适应表单组件与模式元素或属性之间的连接。 它指示从此组件或字段捕获的值在输出XML中可用的`XPath`。 从预填充（预填充）XML预填充字段值时，也使用`bindRef`。
