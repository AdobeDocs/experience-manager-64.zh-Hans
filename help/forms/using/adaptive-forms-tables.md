...
![desktopview](assets/desktopview.png)**图：** *左侧布局带有标题的表的桌面视图*

![headersontheleft](assets/headersontheleft.png)**图：** *左侧布局带有标题的表的移动视图*
...
![collapsed-column](assets/collapsed-column.png)**图：** *在移动设备上仅显示两列的表的折叠列*

![collapsible_column](assets/collapsible_column.png)**图：** *移动设备上表的扩展列*
...
要在运行时将此数据与表合并，我们需要将包含表的自适应表单指向禁用wcmmode的绝对XML位置。 例如，如果自适应表单位于 *http://localhost:4502/myForms/bankTransaction.html* ，而数据XML文件保存在 *C:/myTransactions/bankSummary.xml*，则可以将表与位于以下URL的数据视图:
