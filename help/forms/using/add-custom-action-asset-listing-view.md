...
1. 在新创建的项目文件夹下，为特定资产中的自定义按钮／操作添加一个节点(示例： 下载FlatPDF)，步骤如下：
   ...
   1. 确保“创建节点”对话框具有以下值，然后单击“确 **[!UICONTROL 定”]**:
      ...
      **[!UICONTROL 名称：]** downloadFlatPDF（或要为此属性指定的名称）
...
1. 在apps文件夹中，使用以下步骤创建一个名为js的文件夹，其路径／结构与管理文件夹中的项目文件夹类似：
   ...
   1. 右键单击以 **[!UICONTROL 下路径]** 的js文件夹，然后选择 **[!UICONTROL 叠加节点]**: &quot;
...
1. 在js文件夹中，使用用于按钮操作处理的代码使用以下步骤创建一个名为formaction.js的文件：
   ...
   1. 右键单击以下路 **[!UICONTROL 径的]** js文件夹，然后选择 **[!UICONTROL 创建>创建文件]**:
   ...
   1. 在formaction.js文件（/apps分支下）中，从位于以下位置的formaction.js文件复制代码：
      ...
      然后，在formaction.js文件（/apps分支下）的末尾附加以下代码，并单击“全部 **[!UICONTROL 保存]**:
...
1. 在新创建的项目节点下，为特定资产中的自定义按钮／操作添加一个节点(示例： letterpdfdownloader)。
   ...
   1. 确保“创建节点”对话框具有以下值，然后单击“确 **[!UICONTROL 定”]**:
      ...
      **[!UICONTROL 名称：]** letterpdfdownloader（或要为此属性指定的名称）必须是唯一的。 如果在此处使用其他名称，也可在formaction.js文件的ACTION_URL变量中指定相同的名称。)
...
1. 在以下位置使用命令的操作处理代码创建名为POST.jsp的文件：
   ...
   1. 右键单击以下路 **[!UICONTROL 径的]** admin文件夹，然后选择 **[!UICONTROL 创建>创建文件]**:

      /apps/fd/cm/ma/gui/components/admin/clientlibs/admin

      将文件命名为POST.jsp。 (文件名只需为POST.jsp。)

   1. 多次单击 **[!UICONTROL POST]** .jsp文件，在CRX中打开它。
   1. 将以下代码添加到POST.jsp文件，然后单击“ **[!UICONTROL 全部保存]**”:

      此代码特定于字母渲染服务。 对于任何其他资源，请将该资源的java库添加到此代码中。 有关AEM FormsAPI的详细信息，请参 [阅AEM FormsAPI](https://adobe.com/go/learn_aemforms_javadocs_63_en)。
...
1. 选择“ **[!UICONTROL Forms”>“字母]**”。 通信管理列表系统中可用的信函。
1. 单击 **[!UICONTROL 选择]** ，然后单击字母将其选中。
1. 选择 **[!UICONTROL 更多]** > &lt;下载平面PDF>（使用本文中的说明创建的自定义功能）。 将显示“将信函下载为PDF”对话框。
...
1. 在“以PDF形式下载信函”对话框中，选择要从中填充PDF中数据的相关XML。
   ...
   >[!NOTE]
   >
   >在将字母下载为平面PDF之前，您可以使用“创建报告”选项创建包含字母中数据 **[!UICONTROL 的XML]** 文件。
