...
1. 在apps文件夹中，使用以下步骤创建一个名为items的文件夹，其路径/结构与位于选择文件夹中的items文件夹类似：
   ...
   1. 右键单击以下路径的&#x200B;**[!UICONTROL items]**&#x200B;文件夹，然后选择&#x200B;**[!UICONTROL 叠加节点]**:
...
1. 在新创建的项目文件夹下，为特定资产中的自定义按钮/操作添加一个节点(示例：下载FlatPDF)，步骤如下：
   ...
   1. 确保“创建节点”对话框具有以下值，然后单击&#x200B;**[!UICONTROL 确定]**:
      ...
      **[!UICONTROL 名称：]** downloadFlatPDF（或要给此属性的名称）

      **[!UICONTROL 类型：]** nt:unstructured

   1. 单击您创建的新节点（此处为downloadFlatPDF）。 CRX显示节点的属性。
   1. 将以下属性添加到节点（此处为downloadFlatPDF），然后单击&#x200B;**[!UICONTROL 保存全部]**:
...
1. 在apps文件夹中，使用以下步骤创建一个名为js的文件夹，其路径/结构与管理文件夹中的items文件夹类似：
   ...
   1. 单击&#x200B;**[!UICONTROL 确定]**。文件夹结构将在apps文件夹中创建。 单击&#x200B;**[!UICONTROL 保存全部]**。
...
1. 在js文件夹中，使用以下步骤创建一个名为formaction.js的文件，其中包含用于按钮操作处理的代码：
   ...
   1. 在formaction.js文件（/apps分支下）中，从位于以下位置的formaction.js文件复制代码：
      ...
      然后，在formaction.js文件（/apps分支下）的末尾附加以下代码，并单击&#x200B;**[!UICONTROL 保存全部]**:
...
1. 在新创建的项目节点下，为特定资产中的自定义按钮/操作添加一个节点(示例：letterpdfdownloader)。
   ...
   1. 确保“创建节点”对话框具有以下值，然后单击&#x200B;**[!UICONTROL 确定]**:
      ...
      **[!UICONTROL 名称：]** letterpdfdownloader(或要为此属性指定的名称 — 必须唯一。如果在此处使用其他名称，请在formaction.js文件的ACTION_URL变量中指定相同的名称。)

      **[!UICONTROL 类型：]** nt:unstructured

   1. 单击您创建的新节点（此处为downloadFlatPDF）。 CRX显示节点的属性。

   1. 将以下属性添加到节点（此处为letterpdfdownloader），然后单击&#x200B;**[!UICONTROL 保存全部]**:
...
1. 使用以下位置创建一个名为POST.jsp的文件，其中包含用于命令操作处理的代码：
   ...
   1. 右键单击以下路径的&#x200B;**[!UICONTROL admin]**&#x200B;文件夹，然后选择&#x200B;**[!UICONTROL 创建>创建文件]**:

      /apps/fd/cm/ma/gui/components/admin/clientlibs/admin

      将文件命名为POST.jsp。 (文件名只需为POST.jsp。)

   1. 多次 — 单击&#x200B;**[!UICONTROL POST.jsp]**&#x200B;文件以在CRX中打开它。
   1. 将以下代码添加到POST.jsp文件，然后单击&#x200B;**[!UICONTROL 保存全部]**:
...
1. 选择&#x200B;**[!UICONTROL Forms >字母]**。 Corresponence Management会列表系统中可用的信函。
1. 单击&#x200B;**[!UICONTROL 选择]**，然后单击字母将其选中。
1. 选择&#x200B;**[!UICONTROL 更多]** > &lt;下载平面PDF>（使用本文说明创建的自定义功能）。 将显示“PDF格式下载信函”对话框。
...
1. 在“将信函下载为PDF”对话框中，选择要从中填充PDF中数据的相关XML。
   ...
   >[!NOTE]
   >
   >在将字母下载为平面PDF之前，可以使用&#x200B;**[!UICONTROL 创建报告]**&#x200B;选项创建包含字母中数据的XML文件。
