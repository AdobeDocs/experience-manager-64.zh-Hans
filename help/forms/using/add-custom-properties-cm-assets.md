...
1. 要视图自定义，请将指针悬停在资产(文本、列表、条件或布局片段)或字母上，单击&#x200B;**视图属性**，然后单击&#x200B;**编辑**。 新字段(收件人的位置)将显示在资产／字母属性的“基本”选项卡中。
...
### 方案：向自定义属性／字段{#scenario-add-custom-drop-down-and-values-to-a-custom-property-field}添加自定义下拉框和值
...
1. 在新创建的叠加节点下(/apps/fd/cm/ma/gui/content/cmmetadataproperties/commonproperties/col1/items)
   ...
   为需要为其创建nt:unstructured类型下拉列表（此处`geographicallocation`）的每个属性（字段）创建一个节点。
...
1. 将以下属性添加到节点（此处为地理分配），然后单击&#x200B;**保存全部**:
   ...
   | 名称 | 字符串 | `./extendedproperties/geographicallocation` （保留值与您在项目节点下创建的字段名称相同） |
   | renderReadOnly | 布尔型 | 真 |
   | sling:resourceType | 字符串 | granite/ui/components/coral/foundation/form/select |

1. 在属性节点（此处为地理分配）下，添加名称为`items`的新节点。 在项节点下，为下拉框中的值分别添加一个节点。 作为一种最佳实践，将第一个节点添加为空白，作为下拉框的默认值，并为用户指定字段的任何值提供一个选项。 要添加多个选项／下拉值，请重复以下步骤：
   ...
   1. 将以下属性添加到新创建的节点（此处为item1），然后单击&#x200B;**保存全部**:
      ...
      | 名称 | 类型 | 值 |
      |--- |--- |--- |
      | 文本 | 字符串 | 这是用户可见的下拉选项的值。 对于空值（默认值）保留为空，或输入值，如&#x200B;**International**&#x200B;或&#x200B;**Within US**。 |
...
### 方案：所有资产类型{#scenario-common-tab-for-all-asset-types}的公用选项卡
...
1. 对于在上一步中创建的选项卡节点（此处为公用选项卡），使用以下步骤创建一个名为item的节点：
...
1. 要在自定义选项卡(此处为收件人)中创建字段，请添加一个节点（此处为GeogralLocation）。 此属性与您创建的列相对应。 使用以下步骤创建字段（要创建更多字段／节点，请重复这些步骤。）: 
   ...
   1. 将以下属性添加到字段节点（此处为GeographicalLocation），然后单击&#x200B;**保存全部**。
...
1. 要视图自定义，请将指针悬停在相关资产（此处为字母）上，单击视图属性，然后单击&#x200B;**编辑**。 新选项卡(收件人)和字段(收件人的位置)显示在用户界面中。
...
1. 要视图自定义，请将指针悬停在相关资产（此处为文本）上，单击视图属性，然后单击&#x200B;**编辑**。 新选项卡和字段(收件人的位置)显示在用户界面中。
...
1. 对于字母，确保“叠加节点”对话框具有以下值。 对于其他资产类型，路径在下表中给出：
   ...
   **路径：** /libs/fd/cm/ma/gui/content/createasset/createletter/jcr:content/body/items/form/items/letterWizard/items/properties/properties/items/letterproperties/items
...
1. 在您创建的叠加项目节点下，创建名称为col4（或任何其他名称）的节点，然后单击&#x200B;**保存全部**。
...
1. 对于创建的每个属性，在列节点下创建一个节点，用于在用户界面中创建列。 重复此步骤以在UI中创建更多列：
   ...
   1. 确保“创建节点”对话框具有以下值，然后单击&#x200B;**确定**:
      ...
      **名称：** 您选择的名称（此处为GeographicalLocation）
...
1. （可选）默认情况下，新列在UI中显示为最后一列。 要使列显示在特定位置，请向列节点添加以下属性：
   ...
   | 名称 | 类型 | 值 |
   |--- |--- |--- |
   | sling:orderBefore | 字符串 | 路径“`/libs/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns`”的列节点的名称，在此之前，自定义列需要显示在UI中。 在此，如果希望“Geographical Location”列显示在“Version”列的前面（左侧），请将属性`sling:orderBefore`添加到路径“`/apps/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns/GeoLocation`”的GeoLocation节点，并将属性值设置为版本。 |
...
1. 转到`https://[server]:[port]/[ContextPath]/crx/de`并以管理员身份登录。
1. 转到`/oak:index/cmLucene`并在其下添加一个名为&#x200B;**聚合**&#x200B;的节点。
...
1. 在新建的聚合文件夹下，添加节点cm:resource。 在cm:resource下，添加一个名为include0的节点。
   ...
   1. 右键单击cm:resource文件夹，然后选择&#x200B;**创建** > **创建节点**。 确保“创建节点”对话框具有以下值，然后单击&#x200B;**确定**:
...
1. 要在资产创建页面上隐藏此属性，请向其添加以下属性，然后单击&#x200B;**保存全部**:

   | 名称 | 类型 | 值 |
   |---|---|---|
   | hideOnCreate | 布尔型 | 真 |

1. 要隐藏资产属性页面上的自定义属性，请向其添加以下属性，然后单击&#x200B;**保存全部**:
