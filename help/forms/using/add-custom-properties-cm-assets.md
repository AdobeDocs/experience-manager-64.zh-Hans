...
1. 在新创建的叠加节点下(/apps/fd/cm/ma/gui/content/cmmetadataproperties/commonproperties/col1/items)
   ...
   为需要为其创建nt:unstructured类型下拉列表（此处）的每个属性（字段）创 `geographicallocation`建一个节点。
...
1. 在属性节点（此处为地理分配）下，添加一个名称为新节点 `items`。 在项节点下，为下拉框中的值分别添加一个节点。 作为一种最佳实践，将第一个节点添加为空白，以用作下拉框的默认值，并为用户指定字段的无值选项。 要添加多个选项／下拉值，请重复以下步骤：
   ...
   1. 将以下属性添加到新创建的节点（此处为item1），然后单击“全 **部保存”**:
      ...
      | value | 字符串 | 存储在CRXDE中的文本值。 输入任意唯一关键字。 |
...
1. 对于创建的每个属性，在列节点下创建一个节点，以在用户界面中创建列。 重复此步骤以在UI中创建更多列：
   ...
   1. 将以下属性添加到节点，然后单击“全 **部保存”**。
      ...
      | jcr:title | 字符串 | 地理位置此值显示为UI中的列标题。 |
...
1. （可选）默认情况下，新列在UI中显示为最后一列。 要使列显示在特定位置，请向列节点添加以下属性：
   ...
   | 名称 | 类型 | 值 |
   |--- |--- |--- |
   | sling:orderBefore | 字符串 | 路径“”的列节点的名称，在`/libs/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns`此路径之前，自定义列需要显示在UI上。 在此，如果希望“Geographical Location”列显示在“Version”列的前面（在左侧），请将属性添加到路径“ `sling:orderBefore``/apps/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns/GeoLocation`”的GeoLocation节点，并将属性的值设置为version。 |
