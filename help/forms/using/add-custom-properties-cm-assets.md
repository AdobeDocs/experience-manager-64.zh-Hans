...
1. 在新创建的叠加节点下(/apps/fd/cm/ma/gui/content/cmmetadataproperties/commonproperties/col1/items)
   ...
   为需要为其创建nt:unstructured类型下拉列表（此处）的每个属 `geographicallocation`性（字段）创建一个节点。
...
1. 在属性节点（此处为地理分配）下，添加一个名为的新节点 `items`。 在项节点下，为下拉框中的值分别添加一个节点。 作为一种最佳实践，将第一个节点添加为空白，作为下拉框的默认值，并为用户指定字段的任何值提供一个选项。 要添加多个选项／下拉值，请重复以下步骤：
   ...
   1. 将以下属性添加到新创建的节点（此处为项1），然后单击“全 **部保存**”:
      ...
      | 名称 | 类型 | 值 |
      |--- |--- |--- |
      | 文本 | 字符串 | 这是用户可见的下拉选项的值。 将其留空（默认）值或输入值，如“国际” **或** “ **美国内”**。 |
      | value | 字符串 | 存储在CRXDE中的文本值。 输入任何唯一关键字。 |
...
1. 要为“字母”添加此选项卡，请在以下路径创建一个路径／结构类似于以下项目文件夹的叠加文件夹：
   ...
   要为字母或其他资产创建叠加，请使用以下路径，将资 [产类型] 替换为文本、条件、列表、dataditionary或片段：
...
1. （可选）默认情况下，新列在UI中显示为最后一列。 要使列显示在特定位置，请向列节点添加以下属性：
   ...
   | 名称 | 类型 | 值 |
   |--- |--- |--- |
   | sling:orderBefore | 字符串 | 路径“”的列节点的名称，在`/libs/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns`此路径之前，自定义列需要显示在UI上。 在此，如果希望“Geological Location”列显示在“Version”列的前面（左侧），请将属性添加到路径“ `sling:orderBefore` ”的GeoLocation节点，`/apps/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns/GeoLocation`并将属性的值设置为版本。 |
