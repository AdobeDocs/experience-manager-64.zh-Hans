...
1. 在EMC Documentum Content Server上，安装EMC NetWorker模块，接受所有默认值。
   ...
   在安装过程中，将提示您输入Content Server计算机的服务器名称作为NetWorker服 *务器名称*。 在为数据库安装EMC NetWorker模块时，请选择“完整”安装。

1. 使用下面的示例内容，创建一个名 *为nsrnmd_win.cfg的配置文件* ，并将其保存到Content Server上的可访问位置。 此文件将由备份和还原命令调用。

   以下文本包含换行符的格式。 如果将此文本复制到此文档外的某个位置，则一次复制一个部分，并在将其粘贴到新位置时删除格式字符。
   ...
   将配置文件密码字段保留 `NMDDE_DM_PASSWD` 为空。 您将在下一步设置密码。

1. 按如下方式设置配置文件密码：

   * 打开命令提示符，并更 *[改为NetWorker_root]*\Legato\nsr\bin。
...
1. 创建用于备份数据库的可执行批处理(.bat)文件。 （请参阅NetWorker文档。） 根据您的安装设置批处理文件中的详细信息。
   ...
   * 完整数据库备份(nsrnmddbf.bat):
      ...
      *[NetWorker数据库模块根]* &lt; `-s`*NetWorker服务器名称>用户名&#x200B;*`-U`*[密码&#x200B;]*`-P`*[]*&lt;数据库名`-l full`*称>*

   * 增量数据库备份(nsrnmddbi.bat):

      *[NetWorker数据库模块根]* &lt; `-s`*NetWorker服务器名称>用户名&#x200B;*`-U`*[密码&#x200B;]*`-P`*[]*&lt;数据库名`-l 1 -R`*称>*

   * 数据库日志备份(nsrnmddbl.bat):

      *[NetWorker数据库模块根]* &lt; `-s`*NetWorker服务器名称>用户名&#x200B;*`-U`*[密码&#x200B;]*`-P`*[]*&lt;数据库名`-l incr -R`*称>*
...
完成AEM表单数据的完整备份后，请执行以下任务。 (请参 [阅备份AEM表单数据](/help/forms/using/admin-help/backing-aem-forms-data.md#backing-up-the-aem-forms-data)。)

>[!NOTE]
>
>命令脚本需要您在准备EMC文档内容服务器进行备份和恢复中创建的 [nsrnmd_win.cfg文件的完整路径](backing-recovering-emc-documentum-repository.md#preparing-the-emc-document-content-server-for-backup-and-recovery)。

1. 打开命令提示符，并更 *[改为NetWorker_root]*\Legato\nsr\bin。
1. 运行以下命令：
...
在恢复AEM表单任务之前，请执行以下操作。 (请参 [阅恢复AEM表单数据](/help/forms/using/admin-help/recovering-aem-forms-data.md#recovering-the-aem-forms-data)。)
...
1. 打开命令提示符并更 *[改为NetWorker_root]*\Legato\nsr\bin
1. 运行以下命令：
