...
1. 如果服务有“配置”选项卡，则使用它更改服务的设置。 有关详细信息，请参阅以下链接的列表。
   ...
   >[!NOTE]
   >
   >并非“服务管理”页面上列出的所有服务都具有“配置”选项卡。 对于已创建的进程，仅当已在Workbench中向进程添加了配置参数时，才会显示配置选项卡。 (请参阅* Workbench帮助中的“配 [置参数](https://www.adobe.com/go/learn_aemforms_workbench_63) ”。)*

1. 单击“安全性”选项卡并设置服务的安全设置。 请参 [阅修改服务的安全设置](configure-service-settings.md#modifying-security-settings-for-a-service)。
1. 如果服务有“端点”选项卡，则使用它更改端点设置。 请参 [阅管理端点](/help/forms/using/admin-help/adding-enabling-modifying-or-removing.md)。
1. 单击池选项卡并设置池设置。 请参 [阅配置服务的池](configure-service-settings.md#configuring-pooling-for-a-service)。
1. 单击保存以保存更改，或单击取消以放弃更改。
1. 选中服务名称旁边的复选框，然后单击“开始”以重新启动服务。

## 审核工作流服务设置 {#audit-workflow-service-settings}

Workbench提供记录流程实例在运行时执行时的功能，然后回放它们以观察流程的行为。 (请参 [阅工作台帮](https://www.adobe.com/go/learn_aemforms_workbench_63)助。) 要节省表单服务器文件系统上的空间，可以限制记录存储的进程数据的量。 您可以配置审核工作流服务()的以下 `AuditWorkflowService`属性：

**maxNumberOfRecordingInstances:** 存储的最大录制内容数。 当存储最大数量时，当创建新录制内容时，最旧的录制内容将从文件系统中删除。 如果您倾向于创建许多录制内容并且希望自动删除旧录制内容，则此属性很有用。 默认值为 50。

**MaxNumberOfRecordingEntries:** 每个录制可存储的最大数据条目数。 数据条目是有关进程中操作的信息。 为操作的每次执行存储多个条目，例如操作是否开始、操作是否完成以及通向操作的路由是否完成。 当进程可以包括许多操作执行时（例如，遇到循环时），此属性很有用。 默认值为 50。

## barcoded forms service settings {#barcoded-forms-service-settings}

条形码表单服务从 `(BarcodedFormsService)` 扫描的图像提取条形码数据。 该服务接受条形码表单（TIFF或PDF）作为输入，并提取由条形码编码的数据的机器表示。

对于条码表单服务，可以使用以下设置。

**左阅读：** 选择后，将从右向左水平扫描条形码图像。

**阅读权：** 选择后，条形码图像将从左到右水平扫描。

**阅读：** 选择后，条形码图像将从底部到顶部垂直扫描。

**向下读取：** 选择此选项后，条形码图像将从上到下垂直扫描。
...
例如，如果默认路径设置为， `/LiveCycleES/ConnectorforEMCDocumentum/AssetLinkObjects`且表单模板存储在文件夹中，则资 `/Docbase/forms/`产链接对象存储在以下位置：
...
**资产链接对象默认路径：** IBM FileNet存储库中用于存储Asset Link对象的路径的默认部分。 实际路径由默认路径和表单模板在AEM表单存储库中的位置组成。

例如，如果默认路径设置为， `/LiveCycleES/ConnectorforIBMFileNet/AssetLinkObjects`且表单模板存储在文件夹中，则资 `/Docbase/forms/`产链接对象存储在以下位置：
...
“转换PDF”服 `ConvertPdfService`务()将PDF文档转换为PostScript和多种图像格式（JPEG、JPEG 2000、PNG和TIFF）。 将PDF文档转换为PostScript对于在任何PostScript打印机上基于服务器的无人值守打印都很有用。 在不支持PDF文档的内容管理系统中存档文档时，将PDF文档转换为多页TIFF文件非常实用。
...
**事务超时（秒）:** 基础事务提供程序在回滚正在打包此操作的事务之前应等待的秒数。 如果传播现有事务上下文，将忽略此值。 默认值为 180。

**平滑的阈值分辨率（以dpi为单位）:** 对文本、线状图和图像应用平滑（或消除锯齿）的图像分辨率，如果您为这些元素选择了“将平滑应用于”选项。

**对文本应用平滑：** 控制文本的消除锯齿。 要禁用文本平滑功能并使用屏幕放大镜使文本更清晰、更容易阅读，请清除此复选框。

**将平滑应用于LineArt:** 应用平滑以删除线条中的突变角度。

**对图像应用平滑：** 应用平滑以最大限度地减少图像中的突变。
...
FTP服务() `FTP`使流程能够与FTP服务器交互。 FTP服务操作可以从FTP服务器检索文件，将文件放在FTP服务器上，以及从FTP服务器删除文件。 例如，从进程生成的报告等文档可以存储在FTP服务器上进行分发。 或者，外部系统可以基于进程中的先前步骤生成一些文件。 在该过程的后续步骤中，文件可被传输到远程位置。
...
**默认用户名：** 用于访问FTP服务器的用户帐户的名称。 用户帐户必须具有足够的权限才能执行此服务需要的FTP操作。

**默认密码：** 要与指定用户名一起使用以在FTP服务器上进行身份验证的口令。
...
“指南实用程序”(Guide Utilities)操作允许您向流程添加以下“指南”(Guide)渲染任务:
...
**publicPaths:** 此选项已弃用。 请勿将此选项用于AEM表单。

**pathInfoExpiryInSeconds:** 客户端的路径信息请求到期的时间间隔。 默认值为1。

**contraulsExpiryInSeconds:** 从客户端请求抵押品的时间间隔。 默认为315576000。

**不匹配ExpiryInSeconds:** 客户端的附属品请求到期的时间间隔，当eTag（实体标记）不匹配时。 （eTag是HTTP响应头。） 默认值为1。
...
**macRefreshIntervalInMinutes:** 密钥的激活时间。 在此时间间隔内某个键处于活动状态时，将生成一个新键。 新键变为活动键。 以前处于活动状态的密钥将保留10%的刷新间隔。 此行为允许使用旧键生成的URL在键开关上继续工作。 默认值为144000。

**macOverlapIntervalInMinutes:** 生成新密钥后，前一个密钥将保持有效的时间长度。 默认为1440分钟（1天）。
...
**datasourceName:** 一个字符串值，它表示要用于连接到数据库服务器的数据源的JNDI名称。 数据源必须在承载表单服务器的应用程序服务器上定义。 默认值是AEM表单数据库的数据源的JNDI名称。
...
**其他属性：** 可传递到JNDI服务提供商的属性名称和值对。 这些属性取决于您所使用的提供程序的实现和配置。

属性名称和值对以分号分隔 **;**。 例如，以下文本显示将为名为name1和name2的两个属性指定的值，这两个属性分别具有值1和value2:

`name1=value1;name2=value2`

## LDAP服务设置 {#ldap-service-settings}

LDAP服务()提 `LDAPService`供查询LDAP目录的操作。 LDAP目录通常用于存储单位中人员、组和服务的相关信息。
...
*port* 是LDAP服务使用的通信端口。 默认值为389，这是用于LDAP连接的标准端口。
...
**生存时间：** 此设置用户档案有效并在客户端上缓存的时长（以秒为单位）。 默认值为86400（24小时）。 当客户端应用程序与服务器同步并且指定的时间已经过时，客户端应用程序会从服务器请求新的供应用户档案。
...
**脱机支持：** 使用户即使在与服务器没有连接时（例如，当他们超出单元范围或处于飞机模式时）也能继续使用移动客户端应用程序。 用户还必须在其移动设备上启用“脱机支持”设置。 此功能适用于Android和iOS设备。 默认情况下，此功能处于关闭状态。
...
**擦除前Android最大密码重试:** Android设备具有全局设置，可在指定数量的无效密码尝试后擦除系统。 此全局设置已启用且等于或低于此处指定的值。 默认值为 5。

**删除Android擦除：** 指定在Android设备上发生策略冲突时会发生什么情况。 选择此选项后，帐户将被删除。 如果未选择此选项，则删除存储的帐户密码和缓存的数据。 在用户修复策略违规之前，不会再进行任何同步尝试。
...
**事务超时（秒）:** 基础事务提供程序在回滚正在打包此操作的事务之前等待的秒数。 如果传播现有事务上下文，则忽略此值。
...
**服务器线程池计数：** 控制“生成PDF”服务在内部用于服务涉及插入（转换从主页访问的链接页面）的HTML到PDF转换请求的线程池大小。 默认值为 20。

**PDFG清除扫描秒数：** 有关详细信息，请参阅作业过期秒数部分。

**作业过期秒数：** “生成PDF”服务会在输入文件转换后立即删除它们。 它临时存储输出文件，时间长度由“PDFG清除扫描秒数”和“作业过期秒数”设置确定。
...
PDFG清除扫描秒数的默认值 `43200` 为（12小时）。 作业过期秒数的默认值 `86400` 为（24小时）。

**默认区域设置：** 用于覆盖部署“生成PDF”服务的服务器的默认区域设置（国家／地区+语言）。 如果未指定此参数，则从部署服务的操作系统中确定默认区域设置。 此参数控制将错误消息返回到API的语言。
...
存储库提供程序服务是充当提供程序服务的接口的服务委托。 这允许您连接到通用API，并且不必知道哪个提供程序服务正在实施存储功能。 存储库提供程序服务为存储库服务资源提供数据库存储。

资料库服务可使用以下设置。

**提供商服务：** 用作存储提供者的服务的名称。 默认值为RepositoryProviderService。
...
**认证时嵌入吊销信息：** 指定验证PDF文档时是否嵌入吊销信息。 默认值为false。

**在签名／认证过程中强制嵌入所有证书的吊销信息：** 指定如果未嵌入所有证书的有效吊销信息，则签名或认证操作是否失败。 请注意，如果证书不包含任何CRL或OCSP信息，则即使未检索吊销信息，该信息也被视为有效。 默认值为false。
...
**在验证过程中使用签名中存档的吊销信息：** 指定是否使用签名存档的吊销信息进行吊销检查。 默认值为true。
...
**代理主机：** 代理主机的URL。 仅在提供了一些有效值时使用。 无默认值。
...
**套接字超时：** 在套接字超时（等待数据传输时）发生之前等待的最长时间（以秒为单位）。 最小值为1，最大值为3600。 默认值为 30。

### 路径验证选项 {#path-validation-options}

**需要显式策略：** 指定该路径是否对与签署方证书的信任锚关联的至少一个证书策略有效。 默认值为false。

**禁止任何策略：** 指定如果策略对象标识符(OID)包含在证书中，是否应处理它。 默认值为false。
...
**在证书AIA中关注URI:** 指定在路径发现过程中是否处理证书AIA中的统一资源标识符(URI)。 默认值为false。

**CA证书中需要的基本约束扩展：** 指定CA证书是否必须存在证书颁发机构(CA)基本约束证书扩展。 一些早期的德国认证根证书（7及更早版本）不符合RFC 3280，并且不包含基本约束扩展。 如果已知用户的EE证书链接到这样的德语根，请取消选中此复选框。 默认值为true。

**在链构建过程中需要有效的证书签名：** 指定chain builder对用于构建链的证书是否要求有效签名。 选中此复选框后，chain builder将不会在证书上构建具有无效RSA签名的链。 请考虑链CA > ICA > EE，其中CA在ICA上的签名无效。 如果此设置为真，则链式构建将停止在ICA，而CA将不包括在链中。 如果此设置为false，则生成完整的3证书链。 此设置不影响DSA签名。 默认值为false。

### 时间戳提供者选项 {#timestamp-provider-options}

**TSP服务器URL:** 默认时间戳提供程序的URL。 仅在提供了一些有效值时使用。 无默认值。

**TSP服务器用户名：** 时间戳提供者需要的用户名。 仅在为URL提供了一些有效值时使用。 无默认值。

**TSP服务器密码：** 如果时间戳提供程序要求，上述用户名的口令。 仅在为URL和用户名提供了一些有效值时使用。 无默认值。

**请求哈希算法：** 指定创建时间戳提供程序请求时要使用的散列算法。 默认值为SHA1。

**吊销检查样式：** 指定用于根据时间戳提供程序证书观察到的吊销状态确定时间戳提供程序证书信任状态的吊销检查样式。 默认值为BestEffort。

**发送Nonce:** 指定是否使用时间戳提供程序请求发送nonce。 nonce可以是时间戳、网页上的访问计数器或旨在限制或防止文件未经授权的重放或再现的特殊标记。 默认值为true。

**验证期间使用过期的时间戳：** 选择此选项后，过期时间戳可用于检索签名的验证时间。 默认值为true。

**TSP响应大小：** TSP响应的估计大小（以字节为单位）。 此值应表示配置的时间戳提供程序可返回的时间戳响应的最大大小。 除非您完全确定，否则不要更改此设置。 最小值为60B，最大值为10240B。 默认值为4096B。

**忽略TimeStamp Server扩展**: 选择“忽 **略时间戳服务器扩展** ”选项，以阻止AEM Forms服务器联系指定的时间戳服务器。 选择此选项有助于避免由于AEM Forms和时间戳服务器之间的连接超时而发生的进程故障。

### 证书吊销列表选项 {#certificate-revocation-list-options}

**请首先参考本地URI:** 指定是否应优先于在证书中指定的用于吊销检查的任何位置，在本地URI或CRL查找中提供的CRL位置。 默认值为false。

**CRL查找的本地URI:** 本地CRL提供程序的URL。 仅当“Consult Local URI First”（查询本地URI First）设置设置为true时，才咨询此值。 无默认值。

**吊销检查样式：** 指定用于根据CRL提供程序证书观察到的吊销状态确定证书信任状态的吊销检查样式。 默认值为BestEffort。

**CRL查找的LDAP服务器：** 用于获取CRL的LDAP服务器(作为www.ldap.com)。 CRL的所有基于DN的查询都将定向到此服务器。 无默认值。

**联机：** 指定是否联机以获取CRL。 如果为false，则只咨询缓存的CRL（在本地磁盘上或嵌入签名的CRL）。 默认值为true。

**忽略有效日期：** 指定是否忽略响应的thisUpdate和nextUpdate时间，这可防止这些时间对响应有效性产生负面影响。 默认值为false。

**CRL中需要AKI扩展：** 指定CRL中是否必须包含授权密钥标识符扩展。 默认值为false。

### 联机证书状态协议选项 {#online-certificate-status-protocol-options}

**OCSP服务器URL:** 默认OCSP服务器的URL。 是否使用通过此URL指定的OCSP服务器取决于“要咨询的URL选项”设置。 无默认值。

**要咨询的URL选项：** 控制用于执行状态检查的OCSP服务器的列表和顺序。 默认值为UseAIAInCert。
...
**最大时钟偏斜时间：** 响应时间和本地时间之间允许的最大偏差（以分钟为单位）。 最小值为0，最大值为2147483647m。 默认值为5m。

**响应新鲜时间：** 预建的OCSP响应被视为有效的最长时间（以分钟为单位）。 最小值为1m，允许的最大值为2147483647。 默认值为525600（一年）。

**签署OCSP请求：** 指定是否应对OCSP请求进行签名。 默认值为false。

**请求签署方凭据别名：** 指定在启用签名时用于对OCSP请求进行签名的凭据别名。 仅在启用OCSP请求签名时使用。 无默认值。
...
**下次API调用时清除证书缓存：** 指定在调用下一个签名服务操作时是否清除证书缓存。 调用该操作后，此选项设置回false。 默认值为false。

**在下次API调用时清除CRL缓存：** 指定在调用下一个签名服务操作时是否清除CRL缓存。 调用该操作后，此选项设置回false。 默认值为false。

**在下次API调用时清除OCSP缓存：** 指定在调用下一个签名服务操作时是否清除OCSP缓存。 调用该操作后，此选项设置回false。 默认值为false。
...
**Cron表达式:** 石英使用的cron表达式计划输入目录的轮询。

**重复计数：** 轮询输入目录的次数。 如果端点配置中未指定此值，则使用的默认重复计数。 值-1表示对目录进行不定期扫描。 默认值为-1。

**重复间隔：** 每个投票之间的秒数为默认数。 除非在监视的文件夹端点配置中指定了其他值，否则此值将用作重复间隔。 默认值为 5。有关其他信息，请参阅批量大小设置的说明。

**异步：** 将调用类型标识为异步或同步。 瞬态和同步进程只能同步调用。 默认值为异步。

**等待时间：** 时间的默认值（以秒为单位），之后从输入文件夹检索文件。 如果文件或文件夹比等待时间中指定的时间早，则会选取它进行处理。 默认值为 0。
...
如果丢弃的文件量很大，请使批处理大小变大。 例如，如果监视文件夹端点调用的服务每分钟可处理700个文件，并且用户以相同的速率将文件放入输入文件夹，则将“批处理大小”设置为350，将“重复间隔”设置为30秒将帮助监视文件夹的性能，而不会导致过多扫描监视文件夹的成本。
...
在群集配置中，监视文件夹端点的批处理大小不会缩放到多个群集节点。 例如，如果将两个节点群集的批处理大小 `2` 设置为，并且选择了“限制”选项，则节点将集体处理两个节点的文件，而不是每个节点一次处理两个文件。

**覆盖重复文件名：** 一个布尔字符串，它指定监视的文件夹是否覆盖重复结果文件名以及是否应覆盖保留的同名文档。

**保留文件夹：** 保留文件夹的默认值。 此文件夹用于在输入处理成功时将源文件复制到中。 此值可以是空路径、相对路径或具有文件模式的绝对路径，如“结果文件夹”设置中所述。

**失败文件夹：** 复制失败文件的文件夹的名称。 此值可以是空路径、相对路径或具有文件模式的绝对路径，如“结果文件夹”设置中所述。
...
例如，如果2009年7月17日晚上8点，而您指定 `C:/Test/WF0/failure/%Y/%M/%D/%H/`，则结果文件夹为 `C:/Test/WF0/failure/2009/07/17/20`。

如果路径不是绝对的，而是相对的，则在监视的文件夹内创建文件夹。 有关文件模式的详细信息，请参 [阅关于文件模式](/help/forms/using/admin-help/configuring-watched-folder-endpoints.md#about-file-patterns)。

>[!NOTE]
>
>结果文件夹的大小越小，“监视文件夹”的性能就越好。 例如，如果监视文件夹的估计负载是每小时1000个文件，请尝试类似的模式， `result/%Y%M%D%H` 以便每小时创建一个新的子文件夹。 如果负载较小（例如，每天1000个文件），您可以使用类似的模式 `result/%Y%M%D`。

**暂存文件夹：** 监视文件夹内的舞台文件夹的默认名称。
...
Web服务()使 `WebService`进程能够调用Web服务操作。
...
**密钥存储类型：** 密钥库的类型。 不提供任何值，以使用为运行表单服务器的JVM配置的默认密钥库类型。 否则，请提供以下值之一：
...
**信任存储类型：** 信任存储的类型。 不提供任何值，以使用为运行表单服务器的JVM配置的默认密钥库类型。 否则，请提供以下值之一：
...
### 从安全用户档案中删除主体 {#remove-the-principal-from-a-security-profile}

1. 在“服务管理”页上，选择要配置的服务。
1. 单击“安 **全** ”选项卡，选择要删除的安全用户档案，然后单击“ **删除”**。
