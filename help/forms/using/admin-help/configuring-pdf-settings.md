...
* 创建或编辑您之前创建的PDF设置。 (请参 [阅添加或编辑PDF设置](configuring-pdf-settings.md#add-or-edit-pdf-settings)。)
* 指定默认PDF设置。 (请参 [阅更改默认设置](/help/forms/using/admin-help/configuring-file-type-settings.md#change-the-default-settings))
* 将PDF设置文件上传到服务器。 (请参 [阅上传PDF设置](configuring-pdf-settings.md#upload-pdf-settings)。)
* 删除自定义PDF设置。 (请参阅 [删除PDF设置](configuring-pdf-settings.md#delete-pdf-settings)。)
* 上传和下载序文和尾文件。 (请参 [阅上传和下载序文件和尾文](configuring-pdf-settings.md#uploading-and-downloading-prologue-and-epilogue-files)件。)
...
**印刷质量：** 创建用于高质量印刷制作的PDF文件（例如，在照排机或制版机上）。 在这种情况下，不考虑文件大小。 目标是维护PDF文件中的所有信息，商业打印机或印前服务提供商需要这些信息才能正确打印文档。 这组选项：
...
**绑定：** 指定是显示具有左侧还是右侧绑定的PDF文件。 此设置影响对开页——连续版面中页面的显示和缩览图的并排显示。

**解决方案：** 为输入文件设置打印机分辨率的模拟，这些输入文件根据打印到的打印机的分辨率调整其行为。 对于大多数输入文件，分辨率设置越高，PDF文件越大但质量越高，设置越低，PDF文件越小但质量越低。 最常见的是，分辨率决定渐变或混合中的步骤数。 您可以输入一个介于72到4000之间的值。 除非您计划将PDF文件打印到特定打印机并要模拟原始输入文件中定义的分辨率，否则将此设置保留为默认设置。
...
**优化以实现快速Web视图:** 重新构建文件，以便从Web服务器进行逐页下载（字节服务）。 无论您在“图像”选项卡上选择了何种压缩设置，此选项都可压缩文本和线状图。 通过压缩，从Web或网络下载文件时访问和查看的速度更快。 默认情况下，此选项未启用。
...
“图像”选项指定图像的压缩和重新取样。 您可以尝试这些选项，在文件大小与图像质量之间找到适当的平衡。 有关访问“图像”设置的说明，请参 [阅添加或编辑PDF设置](configuring-pdf-settings.md#add-or-edit-pdf-settings)。

这些选项配置彩色、灰度和单色图像：

**缩减采样：** 为每种类型的图像设置一个值。 要对彩色、灰度或单色图像进行缩减像素采样，PDF Generator会将采样区域中的像素组合成一个更大的像素。 以每英寸点数(dpi)为输出设备提供分辨率，并在“对于上方的图像”框中输入dpi分辨率。 对于分辨率高于此阈值的图像，PDF生成器会根据需要组合像素，以将图像的分辨率（每英寸像素数）降低到指定的dpi设置。 要关闭缩减采样，请选择关闭。 以下是选项：

**平均缩减采样为：** 在采样区域中平均像素，并用指定分辨率下的平均像素颜色替换整个区域。

**双立方缩减采样：** 使用加权平均来确定像素颜色，并且通常比简单的平均缩减采样方法产生更好的结果。 双立方是最慢但最精确的方法，它会产生最平滑的色调渐变。

**子采样至：** 选择样本区域中心的像素，并以指定分辨率用该像素替换整个区域。 与缩减采样相比，次采样可显着缩短转换时间，但会导致图像不那么平滑和连续。
...
**消除锯齿为灰色：** 使单色图像中的锯齿边缘变平滑。 选择2位、4位或8位可指定4、16或256级灰色。 （消除锯齿可能会模糊小文字或细线。）
...
“字体”选项指定要嵌入PDF文件中的字体以及是否嵌入PDF文件中使用的字符子集。 有关访问“字体”选项的说明，请参 [阅添加或编辑PDF设置](configuring-pdf-settings.md#add-or-edit-pdf-settings)。

>[!NOTE]
>
>当您将PDF文件与同一字体子集合时，PDF Generator会尝试合并字体子集。

**嵌入所有字体：** 嵌入文件中使用的所有字体。 PDF/X规范需要字体嵌入。

**当使用的字符百分比小于时，子集化嵌入字体：** 如果选择此选项，请指定一个阈值百分比以仅嵌入字体的子集。 例如，如果阈值为35且使用的字符数不到35%，则PDF生成器仅嵌入这些字符。 只嵌入具有适当权限位的字体。
...
>[!NOTE]
>
>TrueType字体可包含字体设计器添加的用于阻止字体嵌入PDF文件的设置。
...
**保持颜色不变：** 保持与设备相关的颜色不变，并将与设备无关的颜色保留为PDF中最接近的可能等效颜色。 此选项对于打印已校准其所有设备的商店、使用该信息在文件中指定颜色以及仅输出到这些设备非常有用。
...
**文档渲染方法：** 在色彩空间之间映射色彩的方法。 任何特定方法的结果都取决于色彩空间的用户档案。 例如，一些用户档案用不同的方法产生相同的结果。 这些选项可用：
...
**感知：** 在原始像素映射到目标色域时保持它们之间的相对颜色值。 此方法保留颜色之间的视觉关系，尽管颜色值本身可能会发生变化。
...
**CMYK:** 定义文件中所有CMYK图像的色彩空间。 默认为US Web Coated(SWOP)v2。 还可以选择“无”以阻止CMYK图像转换。

>[!NOTE]
>
>为所有三个工作空间选择“无”与选择“保持颜色不变”的效果相同。

**为已校准的CMYK色彩空间保留CMYK值：** 选中后，与设备无关的CMYK值将被视为与设备相关(DeviceCMYK)值，将丢弃与设备无关的色彩空间，PDF/X-1a文件使用“将所有颜色转换为CMYK”值。 如果取消选择，则如果颜色管理策略设置为“将所有颜色转换为CMYK”，则与设备无关的色彩空间将转换为CMYK。
...
**保留底色去除和黑色生成：** 如果PostScript文件中存在这些设置，则保留这些设置。 黑色生成计算您尝试重现特定颜色时要使用的黑色量。 底色去除(UCR)会减少青色、洋红色和黄色组分的量，以补偿黑色代添加的黑色量。 由于使用的墨水较少，UCR通常用于新闻纸和无涂层纸。

**当找到传输函数时：** 确定在找到传输函数时要执行的操作：

**保留：** 保留传统上用于补偿在将图像传输到胶片时可能发生的点增益或点损失的传递函数。 当构成打印图像的墨点比半色调网屏大（例如，由于在纸张上展开）时，会产生网点增大； 当点打印得更小时，点会丢失。 使用此选项，传输函数将作为文件的一部分保留，并在输出文件时应用于文件。

**应用：** 不保留传输函数，而是将其应用到文件，文件会更改文件中的颜色。 此选项对于在文件中创建色彩效果很有用。 默认情况下，为新设置选中此选项。

**删除：** 删除所有已应用的传输函数。 除非PDF文件将输出到源PostScript文件创建时所用的同一设备，否则请删除已应用的传输函数。
...
**保留第2级文本页面语义：** 使用在LanguageLevel 2 PostScript中而不是在LanguageLevel 3 PostScript中定义的文本页运算符。 如果您有一个PostScript文件并选择此选项，则文案页运算符将复制该页面。 如果未选择此选项，则执行与showpage操作等效的操作，但图形状态未重新初始化。

**保留叠印设置：** 保留转换为PDF的文件中的任何叠印设置。 叠印颜色是两个或多个相互印刷的油墨。 例如，当青色油墨在黄色油墨上打印时，生成的叠印为绿色。 如果不进行叠印，则不会打印底色黄色，从而生成青色。
...
**在PDF文件中保存可移植作业票证：** 在PDF文件中保留PostScript作业票证。 作业票证包含有关PostScript文件的信息，如页面大小、分辨率和陷印信息，而不是有关内容的信息。 此信息可在以后的工作流程中使用，或用于打印PDF。

**使用Prolog.ps和Epilog.ps:** 发送每个作业的序文和尾文件。 这些文件有很多用途。 例如，可以编辑prolog文件以指定封面。 可以编辑尾文件以解析PostScript文件中的一系列过程。 您可以上传或下载文件。 （请参阅上传和下载序文件和跋文件。）

**处理DSC注释：** 维护PostScript文件中的DSC信息。 以下子选项可用：

**记录DSC警告：** 在处理过程中显示有问题的DSC注释的警告消息，并将它们添加到日志文件。

**保留DSC中的EPS信息：** 保留信息，如EPS文件的原始应用程序和创建日期。 如果取消选择此选项，则页面将根据页面左上角对象的左上角和右下角对象的右下角调整大小并居中。
...
符合PDF/A规范的文件主要用于存档。 由于长期保存是目标，因此文档必须只包含在文档的整个预定生命周期中打开和查看所需的内容。 例如，符合PDF/A规范的文件只能包含文本、光栅图像和矢量对象； 它们不能包含加密和脚本。 此外，必须嵌入所有字体，才能打开文档并以创建方式查看。 换言之，与PDF/A兼容的文档 *比* PDF/X相同的更薄，后者适用于高端制作。
...
**不符合以下条件时：** 指定如果PostScript文件不符合PDF/X要求，是否创建PDF文件。 当“标准报告和规范”页上的“规范标准”设置为非“无”选项时，此选项可用。

**继续：** 创建PDF文件。

**取消作业：** 仅当PostScript文件满足所选报告选项的PDF/X要求，并且在其他情况下有效时，才创建PDF文件。 如果同时选择了PDF/X报告选项，且PostScript文件仅满足一组PDF/X条件（例如，PDF/X-3），则PDF Generator将创建符合标准的文件。

**如果既未指定裁切框，也未指定艺术框：** 当“标准报告和规范”页上的“规范标准”设置为非“无”选项时可用。

**报告为错误：** 如果选择了某个报告选项，并且任何页面中都缺少修剪框或艺术框，则将PostScript文件标记为不符合。

**将裁切框设置为具有偏移的媒体框：** 如果既未指定修剪框也未指定艺术框，则根据相应页面媒体框的偏移量计算修剪框的点值。 修剪框始终比封闭的媒体框小或小。

**如果未指定出血框：** 当“标准报告和规范”页上的“规范标准”设置为非“无”选项时可用。

**将出血框设置为媒体框：** 如果未指定出血框，则使用出血框的媒体框值。

**将出血框设置为带偏移的裁切框：** 如果未指定出血框，则根据相应页面的裁切框的偏移量计算出出血框的磅值。 出血框总是比封闭的裁切框大或大。

**如果未在文档中指定默认值：** 当“标准报告和规范”页上的“规范标准”设置为非“无”选项时，此选项可用。

**输出方法用户档案名称：** 指示文档所准备的特定打印条件。 如果文档未指定OutputIntent名称，则PDF生成器将使用此菜单中的选定值。 您可以选择提供的名称之一，或在提供的空格中输入名称。 如果您的工作流要求文档指定输出方法，请选择“无”。 任何不符合要求的文档都无法进行规范检查。

**输出条件标识符：** 指示由输出方法用户档案名的注册表指定的引用名称。

**输出条件：** 描述预期的打印条件。 此条目对PDF文档的预期接收者非常有用。

**注册表名称(URL):** 指示网址，以了解有关注册表的详细信息。 会自动为ICC注册表名称输入URL。

**陷阱：** 指示文档中的陷印状态。 PDF/X规范要求值为True或False。 如果文档未指定陷印状态，则使用此处提供的值。 如果您的工作流要求文档指定陷印状态，请选择“保留未定义”。 任何不符合要求的文档都无法进行规范检查。
...
**输出方法用户档案名称：** 指示已准备文档的特定打印条件，要求符合PDF/A规范。 如果您的工作流要求文档指定“输出方法”信息，请选择“无”。 如果未提供此信息，文档将无法进行规范检查。
...
**将窗口调整为初始页面：** 根据您在“文档选项”下选择的选项调整文档窗口以紧贴打开页面。

**屏幕上居中窗口：** 将窗口放在屏幕区域的中心。
...
### 上传序文件或尾文件 {#upload-a-prologue-or-epilogue-file}
