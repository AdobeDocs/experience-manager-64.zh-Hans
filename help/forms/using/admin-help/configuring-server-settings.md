...
默认情况下，AEM表单发送的电子邮件包含指向(JEE上的AEM表单已弃用)Flex工作区的链接。 您可以配置AEM表单以发出包含指向AEM Forms工作区的链接的电子邮件。 要进一步了解AEM Forms工作区比(JEE上的AEM表单已弃用)Flex工作区的优势，请参 [阅本](/help/forms/using/features-html-workspace-available-flex.md) 文。
