...
然后，配置WebLogic，使其使用您创建的自定义标识密钥库和自定义信任密钥库。 另外，禁用WebLogic主机名验证功能，因为用于创建密钥库文件的辨别名不包括承载WebLogic的计算机的名称。
...
<table> 
 ...
 <tbody>
  ...
  <tr> 
   ...
   <td><p>RSA</p><p>您可以根据公司的策略使用不同的算法。</p></td> 
  </tr> 
  <tr> 
   <td><p>-keystore</p></td> 
   <td><p>keystore文件的位置和名称。</p><p>该位置可以包括文件的绝对路径。 或者，它可以相对于输入keytool命令的命令提示的当前目录。</p></td> 
   <td> 
    <ul> 
     <li><p>自定义标识密钥库： <code>[</code><i>appserverdomain]</i><code>/adobe/</code><i>[服务器名]</i><code>/ads-ssl.jks</code></p></li> 
     <li><p>自定义信任密钥库： <code>[</code><i>appserverdomain]</i><code>/adobe/</code><i>[服务器名]</i><code>/ads-ca.jks</code></p></li> 
    </ul></td> 
  </tr> 
  <tr> 
   <td><p>-file</p></td> 
  ...
  <tr> 
   ...
   <td> 
    <ul> 
     ...
     <li><p>自定义信任密钥库：使用与用于自定义标识密钥库的口令相同的口令。</p></li> 
    </ul></td> 
  </tr> 
  <tr> 
   <td><p>-keypass</p></td> 
   <td><p>保护密钥对的私钥的密码。</p></td> 
   <td><p>请使用与选项相同的口 <code>-storepass</code> 令。 密钥密码必须至少为6个字符。</p></td> 
  </tr> 
  <tr> 
   <td><p>-dname</p></td> 
   <td><p>标识密钥库所有者的可辨别名。</p></td> 
...
1. 在命令提示符下，导航到 *[appserverdomain]*/adobe/*[server name]*。
1. 输入以下命令：
   ...
   在appserverdomain [/adobe/server name目录中创建名为“ads-credentials.jks”的自定义标识密钥]库文件[] 。
...
1. 输入以下命令，从ads-credentials密钥库提取证书：
   ...
   >[!NOTE]
   >
   >替 `[JAVA_HOME]` 换为安装JDK的目录，并将 `store`*_*`password`*替换为自定义标识密钥库的口令。*
   ...
   名为“ads-ca.cer”的证书文件在appserverdomain [/adobe/]server name目录中创&#x200B;[*建*] 。
...
1. 通过输入以下命令将证书插入新密钥库文件（自定义信任密钥库）:
   ...
   >[!NOTE]
   >
   >用 `[JAVA_HOME]` 安装JDK的目录替换，用您自己的密码替换 `store`*_*`password`_和`key`*_*`password`*_密码。*
...
1. 在“标识”下，指定以下值：
   ...
   **自定义标识密钥库密码短语**: *mypassword* （自定义标识密钥库密码）
...
1. 在“常规”下，在“配置”中，选 **择SSL**。
1. 默认情况下，Keystore会为“标识和信任位置”选择。 否则，将其更改为keystore。
1. 在“标识”下，指定以下值：
