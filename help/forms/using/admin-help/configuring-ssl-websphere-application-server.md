...
1. （Linux和Solaris）通过在命令提示符下输入( `pwconv` 无参数)来创建卷影密码文件。
   ...
   >[!NOTE]
   >
   >（Linux和Solaris）要使WebSphere Application Server本地OS安全注册表正常工作，必须存在卷影密码文件。 卷影密码文件通 **常命名为/etc/shadow***，并基于/etc/passwd文件。 如果卷影密码文件不存在，则在启用全局安全性并将用户注册表配置为本地操作系统后，将发生错误。*
...
1. 打 [开aem-forms_root]\crx-repository\launchpad\sling.properties文件进行编辑。
