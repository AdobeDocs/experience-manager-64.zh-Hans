---
title: 控制对受策略保护文档的访问
seo-title: 控制对受策略保护文档的访问
description: 了解如何查看、管理和控制对受策略保护文档的访问。
seo-description: 了解如何查看、管理和控制对受策略保护文档的访问。
...
## 查看有关文档{#view-information-about-a-document}的信息
...
>[!NOTE]
>
>必须使用“文档详细信息”页上的“策略名称”链接来访问在Microsoft Outlook中为附加到电子邮件的文档的收件人自动生成的策略。 这些策略不会显示在策略页面上。

**文档名称：** 所选文档的名称。

**文档ID:** 在将策略应用于文档时为文档安全分配的唯一标识符。文档安全使用此编号来跟踪文档。

**文档状态：** 文档的状态（例如，活动或已吊销）。

**发布者：** 将策略附加到文档的用户的名称。

**策略名称：** 用于保护文档的策略名称。您可以单击名称以打开策略。 您必须使用此链接访问Acrobat为Outlook中附加到电子邮件的文档收件人生成的策略。 这些策略不显示在“策略”页面上。
...
## 查看文档{#view-related-iterations-for-a-document}的相关小版本
...
1. 单击查看相关小版本。 仅当启用了该功能时，该选项才可用。 将出现相关小版本的列表。 对于每个小版本，您可以查看以下信息：
   ...
   * **创建者：** 保存小版本的用户。
   * **创建日期：** 保存小版本的日期和时间。
   * **策略：** 用于保护迭代的策略。不同的迭代可能会受到不同策略的保护。

1. 要显示该小版本的“文档详细信息”(Document Detail)页，请单击小版本的文件名。

## 撤消和恢复对文档{#revoking-and-reinstating-access-to-documents}的访问

您可以撤消和恢复对受策略保护文档的访问权限：

**用户：** 可以撤销或恢复对使用其自己的个人策略或已为应用策略的用户启用了撤消功能的共享策略所保护文档的访问权限。无法撤消对文档的访问权限或切换策略的用户需要联系管理员。

**管理员：** 可以撤消或恢复对任何受策略保护的文档（包括受个人或共享策略保护的文档）的访问权限。如果管理员撤销对使用共享策略保护的文档的访问权限，则只有管理员才能恢复该文档的访问权限。

**策略集协调员：** 可以撤消或恢复策略集中策略所保护文档的访问权限。
...
**所有文档：** 管理员和用户保护的所有文档。使用“所有文档”选项进行搜索可能会返回一长串文档。
...
## 对文档列表{#sort-the-document-list}进行排序
...
## 向受策略保护的文档{#add-cover-page-to-policy-protected-documents}添加封面
...
**Protect带有封面的文档：** 使用指定的策略保护PDF文档，并在该文档中添加封面

**提取受保护的文档：** 从带有封面的PDF文档中提取受策略保护的PDF文档

使用以下文档安全API:

**protectDocumentWithCoverPage:** 使用指定的策略保护给定的PDF，并返回包含封面的文档和作为附件的受保护文档
