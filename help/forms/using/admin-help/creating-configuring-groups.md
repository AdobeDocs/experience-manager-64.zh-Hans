...
* 允许根据电子邮件域(如@adobe.com)自动创建动态组。 启用此功能后，用户管理会为AEM Forms数据库中的每个唯一电子邮件域创建一个动态组。 使用cron表达式指定用户管理在AEM表单数据库中搜索新电子邮件域的频率。 这些动态组将添加到DefaultDom本地域，并且名为“所有具有*[电子邮件域] *mail ID的用户”。
