...
PDF Generator提供了对某些类型的文件启用多线程文件转换的功能。 多线程文件转换允许PDF Generator同时执行多个转换，从而改进了PDF Generator的性能。
...
默认情况下，PDF Generator一次只能转换一个OpenOffice、Microsoft word或PowerPoint文档。 如果启用多线程转换，PDF生成器可以同时转换多个文档。 PDF Generator将启动多个OpenOffice或PDFMaker实例（用于执行Word和PowerPoint转换）。
...
OpenOffice或PDFMaker的每个实例都使用单独的用户帐户启动。 您添加的每个用户帐户必须是对表单服务器计算机具有管理权限的用户。 在群集环境中，同一组用户必须对群集的所有节点有效。
...
在Windows操作系统上，用于PDF转换（PDFG用户）的管理员用户帐户将需要替换进程级令牌权限。 您可以使用组策略编辑器添加此权限：

1. 在Windows的“开始”菜单中，单击“运行”，然后输入gpedit.msc。
...
1. 在/etc/sudoers文件中为其他用户（运行表单服务器的管理员除外）添加条目。 例如，如果您以名为lcadm的用户和名为myhost的服务器的身份运行AEM表单，并且要模拟user1和user2，请向/etc/sudoers添加以下条目：
   ...
   此配置使lcadm能以“user1”或“user2”身份在主机“myhost”上运行任何命令，而不提示输入密码。
...
1. 同样在/etc/sudoers文件中，通过在行的开头添加数字符号(#)来找到此行并注释掉它：
...
1. 允许您通过添加用户帐户添 [加的所有用户连接到表单服务器](enabling-multi-threaded-file-conversions.md#add-a-user-account) 。 例如，要允许名为user1的本地用户具有与表单服务器建立连接的权限，请使用以下命令
...
>[!NOTE]
>
>OpenOffice必须安装在所有PDFG用户都可以访问的目录位置中。 您可以以PDFG用户身份登录并检查是否可以无问题地启动OpenOffice来验证这一点。

### 添加用户帐户 {#add-a-user-account}

1. 在管理控制台中，单击“服务”>“PDF生成器”>“用户帐户”。
1. 单击“添加”，然后输入对表单服务器具有管理权限的用户的用户名和密码。 如果要配置OpenOffice用户，请关闭初始的OpenOffice激活对话框。
...
### 将用户从用于多线程文件转换的列表中删除 {#remove-a-user-from-the-list-used-for-multi-threaded-file-conversions}
