...
title: 管理流程
seo-title: 管理流程
description: “进程列表”页面显示用户已启动或已自动启动的进程。 了解有关管理流程的更多信息。
seo-description: “进程列表”页面显示用户已启动或已自动启动的进程。 了解有关管理流程的更多信息。
...
“进程列表”页面显示用户已启动或已自动启动的进程。

1. 在管理控制台中，单击“服务”>“表单工作流”>“表单工作流”。 进程列表显示以下信息：
...
1. 单击进程名称，在“进程实例”页上查看其进程实例。

## 使用进程实例 {#working-with-process-instances}

如果从“进程列表”页面访问“进程实例”页，则会列出所选进程的所有进程实例。 如果在执行搜索后访问“进程实例”页，则仅列出找到的进程实例。
...
* 选择进程实例以查看其详细信息，如其操作和子进程。 选择进程实例时，将显示“进程实例详细信息”页。
* 暂停、取消暂停或终止进程实例。
* 搜索进程实例。 要开始搜索，请单击“搜索”。
...
**注意**:当 *请求更改进程实例的状态（例如暂停或终止）时，请求将进入表单工作流的命令队列。 根据队列的大小和整体处理速度，在重新加载页面一次或多次之前，显示的状态不会更改。*
...
当您终止进程实例时，其状态会变为TERMINATING，然后变为TERMINATING，并且进程将停止在其当前操作处。 不会运行任何其他操作，并且所有关联的操作和任务都会终止。
...
* 选择操作可查看其详细信息。 选择操作时，将显示“操作详细信息”页。
...
如果某个操作或子进程已停止运行，或遇到某些其他错误情况，或者如果需要强制某个操作或子进程停止运行，则可以终止它。
