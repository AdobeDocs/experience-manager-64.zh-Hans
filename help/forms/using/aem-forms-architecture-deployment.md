...
如果您从LiveCycle ES4升级，则此拓扑与您在LiveCycle中已有的拓扑紧密镜像，除了在JEE上向AEM Forms添加内置的AEM author之外。 此外，对于执行升级的客户，群集需求没有变化。 如果您在群集环境中使用AEM表单，则可以在AEM 6.4表单中继续使用相同的功能。 对于使用HTML Workspace的AEM Forms of JEE的全新安装，运行内置到JEE环境的AEM作者实例是一项附加要求。
