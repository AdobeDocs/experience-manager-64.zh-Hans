...
seo-description: 自定义应用程序图标、应用程序名称、启动图像和登录页面，为AEM Forms应用程序提供特定于组织的不同外观。
...
### 要自定义图标并启动图像{#to-customize-icon-and-launch-images}，请执行以下操作

#### 对于iOS {#for-ios}

1. 在Xcode中打开`Capture.xcodeproj`项目。
1. （***要自定义图标***）在Capture的导航器视图中，导航到&#x200B;**[!UICONTROL Capture > Capture > Supporting Files > Capture-info.plist]**。 单击图标文件旁边的下拉菜单。 指定图标文件(.png)的名称，并在&#x200B;**[!UICONTROL Capture > Capture > Resources > icons]**&#x200B;上传文件。 当前支持的维度包括：29x29、50x50、58x58、72x72、100x100和144x144。
1. （***用于自定义launch图像***）请确保图像的文件名为：
...
## 自定义登录页面{#customizing-images-on-the-login-page}上的图像
...
1. 要更改徽标，请将默认的`LC-logo.png`文件替换为自定义的`LC-logo.png`文件。
...
1. 导航到`www/wsmobile/images`文件夹。
1. 要更改徽标，请将默认的`aem_icon.png`文件替换为自定义的`aem_icon.png`文件。
1. 在iOS设备或iOS模拟器上构建和运行AEM Forms应用程序。
