...
# 内联条件，并在交互式通信和字母中重复{#inline-condition-and-repeat-in-interactive-communications-and-letters}
...
有关更多信息，请参阅[Interactive Communications](/help/forms/using/texts-interactive-communications.md)文本中的以文本创建规则。
...
1. 执行以下操作以构建一个条件，用于评估所选/条件化文本是否显示在信件中，然后点按复选标记以保存表达式：
   ...
   有关创建表达式的更多信息，请参阅[表达式生成器](/help/forms/using/expression-builder.md)中的&#x200B;**使用表达式生成器**&#x200B;创建表达式和远程函数。 数据字典中的元素必须支持表达式中指定的值。 有关更多信息，请参阅[数据字典](/help/forms/using/data-dictionary.md)。
