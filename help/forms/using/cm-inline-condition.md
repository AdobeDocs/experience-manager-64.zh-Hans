...
有关详细信息，请参阅在交互通信中以文本 [形式创建规则](/help/forms/using/texts-interactive-communications.md)。
...
1. 执行以下操作以构建一个条件，用于评估所选／条件化文本是否显示在字母中，然后点按复选标记以保存表达式:
   ...
   有关创建表达式的详细信息，请参 **阅使用表达式生成器创建表达式和远** 程功能 [，在](/help/forms/using/expression-builder.md)表达式生成器中。 表达式中指定的值必须支持数据字典中的元素。 有关详细信息，请参 [阅数据字典](/help/forms/using/data-dictionary.md)。
