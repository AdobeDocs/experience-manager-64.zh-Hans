...
# 内联条件，并在Interactive Communications和字母{#inline-condition-and-repeat-in-interactive-communications-and-letters}中重复
...
### 示例：使用规则在交互通信{#example-using-rules-to-conditionalize-inline-text-in-interactive-communication}中条件化内联文本

要在交互通信中对句子、段落或文本字符串进行条件化，可以在相应的文本文档片段中创建规则。 以下示例使用规则仅向交互通信的美国收件人显示免费电话号码。

有关详细信息，请参阅[交互通信中的文本](/help/forms/using/texts-interactive-communications.md)中的文本创建规则。
...
>[!NOTE]
>
>如果您的现有资产包含旧条件／重复表达式（6.2 SP1 CFP 4之前），则资产将显示旧的条件语法并重复。 但是，旧条件／重复可以正常工作。 新旧条件／重复表达式相互兼容，以创建新旧条件／重复表达式的嵌套混合。
...
1. 执行以下操作以构建一个条件，用于评估所选／条件化文本是否显示在字母中，然后点按复选标记以保存表达式:
   ...
   有关创建表达式的详细信息，请参阅[表达式生成器](/help/forms/using/expression-builder.md)中的&#x200B;**使用表达式生成器**&#x200B;创建表达式和远程功能。 表达式中指定的值必须支持数据字典中的元素。 有关详细信息，请参阅[数据字典](/help/forms/using/data-dictionary.md)。
...
1. 预览相关字母，并注意文本会根据内联条件呈现。 您可以使用以下方式输入DD元素“性别”的值：
   ...
   * 附加到相关数据字典的XML数据文件。
   ...
   有关详细信息，请参阅[数据字典](/help/forms/using/data-dictionary.md)。
...
### 示例：在信件格式、结构和显示信用卡交易列表{#example-using-repeat-in-a-letter-to-format-structure-and-display-a-list-of-credit-card-transactions}中使用重复
...
### 在条件{#repeat-within-condition}中重复
...
### 内联条件{#empty-inline-condition}为空
