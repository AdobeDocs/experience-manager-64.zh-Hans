...
description: 了解如何配置Adobe Analytics以发现用户在使用自适应表单、自适应文档和HTML5表单时遇到的交互模式和问题。
...
1. 登录 [https://sc.omniture.com/login/](https://sc.omniture.com/login/)
...
1. 找到 **报表包ID** 字段和添加新的报表包ID。 例如，JJEsquire。 报表包ID显示在报表包ID字段的下方。 它包含自动前缀，该前缀通常为公司名称。

1. 新增 **网站标题**. 例如，JJEsquire快速入门套件。 此标题在Analytics UI中使用。 在代码中使用报表包ID。

1. 选择 **时区** 从下拉菜单中。 进入此报表包的所有数据都会根据定义的时区进行记录。

1. 离开 **基本URL** 和 **默认页面** 字段为空。 这两个值仅用于从Adobe Marketing Cloud界面链接到您的网站。
1. 离开 **起始日期** 设置为今天。 起始日期确定激活报表包的日期。

1. 在 **估计每日页面查看次数** 字段，键入100。 使用此字段可估计您预计网站每天的页面查看次数。 此估计值允许Adobe放置适当数量的硬件来处理您要收集的数据。

1. 选择 **基本货币** 从下拉菜单中。 此报表包中的所有货币数据都将以此货币格式转换和存储。

1. 单击 **创建报表** 套件。 您应会看到页面刷新，并显示一条消息，指出您的报表包已成功创建。
...
1. 在“一般帐户设置”屏幕中，启用 **地理位置报告**，然后单击 **保存**.
1. 导航到 **编辑设置** > **流量** > **流量变量**.
...
1. 导航到 **编辑设置** > **转化** >  **成功事件**. 定义并启用以下成功事件：
   ...
   >[!NOTE]
   >
   >用于配置AEM Forms分析的事件编号和prop编号必须不同于 [AEM analytics](/help/sites-administering/adobeanalytics.md) 配置。
...
1. 以管理员身份登录AEM创作实例。
1. 在左上角，单击 **Adobe Experience Manager** > **工具** ![工具](assets/tools.png)> **Cloud Services** > **旧版Cloud Services**.
...
1. 单击 **[+]** 您的Analytics配置旁边的链接。

   ![Adobe Analytics配置](assets/adobe-analytics-cloud-services.png)
   **图：** *Adobe Analytics配置*

1. 键入 **标题** 和 **名称** 对于框架，选择 **Adobe Analytics** 框架，然后单击 **创建**. 随即会打开框架进行编辑。

1. 在侧面板的报表包部分，单击 **添加项目**，然后使用下拉列表选择框架将与之交互的报表包ID（例如JJEsquire）。
...
1. 在创作实例上，打开AEM Web Console配置管理器，网址为https://&lt;*服务器*>:&lt;*端口*>/system/console/configMgr。
1. 找到并打开AEM Forms Analytics配置

   ![AEM Forms Analytics配置服务](assets/analytics_configuration.png)
   **图：** *AEM Forms Analytics配置服务*
...
1. 为以下字段指定适当的值，然后单击 **保存**.
   ...
   * **获取报表超时：** 指定等待服务器响应分析报表的持续时间（以秒为单位）。 默认时间为120秒。
...
1. 登录AEM Portal( `https://[hostname]:[port]`.
1. 单击 **Forms > Forms与文档**，选择表单或文档，然后单击 **启用Analytics**. 分析已启用。

   ![为表单或文档启用分析](assets/enable-analytics-1.png)
   **图：** *为表单启用分析*

   **A.** “启用Analytics”按钮 **B.** 选定的表单
