...
在通信 [管理中创建信函模板后](/help/forms/using/create-letter.md)，最终用户／代理／声明调整器可以在“创建通信”用户界面中打开信函，并通过输入数据、设置内容和管理附件创建通信。 最后，理赔员或代理人可以在预览模式下管理内容并提交信件。
...
1. 对于基于预览的字母，选择“ **** ”> **预览**。 或者，对于非基于数据字典的字母，选择 **预览**。 您还可以将鼠标悬停在字母上（无需选择它），然后点按字母预览图标以预览它。

   >[!NOTE]
   >
   >如果数据字典与字母不关联，则字母预览打开。 否则，如果字母是基于预览字典的，则“对应管理”会在“预览”菜单中显示“”和“自定义”选项，您可以选择两个选项之一。 还可以将测试数据与数据字典相关联。 当数据 [字典具有关联的测试数据](/help/forms/using/data-dictionary.md#p-working-with-test-data-p)，然后选择预览选项时，将打开普通预览并填充测试数据。
...
1. 要在预览通信时进行渲染，您应是管理员或以下某个组的一部分：
   ...
   * cm-agent-users（用于发布实例上的再现）
   ...
   如果您不具备所需的权限，请请求管理员提供相应的访问权限。 有关创建用户并将用户添加到用户组的更多信息， [请参阅将用户或用户组添加到用户组](/help/sites-administering/security.md)。 如果您尝试在没有相应权限的情况下呈现通信，则会显示404错误页。

1. 如果已选择“ **预览** ” **>“自**&#x200B;定义”，则会打开对话框。 在对话框中，选择与数据字典对应的预览文件，将字母，然后选择 **预览**。 基于特定字母的数据字典创建数据文件。 有关数据文件的详细信息，请参 [阅数据字典](/help/forms/using/data-dictionary.md#p-working-with-test-data-p)。
...
1. 根据需要，在字段中输入数据和内容变量。 填写标有星号(&amp;ast;)的所有必填字段，以启用“提 **交** ”按钮。

   点按HTML字母预览中的数据字段值，以在“数据”选项卡中高亮显示相应的数据字段。

   ![在字母2_1](assets/2_enterdata.png) _ ![enterdata中输入数据](assets/2_1_enterdata.png)

### 管理内容 {#managecontent}

在内容选项卡中，管理字母中的内容，如文档片段和内容变量。

1. Select **Content**. “对应管理”显示信件的内容选项卡。
...
1. 根据需要，在“内容”选项卡中编辑内容模块。 要将焦点放在内容层次结构中的相关内容模块，您可以点按字母预览中的相关行或段落，或直接点按内容层次结构中的内容模块。
   ...
   在“内容”或“数据”选项卡中，点按HTML字母预览左上角的“高亮选择的模块”( ![highlightselectedmodulesincontentccr](assets/highlightselectedmodulesincontentccr.png))，可以禁用或启用在字母预览中选择相关文本、段落或数据字段时转至内容／数据模块的功能。

   有关“创建对应”用户界面中各个模块可用的操作的详细信息，请参 [阅“创建对应”用户界面中的操作和信息](create-letter.md)。

1. 要定位内容模块，请使用“查找”字段。 输入内容模块的完整或部分名称或标题，以在对应中搜索该模块。
1. 点按列表、文 ![本](assets/display.png)、条件或目标区域前面的显示图标（显示），以在字母中显示或隐藏它。
1. 要编辑内联或可编辑的文本模块，请点按相 **关的** “编辑”图 ![标](assets/edittextmodule.png)(edittextmodule)，或在字母预览中按住多次单击相关文本模块。

   系统会显示一个文本编辑器，用于编辑和格式化文本。

   浏览器中的默认拼写检查器在文本编辑器中检查拼写。 要管理拼写和语法检查，您可以编辑浏览器的拼写检查器设置或安装浏览器插件／加载项来检查拼写和语法。

   您还可以使用文本编辑器中的各种键盘快捷键来管理、编辑和设置文本格式。 有关“对应管理键 [盘快捷键](/help/forms/using/keyboard-shortcuts.md#correspondence-management) ”中文本编辑器键盘快捷键的详细信息。
   ...
   >[!NOTE]
   >
   >有关为文本模块和字母设置制表符间距的更多信息，请参 [阅使用制表符间距排列文本的更多信息](https://helpx.adobe.com/aem-forms/kb/cm-tab-spacing-limitations.html)。
...
1. 如果需要，在通信中插入特殊字符。 例如，您可以使用“特殊字符”调板插入：
   ...
   * 标点符号，‟如和
   ...
   Commendergence Management内置了210个特殊字符。 管理员可 [通过自定义添加对更多／自定义特殊字符的支持](/help/forms/using/custom-special-characters.md)。

1. 要在可编辑的内嵌模块中突出显示\突出显示部分文本，请选择文本，然后点按高亮颜色。

   ![背景颜色](assets/letterbackgroundcolor.png)

   您可以直接点按“基本颜 **[色]** ”调板中存在的基本颜色A，或在使用滑块B后点按“选 **择”，以选****[]** 择相应的颜色阴影。

   或者，您也可以转到高级选项卡，选择适当的色相、明度和饱和 **[度]** C以创建精确的颜色，然后点按选择 **[D]** 以应用颜色来高亮显示文本。

   ![textbackgroundcolor-1](assets/textbackgroundcolor-1.png)

1. 进行适当的内容和格式更改，然后点 **按保存**。 点按( ![editnextmoduleccr](assets/editnextmoduleccr.png))可在可编辑文本模块之间移动 **，或点按保存并下一步可** 保存更改，并移至下一个可编辑文本模块。
...
1. 正确填充未填充的变量后，未填充的变量的计数将设置为0。
   ...
   * 布尔变量字段默认为 *false*。 但是，该变量被视为未填充。 这意味着变量计数包括所有值为false的布尔变 *量字段*。
   * 数字变量字段默 *认为0（零）*。 但是，该变量被视为未填充。 这意味着变量计数包括值为0（零）的所 *有数字变量字段*。



#### “创建对应内容”选项卡中的“操作和信息” {#actions-and-info-available-in-the-create-correspondence-content-tab}
...
1. 要高亮显示字母中的所有可编辑内容，请点按高 **亮显示可编辑部分**。
...
1. 根据需要，在“内容”选项卡中编辑内容模块。 要将焦点放在内容层次结构中的相关内容模块，您可以点按字母预览中的相关行或段落，或直接点按内容层次结构中的内容模块。
   ...
   通过点按内容中的高亮显示选 ![定的模块(highlightselectedmodulesincontentccr](assets/highlightselectedmodulesincontentccr.png))，您可以禁用或启用在字母预览中点击相关文本、段落或数据字段时在“内容”选项卡中突出显示内容模块的功能。

   有关“创建对应”用户界面中各个模块可用的操作的详细信息，请参 [阅“创建对应”用户界面中的操作和信息](create-letter.md)。

1. 要向字母中添加分页符，请点按要插入分页符的位置，然后选择“分页前”或“分页后”( ![分页前](assets/pagebreakbeforeafter.png))。
...
1. 要将信件另存为草稿（以后可以继续处理），请点按另存为草稿。 要使用此选项，您的信件需要 [发布](/help/forms/using/publishing-unpublishing-forms.md#publishanasset)。 有关详细信息，请参阅保存草稿 [和提交信函实例下的草稿实例](#savingdrafts)。

   ![saveasdraft](assets/saveasdraft.png)

   将出现“起草字母名称”对话框，其中带有字母实例id。 您可以选择编辑此ID。 记下字母Id，然后点按完 **成**。 您以后可以使用此ID重 [新加载草稿字母](submit-letter-topostprocess.md#reloaddraft)。
...
1. 如果您认为拼合的PDF预览令人满意，请点 **按** “提交”以提交信函。 或者，要更改信件，请点 **击“退出** ”预览，返回信件的“创建对应UI”预览，以更改信件。 点按提交时，如果在发布实例上启用了管理信函实例配置，则会生成提交信函实例。
...
* **草稿实例**:草稿实例捕获您正在预览的字母的当前状态。 要保存草稿实例，请首先确保字母引用的所有资产处于“已发布”状态。 有关发布信件的信息，请参阅 [发布资产](/help/forms/using/publishing-unpublishing-forms.md#publishanasset)。 您需要先发布字母，然后才能将其保存为草稿，因为当您发布字母时，您会创建该字母的版本、其从属资产和数据。 信件的已发布版本不能由您或其他用户编辑，稍后可以恢复，而不会与已发布版本出现任何意外差异。 您以后可以返回到此实例，并从离开的位置继续。
...
有关浏览器与字母预览的兼容性的详细信 [息，请参阅停止NPAPI浏览器插件及其影响](https://helpx.adobe.com/aem-forms/kb/discontinuation-of-npapi-plugins-impact-on-aem-forms.html)。

要更改信函的预览模式，请完成以下步骤：

1. 如有必 `https://[system]:[port]/system/console/configMgr` 要，请转到并以管理员身份登录。
1. 转到“ **[!UICONTROL 对应管理配置]** ”>“ **[!UICONTROL 再现类型]** ”，然后选 **择“HTML再现** （默认）”或“ **** PDF再现”。
1. 单击&#x200B;**[!UICONTROL 保存]**。
