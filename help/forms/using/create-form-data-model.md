...
有关表单数据集成和表单数据模型的更多信息，请参 [阅AEM表单数据集成](/help/forms/using/data-integration.md)。
...
* MySQL数据库，其示例数据在创建第一个自适应表单的“先决条件” [部分中指定](/help/forms/using/create-your-first-adaptive-form.md)
...
您可以配置不同类型的数据源以创建表单数据模型。 在本教程中，我们将配置您配置的MySQL数据库，并填充示例数据。 有关其他受支持数据源的信息以及如何配置它们，请参 [阅AEM Forms数据集成](/help/forms/using/data-integration.md)。
...
1. 将MySQL数据库配置为数据源：
   ...
   1. 在配置对话框中，指定以下详细信息：
      ...
      * **** 借阅测试：启用“借 **阅时测试** ”选项。
      * **** 返回时测试：启用“ **返回时测试** ”选项。
      * **** 验证查询：指定SQL SELECT查询以验证池中的连接。 查询必须至少返回一行。 例如，选 **择&amp;ast;客户详细信息**。
...
AEM Forms提供了一个直观的用户界面，用 [于从配置的数](/help/forms/using/data-integration.md#main-pars-header-1524967585)据源创建表单数据模型。 您可以在表单数据模型中使用多个数据源。 对于我们的用例，我们将使用已配置的MySQL数据源。

执行以下操作以创建表单数据模型：

1. 在AEM作者实例中，导航到“表 **单** ”>“数 **据集**&#x200B;成”。
