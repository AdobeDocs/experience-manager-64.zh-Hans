...
1. 在布局中插入所选数据模块后，您可以点按配置模式，并在点按每个模块的编辑图标后设置以下内容：
   ...
   1. **缩进**:增加或减少信件中模块/内容的缩进。缩进以级别（从0开始）指定。 每个级别缩进36点。 有关自定义表单的更多信息，请参阅[Forms工作流](/help/forms/using/submit-letter-topostprocess.md#formsworkflow)中的&#x200B;**[!UICONTROL 通信管理配置]**。
