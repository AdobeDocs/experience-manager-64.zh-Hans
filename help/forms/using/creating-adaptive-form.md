...
1. 访问AEM Forms创作实例： `https://[server]:[port]/<custom-context-if-any>.`
...
1. 在AEM登录页面上输入您的凭据。
   ...
   登录后，在左上角，点按 **[!UICONTROL Adobe Experience Manage > Forms > Forms与文档]**.

   >[!NOTE]
   >
   >对于默认安装，登录名为 `admin` 密码是 `admin`.

1. 点按 **[!UICONTROL 创建]** 选择 **[!UICONTROL 自适应表单]**.
...
1. 此时会显示“添加属性”选项。 指定以下属性字段的值。 标题和名称字段是必填字段：
   ...
   * **[!UICONTROL 标记：]** 指定用于唯一标识自适应表单的标记。 标记有助于搜索表单。 要创建标记，请在 **标记** 框中。
...
1. 点按 **[!UICONTROL 打开]** 在新选项卡中打开新创建的表单。 此时将打开表单进行编辑，并显示模板中可用的内容。 它还会显示侧栏，以根据需要自定义新创建的表单。
   ...
   根据自适应表单的类型，关联的XFA表单模板、XML架构或JSON架构中存在的表单元素显示在 **[!UICONTROL 数据模型对象]** 选项卡 **[!UICONTROL 内容浏览器]** 中。 您还可以拖放这些元素以构建自适应表单。

   有关自适应表单创作界面和可用组件的信息，请参阅 [创作自适应表单简介](/help/forms/using/introduction-forms-authoring.md).
...
[AEM Forms数据集成](/help/forms/using/data-integration.md) 允许您集成多个数据源，并将其实体和服务整合在一起，以创建表单数据模型。 它是JSON模式的扩展。 您可以使用表单数据模型创建自适应表单。 在表单数据模型中配置的实体或数据模型对象可用作表单创作的数据模型对象。 它们绑定到相应的数据源，用于预填表单并将提交的数据写回相应的数据源。 您还可以使用自适应表单规则调用在表单数据模型中配置的服务。
...
>[!NOTE]
>
>[上传XFA表单模板](/help/forms/using/get-xdp-pdf-documents-aem.md) 在您开始基于表单模板创建自适应表单之前，请先将其发送到AEM Forms。

执行以下操作以使用XFA表单模板作为自适应表单的表单模型：

1. 在 **[!UICONTROL 添加属性]** 页面，打开 **[!UICONTROL 表单模型]** 选项卡。
1. 在“表单模型”选项卡的下拉列表中，选择 **[!UICONTROL 表单模板]**. 所有通过AEM Forms UI上传到存储库的表单模板都将列出以供选择。 从列表中选择模板。
...
1. 点按 **[!UICONTROL 选择架构]** 并执行以下操作之一：
   ...
   * **[!UICONTROL 从磁盘上传]**  — 选择此选项，然后点按上传架构定义，以从文件系统浏览并上传XML架构或JSON架构。 上传的架构文件驻留在表单中，其他自适应表单无法访问该文件。
...
>[!NOTE]
>
>当您打开使用高级模板创建的用于编辑的自适应表单时，会显示一条错误消息。 高级模板具有签名步骤组件，默认情况下为其启用Acrobat Sign。 创建并选择 [Acrobat Sign云配置](/help/forms/using/adobe-sign-integration-adaptive-forms.md) 和 [配置签名者](/help/forms/using/working-with-adobe-sign.md#addsignerstoanadaptiveform) 以解决错误。
