...
表单门户组件为Web开发人员提供了组件，可在使用Adobe Experience Manager(AEM)创作的网站上创建和自定义表单门户。 有关表单门户的快速概述，请参 [阅在门户上发布表单的介绍](/help/forms/using/introduction-publishing-forms.md)。
...
![](assets/component-categories.png) 组件浏览器中的AEM Forms门户组件&#x200B;****&#x200B;图：表 *单门户组件类别*
...
![](assets/fp-grid-viw.png) 在页面中搜索和列表组件图&#x200B;****&#x200B;中：在具 *有网格布局的页面中搜索和制表器组件*
...
![](assets/edit-list.png) 编辑模式下的列表窗格&#x200B;****&#x200B;图：编 *辑模式下的列表窗格*
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>添加项目</td> 
   <td>配置使用AEM Forms UI上传资产的文件夹。 默认情况下，它会列出所有已上传的资产。 有关AEM Forms UI的详细信息，请参阅管 <a href="/help/forms/using/introduction-managing-forms.md" target="_blank">理表单的简介</a>。</td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td> </td> 
   <td>页面文本</td> 
   <td><p>配置页面文本(例如，第 <strong>1 </strong>页，共51页)。 The default value is <strong>Page</strong>.</p> <p>例如，如果在此字段中 <strong>指定应 </strong>用程序表单，且有51页，则页面文本将变为应用程序表单 <strong>1(共51 </strong>页)。</p> </td> 
  </tr> 
  <tr> 
   <td> </td> 
   <td>Of 文本</td> 
...
![](assets/search-with-predicates.png) 带有日期、全文、属性和标记的搜索面板谓词&#x200B;****&#x200B;图：包含 *日期、全文、属性和标记谓词的“搜索”面板*
...
1. 指定以下内容：
   ...
   * **** 类型：唯一可用的选项是“上次 **[!UICONTROL 修改日期”]**。
   * **** 文本：日期谓词组件的标签或题注。 默认值为上次修 **[!UICONTROL 改日期]**。
   * **** 开始日期标签：开始日期字段的标签或题注。
   * **** 结束日期标签：结束日期字段的标签或题注。
...
1. 在“选 **[!UICONTROL 项]** ”选项卡中，点 **[!UICONTROL 按添加项目]**。
...
要列出处于草稿状态的表单以及已提交的表单，请使用“草稿和提 **[!UICONTROL 交”组件]** 。 有关详细信息，请参阅自 [定义草稿和提交组件](/help/forms/using/draft-submission-component.md)。
...
![](assets/search_lister.png) “搜索”和“制表人”组件的屏幕截图&#x200B;****&#x200B;图：搜索 *和制表器组件*

## 自定义表单门户页面 {#customizing-a-forms-portal-page-br}

可以自定义表单门户页面，为页面提供独特的外观。 您还可以添加元数据以改进搜索体验、更改页面布局和添加自定义CSS样式。 有关详细信息，请参 [阅自定义Forms Portal组件的模板](/help/forms/using/customizing-templates-forms-portal-components.md)。

AEM Forms UI允许您向表单添加自定义元数据。 自定义元数据在向最终用户提供列表和搜索表单体验时非常有用。 有关自定义元数据的详细信息，请参 [阅自定义Forms Portal组件的模板](/help/forms/using/customizing-templates-forms-portal-components.md)。

开箱即用的表单门户提供渲染操作。 您可以自定义表单门户以添加更多操作。 有关详细信息，请参 [阅对表单制表人项目添加自定义操作。](/help/forms/using/add-custom-action-form-lister.md)
