...
![AEM Forms Portal组件在组件浏览器](assets/component-categories.png)
**中的组件图：** *Forms Portal组件类别*
...
![页面中的搜索和制表人组](assets/fp-grid-viw.png)
**件图：** *使用网格布局的页面中的搜索和制表人组件*
...
**[!UICONTROL 编辑]**&#x200B;对话框包括若干选项卡，这些选项卡提供下表所述的配置选项。 完成后，点按&#x200B;**[!UICONTROL 确定]**&#x200B;以保存配置。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>添加项目</td> 
   <td>配置使用AEM Forms UI上传资产的文件夹。 默认情况下，它会列表所有上传的资产。 有关AEM Forms UI的详细信息，请参阅<a href="/help/forms/using/introduction-managing-forms.md" target="_blank">管理表单简介</a>。</td> 
  </tr> 
  <tr> 
...
![包含日期、全文、属性和标记的“搜索”面](assets/search-with-predicates.png)
**板谓词图：** *包含日期、全文、属性和标记谓词的“搜索”面板*
...
1. 指定以下内容：
   ...
   * **[!UICONTROL 文本：]** 日期谓词组件的标签或题注。默认值为&#x200B;**[!UICONTROL 上次修改日期]**。
   * **[!UICONTROL 开始日期标签：]** 开始日期字段的标签或题注。
   * **[!UICONTROL 结束日期标签：]** 结束日期字段的标签或题注。
   * **[!UICONTROL 隐藏：]** 要强制使用默认日期过滤器来列表表单。
...
您可以自定义表单门户页面，为页面提供不同的外观。 您还可以添加元数据以改进搜索体验、更改页面布局和添加自定义CSS样式。 有关详细信息，请参阅[自定义Forms Portal组件的模板](/help/forms/using/customizing-templates-forms-portal-components.md)。

AEM Forms UI允许您向表单添加自定义元数据。 自定义元数据对于向最终用户提供列表和搜索表单体验非常有用。 有关自定义元数据的详细信息，请参阅[自定义Forms Portal组件的模板](/help/forms/using/customizing-templates-forms-portal-components.md)。
