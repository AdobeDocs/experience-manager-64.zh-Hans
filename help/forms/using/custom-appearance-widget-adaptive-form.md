...
# 为自适应表单域{#create-custom-appearances-for-adaptive-form-fields}创建自定义外观
...
1. 根据需要更新`integration/javascript`文件夹中的JavaScript文件。
   ...
   * 当`commit`发生事件时，自适应表单框架将调用`getCommitValue`方法。 通常，它是退出事件，但下拉菜单、单选按钮和复选框元素在更改时出现的情况除外)。 有关详细信息，请参阅[自适应Forms表达式](/help/forms/using/adaptive-form-expressions.md#p-value-commit-script-p)。
