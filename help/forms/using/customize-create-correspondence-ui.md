...
title: 自定义创建对应UI
seo-title: 自定义创建对应UI
description: 了解如何自定义创建对应的UI。
seo-description: 了解如何自定义创建对应的UI。
...
![](assets/0_1_introscreenshot.png) “创建对应UI”图中的自定义图&#x200B;****&#x200B;标：创 *建对应UI中的自定义图标*

### 在创建对应UI中更改徽标 {#changing-the-logo-in-the-create-correspondence-ui}

要设置您选择的徽标图像，请执行以下操作：

1. 在CRX中创建相 [应的文件夹结构](#creatingfolderstructure)。
1. [上传您在CRX中创建的文件夹中的新标志文件](#uploadlogo) 。

1. [在CRX上设置CSS](#createcss) ，以引用新徽标。
1. 清除浏览器历史记录并 [刷新“创建对应UI”](#refreshccrui)。
...
1. 在apps文件夹中，创建一个名为的文件夹， `imgs` 其路径／结构与imgs文件夹（位于ccrui文件夹中）类似。
...
将自定义徽标文件上传到CRX。 标准HTML规则控制标志的呈现。 支持的图像文件格式取决于您访问AEM Forms时所使用的浏览器。 所有浏览器都支持JPEG、GIF和PNG。 有关详细信息，请参阅浏览器特定的文档，其中介绍了支持的图像格式。

* 标志图像的默认尺寸为48 px &amp;ast;48 px. 确保图像与此大小或大于48 px &amp;ast;48 px.
...
1. 在“创建文件”对话框中，输入CustomLogo.png作为文件的名称（或徽标文件的名称）。
...
1. 在“编辑jcr:data”对话框中，单击“浏 **览** ”，然后选择要用作标志的图像文件（此处为CustomLogo.png）。
   ...
   支持的图像文件格式取决于您访问AEM Forms时所使用的浏览器。 所有浏览器都支持JPEG、GIF和PNG。 有关详细信息，请参阅浏览器特定的文档，其中介绍了支持的图像格式。

   ![示例自定义徽标文件](assets/geometrixx-outdoors.png)
   **** 图：示 *例——要用作自定义标志的CustomLogo.png*
...
1. 在以下位置创建一个名为customcss.css的文件（您不能使用其他文件名）:
   ...
   1. 在“新建文件”对话框中，将CSS的名称指定为(您 `customcss.css`不能使用其他文件名)，然后单击“确 **定”**。
...
## 刷新“创建对应UI”以查看自定义徽标 {#refreshccrui}

清除浏览器缓存，然后在浏览器中打开创建对应UI实例。 您应当看到您的自定义徽标。

![](assets/0_1_introscreenshot-1.png) 使用自定义徽标创建对应的用户界面&#x200B;****&#x200B;图：创 *建对应UI中的自定义图标*
