...
通信管理使用根据数据字典的结 `` ``构存储的后端、客户或收件人特定数据来创建用于不同客户的通信。 例如，可以使用友好名称创建文档，如“尊敬的{First Name}”、“先生”。 {姓氏}&quot;.
...
1. 预 [览字母时](/help/forms/using/create-letter.md#p-types-of-linkage-available-for-each-of-the-fields-p)，作者选择将字母预览为数据(自定义预览)。 此时将打开该字母，其中预填充作者提供的数据。 这在创建对应界面中打开。 预览此信函的代理可以修改此信函中的内容、数据和附件，并可提交最终信函。 有关创建字母的详细信息，请参阅 [创建通信](/help/forms/using/create-letter.md)。

## 先决条件 {#prerequisite}

安装“ [兼容性包](https://helpx.adobe.com/in/experience-manager/6-4/forms/using/compatibility-package.html) ”，在“ **Forms** ”页上 **视图“数** 据字典”选项。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>DDE类型- COLLECTION<br /> -A DDE节点是在从父COLLECTION节点捕获信息的COLLECTION DDE旁创建的。 为简单／复合数据类型的集合创建相同的内容。 只要您有复合类型的COLLECTION,“数据字典”树就会捕获为捕获类型信息而创建的DDE的子项中的组成字段。<br /> - DDE(COLLECTION)<br /> - DDE(COMPOSITE for type info)<br /> - DDE(STRING)字段1<br /> - DDE(STRING)字段2<br /> <br /> </p> </td> 
...
1. 点按 **警报** 消息中的确定。
   ...
   “对应管理”根据所选数据字典的结构创建XML文件，并将其下载到名为&lt;data-dictionary-name>-SampleData的计算机。 现在，您可以在XML或文本编辑器中编辑此文件，以在创建字母时 [进行数据输入](/help/forms/using/create-letter.md)。
