...
通信管理使用根据数据字典的结构存储的 `` ``后端、客户或收件人特定数据来创建用于不同客户的通信。 例如，可以使用友好名称创建文档，如“尊敬的{First Name}”、“先生”。 {姓氏}&quot;.

通常，商业用户不需要了解元数据表示法，如XSD(XML模式)和Java类。 但是，他们通常需要访问这些数据结构和属性才能构建解决方案。

### 数据字典工作流程 {#data-dictionary-workflow}

1. 作者通 [过上传模式](#createdatadictionary) ，或从头开始创建数据字典。
1. 作者根据数据字典创建字母和交互式通信，并根据需要将字母和交互式通信中的数据字典元素关联起来。
1. 作者可以下载基于数据字典的示例数据XML文件。 作者可以修改样本数据XML文件，该文件可以作为测试数据与数据字典相关联。 在字母预览期间也会使用同样的方法。
1. 在预 [览字母时](/help/forms/using/create-letter.md#p-types-of-linkage-available-for-each-of-the-fields-p)，作者选择将字母与数据预览(自定义预览)。 此时将打开预填充作者提供的数据的字母。 这将在创建对应界面中打开。 预览此信函的代理可以修改此信函中的内容、数据和附件，并可提交最终信函。 有关创建字母的详细信息，请参阅 [创建通信](/help/forms/using/create-letter.md)。

## 先决条件 {#prerequisite}

安装“ [兼容性包](https://helpx.adobe.com/in/experience-manager/6-4/forms/using/compatibility-package.html) ”，以在“表单”页 **面上视图“数** 据字典”选项 **** 。

## 创建数据字典 {#createdatadictionary}

您可以使用数据字典编辑器创建数据字典，也可以上传XSD模式文件以创建基于该数据字典。 然后，您可以通过添加更多必需信息（包括字段）来扩展数据字典。 无论数据字典的创建方式如何，业务流程所有者都不需要后端系统的知识。 业务流程所有者只需要对域对象及其定义的知识，即可对其流程进行操作。
...
1. 在“属性”屏幕中，添加以下内容：
   ...
   * **标题：** （可选）输入数据字典的标题。 标题不需要唯一，并且可以包含特殊字符和非英语字符。 字母和其他文档片段与标题（如果可用）一起引用，例如在缩略图和资产属性中。 数据字典引用的是其名称而非标题。
...
1. （可选）要上传数据字典的XSD模式定义，请点按“数据字典结构”窗格下的“上 **传XML模式”**。 浏览至XSD文件，选择它，然后点按打 **开**。 根据上传的XML模式创建数据字典。 您需要调整数据字典中元素的显示名称和说明。 为此，请点按元素名称，然后在右侧窗格的字段中编辑其说明、显示名称和其他详细信息，以选择这些元素的名称。

   有关计算的DD元素的详细信息，请参 [阅计算的数据字典元素](#computedddelements)。

   >[!NOTE]
   >
   >您可以跳过上传模式文件，然后使用用户界面从头开始构建数据字典。 为此，请跳过此步骤并继续执行后续步骤。
...
1. 在“添加属性”屏幕中，将元素添加到数据字典。 如果已上传模式以获取数据字典的基本结构，则还可以添加／删除元素并编辑其详细信息。
   ...
   * 父DDE（复合）不能有两个同名的子代。
   * enums仅包含简单字符串类型。
   有关合成、集合和基元元素以及使用数据字典元素的详细信息，请参 [阅将数据字典元素映射到XML模式](#mappingddetoschema)。
...
1. 从数据字典的列表中，选择适当的数据字典。 UI会显示复制图标。
1. 点按复制。UI会显示粘贴图标。
...
1. 创建数据条目后，在预览包含测试数据的字母时可以使用此XML文件。
   ...
   因此，在此之后，当您正常预览字母（而非自定义）时，该XML数据会用在字母中。 您还可以点按自定义，然后上传此XML。

## 示例 {#samples}

以下代码示例显示了数据字典的实施详细信息。

### 可上传到数据字典的示例模式 {#sample-schema-that-can-be-uploaded-to-the-data-dictionary}
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>表达式</td> 
   <td>字符串</td> 
   <td>“计算”DDE的表达式。 默认提供的表达式评估服务支持JSP EL表达式。 您可以用自定义实现替换表达式服务。</td> 
  </tr> 
  <tr> 
   <td>valueSet</td> 
   <td>列表</td> 
...
数据字典还可以包括计算元素。 计算数据字典元素始终与表达式相关联。 计算此表达式以在运行时获得数据字典元素的值。 计算的DDE值是其他DDE值或文本的函数。 默认情况下，支持JSP表达式语言(EL)表达式。 EL表达式使用${ }字符，而有效的表达式可以包括文本、运算符、变量（数据字典元素引用）和函数调用。 在表达式中引用数据字典元素时，将使用DDE的引用名称。 对于数据字典中的每个数据字典元素，引用名称是唯一的。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>DDE类型——复合<br /> </p> </td> 
   <td>java.util.Map<br /> </td> 
  </tr> 
  <tr> 
   <td><p>xs:element where maxOccurs &gt; 1<br /> </p> </td> 
   <td><p>DDE类型- COLLECTION-<br /> DDE节点创建在COLLECTION DDE旁边，该DDE从父COLLECTION节点捕获信息。 为简单／复合数据类型的集合创建相同的内容。 只要您有复合类型的COLLECTION,“数据字典”树就会捕获为捕获类型信息而创建的DDE的子项中的组成字段。<br /> - DDE(COLLECTION)<br /> - DDE(COMPOSITE for type info)<br /> - DDE(STRING)field1<br /> - DDE(STRING)field2<br /><br /> </p> </td> 
   <td>java.util.List<br /> </td> 
  </tr> 
  <tr> 
   <td><p>类型属性- xs:id <br /> </p> </td> 
   <td>DDE类型- STRING <br /> </td> 
   <td>java.lang.String<br /> </td> 
  </tr> 
  <tr> 
   <td><p>xs:attribute /xs:element类型- xs:string</p> </td> 
   <td>DDE类型- STRING<br /> </td> 
   <td>java.lang.String<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td>xs:attribute /xs:element of type - xs:多次</td> 
   <td>DDE类型- NUMBER </td> 
   <td>java.lang.Double</td> 
  </tr> 
  <tr> 
...
1. 点按 **警报消** 息中的确定。
   ...
   “对应管理”根据所选数据字典的结构创建XML文件，并将其下载到名为&lt;data-dictionary-name>-SampleData的计算机。 现在，您可以在XML或文本编辑器中编辑此文件，以在创建字母时进 [行数据输入](/help/forms/using/create-letter.md)。
...
1. 在“数据字典”页中，选择“更 **多** ”>“上 **传本地化数据** ”，然后选择包含本地化属性文件的zip文件。
...
您可以从XML模式创建数据字典，也可以使用Data Dictionary用户界面构建它。 数据字典中的所有数据字典元素(DDE)都有一个XML绑定字段，用于存储DDE与XML模式中元素的绑定。 每个DDE中的绑定相对于父DDE。

以下详细信息示例模型和代码示例显示数据字典的实施详细信息。

## 映射简单（简单）元素 {#mapping-simple-primitive-elements}

简单DDE表示本质上是原子的字段或属性。 在复杂类型（复合DDE）或重复元素（集合DDE）的范围外定义的基本DDE可以存储在XML模式中的任何位置。 对应于简单DDE的数据的位置不取决于其父DDE的映射。 Primitive DDE使用“XML绑定”字段的映射信息确定其值，并且映射转换为以下任一内容：
...
集合元素仅映射到其他基数大于1的集合元素。 集合DDE的子DDE与其父项的XML绑定相关（本地）XML绑定。 由于集合元素的子DDE必须与父项的基数相同，因此要求相对绑定以确保基数约束，以便子DDE不指向非重复的XML模式元素。 在以下示例中，“TokenID”基数必须与“Tokens”（其父集合DDE）相同。
...
下面的XML模式声明一个名为Tokens的元素和一个名为“unbounded”的maxOccurs属性。 因此，令牌是一个集合元素。
