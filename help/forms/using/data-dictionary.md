...
通信管理使用根据收件人字典结构存储的后端、客户、`` ``或特定于客户的数据来创建用于不同客户的通信。 例如，可以使用友好名称创建文档，如“尊敬的{First Name}”、“先生”。 {姓氏}&quot;.
...
1. 在[预览字母](/help/forms/using/create-letter.md#p-types-of-linkage-available-for-each-of-the-fields-p)时，作者选择将字母预览为数据(自定义预览)。 此时将打开该字母，其中预填充作者提供的数据。 这在创建对应界面中打开。 预览此信函的代理可以修改此信函中的内容、数据和附件，并可提交最终信函。 有关创建字母的详细信息，请参阅[创建对应](/help/forms/using/create-letter.md)。

## 先决条件 {#prerequisite}

安装[兼容性包](https://helpx.adobe.com/in/experience-manager/6-4/forms/using/compatibility-package.html)以视图&#x200B;**Forms**&#x200B;页上的&#x200B;**数据字典**&#x200B;选项。
...
1. （可选）要上传数据字典的XSD模式定义，请点按“数据字典结构”窗格下的&#x200B;**上传XML模式**。 浏览到XSD文件，选择它，然后点按&#x200B;**打开**。 根据上传的XML模式创建数据字典。 您需要调整数据字典中元素的显示名称和说明。 为此，请点按元素名称，然后在右侧窗格中的字段中编辑其说明、显示名称和其他详细信息，以选择这些元素的名称。

   有关计算DD元素的详细信息，请参阅[计算数据字典元素](#computedddelements)。
...
1. 在“添加属性”屏幕中，将元素添加到数据字典。 如果已上传模式以获取数据字典的基本结构，则还可以添加／删除元素并编辑其详细信息。
   ...
   * enums只包含基元字符串类型。
   ...
   有关数据字典中的验证的信息，请参阅[数据字典编辑器验证](#ddvalidations)。

   ![2_adddpropertys基本](assets/2_addddpropertiesbasic.png)

1. （可选）选择元素后，在高级选项卡中，可以添加属性（属性）。 您也可以点按&#x200B;**添加字段**&#x200B;并扩展DD元素的属性。
...
## 请参阅引用文档字典元素{#see-the-document-fragments-or-documents-that-refer-to-a-data-dictionary-element}的文档片段或
...
1. 创建数据条目后，在预览包含测试数据的字母时，可以使用此XML文件。
   ...
   可以使用DD添加此测试数据（选择DD，然后点按上传测试数据并上传此xml文件）

   因此，当您正常预览字母（非自定义）后，此XML数据将用于字母。 您也可以点按“自定义”，然后上传此XML。

## 示例{#samples}

以下代码示例显示数据字典的实现详细信息。

### 可上传到模式字典{#sample-schema-that-can-be-uploaded-to-the-data-dictionary}的示例
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>表达式</td> 
   <td>字符串</td> 
   <td>“计算”DDE的表达式。 默认情况下，发售的表达式评估服务支持JSP EL表达式。 您可以用自定义实现替换表达式服务。</td> 
  </tr> 
  <tr> 
   <td>valueSet</td> 
   <td>列表</td> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>DDE类型- COMPOSITE<br /> </p> </td> 
   <td>java.util.Map<br /> </td> 
  </tr> 
  <tr> 
   <td><p>xs:element where maxOccurs &gt; 1<br /> </p> </td> 
   <td><p>DDE类型- COLLECTION-<br />在从父COLLECTION节点捕获信息的COLLECTION DDE旁边创建DDE节点。 为简单／复合数据类型的集合创建相同的内容。 只要您有复合类型的COLLECTION,“数据字典”树就会捕获为捕获类型信息而创建的DDE的子项中的组成字段。<br /> - DDE(COLLECTION)<br /> - DDE(COMPOSITE for type info)<br /> - DDE(STRING)字段1<br /> - DDE(STRING)字段2<br /> <br /> </p> </td> 
  ...
  <tr> 
   <td><p>xs：属性/xs：类型元素- xs:string</p> </td> 
   <td>类型DDE - STRING<br /> </td> 
   <td>java.lang.String<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>DDE，类型为<br /> - STRING<br />子类型- ENUM<br /> valueSet - ENUM<br />的允许值 </td> 
   <td>java.lang.String</td> 
  </tr> 
 </tbody> 
</table>
...
1. 点按警报消息中的&#x200B;**确定**。
   ...
   “对应管理”根据所选数据字典的结构创建XML文件，并将其下载到名为&lt;data-dictionary-name>-SampleData的计算机。 现在，您可以在[创建字母](/help/forms/using/create-letter.md)时，在XML或文本编辑器中编辑此文件以进行数据条目。
...
1. 点按&#x200B;**下载本地化数据**。
1. 点按警报中的&#x200B;**确定**。 Corresponcements Management将名为DataDictionary-&lt;DDname>.zip的zip文件下载到您的计算机。
1. Zip文件包含。properties文件。 此文件定义下载的数据字典。 属性文件的内容类似于以下内容：
   ...
   属性文件的结构为描述和数据字典以及数据字典中的每个数据字典元素的显示名称定义一行。 此外，属性文件为每个数据字典元素的枚举值设置定义一行。 与数据字典一样，相应的属性文件可以具有多个数据字典元素定义。 此外，文件可包含一个或多个枚举值集的定义。
