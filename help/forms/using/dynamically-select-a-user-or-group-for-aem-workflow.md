...
分配任务和Acrobat Sign步骤 [OSGi上以Forms为中心的工作流](/help/forms/using/aem-forms-workflow.md) 提供用于动态选择用户的选项。 您可以使用ECMAScript或OSGi包动态地选择“分配任务”步骤的代理人，或选择“签名文档”步骤的签名人。
...
1. 将具有动态选择用户逻辑的ECMAScript添加到.ecma文件中。 单击 **[!UICONTROL 全部保存]**.
   ...
   有关示例脚本，请参阅 [用于动态选择用户或组的ECMAScript示例](/help/forms/using/dynamically-select-a-user-or-group-for-aem-workflow.md#sample-ecmascripts-to-dynamically-choose-a-user-or-a-group).
...
以下示例ECMAScript会动态选择Acrobat Sign步骤的代理人。 在使用以下脚本之前，请确保脚本中提到的用户信息（电子邮件地址和电话号码）正确无误。 如果脚本中提及的用户信息不正确，则相关过程可能会失败。

>[!NOTE]
>
>使用ECMAScript for Acrobat Sign时，脚本必须位于crx-repository中的/apps/fd/workflow/scripts/adobesign/，并且应具有一个名为getAdobeSignRecipients的函数，以返回用户列表。
...
1. 添加使用 [RecipientInfoSpecifier](https://helpx.adobe.com/experience-manager/6-4/forms/javadocs/com/adobe/fd/workflow/adobesign/api/RecipientInfoSpecifier.html) Java界面，用于动态选择“分配”任务步骤的用户或组。 有关示例代码，请参阅 [使用Java界面动态选择用户或组的示例](#-sample-scripts-for).
...
导入包后，用于动态选择用户或组的Java界面选项将在中可用，用于Acrobat Sign和“分配任务”步骤。

### 用于动态选择用户或组的Java代码示例 {#sample-java-code-to-dynamically-choose-a-user-or-a-group}

以下示例代码可动态选择Acrobat Sign步骤的代理人。 在OSGi包中使用代码。 在使用下面列出的代码之前，请确保代码中提及的用户信息（电子邮件地址和电话号码）正确无误。 如果代码中提及的用户信息不正确，则相关过程可能会失败。
