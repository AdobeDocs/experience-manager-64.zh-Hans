...
1. 为什么条码和签名字段不显示在我的表单中？
   ...
   答案： 条码和签名字段与HTML或移动场景无关。 这些字段显示为非交互区域。 但是，AEM Forms设计师提供了一个新的签名涂鸦字段，它可以代替签名字段。 您还可以为条码添 [加自定义](/help/forms/using/custom-widgets.md) 构件并将其集成。
...
1. 是否存在推荐的命名规范，是否存在保留的关键字？
   ...
   * 在AEM Forms设计器中，建议不要以下划线()开头的对象（如子表单或文本字段）的名&#x200B;_称。 要在名称的开头使用下划线，请在下划线后面添加一个前缀，_*&lt;prefix>&lt;objectname>。 *
...
1. HTML5表单是否支持浮动字段？
   ...
   1. 单击“ **全部保存**”。 现在，使用更新的渲染用户档案为HTMLForms启用浮动字段。
