...
# HTML5表单的常见问题解答{#frequently-asked-questions-faq-for-html-forms}
...
1. 为什么条码和签名字段不显示在我的表单中？
   ...
   答案：条码和签名字段与HTML或移动场景无关。 这些字段显示为非交互区域。 但是，AEM Forms设计师提供了一个新的签名涂鸦字段，它可以代替签名字段。 您还可以为条码添加[自定义构件](/help/forms/using/custom-widgets.md)并将其集成。

1. XFA文本字段是否支持富文本？

   答案：不支持在AEM Forms设计器中允许丰富内容的XFA字段，该字段呈现为普通文本，不支持从用户界面设置文本样式。 此外，具有梳状属性的XFA字段显示为普通字段，但根据梳状数字的值，仍对允许的字符数有限制。
...
1. 为什么某些文本会被截断或在HTML5中显示不正确？
   ...
   答案：如果“绘图”或“题注”文本元素没有足够的空间来显示内容，则文本在移动表单再现中会被截断。 此截断操作也可在AEM Forms设计人员的设计视图中看到。 尽管此截断可在PDF中处理，但无法在HTML5表单中处理。 要避免此问题，请提供足够的空间来绘制或描述文本，以便在AEM Forms设计器的设计模式中不截断文本。

1. 我正在观察与缺失内容或重叠内容相关的布局问题。 原因何在？

   答案：如果在同一位置（如矩形）存在“绘图文本”或“绘图图像”元素以及另一个重叠元素，则“绘图文本”内容将不可见(如果稍后按文档顺序出现)(在AEM Forms设计器层次结构视图中)。 PDF支持透明分层，但HTML/浏览器不支持透明分层。
...
1. 在支持的浏览器中打开HTML5表单时，相邻放置的字段的边框不正确对齐或子表单显示重叠。 当在Forms设计器中预览同一HTML5表单时，字段和布局不会出现不对齐，子表单会出现在正确的位置。 如何解决此问题？
...
1. HTMLForms的JavaScript实现有任何限制吗？
...
1. HTML5表单中是否有保留的关键字？
   ...
   答案：所有HTML5表单API都是保留关键字。 对于自定义API/函数，请使用与[HTML5表单API](/help/forms/using/scripting-support.md)不相同的名称。 除保留关键字外，如果您使用以下划线(_)开头的对象名称，建议在下划线后添加唯一前缀。 添加前缀有助于避免与HTML5表单内部API的任何可能冲突。 例如，`_fpField1`
