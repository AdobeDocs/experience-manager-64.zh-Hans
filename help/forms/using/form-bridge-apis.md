...
description: 外部应用程序使用FormBridge API连接到XFA移动表单。 API在父窗口上调度FormBridgeInitialized事件。
seo-description: 外部应用程序使用FormBridge API连接到XFA移动表单。 API在父窗口上调度FormBridgeInitialized事件。
...
您可以使用Form Bridge API在基于XFA的HTML5表单和应用程序之间打开通信通道。 Form Bridge API提供了用于 **创建连接** 的连接API。

**connect** API接受处理函数作为参数。 在基于XFA的HTML5表单与表单桥接器之间成功创建连接后，将调用该句柄。
...
* **输入**:
   ...
   * **上下文**:*handler *函数的上下文(this)设置到的对象。
...
* **输入:**
   ...
   * **** configName:要覆盖的配置名称
      ...
      * **** LoggingConfig:允许用户覆盖日志记录级别、禁用类别的日志记录，或者是显示日志控制台还是发送到服务器。 配置可以被覆盖，如下所示：
...
**setFocus(som)** Sets focus to the field specified in the Som expression
...
* **输入:**
   ...
   * **** som:包含字段的Som表达式的数组。 设置字段值的som表达式。
   * **** value:包含与som数组中提供的Som表达式相应的 **值的数** 组。 如果值的数据类型与fieldType不同，则不修改该值。
