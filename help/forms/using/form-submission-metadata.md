...
1. 在编辑模式下打开自适应表单。
   ...
   要在编辑模式下打开表单，请在表单管理器中选择表单，然后点按&#x200B;**打开**。
...
如果在元数据中添加复选框元素，则选定的值会以逗号分隔的字符串形式存储。 例如，在表单中添加一个复选框组件，并将其名称指定为`checkbox1`。 在复选框组件属性中，添加值0、1和2的“驾驶许可证”、“社会保险号”和“护照”。

![存储复选框中的多个值](assets/checkbox-metadata.png)

您选择自适应表单容器，在表单属性中，添加一个存储`checkbox1.value`的元数据键`cb1`，然后发布表单。 客户填写表单后，将在复选框字段中选择“护照”和“社会保险号”选项。 值1和2作为1、2存储在提交元数据的cb1字段中。
