...
* **通过监视的文件夹触发的进程**: 如果进程由监视的文件夹触发，则无法使用其启动器来标识进程实例。 在这种情况下，用户信息被编码在所存储的数据中。
...
1. 使用具有正确连接设 `ProcessManager` 置的实 `com.adobe.idp.workflow.client.ProcessManager`例创建公 `ServiceClientFactory` 共客户端()的实例。
   ...
   有关详细信息，请参阅类ProcessManager的Java [API参考](https://helpx.adobe.com/experience-manager/6-4/forms/ProgramLC/javadoc/com/adobe/idp/workflow/client/ProcessManager.html)。
