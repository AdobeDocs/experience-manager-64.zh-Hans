...
1. 在“配置表单”页中，您可以执行以下操作：
   ...
   * 数据根：对于表单集中的每个表单，作者可以配置XPATH，在XPATH中，特定表单的数据位于提交的XML中。 默认情况下，值为/。 如果表单集中的所有表单都绑定了模式并共享相同的XML模式，则可以更改此值。 建议每个表单字段都具有在XDP中指定的正确数据绑定。 如果两个不同表单中的两个字段共享公用数据绑定，则第二个表单中的字段将显示第一个表单中的预填值。 请勿将内部内容相同的两个子表单绑定到同一XML节点。 有关表单集的XML结构的详细信息，请参阅[为表单集预填XML](/help/forms/using/formset-in-aem-forms.md#p-prefill-xml-for-form-set-p)。
