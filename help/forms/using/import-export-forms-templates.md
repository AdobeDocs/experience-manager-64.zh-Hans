...
>[!NOTE]
>
>* 在AEM 6.4 Forms中，crx-repository的结构和路径已更改。 如果将资产从先前版本导入AEM 6.4 Forms，并且表单对旧版结构具有某些依赖关系，则必须手动导出依赖关系。 有关存储库结构和路径更改的详细信息，请参阅AEM 6.4](/help/sites-deploying/repository-restructuring.md)中的[存储库重组。
...
借助AEM Forms，您可以创建、下载或上传主题。 与表单、文档和字母等其他资产一样创建主题。 您可以创建主题、下载主题并将其上传到单独的实例以重复使用。 有关主题的详细信息，请参阅AEM Forms](/help/forms/using/themes.md)中的[主题。
...
例如，如果Corresponce Management包含使用图像和文本的字母，则下载的包中还包含图像和与该字母相关的文本。 与资产关联的所有元数据属性（包括自定义属性）也会被下载。 下载包(.cmp)后，可以[将包导入其他AEM Forms实例](/help/forms/using/import-export-forms-templates.md#p-upload-forms-documents-assets-p)。
...
1. 点按&#x200B;**导出**，然后在确认消息中点按&#x200B;**确定**。
   ...
   批处理完成后，将更新上次运行详细信息和用于下载包的链接。 这包括诸如“管理员”登录以及批次或“n”成功或失败等信息。 资源将导出到包，此时会显示“下载导出的包”链接。
...
1. 点按&#x200B;**下载导出的包**&#x200B;链接以下载包文件。
   ...
   要将包中的资源添加到Correponsement Management的另一个实例，[将包导入AEM Forms实例](/help/forms/using/import-export-forms-templates.md#p-upload-forms-documents-assets-p)。
...
>[!NOTE]
>
>在导入旧的Correspondence Management资源进行迁移时，请使用Admin帐户登录。 有关迁移旧通信管理资产的详细信息，请参阅[迁移AEM Forms资产和文档](/help/forms/using/migration-utility.md)。
...
1. 单击&#x200B;**[!UICONTROL 编辑]**&#x200B;并打开&#x200B;**[!UICONTROL 过滤器]**&#x200B;选项卡。 单击&#x200B;**[!UICONTROL 添加过滤器]**。 指定工作流应用程序的路径。 例如，/etc/fd/仪表板/startpoints/homemortgage。
...
![侧面板 — ](assets/search_sidepanel.png)
**滤镜图：** *侧面板 — 过滤器*
