...
# 向AEM Forms进口和出口资产 {#importing-and-exporting-assets-to-aem-forms}
...
>[!NOTE]
>
>* 在AEM 6.4Forms,crx-repository的结构和路径已更改。 如果将资产从先前版本导入AEM 6.4Forms，并且表单对旧版结构有某些依赖关系，则必须手动导出依赖关系。 有关存储库结构和路径更改的详细信息，请参 [阅AEM 6.4中的存储库重组](/help/sites-deploying/repository-restructuring.md)。
...
对于这些资产类型，必须使用AEM Package Manager在源AEM服务器上创建CRX包，并在目标服务器上安装该包。 有关创建和安装包的信息，请参 [阅使用包](/help/sites-administering/package-manager.md)。
...
1. 登录AEM Forms实例。
1. 点按Experience Manager ![adobeexperiencemanager](assets/adobeexperiencemanager.png) 图标>导航罗 ![盘图](assets/compass.png) 标>Forms>Forms和文档。
1. 选择表单资产，然后点按下 **载** 图标。
1. 在下载资产中，选择以下选项之一，然后点按下 **载**。

   * **下载为CRX包：** 使用此选项可将所有选定的资产和相关依赖关系从一个AEM Forms实例下载并移到另一个实例。 它将所有资产和文件夹作为crx包下载。 任何表单资产(包括在AEM（自适应表单、交互式通信和自适应表单片段）中创作的表单)、表单集、表单模板、PDF文档和资源（XSD、XFS、图像）都可以从AEM FormsUI下载为包。
   ...
   * **将资源下载为二进制文件：** 使用此选项仅下载表单模板(XDP)、PDF forms(PDF)、文档(PDF)和资源(图像、模式、样式表)。 您可以使用外部应用程序编辑这些资源。 它以。zip文件的形式下载具有二进制文件的表单资源，如XSD、XDP、图像、PDF和XDP。

      您无法下载自适应表单、交互通信、自适应表单片段、主题和表单集，而 **且下载资产是二进制文件** 。 要下载这些资产，您应使 **用下载为CRX包** 。
   选定的资产将下载为存档（.zip文件）。
...
通过AEM Forms，您可以创建、下载或上传主题。 主题的创建方式与表单、文档和字母等其他资产类似。 您可以创建主题、下载主题并将它上传到单独的实例以重复使用。 有关主题的更多信息，请参阅 [AEM Forms的主题](/help/forms/using/themes.md)。
...
## 在Correponse Management中导入和导出资产 {#import-and-export-assets-in-correspondence-management}
...
### 将所有Corresponce Management资产作为包导出 {#export-all-the-correspondence-management-assets-as-a-package}

使用此选项可从AEM表单实例将所有Corresponce Management资产和相关依赖项作为包下载。

例如，如果Corresponce Management包含使用图像和文本的字母，则下载的包中还包含图像和与字母相关的文本。 还会下载与资产关联的所有元数据属性（包括自定义属性）。 下载包(.cmp)后，可将包导 [入到其他AEM Forms实例](/help/forms/using/import-export-forms-templates.md#p-upload-forms-documents-assets-p)。

要将所有Corresponce Management资产和相关依赖项作为包下载，请完成以下步骤：

1. 以表单用户身份登录到AEM Forms服务器。
1. 点 **按全局** 导航栏中的Adobe Experience Manager。
1. 点按工具( ![tools-1](assets/tools-1.png))，然后点按 **Forms**。
1. Tap **Export Correspondence Management Assets**.

   ![publish-cmp-assets](assets/publish-cmp-assets.png)

   此时将显示“导出所有对应管理资产”页，其中显示有关上次尝试导出过程的信息以及用于下载上次成功导出的包的链接。

   ![export-last-run-details](assets/export-last-run-details.png)

1. 点 **按导** 出，然后在确认消息中点 **按确定**。

   批处理完成后，将更新上次运行详细信息和用于下载包的链接。 这包括诸如管理员登录以及批次“n”成功或失败等信息。 资源将导出到包，并显示“下载导出的包”链接。

   >[!NOTE]
   >
   >启动导出所有资产流程后，将无法取消该流程。 此外，在导出所有操作都处于进行中时，请勿创建、删除、修改或发布任何资产，或启动“发布所有资产”流程。a

1. 点击“ **下载导出的包** ”链接以下载包文件。

   要将包中的资产添加到Corresponce Management的另一个实例， [请将包导入到AEM Forms实例](/help/forms/using/import-export-forms-templates.md#p-upload-forms-documents-assets-p)。
...
>[!NOTE]
>
>在导入旧的Corresponce Management资产进行迁移时，请使用管理员帐户登录。 有关迁移旧通信管理资产的更多信息，请参 [阅迁移AEM Forms资产和文档](/help/forms/using/migration-utility.md)。

1. 在数据字典、字母或文档片段页面上，点 **按创建>文件上传** ，然后选择。cmp文件。
...
1. 单击 **[!UICONTROL 编辑]** ，然后打开 **[!UICONTROL 过滤器]** 选项卡。 单击“ **[!UICONTROL 添加过滤器]**”。 指定工作流应用程序的路径。 例如，/etc/fd/仪表板/startpoints/homemortage。

   单击 **[!UICONTROL 添加规则]**。

1. 打开&#x200B;**[!UICONTROL 高级]**&#x200B;选项卡。在“ **[!UICONTROL ACL处]** 理 **[!UICONTROL ”字]** 段中选择“合并”或“覆盖”。 单击&#x200B;**[!UICONTROL 保存]**。
...
AEM FormsUI允许您搜索内容。 使用顶栏，您可以点按搜 **索** A以搜索您的内容以查找资源，如资产和文档。

搜索资产时，AEM Forms会显示侧面板。 您还可以点 ![按assets-browser-content-only](assets/assets-browser-content-only.png) > Filter **[B]** ，调用侧面板。 使用侧面板中的各种过滤器，您可以缩小搜索范围。 侧面板还允许您保存搜索。

![search_topbar](assets/search_topbar.png)

**A.搜** 索 **B.过滤** 器

![侧面板-过滤器](assets/search_sidepanel.png)**图：** *侧面板-过滤器*
