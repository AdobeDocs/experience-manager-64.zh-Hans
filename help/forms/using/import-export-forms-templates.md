...
>[!NOTE]
>
>* 在AEM 6.4Forms,crx-repository的结构和路径已更改。 如果将资产从先前版本导入AEM 6.4Forms，并且表单对旧版结构有某些依赖关系，则必须手动导出依赖关系。 有关存储库结构和路径更改的详细信息，请参阅AEM 6.4](/help/sites-deploying/repository-restructuring.md)中的[存储库重组。
...
通过AEM Forms，您可以创建、下载或上传主题。 主题的创建方式与表单、文档和字母等其他资产类似。 您可以创建主题、下载主题并将它上传到单独的实例以重复使用。 有关主题的详细信息，请参阅AEM Forms的[主题](/help/forms/using/themes.md)。
...
例如，如果Corresponce Management包含使用图像和文本的字母，则下载的包中还包含图像和与字母相关的文本。 还会下载与资产关联的所有元数据属性（包括自定义属性）。 下载包(.cmp)后，可以[将包导入到其他AEM Forms实例](/help/forms/using/import-export-forms-templates.md#p-upload-forms-documents-assets-p)。
...
1. 点按&#x200B;**下载导出的包**&#x200B;链接以下载包文件。
   ...
   要将包中的资产添加到Corresponce Management的另一个实例，[将包导入AEM Forms实例](/help/forms/using/import-export-forms-templates.md#p-upload-forms-documents-assets-p)。
...
>[!NOTE]
>
>在导入旧的Corresponce Management资产进行迁移时，请使用管理员帐户登录。 有关迁移旧通信管理资产的详细信息，请参阅[迁移AEM Forms资产和文档](/help/forms/using/migration-utility.md)。
...
1. 单击&#x200B;**[!UICONTROL 编辑]**&#x200B;并打开&#x200B;**[!UICONTROL 过滤器]**&#x200B;选项卡。 单击&#x200B;**[!UICONTROL 添加过滤器]**。 指定工作流应用程序的路径。 例如，/etc/fd/仪表板/startpoints/homemortage。
