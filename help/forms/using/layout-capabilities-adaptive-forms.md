...
description: 自适应表单在各种设备上的布局和外观受布局设置控制。 了解各种布局以及如何应用它们。
seo-description: 自适应表单在各种设备上的布局和外观受布局设置控制。 了解各种布局以及如何应用它们。
...
[表单创作简介](/help/forms/using/introduction-forms-authoring.md)
...
**移动** 布局控制移动设备上表单的导航。如果设备宽度为768像素或更高，则布局被视为移动布局并针对移动设备进行优化。
...
![列表自适应表单的根面板的面板布局图](assets/layouts.png)
**:列表** *自适应表单中的面板布局*
...
使用此面板布局在表单内提供导航。 例如，如果您希望在表单中捕获必需信息，同时指导用户分步操作，请使用此布局。
...
使用此布局，您可以将`Panel adaptive form`组件放置到面板中，并可进行折叠样式导航。 使用此布局，您还可以创建可重复的面板。 可重复的面板允许您根据需要动态添加或删除面板。 您可以定义面板重复的最小次数和最大次数。 此外，可以根据在面板项中提供的信息动态地确定面板的标题。
...
使用此布局，您可以将`Panel adaptive form`组件放置到具有选项卡导航的面板中。 这些选项卡位于面板内容左侧。
...
使用此布局，您可以将`Panel adaptive form`组件放置到具有选项卡导航的面板中。 这些选项卡位于面板内容的顶部。
...
### 表单标题{#layout-with-panel-titles-in-the-form-header}中具有面板标题的布局
...
此布局（如名称所示）仅显示没有面板标题的导航菜单和导航栏。 此布局还提供“下一步”和“上一步”图标进行导航。
...
工具栏布局控制您添加到自适应表单的任何操作按钮的定位和显示。 布局可以在表单级或面板级添加。

![列表工具栏布局自适应表单以控制按钮布局](assets/toolbar-layouts.png)
...
当您在自适应表单中添加任何操作按钮时，此布局将被选作默认布局。 选择此布局后，桌面和移动设备的布局将显示相同。
