...
1. 在“数据”面板中，将数据字典元素和占位符元素拖放到文本中。
   ...
   **图：** *Data Dictionary的示例数据文件中指定的占位符元素*

   ![字母中的占位符元素](assets/placeholder_elements_in_text.png)

   **图：CCR** *视图中的占位符元素值，由示例数据文件中指定的“数据字典”变量填充*
...
1. 点按&#x200B;**表**&#x200B;选项卡，并为布局指定以下信息：
   ...
   * **配置**:选择要配置的表。如果表是静态表，则下拉列表中表名的后缀为（静态）；如果表是动态表，则后缀为（动态）。 静态表包含固定数量的行。 静态表可以包含目标区域和字段。 这些目标区域和字段不能绑定到重复的DDE。 绑定到表单元格的数据确定动态表的行数。
   ...
   * **宽度比：** 表列宽的比率。
   ...
   有关在布局片段中使用静态和动态表的详细示例，请参阅[示例文件：在字母](create-letter.md#insert-data-modules-and-layout-fragments-in-a-letter-and-configure-them)中使用静态表和动态表。
...
1. 根据动态XDP创建布局片段。 属性的“表”(Table)选项卡显示表是动态的(“配置”(Configuration For)字段)。 行数(1)和列数(3)是从XDP/布局片段派生的。
   ...
   [单击可放大](assets/dynamictableproperties.png)
...
1. 根据静态XDP创建布局片段。 属性的“表”选项卡显示表是静态的（“配置”字段）。 行数(1)和列数(3)是从XDP/布局片段派生的。
   ...
   [单击可放大](assets/statictableproperties-1.png)
