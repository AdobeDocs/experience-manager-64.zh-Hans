...
* 对于使用 [XFA表单模板的自适应表单](/help/forms/using/prepopulate-adaptive-form-fields.md)，请使用与XFA模板的数据架构兼容的预填XML。
* 对于使用 [XML架构的自适应表单](/help/forms/using/prepopulate-adaptive-form-fields.md#main-pars-header-3)，请使用与XML架构结构兼容的预填XML。
* 对于使用 [JSON架构的自适应表单](/help/forms/using/prepopulate-adaptive-form-fields.md#json-schema-based-adaptive-forms)，请使用符合JSON架构的预填JSON。
* 对于使用FDM架构的自适应表单，请使用与FDM架构兼容的预填JSON。
* 对于没有表单模 [型的自适应表单](/help/forms/using/prepopulate-adaptive-form-fields.md#p-adaptive-form-with-no-form-model-p)，没有绑定数据。 每个字段都是未绑定字段，并使用未绑定的XML预填充。
...
对于基于JSON架构的自适应表单，预填JSON和提交的JSON的结构如下所述。 有关详细信息，请参 [阅使用JSON架构创建自适应表单](/help/forms/using/adaptive-form-json-schema-form-model.md)。
...
1. 输入数据位置或数据文件位置的正则表达式(正 **[!UICONTROL 则表达式)]**。 有效数据文件位置的示例包括：
...
* SERVICE_NAME引用OSGI预填服务的名称。 请参 [阅创建并运行预填服务](/help/forms/using/prepopulate-adaptive-form-fields.md#create-and-run-a-prefill-service)。
