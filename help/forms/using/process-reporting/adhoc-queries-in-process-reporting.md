...
# 临时查询正在处理报告 {#ad-hoc-queries-in-process-reporting}
...
1. 要创建查询，请单 **击** “添加”(+)。
   ...
   **定义筛选器**
   ...
   1. 要向查询添加其他筛选器， **请单击筛选器行右** 侧的“添加(+)”。

      要从查询中删除筛选器， **请单击筛选器行右** 侧的Delete(-)。
...
* **保存**: 保存过滤器。 然后，可以从“我的过滤器”面板查 **看和执行筛** 选器。

### “我的过滤器”面板中的选项 {#options-in-my-filters-panel}

使用“我的 **过滤器** ”面板中的选 **项** Add ![_](assets/lc_pr_add_filter.png)pr_add_pr_add_pr_pr_ ****![](assets/lc_pr_delete_filter.png)****![](assets/lc_pr_edit_filter.png)Edit_lc滤镜pr_pr_pr_删除删除pr_edit_filteran临时查询。

![my_过滤器选项](assets/my_filters_options.png)

### 执行搜索查询 {#to-execute-a-search-query}

1. 要执行查询，请单击“我的过滤器” **面板中的** 筛选器，或者单击“ **运行** ”按钮（如果要创建或编辑筛选器）。
1. 查询结果显示在“流程 **报告** ”窗口的 **“报告”面** 板中。
...
1. 流程搜索结果中显示以下字段：
   ...
   * **进程ID**: 进程的ID。 该字段为超链接。 如果单击此字段中的进程ID，则会将您重定向到该进 **[!UICONTROL 程的“进程详]** 细信息”面板。
   ...
   * **任务ID**: 任务的ID。 该字段为超链接。 如果单击此字段中的任务ID，您将被重定向到该 **[!UICONTROL 任务的“任务]** 详细信息”面板。
   * **发起者**: 启动进程实例的AEM Forms用户
   * **创建时间**: 进程实例开始的日期和时间
...
1. 单击左上角 ![的边栏按钮](assets/lc_pr_rail_button.png) lc_pr_rail_button，折叠“我的过滤器 **”窗格** ，并展开“报告”面板的可 **用空间** 。
1. 使用“报告”面板右上角的 **选项** ，对查询结果执行操作。
...
使用“流 **程详细信息** ”面板视图特定流程的详细信息。

同样，您也可以使 **用“任务** 详细信息”面板视图特定任务的详细信息。

### 视图流程/任务详细信息 {#to-view-process-task-details}

您可以视图特定AEM Forms进程/任务的详细信息：
...
1. 执行进程/任务搜索。 有关详细信息，请 [参阅执行进程搜索查询](/help/forms/using/process-reporting/adhoc-queries-in-process-reporting.md#p-to-execute-a-search-query-p)。

   请注意，结果中显示的进程ID为超链接。

   ![process_id_列表](assets/process_id_list.png)

1. 单击列表中的进程ID，在“进程详细信息”面板中视图此进 **程的详细信** 息。
...
1. 单 **击返回至进程/任务搜索** ，返回至从中向下钻取进程/任务详细信息的搜索结果。
   ...
   但是，如果通过输入特定的进程/任务ID找到进程/任务详细信息，则单击返回至进程/任务搜索将返回至进 **程/任务搜索**，而不显示任何搜索结果。
...
1. 在“流程/任务ID”文本框中，输入流程/任务ID。
   ...
   “流程/任务详 **细信息** ”查询结果中的字段是特定于AEM Forms流程/任务的字段。
