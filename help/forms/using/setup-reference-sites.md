...
* 使业务流程和工作流程实现自动化的表单工作流程。
* 高级用户数据管理和处理功能。
* 与Adobe sign集成，安全地签署和提交自适应表单。
* 与Adobe target集成，提供有针对性的推荐并执行A/B测试，从而最大化表单的投资回报。
* 与Adobe Analytics集成，衡量表单或营销活动的效果并做出明智的决策。
* 增强的表单填写体验。
...
* **AEM Essentials**
   ...
   AEM quickStart、AEM Forms加载项包和参考站点包。 有关 [附加和参考站点包详细信息](https://helpx.adobe.com/aem-forms/kb/aem-forms-releases.html) ，请参阅AEM Forms版本。
...
* AEM 6.4的正在运行的实例（带有Forms加载项包）。 有关详细信息，请参 [阅安装和配置AEM Forms](https://helpx.adobe.com/experience-manager/6-3/forms/using/installing-configuring-aem-forms-osgi.html)。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><a href="#installaemforms">安装和配置AEM表单</a></td> 
   <td>创作和发布</td> 
  ...
  <tr> 
   ...
   <td>创作和发布<br /> </td> 
   <td><p>参考站点使用案例会为不同的事务发送电子邮件。 通过电子邮件发送新闻稿时需要此设置。 它确保URL和图像指向发布实例。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>创作</td> 
   <td>配置AEM DS以在发布实例上提交表单，并在创作实例上配置处理工作流。</td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td><a href="#scheduler">配置Adobe Sign Scheduler</a></td> 
   <td>创作和发布<br /> </td> 
   <td>更改调度程序的配置，以每两分钟检查一次状态。</td> 
  </tr> 
  <tr> 
...
## 安装和配置AEM表单 {#install-and-configure-aem-forms}

按照在OSGi上安装和配置AEM Forms [中所述安装和部署AEM Forms](/help/forms/using/installing-configuring-aem-forms-osgi.md)。
...
1. 查找并点按 **[!UICONTROL Day CQ Link Externalizer配置]** 。
...
1. 转到位于https://&lt;*hostname>*:&lt;*port>*/system/console/configMgr的OSGi配置。
1. 查找并点 **[!UICONTROL 按Day CQ Mail Service]** ，打开它进行配置。
1. 提供SMTP服务器主机名和端口值。
1. 点按&#x200B;**[!UICONTROL 保存]**。
...
在参考站点用例中，电子邮件通信的发布实例需要AEM DS服务设置。 有关在发布实例上配置AEM DS服务设置的详细步骤，请参 [阅配置AEM DS设置](/help/forms/using/configuring-the-processing-server-url-.md)。
...
如果安装成功，您可以访问We.Gov和We.Finance参考站点登录页面。
...
1. 在Microsoft Dynamics实例中，转到“设置”菜 **[!UICONTROL 单中的]** “解决方 **[!UICONTROL 案”]** ，然后单击“ **[!UICONTROL 导入]**”。 选择并导入包。
...
1. 在Microsoft Dynamics实例中，执行以下操作：
   ...
   * 转到“ **[!UICONTROL 销售”>“We.Finance客户”]** ，然后单击“ **[!UICONTROL 导入”]**。
   * 转到“ **[!UICONTROL 销售”>“We.Finance Auto Insurance”]** ，然后单击“ **[!UICONTROL 导入”]**。
   * 转到“ **[!UICONTROL 销售”>“We.Finance Home Mortgage”]** ，然后单击“ **[!UICONTROL 导入”]**。
...
1. 在AEM作者实例中，转到工 **[!UICONTROL 具>云服务>数据源>全局]**。 点按 **[!UICONTROL 刷新Dynamics集成图标]** ，然后点按 **[!UICONTROL 属性]**。
1. 转到Microsoft Azure Active Directory帐户。 将复制的云服务配置URL添加到注册应 **[!UICONTROL 用程序的“回复URL]** ”设置中。 保存配置。
1. 在“身份验证设置”选项卡中， **[!UICONTROL 为Microsoft Dynamics实例指定服务根]**、客户端Id **[!UICONTROL 、]**&#x200B;客户端机密 **[!UICONTROL 和]****** 资源URL。 单 **[!UICONTROL 击重定向到]** Microsoft Dynamics登录页面的“连接到OAuth”。
1. 提供登录凭据。 登录后，您将被重定向到AEM Forms云服务配置页面。 单击 **[!UICONTROL 保存并关闭]**。 将保存云服务配置。
1. 转到“表 **[!UICONTROL 单”>“数据集成”>“We.Finance]**”。 选择自动保险（动态），然后单击编辑。 Microsoft Dynamics实体列在“数据源”选项卡下。 等待所有实体从Microsoft Dynamics中获取并列在数据源选项卡下。
1. 选择AutoInsuranceRenewal **[!UICONTROL 实体]** ，然后单 **[!UICONTROL 击“测试模型对象”]**。 在输入请求部分，将客户ID的值指定为“900001”，然后单击“测 **[!UICONTROL 试”]**。 “输出”部分显示从Microsoft Dynamics获取的客户ID 900001的记录。
1. 在输入请求部分，将客户ID的值指定为“900001”，然后单击“测 **[!UICONTROL 试”]**。 “输出”部分显示从Microsoft Dynamics获取的客户ID 900001的记录。
1. 对发布实例重复步骤1-6。

## 配置Adobe Sign Scheduler {#scheduler}

对作者实例和发布实例执行以下操作：

1. 转到AEM web配置控制台(位于 `https://[server]:[host]/system/console/configMgr`)。
1. 查找并点 **[!UICONTROL 按Adobe sign配置服务]** ，以打开它进行配置。
1. 将状 **[!UICONTROL 态更新调度程序表达式]****配置为0 0/2 &amp;ast;&amp;ast;&amp;ast;?**.
...
1. 转到“工 **[!UICONTROL 具”>“云服务”>“Adobe Sign”>“全局”]**。 选择 **[!UICONTROL AEM Forms Reference Site Sign]** ，然后点按 **[!UICONTROL 属性]**。

   >[!CAUTION]
   >
   >确保将https://[host]:[ssl_port]/mnt/overlay/adobesign/cloudservices/adobesign/properties.html URL添加到Adobe Sign API应用程序的OAuth配置的重定向URL列表中。
...
1. 根据您的环境更新主机和端口设置。
1. 保存设置。
1. (**仅作者实例**)转到工 **[!UICONTROL 具** >**Cloud Services**>**Data Sources**]**> **global Cloud。 选择 **[!UICONTROL 投资回报率]** ，然后点 **[!UICONTROL 按属性]**。 点按 **[!UICONTROL 身份验证设置]** ，并将身份验证类 **[!UICONTROL 型设置为]** “基 **[!UICONTROL 本身份验证”]**。 指定 `admin`/ `admin`作为访问服务的用户名／密码。 点按 **[!UICONTROL 保存并关闭]**。
...
1. 确保AEM云服务中提供We.Finance和We.Gov分析配置。 您可以通过以下任一方式查找云服务：

   * 导航到 **[!UICONTROL 工具>云服务>旧版云服务]** ，或浏览到https://&lt;host>:&lt;port>/libs/cq/core/content/tools/cloudservices.html。
   * 在云服 **[!UICONTROL 务页面]** ，在 **[!UICONTROL Adobe Analytics部分下，单击]**`Show Configurations`。 您可以看到We.Finance和We.Gov配置。 单击以打开配置。 在配置页面中，单击 **[!UICONTROL 编辑]**。 提供有效的公司、用户名、共享机密（密码）和数据中心，然后单击“连 **[!UICONTROL 接到分析”]**。 连接成功对话框后，单击配 **[!UICONTROL 置对话框]** 上的确定。 根据配置分析和报告中所述的分析配置 [下配置框架](/help/forms/using/configure-analytics-forms-documents.md)。
...
1. 导航到位于https://&lt;host>:&lt;port>/aem/forms的表单管理器，然后执行以下操作：
   ...
   * 单击操作工具栏中的启用分析。 为表单启用分析后，单击分析报告。 您可以看到生成的空白报告。 生成空白报告后，您必须提供随refsite包一起提供的种子数据，以生成分析报告以用于演示目的。
   参考站点为分析报告提供信用卡、住房抵押和儿童支持使用案例的种子数据。 有关种子数据的配置，请参 [阅We.Finance参考站点演练](/help/forms/using/finance-reference-site-walkthrough.md) , [以及We.Gov参考站点演练](/help/forms/using/gov-reference-site-walkthrough.md)。

### 配置目标 {#configure-target}

参考站点展示AEM Forms与Adobe Target的集成，允许您在自适应文档中包含有针对性的个性化内容。 它还支持为自适应表单创建A/B测试。

要体验引用站点中的集成，请执行以下操作以在AEM中配置Target:

1. 使用jvm参数启动作者快速入门， `-Dabtesting.enabled=true` 以在服务器上启用A/B测试。
...
1. 在 **[!UICONTROL Adobe Target部分]** ，单击“显 **[!UICONTROL 示配置”]**。 您可以看到We.Finance target配置。 单击以打开配置。 在配置页面中，单击 **[!UICONTROL 编辑]**。 此时将 **[!UICONTROL 打开配置的“编辑组件]** ”对话框。

1. 指定与Target帐户关联的客户代码、电子邮件和密码。 选择API类型作为 **[!UICONTROL REST]**。
1. Click **[!UICONTROL Connect to Adobe target]**. 成功配置Target帐户后，单击“确 **[!UICONTROL 定”]**。 您可以看到打包的配置有一个Target Framework。

1. 转到https://&lt;*hostname*>:&lt;*port*>/system/console/configMgr。

1. 单击 **[!UICONTROL AEM Forms Target配置]**。
1. 选择Target框架。
1. 在“目 **[!UICONTROL 标URL]** ”字段中，指定AEM表单的URL。 例如：https://&lt;*hostname*>:&lt;*port*>。

1. 单击&#x200B;**[!UICONTROL 保存]**。

信用卡应用程序和家庭按揭贷款应用程序使用案例演示了如何执行A/B测试并展示报告以用于演示目的。 有关演练，请参 [阅We.Finance参考站点演练](/help/forms/using/finance-reference-site-walkthrough.md)。
...
* [We.Gov参考网站演练](/help/forms/using/gov-reference-site-walkthrough.md)

* [员工自助服务参考站点演练](/help/forms/using/employee-self-service-reference-site.md)
