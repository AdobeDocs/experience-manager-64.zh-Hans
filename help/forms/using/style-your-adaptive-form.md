...
本教程是创建第一个自 [适应表单系列中的一个步骤](create-your-first-adaptive-form.md) 。 建议按照时间顺序按照系列来了解、执行和演示完整的教程用例。
...
1. 打开自适应表 **单容器的属性**。 在属性浏览器中，导航到“基 **本** ”> **“自适应表单主题”**。 自 **适应表单** “主题”字段列表所有现成的主题和自定义主题。 默认情况下，将应用画布主题。
1. 从自适应表单主题 **字段中选择一个主** 题。 例如， **调查主题**。 点 ![按aem_6_3_forms_save](assets/aem_6_3_forms_save.png) ，应用选定主题。
...
1. 更改标题的现有徽标和文本。 删除标志：
   ...
   1. 点按标题组件中的徽标图像，然后点 ![按](assets/cmppr.png) cmppr属性。 在图像属性中，点按X以删除现有的徽标图像。
   1. 点按上传，选择logo.png，然 ![后点按aem_6_3_forms_save](assets/aem_6_3_forms_save.png) ，以保存更改。 图像已下载到“开始 [前”部分](/help/forms/using/style-your-adaptive-form.md#before-you-start) 。
   1. 点按标题文 `We.Retail`本，然 ![后点按aem_6_3_edit](assets/aem_6_3_edit.png)**编辑**。 将标题文本更改为 `we retail`。 仅在中应用粗 `we`体格 `we retail`式。
...
1. 删除标题并添加占位符文本：
   ...
   1. 将标题字段的内 **容复** 制到占 **位符文本字段** 。
   1. 删除标题字 **段的内** 容，然 ![后点按aem_6_3_forms_save](assets/aem_6_3_forms_save.png)。
...
1. 点按 **主题选** 项 ![主题选项](assets/theme-options.png) > **配**&#x200B;置。 在“ **预览表** ”字段中，选 **择shipping-address-add-update-form自适应表单，点按** aem_6_3_forms_save ![，点按](assets/aem_6_3_forms_save.png)****&#x200B;保存。 现在，主题编辑器已配置为使用您自己的自适应表单而不是默认的自适应表单。 点按 **取消** ，以返回主题编辑器。
...
1. 在“选择 **器** ”面板 **中导** 航“Header > Text”选项。 “选择器”面板位于主题编辑器的左侧。 如果该面板不可见，请点按“切 ![换侧面板](assets/toggle-side-panel.png) ”“切换侧面板”。

1. 在“文本”折叠 **面板中** ，设置以 ![下属性，然后点按aem_6_3_forms_save](assets/aem_6_3_forms_save.png)。
...
1. 点按页脚构件并点 **按页脚**。 展开 **Background** accordion，将Background Color **设置为** ，然 `F6921E`后点按 ![aem_6_3_forms_save](assets/aem_6_3_forms_save.png)。
...
1. 点按客户ID字段，然后点按字 **段构件** 选项。 设置以下属性， ![然后点按aem_6_3_forms_save](assets/aem_6_3_forms_save.png)。
...
1. 点按提 **交按** 钮，然后点 **按按钮** 。 设置以下属性， ![然后点按aem_6_3_forms_save](assets/aem_6_3_forms_save.png)。
...
1. 点按 **提交** 按钮，然后 ![点按aem_6_3_edit图标](assets/aem_6_3_edit.png) 。 设置以下属性：
...
1. 配置主题以使用TypeKit配置。 在创作实例中，在主 **题编辑器中** ，打开全局主题。 在主题编辑器中，导航到“主题选项” ![主题选项>“](assets/theme-options.png) 配置”。 在“ **Typekit配置** ”字段中，选择工具包，然后单击 **保存**。

   添加到Typekit的字体可在所有组件的“文 **本** ”折叠面板中进行选择。
