...
# 自适应表单{#styling-constructs-for-adaptive-forms}的样式构造
...
文章列出了自适应表单的公开可用css类。 您可以利用这些类来设置自适应表单中各个组件的样式。 创作组件的样式（如显示警告的对话框和状态栏）不在本文的涵盖范围内。 仅当您无法使用[主题编辑器](themes.md)来设置组件的样式时，才使用这些样式结构创建样式（使用CSS或更少）。
