...
文章列表公开的自适应表单的css类。 您可以利用这些类来设计自适应表单的各个组件的样式。 创作组件的样式（如显示警告的对话框和状态栏）超出本文的范围。 仅当无法使用主题编辑器设置组件样式时，才使用这些样式构造创建样式(使用CSS或 [更少)](themes.md)。
...
自适应表单包括各种类型的字段。 每个字段都有一个唯一的类名，即字段的名称。 该字段还具有通用类名 `guideFieldNode`。
...
用于字段 **的** HTML元素标签包含左 **侧****或** 顶部的类，具体取决于标签在顶部还是左侧。
...
标签的CSS规则使用guideFieldLabel标 **签进行应** 用。 如果您是作者，请覆盖此规则以使自定义更改可见。
...
作者可以使用简短和详细说明组件在字段中指定帮助内容。 这两个组件都有一个 `.guideHelpDescription` 公用类和 `.long`另一个 `.short`类/，具体取决于描述的类型。 “帮助”内容会包含在段落元素中以覆盖描述的样式。 帮助说明（长和短）使用以widgetshelp开头的变量进行修改，如下表所述：
...
您可以使用CSS规则更改导航器的样式，这些规则使用子代选择器选择 **元素** 。 例如，要向锚点标签添加文本装饰样式：
...
>[!NOTE]
>
>在创作过程中，通过在面板上提供CSS类（表单示例&lt;CLASS_NAME>），可以更改特定导航器的图标。 您为导 **航器的图标添加** &lt;CLASS_NAME>_nav。
...
面板节点被分为导航器和内容。 内 `` `` 容没有单独的样式组件。 描述的变量应用于导航器和内容。
