...
您可以通过导航到 **[!UICONTROL Adobe Experience Manager >工具>模板]**. 在此，模板在为可编辑模板启用的文件夹中进行组织。 AEM提供用于组织模板的全局文件夹。 但是，默认情况下不启用此功能。 您可以请求管理员启用全局文件夹或为模板创建新文件夹。 有关如何创建文件夹的更多信息，请参阅 [模板文件夹](/help/sites-developing/page-templates-editable.md).
...
1. 在模板控制台中，点按 **[!UICONTROL 创建]** 在您创建的文件夹中。
...
创建并启用模板后，在创建自适应表单时，该模板可在表单管理器中使用。 要使用模板和创建自适应表单，请参阅 [创建自适应表单](/help/forms/using/creating-adaptive-form.md).
...
1. 登录到AEM Forms创作实例，然后导航到 **[!UICONTROL “工具”>“操作”>“Web控制台”]**.
...
1. 找到并打开 **[!UICONTROL FormsManager配置]** 设置：
   ...
   * 要显示或隐藏已在AEM 6.0 Forms或AEM 6.1 Forms版本中添加但现已弃用的自适应表单模板，请选中或取消选中 **包含AEM 6.0 AF模板** 选项。 如果选中此选项，则为了生效，它需要 **[!UICONTROL 包含开箱即用的AF和广告模板]** 配置。
