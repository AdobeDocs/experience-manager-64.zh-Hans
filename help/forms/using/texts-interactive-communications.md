...
创建和编辑要在交互式通信中使用的文本文档片段——文本是构建交互式通信所使用的四种文档片段之一。 另外三个是条件、列表和布局片段。
...
1. 选择 **`[!UICONTROL Forms]`** >文 **[!UICONTROL 档片段]**。
...
* 要将FDM属性插入文本，请将光标放在要插入该属性的位置，然后点按左窗格中的属性， **`[A]`** 并点按添加选 **`[B]`** 定 **[!UICONTROL 项]**。 您也只需点击两下属性，即可在光标位置 **`[C]`** 插入它。 表单数据模型属性以褐色背景色突出显示。

* 要允许代理在使用代理UI准备和发送交互式通信时编辑代理UI中的FDM属性值 [](/help/forms/using/prepare-send-interactive-communication.md)**`[D]`** ，请点按该属性的锁图标，并确保它处于已解锁状态。 属性的默认状态已锁定，且代理无法在代理UI中编辑该属性。
...
](assets/toolbarselection.png)字体编 ![辑工具栏](do-not-localize/paragraphtoolbar-1.png)
...
要高亮显示或突出显示可编辑文档片段中的部分文本，请选择文本，然后点按高亮颜色。
