...
1. 指定以下信息：
   ...
   * **[!UICONTROL 表单数据模型]**:（可选）选择“表单数据模型”单选按钮，以基于表单数据模型创建文本。 选择“表单数据模型”单选按钮时， **[!UICONTROL 表单数据模型*]** 字段。 浏览并选择表单数据模型。 在为交互式通信创建文本和条件时，请确保使用打算在交互式通信中使用的相同数据模型。 有关表单数据模型的更多信息，请参阅 [数据集成](/help/forms/using/data-integration.md).
...
* 要将FDM属性插入文本，请将光标放在要插入该属性的位置，然后选择 **`[A]`** 属性，然后点按 **`[B]`** **[!UICONTROL 添加选定项]**. 您也只需双击资产，即可在 **`[C]`** 光标位置。 表单数据模型属性以褐色背景色突出显示。

* 允许代理在代理UI中编辑FDM属性的值，同时 [准备和发送交互式通信](/help/forms/using/prepare-send-interactive-communication.md) 使用代理UI，点按 **`[D]`** 该属性的锁定图标，并确保其处于已解锁状态。 属性的默认状态已锁定，且代理无法在代理UI中编辑该属性。

您还可以使用表单数据模型属性来构建用于显示或隐藏部分内容的规则。 有关更多信息，请参阅 [以文本形式创建规则](#rules).
...
创建或编辑文本文档片段时，可以创建和插入变量。 您创建的变量显示在代理UI的“数据”选项卡中。 代理指定变量的值，而 [使用代理UI准备和发送交互式通信](/help/forms/using/prepare-send-interactive-communication.md).
...
1. 输入以下信息并点按 **[!UICONTROL 创建]**:
   ...
   * **[!UICONTROL 仅允许特定值]**:对于字符串变量和数字变量，您可以确保代理从代理UI中占位符的特定值集中进行选择。 要指定值集，请选择此选项，然后指定 **[!UICONTROL 值]** 字段。
...
在文本中使用规则编辑器，您可以创建规则以根据 **预设条件**. 这些条件可以基于：
...
1. 点按 **[!UICONTROL 创建规则]**.
   ...
   * 您可能已创建的任何变量
   ...
   ![规则编辑器fdm](assets/ruleeditorfdm.png)
...
1. 插入评估表达式、值、数据模型属性或变量。
   ...
   * 在创建或编辑规则时，您还可以点按 ![icon_resize](assets/icon_resize.png) （调整大小）以展开创建规则/编辑规则对话框。 利用扩展的全窗口对话框，可拖放表单数据模型属性和变量以构建规则。 再次点按调整大小，以返回到创建规则对话框。
   * 您还可以在规则中创建多个条件。
...
1. 打开的文本文档片段 [编辑](#edittext).
1. 点按 **[!UICONTROL 查找和替换]**.

1. 在 **[!UICONTROL 查找]** 的新文本（替换文本） **[!UICONTROL 替换]** 文本框和点按 **[!UICONTROL 替换]**.

1. 如果找到搜索的文本，则替换文本。

   * 如果找到搜索文本的另一个实例，则该实例会在文本文档片段中突出显示。 如果您点按 **[!UICONTROL 替换]** 同样，如果找到第三个实例，则替换突出显示的实例，并且光标向前移动。
   * 如果找不到其他实例，“查找并替换”对话框将显示一条消息：模块已结束。

   您还可以点按全部替换，以一次性替换所有匹配项。

   “查找并替换”还包含功能强大的正则表达式搜索功能。 要在搜索中使用正则表达式，请选择 **[!UICONTROL 注册表]** 然后点按 **[!UICONTROL 查找]** 或 **[!UICONTROL 替换]**.
