...
1. 在“创建主题”向导的“基本”选项卡中，提供 **[!UICONTROL 标题]** 和 **[!UICONTROL 名称]** 主题。 这些是必填字段。
...
1. 在“主题”页面中，选择一个主题，然后单击 **[!UICONTROL 下载]**. 将显示一个包含主题详细信息的对话框。
  ...
  <tr> 
   ...
   <td> 
    <ul> 
     ...
     <li>当此值发生更改时，clientlib节点结构将移至输入的新位置。<br /> <em><strong>注意：</strong> 如果更改默认的clientlib位置，请在CRXDE存储库中分配 <code>crx:replicate, rep:write, rep:glob:*, rep:itemNames:: js.txt, jcr:read </code>to <code>forms-users</code> 和 <code>crx:replicate</code>, <code>jcr:read </code>to <code>fd-service</code> 中。 还通过添加 <span class="kbd">deny jcr:addChildNodes</span> 表示 <code>forms-user</code></em></li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td>13.</td> 
...
* **画布**  — 显示在右侧。 它显示了自适应表单或交互式通信示例，所有样式更改都可即时反映在其中。 您还可以直接从画布中选择对象，以查找与其关联的样式，并编辑这些样式。 顶部的设备分辨率标尺控制画布。 从标尺中选择分辨率断点可显示示例表单的预览或相应分辨率的交互式通信。 画布将详细讨论 [下面](/help/forms/using/themes.md#using-canvas).
...
* **侧栏** — 显示在左侧。 具有以下项目：
   ...
   * **查看CSS**:允许您查看所选组件的CSS
   ...
   此外，在侧栏中，底部还显示一个箭头。 单击箭头时，您会再获得两个选项： **模拟成功** 和 **模拟错误。** 这些选项以及上述选项将进行详细讨论 [下面](/help/forms/using/themes.md#using-rail).
...
某些组件（如文本框、数字框、单选按钮和复选框）被分类到“字段”等通用组件下。 例如，您要自定义单选按钮的样式。 要选择用于样式设置的单选按钮，请选择 **字段>小组件>单选按钮**.

单击 **全部展开** 在要查看的侧栏中，选择前期不可见的已分类组件并设置其样式。
...
在资产库中添加样式后，该样式可用于其他主题和 [样式模式](/help/forms/using/inline-style-adaptive-forms.md) 表单编辑器或交互式通信编辑器UI的UI中。 同样，当您使用表单编辑器或交互式通信编辑器UI的样式模式来设置组件样式时，会缓存该样式，并在主题中可用。
...
样式中所做的修改会立即反映在画布中加载的表单或交互式通信中。 实时预览允许您以交互方式定义并查看样式的影响。 更改组件的样式时， **[!UICONTROL 完成]** 按钮。 要保留更改，请使用 **[!UICONTROL 完成]** 按钮。
...
1. 在“常规”选项卡中，浏览并选择 **[!UICONTROL 自适应表单/文档]** 字段。
...
利用主题编辑器，可编辑您创建或上传的主题。 导航到 **[!UICONTROL Forms与文档>主题]**，然后选择一个主题并将其打开。 主题在主题编辑器中打开。
...
在画布中使用标尺为显示屏较小的设备选择断点。 单击模拟器 ![标尺](assets/ruler.png) 在画布中查看标尺和断点。 断点允许您预览表单或交互式通信，以显示与不同设备（如手机和平板电脑）相关的显示大小。 主题编辑器支持多种显示大小。
...
您现在可以通过自适应表单或交互式通信方式使用Web服务中提供的字体。 现成， [Typekit](https://typekit.com/),Adobe的web字体服务可用作配置。 要使用Typekit，请在其中创建工具包和字体，然后从 [Typekit网站](https://typekit.com/).
...
1. 在 **[!UICONTROL Cloud Services]** 页面，导航到 **[!UICONTROL 第三方服务]** >  **[!UICONTROL Typekit]** ，然后单击 **[!UICONTROL 立即配置]** 下。 如果配置已可用，请单击 **+** 按钮以创建新实例。
1. 在 **[!UICONTROL 创建配置]** 对话框中，指定配置的标题并单击 **[!UICONTROL 创建]**.
...
1. 在 **[!UICONTROL Typekit配置]** 字段，选择工具包，然后单击 **[!UICONTROL 保存]**.
...
1. 使用管理权限登录AEM Web Console。 AEM Web Console的URL是 `https://[server]:[port]/system/console/configMgr`.
...
1. 在编辑模式下，选择一个组件，然后单击 ![字段级别](assets/field-level.png) > **[!UICONTROL 自适应表单容器]**，然后单击 ![cppr](assets/cmppr.png).
...
1. 在编辑模式下，选择一个组件，然后单击 ![字段级别](assets/field-level.png) >**[!UICONTROL 文档容器]**，然后单击 ![cppr](assets/cmppr.png).

   您可以在侧栏中编辑表单的属性。

1. 在侧栏中，在 **[!UICONTROL 基本]**，从中选择您的主题 **[!UICONTROL 主题]** 下拉菜单并单击 **[!UICONTROL 完成]** ![check-button](assets/check-button.png).
...
* **表单的上次修改时间：** 更新了关联的主题。
* **A/B测试：** 在A/B测试中，您可以将不同的主题应用于两个版本的表单。 两个主题的信息分别存储在两个引导容器上。
...
* **查看主题CSS** 选项：在画布工具栏中，单击 ![主题选项](assets/theme-options.png) > **查看主题CSS**. 您可以查看从在主题编辑器中定义的属性生成的整个主题CSS。

## 疑难解答、建议和最佳实践 {#troubleshooting-recommendations-and-best-practices}

* **避免使用其他主题中的资产**

   编辑主题时，您可以浏览其他主题中的资产（如图像）并添加这些资产。 例如，您正在编辑页面的背景。 例如，当您选择 **[!UICONTROL 页面]** ![edit-button](assets/edit-button.png)> **[!UICONTROL 背景>添加>图像]**&#x200B;时，您会看到一个对话框，通过该对话框可以浏览并添加其他主题中的图像。
...
* **使用基本clientlib、主题编辑器和内联样式**
   ...
   * **Base clientlib**:
      ...
      1. 在“主题”页面中，选择一个主题并单击 **[!UICONTROL 查看属性]**.
      1. 在打开的属性页面中，单击 **[!UICONTROL 高级]**.
