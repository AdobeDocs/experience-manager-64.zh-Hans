...
当您打开并从组或共享列表声明任务时，该任务会从组或共享列表移入您的单个待办事项列表。 有权访问该列表的其他用户无法处理该任务。

1. 在待办事项页面的左窗格中，选择组或共享待办事项列表。 所有任务都显示在中间窗格中。
1. 执行以下步骤之一：

   * 要从组或共享的待办事项列表中声明任务（不打开任务），请单击 **索赔** 将指针悬停在任务上。 或者，当任务打开时，“声明”按钮在任务窗格下方的操作栏中可用。 申请后，任务会从组或共享的待办事项列表移动到您的列表。
   * 要从组或共享待办事项列表中声明并打开任务，请单击 **声明和打开**.
...
1. 在任意选项卡上，单击 **返回**. 该任务将返回到先前将任务转发给您、与您共享或咨询任务的用户的待办事项列表。
...
1. 在任意选项卡上，单击 **脱机**.
1. 指定要在下保存表单的文件名，然后单击 **保存**. 与任务关联的表单将保存在本地，并且该任务会一直保留在待办事项列表中，直到提交表单为止。
...
1. 在任意选项卡上，点按 **保存**.
   ...
   >
   >有关更多信息，请参阅定期保存草稿(位于 [管理首选项](/help/forms/using/getting-started-livecycle-html-workspace.md).

1. 在另存为草稿对话框中，为任务指定唯一的名称，然后点按 **确定**.
...
* 从待办事项列表中，不打开任务
   ...
   注意：如果 `isMustOpenToComplete` 字段 `Assign Task` 步骤（在Workbench中）。
...
1. 如果出现“确认”对话框，请执行以下操作之一：
   ...
   * 单击 **确定** 如果您已完成任务并准备好注销该任务。
   * 单击 **取消** 如果您希望返回任务，但尚未准备好在其上签名。

>[!NOTE]
>
>在表单中使用流程属性时，您可能会在HTML表单中看到“提交”按钮。 当同一表单呈现为PDF时，此按钮不可见。 要完成任务，请单击位于AEM Forms工作区底部的“提交”按钮，该按钮位于表单外，而不位于表单内的“提交”按钮上。
...
1. 要将表单和数据包含在电子邮件中，请在 **在电子邮件中附加Forms** 列表，选择 **是**.
...
A *截止时间* 确定必须完成任务的日期和时间。 当截止时间过后，服务器将任务路由到进程中的下一步（可以是其他用户的待办事项列表），然后任务上会显示截止时间图标。 无论与流程关联的规则如何，都会显示截止日期图标。

A *提醒* 通知您需要您注意的任务。 提醒会在预定时间发生，然后会定期发生，直到您完成相关任务为止。 当您收到提醒时，任务上会显示提醒图标。
...
1. 在 **队列** 选项卡 **首选项** 选项卡上，单击“用户当前共享我的队列”的“+”图标。
1. 搜索并选择用户的名称。
1. 单击 **共享** 按钮与选定用户共享队列。
1. 选择用户的名称并单击 **共享**.
...
1. 选择用户的名称并单击 **请求**.
   ...
   >[!NOTE]
   >
   >您可以删除对另一个待办事项列表的访问权限，方法是从“我有权访问的用户队列”列表中选择用户名，然后单击 **X** 行末提到了用户的名称。 当访问待办事项列表的请求仍处于待处理状态时，您无法删除对另一个待办事项列表的访问权限。

## 设置不在办公室的首选项 {#setting-out-of-office-preferences}
...
1. 要指定您何时离开办公室，请执行以下步骤之一：
   ...
   * 要指定您现在不在办公室，请在 **我现在** 列表，选择 **外出办公室** 但不要添加日期范围。
   * 要指定您不在办公室的开始日期和时间，请单击“+”， **不在办公时间表**. 使用日历和时间列表指定开始日期和时间。 如果您没有指定结束日期和时间，则会从开始日期和时间开始，无限期地被视为外出，直到您更改首选项为止。

1. 要指定默认处理任务的方式，请从 **外出时：“外出”任务的默认用户** 列表：

   * 选择 **不分配** 将任务保留在待办事项列表中，直到您返回。
   * 选择 **查找用户** 搜索要将任务分配到的用户。 当您选择某个用户时，您还可以查看其离职计划。

1. 要将例外设置为默认值，请单击+表示 **流程例外**，选择创建例外的流程，然后选择其他用户或选择 **不分配** 从 **分配给** 列表。
