...
用户管理将用户数据存储在数据库中，如My Sql、Oracle、MS SQL Server和IBM DB2。 此外，在AEM作者的Forms应用程序中至少登录一次的任何用户都将在AEM存储库中创建该用户。 `https://[*server*]:[*host*]/lc`因此，用户管理存储在以下数据存储中：
...
至少访问过`https://[*server*]:[*host*]/lc`下的Forms应用程序的用户管理数据也存储在AEM存储库中。
...
要从用户管理数据库导出或删除用户数据，您需要使用数据库客户端连接到数据库，并根据用户的某些PII查找主体ID。 例如，要使用登录ID检索用户的主体ID，请在数据库上运行以下`select`命令。
...
要视图在AEM存储库中创建的用户，请使用AEM管理员凭据登录`https://[*server*]:[*port*]/lc/useradmin`。 请注意，URL中的`*server*`和`*port*`是AEM作者实例的实例。 在此，您可以使用用户名搜索用户。 多次-单击用户可视图用户的属性、权限和用户组等信息。 用户的`Path`属性指定在AEM存储库中创建的用户节点的路径。

#### 删除用户数据{#delete-aem}

删除用户：

1. 转至具有AEM管理员凭据的`https://[*server*]:[*port*]/lc/useradmin`。
1. 搜索用户，然后多次单击用户名以打开用户属性。 复制`Path`属性。
1. 转到位于`https://[*server*]:[*port*]/lc/crx/de/index.jsp`的AEM CRX DELite并导航或搜索用户路径。
1. 删除路径，然后单击&#x200B;**[!UICONTROL 全部保存]**&#x200B;从AEM存储库中永久删除用户。
