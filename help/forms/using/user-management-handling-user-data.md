...
用户管理将用户数据存储在数据库中，如My Sql、Oracle、MS SQL Server和IBM DB2。 此外，在AEM作者的Forms应用程序中至少登录一次的任何用户，都 `https://[*server*]:[*host*]/lc`会在AEM存储库中创建该用户。 因此，用户管理存储在以下数据存储中：
...
至少访问过Forms应用程序的用户管理数据也 `https://[*server*]:[*host*]/lc` 存储在AEM存储库中。
