...
description: 用户管理是一个AEM Forms JEE组件，它允许AEM Forms用户创建、管理和授权AEM Forms。 深入了解用户数据和数据存储。 了解如何访问和删除用户数据。
seo-description: 用户管理是一个AEM Forms JEE组件，它允许AEM Forms用户创建、管理和授权AEM Forms。 深入了解用户数据和数据存储。 了解如何访问和删除用户数据。
...
用户管理是一个AEM Forms JEE组件，它允许AEM Forms用户创建、管理和授权AEM Forms。 用户管理使用域作为获取用户信息的目录。 支持以下域类型：

**本地域**:此类域未连接到第三方存储系统。而是在本地创建用户和组并驻留在用户管理数据库中。 密码存储在本地，身份验证使用本地数据库完成。
...
用户管理将用户数据存储在数据库中，如My Sql 、 Oracle 、 MS SQL Server和IBM DB2。 此外，任何已在AEM作者的Forms应用程序中至少登录一次的用户(`https://[*server*]:[*host*]/lc`)，都将在AEM存储库中创建该用户。 因此，用户管理存储在以下数据存储中：
...
>[!NOTE]
>
>存储在第三方存储中的数据不在此文档的范围之内。 直接与第三方供应商联系以管理此类存储中的用户数据。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>存储用户的个人身份信息(PII)。 它包含一个条目，供来自本地、企业和混合域的每个用户使用。</td> 
  </tr> 
  <tr> 
...
对于在`https://[*server*]:[*host*]/lc`下至少访问过Forms应用程序的用户，用户管理数据也存储在AEM存储库中。
...
要从用户管理数据库导出或删除用户数据，您需要使用数据库客户端连接到数据库，并根据用户的某些PII查找主体ID。 例如，要使用登录ID检索用户的主ID，请在数据库上运行以下`select`命令。
...
Forms JEE用户在AEM存储库中拥有其数据(如果他们至少访问了AEM Forms作者实例)。 您可以从AEM存储库访问和删除其用户数据。

#### 访问用户数据{#access-user-data}

要视图在AEM存储库中创建的用户，请使用AEM管理员凭据登录到`https://[*server*]:[*port*]/lc/useradmin`。 请注意，URL中的`*server*`和`*port*`是AEM作者实例的。 在此，您可以使用用户名搜索用户。 多次单击用户可视图用户的属性、权限和用户组等信息。 用户的`Path`属性指定在AEM存储库中创建的用户节点的路径。
...
1. 搜索用户并按住多次单击用户名以打开用户属性。 复制`Path`属性。
1. 转到位于`https://[*server*]:[*port*]/lc/crx/de/index.jsp`的AEM CRX DELite，导航或搜索用户路径。
1. 删除路径，然后单击&#x200B;**[!UICONTROL 保存全部]**&#x200B;以从AEM存储库中永久删除用户。
