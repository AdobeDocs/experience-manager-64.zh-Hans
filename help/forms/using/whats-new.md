...
### 所见即所得文档编辑器 {#wysiwyg-document-editor}
...
有关详细信息，请参 [阅创作交互式通信简介](/help/forms/using/introduction-interactive-communication-authoring.md)。
...
### 使用主题来设计交互式通信的Web渠道 {#use-themes-to-style-web-channel-of-interactive-communication}
...
有关详细信息，请参阅 [AEM Forms的主题](/help/forms/using/themes.md)。
...
### 开箱即用的数据连接器用于预填文档 {#out-of-the-box-data-connectors-to-prefill-documents}
...
### 预览表单和文档（含示例数据） {#preview-forms-and-documents-with-sample-data}
...
数据源实体或属性中的任何更新不会自动反映在关联的表单数据模型中。 表单模式模型编辑器现 ![在具有refresh_forms_di](assets/refresh_forms_di.png) （“刷新数据源定义”）功能，可使服务器缓存失效，并从数据源获取更新的以立即反映在表单数据模型中。

### 使用触控用户界面配置数据源 {#configure-data-sources-using-touch-user-interface}

在此版本中，数据源的云服务配置可在触屏用户界面中使用。 此外，配置云服务的位置已更改为“工 **[!UICONTROL 具”>“Cloud Services”>“数据源]**”。 请参 [阅配置数据源](/help/forms/using/configure-data-sources.md)。
...
借助以Forms为中心的AEM工作流功能，您可以快速为OSGi堆栈上的各种任务构建和部署工作流。 您不再需要在JEE堆栈上安装可用的流程管理功能，从而简化了部署并消除了应用程序服务器和基础架构成本。 有关详细信息，请参 [阅有关OSGi的以Forms为中心的工作流](/help/forms/using/aem-forms-workflow.md)。
...
* 文档安全性提 [供可移植保护库(PPL)](/help/forms/using/document-security-offerings.md) ，可在本地保护文档，而无需将文档发送到AEM Forms服务器。 只有安全凭据和策略详细信息通过网络传送到AEM Forms服务器。 AEM 6.4Forms已采用OSGi捆绑格式引入便携保护库(PPL)。 现在，您可以直接在AEM Forms服务器上安装PPL库，并将AEM和PPL的功能相互结合使用。
