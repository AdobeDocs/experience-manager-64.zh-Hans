...
阅读有关可以使用表单数据模型编辑器执行的各种任务和配置的详细信息。
...
1. 选择要添加到表单数据模型的数据模型对象和服务，然后点按添 **[!UICONTROL 加选定]**。
   ...
   >[!NOTE]
   >
   >除了数据模型对象和服务之外，OData服务元数据文档还包括定义两个数据模型对象之间的关联的导航属性。 有关详细信息，请参 [阅使用OData服务的导航属性](#navigation-properties-odata)。
...
1. 启用计算（如果属性是计算属性）。 基于规则或表达式计算计算属性的值。 有关详细信息，请参阅 [编辑属性](#edit-properties)。
1. 如果数据模型对象绑定到数据源，则添加的子属性将自动绑定到具有相同名称和数据类型的父数据模型对象的属性。

   要手动将子属性与数据模型对象属性绑定，请点按“绑定引用”字段旁边的浏 **[!UICONTROL 览图标]** 。 “选 **[!UICONTROL 择对象]** ”(Select Object)对话框列出父数据模型对象的所有属性。 选择要绑定的属性，然后点按勾号图标。 请注意，只能选择与子属性具有相同数据类型的属性。
...
1. 点按 **[!UICONTROL 编辑属性]**。
1. 在“编 **[!UICONTROL 辑属性]** ”窗格中，点按“绑定”字段旁边的浏览 **[!UICONTROL 图标]** 。 它打开“选 **[!UICONTROL 择对象]** ”对话框，其中列出在表单数据模型中添加的数据源。
...
1. 选中数据模型对象顶部的复选框以将其选中，然后点按编辑 **[!UICONTROL 属性]**。
   ...
   >[!NOTE]
   >
   >除了数据模型对象和服务之外，OData服务元数据文档还包括定义两个数据模型对象之间的关联的导航属性。 将OData服务数据源添加到表单数据模型时，在表单数据模型中有一个可用于数据模型对象中所有导航属性的服务。 您可以使用此服务读取相应数据模型对象的导航属性。
   >
...
1. 点 ![按aem_6_3_edit](assets/aem_6_3_edit.png) ，获取读取服务参数，将该参数绑定到用户配置文件属性、请求属性或文本值并指定绑定值。 它将服务参数绑定到指定的绑定属性或文本值，该值作为参数传递给服务以从数据源获取与指定值关联的详细信息。

   在此示例中，该参 `id` 数将采用用户配置文件的属 `empid` 性的值，并将它作为参数传递给读取服务。 它将从指定的数据模型对象中读取并返 `employee` 回相关属性的值 `empid`。 因此，如果在表单的字段中 `empid` 指定00250，则读取服务将读取员工的详细信息（员工id为00250）。
...
1. 选中数据模型对象顶部的复选框以将其选中，然后点按添加 **[!UICONTROL 关联]**。 此时将打开“添加关联”对话框。
   ...
   >[!NOTE]
   >
   >除了数据模型对象和服务之外，OData服务元数据文档还包括定义两个数据模型对象之间的关联的导航属性。 在表单数据模型中添加关联时，可以使用这些导航属性。 有关详细信息，请参 [阅使用OData服务的导航属性](#navigation-properties-odata)。
...
在表单数据模型中配置OData服务时，实体容器中的所有导航属性都可以通过表单数据模型中的服务来使用。 在TripPin OData服务的此示例中，可以使用表单数据模型中的一个服务读取实体容器 `Person``GET LINK` 中的三个导航属性。
...
>[!NOTE]
>
>NavigationPropertyName参数的 **[!UICONTROL Default Value]** （默认值）字段中 **的可用值取决于****[!UICONTROL Return数组的状态？]** 切换按钮。 启用此选项后，将显示“集合”类型的导航属性。

在此示例中，您还可以选择输出模型对象作为Person，选择导航属性参数作为Friends或BestFriend(取决于 **[!UICONTROL Return数组？]** 启用或禁用)。

![edit-prop-nav-prop2](assets/edit-prop-nav-prop2.png)

同样，在表单数据模型 `GET LINK` 中添加关联时，您可以选择服务并配置其导航属性。 但是，要能够选择导航属性，请确保将“绑 **[!UICONTROL 定到”字段设]** 置为 **[!UICONTROL Literal]**。
...
您有一个工作表单数据模型，它现在可用于自适应表单和交互式通信工作流程。 有关详细信息，请参 [阅使用表单数据模型](/help/forms/using/using-form-data-model.md)。
