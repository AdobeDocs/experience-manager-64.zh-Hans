...
## 关于定义为项目{#agreement-on-kpis-defined-as-goals-for-the-project}目标的KPI的协议
...
## 使内容架构与KPI {#alignment-of-content-architecture-with-kpis}保持一致

确保建议的内容架构与相关关键绩效指标(KPI)保持一致。

## 使客户路线图与项目时间表{#alignment-of-the-customer-roadmap-with-project-timeline}保持一致
...
## 定义的应用程序特定维护任务{#application-specific-maintenance-tasks-defined}
...
## 架构审核板签署{#architecture-review-board-sign-off}
...
## 与KPI {#automated-test-suite-adapted-for-real-content-and-results-compared-to-kpis}相比，适用于真实内容和结果的自动测试套件
...
## 针对实际和预期负载{#automated-testing-strategy-validated-against-realistic-and-expected-load}验证的自动测试策略
...
## 已测试{#backup-and-restore-tested}的备份和恢复
...
## 对已识别并符合ROI和KPI预期的解决方案或体系结构进行任何必要调整后，业务部门予以签收{#business-sign-off-on-any-required-adjustments-to-the-solution-or-architecture-identified-and-aligned-against-roi-and-kpi-expectations}
...
## 将范围和期望告知{#communicate-scope-and-expectations-to-team}团队
...
## {#customizations-and-hotfixes-that-affect-upgrades-documented}中记录的影响升级的自定义和修补程序
...
## 执行的耐用性测试{#durability-test-executed}
...
## 实施 — 根据渗透测试结果{#implementation-changes-based-on-penetration-test-results}进行的更改
...
## 问题跟踪系统进程已设置并集成{#issue-tracking-system-process-is-set-up-and-integrated}
...
## {#list-of-development-tools-to-be-used}要使用的开发工具列表
...
## 向系统工程师传递的监视策略{#monitoring-policy-communicated-to-system-engineer}
...
## 根据要求文档{#poc-tested-and-verified-against-requirement-documentation}测试和验证POC
...
## 业务利益相关方的生产签收{#production-sign-off-from-business-stakeholders}
...
## 在定义的频率{#project-status-reports-within-a-defined-cadence}内报告项目状态
...
## 在生产环境{#release-running-on-production-environment}上运行的版本
...
## 第三方系统{#slas-for-third-party-systems}的SLA
...
## 为系统验证{#smoke-tests-executed-for-system-validation}执行的烟度测试
...
## 已成立解决方案审查委员会并召开会议会议频率集{#solution-review-board-established-and-meeting-cadence-set}
...
## 定义和传递规范审核和批准流程{#specification-review-and-approval-process-defined-and-communicated}

客户签署规范的明确过程应当到位。 此过程可确保要求的范围清晰而稳固。

## 为AEM管理员培训选择的员工{#staff-selected-for-aem-administrator-training}
...
## 已识别并验证的系统风险因素{#system-risk-factors-identified-and-verified}
...
## 转换为测试情景{#use-cases-converted-into-test-scenarios}的用例
