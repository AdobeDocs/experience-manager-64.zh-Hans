...
此术语表列表了项目清单中所有可交付文档的详细 [信息(按比例](/help/managing/best-practices-checklist.md))。
...
## 协调访问测试系统 {#access-to-test-system-coordinated}

确保为所有角色授予所需的系统访问级别。

## Adobe安全核对清单 {#adobe-security-checklist}

Adobe [安全核对清单](/help/sites-administering/security-checklist.md) ，是为确保AEM在安装时的安全而提供的正式核对清单。 它包含您需要执行的安全措施和验证步骤，以确保实例的完整性。
...
为解决方案的行政人员提供培训。 有关详细 [信息，请参](https://training.adobe.com/training/courses.html#solution=adobeExperienceManager) 阅Adobe培训服务。

## AEM作者培训 {#aem-author-training}

为将要为解决方案制作（创作）内容的员工提供培训。 有关详细 [信息，请参](https://training.adobe.com/training/courses.html#solution=adobeExperienceManager) 阅Adobe培训服务。

## AEM认证考试 {#aem-certification-exam}

确保注册相应角色以参加相关的认 [证考试](https://training.adobe.com/certification/exams.html#p=1&amp;solution=adobeExperienceManager)。

## AEM认证 {#aem-certified}

确保相应角色通过了相关的认 [证考试](https://training.adobe.com/certification/exams.html#p=1&amp;solution=adobeExperienceManager)。
...
## 内容架构与KPI的协调 {#alignment-of-content-architecture-with-kpis}

确保建议的内容架构与相关的关键绩效指标(KPI)保持一致。

## 客户路线图与项目时间表的协调 {#alignment-of-the-customer-roadmap-with-project-timeline}

客户路线图包含高级别里程碑和业务目标。 项目时间线必须遵守并与此战略保持一致，因此必须突出和跟踪任何潜在风险和／或可能的偏差。

## 应用程序架构定义 {#application-architecture-definition}

应用 [程序架构](/help/managing/best-practices.md#development-preparation) 应明确定义所建议应用程序的行为。
...
## 与KPI相比，自动测试套件可满足真实内容和结果的要求 {#automated-test-suite-adapted-for-real-content-and-results-compared-to-kpis}
...
## 针对真实和预期的负载验证自动化测试策略 {#automated-testing-strategy-validated-against-realistic-and-expected-load}
...
## 对已识别并符合ROI和KPI预期的解决方案或体系结构进行任何必要调整后，业务部门予以批准 {#business-sign-off-on-any-required-adjustments-to-the-solution-or-architecture-identified-and-aligned-against-roi-and-kpi-expectations}
...
## 执行耐久性测试 {#durability-test-executed}
...
## 从业务利益相关者处注销回退系统 {#fallback-system-sign-off-from-business-stakeholders}
...
在开展项目之前，需要签订最后的合同。 这基于合同 [草稿](#contract-draft)。
...
## 实施——内容体系结构 {#implementation-content-architecture}
...
## 实施——备用系统和过程 {#implementation-fallback-system-and-procedures}
...
## 问题跟踪系统流程的建立与集成 {#issue-tracking-system-process-is-set-up-and-integrated}
...
## 列表要使用的开发工具 {#list-of-development-tools-to-be-used}
...
## 维护任务(AEM特定)已测试并启用 {#maintenance-tasks-aem-specific-tested-and-enabled}
...
## 监测——阈值和干预 {#monitoring-threshold-and-intervention}
...
## 向系统工程师传达监控策略 {#monitoring-policy-communicated-to-system-engineer}

确保系统工程师和操作人员了解和了解任何监控策略。

## 监视报告——结构就地 {#monitoring-reports-structure-in-place}
...
## 生产签字流程和政策 {#production-sign-off-process-and-policy}
...
初 [步估计](#project-efforts-initial-estimates) 是高水平的，是根据执行工作的高水平要求作出的。
...
## 已定义节奏中的项目状态报告 {#project-status-reports-within-a-defined-cadence}
...
## 质量报告格式与节奏 {#quality-report-format-and-cadence}

定义质量报告所需的内容和格式，以及交付频率。

## 已协调的释放 {#release-coordinated}
...
## 在生产环境上运行的版本 {#release-running-on-production-environment}
...
## 可用于支持的资源投入使用 {#resources-available-to-support-go-live}
...
## 业务利益相关方的安全签字 {#security-sign-off-from-business-stakeholders}
...
## 为系统验证执行的烟雾测试 {#smoke-tests-executed-for-system-validation}
...
## 已建立解决方案审查委员会并与会议纪录集会面 {#solution-review-board-established-and-meeting-cadence-set}
...
## 解决方案签署和验收流程 {#solution-sign-off-and-acceptance-process}
...
## 规范审查和批准流程已定义和传达 {#specification-review-and-approval-process-defined-and-communicated}
...
## 已识别和验证系统风险因素 {#system-risk-factors-identified-and-verified}
...
这些计划更详细地概述了针对每个开发阶段的测试执行情况，并基于测 [试策略](#testing-strategy)。
