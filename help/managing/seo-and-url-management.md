...
* URL的可读性越强，越好；在URL中显示关键字将提升值。
   ...
   * 例如： `mybrand.com/products/product-detail.product-category.product-name.html` 首选 `mybrand.com/products/product-detail.1234.html`
...
* 确保每个页面仅通过一个协议提供。
   ...
   * 有时，网站会一直供应 `http` 到用户到达包含结帐或登录表单（在此点它会切换到）的页面为止 `https`。 在从此页面链接时，如果用户可以返回到页面并 `http` 通过这些页面进行访问， `https`则搜索引擎将作为两个单独的页面跟踪这些页面。
...
* 允许您直接对内容进行地址处理，而不是使用检索内容的通用servlet。 这样，您就可以从应用到存储库的ACL以及应用到调度程序的过滤器中受益。
...
Bin Servlets遵循许多开发人员从J2EE编程中使用的模式。 Servlet在特定路径中注册，在AEM的情况下，通常在下面 `/bin`，您可以从查询字符串中提取所需的请求参数。
...
如果作者希望从另一个位置访问页面以进行促销，则AEM的虚URL（逐页定义）可能很有用。 要为页面添加虚URL，请在“站点”控制台中导航到该页 **[!UICONTROL 面]** ，然后编辑页面属性。 在“基本 **** ”选项卡的底部，您会看到可添加虚URL的部分。 请记住，通过多个URL访问页面将分割页面的SEO值，因此应向页面添加规范的URL标记以避免出现此问题。
...
>[!NOTE]
>
> 编辑 `sling:alias` 页面属性时，可以使用“ [别名”属性设置该属性](/help/sites-authoring/editing-page-properties.md#advanced)
...
规范的URL标记是放置到HTML文档头中的链接标记，用于说明搜索引擎在为内容编制索引时应如何处理页面。 他们提供的好处是确保（不同版本）页面的索引将与相同的版本，即使页面的URL可能包含差异也是如此。
