...
AEM Mobile随时准备帮助客户履行其GDPR合规义务。 AEM Mobile中不存储任何个人数据。 如果您已配置，则可以使用Adobe ID登录Adobe Experience Mobile。

[https://aemmobile.adobe.com/signin/index.html](https://aemmobile.adobe.com/signin/index.html)

## Adobe Digital Publishing Suite {#adobe-digital-publishing-suite}

Adobe的数字出版产品(在AEM Mobile之前)支持Adobe的GDPR就绪计划。 请参阅[https://www.adobe.com/privacy/general-data-protection-regulation.html](https://www.adobe.com/privacy/general-data-protection-regulation.html)。 以下内容将提供Digital Publishing Suite产品中有关支持GDPR相关功能的具体信息，包括如何与Adobe合作来发起GDPR请求。

为确保您不会将AEM Mobile与旧版Digital Publishing Suite产品混淆，可以在以下网址登录Digital Publishing Suite产品：
...
需要以下ID来查找客户数据。 收到的任何子集都意味着其他ID不适用于此用户。
...
Adobe将删除其在App Store购买时所拥有的信息(订阅等) 但在App商店，购买仍将记录在案。 如果应用程序（最终用户）登录到App Store，则这些收据将再次被拾取并发送到Adobe，随后这些收据将被视为新购买，并将由应用程序恢复以重新获得访问权。

**Adobe在启动DELETE请求时是否会删除客户提供的权利？**

Adobe将删除其拥有的客户额外直接授权许可的信息。 如果应用程序（最终用户）登录到客户使用的OAuth机制，它将向Adobe发送信息，服务将再次获取额外的权利。

**最终用户期待什么？**

由于向应用程序分配授权的键作为查看器软件的一部分驻留在设备上，因此最终用户应卸载应用程序。 最终用户应意识到，如果重新安装应用程序，则现有购买（与App Store用户关联）和直接授权许可（与客户的OAuth用户关联）仍将恢复。

**当应用程序在设备上的人与人共享时，会出现什么情况？**

Adobe几乎没有直接关联到特定用户的信息。 它使用随机创建的UUID关联数据，UUID保存在应用程序的数据中，并在应用程序启动的每个请求中传递。 这意味着在同一设备上共享应用程序的最终用户将使用相同的UUID，且所有数据都将被认为是GDPR请求者所拥有的。 对于访问和删除请求，DPSC将考虑将应用程序共享为一个人的人员。

**Analytics跟踪哪些个人数据？**

无. 正在跟踪数据，但数据位于应用程序级别（而非个人）。 这包括启动、崩溃、关闭、活动、购买或作品集叠加等事件。 不会跟踪地理位置、名称、设备ID或IP地址。
...
以下是Digital Publishing Suite GDPR请求的输入和结果的示例：
