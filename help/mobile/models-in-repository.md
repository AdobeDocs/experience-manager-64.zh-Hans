...
下面提供了两种系统提供的模型类型 */libs/settings/mobileapps/model-types*. 如果要覆盖系统模型，请键入 *mobileapps/model-types* 需要在您希望发生覆盖的配置节点下创建节点。

例如，如果您在 */conf/myconf1* 和 */conf/myconf2* 要在 *conf1* 只有这样，您才能创建 *mobileapps/model-types* 的 *conf1*.

如果要允许将数据类型添加到模型，则模型类型必须具有名为“scaffolding”的“cq:Page”类型子节点和资源类型为 *wcm/基架/组件/基架*.

基架页面还必须包含 *dataTypesConfig* 允许使用PageContent节点上的属性来指示从此类型创建的数据类型模型。

>[!NOTE]
>
>A **基架** 是一个页面，用于定义实体可基于模型编辑的数据类型。 还可以对每种数据类型进行配置，以定义字段在UI中的显示方式以及保留数据值的方式。
...
| fieldProperties | fieldPropResourceType为 *mobileapps/caas/gui/components/models/editor/datatypes/field* |
...
叠加 */libs/settings/mobileapps/models/formbuilderconfig/datatypes* 将需要创建，然后根据需要进行自定义。

例如，可以添加字符串数据类型的叠加，以将fieldResourceType更改为自定义组件。

有关Sling资源合并的更多信息，请参阅 [在AEM中使用Sling资源合并器](/help/sites-developing/sling-resource-merger.md).
...
要了解有关从仪表板创建和管理模型的信息，请参阅 [创建模型](/help/mobile/administer-mobile-apps.md) “移动设备应用程序”的“创作”部分下。
...
>[!NOTE]
>
>的 *允许的子女* 和 *允许的父母* 属性遵循与页面模板相同的规则。 有关更多信息，请参阅 [页面模板](/help/sites-developing/page-templates-static.md).
>
>参考 *模型类型* 属性，所有模型都必须具有超类型 *mobileapps/caas/components/data/entity* 但可以具有允许自定义内容交付的子类型。 确保所有模型类型都是唯一的，也可以帮助内容服务的客户端区分数据中的对象。
...
>[!NOTE]
>
>所有模型都是模板，因此它们遵循所有AEM模板规则。 这允许使用属性，例如 *allowedParents*&#x200B;和 *allowedChildren* 属性。 在基于模型创建新图元时，这些操作非常有效。 模板规则将确保实体只能基于某些模型（具体取决于其层次结构）。
>
>要了解有关从仪表板编辑模型的信息，请参阅 [创建模型](/help/mobile/administer-mobile-apps.md) “移动设备应用程序”的“创作”部分下。

### 系统模型 {#system-models}

提供了两种类型的预定义系统模型，以便简单地重复使用内容。 无法编辑这些模型。

**页面模型** “页面”模型提供了一种快速方法来重复使用站点中的现有内容，以便由内容服务进行交付。

基于页面模型的实体的resourceType为：mobileapps/caas/components/data/pages

路径：站点页面的路径。 此路径中的内容（及其子项）将由内容服务处理程序渲染。

**资产模型** “资产”模型提供了一种快速方法，可重复使用“资产”中的现有内容以便由内容服务交付。
...
>[!NOTE]
>
>要了解有关使用这些模板从功能板创建模型的更多信息，请参阅 [创建模型](/help/mobile/administer-mobile-apps.md) “移动设备应用程序”的“创作”部分下。
