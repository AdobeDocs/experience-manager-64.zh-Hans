...
AEM Mobile内容个性化功能使AEM作 [者能够利用](#author) Adobe Target [，个性化移动应用程](https://www.adobe.com/ca/marketing-cloud/testing-targeting.html)序内容。 这允许将目标优惠投放给移动应用程序用户。 Adobe Experience Manager Mobile提供创建、目标和交付内容的能力，这些内容将为用户提供特定于其个人口味的内容。

正如AEM中经常出现的情况一样，为了作者能够开始创建此内容，管理员和开发人员需要先准备环境。

[AEM管理员](#administrator) 需要在AEM Mobile和Adobe TargetCloud Service之间建立联系。

同时，AEM Mobile [开发人](#developer) 员需要修改其现有脚本，以便于有针对性的内容创作。

## 适用于管理员 {#for-administrators}

在内容作者可以开始为移动应用程序生成目标内容之前，需要先完成许多步骤： 您可以为用户和用户组获取正确的权限集，创建云服务，为活动配置应用程序，最后生成内容。

本文将指导您完成配置AEM Mobile混合引用应 [用程序进行定位](https://github.com/Adobe-Marketing-Cloud-Apps/aem-mobile-hybrid-reference) 的过程。

今后的假设是，AEM Mobile混合参考应用程序已经成功部署，并可通过AEM Mobile仪表板访问。

在作者可以在应用程序中生成目标内容之前，需要先为AEM实例 [配置Adobe TargetCloud Service。](/help/mobile/aem-mobile-configuring-cloud-service.md)

### 权限 {#permissions}

需要访问个性化控制台的用户必须属于组 `target-activity-authors` 中。
...
**入门项目：** 如上所述，管理员需要先配置到Adobe Target服务的连接，然后作者才能生成新的目标内容。
...
head.html和body.html的示例可在AEM Mobile混合参考应 [用程序中找到](https://github.com/Adobe-Marketing-Cloud-Apps/aem-mobile-hybrid-reference) ，该应用程序显示开发人员将脚本块放在两个脚本中的位置。
...
>[!NOTE]
>
>AEM Mobile [混合引用应用程序](https://github.com/Adobe-Marketing-Cloud-Apps/aem-mobile-hybrid-reference) (Hybrid Reference App)具有默认mobileappoffer处理程序的配置。 示例中的路径属性为空，因为它取决于活动位置。 在活动作者创建活动后，应用程序管理员应通过指定指向活动的路径属性，将活动与处理函数相关联。
...
对于每个mbox，都会添加 **cq定位属性** ，使应用程序开发人员能编写自定义代码，随心所欲地使用。 AEM Mobile [混合参考应用程序](https://github.com/Adobe-Marketing-Cloud-Apps/aem-mobile-hybrid-reference) (Hybrid Reference App)有一个使用cq-targeting属性的Angular指令的示例。 内容替换的概念在何时以及如何完成，这完全取决于移动应用程序开发者。 有一个通过AEM /etc/clientlibs/mobileapps/js/mobileapps.js交付的Mobile SDK，它提供一个API来调用Adobe定位服务。 由应用程序开发人员根据应用程序的设计指定何时应进行该调用。
