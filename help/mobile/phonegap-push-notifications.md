...
将推送通知与AEM结合使用需要一些不同的技术。 首先，必须使用推送通知服务提供商管理通知和设备(AEM尚未这样做)。 两个提供者已通过AEM现成配置：[Amazon简单通知服务](https://aws.amazon.com/sns/)（或SNS）和[Pushwoosh](https://www.pushwoosh.com/)。 其次，针对给定移动操作系统的推送技术必须通过适当的服务——针对iOS设备的Apple推送通知服务（或APNS）;和Google Cloud Messaging（或GCM）（针对Android设备）。 尽管AEM不直接与这些平台特定的服务通信，但AEM必须随通知提供一些相关配置信息，以使这些服务执行推送。
