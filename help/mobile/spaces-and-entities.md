...
对于首次将内容服务配置为使用空格的用户，AEM Mobile功能板在选择 **内容服务**.

>[!CAUTION]
>
>**添加空格的先决条件**
>
>检查 **启用AEM Content Services** 以与Spaces结合使用，并在AEM Mobile应用程序功能板中启用它。
>
...
1. 选择 **创建** 创建空间。 输入 **标题**, **名称**&#x200B;和 **描述** 就是空间。
...
的 **语言副本** 功能允许作者复制其主控语言副本，然后创建项目和工作流以自动翻译内容。 语言副本创建正确的结构。 在空间中添加文件夹后，即可在空间中添加语言副本。
...
1. 选择 **内容包语言(&amp;A);** 然后输入 **标题(&amp;A);** in **创建语言副本** 对话框。

   单击&#x200B;**创建**。

   ![chlimage_1-90](assets/chlimage_1-90.png)

1. 创建语言副本后，该副本会显示在 **语言母版**.
...
1. 选择实体的模型。 这是要创建的实体类型。 单击下一步。
   ...
   >[!NOTE]
   >
   >您可以选择 **资产模型**, **页面模型**，或之前创建的实体类型模型。
   >
   >请参阅 [创建模型](/help/mobile/administer-mobile-apps.md)，以创建自定义实体。

1. 输入 **标题**, **名称**, **描述**&#x200B;和 **标记** 的值。 单击&#x200B;**创建**。
...
1. 选择要编辑的实体并单击 **编辑**.
   ...
   ***如果选择用于将实体创建为资产模型的模板，***，单击 **编辑** 允许您添加资产，如下图所示：

   ![chlimage_1-97](assets/chlimage_1-97.png)

   或者，您也可以单击 **预览** 查看json链接。

   ![chlimage_1-98](assets/chlimage_1-98.png)

   ***如果选择用于创建实体的模板作为“页面模型”(Pages Models)***，单击 **编辑** 允许您添加资产，如下图所示：
   ...
   >[!NOTE]
   >
   >添加实体后，必须保存该实体，预览链接才能正常工作。 要查看预览，请单击 **保存**. 单击 **预览** 显示已添加资产的json，如下图所示：

   ![chlimage_1-101](assets/chlimage_1-101.png)

   >[!NOTE]
   >
   >将资产添加到实体后，您可以选择 **保存** 保存更改或选择 **保存并关闭** 保存并重定向到定义实体的空间控制台列表。
