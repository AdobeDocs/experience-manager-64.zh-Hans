...
另请参阅 [安装和配置AEM 3D资产](/help/assets/install-config-3d.md)[和高级配置设置](/help/assets/advanced-config-3d.md)。
...
另请参 [阅将AEM 3D与Autodesk Maya集成](/help/assets/integrate-maya-with-3d.md) , [将AEM 3D与Autodesk 3ds Max集成](/help/assets/integrating-aem-3d-with-autodesk-3ds-max.md)。
...
| MA, MB | 本机Autodesk Maya | 仅限Windows | “作者”节点上需要Autodesk Maya才能启用这些文件格式。 请参 [阅将AEM 3D与Autodesk Maya集成](/help/assets/integrate-maya-with-3d.md)。 |
| JT | Siemens PLM Open CAD | 仅限Windows | “作者”节点上需要Autodesk Maya才能启用这些文件格式。 请参 [阅将AEM 3D与Autodesk Maya集成](/help/assets/integrate-maya-with-3d.md)。 |
| * | 可以启用Autodesk Maya支持的其他3D输入格式。 请参 [阅启用Maya支持的其他格式](/help/assets/integrate-maya-with-3d.md#enabling-additional-formats-supported-by-maya)。 | 仅限Windows | “作者”节点上需要Autodesk Maya才能启用这些文件格式。 请参 [阅将AEM 3D与Autodesk Maya集成](/help/assets/integrate-maya-with-3d.md)。 |
| MAX | 本机Autodesk 3ds Max | 仅限Windows | 创作节点上需要Autodesk 3ds Max才能启用此文件格式。 请参 [阅将AEM 3D与Autodesk 3ds Max集成](/help/assets/integrating-aem-3d-with-autodesk-3ds-max.md)。 |
...
* **下载包含依赖项和演绎版的** 3D资产——从AEM下载3D资产时 **** ，如果同时选中“依赖项” **和“演绎版”** ，则下载不仅包括主3D资产的演绎版，还包括所有依赖项的演绎版。
...
* **上传后未解析的文件依赖** -当使用相同的上传操作上传3D资产及其依赖项时，某些依赖项可能无法自动解析。 如果从属文件很大，则更可能出现此问题。 要更正此问题，请访问资产 **的“属性** /依赖关系”页，该页在上传后显示未解析的依赖项。 现在应显示以前未解析的依赖项。 单击 **保存** ，以完成资产。 要防止将来出现此问题，您可以在上传3D对象之前，在单独的事务中上传所有依赖项。

* **区分大小写** -自动相关性解析尝试以区分大小写的方式匹配文件名。 例如，如果在3D资产中找到的原始依赖关系是 `image.jpg`，则该依赖关系将解析为名为、或任何其 `Image.jpg`他大 `image.JPG`小写变体的资产。
...
* **具有IBL照明的FBX舞台** -您可以上传具有IBL照明的FBX舞台。 但是，FBX格式没有传输IBL图像名称的规定。 因此，文件依赖关系解析失败。 上传后，必须手动将IBL图像分配给舞台。 您可以将相同的32位TIFF图像分配给三个依赖项，即扩 **散照明环境图像**、 **反射环境图像和**&#x200B;背景环境图像 ****，或者可以分配不同的图像。
...
* **在站点中预览时3D视图缺失** -在站点中 **使用** 预览时，必须在浏览器中重新加载页面才能完全初始化3D查看器。 在作者节点或发布节点上直接视图网页(即从 `edit.html` 路径中删除时)时，这不是问题。
