...
## AEM 6.4.8.4中包含的内容 {#what-s-included-in-aem}
...
>[!NOTE]
>
>AEM累积修补程序包不包括适用于AEM Forms的修补程序。 它们使用单独的Forms附加组件包交付。 此外，还发布了累积安装程序，其中包含JEE上的AEM Forms修复。 有关更多信息，请参阅 [安装AEM Forms附加组件包](#install-aem-forms-add-on-package) 和 [安装AEM Forms JEE安装程序](#install-aem-forms-jee-installer).
...
* 在具有MongoDB和多个实例的部署中，使用包管理器在其中一个创作实例上安装AEM 6.4.8.4。
* 在安装累积修补程序包之前，请确保拥有AEM实例的快照或新备份。
* 安装前重新启动实例。 尽管仅当实例仍处于更新模式时才需要执行此操作（这种情况下实例刚从早期版本更新），但通常建议当实例运行较长时间时执行此操作。

>[!NOTE]
>
>Adobe不建议移除或卸载AEM 6.4.8.4包。
...
>[!NOTE]
>
>**在6.4.8.4的安装过程中，有时会出现包管理器UI上的对话框**
>
>因此，建议在访问实例之前等待错误日志稳定下来。 在确保安装成功之前，用户必须等待与卸载更新程序包相关的特定日志。 它通常在Safari上发生，但可能间歇性地在任何浏览器上发生。

### 自动安装 {#auto-installation}

有两种方法可自动将AEM 6.4.8.4安装到正在运行的实例中：

A.将包放入……*/crx-quickstart/install* 文件夹。 包会自动进行安装。

B.使用 [包管理器中的HTTP API](https://docs.adobe.com/content/docs/en/crx/2-3/how_to/package_manager.html)  — 确保使用 `cmd=install&recursive=true`  — 以便安装嵌套包。

>[!NOTE]
>
>AEM 6.4.8.4不支持Bootstrap安装。

### 验证安装 {#validate-install}

1. 产品信息页面(*/system/console/productinfo*)此时应在“已安装的产品”下显示更新的版本字符串“Adobe Experience Manager，版本6.4.8.4”。
...
>[!NOTE]
>
>如果您没有使用AEM Forms，请跳过。 AEM Forms中的修复通过单独的附加组件包提供。
...
>[!NOTE]
>
>如果您没有在JEE中使用AEM Forms，请跳过。 AEM Forms JEE中的修复通过单独的安装程序提供。
...
| 区域 | 专题 | 替换 | 版本号 |
|---|---|---|---|
| Assets | 管理子资产的标记操作 | 无替换项 | AEM 6.4.2.0 |
| 资产和Adobe Creative Cloud集成 | AEM 6.2 中引入了 [AEM 到 Creative Cloud 的文件夹共享](https://experienceleague.adobe.com/docs/experience-manager-64/assets/administer/aem-cc-folder-sharing-best-practices.html)功能，以便创意用户可以从 AEM 访问资产。Creative Cloud应用程序Adobe资产链接中发布的一项新功能，提供了更好的用户体验，并可直接从Photoshop、InDesign和Illustrator内部访问AEM的资产。 Adobe将不会进一步增强文件夹共享功能。 虽然AEM中包含该功能，但强烈建议客户使用替换。 | Adobe资产链接或桌面应用程序。 有关更多信息，请参阅 [AEMCreative Cloud集成](/help/assets/aem-cc-integration-best-practices.md) 文章。 | AEM 6.4.4.0 |
...
以下文本文档列出了AEM 6.4.8.4中包含的OSGi包和内容包。
