---
title: AEM 6.4 Service Pack发行说明
...
# AEM 6.4 Service Pack发行说明 {#aem-service-pack-release-notes}
...
| 类型 | Service Pack版本 |
| 日期 | 2020 年 3 月 5 日 |
| 下载 URL | AEM 6.4.8.0 on [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq640/servicepack/aem-service-pkg-6.4.8.zip) |

## AEM 6.4.8.0中包含的内容 {#what-s-included-in-aem}

AEM 6.4.8.0是一项重要更新，其中包括自AEM 6.4在 **2018年4月。**
...
* AEM Assets现在通过Brand Portal配置 [!DNL Adobe I/O].
...
* 在使用Dynamic Media Classic配置将资产从一个文件夹移到另一个Experience Manager时，会在Dynamic Media Classic中创建一个无名称的文件夹(NPR-32440)。

* 在以Dynamic Media Scene7模式运行的Experience Manager中，PDF文件的“资产详细信息”页面不显示操作按钮(NPR-32316)。
...
* PSD资产摄取导致CPU尖峰和Experience Manager创作实例无响应(NPR-32165)。
...
* 将Dynamic Media云配置放置到专用文件夹中后，无法上传Scene7视频编码 `/conf` 而不是 `/conf/global` (NPR-31779)。
...
* 的 **公司** 和 **报表** 包下拉菜单隐藏一次 **报表源** 在Experience Manager云服务中配置Adobe Analytics时选中此选项(NPR-31729)。
...
>[!NOTE]
>
>AEM Service Pack不包含针对AEM Forms的修复。 它们使用单独的Forms附加组件包交付。 此外，还发布了累积安装程序，其中包含JEE上的AEM Forms修复。 有关更多信息，请参阅 [安装AEM Forms附加组件包](#install-aem-forms-add-on-package) 和 [安装AEM Forms JEE安装程序](#install-aem-forms-jee-installer).

* 设计器：如果启用了标记选项，则子表单边框会在生成的PDF输出中消失(NPR-32546、NPR-32322)。
...
* 文档服务：尝试使用toPDFA转换方法将PDF文件转换为PDF/文档时，会显示错误消息(NPR-32663)。

* 文档服务：对Reader文件应用PDF扩展服务时，会显示异常(NPR-32639)。
...
AEM 6.4.7.0是一项重要更新，其中包括自中的AEM 6.4正式发布以来发布的性能、稳定性、安全性以及关键客户修复和增强功能 **2018年4月。**
...
* 已在 **DAM列表** 查看和查看资产搜索结果 **列表** 视图(NPR-31311)。
* 资产排序依据 **名称** 列在 **列表** 中。
* 重新处理和批量上传的批处理大小和工作流步骤超时现在可以从Dynamic Media的UI中进行配置。
* 的 `pdfBrochure` 已在Scene 7云配置中设置为false，以在IPS中保存内存。

##### Assets {#assets-6470}

**产品增强功能**

* API包的导出版本 `package com.day.cq.dam.handler.standard.msoffice` 受支持 `dam-handler` 包已升级到6.0.0(CQ-4279059)。
如果您使用的是包 `com.day.cq.dam.handler.standard.msoffice` 在自定义实施中，建议您编译 `dam-handler` 与最新的uber jar捆绑在一起。
...
* 某些PDF文档的元数据未更新，且在修改其标题时会保存到PDF中(NPR-31575)。
...
* 使用Tika解析器无法解析具有.docx和.xlsx文件扩展名的MS Office文件(包含JPEG图像)(NPR-31693)。
...
* 将ReaderExtension权限应用于PDF会导致出现错误消息(NPR-30930)。
...
* 转换PDF服务无法将PDF文档转换为PostScript，并显示一条错误消息(NPR-31267)。

* 在应用修补程序以修复PDF失败的HTML后，SOAP端点配置将重置(NPR-31309)。
...
AEM 6.4.6.0是一项重要更新，它包括自中的AEM 6.4正式发布以来发布的性能、稳定性、安全性以及关键客户修复和增强功能 **2018年4月。**
...
* 文件夹的资产共享链接，其中包含空格和 `&` 名称中的字符会为某些资产显示空白灰色卡片。 NPR-29934：适用于 CQ-4270187 的修补程序
* 为AEM创建MP4资产时，DAM工作流崩溃。 NPR-30031：适用于 CQ-4271352 的修补程序
* Adobe通过Datapower的智能标记连接问题。 NPR-30026：适用于 CQ-4269457 的修补程序
* PDF无法使用OmniSearch找到。 NPR-30046：适用于 GRANITE-26290 的修补程序
* ACP API生成的URL和文件夹元数据中的资产路径不进行URL编码。  GRANITE-26198:适用于CQ-4271814的修补程序
...
* 按相关性对搜索查询进行排序，会返回InDesign文档和InDesign模板。 适用于 CQ-4273864 的修补程序
* 如果用户的电子邮件ID为大写，则用户将无法签入之前已签出的资产。 适用于 CQ-4276575 的修补程序
* 在中配置Dynamic MediaCloud Services `DMHybrid` 模式会导致在AEM中创建多个空报表包，而中未存储任何报表包id，从而导致报表包重复。 适用于 CQ-4276855 的修补程序
...
* (IE 11)在富文本编辑器组件中粘贴HTML内容时，如果默认粘贴模式为纯文本，则HTML会粘贴格式，因此，默认粘贴模式不起作用。 NPR-30045：适用于 GRANITE-26094 的修补程序
...
* AEM与Campaign集成期间出现ContextHub错误。 NPR-30625：适用于 CQ-4250790 的修补程序
* resourceType请求参数的值将复制到HTML标记属性的值中，该值封装在双引号中。 NPR-29832：适用于 CQ-4255365 的修补程序
...
* 打开包含字符美元的内容片段时解析错误(`$`)和大括号(`{`)一个接一个。 适用于 CQ-4270266 的修补程序
* 尝试在时间轴中显示体验片段的版本时，VersionPreviewServlet在NullPointerException中失败。 NPR-30074：适用于 CQ-4271881 的修补程序
* 无法通过签入功能锁定内容片段。 NPR-29923：适用于 CQ-4258785 的修补程序
* SAML身份验证处理程序中的签名验证失败。 NPR-30379:GRANITE-26567的后端请求

**复制**

* 在OAuth实施期间，每次复制到MAC / Brand Portal时，JCR会话/资源解析程序都会被泄漏。 NPR-30000：适用于 Granite-26196 的修补程序
...
* AEM默认邮件发送者无法通过TLS v1.2向远程SMTP服务器发送邮件。NPR-30476:适用于GRANITE-26605的修补程序
...
* 在重新启动实例之前，发布实例上无法正确显示自定义内容。 NPR-30421：适用于 CQ-4273706 的修补程序

**项目**

* dam:folderThumbnailPaths值不会刷新并显示旧缩略图，即使在删除文件夹中的资产后也是如此。 NPR-30424：适用于 CQ-4273667 的修补程序
...
* PDF/Acrobat预检中缺少标签错误。 NPR-30594：适用于 CQ-4276032 的修补程序
* PDF中的单字符数据绑定会导致Reader扩展失败，并出现错误“java.lang.StringIndexOutOfBoundsException:字符串索引超出范围：1”。 NPR-30128：适用于 CQ-4273878 的修补程序
* 在HTMLPDF服务时执行加载测试时，测试失败，并出现错误，并且文件类型设置将从AEM Forms服务器中删除。 NPR-30085：适用于 CQ-4272631 的修补程序
* 使用Adobe Acrobat 9.1（XFA版本3.0）拼合PDF时，不会保留PDF表单状态：窗体上不可见的元素会重新设置为可见状态。 NPR-29978：适用于 CQ-4270888 的修补程序

**表单 - 文档安全**

* 应用带有时间戳的签名失败，并出现错误：ALC-DSC-003-000:com.adobe.idp.dsc.dscinvationException:调用错误。 NPR-30696、NPR-30537：适用于 CQ-4273778 的修补程序
* 配置管理器不会为本地化的表列插入日语字符串。 NPR-30496：适用于 CQ-4274868 的修补程序

**PDFG 服务**

* 尝试在Windows Server 2016上将Word文档转换为PDF时出现连接错误。 NPR-30597：适用于 CQ-4275652 的修补程序
* 尝试使用HTML通过“phantomjs”库PDF后端服务时，出现权限拒绝异常。 NPR-30456：适用于 CQ-4258077 的修补程序
* JBoss管理控制台未显示HTML到PDF服务的maxReuseCount。 NPR-30303、NPR-30135：适用于 CQ-4273763 的修补程序

#### AEM 6.4.5.0 {#experience-manager-6450}

AEM 6.4.5.0是一个重要更新，它包括自中推出AEM 6.4通用版本以来发布的性能、稳定性、安全性以及关键客户修复和增强功能 **2018年4月。**
...
* 已禁用 `AssetDownloadServlet` 默认情况下，AEM发布实例上的OSGi组件。 有关更多信息，请参阅 [从AEM下载资产](/help/assets/download-assets-from-aem.md).
* 为资产启用了多站点管理器支持。 有关更多信息，请参阅 [使用MSM为资产重复使用资产](/help/assets/reuse-assets-using-msm.md).
...
* VersioningTimelineEventProvider应提供根版本和版本注释。 适用于GRANITE-26063的修补程序
* 插入数据会导致在服务器端执行代码。 适用于 CQ-4270246 的修补程序
* 为资产启用了多站点管理器支持。 适用于CQ-4271453、CQ-4268621、CQ-4257491的修补程序
* AEM界面应在时间轴历史记录中为资产的当前版本显示一个额外条目，以显示Adobe资产链接中的最新签入评论。 适用于 CQ-4262864 的修补程序
...
* 与多字段一起使用时，CoralUI会在组件级别而不是多字段级别存储fileReferenceParameter。 NPR-29536：适用于 CQ-4266129 的修补程序
...
* (IE 11)在RTE组件中复制粘贴HTML内容（默认粘贴模式为纯文本）时，不会将内容粘贴为纯文本。 NPR-29432、NPR-29171:适用于GRANITE-24941的修补程序
* 富文本编辑器拼写检查不再适用于其他语言。 NPR-29506：适用于 CQ-4264990 的修补程序
* Campaign页面上的跨站点脚本(XSS)。 NPR-29614：适用于 CQ-4269322 的修补程序
* 在源代码编辑模式下，从全屏最小化富文本编辑器会导致内容丢失。 NPR-29574：适用于 CQ-4260584 的修补程序
...
* 在实例上启用缩小后，在将组件添加到WCM页面时会生成错误警报。 NPR-29396：适用于 CQ-4266196 的修补程序
* “样式系统”节点从父节点继承时出现问题。 NPR-29296：适用于 CQ-4266041 的修补程序
* 使用时间扭曲还原的页面应引用版本控制时正确的图片。  NPR-29431：适用于 CQ-4262638 的修补程序
...
* 在使用图像就地编辑器后，图像组件不会调用“afteredit”侦听器。 NPR-29616：适用于CQ-4268065的修补程序
* 设置向Facebook的社交发帖时出现问题。 NPR-29212：适用于 CQ-4266630 的修补程序
* 提升已修改页面的启动项时，会考虑源分支和启动项分支中的修改。 NPR-29308：适用于 CQ-4266746 的修补程序
* 内容片段中呈现的缩略图显示“日期和时间”字段的内部日历表示形式。 NPR-29531：适用于 CQ-4269362 的修补程序
...
* 用户提供的数据不会在cq/replication/components/agent组件的输出中进行转义，从而导致存储型跨站点脚本(XSS)漏洞。 适用于 CQ-4266263 的修补程序
...
* 没有审核者权限的访客和成员可以通过粘贴URL来查看未批准/待决帖子。 NPR-29726:适用于CQ-4271124、CQ-4271441的修补程序
* 在用户登录社区时，可观察到长达40-50秒的高响应时间。 NPR-29678：适用于 CQ-4269444 的修补程序
...
* BrightEdge凭据失败，出现连接错误。  NPR-29167：适用于 CQ-4265872 的修补程序
...
* 无法将资产从AEM创作/content/dam/mac文件夹发布到Brand Portal。 NPR-29820：适用于 CQ-4271118 的修补程序
...
* 删除Live Copy中的子节点时，应该分离LiveRelationship。 适用于 CQ-4270395 的修补程序
* 升级到AEM 6.4.3会使多站点管理器需要很长时间才能推出。 适用于 CQ-4271410 的修补程序
...
* 为Microsoft Dynamics内部部署集成启用了对Active Directory联合身份验证服务(ADFS)v3.0的支持。 NPR-30003、NPR-29484：适用于 CQ-4270586 的修补程序
...
* 无效的PDF/文件会报告为使用isPDFA操作的有效PDF/A。 NPR-29076：适用于 CQ-4261541 的修补程序
* PDF无法转换为PDF/A-1b，但表单字段没有外观dict。 NPR-29534：适用于 CQ-4269618 的修补程序
* PDF/使用输出服务生成的PDF转换未通过Acrobat DC验证。 NPR-29647：适用于 CQ-4270448 的修补程序
* Apache POI包失败，出现异常。 NPR-27861、NPR-28048：适用于 CQ-4245898、CQ-4244778 的修补程序

**Forms — 设计器**

* 为使用设计器和输出服务生成的XML Forms架构(XFA)表单添加了PDF/UA支持。 NPR-23022
...
* 为资产启用了多站点管理器支持。 有关更多信息，请参阅 [使用MSM为资产重复使用资产](/help/assets/reuse-assets-using-msm.md). NPR-26450：适用于 CQ-4259922 的修补程序
...
AEM 6.4.4.0是一项重要更新，它包括自中的AEM 6.4正式发布以来发布的性能、稳定性、安全性以及关键客户修复和增强功能 **2018年4月。**
...
* AEM OOTBPDF处理程序卡在带有外字符的处理PDF上。 NPR-28778：适用于 CQ-4254234 的修补程序
...
* 桌面操作不适用于Omnisearch的 `aem/start.html` 页面。 NPR-27242：适用于 CQ-4248176 的修补程序
...
* SugmentationHandler中的6.4跨站点脚本(XSS)。NPR-28693:适用于CQ-4253821的修补程序
...
* 在“发布”中为AEM Communities启用组件启用了键盘导航。 NPR-27728：适用于 CQ-4254028 的修补程序
...
* 无法使用API删除用户生成内容中的注释。 NPR-28075：适用于 CQ-4261135 的修补程序
...
* `com.day.cq.personalization.impl.TeaserResourceEventHandler` 进入无限循环，并导致对发布实例上的节点进行更新。 NPR-28561：适用于 CQ-4263096 的修补程序
...
* 自适应表单不使用富文本编辑器，而是加载其库。  NPR-28001:适用于CQ-4259703、CQ-4259193的修补程序
...
在发行版640分支上安装Forms管理包后，一个包仍处于安装状态。 提交的CQ-4265410 Forms的修补程序及其中的附件不会显示在工作流中，并选中提交操作“调用AEM Forms工作流”并启用门户提交。 适用于 CQ-4263110 的修补程序
...
* XDP模板中的某些JPEG图像无法正确呈现。  NPR-26702:适用于LC-3917457的修补程序

**表单 - 工作流**

* HTML5在an.lca中具有默认提交流程的Forms在JBoss 7上不起作用。 NPR-28675：适用于 CQ-4243928 的修补程序
...
* OSGi配置中已启用的功能 **AEM工作流提取功能标记**. 撤消操作应在设置标记后终止工作流实例。 NPR-26451：适用于 CQ-4259090 的修补程序
...
* 如果启用了IDS分离选项，则DAM更新资产工作流不会从INDD文件中提取引用。 NPR-26243;适用于CQ-4250933的修补程序
* 使用资产批量编辑器发布资产时，不会显示成功消息。 NPR-26252;适用于CQ-4251688的修补程序。
...
* 列表视图中用于重新排序文件夹和内容片段的选项不会针对适用的文件夹显示。 NPR-27153;适用于CQ-4255873的修补程序
* 用户无法将资产添加到新收藏集，因为这会导致错误弹出对话框中出现图像损坏的错误消息。 NPR-22431;适用于CQ-4237086的修补程序
* 动态下拉列表不支持级联下拉列表。 NPR-27043;适用于CQ-4252564的修补程序
* 如果用户在DMS7云配置中为“公司根文件夹”设置了非默认值，则查看器预设将无法按预期工作。 NPR-26360;适用于CQ-4249505的修补程序
* 对于具有大量资产的实例，资产报告失败。 NPR-27278;适用于CQ-4256748的修补程序
* 子文件夹不会继承父文件夹的SmartCrop图像配置文件。 NPR-27197;适用于CQ-4256273的修补程序
* 启用Dynamic Media集成后，会修改Assets的某些元数据属性。 将不显示宽度、高度和位置属性。 NPR-27203;适用于CQ-4256258的修补程序
* Dynamic Media不会为某些类型的资产使用配置的代理。 NPR-25211;适用于CQ-4244871的修补程序
* “元数据编辑器”页包含无效项目参数的空指针异常。 NPR-26169;适用于CQ-4241368的修补程序。
...
* 当富文本编辑器与多个字段组合时，未捕获类型错误：fieldAPI.getName不是foundation.js错误时的函数。 NPR-27146：适用于 CQ-4253155 的修补程序
...
* 用户无法在页面属性中编辑已关闭的用户群组权限。 NPR-27138:适用于CQ-4256089的修补程序无法通过标记来编辑标记。 NPR-26957：适用于 CQ-4254858 的修补程序
...
* 如果将data-sly-use与具有相同名称的类一起使用，则会产生不可兼容的代码。 NPR-27282:适用于Sling-7581的修补程序
...
* 在资产(如PDF)中创建注释后，UI会启动无限的页面刷新循环。 NPR-27052：适用于 CQ-4255396 的修补程序

**DAM - DM服务**

* Dynamic Media不会为某些类型的资产使用配置的代理。 NPR-10727;适用于CQ-4244871的修补程序
...
* 论坛帖子和回复会添加在第二页顶部，刷新后，帖子将移至第一页顶部的正确位置。 NPR-27342：适用于 CQ-4247338 的修补程序
...
* 在论坛中上传图像以外的附件（例如.pdf）时发现错误。 NPR-27360：适用于 CQ-4257753 的修补程序
* 如果使用MySQL DSRP设置了“创作 — 发布”，则对论坛帖子取消固定或取消功能不起作用。 NPR-26125;适用于CQ-4251520的修补程序
...
* 其他用户的活动和删除了在使用审核者凭据登录时可见的UGC，并在审核者的配置文件URL末尾添加“#social-activities”或“#tabs-2”。 适用于 CQ-4251355 的修补程序
...
* 用户订阅限制为1000。 NPR-27614：适用于 CQ-4254689 的修补程序
* 在角色设置中编辑站点和编辑角色会引发空指针异常。 NPR-27377;适用于CQ-4255909的修补程序
...
* 无法加载站点和表单编辑器。 NPR-26977;适用于CQ-4249170的修补程序
* 使用键盘将附件添加到自适应表单时出现问题。 NPR-25913;适用于CQ-4249456的修补程序
...
* 如果信件包含列表模块，则在从信件生成PDF时，项目符号和文本之间的空格将丢失。 适用于 CQ-4250588 的修补程序
...
* 无法显示PDF生成器的标准报告和合规性设置。 NPR-26715：适用于 CQ-4253384 的修补程序
* AIX Forms附加组件包中缺少convertpdf二进制文件，这会在调用PDFA服务时导致失败。 适用于 CQ-4257873 的修补程序
* 处理TIFF文件时，纸面捕获服务崩溃。 NPR-28079：适用于 CQ-4240649 的修补程序
...
AEM 6.4.2.0是一项重要更新，它包括自中的AEM 6.4正式发布以来发布的性能、稳定性、安全性以及关键客户修复和增强功能 **2018年4月。**
...
* 已将solr的搜索结果最大限制修改为MAX_INT-10000。
...
* HTML模板语言规范1.4中支持到AEM 6.4.2.0的功能NPR-24585
* 复制Live Copy页面后，本地组件上的继承错误。 NPR-25920:适用于CQ-4236737、CQ-4248957的修补程序
...
* siteadmin / damadmin中的HTML结构错误，会中断IE11。 NPR-24686:适用于CQ-4246363、CQ-4248402的修补程序
* （管理发布向导）在范围步骤中执行某些操作后，选项步骤中激活日期的日历未打开。 NPR-25681：适用于 CQ-4250205 的修补程序
* 由于弃用，经典UI无法编辑CUG。 NPR-25075：适用于 4241823 的修补程序
* 创建选项不可用于创建体验片段。 NPR-26053：适用于 CQ-4249923 的修补程序
* XF变量因此被激活两次，从而为同一项目生成重复的ID。 NPR-24179：适用于 CQ-4245093 的修补程序
* 基础表容易遭受存储型跨站点脚本攻击。 NPR-25185：适用于 CQ-4240760 的修补程序
* 将组件从AEM 6.2.1.13迁移到AEM 6.4时出现“无效的递归选择器值”错误。NPR-24146

**WCM - 页面编辑器**

* 由于长时间运行的查询（超过6个）而导致多个堆叠的Parsys导致AEM运行缓慢。 适用于 CQ-4240247 的修补程序
...
* Target引擎(mbox.js、at.js)不使用损坏的URL，而是使用包含冒号的URL，在某些部署中，冒号可能会失败。 NPR-25339：适用于 CQ-4237854 的修补程序
...
* 在应用ContentDispositionFilter两次后，响应中未传播和缺少Content-Type。 NPR-24175:适用于Sling-7525的修补程序
...
* 修复了几个Adobe Social问题。 NPR-24247：适用于 CQ-4245054、CQ-4245120、CQ-4245296 的修补程序
* 如果作者发布在不同的上下文路径上运行，则成员和组控制台的无限滚动失败。 NPR-24437：适用于 CQ-4246013 的修补程序
* 即使从已应答状态中删除帖子，帖子也不会返回未应答状态，并且得分不会减少。 NPR-24419:适用于CQ-4245797、CQ-4245932的修补程序
...
* 在UI中，启用Scorm资源完成状态标签显示为空。 NPR-26034：适用于 CQ-4249994 的修补程序
...
* 启用“允许特权”后，特权成员只能为社区成员的用户撰写内容。 NPR-25877：适用于 CQ-4248450 的修补程序
* 阻止深层链接以启用。 NPR-25966：适用于 CQ-4251478 的修补程序
* 在删除回复时，论坛分页不会动态更新。 NPR-25970:适用于CQ-4248975、CQ-4252843的修补程序
...
* 嵌入分辨率非常高的图像时，论坛线程出现问题。 NPR-26037:适用于CQ-4244453、CQ-4253099的修补程序
...
* 主动式Foundation UI后台。 NPR-24344、NPR-24345、NPR-25176、NPR-25095、NPR-24332、NPR-25653、NPR-25932、NPR-25935、NPR-25976
...
* 包SymbolicName中的Typo会导致重复的包。 适用于Granite - 22155的修补程序
* CUGConfiguration不能选取CugExclude。 适用于Granite - 21109的修补程序
...
* 当对Web服务的调用在客户端中打开HTML5表单时，该调用失败，并返回一条错误消息。 NPR-26048：适用于 CQ-4244716 的修补程序
* 在AEM Forms Windows应用程序6.3中调用服务时出现问题。NPR-26468:适用于CQ-4252341的修补程序

**移动设备表单**

* （表单集）预览时，SSN和移动设备字段验证出现问题。 NPR-24458：适用于 CQ-4244983 的修补程序
* 在HTML预览中，预填多行字段时，不会显示数据。 NPR-24549：适用于 CQ-4244212 的修补程序
...
* PDF预览URL在发布时不显示事务报表。 适用于 CQ-4246158 的修补程序
...
* 管理员UI项目链接中的跨站点脚本(XSS)。 NPR-23272：适用于 CQ-4241795 的修补程序
...
* com.day.cq.analytics.sitecatalyst中的ResourceResolver未关闭。 NPR-22340：适用于 CQ-4236515 的修补程序
* 在长时间运行的查询期间，TargetContentImpl使AEM运行缓慢。 NPR-22359：适用于 CQ-4236907 的修补程序
* Target引擎(mbox.js、at.js)不使用损坏的URL，而是使用包含冒号的URL，在某些部署中，冒号可能会失败。 NPR-22434：适用于 CQ-4237854 的修补程序
* 在Target模式下，作者可以修改从Blueprint继承的组件，而无需取消继承。 NPR-22865：适用于 CQ-4237907 的修补程序
...
* 配置引用复制失败，中间文件夹结构为。 NPR-23485：适用于 CQ-4242751 的修补程序
* （个性化）由代理Servlet阻止的clientlib。 NPR-23521：适用于 CQ-4240995 的修补程序
* （Adobe IMS控制台）配置向导中未提取已注册的云解决方案。 NPR-23977：适用于 CQ-4244549 的修补程序
* 在没有HTML扩展的页面上加载目标内容时，无限循环。 NPR-23522：适用于 CQ-4223600 的修补程序
* 具有继承的动态Tag Management配置引用的页面激活失败。 NPR-23485：适用于 CQ-4242751 的修补程序
...
* 站点内的内容片段未翻译。 适用于CQ-4243482、CQ-4243483、CQ-4245687的修补程序
...
* (DMS7-UA)单击已发布视频资产的“嵌入”按钮时，不会发生任何情况。 “嵌入”对话框应会随HTML代码一起显示。 适用于 CQ-4245237 的修补程序
* （DM混合）获取“`[object Object]`”。 适用于 CQ-4245236、CQ-4245451 的修补程序
...
* (DMHybrid)6.3和gt；之后6.4升级时，目录设置将移至错误节点。 适用于 CQ-4239974、CQ-4239975 的修补程序
* (DMHybrid)pushviewerpresets脚本不会创建修改目录设置所需的节点。 适用于 CQ-4240076 的修补程序
* 使用“格式”下拉选择并选择PNG或JPG格式时，下载的文件会显示为超饱和且比原始资产深。 适用于 CQ-4240073 的修补程序
...
* 在 `ACTION TAKEN` “争议类型”选项卡中的“支持我的声明的其他文档”部分保存了“其中”的其他字段校样类型。 适用于 CQ-4239873 的修补程序
...
* HTML5设置为浮动或小数的数值框在不同的浏览器中会给出不同的验证结果。 NPR-23528：适用于 CQ-4244097 的修补程序
...
* 汇编程序服务中的事务记录。 适用于CQ-4245018、CQ-4245017、CQ-4245016的修补程序。
...
* (HTML工作区)在首次打开草稿时，准备两次数据调用。 适用于 CQ-4243280 的修补程序
* (HTML工作区)关闭表单时触发“准备数据”流程。 适用于 CQ-4239456 的修补程序
* 在两个任务之间遍历时，工作区会挂起。 适用于 CQ-4237125 的修补程序

**Workbench**

* 管理操作配置文件当默认渲染进程配置设置为“HTML”时，准备数据进程失败。 适用于 CQ-4244292 的修补程序

**Forms Designer**

* 将PDF/UA支持添加到通过Designer和输出服务生成的XML表单。 NPR-23022
* 当从Designer中另存为PDF时，在Designer中定义的内嵌超链接既未标记，也没有替换文本。 NPR-23023
...
* 可在 [软件分发门户](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html) 下载。
* 在具有MongoDB和多个实例的部署中，使用包管理器在其中一个创作实例上安装AEM 6.4.8.0。
* 在安装Service Pack之前，请确保拥有AEM实例的快照或新备份。
* 安装前重新启动实例。 尽管仅当实例仍处于更新模式时才需要执行此操作（这种情况下实例刚从早期版本更新），但通常建议当实例运行较长时间时执行此操作。

>[!NOTE]
>
>Adobe不建议移除或卸载AEM 6.4.8.0包。

### 通过包管理器安装Service Pack {#install-the-service-pack-via-package-share}

执行以下步骤以在现有AEM 6.4实例上安装Service Pack:

1. 从Software Distribution下载包。

1. 在AEM中，登录到包管理器，并添加下载的AEM 6.4.8.0包。 选择上传的包，然后单击 **[!UICONTROL 安装]**.

>[!NOTE]
>
>**在6.4.8.0的安装过程中，有时会出现包管理器UI上的对话框**
>
>因此，建议在访问实例之前等待错误日志稳定下来。 在确保安装成功之前，用户必须等待与卸载更新程序包相关的特定日志。 它通常在Safari上发生，但可能间歇性地在任何浏览器上发生。

### 自动安装 {#auto-installation}

有两种方法可自动将AEM 6.4.8.0安装到正在运行的实例中：

A.将包放入……*/crx-quickstart/install* 文件夹。 包会自动进行安装。

B.使用 [包管理器中的HTTP API](/help/sites-administering/package-manager.md)  — 确保使用 `cmd=install&recursive=true`  — 以便安装嵌套包。

>[!NOTE]
>
>AEM 6.4.8.0不支持Bootstrap安装。
...
>[!NOTE]
>成功安装包后，会显示一条信息性消息，指示已成功安装内容包，例如 **&quot;已成功安装内容包AEM-6.4-Service-Pack-7。&quot;**
...
#### 安装AEM Forms附加组件 {#installaemformsaddon}
...
>[!NOTE]
>
>AEM 6.4.8.0包含新版本的 [AEM Forms兼容包](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html). 如果您使用的是旧版AEM Forms兼容包并更新到AEM 6.4.8.0，请在安装Forms附加组件包后安装最新版本的AEM Forms兼容包。

1. 确保您已安装AEM Service Pack。
1. 下载列于的相应Forms附加组件包 [AEM Forms版本](https://helpx.adobe.com/aem-forms/kb/aem-forms-releases.html) 操作系统。
...
有关安装AEM Forms JEE的累积安装程序和部署后配置的信息，请参阅 [AEM Forms JEE补丁安装程序0015](https://helpx.adobe.com/aem-forms/quick-fixes/6-4/jee-patch-0015.html).
...
适用于AEM 6.4.8.0的Uber Jar可在 [Adobe公共Maven存储库](https://repo.adobe.com/nexus/content/groups/public/com/adobe/aem/uber-jar/6.4.8/).
...
* 安装过程中可能显示以下错误和警告：
   ...
   * `com.adobe.cq.com.adobe.cq.ui.commons bundle com.adobe.cq.com.adobe.cq.ui.commons:1.2.28 (204)[com.adobe.cq.ui.wcm.commons.internal.servlets.rte.RTEFilterServletFactory(573)]`:unbindAmpendment方法引发异常(java.lang.IllegalStateException:服务已取消注册)。
这些错误不需要执行任何操作，因为它们不会影响AEM实例。
...
以下文本文档列出了AEM 6.4.8.0中包含的OSGi包和内容包。
