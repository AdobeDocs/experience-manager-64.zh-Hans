...
* 已将捆绑在ContextHub中的jQuery版本更新为3.4.1。
...
* 计划激活的日历图标不会显示在“状态”列（DAM资产列表的经典UI中）中，该激活的资产将安排在以后的日期和时间内(NPR-32198)。
...
* 在“项目”中选择“日历视图”后，在卡视图、列表视图和日历视图之间切换的功能不起作用(NPR-31829)。
...
* 尽管在后端删除了AEM资产中的元数据模式(CQ-4286871)，但在AEM资产中取消发布元数据模式会填充一条错误消息。
...
* 文档安全：将DisableGlobalOfflineSynchronizationData选项设置为True时，受保护的PDF文件无法脱机打开(NPR-32080)。
...
* 添加了在删除站点页面时设置其版本的支持。
* 在 **DAM列表视图中添加了可排序的创建日期的新列，在****** 列表视图中添加了资产搜索结果(NPR-31311)。
* 在列表 **视图中** ，已允许基于名称列的资 **产排序** 。
...
* DAM事件清除功能会删除最新的(maxSavedActivities)事件数据并保存之前创建的数据(NPR-30870)。
...
* 当场景7中已存在具有重复名称的资产时，AEM中的资产重命名无法同步到场景7(CQ-4276763)。
...
* 当LiveCopy的数量较大时，LiveCopy概述需要很长时间才能呈现预览(NPR-30945)。
...
* 内容片段模型查询效率低下。 如果实例包含大量页面并导致错误，则速度很慢(NPR-30666)。
...
* 更新了Sling分布式事件管理员至版本1.1.4，以改进群集环境中日志的质量(NPR-29256)。

##### 基础UI {#foundation-6470}

* 滚动到搜索结果页面末尾时，会导致浏览器崩溃(NPR-31332)。

* 在搜索结果页面上从卡视图切换到列表视图时，在可滚动页面之前会有延迟(NPR-31280)。
...
* 由于有效负荷未定义，“创建审阅任务”功能不起作用。 NPR-30469:修补程序CQ-4274263
* 在资产选取器上执行OmniSearch后，将视图从卡视图切换为列表视图的功能消失，反之亦然。 NPR-29852：适用于 CQ-4269369 的修补程序
...
* 在开始编辑页面的“at”活动工作流对话框中，跨站点脚本(XSS)。 NPR-29747：适用于 CQ-4271067 的修补程序
...
* 在基础API的跟踪事件中增加了对跟踪动态UI状态的支持。 NPR-30742、GRANITE-26322:GRANITE-26036的修补程序
...
* 当文档片段的描述包含属性中小于(&lt;)、大于(>)和和号(&amp;)等特殊字符时，字母草稿重新加载将失败，但有例外情况。 NPR-29930：适用于 CQ-4252762 的修补程序
...
* 未捕获的TypeError错误：无法读取未定义的属性&#39;split&#39;在尝试视图搜索结果时显示。 NPR-28803：适用于 CQ-4248371 的修补程序
...
* 计划激活图标不显示在站点管理中。 NPR-28725：适用于 CQ-4263917 的修补程序
...
* 访客和成员没有审查方权限，可以通过粘贴URL来查看未批准／待决帖子。 NPR-29726：适用于 CQ-4271124/CQ-4271441 的修补程序
...
* 将搜索结果的分页限制提高到100页卡视图,200页列表视图。 NPR-29332：适用于 GRANITE-24644 的修补程序
...
* （触屏UI）开始工作流程选项不会弹出用于配置的对话框。 NPR-29521：适用于 CQ-4269050 的修补程序
...
* 元数据模式编辑器在使用路径浏览器选择数据时的问题。 NPR-28674：适用于 CQ-4263005 的修补程序
...
* 转出显示从6.1升级到6.4之后的重复转出路径。NPR-27845:CQ-4259487的修补程序
...
* 只有读取权限，用户／用户组就无法选择XF，也无法选择视图XF及其页面属性。 NPR-28341：适用于 CQ-4260412 的修补程序
...
* 增强ACP API，使查询的结果仅限于集合的成员。 适用于 CQ-4258708 的修补程序
...
* 随机开始上sling-oak-1线程中stderr.log中的空指针异常。 NPR-28665：适用于 CQ-4237845 的修补程序
...
* 添加了用于键盘事件以打开视频的逻辑。 NPR-27726：适用于 CQ-4254026 的修补程序
...
* 增加了使用数字签名或将文档认证为可开单事务处理的支持。 NPR-28495：适用于 CQ-4260236 的修补程序
* 添加了数字签名和认证到收费API。 适用于 CQ-4260236 的修补程序

**表单管理**

增加了在Forms Manager的开始审阅向导和移动资产向导中用下划线替换handlebars客户端库的使用支持。 NPR-27643：适用于 CQ-4246536 的修补程序.
...
* 资产元数据在安装服务包后显示无效的命名空间错误。 NPR-22341;CQ-4237202的Quickfix
* 对于适用的文件夹，不会显示在列表视图中对文件夹和内容片段重新排序的选项。 NPR-27153;CQ-4255873的修补程序
...
* 资产报告对于资产数量非常大的实例失败。 NPR-27278;CQ-4256748的修补程序
...
* 当您在以后的日期使用“管理发布”选项计划页面进行激活并切换到列表视图时，日历图标缺失。 NPR-26974：适用于 CQ-4239206 的修补程序
...
* 在将视频收录到与AVS视频处理用户档案关联的文件夹中之后，浏览器窗口会反复刷新。 适用于 CQ-4253563 的修补程序
* 使用包含大写字符的临时标签时，YouTube发布会失败。 适用于 CQ-4252477 的修补程序
* 在PDF等资产中创建注释时，UI会开始一个无限的页面刷新循环。 NPR-27052：适用于 CQ-4255396 的修补程序
...
* ConfigMgr中提供了在“日志”组件中启用／禁用“计划帖子”搜索的选项，并优化了SearchScheduledPosts作业。 NPR-26923：适用于 CQ-4250463 的修补程序
...
* 除非重新加载主题（或回复），否则不会为构思触发分页，列表主题（或回复）的时间会更长。 NPR-26193：适用于 CQ-4252104 的修补程序
...
* 如果活动和体验的名称包含日语字符，则无法创建多个体验。 NPR-27271：适用于 CQ-4256857 的修补程序
...
* 在“打印渠道”的“数据源”部分中显示表名，而不是FDM关联的标题。 适用于 CQ-4253669 的修补程序
...
* 如果执行“表单”工作流时任务名称中包含大括号，则日志中会显示异常。 适用于 CQ-4256626 的修补程序
...
* 无法显示PDF生成器的“标准报告”和“规范”设置。 NPR-26715：适用于 CQ-4253384 的修补程序
...
* Assets添加了对动态条件元架构的支持以及为资产文件夹设置元数据模式的功能。
* 在每个组件中添加了配置以启用／禁用文件夹缩略图创建／更新功能。
* 页面创作时的图像编辑器增强功能。
* 社区中的回归修复，当选定答案被删除时，无法正确处理评分事件。
...
* （支持）辅助功能增强：重复ID:id属性值必须唯一，标签：表单元素必须具有标签和链接名称：链接必须具有可辨认的文本。 NPR-24252：适用于 CQ-4250905、CQ-4250906、CQ-4250907 的修补程序
...
* (Dynamic Media)将DM查看器更新为5.10.1版本，它允许在“图像预设”页面上检查重复名称。 请参阅更新Dynamic Media查看器(5.10.1)。 NPR-24403：适用于 CQ-4247554 的修补程序
...
* （管理发布向导）在“激活”步骤中的某些操作之后，“选项”步骤中的“发布日期”日历未打开。 NPR-25681：适用于 CQ-4250205 的修补程序
* 由于已弃用，经典 UI 无法用于编辑 CUG。NPR-25075：适用于 4241823 的修补程序
* 创建选项不可用于创建体验片段。 NPR-26053：适用于 CQ-4249923 的修补程序
* 因此，XF变量将被激活两次，从而为同一项生成重复ID。 NPR-24179：适用于 CQ-4245093 的修补程序
...
* 缺少相应优惠的目标组件将呈现整个页面，而不是空的目标组件，将添加页面的另一个完整再现。 NPR-25273：适用于 CQ-4248003 的修补程序
...
* (Chrome)时区转换在社区事件中失败。 NPR-24881：适用于 CQ-4247115 的修补程序
...
* (Communities Calendar)在保存包含新封面图像的事件时，会生成ClassCastException。 NPR-25167：适用于 CQ-4248662 的修补程序
* （社区）消息被截断。 NPR-25326
...
* 创建新社区组时，会在活动／活动mongoMK群集上创建重复社区组。 NPR-26032：适用于 CQ-4245884 的修补程序
...
* 支持性能修复——在创作和发布时分组分页、在创作时分组搜索、避免为日志、日历和构思序列化回复以及在查看组列表页面时延迟加载以获取每个组的组成员关系（邀请／取消邀请）按钮。 NPR-24538：适用于 CQ-4246515 的修补程序
...
* PersonalizationWizard的第二页(由“开始定位”启动)为空并引发控制台错误。 适用于 CQ-4253277 的修补程序
...
* (卡视图)在视频资产上使用复制和粘贴时，不显示处理指示符。 适用于 CQ-4249037 的修补程序
...
* 从6.3升级到6.4及更高版本的Analytics预设后，将创建一个新的报表包和分析预设，该预设会丢失用户的旧版报告数据。 适用于 CQ-4244529 的修补程序
...
* 视频用户档案:“全选”按钮不起作用。 适用于 CQ-4253076、CQ-4251447 的修补程序
...
* (HTML Workspace)当用户声明任务时，将为该特定用户刷新队列计数，但不为其他用户刷新队列计数，除非刷新页面。 此问题已由新属性修复。 要将此新属性配置到AEM实例，请参阅其配置设置。 NPR-24536：适用于 CQ-4233665 的修补程序
* 无法在6.4上的AEM Forms应用程序中加载大型表单。NPR-24463:CQ-4245091的修补程序
* 在Mobile Workspace应用程序中尝试视图共享任务时出现问题。 NPR-25177：适用于 CQ-4248733 的修补程序
...
* IC同步与打印渠道到网络渠道同步相关的问题。 适用于 CQ-4251505 的修补程序
...
* 能够为资产文件夹设置元数据模式。 有关详细信息，请参 [阅文件夹元数据模式](/help/assets/folder-metadata-schema.md) ，并参阅AEM 6.4.2.0安装后的“配置设置 [](#configuration-settings-required-for-npr) ”。 NPR-21268：适用于 CQ-4221574 的修补程序
...
* 引入了增强的智能标签。
* 对“设计”选取器、“标记”选取器进行了修复(将源VM和目标VM更改为自动。)
...
* (详细信息视图)“下一步／后退导航”功能在“动态再现预览”区域中保留DIV叠加，从而阻止对查看器的访问。 NPR-23043：适用于 CQ-4238499 的修补程序
* CMYK图像再现的饱和度不正确。 NPR-23048：适用于 CQ-4235470 的修补程序
* Scene7ListInfoProvider的XMP元数据提取占用大量资源。 NPR-23754
* (dam-投放)HTTP转发程序不遵守HTTP代理设置。 NPR-24002：适用于 CQ-4244140 的修补程序

**站点**

* 在移动时重命名页面时，页面移动会成功，但重命名功能不起作用。 NPR-22923：适用于 CQ-4235907 的修补程序
* 在Adobe Campaign中发布指向导入程序页面的Live Copy页面时出错。 NPR-23053：适用于 CQ-4237164 的修补程序
...
* （经典UI）（触屏UI）在尝试通过资产搜索模式中的标记谓词浏览标记时，标记选取器不显示并引发异常。 NPR-23049：适用于 CQ-4239371 的修补程序
* （经典UI）使用xtype=tags的组件返回null，并且无法从标记的其他列表中选择。 NPR-23050：适用于 CQ-4239937 的修补程序
...
* （经典UI）编辑工作流启动器时，这些条件会消失，导致工作流在没有任何条件的情况下启动。 NPR-23835：适用于 CQ-4239153 的修补程序
...
* (MSM)事件监听的实际发生与事件处理之间的时间差。 适用于 CQ-4227766 的修补程序
...
* 链接共享电子邮件投放对于非“管理员”发件人已断开。 适用于 CQ-4240357 的修补程序
* 修复IT故障。 适用于 CQ-4249891 的修补程序

**DAM - Brand Portal**

* 资产属性列表第一个选项卡上的3个属性，其余所有选项卡都显示为空。 适用于 CQ-4242503 的修补程序
...
* (DMHybrid)视频的“详细信息”视图页面不包含视频资产显示的预览，并将错误消息输出到控制台。 适用于 CQ-4244320 的修补程序
...
* 拖放两个OOTB AVS视频处理用户档案列在“文件夹属性”下，并且只有一个包含定义的编码。 适用于 CQ-4238096 的修补程序
...
* 将颜色用户档案添加到投放服务器上的客户设置。 适用于 CQ-4241818、CQ-4241819 的修补程序
...
* AEM Forms引入了交 [易报告功能](https://helpx.adobe.com/experience-manager/6-4/forms/using/transaction-reports-overview.html) ，可跟踪和记录已提交的表单、已处理的文档和在AEM Forms部署中呈现的文档等交易。 它提供有关产品使用情况的洞察，并帮助商业用户了解数字处理量。
* 为XML表单启用了PDF/UA支持。
...
* (FDM WSDL)获取真理异常：属性类型数组中的列表数据除外。 NPR-24265
...
* 在“ `ACTION TAKEN` 争议类型”选项卡的部分下，“用于支持我的索赔的其他文档”中保存了“”的其他字段验证类型。 适用于 CQ-4239873 的修补程序
...
* FM创建按钮列表，按字母顺序排序。 适用于 CQ-4242307 的修补程序
...
* 在OSGI任务中的表单集成中，分配工作流的委派不起作用。 NPR-23709：适用于 CQ-4243700 的修补程序
* （工作流模型编辑器）某些工作流模型无法通过经典UI WF模型编辑器进行编辑。 适用于 CQ-4241151 的修补程序

**多渠道文档**

* 模板中的日期字段与IC创作中的子表单重叠。 适用于 CQ-4240110 的修补程序
* 在IC Web渠道创作中，不应允许删除标题或上下移动标题。 适用于 CQ-4243358 的修补程序
* （IC编辑器）表组件的默认行设置为1。 适用于 CQ-4244848 的修补程序
* 目标区域，即使在将内容拖放到该区域后，该区域也仍可见。 适用于 CQ-4244534 的修补程序
...
* 表标题在自动生成时显示为Web渠道中的变量。 适用于 CQ-4244242 的修补程序
...
* 在列表片段中，默认情况下选择分配。 适用于 CQ-4240097 的修补程序
...
* 管理操作用户档案当默认渲染进程配置设置为HTML时，准备数据进程将失败。 适用于 CQ-4244292 的修补程序
...
<p id="Dynamic">AEM 6.4.8.0包含新版Dynamic Media查看器(5.10.1)，该查看器支持在“图像预设”页面上检查重复名称。 建议Dynamic Media客户运行以下命令，将框查看器预设调出为最新状态。
...
默认情况下，对于其他用户，当用户声明任务时，共享队列的计数不会刷新。 为此，我们引入了一个新的属性。 请按照以下步骤在AEM实例上配置此属性：
