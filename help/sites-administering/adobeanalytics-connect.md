...
在继续之前，请确保您的凭据允许您使用以下任一方法登录到Adobe Analytics:
...
2. 滚动到 **Adobe Analytics** 单击 **[+]** 下一页 **可用配置**.
3. 单击 **[+]** 链接到Adobe Analytics配置旁边。
...
5. 在 **报表包** 单击侧面板的部分（主面板的右侧） **添加项目**. 然后，使用下拉列表选择报表包ID(例如， `geometrixxauth`)，框架将与其进行交互。
...
7. 要使框架在网站的发布实例上可用，请在 **页面** 选项卡，单击 **激活框架。**
...
请参阅 [查看页面分析数据](/help/sites-authoring/pa-using.md) 以了解更多详细信息。
