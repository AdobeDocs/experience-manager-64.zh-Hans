...
将网页与框架关联后，框架将对该页面和该页面的后代执行跟踪。 然后，可从Adobe Analytics检索页面视图并在站点控制台中显示。
...
>[!CAUTION]
>
>提 **供管理员** （在Adobe Analytics中）权限不足以允许用户从AEM连接到Adobe Analytics。 该帐户还必须具有 **Web服务访问权限** 。
...
2. 滚动到 **Adobe Analytics** ，然后单 **[击“可]** 用配置”旁 **边的+**。
3. 单击Adobe **[Analytics配置旁]** 的+链接。
...
5. 在侧面 **窗格(主面板右侧** )的“报表包”部分，单击“添 **加项目”**。 然后，使用下拉框选择框架将与之交互的报表包ID( `geometrixxauth`例如)。

   >[!NOTE]
   >
   >选择报表包ID时，左侧的内容查找器中会填充Adobe Analytics变量（SiteCatalyst变量）。

6. 然后，使 **用“运行模式** ”下拉框（在“报表包ID”旁边）选择要向“报表包”发送信息的服务器实例。

   ![aa-framework-01](assets/aa-framework-01.png)

7. 要使框架在站点的发布实例上可用，请在Sidekick的“页面”选 **项卡上** ，单击“激 **活框架”。**
...
有关更 [多详细信息，请参阅查看页面](/help/sites-authoring/pa-using.md) 分析数据。
...
* **轮询间隔**:
   ...
   服务从Adobe Analytics检索页面查看数据的时间间隔（以秒为单位）。
