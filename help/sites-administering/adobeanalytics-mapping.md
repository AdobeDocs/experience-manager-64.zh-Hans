---
title: 使用AdobeAnalytics属性映射组件数据
seo-title: 使用AdobeAnalytics属性映射组件数据
...
# 使用AdobeAnalytics属性映射组件数据{#mapping-component-data-with-adobe-analytics-properties}

将组件添加到框架中，以收集要发送到AdobeAnalytics的数据。 旨在收集分析数据的组件将数据存储在相应的CQ **变量中**。 当您将此类组件添加到框架时，框架会显示CQ变量的列表，以便您可以分别添加到相应的 **Analytics变量**。

![aa-11](assets/aa-11.png)

打开AEM **视图** 时，内容查找器中会显示Analytics变量。

![aa-12](assets/aa-12.png)

可以用相同的CQ变量映射多个Analytics **变量**。

![chlimage_1-155](assets/chlimage_1-155.png)

加载页面并满足以下条件时，将映射的数据发送到AdobeAnalytics:

* 页面与框架关联。
* 该页面使用添加到框架的组件。

请按照以下过程将CQ组件变量与AdobeAnalytics报表属性进行映射。
...
1. 要将AdobeAnalytics变量与组件中定义的变量进行映射，请将一个 **Analytics变量从内容查** 找器拖动到跟踪组件上的字段上。 例如，拖 `Page Name (pageName)` 动 `pagedata.title`到。

   ![aa-14](assets/aa-14.png)

   >[!NOTE]
   >
   >为框架选择的报表包ID(RSID)决定内容查找器中显示的AdobeAnalytics变量。
...
AEM使用一项约定来命名要映射到AdobeAnalytics产品相关属性的产品相关变量和事件:
...
>[!NOTE]
>
>请勿将AdobeAnalytics产品属性映射到CQ变量。 如表中所述配置与产品相关的映射与映射产品变量等效。

## 检查AdobeAnalytics的报告 {#checking-reports-on-adobe-analytics}

1. 使用提供给AEM的相同凭据登录AdobeAnalytics网站。
...
## 将内容查找器(cf#)与AdobeAnalytics框架结合使用 {#using-the-content-finder-cf-with-adobe-analytics-frameworks}

最初，打开AdobeAnalytics框架时，内容查找器包含预定义的Analytics变量：
...
1. 在AEM **视图中**，可以根据使用3个筛选器按钮选择的变量类型来筛选列表:
...
1. 在AdobeAnalytics视图下，内容查找器显示属于在CQ视图中拖动的跟踪组件的所有CQ变量。

   * 例如，如果**下载组件**是CQ视图中 *唯一拖动的* (具有两个可映射的变量 *eventdata.downloadLink和* eventdata.事件.startDownload **)，则切换到AdobeAnalytics视图时，内容查找器将如下所示：
   ![aa-22](assets/aa-22.png)

   * 这些变量可以拖放到属于3个变量部分(流量、转换和&#x200B;**事件**)之 **一的** 任 **何Adobe Analytics变**&#x200B;量上。
   * 在CQ视图中将新的跟踪组件拖到框架上时，属于该组件的CQ变量将自动添加到AdobeAnalytics视图的内容查找器(cf#)中。
   >[!NOTE]
   >
   >一次只能将一个CQ变量映射到AdobeAnalytics变量

## 使用AEM视图和Analytics视图 {#using-aem-view-and-analytics-view}

在任何给定时间，用户都可以选择在框架页面上查看AdobeAnalytics映射的两种方式之间切换。 2个视图从2个不同的角度提供了框架内映射的更好概述。
...
1. 这是框架打开时的默认视图。
1. 左侧： 内容查找器(cf#)由AdobeAnalytics变量根据所选RSID填充。
1. 选项卡标题(**AEM视图** 和 **Analytics视图**): 使用这些在两个视图之间切换。
...
1. 切换到框架上的“ **Analytics视图** ”选项卡即可访问此视图。
1. 左侧： 内容查找器(cf#)根据在CQ视图中拖动到框架上的组件由CQ变量填充。
1. 选项卡标题(**AEM视图** 和 **Analytics视图**): 使用这些在两个视图之间切换。

1. 三个表(流量、转换、事件)列表所有可用的AdobeAnalytics变量。 属于所选RSID。 此处显示的映射应与AEM视图中的映射相同：
...
>* `'`* `Adobe:'+pagedata.title+':'+pagedata.sitesection`\
   >  要发送与其 *sitesection连接* 的页面的标题，请 *使用**:* （冒号），并在Adobe的前 *缀* (作为 `prop3`


>[!CAUTION]
>
>在任何给定时间，只能将一个CQ变量映射到AdobeAnalytics变量。
