...
1. 在框架页面上，选择一个RSID并将用法设置为全部。 ([http://localhost:4502/cf#/etc/cloudservices/sitecatalyst/videoconf/videofw.html](http://localhost:4502/cf#/etc/cloudservices/sitecatalyst/videoconf/videofw.html))
...
>[!NOTE]
>
>您可以通过打开视频以在DAM中进行编辑，并将“标题 **** ”元数据字段设置为所需名称，来设置视频的**用户友好的**名称。
...
1. 要将CQ变量映射到Adobe Analytics属性，请将Adobe Analytics属性从ContentFinder拖动到组件上CQ变量旁。
   ...
   有关优化映射的信息，请参阅Adobe Analytics [中的测量视频](https://marketing.adobe.com/resources/help/en_US/sc/appmeasurement/hbvideo/setup-overview.html) 。
...
* 为 [与Adobe Analytics属性进行映射而定义的CQ变量的静态集](#milestones) 。
...
>[!NOTE]
>
>您可以通过打开视频以在DAM中进行编辑，并将**标题**元数据字段设置为所需名称，来设置视频的**用户友好的**名称。 您还需要保存完成后所做的更改。
