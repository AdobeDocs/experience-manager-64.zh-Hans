...
要将AEM内容配置为显示在门户中，请参阅 [在portlet中安装、配置和使用AEM](#installing-configuring-and-using-aem-in-a-portlet).
...
通过AEM内容portlet，您可以在门户中显示AEM内容。 Portlet在 `/crx-quickstart/opt/portal`、和的自定义方式。 例如，您可以通过部署您自己的身份验证服务来生成AEM覆盖默认行为所需的身份验证信息，从而自定义SSO/身份验证处理。 插件使用定义的API，通过该API，您可以根据API构建插件，以添加自己的功能。 该插件可以部署到正在运行的portlet中。 要正常运行，需要配置AEM创作和发布实例以及内容路径，才能在启动时显示。

某些配置可以通过portlet首选项进行更改，而其他配置则通过OSGi服务配置进行更改。 您可以使用 **配置** 文件或OSGi Web控制台。

### Portlet首选项 {#portlet-preferences}

可以在部署时在门户服务器中或通过编辑 **WEB-INF/portlet.xml** 文件。 默认情况下，portlet.xml文件如下所示：
...
<table>
 <tbody>
  <tr>
   <td>startPath</td>
   <td><p>这是portlet的开始路径：它定义最初显示的内容。</p> <p><strong>重要信息</strong>:如果portlet配置为连接到AEM创作和发布在与不同的上下文路径上运行的实例<strong> /</strong>，则需要启用强制 <strong>CQUrlInfo</strong> 在这些AEM实例的Html库管理器配置中（例如，通过Felix Webconsole）或编辑将不起作用，并且将不显示首选项对话框。</p> </td>
  </tr>
  <tr>
   <td>htmlSelector</td>
   <td>附加到每个URL的选择器。 默认情况下，此值为 <strong>port</strong>，因此所有html页面请求都使用以 <strong>.portlet.html。</strong> 这允许在AEM中使用自定义脚本来渲染portlet。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>默认情况下，Portlet在第一次调用时对整个门户页面执行Javascript重定向。 这是为了支持现代门户服务器的拖放方案。 在生产中，很少需要此重定向，因此可以在将此首选项设置为 <em>false</em>.</td>
  </tr>
 </tbody>
</table>
...
假定门户服务器在主机localhost上运行，端口8080，并且AEM portlet Web应用程序在Web应用程序上下文中装载 *cqportlet*，则web控制台的url为 `http://localhost:8080/cqportlet/cqbridge/system/console`. 默认的用户和密码为 **管理员**.

打开 **配置** 选项卡，选择 **门户目录CQ服务器配置**. 在此，您可以指定作者和发布实例的基本URL。 此过程在 [配置Portlet](#configuring-the-portlet).
...
如果设置了环境，则会在类路径* *中搜索配置文件&#x200B;**com/day/cq/portlet/{env}.config** where **env** 替换为环境的实际值。 此文件应列出此环境的所有配置文件。 这些文件会相对于配置文件的位置进行搜索。 例如，如果文件包含行 `my.service.xml,` 此文件从位于的类路径中读取 `com/day/cq/portlet/my.service.config.` 文件的名称由服务的持久性ID组成，后跟 **.config**. 在上一个示例中，持久性ID为 **my.service**. 配置文件的格式是Apache Sling OSGi安装程序使用的格式。
...
您可以在运行时部署此包，也可以将其添加到Portlet Web应用程序中 `WEB-INF/lib/resources/bundles` 部署之前。

部署缓存后，Portlet会缓存发布实例中的内容。 使用AEM的调度程序刷新，Portlet缓存可以失效。 要配置Portlet以使用其自己的缓存，请执行以下操作：

1. 在作者中配置以门户服务器为目标的复制代理。
1. 假定门户服务器在主机上运行 **localhost**、 **端口8080 **和AEM portlet web应用程序已装载到上下文中 **cqportlet**，则用于刷新缓存的url为 `http://localhost:8080/cqportlet/cqbridge/cqpcache?Path=$(path)`. 使用GET作为方法。

   **注意：** 您可以发送一个名为的http标头，而不是使用请求参数 **路径**.
...
1. 登录到创作实例。
1. 在网站选项卡中，单击 *工具* 选项卡。
1. 单击 **新页面……** 在复制代理中 **新建……** 菜单。

   ![screen_shot_2012-02-15at40647pm](assets/screen_shot_2012-02-15at40647pm.png)

1. 在 *模板*，选择 *复制代理*，然后输入代理的名称。 单击&#x200B;*创建*。
...
1. 单击 **编辑。**
1. 在 **设置** 选项卡，选择 **已启用** 选中框，选择**调度程序刷新**作为序列化类型，然后输入重试超时(例如，60000)。
...
1. 在 **HTTP头** 字段，单击 **+** 添加新条目和类型 **路径：{path}**.
1. 如有必要，请单击 **代理** 选项卡，并向代理输入代理信息。
1. 单击 **确定** 保存更改。
1. 要测试连接，请单击 **测试连接** 链接。 出现日志消息，指示复制测试是否成功。 例如：
...
您可以通过访问为复制代理配置的相同URL来手动刷新Portlet缓存。 请参阅 [刷新缓存](#flushing-the-cache-via-replication-agent) （对于URL的形式）。 此外，还需要使用URL参数Path=扩展URL&lt;path> 以指示要刷新的内容。

例如：

`https://10.0.20.99:10040/wps/PA_CQ5_Portlet/cqbridge/cqpcache?Path=*` 刷新完整缓存。 `https://10.0.20.99:10040/wps/PA_CQ5_Portlet/cqbridge/cqpcache?Path=/content/mypage/xyz` 刷新 `/content/mypage/xyz` 从缓存中。
...
在默认模式下，Portlet为AEM WCM创作实例发出的所有请求都使用相同的技术用户进行身份验证，而不考虑当前的门户用户。 默认情况下，“技术用户”模式处于启用状态。 在OSGi管理控制台的相应配置屏幕中启用/禁用此模式：

指定的技术用户必须存在于AEM WCM创作实例和发布实例(如果 **发布时验证** 启用。 确保为用户授予足够的访问权限，以便进行创作工作。

#### 单点登录 {#sso}

Portlet支持开箱即用的AEM SSO。 验证器服务可以配置为使用SSO并以格式传输当前门户用户 **基本** 作为名为的Cookie `cqpsso` 到AEM。 AEM应配置为对路径/使用SSO身份验证处理程序。 此处还需要配置Cookie名称。

的 `crx-quickstart/repository/repository.xml` 对于AEM存储库，需要相应地配置：
...
Portlet可以使用单点登录(SSO)方案来验证AEM WCM。 在此模式下，当前登录到门户的用户将以SSO Cookie的形式转发到AEM WCM。 如果使用SSO模式，则对AEM Portlet具有访问权限的所有门户用户都必须知晓基础AEM WCM实例，通常以AEM WCM连接到LDAP的形式，或通过事先手动创建用户的方式。 此外，在portlet中启用SSO之前，基础AEM WCM创作实例(以及发布实例，如果 **发布时验证** 启用)需要配置为接受基于单点登录(SSO)的请求。
...
1. 在XML文件中，找到 **LoginModule** 并将trust_credentials_attribute添加到其配置中：
...
1. 通过控制台的URI(https://)访问该控制台&lt;aem-host>:&lt;port>/system/console.
...
1. 单击 **保存** 启用单点登录。 SSO现在是主要的身份验证方案。
...
为了使基础AEM WCM实例接受SSO请求，必须切换Portlet的身份验证模式 **技术** to **单点登录**.
...
如果您没有使用AEM内容portlet的默认内联编辑功能，而是希望门户之外的创作和管理部分直接在AEM创作实例中使用，则应该启用PIN身份验证。 您还需要更改管理按钮的配置。
...
1. 在OSGi配置控制台中，默认位于http://localhost:4502/system/console/configMgr ，选择 **CQ PIN身份验证处理程序** 下拉菜单中。
1. 编辑 **URL根路径** 仅包含单个值的参数 **/**.
...
OSGi Web控制台列出了 **配置** 选项卡。 对于每个按钮，您可以定义此按钮在哪种模式下显示。 例如，您可以通过删除所有模式来禁用按钮。

默认情况下， AEM内容portlet使用内联编辑功能。 但是，如果您希望切换到AEM创作实例进行编辑，请启用 **SiteAdmin按钮** 和 **“内容查找器”按钮**，但禁用 **编辑按钮**. 在这种情况下，请确保在AEM中正确配置PIN身份验证。
...
HTML/CSS/图像位于/com/day/cq/portlet/toolbar/layout文件夹内，这一事实是Portlet强制要求的，不能更改。 同样，MANIFEST.MF中的Import-Package和Export-Package标头也必须称为/com/day/cq/portlet/toolbar/layout。 Bundle-SymbolicName必须是唯一且完全限定的包名称。
...
发布视图只有一个按钮，可将工具栏切换到管理视图。 发布视图由 [上一包](/help/sites-deploying/configuring-osgi.md). 在HTML中，您可以使用以下占位符，在呈现时，这些占位符将由portlet替换为相应的内容：
...
“管理”视图包含四个按钮：“编辑”、“网站”选项卡、“刷新”和“返回”。 管理视图由 [上一包](/help/sites-deploying/configuring-osgi.md). 在HTML中，您可以使用以下占位符，在呈现时，这些占位符将由portlet替换为相应的内容：
...
要安装自定义布局，请访问Portlet的OSGI Web控制台 **包** 部分并上传包。
...
所有链接都将被重写，以便在Portal上下文中工作。 默认情况下，会使用带有呈现参数的链接。 可以将门户DirectorHTML重写器配置为改用操作链接。

您还可以定义要查询的其他请求参数以查看要显示的内容路径。 例如，如果存在从外部到特定内容的链接，则此操作非常有用。

此外，可以为门户DirectorHTML重写器配置定义的排除链接重写的正则表达式列表。 例如，如果您具有与外部系统的相对链接，则应将其添加到此排除列表。
...
1. 门户Director区域设置处理程序处理当前请求的本地化。 它采用请求内容的路径，例如 `/content/geometrixx/en/company.html`根据配置，它会重写 **en** 以及用户的实际区域设置。

可以为门户Director区域设置处理程序配置用于检查区域设置信息的路径 — 通常，这包括 `/content` 和路径中区域设置信息的位置。 默认情况下，区域设置处理程序遵循在AEM中构建多语言站点的重新通用化。
...
<table>
 <tbody>
  ...
  <tr>
   <td>TextMapper</td>
   <td>允许您发布进程HTML、CSS和Javascript文件。</td>
  </tr>
  <tr>
   <td>工具栏按钮</td>
   <td>在工具栏中添加您自己的按钮。</td>
...
实施此类服务时，请确保将 **service.ranking** 属性更改为正值。 默认实施使用排名** 0**,Portlet使用排名最高的服务。
...
1. 使用包管理器 [上传和安装](/help/sites-administering/package-manager.md#uploading-packages-from-your-file-system) cq-portlet-components.zip包。
...
1. 在 `Design of par` 标题，单击 **编辑**.

1. 在 **常规** 组件类别中，选中Portlet组件旁边的复选框，然后单击“确定”。
...
1. 保存web.xml文件并重新打包WAR文件。
   ...
   **提示：** 的 `jar cvf nameofapp.war *` 命令会将当前目录的内容添加到nameofapp.war文件中。

1. 将Portlet应用程序部署到应用程序服务器。 有关信息，请参阅应用程序服务器相关文档。

### 将portlet添加到您的AEM页面 {#adding-portlets-to-your-aem-page}
...
默认情况下，Portlet会连接到localhost:4503处的发布实例，并连接到localhost:4502处的创作实例。 在部署Portlet期间，可以更改这些值。 门户控制器在/libs/portal/directory下的存储库中以内容形式提供。 您需要先下载应用程序战争文件，然后再使用它。
...
1. 单击“安装”并浏览到您下载的AEM portlet应用程序(cq-portlet-webapp.war)，然后输入有关portlet的其他重要信息。
   ...
   对于其他基本Portlet信息，您可以接受默认值或更改值。 如果您接受默认值，则此portlet可在https://上找到&lt;wps-host>:&lt;port>/wps/PA_CQ5_Portlet。 Portlet提供的OSGi管理控制台位于https://&lt;wps-host>:&lt;port>/wps/ PA_CQ5_Portlet/cqbridge/system/console（默认用户名/密码为admin/admin）。
...
1. 根据需要配置portlet。 例如，您可能需要更改创作和发布实例的URL以及开始路径的URL。 默认配置在 [Portlet首选项](/help/sites-administering/aem-as-portal.md#portlet-preferences).

   >[!NOTE]
   >
   >如果portlet配置为连接到AEM创作实例和发布实例，这些实例在与** /**不同的上下文路径上运行，则需要启用强制 **CQUrlInfo** 在这些AEM实例的Html库管理器配置中（例如，通过Felix Webconsole）或编辑将不起作用，并且将不显示首选项对话框。

1. 在应用程序服务器中保存配置更改。

1. 导航到Portlet的OSGI管理控制台。 默认位置为 `https://<wps-host>:<port>/wps/PA_CQ5_Portlet/cqbridge/system/console/configMgr`. 默认用户名/密码为 **管理员/管理员**.

1. 选择 **Day Portal Director CQ服务器配置** 配置和编辑以下值：
...
当从AEM请求内容时，Portlet使用当前显示模式（发布或创作）和当前路径来组合完整的URL。 如果使用默认值，则第一个url为 `http://localhost:4503/content/geometrixx/en.portlet.html`. 的值 `htmlSelector` 会在扩展之前自动添加到URL中。

如果Portlet切换到帮助模式，则 `appendHelpViewModeAsSelector` ，则 `help` 此外，还附加了选择器，例如， `http://localhost:4503/content/geometrixx/en.portlet.html.help`. 如果Portlet窗口已最大化，并且 `appendMaxWindowStateAsSelector` ，则也会附加选择器，例如， `http://localhost:4503/content/geometrixx/en.portlet.max.help`.
...
#### 将Portlet添加到Portal页 {#adding-the-portlet-to-the-portal-page}
