...
有关详细信息，请参 [阅下面的](/help/sites-administering/backup-and-restore.md#package-backup) “包备份”。
...
此备份方法可创建整个存储库的备份，包括其下部署的任何应用程序，如AEM。 备份包括内容、版本历史记录、配置、软件、修补程序、自定义应用程序、日志文件、搜索索引等。 如果您使用群集，并且共享文件夹是(物理上 `crx-quickstart` 或使用软链接)的子目录，则还会备份共享目录。
...
>[!NOTE]
>
>如果AEM联机备份功能用于具有自定义blobstore配置的AEM实例，建议将数据存储的路径配置为位于“ ”目录之外，并单独 `crx-quickstart`备份数据存储。

>[!CAUTION]
>
>联机备份只备份文件系统。 如果将存储库内容和／或存储库文件存储在数据库中，则需要单独备份该数据库。 如果您正在将AEM与MongoDB结合使用，请参阅有关如何使用MongoDB本 [机备份工具的文档](https://docs.mongodb.org/manual/tutorial/backup-with-mongodump/)。
...
>[!CAUTION]
>
>请勿与数据存储垃圾收集或修订清 [理同时运行AEM](/help/sites-administering/data-store-garbage-collection.md) Online [Backup](/help/sites-deploying/revision-cleanup.md#how-to-run-offline-revision-cleanup)。 它将对系统性能产生负面影响。

启动备份时，可以指定 **目标路** 径和／或 **延迟**。

**目标路** 径备份文件通常保存在快速启动jar文件(.jar)所在文件夹的父文件夹中。 例如，如果AEM jar文件位于/InstallationKits/AEM下，则备份将在/InstallationKits下生成。 您还可以指定目标到您选择的位置。

如果 **TargetPath** 是目录，则会在此目录中创建存储库的图像。 如果同一目录被多次（或始终）用于存储备份，
...
>[!NOTE]
>
>如 **果TargetPath** 设置为扩展名为。zip的文件名 ****，则存储库将备份到临时目录，然后压缩此临时目录的内容并存储在ZIP文件中。
>
>这种方法是不受欢迎的，因为
>
...
>[!NOTE]
>
>有关 [该过程的内部详细信息](#how-aem-online-backup-works) ，请参见AEM Online Backup的工作方式。
...
可以使用或HTTP客户端自 `wget` 动执 `curl` 行备份。 下面显示了如何使用curl自动备份的示例。

#### 备份到默认目标目录 {#backing-up-to-the-default-target-directory}

>[!CAUTION]
>
>在以下示例中，可能需要为 `curl` 您的实例配置命令中的各种参数； 例如，hostname() `localhost`、port()、 `4502`admin password() `xyz`和file name( `backup.zip`)。

```shell
curl -u admin:admin -X POST http://localhost:4502/system/console/jmx/com.adobe.granite:type=Repository/op/startBackup/java.lang.String?target=backup.zip
```

备份文件／目录是在包含该文件夹的文件夹的父文件夹中的服务器上 `crx-quickstart` 创建的（与使用浏览器创建备份时相同）。 例如，如果已在目录中安装AEM, `/InstallationKits/crx-quickstart/`则会在目录中创建备 `/InstallationKits` 份。
...
通常，备份文件／目录是在包含该文件夹的文件夹的父文件夹中在服务器上创 `crx-quickstart` 建的。

如果要将备份（按任一排序）保存到其他位置，可在命令中将绝对路径“” `target` 设置为参 `curl` 数。

例如，要在目 `backupJune.zip` 录中生成 `/Backups/2012`:
...
>[!NOTE]
>
>还可以使用AEM提供 [的MBean触发备份](/help/sites-administering/jmx-console.md)。
...
1. 创建zip文件时，第一步是创建或找到目标目录。
   ...
   * 如果备份到zip文件，将创建临时目录。 目录名称开始 `backup.` 以及结尾 `.temp`; 例如 `backup.f4d3.temp`,
   * 如果备份到目录，则使用在目标路径中指定的名称。 可以使用现有目录，否则将创建新目录。

      备份目标时， `backupInProgress.txt` 将在开始目录中创建名为空的文件。 备份完成后，将删除此文件。

1. 文件从源目录复制到目标目录（或创建zip文件时的临时目录）。 在数据存储之前复制段存储区，以避免存储库损坏。 创建备份时，将忽略索引和缓存数据。 因此，备份中不 `crx-quickstart/repository/cache` 包括 `crx-quickstart/repository/index` 来自和来自的数据。 创建zip文件时，进程进度栏指示符介于0% - 70%之间；如果未创建zip文件，则进度栏指示符介于0% - 100%之间。
...
1. 根据目标:
   ...
   * 如果目标是目录，则删除名为的空 `backupInProgress.txt` 文件，以指示备份已完成。
