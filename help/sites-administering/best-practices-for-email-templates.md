...
>[!NOTE]
>
>所有营销活动内容都应在类型 `master` 的页面下创建 `cq/personalization/components/ambitpage`。 例如，如果您计划的是营销活动结构，则类似于：
>
...
| **最佳实践** | **实施** |
|---|---|
| 使用W3C验证程序更正HTML代码。 确保正确关闭所有打开的标记。 | 代码已验证。 对于XHTML transitional Doctype，只缺少元素的缺少xmlns `<html>` 属性。 |
...
为了提供坚实的基础并扩大内容流的各种可能性，开箱即用的模板类型有三种略有不同。 您可以轻松使用这些工具构建自定义Newletter。
