...
>[!NOTE]
>
>所有活动内容都应在类型 `master` 的页面下创 `cq/personalization/components/ambitpage`建。 例如，如果您计划的是活动结构，则类似于：
>
...
| 不用再使用JavaScript或Flash-电子邮件客户端大多不支持这些技术。 | JavaScript和Flash都不在Newsletter模板中使用。 |
