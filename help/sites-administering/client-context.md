...
>[!NOTE]
>
>Client Context已被ContextHub取代。 有关详细信息，请参阅相关的[配置](/help/sites-administering/contexthub-config.md)和[开发人员](/help/sites-developing/contexthub.md)文档。
...
* **编**
辑此时将打开一个新页面， [允许您编辑、添加或删除用户档案属性](#editing-property-details)。
...
JSONP标准是对JSON的补充，它允许规避相同的来源策略（使Web应用程序无法与位于另一个域的服务器通信）。 它包括将JSON对象打包到函数调用中，以便能够从另一个域加载它(这是同一来源策略允许的例外)。
...
编辑Client Context可用于设置（或重置）某些属性的值。 这允许您测试特定场景(对[segmentation](/help/sites-administering/campaign-segmentation.md)和[活动](/help/sites-authoring/personalization.md)尤为有用)。
...
1. 打开AEM作者实例上Geometrixx Outdoors站点的英语主页。 ([http://localhost:4502/content/geometrixx-outdoors/en.html](http://localhost:4502/content/geometrixx-outdoors/en.html))。
...
1. 在Web浏览器中打开“工具”控制台([http://localhost:4502/miscadmin#/etc](http://localhost:4502/miscadmin#/etc))。
