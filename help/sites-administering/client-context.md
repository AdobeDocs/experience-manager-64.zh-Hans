...
* **编辑**&#x200B;此时将打开一个新页面，允许您 [编辑、添加或删除配置文件属性](#editing-property-details)。
...
JSONP标准是对JSON的补充，它允许规避相同的源策略（使Web应用程序无法与位于另一个域上的服务器通信）。 它包括将JSON对象打包在函数调用中，以便能够从另一个域加载它（这是同一源策略允许的例外）。
...
1. 双击表示当前用户的图标。 选择器将打开，使用箭头导航并查看可用的配置文件：
   ...
   ![clientcontext_profiles选择器](assets/clientcontext_profileselector.png)
...
编辑Client Context可用于设置（或重置）某些属性的值。 这允许您测试特定方案(对于分段和营销活动 [尤其](/help/sites-administering/campaign-segmentation.md)[有用](/help/sites-authoring/personalization.md))。
...
1. 打开AEM作者实例上Geometrixx Outdoors站点的英文主页。 ([http://localhost:4502/content/geometrixx-outdoors/en.html](http://localhost:4502/content/geometrixx-outdoors/en.html)).
