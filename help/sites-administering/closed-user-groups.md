...
`granite:loginPath`属性也是如此。 只有通过身份验证相关的混音类型定义它，才会尊重它。 在非结构化JCR节点上添加具有此名称的剩余属性将不显示所需效果，负责更新OSGi注册的处理程序将忽略该属性。
...
>[!NOTE]
>
>请注意`getEffectivePolicies`与随后的代码示例之间的差异，该示例将沿层次结构向上走，以查找给定路径是否已经是现有CUG的一部分。
...
`JackrabbitAccessControlManager`定义的允许按主体编辑访问控制策略的扩展在CUG访问控制管理中未实现，因为定义中，CUG策略始终影响所有主体：与`PrincipalSetPolicy`一起列出的内容将被授予读取访问权限，而所有其他主体将被阻止读取由目标节点定义的树中的内容。
...
在新的实施中，默认授权模型的访问控制设置不会受到创建、修改或删除的任何CUG的影响。 而是将名为`PrincipalSetPolicy`的新类型策略作为附加访问控制内容应用于目标节点。 此附加策略将作为目标节点的子项，并且如果存在，将作为默认策略节点的同级。
...
从旧的CUG支持转向其他授权模型，改变评估有效读取权限的方式。 如[Jackrabbit文档](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html)中所述，仅当Oak存储库中配置的所有型号的权限评估授予读取访问权限时，才授予允许视图`CUGcontent`的给定主体读取权限。
...
1. 输入要检查CUG的根路径，然后按&#x200B;**执行练习**&#x200B;按钮。 这将扫描所选位置中符合转换条件的CUG。
