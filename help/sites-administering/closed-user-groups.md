...
* 但是，它既不影响接入控制节点的同级，也不影响接入控制节点的祖先；
* 给定CUG的继承会在嵌套CUG处停止。
...
`granite:loginPath`属性也是如此。 只有在由身份验证相关的混音类型定义时，才会尊重它。 在非结构化JCR节点上添加具有此名称的剩余属性将不会显示所需效果，而负责更新OSGi注册的处理函数将忽略该属性。
...
在配置的受支持路径中添加`granite:AuthenticationRequired`混音类型将导致更新负责处理函数的OSGi注册，该注册包含具有`sling.auth.requirements`属性的新的附加条目。 如果给定的身份验证要求指定了可选的`granite:loginPath`属性，则该值将附加地注册到身份验证器中，具有“ — ”前缀，以便从身份验证要求中排除。
...
* 建模存储库内容，使身份验证要求适用于整个树，无需再次将嵌套子树排除在需求之外。
* 要避免指定，并随后注册多余的登录路径：
...
>[!NOTE]
>
>请注意`getEffectivePolicies`与后续代码示例之间的差异，该示例将向上遍历层次结构以查找给定路径是否已是现有CUG的一部分。
...
`JackrabbitAccessControlManager`定义的允许按主体编辑访问控制策略的扩展未通过CUG访问控制管理实现，因为根据定义，CUG策略始终会影响所有主体：与`PrincipalSetPolicy`一起列出的内容将被授予读访问权限，而所有其他主体将被阻止读取由目标节点定义的树中的内容。
...
>[!NOTE]
>
>如果`RequirementHandler`已配置，且目标包含在由支持的路径定义的树中，则上述对给定目标节点的修改仅会反映在Apache Sling Authenticator上（请参阅配置选项一节）。
>
...
>[!NOTE]
>
>但请注意，删除配置不会取消注册mixin类型，该类型仍适用于节点，但不会生效。
...
### CUG设置和配置{#diferences-in-cug-setup-and-configuration}中的差异
...
在新的实施中，默认授权模型的访问控制设置不会受到任何正在创建、修改或删除的CUG的影响。 而是将名为`PrincipalSetPolicy`的新策略类型作为附加访问控制内容应用到目标节点。 此附加策略将作为目标节点的子级，并且如果存在，将作为默认策略节点的同级。
...
请注意，将CUG策略仅放置在给定页面下方的jcr:content节点上将仅限访问给定页面的内容，但不会对任何同级或子页面生效。 这可能是一个有效的用例，并且可以通过允许应用细粒度访问内容的存储库编辑器来实现。 但是，它与前一个实现形成了对比，在前一个实现中，将cq:cugEnabled属性放在jcr:content节点上是在内部重新映射到页面节点的。 不再执行此映射。
...
在新的AEM默认设置CUG策略评估中，它仅通过“发布”运行模式启用。 有关详细信息，请参阅自AEM 6.3](#default-configuration-since-aem)以来的[默认配置的详细信息。 这可以通过将给定路径的有效策略与存储在内容中的策略进行比较来验证。 只有在启用CUG的权限评估时，才会显示有效策略。
...
1. 输入要检查CUG的根路径，然后按&#x200B;**执行练习**&#x200B;按钮。 这将扫描选定位置中符合转换条件的CUG。
