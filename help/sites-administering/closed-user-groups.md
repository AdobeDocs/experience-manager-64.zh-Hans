...
这同样适用于该属 `granite:loginPath` 性。 只有在由身份验证相关混音类型定义时，才会遵守该规则。 在非结构化JCR节点上添加具有此名称的剩余属性将不显示所需效果，并且负责更新OSGi注册的处理函数将忽略该属性。
...
在配置 `granite:AuthenticationRequired` 的受支持路径中添加混音类型将导致更新负责处理函数的OSGi注册，该注册包含带有该属性的新附加条 `sling.auth.requirements` 目。 如果给定的身份验证要求指定了可选属性，则该值附加地向身份验证器注册以“-”前缀，以便从身份验证要求中排除。 `granite:loginPath`
...
* 模型库内容，使得身份验证要求适用于整个树，无需再次将嵌套子树排除在需求之外。
* 要避免指定，并随后注册冗余登录路径：
...
编辑现有CUG策略需要执行以下步骤。 请注意，修改后的策略需要回写，并且需要使用保留更改 `javax.jcr.Session.save()`。
...
>[!NOTE]
>
>请注意与随后的代 `getEffectivePolicies` 码示例之间的区别，该示例在层次结构中向上遍历以查找给定路径是否已包含在现有CUG中。
...
由允许按主体编 `JackrabbitAccessControlManager` 辑访问控制策略定义的扩展不会通过CUG访问控制管理实现，因为根据定义，CUG策略始终会影响所有主体：将授予与列出 `PrincipalSetPolicy` 的内容一起读取的访问权限，而将阻止所有其他承担者读取目标节点定义的树中的内容。
...
创建新身份验证要求的步骤，包括登录路径。 请注意，只有为包含目标节点的树配置了Apache Sling Authenticator，才会向Apache Sling Authenticator注册登录路径的要求和 `RequirementHandler` 排除项。
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    ...
    </ul> <p>另请参阅下 <a href="/help/sites-administering/closed-user-groups.md#configuration-options" target="_blank">面的配置选</a> 项。</p> </td> 
  </tr> 
  <tr> 
   <td>配置策略</td> 
...
>[!NOTE]
>
>但是，请注意，删除配置不会取消注册mixin类型，该类型在未生效的情况下仍适用于节点。
...
通过添加一个额外节点和一个额外属性，与LiveCopy相结合配置CUG，如下所示：
...
授权方面的主要差异在以下列表中进行了总结：

**CUG的专用访问控制内容**

在旧的实现中，默认授权模型用于控制发布时的访问控制列表策略，由CUG授权的设置替换任何现有ACE。 这是由编写常规的、剩余的JCR属性触发的，这些属性在发布时进行了解释。

在新的实施中，默认授权模型的访问控制设置不受创建、修改或删除的任何CUG的影响。 而是会将名为的新类型的策 `PrincipalSetPolicy` 略作为附加访问控制内容应用到目标节点。 此附加策略将作为目标节点的子级进行定位，并且如果存在，将作为默认策略节点的同级。
...
请注意，将CUG策略仅放在位于给定页面下方的jcr:content节点上将仅限制对给定页面内容的访问，但不会对任何同级或子页面生效。 这可能是一个有效的用例，并且可以使用允许应用精细访问内容的存储库编辑器来实现。 但是，它与前一个实现形成了对比，在前一个实现中，将cq:cugEnabled属性放在jcr:content节点上是在内部重新映射到页面节点的。 不再执行此映射。
...
1. 输入要检查CUG的根路径，然后按“ **Perform dry run** ”按钮。 这将扫描选定位置中符合转换条件的CUG。
