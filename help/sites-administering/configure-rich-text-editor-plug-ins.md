...
有关其他RTE配置的详细信息，请参 [阅配置富文本编辑器](/help/sites-administering/rich-text-editor.md)。
...
>[!NOTE]
>
>为避免在本文中出现重复，各&#x200B;*自的rtePlugins节点称*&#x200B;为&lt;rtePlugins-node>。
...
>[!NOTE]
还可以为表或表单元 [格定义样式](/help/sites-administering/configure-rich-text-editor-plug-ins.md#tablestyles)。 这些配置需要单独的过程。
...
>[!CAUTION]
配置RTE的段落格式时，请勿删除段落标记&lt;p>作为格式选项。 如果删除了&lt;p>标记，则内容作者无法选择“段落格 **式”选项** ，即使配置了其他格式也是如此。
...
1. 执行下列操作之一以查找您选择的语言的词典：
   ...
   * 在https://wiki.openoffice.org/wiki/User:Khirano/Dictionaries上搜索v2.x词典 [文件](https://wiki.openoffice.org/wiki/User:Khirano/Dictionaries)。
...
>[!NOTE]
RTE拼写检查器可按需使用。 它不会在您开始键入文本时自动运行。 要运行拼写检查器，请单击工 [!UICONTROL 具栏中] 的“拼写检查器”。 RTE检查单词的拼写并突出显示拼写错误的单词。
