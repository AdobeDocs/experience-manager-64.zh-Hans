...
1. 在您的组件中，导航到`<rtePlugins-node>/edit`节点。 如果节点不存在，则创建这些节点。 有关详细信息，请参阅[激活插件](#activateplugin)。
1. 在`edit`节点中，使用以下详细信息创建属性：

   * **名称** `defaultPasteMode`
   * **类型** `String`
   * **值** 所需的粘贴模式 `browser`之 `plaintext`一、或 `wordhtml`。

### 配置粘贴内容{#pasteformats}时允许的格式

可以进一步配置粘贴为Microsoft-Word(`paste-wordhtml`)模式，以便您能够明确定义从其他项目（如Microsoft Word）在AEM中粘贴时允许使用哪些样式。
...
1. 在您的组件中，导航到节点`<rtePlugins-node>/edit`。 如果节点不存在，则创建这些节点。 有关详细信息，请参阅[激活插件](#activateplugin)。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>定义粘贴表时的行为。<br /> </p> <p>此节点必须具有属性<code>allow</code>（类型<code>Boolean</code>），以定义是否允许粘贴表。</p> <p>如果<code>allow</code>设置为<code>false</code>，则必须指定属性<code>ignoreMode</code>（类型<code> String</code>）以定义如何处理粘贴的表内容。 <code>ignoreMode</code>的有效值为：</p> 
    <ul> 
  ...
  <tr> 
   <td>列表</td> 
   <td>nt：非结构化</td> 
   <td><p>定义粘贴列表时的行为。<br /> </p> <p>必须具有属性<code>allow</code>（类型<code>Boolean</code>）来定义是否允许粘贴列表。</p> <p>如果<code>allow</code>设置为<code>false</code>，则必须指定属性<code>ignoreMode</code>（类型<code>String</code>）以定义如何处理粘贴的任何列表内容。 <code>ignoreMode</code>的有效值为：</p> 
    <ul> 
...
1. 在您的组件中，导航到节点`<rtePlugins-node>/styles`。 如果节点不存在，则创建这些节点。 有关详细信息，请参阅[激活插件](#activateplugin)。
...
1. 在节点`styles`下，创建一个新节点（也称为`styles`），以保留列表可用：
...
1. 在您的组件中，导航到节点`<rtePlugins-node>/paraformat`。 如果节点不存在，则创建这些节点。 有关详细信息，请参阅[激活插件](#activateplugin)。
...
>[!CAUTION]
配置RTE的段落格式时，请勿删除段落标记&lt;p>作为格式选项。 如果删除了&lt;p>标记，则内容作者无法选择&#x200B;**段落格式**&#x200B;选项，即使配置了其他格式也是如此。
...
在标准AEM安装中，当为特殊字符(`specialchars`)启用`misctools`插件时，将立即提供默认选择供使用；例如，版权和商标符号。
...
1. 在您的组件中，导航到节点`<rtePlugins-node>/misctools`。 如果节点不存在，则创建这些节点。 有关详细信息，请参阅[激活插件](#activateplugin)。
...
1. 保存更改。
   ...
   ![在CRXDE中，定义要在RTE中可用的字符范围](assets/chlimage_1-413.png)

         *在CRXDE中，定义要在RTE*&#x200B;中使用的字符范围

   ![RTE中的特殊字符会在弹出窗口中显示给作者](assets/rtepencil.png)

         RTE中的&#x200B;*特殊字符在弹出窗口中向作者显示*
...
1. 在组件中，导航到节点`<rtePlugins-node>/table`。 如果节点不存在，则创建这些节点。 有关详细信息，请参阅[激活插件](#activateplugin)。
...
>[!NOTE]
如果检查未安装的语言时，会看到消息`Spell checking failed`。 标准词典与相应的自述文件一起位于`/libs/cq/spellchecker/dictionaries`。 请勿修改文件。

标准AEM安装包括美国英语(`en_us`)和英国英语(`en_gb`)的词典。 要添加更多字典，请按照以下步骤操作。
...
>[!NOTE]
RTE拼写检查器可按需使用。 它不会在您开始键入文本时自动运行。 要运行拼写检查器，请单击工具栏中的[!UICONTROL 拼写检查器]。 RTE检查单词的拼写并突出显示拼写错误的单词。
...
1. 在您的组件中，导航到节点`<rtePlugins-node>/keys`。 如果节点不存在，则创建这些节点。 有关详细信息，请参阅[激活插件](#activateplugin)。
1. 在`keys`节点上创建属性：
