...
[响应](/help/sites-authoring/responsive-layout.md) 式布局是实现响应式 [Web设计的机制](https://en.wikipedia.org/wiki/Responsive_web_design)。这允许用户创建具有根据用户使用的设备而定的布局和尺寸的网页。

>[!NOTE]
>
>这可以与使用自适应Web设计（主要针对经典UI）的[移动Web](/help/sites-developing/mobile-web.md)机制进行比较。
...
>[!CAUTION]
>
>尽管&#x200B;**布局容器**&#x200B;组件在经典UI中可用，但其完整功能仅在触屏优化UI中可用。
...
>[!NOTE]
>
>在开箱即用的安装中，已为[We.Retail参考站点](/help/sites-developing/we-retail.md)配置响应式布局。 您仍必须[激活其他页面的布局容器组件](#enable-the-layout-container-component-for-page)。
...
要使模拟器能够支持您的页面，您必须注册您的页面组件。 请参阅[注册页面组件以模拟](/help/sites-developing/responsive.md#registering-page-components-for-simulation)。
...
要包含模拟器，您需要将站点链接到设备组。 请参阅[添加设备列表符](/help/sites-developing/responsive.md#adding-the-devices-list)（适用于经典和触屏优化UI）。

## 激活站点{#activate-layout-mode-for-your-site}的布局模式

这些过程用于在您的站点上启用&#x200B;**布局**&#x200B;模式。
...
* 继承自父节点层次结构，可以随意覆盖。
* 有一个默认（现成）断点，它涵盖上一个&#x200B;*已配置*&#x200B;断点之上的所有断点。
...
断点位于`.context.html`的`<jcr:content>`部分，位于相应的模板（或内容）文件夹下。
...
## 为页面{#enable-component-resizing-for-the-page}启用组件大小调整
...
AEM使用LESS生成必要的CSS部分，这些部分需要包含在您的项目中。
...
要正确调整响应式网格中包含的自适应图像内容的大小和更新其内容，您需要将设置为`REFRESH_PAGE`的监听器的`afterEdit`添加到每个包含的组件的`EditConfig`文件中。
...
* **组件定义**
   ...
   在定义组件时使用`allowedComponent`或静态包含。
