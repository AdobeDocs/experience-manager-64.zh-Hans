...
* **JCR节点：** 要显示UI，请将布尔 `com.adobe.granite.contexthub.show_ui` 属性设 `true`置。 要隐藏UI，请将属性设置为 `false`。
...
Javascript [类的](/help/sites-developing/contexthub-api.md#init-name-config) init函数定义一个对 `ContextHub.Store.JSONPStore``config` 象，该对象初始化此存储候选项。 该 `config` 对象包含 `service` 一个包含JSONP服务详细信息的对象。 要配置商店，您需要 `service` 将JSON格式的对象作为“详细配置”属性的值。

要保存jsontest.com站点的MD5服务中的数据，请使用以下属 [性创建ContextHub](/help/sites-administering/contexthub-config.md#creating-a-contexthub-store) Store中的步骤：
...
使用添加UI [模块中的步骤](/help/sites-administering/contexthub-config.md#adding-a-ui-module) ，将UI模块添加到现有UI模式，如示例Perona UI模式。 对于UI模块，请使用以下属性值：
