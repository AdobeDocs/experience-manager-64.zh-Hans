...
* 使用CRXDE Lite设置属性 `disabled` to **true** 在 `/libs/settings/cloudsettings`
...
要保存jsontest.com站点的MD5服务中的数据，请使用 [创建ContextHub存储](/help/sites-administering/contexthub-config.md#creating-a-contexthub-store) 使用以下属性：
...
在 [添加UI模块](/help/sites-administering/contexthub-config.md#adding-a-ui-module) 将UI模块添加到现有UI模式，例如示例的Perona UI模式。 对于UI模块，请使用以下属性值：
