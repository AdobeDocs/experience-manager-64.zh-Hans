...
由于构建AEM的Sling框架具有RESTful特性，因此大多数任务可以简化为URL调用，可通过cURL执行。 [使用cURL](/help/sites-administering/curl.md#common-content-manipulation-aem-curl-commands) 可以自动执行内容处理任务(如激活页面、启动工作流 [)](/help/sites-administering/curl.md#common-operational-aem-curl-commands) 以及包管理和管理用户等操作任务。此外，您还可以为AEM中的大多数任务创建您自己的cURL](/help/sites-administering/curl.md#building-a-curl-ready-aem-command)命令。[
...
cURL是macOS和某些Linux版本的标准部分。 但是，它适用于大多数操作系统。 可在[https://curl.haxx.se/download.html](https://curl.haxx.se/download.html)找到最新下载内容。
...
1. 将cURL命令复制到文本编辑器，并从命令中删除所有标头，该命令与`-H`开始（下图中以蓝色突出显示），然后添加正确的身份验证参数，如`-u <user>:<password>`。
...
#### 安装{#install-a-package}包

```shell
curl -u <user>:<password> -F cmd=install http://localhost:4502/crx/packmgr/service/.json/etc/packages/my_packages/test.zip
```

#### 卸载{#uninstall-a-package}包

```shell
curl -u <user>:<password> -F cmd=uninstall http://localhost:4502/crx/packmgr/service/.json/etc/packages/my_packages/test.zip
```

#### 删除{#delete-a-package}包
...
#### 设置用户的用户组成员{#set-a-user-s-group-membership}
...
有关详细信息，请参阅[Scoring and Badges Essentials](/help/communities/configure-scoring.md#example-setup)。
...
有关详细信息，请参阅[以编程方式](/help/sites-developing/workflows-program-interaction.md)与工作流交互。
...
#### 使用Sling PostServlet {#upload-files-using-sling-postservlet}上载文件
