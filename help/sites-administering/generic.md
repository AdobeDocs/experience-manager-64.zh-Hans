...
AEM通用解决方案提供了管理存储库中保存的商务信息的方法（与使用外部电子商务引擎相比）。 这包括：
...
* [代理页面](/help/sites-administering/concepts.md#proxy-pages)
...
在创建产品之前，您需要定义一个 [scaffold](/help/sites-authoring/scaffolding.md)。 这指定了定义产品以及编辑产品时所需的字段。

每种不同的产品类型都需要一个scaffold。 相应的scaffold通过以下任一方式与产品关联：
...
>[!NOTE]
>
>Geometrixx-Outdoors商店具有单一产品类型（因此是单个scaffold）:
>
...
1. 指定：
   ...
   * **导入程序**
      ...
      默认情况下，特定商 [务提供者的导](/help/sites-administering/concepts.md#commerce-providers)入程序 `Geometrixx`。
   ...
   * **增量导入**
   ...
   >[!NOTE]
   >
   >增量导入（属于示例geometrixx-outdoor导入程序）在产品级别操作。
   >
   >可以根据需要定义自定义的导入程序以运行。

1. 选择 **下一步** ，导入产品后，将显示所执行操作的日志。
   ...
   >[!NOTE]
   >
   >重复使 **用** Next **和Back** 将重复导入产品定义。 但是，由于它们具有相同的SKU，因此存储库中的现有信息将被覆盖。

1. 选择 **完成** ，关闭向导。

#### 导入产品——经典UI {#importing-products-classic-ui}

1. 使用工 **具控制台** ，打开“商 **务** ”文件夹。
1. 双击以打开产品导 **入程序**:
...
1. 指定：
   ...
   * **商业提供程序**
      ...
      您的商务提供商 [的导入程序](/help/sites-administering/concepts.md#commerce-providers);默认情况下为Geometrixx。
...
>[!NOTE]
>
>标准产品管理是基本的，因为Geometrixx-Outdoors产品集一直保持基本。 复杂性基于产品基架 [](/help/sites-authoring/scaffolding.md)，因此使用您自己的产品基架可以实现更复杂的编辑。

#### 创建产品信息——触屏优化UI {#creating-product-information-touch-optimized-ui}

1. 使用产 **品控制台** (通过 **商务**)导航到所需的位置。
1. 使用创 **建图标** ，可选择以下任一选项（具体取决于结构和位置）:

   * **创建产品**
   * **创建产品变量**
   ![](do-not-localize/chlimage_1-14.png)

1. 此时将打开向导。 使用“ **基本** ”( **Basic** )和“产品”( [Product)选项卡输](/help/sites-administering/concepts.md#product-attributes) 入新产品或产品变体的产品属性。

   >[!NOTE]
   >
   >**标题** 和 **** SKU是创建产品或变体所需的最低要求。

1. 选择 **创建** ，以保存信息。

>[!NOTE]
>
>许多产品提供各种颜色和／或尺寸。 有关基本产品和相关产品变体的信息都可以从“产品”控制台进 **行管理** 。
>
>产品及其变体以树结构的形式存储，产品信息位于顶部，变体位于下面（此结构由UI强制实施）。

### 编辑产品信息 {#editing-product-information}

>[!NOTE]
>
>Geometrixx-outdoors中的产品图像由以下位置提供：
>
>`/etc/commerce/products/...`
>
>这意味着，默认情况下，调度程序会阻止 [它们](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html)，因此请根据需要进行配置。

#### 编辑产品信息——触屏优化UI {#editing-product-information-touch-optimized-ui}

1. 使用产 **品控制台** (通过商 **务**)导航到您的产品信息。
1. 使用以下任何一种方式：
...
1. 将 [显示产品](/help/sites-administering/concepts.md#product-attributes) 属性。 使用 **编辑****和完** 成进行任何更改。
...
1. 单击／点按产品可打开它。 您还可以发布产品或查看产品数据。

#### 扩展搜索 {#extending-search}

您可以使用CRXDE Lite修改现有facet或添加新facet:
...
两个资产现在存储在您的产品组件中。 您可以配置将在产品页面上显示哪个页面。 这适用于类别系统。 首先，您需要向单个资产添加类别：

1. 点按／单击 **查看产品数据**。
1. 在资产 **下键入资产类别** ，例如 `cat1` 和 `cat2`。
...
>[!NOTE]
>
>如果未设置类别，则第一个资产将显示在产品页面上。

选择要显示的图像的机制如下：

1. 验证是否已为产品页面设置类别。
1. 如果未设置，请验证是否为目录设置了类别。
...
1. 导航到您的产品页面。
1. **编辑** “产品”组件。
1. 键入您 **选择的图像类别** ( `cat1` 例如)。
1. 点按／单击 **完成**。 页面将刷新，此时应显示正确的资产。
...
您可以在Web控制台中的OSGI事件()中查 `/system/console/events`看它们，例如：
...
通过添加到购物车链接的图像组件，您可以通过在图像上创建与产品链接的热点来快速将产品添加到购物车。

单击热点会打开一个对话框，通过该对话框可以选择产品的大小和数量。

1. 导航到要添加组件的页面。
1. 将组件拖放到页面中。
1. 从资产浏览器在组件中拖放一 [个图像](/help/sites-authoring/author-environment-tools.md#assets-browser)。
1. 您可以：
...
1. 根据需要修改和移动形状。
1. 单击形状。
1. 单击浏览图标可打开资产 [选择器](/help/assets/asset-selector.md#using-the-asset-selector)。

   >[!NOTE]
   >
   >或者，您也可以直接键入必须在产品级别而非变体级别的产品路径。

   ![chlimage_1-332](assets/chlimage_1-332.png)

1. 单击确认图标两次，然后单击退出全屏。
1. 单击组件旁边的页面上的某处。 页面应刷新，并且您应在图像上看到以下符号：

   ![](do-not-localize/chlimage_1-22.png)

1. Switch to [preview](/help/sites-authoring/editing-content.md#previewing-pages) mode.
1. 单击+热点。 此时会打开一个对话框，您可以在其中选择您在路径中输入的产品的大小和 **数量**。

   ![chlimage_1-333](assets/chlimage_1-333.png)

1. 输入大小和数量。
1. 单击“添加到购物车”按钮。 对话框将关闭。
1. 导航到购物车。 产品应该在此处。
...
1. 点按／单 **击** “完成”按钮将返回到“站点”控制台，您将能够在该控制台中看到您的目录。

   点按／单 **击“打开目录** ”按钮可打开您的目录(例如 `http://localhost:4502/editor.html/content/test-catalog.html`)。

#### 生成目录——经典UI {#generating-a-catalog-classic-ui}

>[!NOTE]
>
>目录将引用您的 [产品数据](#editing-product-information)。

1. 使用“网 **站** ”控制台，导航到您的“目 **录蓝图**”，然后导航到“基本目录”。

   例如：

   `http://localhost:4502/siteadmin#/content/catalogs/geometrixx-outdoors/base-catalog`

1. 使用章节Blueprint模板创 **建新页面** 。

   For example, `Swimwear`.

1. 打开新页 `Swimwear` 面，然后单 **击编辑Blueprint** ，打开“属性” **对话框** ，在该对话框中可以设置“产品 **** ”选择。

   例如，打开“标 **记／关键字** ”字段以选择“活动”，然后从Geometrixx-Outdoors部分中选择“游泳”。

1. 单击 **确定** ，以保存您的属性；示例产品将显示在Blueprint页 **面的“产品选择标准** ”下。
1. 单击转 **出更改……**，选择转出页 **面和所有子页面**，然后单击下一步 **,******&#x200B;转出转出。 成功完成转出后，状 **态指示符** 将显示为绿色。
1. 您现在可以单击“ **关闭** ”并检查新目录部分；例如，on和under:

   `http://localhost:4502/cf#/content/geometrixx-outdoors/en/swimwear.html`

1. 再次从Blueprint页面单击“编 **辑Blueprint** ”，在“属性”对话 **框中打开“** 生成的页面 **** ”选项卡。 在横幅列表字段中，选择要显示的图像；例如， `summer.jpg`
1. 单击 **确定** ，以保存您的属性；横幅信息将显示在Blueprint页面 **的“产品选择条件** ”下。
1. 转出这些新更改。
...
1. 在向导中，根据需要设置转出，然后点按／单击转出 **更改**。
1. 此时将打开一个对话框。 在进程完成时点按／单击**完成**。
...
1. 通过“商 **务** ”导航到目 **录控制台**。
1. 导览至要导入目录Blueprint的位置。
1. 点按／单击“导入 **Blueprints** ”图标。

   ![](do-not-localize/chlimage_1-25.png)

1. 在向导中，根据需要选择源，然后点按／单击下 **一步**。

   ![chlimage_1-340](assets/chlimage_1-340.png)

1. 完成导入 **后点按** /单击完成。

#### Blueprint Importer —— 经典UI {#blueprint-importer-classic-ui}
...
>[!NOTE]
>
>以下示例处理直接在营销活动中持有的 [促销](/help/sites-authoring/personalization.md)，它用于凭证。
>
>促销活动也可以包含在 [营销](/help/sites-authoring/personalization.md) 活动中的体验中。
>
>有关详细信息，请参 [阅Promotions and Vouchers](#promotions)。

1. 打开创 **作实例的** “网站”控制台。
1. 在左侧窗格中，选择所需的营销 **活动**。
1. 单击“ **New**”，选择“ **Promotion** ”模板，然后为新凭证指定标题 **(如果需要，****** 则为NameNameName)。
...
1. 打开创 **作实例的** “网站”控制台。
1. 在左侧窗格中，选择所需的营销 **活动**。
1. 单击“ **New**”，选择“ **Voucher** ”模板，然后为新凭证指定标题 **(如果需要，****** 则为NameNameNameFild)。
1. 单击&#x200B;**创建**。右侧窗格中将显示新的凭证页面。

1. 通过双击打开新的凭证页面，然后单击“ **Edit** ”以根据需要配置信息。
1. 单击&#x200B;**确定**&#x200B;进行保存。

1. 您现在可以激活您的优惠券，以便购物者可以在发布实例上的购物车中使用它。
...
* 取消激活凭证——它将在创作环境中保持可用，因此您可以在以后重新激活它。
* 完全删除它。
...
要更改优惠券或促销的属性，可在“网站”控制台上双击该优惠券或 **促销** ，然后单击“编 **辑”**。 保存后，您应激活它，以便将更改推送到发布实例。

### 将优惠券添加到购物车 {#adding-vouchers-to-a-cart}

要允许用户向购物车添加凭证，您可以使用内置的 **Vouchers** 组件（商务类别）。 您需要将其添加到显示购物车的页面（但不是必需的）。 The vouchers component is a form which the user can enter a voucher code, it is the shopping cart component that actially shows the list of applied vouchers and their discount.

在演示站点(Geometrixx Outdoors - English)中，您可以在购物车页面上的实际购物车下看到凭证表单。

## 订单 {#orders}

>[!NOTE]
>
>应记住，开箱即用的AEM没有与订单相关的标准功能（如退回商品、更新订单状态、执行履行、生成包装单）所需的操作。 它主要用于技术预览。
>
>AEM中的通用订单管理一直是基本的；向导中可用的字段取决于scaffold:\
>`/etc/scaffolding/geometrixx-outdoors/order/jcr:content/cq:dialog`
>
>如果创建自定义的基架，则可以存储更多订单信息。
...
1. 使用“订 **单** ”控制台导航到所需的位置。
1. 使用创 **建图** 标选择创 **建顺序**。

   ![](do-not-localize/chlimage_1-26.png)

1. 此时将打开向导。 使用“基 **本**”、“ **内容**”、“付款 **”和“******[](/help/sites-administering/concepts.md#order-information)履行”标签输入关于新订单的履行信息。
...
1. 将 [显示订单](/help/sites-administering/concepts.md#order-information) 。 使用 **编辑****和完** 成进行任何更改。
