...
>[!NOTE]
>
>AEM-LF包的某些功能取决于社交组件框架(SCF)。 如果您将Livefyre功能包用作非社区站点的一部分，则必须声明 *cq.social.scf* 作为网站作者clientlibs中的依赖项。 如果您将LF功能包用作社区网站的一部分，则应该已声明此依赖项。

1. 在AEM主页上，单击 **工具** 图标。
1. 导航到 **部署>包**.
1. 在包管理器中，滚动直到您看到预安装的Livefyre功能包，然后单击包标题 **cq-social-livefyre-pkg-1.2.6.zip** 以展开选项。
1. 单击 **更多>卸载**.
...
1. 从包管理器中，安装下载的包。 有关在AEM中使用Software Distribution和包的说明，请参阅 [如何使用包](/help/sites-administering/package-manager.md).
...
1. 命名您的配置并检查 **云配置** 复选框。
   ...
   这将在下创建文件夹 **工具>部署> Livefyre配置** ，并提供名称。
...
1. 使用CRXDE Lite，复制 */libs/social/intergations/livefyre/components/authorizablecomponent/authclientlib* to */apps/social/intergations/livefyre/components/authorizablecomponent/authclientlib*.
1. 编辑和保存 */apps/social/integrations/livefyre/components/authorizablecomponent/authclientlib/auth.js* 以实施满足您需求的Livefyre Auth委托。

   有关自定义身份验证委托的更多信息，请参阅 [身份集成](https://experienceleague.adobe.com/docs/livefyre/implementation/identity-integration/t-about-identity-integration.html).

   有关AEM Clientlibs的更多信息，请参阅 [使用客户端库](/help/sites-developing/clientlibs.md).
...
为页面启用Livefyre后，必须将容器配置为允许使用Livefyre组件。 请参阅 [在设计模式下配置组件](/help/sites-authoring/default-components-designmode.md) 以了解有关如何启用不同组件的说明。

>[!NOTE]
>
>在自定义单点登录集成中配置了身份验证后，需要进行身份验证的应用程序才能正常运行。

1. 从 **组件** 在设计模式下选择侧面板 **Livefyre** 列表，以将列表限制为可用的Livefyre组件。
...
1. 选择是创建新的Livefyre应用程序还是嵌入现有应用程序。
   ...
   有关插入组件的更多信息，请参阅 [编辑页面内容](/help/sites-authoring/editing-content.md).
...
1. 选择要导入的资产。 您选择的资产将在 **已选择** 选项卡。
1. **可选**:单击 **已选择** 选项卡，并查看要导入的选定UGC内容。
1. 单击&#x200B;**下一步**。
...
1. 对于权限请求，请为每个资产选择以下选项之一：
   ...
   * **手动请求权限** 以获取可复制并粘贴的消息，以及通过Instagram手动发送给内容所有者的消息。
   * **手动属性内容权限** 覆盖单个资产的权限。
...
1. 检查 **同步到Livefyre**.
1. 填写 **Livefyre页面前缀** 将此收藏集链接到AEM中的特定页面。
