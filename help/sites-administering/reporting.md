...
>[!NOTE]
>
>预配置的报告不会占用大量的性能，但仍建议在生产环境上使用每日快照。 如果可能，在网站上活动不多时运行这些每日快照；可以使用&#x200B;**Day CQ报告配置**&#x200B;的`Daily snapshots (repconf.hourofday)`参数定义；有关如何配置此配置的详细信息，请参阅[ OSGI配置](/help/sites-deploying/configuring-osgi.md)。
