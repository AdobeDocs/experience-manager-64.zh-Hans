...
>title="磁盘使用情况报告"
>abstract="作为 CTT 计算器的输入，用户可以通过运行“http://HOST:PORT/etc/reports/diskusage.html”下的磁盘使用情况报告来估算 AEM 存储库的大小。用户还可以使用“path”参数来估计特定存储库路径的大小，例如：“http://HOST:PORT/etc/reports/diskusage.html?path=/content/dam”。"
