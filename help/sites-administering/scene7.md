...
>* 如果您使用的是具有自定义代理配置的Dynamic Media Classic，请配置两个HTTP客户端代理配置，因为Experience Manager的某些功能使用3.x API，而其他一些功能使用4.x API。 3.x的配置 [http://localhost:4502/system/console/configMgr/com.day.commons.httpclient](http://localhost:4502/system/console/configMgr/com.day.commons.httpclient) 和4.x的配置 [http://localhost:4502/system/console/configMgr/org.apache.http.proxyconfigurator](http://localhost:4502/system/console/configMgr/org.apache.http.proxyconfigurator).
>
...
* 将集成的Experience Manager实例与 *Dynamic Media Classic*.
* 使用 *Dynamic Media* 集成到Experience Manager中。

如果最适合您，请使用以下标准确定哪种解决方案：

* 如果您是 **现有** Dynamic Media Classic客户的富媒体资产驻留在Dynamic Media Classic中进行发布和交付，但您希望将这些资产与站点(WCM)创作和/或Experience Manager Assets进行管理相集成，然后使用 [Experience Manager/Dynamic Media Classic点对点集成](#aem-scene-point-to-point-integration) 如本文档所述。
...
* **[!UICONTROL Dynamic Media]** - **[!UICONTROL Dynamic Media]** 组件是智能的 — 根据您添加的是图像还是视频，您可以选择各种选项。 该组件支持图像预设、基于图像的查看器（如图像集、旋转集、混合媒体集和视频）。 此外，查看器是响应式的 — 屏幕大小会根据屏幕大小自动更改。 所有查看器均为HTML5查看器。

* **[!UICONTROL 交互式媒体]** - **[!UICONTROL 交互式媒体]** 组件适用于具有交互性的轮播横幅、交互式图像和交互式视频，例如热点或图像映射。 此组件是智能的。 根据您添加的是图像还是视频，您可以选择多种选项。 此外，查看器是响应式的。 也就是说，屏幕的大小会根据屏幕大小自动更改。 所有查看器均为HTML5查看器。
...
1. 按照特定于您的用例的具体说明操作：
   ...
   <table> 
    ...
    <td><p>将资产上传到Experience Manager，然后使用Dynamic Media生成要下载/共享的演绎版。 自动将Experience Manager Assets发布到Dynamic Media Classic以进行交付。</p> <p><strong>重要信息：</strong> 处理过程出现重复，且在Experience Manager中生成的演绎版未同步到Dynamic Media Classic</p> </td> 
    <td><p>开启</p> <p>（请参阅步骤3）</p> </td> 
    <td>关闭</td> 
    <td>关闭</td> 
    <td><p><a href="#configuringautouploadingfromaemassets">开启</a></p> <p>（请参阅步骤4）</p> </td> 
...
如果资产已在Dynamic Media Classic中，并且在通用存储库中管理这些资产并不重要，则只需将这些资产导出到Experience Manager Assets以将其同步回Dynamic Media Classic以进行交付，这是不必要的往返操作。 否则，最好将资产保留在单个存储库中并同步到Dynamic Media Classic以仅交付。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>临时文件夹</td> 
   <td>手动上传到WCM中Dynamic Media Classic的目标文件夹以外的资产会自动放置在Dynamic Media Classic上的单独临时文件夹中。 您可以在云配置中以Experience Manager配置此功能。</td> 
  </tr> 
  <tr> 
   <td>混合媒体</td> 
...
如果在代理后运行Experience Manager或具有特殊的防火墙设置，则必须明确启用不同区域的主机。 服务器在 `/etc/cloudservices/scene7/endpoints` 和可根据需要进行自定义。 点按一个URL，然后进行编辑，以根据需要更改该URL。 在早期版本的Experience Manager中，这些值进行了硬编码。

如果您导航到 `/etc/cloudservices/scene7/endpoints.html`，则会看到列出的服务器（并可通过单击URL对其进行编辑）：
...
>[!NOTE]
>
>您可以有多个配置：一个云配置表示Dynamic Media Classic公司的一个用户。 如果要访问其他Dynamic Media Classic公司或用户，请创建多个配置。

要配置Experience Manager以能够将资产发布到Dynamic Media Classic，请执行以下操作：

1. 点按Experience Manager图标，然后导航到 **[!UICONTROL 部署>Cloud Services]** 访问Adobe Dynamic Media Classic。

1. 点按 **[!UICONTROL 立即配置]**.

   ![chlimage_1-297](assets/chlimage_1-297.png)

1. 在 **[!UICONTROL 标题]** 字段中，或者在 **[!UICONTROL 名称]** 字段，输入相应的信息。 点按 **[!UICONTROL 创建]**.
...
1. 输入Dynamic Media Classic帐户的电子邮件地址、密码和区域，然后点按 **[!UICONTROL 连接到Dynamic Media Classic]**. 您已连接到Dynamic Media Classic服务器，此时对话框将展开，显示更多选项。
...
1. 点按 [!UICONTROL 工具] 图标，然后导航到 **[!UICONTROL “操作”>“Web控制台”]**. 此时将打开Web控制台。
1. 导航到 **[!UICONTROL Adobe CQ Dynamic Media Classic Dam侦听器]** ，然后选择 **[!UICONTROL 已启用]** 复选框。
...
如果将Experience Manager实例配置为通过Dynamic Media Classic处理视频编码，则默认情况下，任何上传作业都会有35分钟的超时。 要适应可能运行时间较长的视频编码作业，您可以配置以下设置：
...
1. 根据需要，在 **[!UICONTROL 活动作业超时]** 字段。 以秒为单位接受任何非负数。 默认情况下，此数字设置为2100。
   ...
   >[!NOTE]
   >
   >最佳实践：大多数资产最多会在几分钟内摄取（例如，图像）。 但在某些情况下（如较大的视频），会将超时值增加到7200秒（2小时），以适应较长的处理时间。 否则，此Dynamic Media Classic上传作业将标记为 **[!UICONTROL 上载失败]** 在JCR元数据中。

1. 点按 **[!UICONTROL 保存]**.

### 奥图普洛阿丁从Experience Manager Assets {#autouploading-from-aem-assets}

从Experience Manager6.3.2开始，您现在可以为您配置Experience Manager Assets ，这样当您上传到数字资产管理器的任何数字资产位于CQ目标文件夹中时，这些资产就会自动更新到Dynamic Media Classic 。
...
要从Experience Manager Assets配置自动上传，请执行以下操作：

1. 点按Experience Manager图标，然后导航到 **[!UICONTROL 部署>Cloud Services]** 然后，在Dynamic Media标题的可用配置下，点按 **[!UICONTROL dms7(Dynamic Media)]**)
1. 点按 **[!UICONTROL 高级]** 选项卡，选择 **[!UICONTROL 启用自动上传]** 复选框，然后点按 **[!UICONTROL 确定]**. 现在，您必须配置DAM资产工作流，以包含上传到Dynamic Media Classic。
...
1. 导航回Experience Manager欢迎页面并点按 **[!UICONTROL 工作流]**. 双击 **DAM更新资产** 工作流将其打开。
1. 在Sidekick中，导航到 **[!UICONTROL 工作流]** 组件，然后选择 **[!UICONTROL Dynamic Media Classic]**. 拖动 **[!UICONTROL Dynamic Media Classic]** ，然后点按 **[!UICONTROL 保存]**. 目标文件夹中添加到Experience Manager Assets的资产会自动上传到Dynamic Media Classic。
...
1. 按照 [设置Dynamic Media Classic帐户的安全预览。](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish)
...
1. 点按 **[!UICONTROL 高级]** 选项卡。 在 **[!UICONTROL 启用安全视图]** 下拉菜单，选择 **[!UICONTROL 激活AEM发布时]** 将资产推送到Dynamic Media Classic而不发布。 (默认情况下，此值设置为 **[!UICONTROL 立即]**，立即发布Dynamic Media Classic资产。)
...
#### 在触屏优化用户界面中启用Dynamic Media Classic for WCM {#enabling-scene-for-wcm-in-the-touch-optimized-user-interface}

要在触屏优化UI中为WCM启用Dynamic Media Classic，请执行以下操作：

1. 点按Experience Manager图标，然后导航到 **[!UICONTROL 站点]** 然后是您网站的根页面（不特定于语言）。

1. 在工具栏中，选择 [!UICONTROL 设置] 图标，点按 **[!UICONTROL 打开属性]**.

1. 点按 **[!UICONTROL Cloud Services]** 点按 **[!UICONTROL 添加配置]** 选择 **[!UICONTROL Dynamic Media Classic]**.
1. 在 **[!UICONTROL Adobe Dynamic Media Classic]** 下拉列表中，选择所需的配置并点按 **[!UICONTROL 确定]**.

   ![chlimage_1-303](assets/chlimage_1-303.png)

   该Dynamic Media Classic配置中的视频预设可用于与该页面和子页面上的Dynamic Media Classic视频组件Experience Manager。

#### 在经典用户界面中启用Dynamic Media Classic for WCM {#enabling-scene-for-wcm-in-the-classic-user-interface}

要在经典UI中为WCM启用Dynamic Media Classic，请执行以下操作：

1. 在Experience Manager中，点按 **[!UICONTROL 网站]** 并导航到网站的根页面（不特定于语言）。

1. 在Sidekick中，点按 **[!UICONTROL 页面]** 图标，点按 **[!UICONTROL 页面属性]**.
...
1. 点按 **[!UICONTROL 高级]** 选项卡。 在 **[!UICONTROL 临时文件夹]** 字段，您可以修改 **临时** 文件夹。 默认情况下， **name_of_the_company/CQ5_adhoc**.
...
1. 点按Experience Manager图标，然后导航到 **[!UICONTROL “工具”>“操作”>“Web控制台”]**.
1. 在Adobe Experience Manager Web控制台配置面板中， **[!UICONTROL OSGi]** 菜单，点按 **[!UICONTROL 配置]**.
1. 在名称列下，查找并点按 **[!UICONTROL Adobe CQ Dynamic Media Classic资产MIME类型服务]** 以编辑配置。
...
1. 在 **[!UICONTROL jobParam]** 值字段。
   ...
   >
   >确保您的PSD文件具有“层”。 如果它是严格意义上的一个图像或带有蒙版的图像，则它仅作为图像处理，因为没有要处理的图层。
...
* 检查您尝试上传的资产是否位于 **[!UICONTROL CQ目标]** 文件夹(您可以在Dynamic Media Classic云配置中指定此文件夹)。
...
* 如果您的视频上传失败，并且您正在使用Experience Manager通过Dynamic Media Classic集成对视频进行编码，请参阅 [向Dynamic Media Classic上传工作流添加可配置超时](#adding-configurable-timeout-to-scene-upload-workflow).

>[!CAUTION]
>
>从现有Dynamic Media Classic公司帐户导入资产可能需要较长时间，才能在Experience Manager中显示。 确保在Dynamic Media Classic中指定的文件夹的资产不太多（例如，根文件夹的资产通常不太多）。
>
>如果要测试集成，请让根文件夹仅指向子文件夹，而不是整个公司。
