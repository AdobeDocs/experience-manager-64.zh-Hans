...
1. 导航到** Apache Felix OSGi管理控制台**并更改用 **户名** 和 **密码**。
...
>[!NOTE]
>
>如果此实例在生产就绪模式下运行，则删除示例We.Retail应 [用程序](/help/sites-administering/production-ready.md)。 如果由于任何原因，不是这样，您可以通过转到“包管理器”来卸载示例内容，然后搜索并卸载所有We.Retail包。 有关更多信息，请参 [阅如何使用包](package-manager.md)。
...
1. 检查默认呈示器的配置，该呈示器可能是一个问题区域。
   ...
   * 尤其是JSON渲染器，它可以跨多个级别横向于树结构。
      ...
      **JSON最大结果** ( `json.maximumresults`)
      ...
      在 [Apache Sling GET servlet的配置中](/help/sites-deploying/osgi-configuration-settings.md)。 超出此限制时，渲染将折叠。 AEM中Sling的默认值是 `200`。

   * 作为预防措施，请禁用其他默认呈示器（HTML、纯文本、XML）。 再次通过配置 [Apache Sling GET Servlet](/help/sites-deploying/osgi-configuration-settings.md)。
   >[!CAUTION]
   >
...
务必确保不会在存储库用户主路径中显示任何个人不可识别的信息，从而保护用户。
...
1. 转到https://&lt;serveraddress>:4502/crx/de/index.jsp，打 *开CRXDE Lite*
1. 选择节 `/etc/key` 点。
1. 转到“复 **制** ”选项卡。
1. 按“复 **制** ”按钮。
