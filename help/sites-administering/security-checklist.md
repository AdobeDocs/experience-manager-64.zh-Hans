...
有关详细信息，请参阅[在生产就绪模式中运行AEM](/help/sites-administering/production-ready.md)。

### 为传输层安全性{#enable-https-for-transport-layer-security}启用HTTPS
...
确保您已安装Adobe](https://helpx.adobe.com/cn/experience-manager/kb/aem63-available-hotfixes.html)提供的最新[安全修补程序。
...
可以通过[Granite Operations - Users](/help/sites-administering/granite-user-group-admin.md)控制台更改AEM管理帐户的口令。
...
您还必须更改用于访问Web控制台的口令。 要完成此操作，请配置[Apache Felix OSGi管理控制台](/help/sites-deploying/osgi-configuration-settings.md)的以下属性：
...
### 完整的调度程序安全核对清单{#complete-dispatcher-security-checklist}

AEM Dispatcher是您的基础架构的关键部分。 Adobe强烈建议您完成[调度程序安全核对清单](https://helpx.adobe.com/cn/experience-manager/dispatcher/using/security-checklist.html)。
...
AEM的标准安装将`admin`指定为默认[复制代理](/help/sites-deploying/replication.md)内传输凭据的用户。 此外，管理员用户还用于在作者系统上源复制。
...
该仪表板还附带一系列安全运行状况检查。 建议您在与生产实例一起上线之前检查所有安全运行状况检查的状态。 有关详细信息，请查阅[操作仪表板文档](/help/sites-administering/operations-dashboard.md)。
...
>[!NOTE]
>
>如果此实例在[生产就绪模式](/help/sites-administering/production-ready.md)中运行，则删除示例We.Retail应用程序。 如果由于任何原因，您不能卸载示例内容，方法是转到“包管理器”，然后搜索并卸载所有We.Retail包。 有关详细信息，请参阅[如何使用包](package-manager.md)。
...
[适用于Eclipse的AEM Developer Tools](/help/sites-developing/aem-eclipse.md)部署了Apache Sling工具支持安装(org.apache.sling.tooliping.support.install)。
...
#### Sling推荐人滤镜{#the-sling-referrer-filter}
...
1. 编辑此过滤器应用于`Filter Methods`字段检查的方法。
...
>[!NOTE]
>
>除&#x200B;**“Day CQ WCM调试过滤器**”之外，以下所有设置都自动由[生产就绪模式](/help/sites-administering/production-ready.md)覆盖。 因此，我们建议在高效环境部署实例之前先查看所有设置。

对于以下每项服务，需要更改指定的设置：

* [Adobe Granite HTML库管理器](/help/sites-deploying/osgi-configuration-settings.md):
...
* 通过请求具有不限数量URL的内容页面，URL可以包括句柄、某些选择器、扩展和后缀 — 其中任何一个都可以修改。
   ...
   * `.../en.html/SuffixDosAttack`
   ...
   调度程序将对所有有效变量（例如返回`200`响应并配置为缓存）进行缓存，最终导致完整文件系统，并且不提供任何服务用于进一步请求。
...
1. 在应用程序级别加入控件；由于可能的变量数，默认配置不可行。
   ...
   * 控制应用程序中的选择器，以便您&#x200B;*仅*&#x200B;提供所需的显式选择器，并为所有其他选择器返回`404`。
   * 防止输出不限数量的内容节点。
...
1. 检查默认呈示器的配置，该呈示器可能是问题区域。
   ...
   * 尤其是可以跨多个级别横向树结构的JSON渲染器。
      ...
      **JSON最大结果** ( `json.maximumresults`)
      ...
      中。 [](/help/sites-deploying/osgi-configuration-settings.md)超过此限制时，渲染将折叠。 AEM中的Sling的默认值为`200`。

   * 作为预防措施，禁用其他默认呈示器（HTML、纯文本、XML）。 同样，通过配置[Apache SlingGETServlet](/help/sites-deploying/osgi-configuration-settings.md)。
   >[!CAUTION]
   >
...
由于AEM不为`FormChooserServlet`提供开箱即用索引，在查询中使用表单选择器将触发代价高昂的存储库遍历，通常会使AEM实例陷入停顿。 表单选择器可以通过存在&#x200B;**&amp;ast;.form来检测。&amp;ast;**&#x200B;字符串(查询)。
...
如果您的设置要求启用资产下载服务器，请参阅[此文章](/help/assets/download-assets-from-aem.md#disable-asset-download-servlet)以了解详细信息。
...
### 验证您没有在用户主路径{#verify-that-you-are-not-disclosing-personally-identifiable-information-in-the-users-home-path}中披露个人可识别信息

务必通过确保不会在存储库用户主路径中公开任何个人不可识别信息来保护用户。

自AEM 6.1起，用户（也称为可授权）ID节点名称的存储方式随`AuthorizableNodeName`接口的新实现而改变。 新界面将不再在节点名称中显示用户ID，而是生成随机名称。

不需要执行任何配置来启用它，因为这是在AEM中生成可授权ID的默认方式。

尽管不推荐，但您可以禁用它，以备需要旧实现以向后兼容现有应用程序。 为此，您需要：
...
>[!NOTE]
>
>有关详细信息，请参阅[可授权节点名称生成](https://jackrabbit.apache.org/oak/docs/security/user/authorizablenodename.html)的Oak文档。

### 防止Clickjacking {#prevent-clickjacking}

为防止点击劫持，我们建议您将Web服务器配置为提供设置为`SAMEORIGIN`的`X-FRAME-OPTIONS` HTTP头。

有关clickjacking的更多[信息，请参阅OWASP站点](https://www.owasp.org/index.php/Clickjacking)。
...
1. 复制HMAC和主控文件。
1. 然后，转到要将HMAC键重复到的目标实例，并导航到数据文件夹。 例如：
...
#### 复制AEM 6.2及更早版本{#replicating-keys-for-aem-and-older-versions}的密钥
