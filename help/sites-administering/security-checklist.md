...
1. 导航至** Apache Felix OSGi管理控制台**并更改用 **户名** 和密 **码**。
...
>[!NOTE]
>
>如果此实例在生产就绪模式下运行，则删除示 [例We.Retail应用程序](/help/sites-administering/production-ready.md)。 如果由于某种原因，不是这样，您可以通过转到包管理器来卸载示例内容，然后搜索并卸载所有We.Retail包。 有关详细信息，请参 [阅如何使用包](package-manager.md)。
