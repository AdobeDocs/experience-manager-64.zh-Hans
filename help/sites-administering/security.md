...
权限定义允许谁对资源执行哪些操作。 权限是 [访问控制](#access-control-lists-and-how-they-are-evaluated) 评估。
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>用户可以：</p> 
    <ul> 
     <li>创建新页面或子页面。</li> 
    </ul> <p>如果 <strong>修改</strong> 拒绝jcr:content下的子树被明确排除，因为创建jcr:content及其子节点被视为页面修改。 这仅适用于定义jcr:content子节点的节点。</p> </td> 
  </tr> 
  <tr> 
   <td>删除</td> 
   <td><p>用户可以：</p> 
    <ul> 
     ...
     <li>删除页面或子页面。</li> 
    </ul> <p>如果 <strong>修改</strong> 拒绝删除jcr:content下的任何子树，因为删除jcr:content时，其子节点会被视为页面修改。 这仅适用于定义jcr:content子节点的节点。</p> </td> 
  </tr> 
  <tr> 
   <td>读取 ACL</td> 
...
>[!NOTE]
>
>AEM会自动为中的角色分配（所有者、编辑者、查看者）生成用户组 [收藏集](/help/assets/managing-collections-touch-ui.md). 但是，手动为此类组添加ACL可能会在AEM中引入安全漏洞。 Adobe建议您避免手动添加ACL。
...
>对于CQ 5.3用户：
>
>与以前的CQ版本相比， **创建** 和 **删除** 如果用户只需要修改页面，则不再授予该权限。 相反，应授予 **修改** 操作。
>
...
在修改任一权限之前，请确保您了解这些权限的工作方式和相互关系。 请参阅CRX文档以说明AEM WCM的方式 [评估访问权限](/help/sites-administering/user-group-ac-admin.md#how-access-rights-are-evaluated) 以及有关设置访问控制列表的示例。
...
使用 [模拟功能](/help/sites-authoring/user-properties.md#user-settings) 用户可以代表其他用户工作。
...
>[!NOTE]
>
>为了模拟以便非管理员用户工作，模拟器（在上例中为user-B）需要在 `/home/users` 路径。
>
>有关如何实现此操作的更多信息，请参阅 [AEM中的权限](/help/sites-administering/security.md#permissions-in-aem).
...
| 制表符 | 描述 |
|--- |--- |
| 过滤器框 | 用于筛选列出的用户和/或组的机制。 请参阅 [筛选用户和群组](#filtering-users-and-groups). |
| 隐藏用户 | 一个切换开关，将隐藏列出的所有用户，仅保留群组。 请参阅 [隐藏用户和群组](#hiding-users-and-groups). |
| 隐藏组 | 一个切换开关，将隐藏列出的所有组，仅保留用户。 请参阅 [隐藏用户和群组](#hiding-users-and-groups). |
...
1. 导航到要添加权限的路径并单击 **详细信息**. 详细信息窗口的下半部分提供有关谁具有该页面权限的信息。

   ![chlimage_1-349](assets/chlimage_1-349.png)

1. 选中 **会员** 列。 清除要删除其权限的成员的复选框。 在您对进行更改的单元格中，会出现一个红色三角形。
1. 单击 **确定** 以保存更改。
...
1. 导航到要删除权限的路径并单击 **详细信息**. 详细信息窗口的下半部分提供有关谁具有该页面权限的信息。
...
当部署为 [发布场](/help/sites-deploying/recommended-deploys.md#tarmk-farm)，则需要在所有发布节点之间同步用户和组。
...
自定义权限的定义和注册正式属于 [Jackrabbit API](https://jackrabbit.apache.org/api/2.8/org/apache/jackrabbit/api/security/authorization/PrivilegeManager.html) 截至版本2.4(另请参阅 [JCR-2887](https://issues.apache.org/jira/browse/JCR-2887))。 JCR访问控制管理(如 [JSR 283](https://jcp.org/en/jsr/detail?id=283) （第16节）。 此外，Jackrabbit API还定义了几个扩展。
