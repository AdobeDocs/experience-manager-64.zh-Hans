...
>[!NOTE]
>
>如果需要会话，建议使用SSO解决方案或使用粘性会话，并让客户在切换到其他发布者时登录。

>[!CAUTION]
>
>*管理员&#x200B;**** *组不支持同步，即使启用了用户同步。 相反，“导入差异”失败将记录在错误日志中。
>
>因此，当部署是发布场时，如果将用户添加到***administrators** *group或从中删除用户，则必须在每个发布实例上手动进行修改。
...
### 3.Adobe Granite分发——加密密码传输机密提供商 {#adobegraniteencpasswrd}
...
* **节点类型**
   ...
   这是将同步的节点类型的列表。 除sling:Folder之外的任何节点类型都需要在此列出（sling:folder需要单独处理）。
...
* **可忽略属性**
   ...
   这是属性的列表，如果检测到任何更改，将忽略这些属性。 对这些属性所做的更改可能会作为其他更改的副作用而同步（因为同步始终在节点级别），但对这些属性所做的更改本身不会触发同步。
...
为了使更新正确同步，必须修改Vault包生成器以进行用户同步：
...
* 查找 `Apache Sling Distribution Packaging - Vault Package Builder Factor`

   * `Builder name: socialpubsync-vlt`

* 选择编辑图标
* 添加两个 `Package Filters` :
...
当从创作环境运行诊断时，通过／失败结果将包含一个 [INFO] （信息）部分，其中显示已配置发布实例的列表以进行确认。

该列表包含每个发布实例的URL，该实例将为该实例运行诊断。 url参数 `syncUser` 将附加到诊断URL中，其值设置为在步骤2中创 *建的授权同步* 用户 [](/help/sites-administering/sync.md#createauthuser)。

**注意** :在启动URL之前，授权 *的同步用户* ，必须已登录到该发布实例。

![chlimage_1-398](assets/chlimage_1-398.png)

### 配置添加错误 {#improperconfig}
...
#### （作者）一个Adobe Granite分发——加密密码传输机密提供商 {#author-one-adobe-granite-distribution-encrypted-password-transport-secret-provider}
...
#### （发布）一个Adobe Granite分发——差异观察器工厂 {#publish-one-adobe-granite-distribution-diff-observer-factory}
