...
用户数据及其[ACL](/help/sites-administering/security.md)存储在[Oak Core](/help/sites-deploying/platform.md)中，即Oak JCR下的层，并使用[Oak API](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/org/apache/jackrabbit/oak/api/package-tree.html)进行访问。 如果更新不频繁，则使用[Sling内容分发](https://github.com/apache/sling/blob/trunk/contrib/extensions/distribution/README.md)（Sling分发）将用户数据与其他发布实例同步是合理的。
...
该列表中包含每个发布实例的URL，该实例将运行该实例的诊断。 url参数`syncUser`被附加到诊断URL中，其值设置为在[步骤2](/help/sites-administering/sync.md#createauthuser)中创建的&#x200B;*授权同步用户*。
