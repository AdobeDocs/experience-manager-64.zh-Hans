...
>
>因此，当部署是发布场时，如果将用户添加到*administrators **** *group或从中删除，则必须对每个发布实例手动进行修改。
...
### 5.Adobe花岗岩分布——差异观察工厂 {#diffobserver}
...
* **在作者**
   ...
   * 定位 `Apache Sling Distribution Trigger - Scheduled Triggers Factory`
      ...
      * 将时间 `Interval in Seconds` 间隔设置为所需的间隔
      * select `Save`
...
* 定位 `Apache Sling Distribution Packaging - Vault Package Builder Factor`
...
该列表中包含每个发布实例的URL，该实例将运行该实例的诊断。 url参数会 `syncUser` 附加到诊断URL，其值会设置为在步骤2中创 *建的授权同步*[用户](/help/sites-administering/sync.md#createauthuser)。

**注意** : 启动URL之前，授权 *的同步用户* ，必须已登录到该发布实例。

![chlimage_1-398](assets/chlimage_1-398.png)

### 未正确添加配置 {#improperconfig}
...
#### （作者）一个AdobeGranite分发——加密密码传输机密提供程序 {#author-one-adobe-granite-distribution-encrypted-password-transport-secret-provider}
