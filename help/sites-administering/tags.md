...
标记是对网站中的内容进行分类的一种快速而简便的方法。 它们可以被视为关键字或标签（元数据），以便在搜索结果中更快地找到内容。
...
* 启用资源(请参阅 [标记支持资源](/help/communities/functions.md#catalog-function))

## 标记功能 {#tag-features}

AEM中标记的一些功能包括：

* 标记可以分组为各种命名空间。 此类层次结构允许构建分类。 这些分类在整个AEM中是全局性的。
...
* 标记可由作者和网站访客应用。 无论其创建者是谁，在分配到页面或进行搜索时，都会提供所有形式的标记供选择。
* “标签管理员”组的成员和具有修改权限的成员可以创建标签并修改其分类 `/content/cq:tags`.

   * 包含子标记的标记称为容器标记
   * 非容器标记的标记称为叶标记
   * 标记命名空间是叶标记或容器标记

* 标记由 [搜索组件](https://helpx.adobe.com/experience-manager/core-components/using/quick-search.html) 以便于查找内容。
...
命名空间本身是一个标记，不需要包含任何子标记。 但是，要继续创建分类， [创建子标签](#creating-tags)，这反过来可以是叶标记或容器标记。
...
如果浏览器窗口不够宽，无法显示所有图标，则最右侧的图标将分组在 **`... More`** 图标，该图标将在选中时显示隐藏操作图标的下拉列表。
...
选择命名空间或其他标记后，选择 **`View Properties`** 图标，可显示 `name`、上次编辑时间和引用数。 如果已发布，则会显示上次发布时间和发布者的ID。 此信息将显示在标记列左侧的列中。
...
选择命名空间或其他标记（通过选择标题旁边的图标）后，可通过选择 **`Create Tag`** 图标。
...
选择命名空间或其他标记后，可以更改标题、描述，并通过选择**`Edit`**图标。
...
>[!NOTE]
>
>建议仅允许作者 [编辑](#editing-tags) 标签 `title`，不会移动或重命名标记。
...
当分类具有重复项时，可以使用合并标记。 当标记A合并到标记B中时，所有使用标记A标记的页面都将使用标记B进行标记，并且标记A不再可供作者使用。
...
选择命名空间或其他标记后，选择**`Publish`**图标来激活发布环境中的标记。 与页面内容类似，只会发布选定的标记，而不考虑该标记是否为容器标记。

要发布分类（命名空间和子标记），最佳做法是创建 [软件包](/help/sites-administering/package-manager.md) 命名空间(请参阅 [分类根节点](/help/sites-developing/framework.md#taxonomy-root-node))。 一定要 [应用权限](#setting-tag-permissions) 命名空间。

### 取消发布标记 {#unpublishing-tags}

![chlimage_1-202](assets/chlimage_1-202.png)

选择命名空间或其他标记后，选择**`Unpublish`**图标将在创作环境中停用标记，并将其从发布环境中删除。 与 `Delete`操作时，如果所选标记是容器标记，则其所有子标记将在创作环境中停用，并从发布环境中删除。

### 删除标记 {#deleting-tags}

![chlimage_1-203](assets/chlimage_1-203.png)

选择命名空间或其他标记后，选择**`Delete`**图标将从创作环境中永久删除标记。 如果标记已发布，则也会从发布环境中将其删除。 如果所选标记是容器标记，则其所有子标记也将被删除。

## 设置标记权限 {#setting-tag-permissions}

标记权限包括 [“安全（默认）”](/help/sites-administering/production-ready.md);发布环境的最佳实践，它要求明确允许标记具有读取权限。 基本上，这是通过在创作上设置权限后创建标记命名空间包，并在所有发布实例上安装该包来完成的。
...
* 在创作实例中
   ...
   * 在左窗格中，选择其所在的组（或用户） [读取权限](/help/sites-administering/security.md#permissions) 将授予
   * 在右侧窗格中，找到**标记命**空间的路径
...
* 确保所有发布实例具有相同的权限
   ...
   * 一种方法是 [创建资源包](/help/sites-administering/package-manager.md#package-manager) 作者命名空间的

      * on `Advanced` 选项卡， `AC Handling` 选择 `Overwrite`
   * 复制包
...
的 `title`标记的属性可以翻译成多种语言。 翻译完毕后，将显示相应的标记 `title`可以根据用户语言或页面语言显示。

### 定义多种语言的标记标题 {#defining-tag-titles-in-multiple-languages}

下面介绍了如何翻译 `title`标记 **动物** 从英语到德语和法语。

首先，在 **Stock摄影** 命名空间和选择**`Edit`**图标(请参阅 [编辑标记](#editing-tags) )。
...
通常，为标记选择的语言是从页面语言（如果可用）中获取的。 当 [ `tag` 小组件](/help/sites-developing/building.md#tagging-on-the-client-side) 在其他情况下（例如在表单或对话框中），标记语言取决于上下文。

标记控制台不使用页面语言设置，而是使用用户语言设置。 在“标记”控制台中，对于“Animals”标记，将为用户属性中将语言设置为法语的用户显示“Animaux”。

要向对话框中添加新语言，请参阅 [向“编辑标记”对话框添加新语言](/help/sites-developing/building.md#adding-a-new-language-to-the-edit-tag-dialog).
