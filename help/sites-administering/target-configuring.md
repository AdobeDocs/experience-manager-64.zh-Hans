...
要找到云配置和框架，请导航到 **Cloud Services** 通过 **工具** > **部署** > **云**. ([http://localhost:4502/libs/cq/core/content/tools/cloudservices.html](http://localhost:4502/libs/cq/core/content/tools/cloudservices.html))\
...
1. 在对话框中，提供这些属性的值。
   ...
   * **密码**:目标帐户密码。
   * **API类型**:REST或XML
   * **A4T Analytics Cloud 配置**：选择用于 Target 活动目标和量度的 Analytics Cloud 配置。如果您在定位内容时使用 Adobe Analytics 作为报告源，则需要此项。如果您看不到云配置，请参阅 [配置A4T Analytics Cloud配置](#configuring-a-t-analytics-cloud-configuration).
...
1. 单击&#x200B;**连接到 Target** 可初始化与 Target 的连接。如果连接成功，则会显示“**连接成功**”消息。 单击消息上的&#x200B;**确定**，然后单击对话框上的&#x200B;**确定**。
...
配置 Target 云配置后，可以添加 Target 框架。该框架可识别从可用 [Client Context](/help/sites-administering/client-context.md) 或 [ContextHub](/help/sites-administering/contexthub-config.md) 组件。 Target 使用参数来确定适用于当前上下文的分段。

您可以为单个 Target 配置创建多个框架。在需要针对网站的不同部分向 Target 发送一组不同的参数时，多个框架会很有用。为需要发送的每组参数创建一个框架。将网站的每个部分与适当的框架关联。请注意，网页一次只能使用一个框架。
...
1. 在“创建框架”对话框中，指定&#x200B;**标题**，选择 **Adobe Target 框架**，然后单击&#x200B;**创建**。
   ...
   这将打开框架页面。Sidekick提供表示 [Client Context](/help/sites-administering/client-context.md) 或 [ContextHub](/help/sites-administering/contexthub-config.md) 你可以映射的。
