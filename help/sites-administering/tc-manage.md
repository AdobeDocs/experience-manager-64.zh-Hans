...
翻译项目使您能够管理AEM内容的翻译。 翻译项目是一种AEM项 [目类型](/help/sites-authoring/projects.md) ，它包含要翻译为其他语言的资源。 这些资源是从语言母版创建 [的语言副本](/help/sites-administering/tc-prep.md) 的页面和资产。
...
翻译后，您可以在 [覆盖包含该翻译的语言副本](#reviewing-and-promoting-updated-content) 之前审阅翻译。 当选定页面不存在语言副本时，将显示“创建并翻译”选项卡以提供对项目相关命令的访问。
...
1. 单击或点按创建和翻译，然后配置翻译作业：
   ...
   * 使用“语言”下拉列表选择要翻译的语言副本。 根据需要选择其他语言。 列表中显示的语言与您创 [建的语言根相对应](/help/sites-administering/tc-prep.md#creating-a-language-root)。
...
将页面添加到翻译项目时，这些页面将包含在新的翻译作业中。 您还可以将 [页面添加到现有作业](#adding-pages-assets-to-a-translation-job)。
...
1. 或者，您也可以选择搜索图标以轻松查找要添加到翻译作业的页面或资产。
...
>[!NOTE]
>
>有关i18n词典的详细信息，请阅读使用 [翻译器管理词典](/help/sites-developing/i18n-translator.md)。
...
| 状态 | 描述 |
|---|---|
| 个草稿 | 翻译作业尚未启动。 创建翻译作业时，它们处于“草稿”状态。 |
| 已提交 | 翻译作业中的文件在成功发送到翻译服务后具有此状态。 在发出“请求范围”命令或“开始”命令后，可能会出现此状态。 |
| 已请求设定范围 | 对于“人工翻译”工作流程，作业中的文件已提交给翻译供应商进行范围界定。 此状态在发出“请求范围”命令后显示。 |
| 已完成范围的设定 | 供应商已对翻译作业进行了作用范围。 |
| 承诺翻译 | 项目所有者已接受该范围。 此状态表示翻译供应商应开始翻译作业中的文件。 |
| 正在翻译 | 对于作业，作业中一个或多个文件的翻译尚未完成。 对于作业中的某个项目，该项目正在翻译。 |
| 已翻译 | 对于作业，作业中所有文件的翻译完成。 对于作业中的某个项目，该项目将进行翻译。 |
...
1. 当作业状态更改为SCOPE_COMPLETED时，在“转换作业”拼贴上单击或点按命令菜单，然后单击或点按显示范围。
...
填充您的语言副本，使其包含您正在翻译的主语言中的内容。 在填充语言副本之前，必须已 [创建语言副本的语言根](/help/sites-administering/tc-prep.md#creating-a-language-root) 。
...
## 使用“项目”控制台创建翻译项目 {#creating-a-translation-project-using-the-projects-console}

如果您希望使用“项目”控制台，可以手动创建翻译项目。

在手动创建翻译项目时，除了基本属性之外，还必须为下列翻译相关属性提供 [值](/help/sites-authoring/touch-ui-managing-projects.md#creating-a-project):
...
1. 单击或点按创建。在确认框中，单击或点按完成以返回到项目控制台，或者单击或点按打开项目以打开并开始管理项目。
...
1. 在“导出”对话框中，单击或点按“下载导出的文件”，如有必要，使用Web浏览器对话框保存文件。
1. 在“导出”对话框中，单击或点按关闭。
