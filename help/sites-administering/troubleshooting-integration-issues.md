...
分析设计时考虑了继承机制。 通常，您可以通过在页面属性云服务选项卡中添加对Analytics配置的引用来为站点启 [用Analytics](/help/sites-developing/extending-cloud-config.md) 。 然后，该配置会自动继承到所有子页面，无需再次引用它，除非页面需要其他配置。 添加对站点的引用还会自动创建多个节点（AEM 6.3及更早版本为12,AEM 6.4为6），这些节点将实例化用于将Analytics数据导入AEM的PollingImporters `cq;PollConfig` 类型。 因此：
