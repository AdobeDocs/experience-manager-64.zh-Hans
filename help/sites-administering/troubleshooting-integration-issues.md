...
在设计Analytics时考虑了继承机制。 通常，您通过在页面属性[Cloud Services](/help/sites-developing/extending-cloud-config.md)选项卡中添加对Analytics配置的引用，来为站点启用Analytics。 然后，该配置会自动继承到所有子页面，而无需再次引用它，除非页面需要其他配置。 添加对站点的引用还会自动创建多个类型为`cq;PollConfig`的节点(AEM 6.3及更早版本的节点12或AEM 6.4的节点6)，这些节点可实例化用于将Analytics数据导入AEM的PollingImporter。 因此：
