...
* 手动：
   ...
   * 从工作 [流模型](#workflow-models)。
   * 使用工作流包进 [行批处理](#workflow-packages-for-batch-processing)。
...
您可以基于“工 [作流模型”控制台中列](/help/sites-administering/workflows.md#workflow-models-and-instances) 出的某个模型来开始工作流。 唯一的必需信息是有效负荷，但也可以添加标题和／或评论。
...
* `/var/statistics`
   ...
   * 异常： 对以下节点的更 `/var/statistics/tracking` 改 *会导致* 工作流启动。

标准安装中包含各种定义。 这些功能适用于数字资产管理和社交协作任务:

![wf-100](assets/wf-100.png)

## 批处理的工作流包 {#workflow-packages-for-batch-processing}
...
1. 使用工具 **、工作流** ，然后使用模型 **导航**&#x200B;到“模型 **”控**&#x200B;制台，然 **后访问**&#x200B;模型。
...
1. 使用工具、工 **作流** ，然后使 **用Laucher** Launchers **导航**&#x200B;到Workflow Launchers控 ****&#x200B;制台。
1. 选择 **创建**, **然后选择添加启动** 器，以打开对话框：
   ...
   * **运行模式**
      ...
      工作流启动器应用的服务器类型。 选择 **“作者**”、“ **发布**”或“作 **者与发布”**。
   ...
   * **排除列表**
      ...
      * `property-name` 忽略在 `jcr` 指定属性名称上触发的任何事件。 &quot;
      * `event-user-data:<*someValue*>` 忽略包含通过 `*<someValue*`API `user-data` 设置的 [ >的 `ObservationManager` 任]何事件(https://docs.adobe.com/content/docs/en/spec/jsr170/javadocs/jcr-2.0/javax/jcr/observation/ObservationManager.html#setUserData(java.lang.String)。
...
创建启动器配置后，您可以使用同一控制台选择实例，然后 **视图属性** （并编辑属性） **或删除**。
