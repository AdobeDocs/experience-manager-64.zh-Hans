...
组件浏览器是侧面板中的一个选项卡(与资产浏览器 [和内容树](/help/sites-authoring/author-environment-tools.md#assets-browser) 一 [起使用](/help/sites-authoring/author-environment-tools.md#content-tree))。 要打开（或关闭）侧面板，请使用工具栏左上方的图标：
...
* **移动设备（例如 iPad）**
   ...
   要向页面添加组件，请按住所需的组件并向右移动，组件浏览器将关闭以再次显示页面，您可以在该页面中放置组件。
...
内容树是侧面板中的一个选项卡（侧面板中还有组件浏览器和资产浏览器）。要打开（或关闭）侧面板，请使用工具栏左上角的图标：
...
页面信息（均衡器图标）会打开一个菜单，其中还提供有关上次编辑和上次发布的详细信息。 根据页面（及其站点）的特性，可用的选项可能会更多，也可能会更少：
...
>* 有一个[键盘快捷键](/help/sites-authoring/page-authoring-keyboard-shortcuts.md) (`Ctrl-Shift-M`) 可在&#x200B;**预览模式**&#x200B;和当前选择的模式（例如，**编辑**、**布局**&#x200B;等）之间进行切换。
>
