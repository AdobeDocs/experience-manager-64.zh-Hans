...
您可以将 AEM 或 Adobe Target 用作定位引擎（您必须拥有有效的 Adobe Target 帐户才能使用 Adobe Target）。如果您使用的是 Adobe Target，则必须先配置集成。See [instructions for integrating with Adobe Target](/help/sites-administering/target.md).
...
1. Click or tap **Targeting**. 定位选项随即会显示在页面顶部。
...
1. 在&#x200B;**定位**&#x200B;引擎下拉菜单中，选择您的定位引擎。
   ...
   * If you select **ContextHub AEM**, the remaining fields are dimmed and not available. 单击或点按&#x200B;**创建**。
...
如果访客属性与映射到体验的任何区段都不匹配，则也会显示默认体验。See [Adding Experiences using Targeting Mode](#adding-and-removing-experiences-using-targeting-mode).
...
>[!NOTE]
>
>Offers that you create using the [Offers console](/help/sites-authoring/offerlib.md) can also contain several components. 此类选件是库选件，可以在多个体验中使用。
...
1. 选择体验以显示库选件。
1. 将库选件转换为本地/自定义选件。See [Converting a Library Offer to a Custom Library](#converting-a-library-offer-to-a-custom-library).
1. 编辑选件的内容。

1. 将选件重新保存到库。See [Adding a Custom Offer to a Library](#adding-a-custom-offer-to-a-library).
...
The Goals &amp; Settings step of [the targeting process](/help/sites-authoring/content-targeting-touch.md#the-targeting-process-create-target-and-goals-settings) involves configuring the behavior of the brand activity. 这包括指定活动的开始和结束时间，以及指定活动优先级。此外，还可以跟踪目标。具体来说，您可以决定要通过活动衡量的内容。
