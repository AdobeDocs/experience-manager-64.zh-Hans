...
>[!CAUTION]
>此基础组件已弃用。 Adobe建议利用 [核心组件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html?lang=zh-Hans) 中。
...
>[!CAUTION]
>此基础组件已弃用。 Adobe建议利用 [核心组件图像组件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/components/image.html) 中。
...
* **列表**
   ...
   * **生成列表对象**
      ...
      请注意，无论选择哪种方法，列表中包含的页面都应该已经有一个与页面关联的图像，该图像将显示在轮播中。 如果给定页面的“页面属性”下没有该页面的图像，则应该在开始之前将图像与页面相关联，否则传送将显示空白（或大部分为空白）页面。 请参阅 [编辑页面属性](/help/sites-authoring/editing-page-properties.md).
...
>[!NOTE]
>
>您可以为Adobe Experience Manager创建一个自定义轮播组件，以显示位于AEM DAM中的数字资产。 有关信息，请参阅 [为Adobe Experience Manager创建自定义轮播组件](https://helpx.adobe.com/experience-manager/using/custom-carousel-components.html).
...
>[!NOTE]
>
>您可以创建一个自定义AEM图表控件，以显示位于AEM JCR中的数据。 有关信息，请参阅 [在图表中显示Adobe Experience Manager数据](https://helpx.adobe.com/experience-manager/using/displaying-experience-manager-data-chart.html).
...
>[!NOTE]
>
>根据您的用例，还有其他选项可用于外部应用程序的集成，例如。 the [Portlet的集成](/help/sites-administering/aem-as-portal.md).
...
此组件提供了一个网格段落系统，允许您在 [响应式网格](/help/sites-authoring/responsive-layout.md). 这允许您根据目标设备（包括一系列手机、平板电脑和台式机）的宽度来定义不同的内容布局。

![chlimage_1-196](assets/chlimage_1-196.png)

>[!NOTE]
>
>此组件已通过 [HTML模板语言(HTL)](https://helpx.adobe.com/experience-manager/htl/user-guide.html).
...
的 **参考** 组件允许您从AEM网站的其他页面引用文本（在当前实例中）。 引用段落的内容随后会像在当前页面上一样显示。 当源段落发生更改时，内容将会更新（可能需要刷新页面）。
...
>[!CAUTION]
>此基础组件已弃用。 Adobe建议利用 [快速搜索核心组件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/components/quick-search.html) 中。
...
>[!CAUTION]
>此基础组件已弃用。 Adobe建议利用 [轮播核心组件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/components/carousel.html) 中。
...
>[!CAUTION]
>此基础组件已弃用。 Adobe建议利用 [文本核心组件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/components/text.html) 中。

>[!NOTE]
>
>的 **表** 组件基于 [富文本编辑器](/help/sites-authoring/rich-text-editor.md)，与 **[文本](#text)** 组件。
>
>建议使用 **表** 组件，但也可以使用 **文本** 组件。
...
>[!NOTE]
>
>的 **文本** 组件基于 [富文本编辑器](/help/sites-authoring/rich-text-editor.md)，与 **[表](#table)** 组件。
>
>建议使用 **表** 组件，但也可以使用 **文本** 组件。
...
>[!NOTE]
>
>有关开发和进一步自定义表单的信息，请参阅 [开发Forms页面](/help/sites-developing/developing-forms.md). 这包括添加操作、约束、预加载字段和使用脚本调用服务以执行操作等。
...
表单组件使用 **表单开始** 和 **表单结尾** 元素。 这些表单始终是成对的，以确保正确定义表单。
...
>[!NOTE]
>
>您可以创建自定义上传组件以将文件上传到Sling Servlet。 有关信息，请参阅 [将文件上传到Adobe Experience Manager](https://helpx.adobe.com/experience-manager/using/uploading-files-aem1.html).
