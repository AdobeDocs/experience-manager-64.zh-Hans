...
这提供了一个段落系统，允许您在响应式网格内放置组件。 此网格可根据设备/窗口大小和格式重新排列布局。 此组件可与&#x200B;[**布局**&#x200B;模式](/help/sites-authoring/responsive-layout.md#defining-layouts-layout-mode)配合使用，让您能够根据设备创建和编辑响应式布局。

布局容器：

* 提供水平对齐网格功能，以及将组件并排放置到网格中并定义它们应何时折叠/重排的功能。
* 使用预定义的断点（例如，手机、平板电脑等） 以定义相关设备/方向所需的内容行为。

   * 例如，您可以自定义组件大小，或者是否可以在特定设备上查看组件。

* 可以嵌套以允许列控件。

随后，用户可以使用模拟器查看内容在特定设备的呈现方式。

>[!CAUTION]
>
>尽管布局容器组件在经典UI中可用，但仅在触屏UI中可用并支持其完整功能。
...
使用模拟器时，您需要选择特定的设备来进行模拟和定义布局，并且相关的断点也会突出显示。 您所做的任何布局更改将适用于断点所适用的其他设备，即位于活动断点标记左侧但位于下一个断点标记之前的任何设备。

例如，当您选择设备时 **iPhone 6 Plus** （定义为宽度540像素）用于模拟和布局，断点 **电话** （定义为768像素）也将被激活。 您为 **iPhone 6** 将适用于 **手机** 断点，如 **iPhone 5** （定义为320像素）。
...
1. 此时将打开模拟器工具栏。
   ...
   * **旋转装置**  — 允许您将设备从垂直（纵向）方向旋转到水平（横向）方向，反之亦然。
...
1. 选择特定设备后，您可以：
   ...
   * 蓝色虚线表示 *折叠* (此处为 **iPhone 6**)。

   ![screen_shot_2018-03-23at084947](assets/screen_shot_2018-03-23at084947.png)

   * 折页也可视为分页符(请不要与 [断点](/help/sites-authoring/responsive-layout.md#layout-definitions-device-emulation-and-breakpoints))。 显示此图标是为了方便用户显示在滚动之前用户将在设备上看到的内容部分。
   * 如果所模拟设备的高度高于屏幕大小，则不会显示折页线。
...
>[!NOTE]
>
>如果尚不可用，则 **布局容器** 必须明确 [为段落系统/页面激活](/help/sites-administering/configuring-responsive-layout.md) (例如，使用 [**设计** 模式](/help/sites-authoring/default-components-designmode.md))。
...
>[!CAUTION]
>
>由于布局容器是段落系统，因此删除组件将同时删除布局网格以及容器中保留的所有组件（及其内容）。

1. 如果将鼠标悬停在或点按网格占位符，则会显示操作菜单。
...
1. 如果嵌套了布局组件，请选择 **父项** 选项会显示一个下拉选项，允许您选择嵌套布局容器或其父容器。

   将鼠标悬停在下拉菜单中的容器名称上时，页面上将显示其轮廓。
...
* When [编辑单个组件。](/help/sites-authoring/editing-content.md#edit-component-layout)

   * 通过使用 **布局** 选项，您可以切换到 **布局** 模式。
   * **布局** 模式在编辑组件时持续存在，并会还原为 **编辑** 模式。

在布局模式下，您可以对网格执行各种操作：

* 使用蓝色圆点调整内容组件的大小。 调整大小将始终与网格对齐。 调整大小时，将显示背景网格以帮助进行对齐：
...
* 在&#x200B;**布局**&#x200B;模式下，您可以点按/单击&#x200B;**将组件拖动到此处**&#x200B;来选择整个组件。此时将显示此模式的工具栏。
   ...
   根据布局组件的状态以及属于该布局组件的组件，工具栏将具有不同的选项。 例如：
