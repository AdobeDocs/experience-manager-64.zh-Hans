...
description: 模板可定义生成页面的结构，使用模板编辑器创建和维护模板不再只是开发人员的任务
...
创建页面时，必须选择一个模板，该模板将用作创建新页面的基础。 模板可定义生成页面的结构、任何初始内容以及可使用的组件。

使用模 **板编辑器**，创建和维护模板不再只是开发人员的任务。 高级用户(称为模板作者 **)也可能**。 开发人员仍需要设置环境、创建客户端库和创建要使用的组件，但是，在这些基础知识到位后，模板作者就可以灵活地创建和配置模板，而无需开发项目。 ****

的 **“模板”控制台** 允许模板作者执行以下操作：

* 创建新模板，或复制现有模板。
* 管理模板的生命周期。

的 **模板编辑器** 允许模板作者执行以下操作：

* 将组件添加到模板，并将它们放置在响应式网格上。
* 预先配置组件。
* 定义可以在使用该模板创建的页面上编辑的组件。

本文档介绍 **模板作者** 可以使用“模板”控制台和编辑器创建和管理可编辑的模板。
...
* 根据实例的配置方式，AEM现在提供 [两种基本的模板类型](/help/sites-authoring/templates.md#editable-and-static-templates). 这不会影响实际操作方式 [使用模板创建页面](#using-a-template-to-create-a-page)，但是这会影响您可以创建的模板类型以及页面与其模板之间的关系。
...
由于某些任务（如创建文件夹）的性质，需要开发环境，这需要知识/经验。

本文档中详细列出的任务以及负责执行这些任务的角色。

### 可编辑和静态模板 {#editable-and-static-templates}

AEM现在提供两种基本类型的模板：

* 可编辑模板

   * 可以 [已创建](#creating-a-new-template-template-author) 和 [编辑](#editing-templates-template-authors) 按模板作者使用 **模板** 控制台和编辑器。 的 **模板** 控制台可在 **常规** 部分 **工具** 控制台。
   * 创建新页面后，页面与模板之间将保持动态连接。 这意味着对模板结构和/或锁定内容所做的更改将反映在使用该模板创建的任何页面上。 不会反映对已解锁（即初始）内容所做的更改。
   * 使用内容策略（可从模板编辑器中定义这些策略）来保留设计属性。 页面编辑器中的设计模式不再用于可编辑的模板。

* 静态模板

   * 静态模板已用于多个版本的AEM。
   * 是 [由您的开发人员提供](/help/sites-developing/page-templates-static.md)，因此作者无法创建或编辑这些量度。
   * 将复制以创建新页面，但此后不存在动态连接（尽管已注册模板名称以获取信息）。
...
根据定义，模板控制台和模板编辑器仅允许创建和编辑可编辑的模板。 因此，本文档专门介绍可编辑的模板。

### 使用模板创建页面 {#using-a-template-to-create-a-page}

使用模板 [创建新页面](/help/sites-authoring/managing-pages.md#creating-a-new-page) 静态模板和可编辑模板之间没有明显差异，也没有任何指示。 对于页面作者而言，该过程是透明的。

## 创建和管理模板 {#creating-and-managing-templates}

创建新可编辑的模板时，您可以：

* 使用 **模板** 控制台。 它位于 **常规** 部分 **工具** 控制台。
...
* [定义其他属性](#defining-template-properties-template-author) （如果需要）
* [编辑模板](#editing-templates-template-authors) 要定义：
...
* [启用模板](#enabling-a-template-template-author) 用于创建页面时
* [允许模板](#allowing-a-template-author) 的所需页面或分支
...
>[!CAUTION]
>
>切勿输入任何需要 [国际化](/help/sites-developing/i18n.md) 模板。

### 创建模板文件夹 – 管理员 {#creating-a-template-folder-admin}

您应该为项目创建模板文件夹，以保存特定于项目的模板。这是一项管理员任务，在文档中有相关说明 [页面模板 — 可编辑](/help/sites-developing/page-templates-editable.md#template-folders).
...
1. 打开 **“模板”控制台** (通过 **工具** -> **常规**)，然后导航到所需的文件夹。
   ...
   >
   >建议最好使用 [为项目创建的模板文件夹](/help/sites-developing/page-templates-editable.md#template-folders).
...
1. 选择 **模板类型**，然后选择 **下一个**.
   ...
   >[!NOTE]
   >
   >模板类型是预定义的模板布局，可以视为模板的模板。 这些权限由开发人员或系统管理员预定义。 有关更多信息，请参阅开发人员文档 [页面模板 — 可编辑](/help/sites-developing/page-templates-editable.md#template-type).
...
* 图像
   ...
   * 要用作 [模板缩略图](/help/sites-authoring/templates.md#template-thumbnail-image) 以帮助进行选择，例如在创建页面向导中。
...
可以启用或禁用模板，以使其在 **创建页面** 向导。

>[!CAUTION]
>
>启用模板后，当模板作者开始进一步更新模板时，将显示警告。 这是为了告知用户可能已引用模板，因此任何更改都可能会影响引用模板的页面。
...
* [布局](#editing-a-template-layout-template-author)
   ...
   在此，您可以为所需的设备格式预定义模板布局。 模板创作的&#x200B;**布局**&#x200B;模式与&#x200B;**[页面创作的布局](/help/sites-authoring/responsive-layout.md#defining-layouts-layout-mode)**&#x200B;模式具有相同的功能。
...
>[!CAUTION]
>
>如果作者开始编辑已启用的模板，则会显示警告。 这是为了告知用户可能已引用模板，因此任何更改都可能会影响引用模板的页面。

### 编辑模板 – 结构 – 模板作者 {#editing-a-template-structure-template-author}

在 **结构** 模式，为模板定义组件和内容，并为模板及其组件定义策略。
...
* **添加组件**
   ...
   * 从 **组件** 浏览器。
   * 通过使用 **插入组件** 选项(**+** 图标)，该图标可在模板中现有的组件工具栏上使用，或者 **将组件拖动到此处** 框中。
   * 通过拖动资产(从 **资产** 浏览器)直接置于模板上以就地生成相应的组件。
   ...
   * 组件解锁后显示的标记
   ...
   >[!NOTE]
   >
   >将组件和资产添加到模板的操作与在 [页面创作](/help/sites-authoring/editing-content.md).

* **组件操作**

   将组件添加到模板后，对它们执行操作。 每个实例都有一个允许您访问可用操作的工具栏，该工具栏取决于组件类型。
...
* **指示结构的边框**
   ...
   在中工作时 **结构** 模式橙色边框表示当前选定的组件。 虚线还指示父组件。

   例如，在下面的屏幕截图中， **文本** 在 **布局容器** （响应式网格）。

   ![chlimage_1-364](assets/chlimage_1-364.png)

* **策略和属性（常规）**

   内容（或设计）策略可定义组件的设计属性。 例如，可用组件或最小/最大维度。 这些值适用于模板（以及使用该模板创建的页面）。
   ...
   * 在对话框右侧的&#x200B;**属性**&#x200B;下方，您可以设置特定于组件类型的属性。
   ...
   可用属性取决于选定的组件。 例如，对于文本组件，属性定义了复制和粘贴选项、格式选项以及段落样式等其他选项。
   ...
   **策略说明**&#x200B;字段中的策略说明是可选的。
   ...
   >[!NOTE]
   >
   >如果将同一类型的多个组件添加为初始内容，则同一策略适用于所有这些组件。这反映了 **[设计模式](/help/sites-authoring/default-components-designmode.md)** （对于静态模板）。
   ...
   *主要*
   ...
   如果设置允许进行多个配置，请单击或点按 **添加** 按钮以添加其他配置。
   ...
   *功能*
   ...
   >[!CAUTION]
   >
   >请注意，在 AEM 中，裁剪比例被定义为&#x200B;**高宽比**。这与常规的宽度/高度定义不同，这是出于对旧版兼容性的考虑。 只要您清楚地定义&#x200B;**名称**，页面创作用户便不会察觉到任何差异，因为您定义的名称才是 UI 中显示的内容。
...
* **策略和属性（布局容器）**
   ...
   布局容器的策略和属性设置与常规用法类似，但有一些不同。

   >[!NOTE]
   >
   >对于容器组件，必须配置策略，因为它允许您定义将在容器中可用的组件。

   配置窗口分为两个部分，与窗口的常规用法一样。
   ...
   ***属性***
   ...
   在 **属性** 标题，您可以选择可用于布局容器的组件并定义其设置。 标题包含三个选项卡：
   ...
   *默认组件*
   ...
   在 **默认组件** 选项卡，您可以定义与给定媒体类型自动关联的组件，以便作者从资产浏览器中拖动资产时，AEM知道要将资产与哪个组件关联。 请注意，只有具有拖放区域的组件才可用于此类配置。
...
* **解锁/锁定组件**
   ...
   您可以解锁/锁定组件以定义内容是否可在 **初始内容** 模式。

   解锁组件后：

   * 边框中显示一个打开的挂锁指示器。
   * 组件工具栏将相应地进行调整。
   * 已输入的任何内容将不再显示在 **结构** 模式。
   ...
   * 无法移动、剪切或删除已解锁组件的父组件。
   ...
   为了节省空间，布局容器不会扩大以容纳允许的组件列表。 容器而是变为可滚动列表。
...
**初始内容** 模式用于定义在首次基于模板创建页面时显示的内容。 然后，页面作者可以编辑初始内容。
...
>[!NOTE]
>
>初始内容用于准备组件和页面布局，以作为创建内容的起点。 它并不打算作为实际内容而保持原样。 因此，无法翻译初始内容。
...
1. 的 **页面设计** 对话框打开，分为两个部分：

   * 左半部分定义 [页面策略](/help/sites-authoring/templates.md#page-policies)
...
* 指定要应用于使用此模板创建的页面的客户端库。 在 **客户端库** 中。
...
1. 从模板创建页面后对模板所做的更改会对模板产生影响。

   以下是对模板可能执行的不同操作的列表，以及这些操作对模板创建的页面有何影响：
   ...
   * 对初始内容的更改：
      ...
      * 这些规则仅适用于在对模板进行更改后创建的页面。
   * 布局的更改取决于修改的组件是否属于以下内容的一部分：
   ...
   * 锁定或解锁已启用模板上的组件。
   * 这可能会产生副作用，因为现有页面可能已在使用它。 通常：

      * 解锁组件（已锁定）的组件在现有页面上将丢失。
      * 锁定组件（之前可编辑）将隐藏该内容，使其不会显示在页面上。

   >[!NOTE]
   >
   >AEM在不再为草稿的模板上更改组件的锁定状态时会显式发出警告。

1. [创建您自己的文件夹](#creating-a-template-folder-admin) ，以访问特定于网站的模板。
