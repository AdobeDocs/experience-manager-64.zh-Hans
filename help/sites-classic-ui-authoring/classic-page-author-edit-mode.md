...
* **列表**
   ...
   * 根据您选择的项目，将显示一个新面板：
      ...
      * **搜索选项**
         ...
         * **开始于**  — 手动或使用选择器输入开始路径。搜索查询
可以输入纯文本搜索查询。
      * **高级搜索选项**

         * **QueryBuilder谓词记号**  — 您可以使用QueryBuilder谓词记号输入搜索查询。例如，您可以输入“fulltext=Marketing”，以使内容中具有“Marketing”的所有页面都显示在轮播中。有关查 [询表达](/help/sites-developing/querybuilder-api.md) 式和更多示例的完整讨论，请参阅QueryBuilder API。
      * **标记**  — 指定父 **页面**、标记/ **关键字** 和所需的匹配条件。
   * **显示方式**
...
* 视频
   ...
   * **视频资产**  — 上传或拖放您的视频资产。
   * **大小**  — 视频的本机大小（宽x高，以像素为单位）将显示在“大小”（请参阅上文）旁边的框中。 如果要覆盖视频的本机尺寸，请在此处手动输入宽度和高度尺寸。单击 **** 确定关闭对话框。
