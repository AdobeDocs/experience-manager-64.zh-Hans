...
# AEM与MongoDB{#aem-with-mongodb}

本文旨在提高对成功部署MongoDB的Adobe Experience Manager所需任务和考虑事项的了解。

有关更多部署相关信息，请 [参阅文档的](/help/sites-deploying/deploy.md) “部署和维护”部分。
...
>[!NOTE]
>
>有关作者实例大小调整和并发用户定义的其他信息，请参阅硬件 [大小调整准则](/help/managing/hardware-sizing-guidelines.md#authors-working-in-parallel)。
...
最小部署需要3 `mongod` 个实例配置为复制副本集。 一个实例将被选为主实例，而另一个实例是辅助实例，其选举由管理 `mongod`。 附加到每个实例的是本地磁盘。 为了使群集支持负载，建议使用最低吞吐量12MB/秒，每秒操作数超过3000次(IOPS)。

AEM作者连接到实例， `mongod` 每个AEM作者都连接到所有三个 `mongod` 实例。 写入被发送到主实例，读取可以从任何实例中读取。 流量根据调度程序对任何一个活动AEM作者实例的负载进行分配。 OAK数据存储是一个 `FileDataStore`,MongoDB监控由MMS或MongoDB Ops Manager提供，具体取决于部署位置。 操作系统级别和日志监视由第三方解决方案（如Splunk或Ganglia）提供。

在此部署中，成功实施需要所有组件。 任何缺少的组件都将使实施无法正常工作。

### 操作系统 {#operating-systems}

有关AEM 6支持的操作系统的列表，请参 [阅技术要求页](/help/sites-deploying/technical-requirements.md)。
...
虽然MongoDB 3.0中的WiredTiger存储引擎也存在同样的限制，但工作集、内存和页面故障之间的关联并不像WiredTiger没有像MMAP存储引擎那样使用内存映射那么强。
...
由于MongoDB工作集的限制，强烈建议将数据存储与MongoDB无关地进行维护。 在大多数环境 `FileDataStore` 中，应使用所有AEM实例都可用的NAS。 对于使用AmazonWeb服务的情况，还有一个 `S3 DataStore`。 如果由于任何原因在MongoDB中维护数据存储，则应将数据存储的大小添加到数据库总大小中，并相应地调整工作集计算。 这可能意味着设置大量RAM以保持性能，而不出现页面错误。
...
虽然可以使用命令行工具快速获取群集操作的快照，但在许多主机上实时执行此操作几乎是不可能的。 命令行工具很少提供几分钟以上的历史信息，并且不允许不同类型的度量之间的交叉关联。 短暂的后台同步缓慢 `mongod` 需要大量手动操作来关联I/O等待或从明显未连接的虚拟机向共享存储资源的过多写入级别。
...
* `customBlobStore`
   ...
   如果部署将二进制文件存储在数据库中，它们将构成工作集的一部分。 因此，建议不要在MongoDB中存储二进制文件，向NAS上的数据存储等备用 `FileSystem` 数据存储区提供。

* `cache`

   缓存大小(MB)。 它分布于中使用的各种高速缓存 `DocumentNodeStore`。 默认为256MB。 但是，Oak读取性能将得益于更大的缓存。
...
数据存储用于存储大于阈值的文件。 低于此阈值时，文件将作为属性存储在文档节点存储中。 如果使 `MongoBlobStore` 用，则在MongoDB中创建专用集合以存储blob。 此集合有助于实例的工作 `mongod` 集，并且需要具有更 `mongod` 多内存以避免性能问题。 因此，建议的配置是避免进行生产部 `MongoBlobStore` 署，并使用在所 `FileDataStore` 有AEM实例之间共享的NAS作为备份。 由于操作系统级缓存在管理文件方面非常有效，因此应将磁盘上文件的最小大小设置为接近磁盘的块大小，这样文件系统就可以高效地使用，并且许多小的文档不会过度地对实例的工作集贡 `mongod` 献。
...
* `minRecordLength`
   ...
   大小（字节）. 小于或等于此大小的二进制文件随文档节点存储存储存储。 二进制文件的内容不是存储blob的ID。 对于大于此大小的二进制文件，二进制文件的ID作为文档的属性存储在节点集合中，而二进制文件的正文存储在 `FileDataStore` 磁盘中。 4096字节是典型的文件系统块大小。

* `path`

   数据存储根的路径。 对于MongoMK部署，此部署必须是所有AEM实例都可用的共享文件系统。 通常使用网络连接存储(NAS)服务器。 对于AmazonWeb服务等云部署， `S3DataFileStore` 也提供。

* `cacheSizeInMB`

   二进制缓存的总大小(MB)。 它用于缓存小于设置的二进制 `maxCacheBinarySize` 文件。
...
建议为MongoDB部署启用永久缓存配置，以最大化具有高I/O读取性能的环境的速度。 有关详细信息，请参 [阅Jackrabbit Oak文档](https://jackrabbit.apache.org/oak/docs/nodestore/persistent-cache.html)。

## MongoDB操作系统优化 {#mongodb-operating-system-optimizations}

### 操作系统支持 {#operating-system-support}

MongoDB 2.6使用内存映射存储引擎，该引擎对RAM和磁盘之间操作系统级别管理的某些方面很敏感。 查询和读取MongoDB实例的性能依赖于避免或消除通常称为页面错误的慢速I/O操作。 这些是特别适用于该进 `mongod` 程的页面错误。 它们不应与操作系统级别的页面错误混淆。
...
MongoDB可运行于多种操作系统上，包括各种Linux版本、Windows和Mac OS。 有关其 [他详细](https://docs.mongodb.com/manual/installation/#supported-platforms) 信息，请参阅https://docs.mongodb.com/manual/installation/#supported-platforms。 根据您的操作系统选择，MongoDB有不同的操作系统级别建议。 https://docs.mongodb.com/manual/administration/production-checklist-operations/#operating-system-configuration上有相关文档 [介绍](https://docs.mongodb.com/manual/administration/production-checklist-operations/#operating-system-configuration) ，为了方便起见，请在此处进行总结。

#### Linux {#linux}

* 关闭透明的页面和碎片整理。 有关 [详细信息，请参阅](https://docs.mongodb.com/manual/tutorial/transparent-huge-pages/) “透明大页设置”。
* [调整存储数据库](https://docs.mongodb.com/manual/administration/production-notes/#readahead) 文件的设备上的Readaed设置以适合您的用例。

   * 对于MMAPv1存储引擎，如果工作集大于可用RAM，且文档访问模式是随机的，请考虑将预读数降低到32或16。 评估不同设置，以找到最佳值，最大化驻留内存并减少页面故障数。
   * 对于WiredTiger存储引擎，无论存储介质类型（旋转、SSD等）如何，请将readaew设置为0。 通常，除非测试显示更高的预读值具有可衡量、可重复和可靠的优势，否则请使用建议的预读设置。 [MongoDB专业支持](https://docs.mongodb.com/manual/administration/production-notes/#readahead) ，可以就非零预读配置提供建议和指导。
...
* 禁用NUMA或将vm.zone_recallim_mode设置为0并运行具有节 [点交](https://docs.mongodb.com/manual/administration/production-notes/#readahead) 织的mongod实例。 请参阅： [MongoDB和NUMA硬件](https://docs.mongodb.com/manual/administration/production-notes/#readahead) ，了解更多信息。

* 调整硬件上的上限值以适合您的用例。 如果多 [个mongod](https://docs.mongodb.com/manual/reference/program/mongod/#bin.mongod) 或 [mongos实例在同一用户](https://docs.mongodb.com/manual/reference/program/mongos/#bin.mongos) 下运行，请相应地缩放上限值。 请参阅： [UNIX限制设置](https://docs.mongodb.com/manual/reference/ulimit/) ，以了解详细信息。

* 对dbPath装 [入点使](https://docs.mongodb.com/manual/reference/configuration-options/#storage.dbPath) 用noatime。
...
写入磁盘时，WiredTiger将所有数据以一致的方式写入快照中的所有数据到磁盘，并跨所有数据文件。 现在持久 [的数据](https://docs.mongodb.com/manual/reference/glossary/#term-durable) ，充当数据文件中的检查点。 检查点确保数据文件在最后一个检查点之前的一致性； 即检查点可以充当恢复点。
...
使用WiredTiger，即使没有 [日志](https://docs.mongodb.com/manual/reference/glossary/#term-durable),MongoDB也可以从最后一个检查点恢复； 但是，要恢复在上一个检查点之后所做的更改，请使用日 [志运行](https://docs.mongodb.com/manual/core/wiredtiger/#storage-wiredtiger-journal)。

#### 日志 {#journal}

WiredTiger结合检查点使用预写事务日志 [](https://docs.mongodb.com/manual/core/wiredtiger/#storage-wiredtiger-checkpoints) ，确保数据耐用。

WiredTiger日志在检查点之间保留所有数据修改。 如果MongoDB在检查点之间退出，则它使用日志重播自上次检查点以来修改的所有数据。 有关MongoDB将日志数据写入磁盘的频率的信息，请参阅日 [志处理](https://docs.mongodb.com/manual/core/journaling/#journal-process)。

WiredTiger日志是使用快 [照压缩](https://docs.mongodb.com/manual/core/journaling/#journal-process) 库压缩的。 要指定替代压缩算法或无压缩，请使 [用存储.wiredTiger.engineConfig.journalCompressor设置](https://docs.mongodb.com/manual/reference/configuration-options/#storage.wiredTiger.engineConfig.journalCompressor) 。

有关详细信息，请参阅： [跟WiredTiger联系](https://docs.mongodb.com/manual/core/journaling/#journaling-wiredtiger)。

>[!NOTE]
>
>WiredTiger的最小日志记录大小为128字节。 如果日志记录为128字节或更小，则WiredTiger不压缩该记录。
>
>您可以通过将 [存储.日志](https://docs.mongodb.com/manual/reference/configuration-options/#storage.journal.enabled) .enabled设置为false来禁用日志，这可以减少维护日志的开销。
>
>对于 [独立实例](https://docs.mongodb.com/manual/reference/glossary/#term-standalone) ，不使用日志意味着当MongoDB意外退出检查点时，您将丢失一些数据修改。 对于副本集 [的成员](https://docs.mongodb.com/manual/reference/glossary/#term-replica-set)，复制过程可以提供足够的耐用性保证。

#### 压缩 {#compression}

借助WiredTiger,MongoDB支持对所有集合和索引进行压缩。 压缩可以最大限度地减少存储的使用，而不需要额外的CPU。

默认情况下，WiredTiger对所有集合使用 [snappy](https://docs.mongodb.com/manual/reference/glossary/#term-snappy) compression库对块压缩，对所 [有索引使用](https://docs.mongodb.com/manual/reference/glossary/#term-prefix-compression) 前缀压缩。

对于集合，还提供 [带zlib](https://docs.mongodb.com/manual/reference/glossary/#term-zlib) 的块压缩。 要指定替代压缩算法或无压缩，请 [使用存储.wiredTiger.collectionConfig.blockCompressor设置](https://docs.mongodb.com/manual/reference/glossary/#term-zlib) 。

对于索引，要禁 [用前缀压缩](https://docs.mongodb.com/manual/reference/glossary/#term-prefix-compression)，请使 [用存储.wiredTiger.indexConfig.prefixCompression设置](https://docs.mongodb.com/manual/reference/configuration-options/#storage.wiredTiger.indexConfig.prefixCompression) 。

在创建集合和索引期间，还可以按每个集合和每个索引配置压缩设置。 请参 [阅指定存储](https://docs.mongodb.com/manual/reference/method/db.createCollection/#create-collection-storage-engine-options)[引擎选项和db.collection.createIndex()storageEngine选项](https://docs.mongodb.com/manual/reference/method/db.collection.createIndex/#createindex-options) 。

对于大多数工作负载，默认的压缩设置平衡了存储效率和处理要求。

默认情况下，WiredTiger日志也会被压缩。 有关日志压缩的信息，请参 [阅日志](https://docs.mongodb.com/manual/core/wiredtiger/#storage-wiredtiger-journal)。
...
要调整WiredTiger内部缓存的大小，请参 [阅存储.wiredTiger.engineConfig.cacheSize](https://docs.mongodb.com/manual/reference/configuration-options/#storage.wiredTiger.engineConfig.cacheSizeGB) GB [和wiredTigerCacheSizeGB](https://docs.mongodb.com/manual/reference/program/mongod/#cmdoption-wiredtigercachesizegb)。 避免将WiredTiger内部缓存大小增加到其默认值以上。

### NUMA {#numa}

非均匀内存访问(NUMA)允许内核管理如何将内存映射到处理器核心。 尽管NUMA试图加快内存访问速度，确保内核能够访问所需数据，但MMAP会干扰MMAP引入额外的延迟，因为读数无法预测。 因此，在具有该功能的所有操作系 `mongod` 统上，都需要禁用NUMA。
...
在许多将计算机视为多核统一内存体系结构的数据库中，这会导致初始CPU首先满足，而次CPU稍后填充，尤其是当中央线程负责分配内存缓冲区时。 解决方案是更改用于开始进程的主线程的NUMA策 `mongod` 略。
...
此策略在所有CPU节点上以循环方式分配内存，确保在所有节点上均匀分配。 它不会像在具有多CPU硬件的系统中那样生成对内存的最高性能访问。 大约一半的内存操作会比较慢，而且在总线上也比较慢，但 `mongod` 是没有以最佳方式写入目标NUMA，因此这是一个合理的折衷。

### NUMA问题 {#numa-issues}

如果 `mongod` 进程从文件夹以外的位置 `/etc/init.d` 启动，则很可能不会使用正确的NUMA策略启动。 根据默认策略的不同，可能会出现问题。 这是因为MongoDB的各种Linux包管理器安装程序还安装了一个包含配置文件的服务，这些配置文件位 `/etc/init.d` 于其中执行上述步骤。 如果直接从存档()安装和运 `.tar.gz`行MongoDB，则需要在该过程中手动运行 `numactl` Mongod。
...
某些策略可能导致向该进程提供的可用RAM少于所 `mongod` 有。 与MySQL不同，MongoDB主动避免操作系统级别分页，因此进程可 `mongod` 能获得的可用内存更少。
...
#### 数据库磁盘的推荐设置 {#recommended-settings-for-database-disks}
...
Linux允许通过命令对资源分配进行可配置的 `ulimit` 控制。 这可以由用户完成，也可以按每个流程完成。

建议根据MongoDB建议的ulimit设置为mongod进 [程配置ulimit](https://docs.mongodb.org/manual/reference/ulimit/#recommended-ulimit-settings)。

#### 测试MongoDB I/O性能 {#test-mongodb-i-o-performance}

MongoDB提供一种名为 `mongoperf` 的工具，用于测试I/O性能。 建议您使用它测试构成基础结构的所有MongoDB实例的性能。

有关如何使用的信 `mongoperf`息，请视图 [MongoDB文档](https://docs.mongodb.org/manual/reference/program/mongoperf/)。

>[!NOTE]
>
>请注 `mongoperf` 意，它设计为在其上运行的平台上的MongoDB性能指标。 因此，不应将结果视为生产系统性能的确定结果。
>
>要获得更准确的性能结果，您可以使用Linux工具运行 `fio` 补充测试。

**在构成部署的虚拟机上测试读取性能**

安装该工具后，切换到MongoDB数据库目录以运行测试。 然后，使用此配置运行开始 `mongoperf`第一个测试：
...
通过设置参数，再次运行测试，此时使用内存映射文 `mmf:true` 件：
...
1. 使用存储I/O控件为进程分配足够的I/ `mongod` O。
1. 通过设置CPU保留来保证承载MongoDB的计算机的 [CPU资源](https://pubs.vmware.com/vsphere-4-esx-vcenter/index.jsp?topic=/com.vmware.vsphere.vmadmin.doc_41/vsp_vm_guide/configuring_virtual_machines/t_allocate_cpu_resources.html)

1. 考虑使用ParaVirtual I/O驱动程序。 有关如何执行此操作的详细信息，请查看此知识 [库文章](https://kb.vmware.com/selfservice/microsites/search.do?language=en_US&amp;cmd=displayKC&amp;externalId=1010398)。

### AmazonWeb服务 {#amazon-web-services}

有关如何使用AmazonWeb服务设置MongoDB的文档，请查看以下AWS上的 [MongoDB指南](https://docs.aws.amazon.com/quickstart/latest/mongodb/welcome.html)。

## 部署前保护MongoDB {#securing-mongodb-before-deployment}

请参阅此 [文章，安全部署MongoDB](https://blogs.adobe.com/security/2015/07/securely-deploying-mongodb-3-0.html) ，以获得有关如何在部署之前保护数据库配置的建议。

## Dispatcher {#dispatcher}

### 为调度程序选择操作系统 {#choosing-the-operating-system-for-the-dispatcher}

为了正确服务您的MongoDB部署，将托管调度程序的操作系统必须运行 **Apache httpd版本2.4或更高版本。**
...
有关如 [何配置Dispatcher](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher.html) 的更多信息，请查阅Dispatcher文档。
...
建议为发往AEM实例的所有内层路由请求启用粘性连接，从而鼓励后续请求访问同一AEM实例。 这将有助于最小化在实例之间更新内容时会注意到的延迟。
...
因此，建议向调度程序所 `no-sniff` 服务的资源添加标头。 但是，调度程序不缓存标头。 这意味着，从本地文件系统提供的任何内容都将由其扩展确定其内容类型，而不是使用其AEM服务器的来源的原始内容类型头。
...
>[!NOTE]
>
>有关此操作方式的详细信息，请参 [阅内容安全策略上的OWASP页](https://www.owasp.org/index.php/Content_Security_Policy)。

### 大小调整 {#sizing}

有关调整大小的详细信息，请参阅硬 [件调整大小指南](/help/managing/hardware-sizing-guidelines.md)。
