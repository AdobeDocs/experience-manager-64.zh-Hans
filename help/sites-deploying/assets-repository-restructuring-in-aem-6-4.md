...
如AEM 6.4](/help/sites-deploying/repository-restructuring.md)中的父级[存储库重组页所述，升级到AEM 6.4的客户应使用此页评估与影响AEM Assets解决方案的存储库更改相关的工作。 某些更改需要在AEM 6.4升级过程中进行工作，而其他更改可推迟到6.5升级。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>如果任何自定义代码依赖于此位置(即 代码显式依赖于此路径)，则必须更新代码才能在升级前使用新位置；理想情况下，使用Java API来减少对JCR中任何特定路径的依赖关系。</p> <p>存放zip文件以便客户端下载的临时位置。 由于客户端请求下载资产，因此无需进行更新。 它将在新位置生成文件。</p> </td> 
  </tr> 
  <tr> 
   ...
   <td><p>如果客户修改了电子邮件模板，请执行以下操作以与新的存储库结构保持一致：</p> 
    <ol> 
     ...
     <li>删除文件夹：<strong><code>/etc/dam/notification/email/default</code></strong>。<br /> 
      <ol> 
       <li>如果未对<strong> <code>/etc/notification/email/default</code></strong>下的电子邮件模板进行任何更新，则可以删除该文件夹，因为作为AEM 6.4安装的一部分，<strong><code>/libs/settings/notification/email/default</code></strong>下存在原始电子邮件模板。</li> 
      </ol> </li> 
    </ol> </td> 
  </tr> 
  <tr> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><strong>重组指导</strong></td> 
   <td><p>对于在SCM中管理、而不是在运行时通过设计对话框写入的任何设计，请执行以下操作以与最新模型对齐：</p> 
    <ol> 
    ...
    </ol> <p>对于任何未在SCM中管理的设计，以及通过设计对话框修改的运行时，请勿将可创作设计移出<code>/etc</code>。</p> </td> 
  </tr> 
  <tr> 
   <td><strong>注释</strong></td> 
   <td>不适用<br /> </td> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><strong>重组指导</strong></td> 
   <td><p>如果电子邮件模板（<strong>downloadasset</strong>或<strong>transintworkflowcompleted</strong>）已修改，请按照以下步骤与新结构对齐：</p> 
    <ol> 
     ...
     <li>删除文件夹：<code>/etc/dam/workflow/notification/email/downloadasset </code>移动电子邮件模板后。<br /> 
      <ol> 
       <li>如果未对<strong> <code>/etc</code></strong>下的电子邮件模板进行任何更新，则可以删除该文件夹，因为作为AEM 6.4安装的一部分，<strong><code>/libs/settings/dam/workflownotification/email/downloadasset</code></strong>下存在原始电子邮件模板。</li> 
      </ol> </li> 
    </ol> </td> 
  </tr> 
  <tr> 
   <td><strong>注释</strong></td> 
   <td>虽然在技术上支持<code>/conf/global/settings/dam/workflownotification/email/downloadasset</code>查找(优先于/apps（通过常见的Sling CAConfig查找，但在<code>/etc</code>之后），模板可以放置在<code>/conf/global/settings/dam/workflownotification/email/downloadasset</code>中。 但是，不建议这样做，因为没有运行时UI来方便电子邮件模板的编辑。</td> 
  </tr> 
 </tbody> 
</table>
   ...
   <td><p>如果客户修改了电子邮件模板，则要与新的存储库结构保持一致：</p> 
    <ol> 
     ...
     <li>删除文件夹：<strong><code>/etc/dam/adhocassetshare</code></strong>。<br /> 
      <ol> 
       <li>如果未对<strong> <code>/etc</code></strong>下的电子邮件模板进行任何更新，则可以删除该文件夹，因为作为AEM 6.4安装的一部分，<strong><code>/libs/settings/dam/adhocassetshare</code></strong>下存在原始电子邮件模板。</li> 
      </ol> </li> 
    </ol> </td> 
  </tr> 
  <tr> 
   <td><strong>注释</strong></td> 
   <td>虽然在技术上支持查找<code>/conf/global/settings/dam/adhocassetshare</code>（它优先于<code>/apps</code>（通过通常的Sling CAConfig查找），但优先于<code>/etc</code>），模板可以放置在<code>/conf/global/settings/dam/adhocassetshare</code>中。 但是，不建议这样做，因为没有运行时UI来便于电子邮件模板的编辑</td> 
  </tr> 
 </tbody> 
</table>
   ...
   <td><p>要与新存储库结构对齐：</p> 
    <ol> 
     <li>将所有自定义或修改的脚本从<strong><code>/etc/dam/indesign/scripts</code></strong>复制到<strong><code>/apps/settings/dam/indesign/scripts</code></strong><br /> 
      <ol> 
       <li>只有作为AEM提供的未修改脚本复制新的或修改的脚本才能通过AEM 6.4中的<strong><code>/libs/settings</code></strong>使用</li> 
      </ol> </li> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><strong>重组指导</strong></td> 
   <td><p>项目级别自定义需要剪切并粘贴到相应的<code>/apps</code>或<code>/conf</code>路径下。</p> <p>要与AEM 6.4存储库结构对齐：</p> 
    <ol> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><strong>重组指导</strong></td> 
   <td><p>对于开箱即用的查看器预设，查看器预设将仅在新位置中可用。</p> <p>对于“自定义查看器”预设：</p> 
    <ul> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><strong>重组指导</strong></td> 
   <td><p>使用<code>/etc.clientlibs/</code>允许代理前缀，调整任何引用以指向<code>/libs</code>下的新资源。</p> <p>最后，通过从中删除迁移的客户端的文件夹来清理 <code>/etc/clientlibs/foundation/</code></p> </td> 
  </tr> 
  <tr> 
   <td><strong>注释</strong></td> 
   <td>不适用<br /> </td> 
