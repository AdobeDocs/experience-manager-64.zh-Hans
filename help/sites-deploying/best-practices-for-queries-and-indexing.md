...
与在AEM 6公司对Oak公司的过渡一样，对查询和索引的管理方式进行了一些重大改变。 在Jackrabbit 2下，默认情况下所有内容都已编制索引，并可以自由查询。 在Oak中，必须在节点下手动创建 `oak:index` 索引。 查询可以在没有索引的情况下执行，但对于大数据集，执行速度会非常慢，甚至会中止。

本文将概述何时以及何时不需要索引、避免在不需要查询时使用索引的技巧以及优化索引和查询以尽可能优化其性能的技巧。

此外，请务必阅读Oak文 [档，介绍如何编写查询和索引](/help/sites-deploying/queries-and-indexing.md)。 除了索引是AEM 6中的一个新概念之外，Oak查询中还存在语法差异，在从以前的AEM安装迁移代码时，需要考虑这些差异。
...
此外，在设计分类时，务必考虑排序是否重要。 如果不需要显式排序并且需要大量同级节点，则最好使用未排序的节点类型，如 `sling:Folder` 或 `oak:Unstructured`。 如果需要订购， `nt:unstructured` 则 `sling:OrderedFolder` 更合适。

### 查询 {#queries-in-components}

由于查询可能是在AEM系统上执行的较繁琐的操作之一，因此最好在组件中避免使用它们。 每次呈现页面时执行多个查询通常会降低系统性能。 呈现组件时，有两种策略可用于避免执行查询: **遍历节点** ，并 **预取结果**。
...
可以 `/content/myUnstructuredContent/parentCategory/childCategory` 简单地检索节点，可以分析其子节点并使用其呈现组件。
...
运行未使用索引的查询时，将记录有关节点遍历的警告。 如果这是将经常运行的查询，则应创建索引。 要确定给定查询正在使用哪个索引，建 [议使用“解释](/help/sites-administering/operations-dashboard.md#explain-query) ”查询工具。 有关详细信息，可以为相关搜索API启用DEBUG日志记录。
...
与任何查询语言一样，优化查询的第一步是了解如何执行该语言。 要启用此活动，您可以使 [用“解释查询](/help/sites-administering/operations-dashboard.md#explain-query) ”工具，它是“操作”仪表板的一部分。 使用此工具，可插入并解释查询。 如果查询将导致大型存储库以及执行时间和将使用的索引出现问题，将显示警告。 该工具还可以加载一列表慢速和流行的查询，然后可以解释和优化这些数据。

### 查询的调试记录 {#debug-logging-for-queries}

要获取有关Oak如何选择要使用的索引以及查询引擎如何实际执行查询的其他信息，可 **以为以下** 包添加DEBUG日志配置：
...
登录JMX控制台后，请搜索Lucene索引统 **计信息** ，以找到它。 其他索引统计信息可在IndexStats **MBean中找** 到。

有关查询统计信息，请查看名为Oak查询统 **计信息的MBean**。

如果要使用Luke等工具挖掘索引 [](https://code.google.com/p/luke/)，您需要使用Oak控制台将索引从文件系统 `NodeStore` 目录转储到文件系统目录。 有关如何执行此操作的说明，请阅读Lucene [文档](https://jackrabbit.apache.org/oak/docs/query/lucene.html)。
...
设置低阈 `oak.queryLimitInMemory` 值(例如 10000)和橡木。 `queryLimitReads` (例如 5000)，并在点击UnsupportedOperationException时优化昂贵的查询，说“查询读取的节点数超过x节点……”

这有助于避免资源密集型查询(即 不受任何索引或覆盖索引较少支持)。 例如，读取100万个节点的查询会导致I/O的增加，并对应用程序的整体性能产生负面影响。 任何因超出限制而失败的查询应进行分析和优化。

#### **部署后** {#post-deployment}

* 监视日志，以发现触发大节点遍历或大堆内存消耗的查询: &quot;
...
创建或优化索引时要问的第一个问题是，是否确实需要索引才能满足特定情况。 如果您只要通过批处理在非高峰时间运行有关查询一次或仅偶尔运行，则最好不要创建索引。

创建索引后，每次更新索引数据时，索引也必须更新。 由于这会对系统产生性能影响，因此仅应在实际需要索引时才创建索引。

此外，只有在索引中包含的数据足够唯一以保证其安全时，索引才有用。 请考虑书籍中的索引及其所涵盖的主题。 在为文本中的一组主题建立索引时，通常会有成百上千的条目，这样您就可以快速跳转到页面的子集，以快速找到您要查找的信息。 如果该索引只有两三个条目，每个条目指向几百个页面，则该索引将不太有用。 这一概念同样适用于数据库索引。 如果只有两个唯一值，则索引将不会很有用。 话虽如此，一个指数也变得太大而无用。 要查看索引统计信息，请参阅上 [面的索引统](/help/sites-deploying/best-practices-for-queries-and-indexing.md#index-statistics) 计信息。
...
可以将Solr索引配置为运行嵌入在AEM服务器上以用于开发环境，也可以将索引卸载到远程实例，以提高生产和分阶环境上的搜索可伸缩性。 虽然卸载搜索可提高可伸缩性，但会引入延迟，因此除非需要，否则不建议这样做。 有关如何配置Solr集成以及如何创建Solr索引的更多信息，请参阅 [Oak查询和索引文档](/help/sites-deploying/queries-and-indexing.md#the-solr-index)。

>[!NOTE]
>
>而采用集成的Solr搜索方法将允许将索引分载到Solr服务器。 如果通过基于Crawler的方法使用Solr服务器的更高级功能，则需要额外的配置工作。 Headwire已经创建了开 [放源连接器](https://www.aemsolrsearch.com/#/) ，以加速这些类型的实现。
...
* 如果查询使用排序，则为sorted属性定义一个显式属性，并为其 `ordered` 设置 `true` 为属性。 这将允许结果在索引中按此顺序排序，并在查询执行时节省代价高昂的排序操作。

* 只将所需内容放入索引中。 添加不需要的功能或属性将导致索引增长并降低性能。
* 在属性索引中，具有唯一属性名称有助于减小索引的大小，但对于Lucene索引，应使用 `nodeTypes` 并 `mixins` 实现内聚索引。 查询特定 `nodeType` 或 `mixin` 比查询更有效 `nt:base`。 使用此方法时，请 `indexRules` 为相关 `nodeTypes` 方法定义。
...
如果远程存 `NodeStore` 储，则可以启用名为 `CopyOnRead` 的选项。 此选项将在读取远程索引时将其写入本地文件系统。 这有助于提高通常针对这些远程索引运行的查询的性能。

可以在OSGi控制台的LuceneIndexProvider服 **务下配置** ，默认情况下从Oak 1.0.13开始启用。

### 删除索引 {#removing-indexes}

删除索引时，始终建议将属性设置为临时禁用索引，并 `type` 执行测 `disabled` 试以确保应用程序在实际删除之前能够正常运行。 请注意，索引在禁用时不会更新，因此，如果重新启用并且可能需要重新编制索引，则它可能没有正确的内容。

在删除TarMK实例上的属性索引后，需要运行压缩以回收正在使用的任何磁盘空间。 对于Lucene索引，实际索引内容位于BlobStore中，因此需要数据存储垃圾收集。

删除MongoDB实例上的索引时，删除成本与索引中的节点数成比例。 由于删除大索引可能会导致问题，建议的方法是使用oak-mongo.js等工具禁用索引并仅在维护窗口 **中删除它**。 请注意，此方法不应用于常规节点内容，因为它可能会导致数据不一致。

>[!NOTE]
>
>有关oak-mongo.js的更多信息，请参 [阅Oak文档的命令行](https://jackrabbit.apache.org/oak/docs/command_line.html) “工具”部分。

## 重新索引 {#re-indexing}

本节概述了重新 **编制** Oak索引的唯一可接受理由。

在下述原因之外，启动Oak索引的重新索引不会 **改变行** 为或解决问题，并且不必增加AEM的负载。
...
>
>* 查询正确
>* 查询解析为预期索引(使用“解 [释查询](/help/sites-administering/operations-dashboard.md#diagnosis-tools)”)
>* 索引过程已完成
...
* 如何验证：
   ...
   * 根据索 `jcr:created` 引的 `jcr:lastModified` 修改时间验证任何缺失节点的或属性
...
* 如何解决：
   ...
   * Oak版本1.6+
      ...
      * 如果现有内容不受更改影响，则只需刷新
         ...
         * [通过设](https://jackrabbit.apache.org/oak/docs/query/lucene.html#stored-index-definition) 置oak:queryIndexDefinition [@refresh]=true刷新lucene索引
      * 否则， [重新为lucene指](#how-to-re-index) 数编制索引
...
如果AEM遇到的问题与下面列出的条件不匹配， **请不要** 重新索引任何索引，因为它不会解决该问题。
...
* 如何解决：
   ...
   * 否则， [重新索引所](#how-to-re-index)*有lucene索引*
   * 注意:
...
* 如何解决：
   ...
   * 删除lucene索引的本地副本
      ...
      1. 重新启动AEM
   * 如果这无法解决问题，且例外情 `AsyncIndexUpdate` 况持续存在：

      1. [重新索引](#how-to-re-index) erring索引
      1. 另请填写 [Adobe支持](https://helpx.adobe.com/support.html) 票证


### 如何重新编制索引 {#how-to-re-index}

>[!NOTE]
>
>在AEM 6.4中， [oak-run.jar是在MongoMK或RDBMK存储库上重新建立索引](/help/sites-deploying/indexing-via-the-oak-run-jar.md#reindexingapproachdecisiontree) ，唯一支持的方法。
...
* 通过PropertyIndexAsyncReindex MBean使用Web控制台异步重新编 **制属性索引** ;
...
* 使 [用oak-run.jar重新索引Lucene属性](/help/sites-deploying/oak-run-indexing-usecases.md#usecase3reindexing) 索引。
...
>[!NOTE]
>
>前一节总结并框定了Apache Oak文档中AEM环境 [下的Oak重新索引](https://jackrabbit.apache.org/oak/docs/query/indexing.html#reindexing) （索引）指南。
...
* 文本预提取将仅有利于Lucene索引的重新索引，而非Oak属性索引，因为属性索引不会从二进制文件中提取文本。
...
在启用二进制提取 **的情况下** ，重新索引现有lucene索引

* 重新索引处理 **存储库中** 的所有候选内容； 当要从中提取全文的二进制文件数量众多或复杂时，执行全文提取的计算负担将增加到AEM上。 文本预提取将文本提取的“计算成本高的工作”移入一个隔离的过程，该过程直接访问AEM数据存储，避免了AEM中的开销和资源争用。

支持在启用二进制 **提取的情况下** ，将新的lucene索引部署到AEM
...
1a。 执 `oak-run.jar --generate` 行以创建预提取其文本的列表节点。

1b。 节点列表(1a)作为CSV文件存储到文件系统

请注意，每次执行时都会遍历整个节点存储区(由oak-run命令中的路径指 `--generate` 定)，并会创 **建一个** 新的CSV文件。 文本预提取 **过程** （步骤1 - 2）的离散执行之间不会重复使用CSV文件。

**将文本预提取到文件系统**

*步骤2(a-c)可在AEM的正常操作期间执行，因为它只与数据存储交互。*

2a。 执 `oak-run.jar --tika` 行以预提在(1b)中生成的CSV文件中枚举的二进制节点的文本
