...
description: 了解如何进行必要的更改，以便迁移到适用于社区的AEM 6.4中的新存储库结构。
seo-description: 了解如何进行必要的更改，以便迁移到适用于社区的AEM 6.4中的新存储库结构。
...
# 6.4{#repository-restructuring-for-aem-communities-in}中AEM Communities的存储库重构

如AEM 6.4](/help/sites-deploying/repository-restructuring.md)中的父级[存储库重组页所述，升级到AEM 6.4的客户应使用此页评估与影响AEM Communities解决方案的存储库更改相关的工作。 某些更改需要在AEM 6.4升级过程中进行工作，而其他更改可推迟到6.5升级。
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td><p>如果要移动到“<code>/apps/settings</code>”下的新路径，则需要手动迁移。 您可以使用Granite Configuration Manager执行迁移。</p> <p>可以通过在“<code>/libs/settings/community/subscriptions</code>”节点上将属性<code>mergeList</code>设置为<code>true</code>并添加一个<code>nt:unstructured</code>子节点来执行迁移。</p> </td> 
  </tr>
  <tr>
...
<table> 
 <tbody>
  ...
  <tr>
   <td><strong>重组指导</strong></td> 
   <td>“延迟迁移”任务可用于清除“社区配置”。<br /> <p>任务将口令从<code>/etc/watchwords</code>移至<code>/conf/global/settings/community/watchwords</code>。</p> <p>如果自定义的口令存储在SCM中，则应将其部署到<code>/apps/settings/...</code>，并且您必须确保不存在优先的覆盖<code>/conf/global/settings/...</code>配置。</p> <p>迁移任务将删除<code>/etc</code>位置。</p> </td> 
  </tr>
  <tr>
   <td><strong>注释</strong></td> 
   <td>不适用<br /> </td> 
...
<table> 
 <tbody>
  ...
  <tr>
   <td><strong>重组指导</strong></td> 
   <td><p>需要手动迁移。</p> <p>如果您的实例已自定义徽章/评分规则，则无法自动将所有规则放在存储桶下。 需要客户输入，您要在其上为网站使用的会议时段（全局或站点特定）。</p> <p>没有可用于为站点配置标记和评分的UI。</p> <p>要与新的存储库结构对齐，请执行以下操作：</p> 
    <ol> 
     ...
     <li>将相关徽章规则和评分规则从<code>/etc/community/*</code>移至在上一步中创建的站点上下文存储段。</li> 
    ...
    <ol> 
     <li>将“上一位置”中的现有配置迁移到“新位置”。
      <ol> 
       ...
       <li>将任何新Facebook云配置从上一个位置复制到<code>/conf/global or /conf/&lt;tenant&gt;</code>下的相应新位置。</li> 
      </ol> </li> 
  ...
  <tr>
   ...
   <td><p>任何新的Pinterest云配置都必须迁移到新位置。</p> 
    <ol> 
     <li>将“上一位置”中的现有配置迁移到“新位置”。
      <ol> 
       <li>通过AEM创作UI在<strong>“工具”&gt;“Cloud Services”&gt;“Pinterest社交登录配置”中手动重新创建新的Pinterest社交登录配置</strong>。<br /> 或</li> 
       <li>将任何新Pinterest云配置从上一个位置复制到<code>/conf/global or /conf/&lt;tenant&gt;</code>下的相应新位置。</li> 
      </ol> </li> 
...
<table> 
 <tbody>
  ...
  <tr>
   <td><strong>重组指导</strong></td> 
   <td><p>要与新的存储库结构对齐，评分规则可以存储在<code>/apps/settings/</code>或/<code>conf/.../settings</code></p> 
    <ol> 
    ...
    <ol> 
     ...
     <li>Communities站点必须设置<code>cq:conf </code>属性属性。
      <ol> 
  ...
  <tr>
   ...
   <td><p>任何新的Twitter云配置都必须迁移到新位置。</p> 
    <ol> 
     <li>将“上一位置”中的现有配置迁移到“新位置”。
      <ol> 
       <li>通过AEM创作UI在<strong>“工具”&gt;“Cloud Services”&gt;“Twitter社交登录配置”</strong>手动重新创建新的Twitter社交登录配置。<br /> 或 <br /> </li> 
     ...
     <li>取消旧版Twitter ConnectCloud Service与任何更新为引用新位置的AEM Communities站点根目录的关联。</li> 
    </ol> </td> 
  </tr>
  <tr>
   <td><strong>注释</strong></td> 
