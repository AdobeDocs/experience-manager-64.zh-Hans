...
关键问题是网站响应访客请求所花费的时间。 尽管此值会因每个请求而异，但可以定义平均目标值。 一旦这一价值被证明是可实现的和可维护的，它就可以用于监控网站的性能并指示潜在问题的发展。

在创作和发布环境中，您要针对的响应时间会有所不同，反映目标受众的不同特征：
...
>[!NOTE]
>
>* 在进行性能优化配置后，请按照 [Tough Day](/help/sites-developing/tough-day.md) （艰难日）中的步骤测试大负载环境。
...
CQ项目的性能优化方法可以归纳为五个非常简单的规则，这些规则可以遵循，从一开始就避免性能问题：
...
如果可能，首先应将项目软启动给有限的受众，以收集实际体验并进行进一步优化，而不会受到完全宣布后的额外压力。

“实时”后，性能优化不会结束。 这是您体验系统上“真实”负载的时间点。 在启动后计划进行其他调整很重要。

由于系统负载发生变化，并且系统的性能配置文件会随时间发生变化，因此应以6-12个月的间隔安排性能“调整”或“运行状况检查”。
...
性能调整是一个迭代过程，涉及到测量、分析、优化和验证，直到达到目标。 为了正确考虑这一方面，在优化阶段实现灵活的验证过程，而不是每次迭代后重量较大的测试过程。
...
性能（或缺乏性能）是用户首先注意到的问题之一，因此，与具有用户界面的任何应用程序一样，性能是关键的。 要优化CQ安装的性能，您需要监视实例的各种属性及其行为。
...
一个基本的出发点是，当系统正常运行时，您对系统有很好的了解。 除非您知道环境在正常运行时的“外观”和“行为”，否则在性能恶化时很难找到问题所在。 这意味着，在系统顺利运行时，您应该花一些时间调查系统，并确保收集性能信息是一项持续的任务。 这将为您提供在性能受到影响时进行比较的基础。
...
限制并发运行的工作流进程的数量，以提高性能。 默认情况下，工作流引擎并行处理的工作流数与Java VM可用的处理器数相同。 当工作流步骤需要大量处理资源（RAM或CPU）时，并行运行其中几个工作流可能会对可用服务器资源产生高需求。

例如，当上传图像（或通常为DAM资产）时，工作流会自动将图像导入DAM。 图像通常是高分辨率的，并且处理时可轻松消耗数百MB的堆。 并行处理这些图像会给存储子系统和垃圾收集器带来高负载。

工作流引擎使用Apache Sling作业队列来处理和调度工作项处理。 默认情况下，已从Apache Sling作业队列配置服务工厂创建以下作业队列服务以处理工作流作业：

* Granite工作流队列：大多数工作流步骤（如处理DAM资产的步骤）都使用Granite工作流队列服务。
* Granite工作流外部进程作业队列：此服务用于特殊的外部工作流步骤，通常用于联系外部系统和轮询结果。 例如，InDesign媒体提取过程步骤作为外部过程来实现。 工作流引擎使用外部队列来处理轮询。 (请参 [阅com.day.cq.workflow.exec.WorkflowExternalProcess](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/workflow/exec/WorkflowExternalProcess.html)。)
...
如果您使用sling:OsgiConfig [节点配置服务](/help/sites-deploying/configuring-osgi.md#adding-a-new-configuration-to-the-repository)，则需要查找现有服务的PID，例如：org.apache.sling.event.jobs.QueueConfiguration.370aad73-d01b-4a0b-abe4-20198d85f705。 您可以使用Web控制台发现PID。
...
1. 执行要为其创建作业队列的工作流模型，以便生成主题统计信息。 例如，向资产中添加一个图像以执行DAM更新资产工作流。
...
本节旨在对与定义测试概念相关的问题进行标准化概述，这些问题专门针对发布环境中的性能 *测试* 。 这主要是QA工程师、项目经理和系统管理员感兴趣的问题。

下面介绍了在 *Publish环境中对CQ应用程序进行性能测试的标准化方* 法。 这涉及以下5个阶段：
...
您应清楚记录用于性能测试的测试环境的架构。

您需要复制计划内的生产发布环境，以及调度程序和负载平衡器。

#### 应用程序映射 {#application-map}

要获得清晰的概述，您可以创建整个应用程序的映射（您很可能在创作环境的测试中获得该映射）。

以图表形式表示应用程序内部元素，可以给出测试要求的概述；使用颜色编码，它还可以作为报告的基础。
...
* 每个组件在性能方面都有特定的断点。 这意味着组件可以显示良好的性能，直到达到特定点，之后性能会迅速下降。
...
组合测试组件可以更仔细地反映应用程序行为。 同样，必须测试平均和峰值条件。
...
需要不断的报告，以便让所有人了解情况；正如之前通过颜色编码提到的，架构图可用于此。
...
Dispatcher提供了许多内置机制，在您的网站利用这些机制时，您可以使用这些机制来优化性能。 本节将告诉您如何设计网站以最大化缓存的益处。
...
[粘性连接](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher.html#the-benefits-of-load-balancing) ，可确保一个用户的文档都在同一台服务器上完成。 如果用户离开此文件夹，稍后又返回到该文件夹，则连接仍然保持。 定义一个文件夹，以保存所有需要网站粘性连接的文档。 请尽量不要在其中包含其他文档。 如果使用个性化的页面和会话数据，这会影响负载平衡。
...
本节介绍一系列基准测试，用于评估CQ备份的性能以及备份活动对应用程序性能的影响。 CQ备份在运行时对系统造成很大负载，我们会测量这一情况，以及尝试调制这些效果的备份延迟设置的影响。 其目标是提供有关实际配置和生产数据数量中备份的预期性能的一些参考数据，并就如何估计计划系统的备份时间提供指导。

### 参考环境 {#reference-environment}

#### 物理系统 {#physical-system}

本文中报告的结果是从参考环境中运行的基准中得到的，其配置如下。 此配置设计为类似于数据中心中的典型生产环境：
...
此服务器上的磁盘子系统速度相当快，它代表了可能在生产服务器中使用的高性能RAID配置。 备份性能对磁盘性能可能很敏感，而此环境中的结果反映了在非常快速的RAID配置上的性能。 VMWare映像配置为在RAID阵列上具有物理驻留在本地磁盘存储中的单个大磁盘卷。

CQ配置将存储库和数据存储放在同一逻辑卷上，并与所有操作系统和CQ软件一起放置。 备份的目标目录也位于此逻辑文件系统中。

#### 数据卷 {#data-volumes}

下表说明了备份基准中使用的数据卷的大小。 首先安装初始基准内容，然后添加额外的已知数据量以增加备份内容的大小。 将以特定增量创建备份，以代表内容和一天内可能产生的内容大幅增加。 内容（页面、图像、标记）的分发大致基于真实的制作资产组合。 页面、图像和标记最多将限制为800个子页面。 每个页面将包括标题、Flash、文本／图像、视频、幻灯片、表单、表、云和轮盘组件。 图像将从400个大小从37 kB到594 kB的唯一文件池上传。
...
**执行空闲状态** “备份”时，CQ上没有其他活动。

**在“Load** Backups”（负载备份）下，当系统从联机进程中负载不足80%时执行备份。 备份延迟会随负载变化而变化。

从CQ服务器日志中获取备份时间和生成备份的大小。 通常建议在CQ空闲时（例如在半夜）为非时间时间安排备份。 此方案代表了建议的方法。

加载将包括页面创建／删除、遍历和查询，其中大部分负载来自页面遍历和查询。 添加和删除过多页面会不断增加工作区大小并阻止备份完成。 脚本将使用的加载分布为75%的页面遍历、24%的查询和1%的页面创建（单个级别没有嵌套子页面）。 空闲系统上每秒的峰值平均事务量是通过4个并发线程实现的，这是在负载下测试备份时使用的。
...
当前备份的应用程序吞吐量的实际影响取决于应用程序和基础架构的详细信息。 延时值的选择应通过应用程序的经验分析，但应尽可能小，以便尽可能快地完成备份。 由于选择延迟值与对应用程序吞吐量的影响之间只存在微弱的相关性，因此选择延迟应有利于缩短总体备份时间，以将备份的总体影响降至最低。 完成备份需要8小时，但吞吐量影响为-20%，与完成备份需要2小时相比，备份的总体影响可能更大，但吞吐量影响为-30%。
