...
本节介绍安装独立AEM实例时可用的选项。 您还可以阅读 [存储元素](/help/sites-deploying/storage-elements-in-aem-6.md) ，以了解有关在刚刚安装AEM 6之后选择后端存储类型的更多信息。

## 重命名文件以更改端口号 {#changing-the-port-number-by-renaming-the-file}
...
* 重命名文件时，它必须与中 `cq;` 开始 `cq5-publish-p4503.jar`。

* 建议您始终在端 *口号* 前添加-p前缀； 如cq5-publish-p4503.jar或cq5-author-p6754.jar中所示。
...
>[!NOTE]
>
>您还可以使用开始命令中的选 `-port` 项更改端口号。

## 运行模式 {#run-modes}

**运行模式** ，允许您针对特定目的调整AEM实例； 例如，创作或发布、测试、开发、内部网等。 这些模式还允许您控制示例内容的使用。 此示例内容是在构建快速启动之前定义的，可以包括包、配置等。 当您希望保持安装精简且无需示例内容时，此功能对于生产就绪型安装尤为有用。 有关详细信息，请参阅：

* [运行模式](/help/sites-deploying/configure-runmodes.md)

## 添加文件安装提供程序 {#adding-a-file-install-provider}

默认情况下，文件 `crx-quickstart/install` 夹会被监视。\
此文件夹不存在，但只能在运行时创建。
...
* 在快 `crx-quickstart/install` 速启动之前，可以创建文件夹，还可以放置其他包。

>[!NOTE]
>
>另请参 [阅如何在服务器启动时自动安装CRX包](https://helpx.adobe.com/experience-manager/kb/HowToInstallPackagesUsingRepositoryInstall.html) ，以获取示例。

## 将Adobe Experience Manager作为Windows服务进行安装和启动 {#installing-and-starting-adobe-experience-manager-as-a-windows-service}

>[!NOTE]
>
>请确保在以管理员身份登录时执行以下过程，或者使用以管理员身份运行上下文菜 **单选择来开始** /运行这些步骤。
>
>以具有管理员权限的用户身份登录 **是不够的**。 如果您在完成这些步骤时未以管理员身份登录，则会收到“拒绝 **访问** ”错误。
...
1. 要防止该进程进入多个进程，请增加最大堆大小和PermGen JVM参数。 找到该 `set jvm_options` 命令并按如下方式设置值：
...
1. 打开命令提示符，将当前目录更改为AEM安装的crx-quickstart/opt/helpers文件夹，然后输入以下命令以创建服务：
   ...
   要验证是否已创建服务，请在“管理工具”控制面板中打开服务，或在“命令提 `start services.msc` 示符”中键入。 cq5服务显示在列表中。
...
1. Windows指示服务正在运行。 AEM开始和prunsrv可执行文件显示在任务管理器中。 在Web浏览器中，导航到AEM，例如，使用 `http://localhost:4502` AEM开始。
...
>[!NOTE]
>
>在安装AEM作为服务时，必须从Configuration Manager中为中的日志目录提供 `com.adobe.xmp.worker.files.ncomm.XMPFilesNComm` 绝对路径。

要卸载服务，请在“ **服务** ”控制 **面板或命** 令行中 `instsrv.bat -uninstall cq5`，单击“停止”，导览至文件夹并键入。 当您键入时，服务会从“服 **务** ”控制面板的列表或命令行的列表中删除 `net start`。

## 重新定义临时工作目录的位置 {#redefining-the-location-of-the-temporary-work-directory}

Java计算机临时文件夹的默认位置为 `/tmp`。 AEM也使用此文件夹，例如在构建包时。

如果要更改临时文件夹的位置（例如，如果您需要具有更多可用空间的目录），请通过添 `<new-tmp-path>` 加JVM参数来定义：
...
## 在AmazonEC2环境中安装AEM {#installing-aem-in-the-amazon-ec-environment}

在AmazonElastic Compute Cloud(EC2)实例上安装AEM时，如果在EC2实例上同时安装作者和发布，则按照如何安装AEM实例的过程正确安 [装作者实例](/help/sites-deploying/custom-standalone-install.md); 但是，Publish实例将变为作者。
...
* 查看随AEM WCM一起安装的默认用户和用户组的列表。 检查您是否要对任何其他帐户执行操作——有关更 [多详细信息，请参](/help/sites-administering/security.md) 阅安全和用户管理。
...
要打开CRXDE Lite，您可以从欢 **迎屏** 幕中选择CRXDE Lite，或使用浏览器导航到
...
要访问Adobe CQWeb控制台，您可以从欢 **迎屏幕中选** 择OSGi Console，或使用浏览器导航到
