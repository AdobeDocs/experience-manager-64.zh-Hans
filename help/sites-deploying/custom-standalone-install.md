...
1. Windows指示服务正在运行。 AEM开始和prunsrv可执行文件显示在任务管理器中。 在Web浏览器中，导航到AEM，例如，`http://localhost:4502`使用AEM开始。
...
在AmazonElastic Compute Cloud(EC2)实例上安装AEM时，如果在EC2实例上同时安装作者和发布，则按照如何[安装AEM](/help/sites-deploying/custom-standalone-install.md)实例的过程正确安装作者实例；但是，Publish实例将变为作者。
