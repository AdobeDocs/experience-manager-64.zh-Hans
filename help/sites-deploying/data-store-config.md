...
>[!CAUTION]
>
>Oak的较新版本采用新的命名方案和格式用于OSGi配置文件。 新的命名方案要求将配置文件命名为&#x200B;**.config**，并且新格式要求输入值，并在此](https://sling.apache.org/documentation/development/slingstart.html#default-configuration-format)中说明。[
>
>如果您从旧版Oak升级，请确保先备份`crx-quickstart/install`文件夹。 升级后，将文件夹的内容恢复到升级后的安装，并将配置文件的扩展名从&#x200B;**.cfg**&#x200B;修改为&#x200B;**.config**。
>
>如果您正在阅读本文以准备从&#x200B;**AEM 5.x**&#x200B;安装进行升级，请确保首先查阅[升级](https://docs.adobe.com/content/docs/en/aem/6-0/deploy/upgrade.html)文档。

### 段节点存储{#segment-node-store}

细分节点存储是Adobe在AEM6中实现TarMK的基础。 它使用`org.apache.jackrabbit.oak.segment.SegmentNodeStoreService` PID进行配置。

>[!CAUTION]
>
>段节点存储的PID已从AEM 6的`org.apache.jackrabbit.oak.plugins.segment.SegmentNodeStoreService in previous versions`更改为AEM 6.3的`org.apache.jackrabbit.oak.segment.SegmentNodeStoreService`。请确保进行必要的配置调整以反映此更改。

您可以配置以下选项：

* `repository.home`:存储存储库相关数据的存储库主目录的路径。默认情况下，段文件存储在`crx-quickstart/segmentstore`目录下。
...
>[!NOTE]
>
>要启用自定义数据存储，您需要确保在相应的节点存储配置文件([段节点存储](/help/sites-deploying/data-store-config.md#segment-node-store)或[文档节点存储](/help/sites-deploying/data-store-config.md#document-node-store))中将`customBlobStore`设置为`true`。
...
## AmazonS3数据存储{#amazon-s-data-store}
...
>[!NOTE]
>
>将AEM 6.4与TarMK一起使用时，二进制文件默认存储在`FileDataStore`中。 要将TarMK与S3数据存储一起使用，您需要使用`crx3tar-nofds`运行模式开始AEM，例如：
...
1. 如果AEM已配置为与Tar或MongoDB存储一起使用，请先从`aem-install/crx-quickstart/install`文件夹删除任何现有配置文件，然后再继续操作。 需要删除的文件包括：
...
1. 返回到提取了功能包的临时位置，并复制以下文件夹的内容：
   ...
   * `<aem-install>/crx-quickstart/install`
   ...
   确保仅复制当前配置所需的配置文件。 对于专用数据存储和共享数据存储设置，都复制`org.apache.jackrabbit.oak.plugins.blob.datastore.S3DataStore.config`文件。
...
### 升级到1.8.x S3连接器的新版本{#upgrading-to-a-new-version-of-the-x-s-connector}
...
1. 备份后，删除`<aem-install>/crx-quickstart/install/15`文件夹中的所有jar文件，删除旧版S3 Connector及其依赖项，例如：
...
* accessKey:AWS访问密钥。
* secretKey:AWS秘密访问密钥。 **注意：** 或者， [IAM](https://docs.aws.amazon.com/sdk-for-java/v1/developer-guide/java-dg-roles.html) 角色也可用于身份验证。如果您使用的是IAM角色，则不再需要指定`accessKey`和`secretKey`。
* s3Bucket:存储段名称。
* s3地区：桶区。
...
从旧缓存实现（Oak 1.6之前）升级时，本地文件系统缓存目录的结构存在差异。 在旧缓存结构中，下载的文件和上传的文件都直接放在缓存路径下。 新结构将下载和上传分离，并将它们存储在缓存路径下名为`upload`和`download`的两个目录中。 升级过程应是无缝的，任何挂起的上传应计划上传，并且在初始化时，缓存中之前下载的任何文件都将放入缓存中。
...
在从DataStore访问所请求的文件/blob之前，将检查本地缓存是否有记录。 当缓存在缓存中添加文件时超出配置的限制（请参见`cacheSize`参数）时，将逐出一些文件以回收空间。

**异步上传**

缓存支持异步上传到DataStore。 文件将在本地、缓存中（在文件系统上）和异步作业开始上传文件。 异步上传的数量受暂存缓存大小的限制。 使用`stagingSplitPercentage`参数配置暂存缓存的大小。 此参数定义用于暂存缓存的缓存大小百分比。 此外，可下载的缓存百分比计算为&#x200B;**(100 - `stagingSplitPercentage`)&amp;ast;`cacheSize`**。

异步上传是多线程的，并且使用`uploadThreads`参数配置线程数。

上载完成后，文件将移至主下载缓存。 当暂存缓存大小超出其限制时，文件将同步上传到DataStore，直到以前的异步上传完成，并且暂存缓存中的空间再次可用。 上传的文件由周期性作业从暂存区域删除，该作业的间隔由`stagingPurgeInterval`参数配置。

失败的上载（例如，由于网络中断）将被置于重试队列中并定期重试。 使用`stagingRetryInterval parameter`配置重试间隔。

### 使用AmazonS3 {#configuring-binaryless-replication-with-amazon-s}配置无二进制复制

要使用S3配置无二进制复制，需要执行以下步骤：

1. 安装作者实例和发布实例，并确保它们正确启动。
1. 打开到&#x200B;*http://localhost:4502/etc/replication/agents.author/publish.html*&#x200B;的页面，转到复制代理设置。
1. 按&#x200B;**设置**&#x200B;部分中的&#x200B;**编辑**&#x200B;按钮。
...
1. 首先，在共享数据存储所需的每个实例上创建数据存储配置文件：
   ...
   * 如果您使用`FileDataStore`，请创建一个名为`org.apache.jackrabbit.oak.plugins.blob.datastore.FileDataStore.config`的文件，并将其放在`<aem-install>/crx-quickstart/install`文件夹中。
...
1. 如果实例已从现有服务器中克隆，则需要在存储库脱机时使用最新的oak-run工具删除新实例的`clusterId`。 您需要运行的命令是：
   ...
   >[!NOTE]
   >
   >如果已配置段节点存储，则需要指定存储库路径。 默认情况下，路径为`<aem-install-folder>/crx-quickstart/repository/segmentstore.`如果配置了文档节点存储，则可以使用[ Mongo连接字符串URI](https://docs.mongodb.org/manual/reference/connection-string/)。
...
1. 最后，验证配置。 为此，您需要查找每个共享数据存储库添加到数据存储的唯一文件。 文件的格式为`repository-[UUID]`，其中UUID是每个独立存储库的唯一标识符。
   ...
   * 对于`FileDataStore`，文件创建在数据存储文件夹的根路径下。
   * 对于`S3DataStore`，在配置的S3存储桶中创建文件，位于`META`文件夹下。

## Azure 数据存储 {#azure-data-store}

AEM可以配置为在Microsoft的Azure存储服务中存储数据。 它使用`org.apache.jackrabbit.oak.plugins.blob.datastore.AzureDataStore.config` PID进行配置。

要启用Azure数据存储功能，需要下载并安装包含Azure连接器的功能包。 转至[Adobe库](https://repo.adobe.com/nexus/content/groups/public/com/adobe/granite/com.adobe.granite.oak.azureblobconnector/)，从功能包的1.6.x版本下载最新版本（例如，com.adobe.granite.oak.azureblobconnector-1.6.3.zip）。

>[!NOTE]
>
>将AEM 6.4与TarMK一起使用时，二进制文件默认存储在FileDataStore中。 要将TarMK与Azure DataStore一起使用，您需要使用`crx3tar-nofds`运行模式开始AEM，例如：
...
1. 如果AEM已配置为与Tar或MongoDB存储一起使用，请先从`/crx-quickstart/install`文件夹删除任何现有配置文件，然后再继续操作。 需要删除的文件包括：
...
* accessKey=&quot;&quot;:存储帐户名称。 有关Microsoft Azure身份验证凭据的详细信息，请参阅[官方文档](https://azure.microsoft.com/en-us/documentation/articles/storage-create-storage-account)。

* secretKey=&quot;&quot;:存储访问密钥。 确保“=”字符以“\=”的方式转义。
* 容器=&quot;&quot;:Microsoft Azure blob存储容器名。 容器是一组块的分组。 有关更多详细信息，请阅读[官方文档](https://msdn.microsoft.com/en-us/library/dd135715.aspx)。
...
1. 在共享数据存储的&#x200B;**所有**&#x200B;存储库实例上分别运行[二进制垃圾收集](/help/sites-deploying/data-store-config.md#data-store-garbage-collection)中所述的步骤。 但是，在单击“调用”按钮之前，请确保为`markOnly`参数输入`true`:
