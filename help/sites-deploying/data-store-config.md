...
* `repository.home`:存储各种存储库相关数据的存储库主页的路径。 默认情况下，二进制文件将存储在 `crx-quickstart/repository/datastore` 目录访问Advertising Cloud的帮助。
...
要启用S3数据存储功能，需要下载和安装包含S3数据存储连接器的功能包。 转到 [Adobe存储库](https://repo.adobe.com/nexus/content/groups/public/com/adobe/granite/com.adobe.granite.oak.s3connector/) 并从1.8.x版本的功能包下载最新版本（例如，com.adobe.granite.oak.s3connector-1.8.0.zip）。 此外，您还需要下载并安装上列出的最新AEM Service Pack [AEM 6.4 Service Pack发行说明](https://helpx.adobe.com/cn/experience-manager/6-4/release-notes/feature-packs-release-notes.html) 页面。

>[!NOTE]
>
>将AEM 6.4与TarMK结合使用时，默认情况下，将在 `FileDataStore`. 要将TarMK与S3数据存储结合使用，您需要使用 `crx3tar-nofds` 运行模式，例如：
...
1. 如果AEM已配置为与Tar或MongoDB存储一起使用，请从 `aem-install/crx-quickstart/install` 文件夹。 需要删除的文件包括：
...
如果您需要升级到新版本的1.8.x S3连接器（例如，从1.8.0升级到1.8.1），请执行以下步骤：
...
缓存支持异步上传到DataStore。 文件将在本地缓存中（在文件系统上）存放，异步作业开始上传文件。 异步上传的数量受暂存缓存大小的限制。 暂存缓存的大小通过使用 `stagingSplitPercentage` 参数。 此参数定义用于暂存缓存的缓存大小百分比。 此外，可下载的缓存百分比计算为 **(100 - `stagingSplitPercentage`)&amp;ast;`cacheSize`**.
...
1. 通过打开 *http://localhost:4502/etc/replication/agents.author/publish.html*.
1. 按 **编辑** 按钮 **设置** 中。
...
要启用Azure数据存储功能，需要下载并安装包含Azure连接器的功能包。 转到 [Adobe存储库](https://repo.adobe.com/nexus/content/groups/public/com/adobe/granite/com.adobe.granite.oak.azureblobconnector/) 并从1.6.x版本的功能包下载最新版本（例如，com.adobe.granite.oak.azureblobconnector-1.6.3.zip）。

>[!NOTE]
>
>将AEM 6.4与TarMK结合使用时，默认情况下，二进制文件将存储在FileDataStore中。 要将TarMK与Azure DataStore结合使用，您需要使用 `crx3tar-nofds` 运行模式，例如：
