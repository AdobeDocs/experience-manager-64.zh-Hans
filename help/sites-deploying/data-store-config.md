...
* `db`:Mongo数据库的名称。默认值为&#x200B;**Oak**。 但是，新的AEM 6安装使用&#x200B;**aem-author**&#x200B;作为默认数据库名。
...
1. 如果AEM已配置为使用Tar或MongoDB存储，请在继续操作之前，从`aem-install/crx-quickstart/install`文件夹中删除任何现有配置文件。 需要删除的文件有：
...
### 升级到1.8.x S3连接器{#upgrading-to-a-new-version-of-the-x-s-connector}的新版本
...
缓存支持异步上载到DataStore。 文件将在本地存放在缓存中（在文件系统上），并执行异步作业开始以上传文件。 异步上载的数量受暂存缓存大小的限制。 使用`stagingSplitPercentage`参数配置暂存缓存的大小。 此参数定义用于暂存缓存的缓存大小百分比。 此外，可下载的缓存百分比计算为&#x200B;**(100 - `stagingSplitPercentage`)&amp;ast;`cacheSize`**。
...
>[!NOTE]
>
>将AEM 6.4与TarMK一起使用时，默认情况下，二进制文件将存储在FileDataStore中。 要将TarMK与Azure DataStore一起使用，您需要使用`crx3tar-nofds`运行模式开始AEM，例如：
