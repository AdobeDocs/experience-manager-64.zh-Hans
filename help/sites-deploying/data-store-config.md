...
文档节点存储是AEM MongoMK实现的基础。 它使用 `org.apache.jackrabbit.oak.plugins.document.DocumentNodeStoreService` PID **控制**。 提供以下配置选项：

* `mongouri`: 连接 [到](https://docs.mongodb.org/manual/reference/connection-string/) Mongo数据库所需的MongoURI。 默认为 `mongodb://localhost:27017`

* `db`: Mongo数据库的名称。 默认为 **Oak** 。 但是，新的AEM 6安装 **使用aem** -author作为默认数据库名。
...
为了启用S3数据存储功能，需要下载并安装包含S3数据存储连接器的功能包。 转到 [Adobe库](https://repo.adobe.com/nexus/content/groups/public/com/adobe/granite/com.adobe.granite.oak.s3connector/) ，从1.8.x版的功能包下载最新版本（例如，com.adobe.granite.oak.s3connector-1.8.0.zip）。 此外，您还需要下载并安装AEM 6.4 Service Pack发行说明页 [所列的最新AEM service](https://helpx.adobe.com/experience-manager/6-4/release-notes/feature-packs-release-notes.html) pack。

>[!NOTE]
>
>将AEM 6.4与TarMK一起使用时，二进制文件将默认存储在 `FileDataStore`中。 要将TarMK与S3数据存储一起使用，您需要使用运行模 `crx3tar-nofds` 式开始AEM，例如：
...
1. 如果AEM已配置为与Tar或MongoDB存储一起使用，请先从文件夹删除任何现有配置文件，然 `aem-install/crx-quickstart/install` 后再继续操作。 需要删除的文件包括：
...
缓存支持异步上传到DataStore。 文件将在本地、缓存中（在文件系统上）和异步作业开始上传文件。 异步上传的数量受暂存缓存大小的限制。 使用参数配置暂存缓存的大 `stagingSplitPercentage` 小。 此参数定义用于暂存缓存的缓存大小百分比。 此外，可下载的缓存百分比计算 **为(100 -`stagingSplitPercentage`)&amp;ast;`cacheSize`**.
...
1. 打开页面到http://localhost:4502/etc/replication/agents.author/publish.html，转到复制代理 *设置*。
1. 按“设 **置** ”部分中的“ **编辑** ”按钮。
1. 将“序列化 **类型** ”选项更改 **为“无二进制**”。
...
>[!NOTE]
>
>将AEM 6.4与TarMK一起使用时，二进制文件默认存储在FileDataStore中。 要将TarMK与Azure DataStore一起使用，您需要使用运行模式 `crx3tar-nofds` 开始AEM，例如：
