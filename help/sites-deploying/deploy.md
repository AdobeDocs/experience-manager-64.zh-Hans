...
* **Web应用程序服务器**：AEM能够以独立模式（包括集成的Jetty Web服务器）部署，也可以作为第三方应用程序服务器（WebLogic、WebSphere等）中的Web应用程序部署。
...
最后，客户可以使用这些基础架构和应用程序级别的构建块，通过构建他们自己的应用程序来创建定制的解决方案。
...
您可以在公司环境中的服务器上安装AEM。 典型安装实例包括：开发、测试和发布环境。 请参阅 [快速入门](/help/sites-deploying/deploy.md#getting-started) 部分，了解有关如何获取AEM软件以将其安装到本地的基本详细信息。
...
**更快的上市时间：** 借助Adobe Managed Services的灵活云基础架构，组织可以快速规划、启动和优化成功的数字体验。 Adobe管理云架构，无需额外的资金、硬件或软件，Adobe的客户成功工程师可协助AEM架构、配置、连接到后端应用程序的自定义以及上线最佳实践。
...
* **cq-quickstart-6.4.0.jar：** 独立的可执行文件 *jar* 文件，其中包含启动和运行所需的一切。

* **cq-quickstart-6.4.0.war：** A *战争* 文件，以便在第三方应用程序服务器中部署。
...
1. 要在GUI环境中启动实例，只需双击 **`cq-quickstart-6.4.0.jar`** 文件。
   ...
   或者，您可以从命令行启动AEM。 对于32位Java VM，输入以下内容：
...
如果实例是从GUI安装的，则浏览器窗口将自动打开，桌面应用程序窗口也将打开，其中显示实例的主机和端口以及打开/关闭开关：
