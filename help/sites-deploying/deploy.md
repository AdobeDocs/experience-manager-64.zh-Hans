...
* **Web应用程序服务器**:AEM可以以独立模式部署（它包括集成的Jetty web服务器），也可以作为第三方应用程序服务器（WebLogic、WebSphere等）中的Web应用程序部署。
...
您可以在企业环境中的服务器上安装AEM。 典型安装实例包括：开发、测试和发布环境。 有关如何获取AEM [软件以本地安装](/help/sites-deploying/deploy.md#getting-started) ，请参阅入门部分。
...
要了解有关Cloud Manger及其资源的更多信息，请参阅《 [**Cloud Manager用户指南》**](https://helpx.adobe.com/experience-manager/cloud-manager/user-guide.html)。
...
在客户端，AEM可与所有现代浏览器(**Microsoft Edge**、 **Internet Edge** 11、 **Chrome51+、********** Firefox47+、Safari8+)一起在桌面和操作系统上运行。 有关详细 [信息，请参阅支持的客户端平台](/help/sites-deploying/technical-requirements.md#supported-client-platforms) 。
...
* **** cq-quickstart-6.4.0.jar:独立的可执 *行文件* jar文件，包含启动和运行所需的一切。

* **** cq-quickstart-6.4.0.war:用 *于部署* 在第三方应用程序服务器中的war文件。
...
1. 要在GUI环境中启动实例，只需双击该文 **`cq-quickstart-6.4.0.jar`** 件。
...
如果实例是从GUI安装的，则浏览器窗口将自动打开，桌面应用程序窗口也会打开，显示实例的主机和端口以及开／关交换机：
