...
部署AEM实例后，需要某些任务来监视和维护其操作、性能和完整性。
...
| 灾难恢复计划。 | 您的公司的灾难恢复指南。 |  |
| 错误跟踪系统可用于解决报告问题。 | 例如，[bugzilla](https://www.bugzilla.org/)、[jira](https://www.atlassian.com/software/jira/)或其他许多其中之一。 |  |
...
| 定期清除工作流实例。 | 存储库大小和工作流性能。 | 请参阅[常规清除工作流实例](/help/sites-administering/workflows-administering.md#regular-purging-of-workflow-instances)。 |
...
>[!NOTE]
>
>有关备份性能的详细信息，请阅读[备份性能](/help/sites-deploying/configuring-performance.md#backup-performance)一节。
...
>[!CAUTION]
>
>如果您运行的是第三方应用程序服务器，则其他文件夹可能位于不同的位置，并且可能还需要备份。 有关安装应用程序服务器的信息，请参见[如何将AEM与应用程序服务器](/help/sites-deploying/application-server-install.md)一起安装。
...
**清除版本**&#x200B;工具](/help/sites-administering/tools-consoles.md)控制台&#x200B;**位于**&#x200B;版本控制&#x200B;**下，或直接位于：**[

`https://<server>:<port>/etc/versioning/purge.html`

![screen_shot_2012-03-15at14418pm](assets/screen_shot_2012-03-15at14418pm.png)

**开始** 路径必须执行清除的绝对路径。您可以通过单击存储库树导航器来选择开始路径。

**递** 归在清除数据时，您可以通过选择递归选择在一个节点上或整个层次上执行操作。在最后一种情况下，给定路径定义层次的根节点。

**要保留的** 最大版本要为节点保留的最大版本数。当这些数字超过此值时，将清除最旧版本。

**最大版** 本期限节点版本的最大期限。当版本的年龄超过此值时，将清除该版本。

**干** 运行由于删除内容的版本是确定的，并且在不恢复备份的情况下无法还原，因此清除版本工具提供了干运行模式，允许您预览清除的版本。要启动清除流程的练习，请单击“练习”。

**清** 除启动在开始路径定义的节点上清除版本。
...
**干运行**&#x200B;和&#x200B;**清除**&#x200B;将列表所有已处理的节点。 在此过程中，节点可以具有以下状态之一：
...
* **衬衫**&#x200B;版本被清除，因为其版本年龄大于2天。
* **汤加时尚！** 将清除版本，因为其版本数大于5。

![global_version_screenshot](assets/global_version_screenshot.png)

## 使用审计记录和日志文件{#working-with-audit-records-and-log-files}

与Adobe Experience Manager(AEM)相关的审计记录和日志文件可在不同位置找到。 下面提供了您可以在哪里找到的内容的概述。
...
* 根据模式{original_filename} `.yyyy-MM-dd`重命名`error.log`文件。 例如，在2010年7月11日，当前日志文件被重命名为`error.log-2010-07-10`，然后创建新的`error.og`。
...
* `<cq-installation-dir>/crx-quickstart/logs`
   ...
   * `upgrade.log`
      ...
      提供从`com.day.compat.codeupgrade`和`com.adobe.cq.upgradesexecutor`软件包运行的所有升级操作的日志。
...
>[!NOTE]
>
>从&#x200B;**system/console/status-Bundlelist**&#x200B;页面生成的&#x200B;**下载完整**&#x200B;包中不包含ImageServer和s7access日志。 出于支持目的，如果您有Dynamic Media问题，请在联系客户支持时附加ImageServer和s7access日志。

### 激活DEBUG日志级别{#activating-the-debug-log-level}

默认日志级别[Apache Sling日志记录配置](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)为信息，因此调试消息不会记录。

要激活记录器的调试日志级别，请在存储库中将属性`org.apache.sling.commons.log.level`设置为debug。 例如，在`/libs/sling/config/org.apache.sling.commons.log.LogManager`上配置[全局Apache Sling日志](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)。
...
|---|---|---|
| 1 | 错误 | 操作失败。 安装将继续进行，但AEM WCM的一部分安装不正确，将无法工作。 |
...
>[!NOTE]
>
>与Adobe Experience Manager合作时，有几种方法管理此类服务的配置设置；请参阅[配置OSGi](/help/sites-deploying/configuring-osgi.md)以了解更多详细信息和建议的做法。

在某些情况下，您可能希望创建具有不同日志级别的自定义日志文件。 您可以通过以下方式在存储库中执行此操作：

1. 如果项目`/apps/<project-name>/config`尚未存在，请为其新建一个配置文件夹(`sling:Folder`)。
1. 在`/apps/<project-name>/config`下，为新的[Apache Sling日志记录器配置](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)创建一个节点：

   * 名称:

   `org.apache.sling.commons.log.LogManager.factory.config-<identifier>` （因为这是记录器）

   其中`<identifier>`被替换为自由文本，您（必须）输入该文本以标识实例（您不能忽略此信息）。 例如，`org.apache.sling.commons.log.LogManager.factory.config-MINE`
...
1. 在此节点上设置以下属性：
   ...
   * 根据需要配置其他参数：
   ...
   >{5}日志消息
   >
   >如果日志调用包含`Throwable`，则stacktrace将附加到消息中。
...
1. 仅当需要新的写入程序时（即配置与默认写入程序不同时），此步骤才是必需的。
   ...
   在`/apps/<project-name>/config`下，为新的[Apache Sling日志记录写入程序配置](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)创建一个节点：

   * 名称：`org.apache.sling.commons.log.LogManager.factory.writer-<identifier>`（因为这是作者）

      与记录器一样，`<identifier>`被替换为自由文本，您（必须）输入该文本以标识实例（您不能忽略此信息）。 例如，`org.apache.sling.commons.log.LogManager.factory.writer-MINE`
   ...
   * 根据需要配置其他参数：
   ...
   >
   >* 可以使用数字指定大小限制。 如果未提供大小指示符，则此指示符将作为字节数，或者您可以添加一个大小指示符- `KB`、`MB`或`GB`（忽略大小写）。
...
Felix控制台还提供有关Sling日志支持的信息，网址为`../system/console/slinglog`;例如`http://localhost:4502/system/console/slinglog`。

### 查找审计记录{#finding-the-audit-records}

保留审计记录，以便提供由谁执行什么和何时执行的记录。 为AEM WCM和OSGi事件生成不同的审计记录。

#### AEM WCM在页面创作{#aem-wcm-audit-records-shown-when-page-authoring}时显示审核记录
...
在`/var/audit`文件夹中，根据资源保留审核记录。 您可以向下展开，直到您看到单个记录及其包含的信息。

这些条目包含的信息与编辑页面时显示的信息相同。

#### Web控制台{#osgi-audit-records-from-the-web-console}中的OSGi审核记录

OSGi事件还生成审计记录，可从AEM Web控制台的&#x200B;**配置状态**&#x200B;选项卡-> **日志文件**&#x200B;选项卡中查看该记录：

![screen_shot_2012-02-13at50346pm](assets/screen_shot_2012-02-13at50346pm.png)

## 监视复制代理{#monitoring-your-replication-agents}

您可以监视[复制队列](/help/sites-deploying/replication.md)以检测队列何时关闭或被阻止——这又可能表示发布实例或外部系统出现问题：

* 是否所有必需队列都已启用？
* 是否仍需要任何禁用的队列？
* 所有`enabled`队列应具有状态`idle`或`active`，这表示正常操作；不应有`blocked`队列，这通常是接收方出现问题的一个迹象。
...
同样，您可以开发一种解决方案来检测所有复制代理（位于`/etc/replication/author`或`/etc/replication/publish`下），然后检查代理(`enabled`、`disabled`)和基础队列(`active`、`idle`、`blocked`)的状态。

## 监视性能{#monitoring-performance}

[性能优](/help/sites-deploying/configuring-performance.md) 化是一个交互式过程，在开发过程中受到关注。部署后，通常在特定时间间隔或事件后对其进行审核。
...
### 用于监视和分析性能的工具{#tools-for-monitoring-and-analyzing-performance}
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>桁架／地形</td> 
   <td>页面加载</td> 
   <td><p>用于跟踪系统调用和信号的Unix/Linux命令。 将日志级别增加到<code>INFO</code>。</p> <p>分析每个请求的页面加载次数，以及哪些页面等。</p> </td> 
  </tr> 
  <tr> 
   <td>线程转储</td> 
  ...
  <tr> 
   <td>系统调用</td> 
   <td>确定时间问题。</td> 
   <td><p>对<code>System.currentTimeMillis()</code>或<code>com.day.util</code>的调用。Timing用于根据您的代码或通过<a href="#html-comments">HTML-comments</a>生成时间戳。</p> <p><strong>注意：</strong> 应实现这些功能，以便根据需要激活／取消激活它们；当系统运行顺利时，不需要收集统计的开销。</p> </td> 
  </tr> 
  <tr> 
   <td>Apache Bench</td> 
  ...
  <tr> 
   ...
   <td><p>要查看页面呈现的时间统计信息，可以将<strong>Ctrl-Shift-U</strong>与URL中设置的<code>?debugClientLibs=true</code>结合使用。</p> </td> 
  </tr> 
  <tr> 
   <td>CPU和内存分析工具<br /> </td> 
  ...
  <tr> 
   ...
   <td>您的安装的当前状态。</td> 
   <td>尽可能地了解您的安装还可以帮助您找到可能导致性能变化的原因，以及这些更改是否合理。 这些指标需要定期收集，以便您能够轻松看到重大变化。</td> 
  </tr> 
 </tbody> 
</table>
...
`request.log`优惠一种内置方式，查看请求需要多长时间。 出于开发目的，`tail -f`和`request.log`可观察响应速度慢。 要分析更大的`request.log`，建议使用[`rlog.jar`，以便对响应时间](#using-rlog-jar-to-find-requests-with-long-duration-times)进行排序和筛选。

我们建议将“慢”页面与`request.log`隔离，然后单独调整它们以获得更好的性能。 这通常通过包括每个组件的性能指标或使用性能分析工具（如` [yourkit](https://www.yourkit.com/)`）来完成。

#### 监视网站上的流量{#monitoring-traffic-on-your-website}
...
通过在特定时间段内（例如，在不同的24小时时间段内）汇总所有GET条目，您可以对网站的平均流量进行声明。
...
#### 监视并发用户数和影响{#monitoring-the-number-and-impact-of-concurrent-users}

同样，`request.log`可用于监视并发以及系统对它的反应。

必须进行测试，以确定系统可以处理的并发用户数，然后才能看到负面影响。 脚本同样可用于从日志文件中提取结果：

* 监视在特定时间范围内发出的请求数，如1分钟
* 测试同一时间（尽可能接近）发出相同请求的特定用户数的效果；例如，30个用户同时单击&#x200B;**保存**。
...
例如，可以运行它，将`request.log`文件指定为参数并显示持续时间最长的10个第一个请求：
...
为了最大限度地减少特殊情况（如垃圾收集等）的影响，建议使用`apachebench`等工具（例如，[ab](https://httpd.apache.org/docs/2.2/programs/ab.html)以获取进一步文档）来帮助识别内存泄漏并有选择地分析响应时间。
...
以上数字取自访问geometrixx公司页的标准MAcBook Pro笔记本电脑（2010年中），如默认AEM安装中所示。 页面很简单，但未针对性能进行优化。

`apachebench` 还以平均方式显示每个请求的时间，涵盖所有并发请求；请参 `Time per request: 54.595 [ms]` 阅（平均，跨所有并发请求）。您可以更改并发参数`-c`（一次要执行的多个请求数）的值，以查看任何效果。

### 请求计数器{#request-counters}

有关请求流量的信息（特定时间段内的请求数）会指示实例的负载。 此信息可从[request.log](#interpreting-the-request-log)中提取，但使用计数器将自动收集数据，以便您查看：
...
建议每个项目都包含`html comments`以实现服务器性能。 许多好的公示例子，选择页面，打开页面源进行查看并滚动到底部，可以看到以下代码：
...
1. 开始AEM实例。
   ...
   >[!NOTE]
   >
   >如果使用Java 5，则可以将`-Dcom.sun.management.jmxremote`参数添加到开始JVM的java命令行。 默认情况下，JMX在Java 6中处于启用状态。
...
尽可能地了解您的安装有助于您了解可能导致性能变化的原因，以及这些更改是否合理。 这些指标需要定期收集，以便您能够轻松看到重大变化。
...
* [如果您使用AEM Assets，您当前在资产中维护多少资产？](#if-you-use-aem-assets-how-many-assets-do-you-currently-maintain-in-assets)
...
1. 使用命令行：
   ...
   要获得平均大小，请将全局大小除以`/var/dam`（在上面获取）中的资产总数。
