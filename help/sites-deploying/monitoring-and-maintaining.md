...
1. 导航到&#x200B;**[工具](/help/sites-administering/tools-consoles.md)控制台**，选择&#x200B;**版本控制**，然后多次单击&#x200B;**清除版本**。
...
* `<cq-installation-dir>/crx-quickstart/logs`
   ...
   * [ `ImageServer-<PortId>-yyyy>-<mm>-<dd>.log`](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/config-admin/server-logging/c-image-server-log.html)
      ...
      此日志仅在Dynamic Media处于启用状态时使用。 它提供用于分析内部ImageServer进程的行为的统计和分析信息。
   ...
   * [ `s7access-<yyyy>-<mm>-<dd>.log`](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/config-admin/server-logging/c-access-log.html)
      ...
      此日志仅在Dynamic Media处于启用状态时使用。 s7access日志记录通过`/is/image`和`/is/content`向Dynamic Media发出的每个请求。

   * `stderr.log`

      保留在启动过程中生成的、严重性级别不同的错误消息。 默认情况下，日志级别设置为`Warning`(`WARN`)
...
>[!NOTE]
>
>从&#x200B;**system/console/status-Bundlelist**&#x200B;页面生成的&#x200B;**下载完整**&#x200B;包中不包含ImageServer和s7access日志。 出于支持目的，如果您有Dynamic Media问题，请在与客户支持联系时附加ImageServer和s7access日志。
...
要激活Logger的调试日志级别，请在存储库中将属性`org.apache.sling.commons.log.level`设置为调试。 例如，在`/libs/sling/config/org.apache.sling.commons.log.LogManager`上配置[全局Apache Sling日志](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)。
...
1. 在`/apps/<project-name>/config`下，为新的[Apache Sling日志记录记录器配置](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)创建一个节点：

   * 名称:

   `org.apache.sling.commons.log.LogManager.factory.config-<identifier>` （因为这是记录器）

   其中，`<identifier>`被替换为自由文本，您（必须）输入该文本以标识实例（您无法忽略此信息）。 例如，`org.apache.sling.commons.log.LogManager.factory.config-MINE`
...
1. 仅当需要新的Writer时（即，配置与默认Writer不同时），才需要此步骤。
   ...
   在`/apps/<project-name>/config`下，为新的[Apache Sling日志记录编写器配置](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)创建一个节点：
...
但是，一旦确定了响应时间，您可能需要分析请求为何需要花费时间，以及可以采取什么措施来改进响应。
...
1. 运行以下任一选项：
   ...
   * `visualvm`:可从VisualVM( [最新](https://visualvm.github.io/releases.html) 的边缘版本)下载
