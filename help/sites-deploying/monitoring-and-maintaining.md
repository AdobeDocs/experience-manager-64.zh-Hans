...
* `<cq-installation-dir>/crx-quickstart/logs`
   ...
   * [ `s7access-<yyyy>-<mm>-<dd>.log`](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/config-admin/server-logging/c-access-log.html)
      ...
      仅当启用了Dynamic Media时，才使用此日志。 s7access日志记录通过`/is/image`和`/is/content`向Dynamic Media发出的每个请求。
...
>[!NOTE]
>
>从&#x200B;**system/console/status-Bundlelist**&#x200B;页面生成的&#x200B;**Download Full**&#x200B;包中未包含ImageServer和s7access日志。 出于支持目的，如果您遇到Dynamic Media问题，请在联系客户支持时还附加ImageServer和s7access日志。

### 激活DEBUG日志级别{#activating-the-debug-log-level}

默认日志级别[Apache Sling日志记录配置](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)为“信息”，因此调试消息不会被记录。

要激活日志记录器的调试日志级别，请将属性`org.apache.sling.commons.log.level`设置为在存储库中进行调试。 例如，在`/libs/sling/config/org.apache.sling.commons.log.LogManager`上配置[全局Apache Sling日志记录](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)。
...
1. 在`/apps/<project-name>/config`下，为新的[Apache Sling日志记录器配置](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)创建节点：

   * 名称:

   `org.apache.sling.commons.log.LogManager.factory.config-<identifier>` （因为这是一个日志记录器）

   其中， `<identifier>`被替换为您（必须）输入以标识实例的自由文本（您不能忽略此信息）。 例如，`org.apache.sling.commons.log.LogManager.factory.config-MINE`
...
Felix控制台还在`../system/console/slinglog`提供了有关Sling日志支持的信息；例如`http://localhost:4502/system/console/slinglog`。
...
1. 运行以下任一操作：
   ...
   * `visualvm`:可以从VisualVM  [](https://visualvm.github.io/releases.html) （放血边缘版本）下载
