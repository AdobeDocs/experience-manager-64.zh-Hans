...
检查损坏的索引可以通过以下操作 `--index-consistency-check` 执行：

```shell
java -jar oak-run*.jar index --fds-path=/path/to/datastore  /path/to/segmentstore/ --index-consistency-check
```

这将在中生成报告 `indexing-result/index-consistency-check-report.txt`。 有关示例报告，请参阅以下内容：
...
根据情 [况](https://jackrabbit.apache.org/oak/docs/query/indexing.html#reindexing)，在某些情况下需要执行重新索引。 当前，通过CRXDE或通过索 `reindex` 引管理器 `true` 用户界面将标志设置为索引定义节点，从而完成重新索引。 设置标志后，异步完成重新索引。

有关重新编制索引的注意事项有：

* 与所有内容都是本地内容 `DocumentNodeStore` 的设置相比， `SegmentNodeStore` 设置重新索引的速度要慢得多；

* 在当前设计下，当重新索引时，异步索引器被阻止，所有其他异步索引都变旧，并且在索引期间不获取更新。 因此，如果系统正在使用，用户可能看不到最新的结果；
* 重新索引涉及遍历整个存储库，这会给AEM设置带来高负载，从而影响最终用户体验；
* 对于重 `DocumentNodeStore` 新编制索引可能需要相当长时间的安装，如果在操作过程中与Mongo数据库的连接失败，则必须从头开始重新编制索引；
...
* **简化的DocumentNodeStore安装的重新索引** -对 `DocumentNodeStore` 于安装，可以使用一个命令重新索引，这可确保以最佳方式重新索引；

* **支持更新索引定义和引入新的索引定义**

### 重新索引- DocumentNodeStore {#reindexdocumentnodestore}

对 `DocumentNodeStore` 于安装，可通过单个oak-run命令重新建立索引：
...
* 用户还可以通过选项提供新索引或更新索引的 `--index-definitions-file` JSON。
...
按照既定的方式，通过设置标志重新编制 `reindex` 索引。

#### 在线重新索引- SegmentNodeStore -AEM实例正在运行 {#onlinereindexsegmentnodestoretheaeminstanceisrunning}

对于 `SegmentNodeStore` 安装，只有一个进程可以在读写模式下访问段文件。 由于这一原因，在oak-run索引中的一些操作需要额外的手动步骤。

这将涉及以下内容：

1. 步骤文本
1. 将连接 `oak-run` 到AEM在只读模式下使用的同一存储库，然后执行索引。 有关如何实现此目的的示例：

   ```shell
   java -jar oak-run-1.7.6.jar index --fds-path=/Users/dhasler/dev/cq/quickstart/target/crx-quickstart/repository/datastore/ --checkpoint 26b7da38-a699-45b2-82fb-73aa2f9af0e2 --reindex --index-paths=/oak:index/lucene /Users/dhasler/dev/cq/quickstart/target/crx-quickstart/repository/segmentstore/
   ```

1. 最后，通过操作从oak-run运 `IndexerMBean#importIndex` 行上述命令后保存索引文件的路径导入创建的索引文件。
...
1. 通过JMX操作创建检查点。 您可以转到JMX控制 [台并搜索](/help/sites-administering/jmx-console.md) ，以实现此目 `CheckpointManager`的。 然后，使用高 **值单击createCheckpoint** (long p1)操作，在几秒内过期( **例如2592000**)。
...
在此用例下，假定数据存储可在另一个实例上访问，如果放置在基于云的存储解决方案( `FileDataStore` 如EBS)上，则该实例可能无法访问。 这不包括同时克隆 `FileDataStore` 的方案。 如果索引定义不执行全文索引，则不需要访 `DataStore` 问该索引。

## 用例4 —— 更新索引定义 {#usecase4updatingindexdefinitions}

目前，您可以通过ACS Ensure Index包发 [送索引定义更](https://adobe-consulting-services.github.io/acs-aem-commons/features/ensure-oak-index/index.html) 改。 这允许通过内容包发送索引定义，稍后需要通过将标志设置为来执行重新 `reindex` 索引 `true`。
...
1. 开发人员将更新本地实例上的索引定义，然后通过选项生成索引定义JSON文 `--index-definitions` 件
