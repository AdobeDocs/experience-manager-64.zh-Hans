...
* **安装文件夹名称正则表达式** 和 **安装文件夹的最大层次结构深度**  — 指定在何处和到哪个深度搜索要安装的资源库文件夹。 使用通配符时（如中所示）。&amp;ast;/install)将搜索所有相应匹配项，例如， `/libs/sling/install` 和 `/libs/cq/core/install`.
...
* 使用本地安装(例如，使用 `http://localhost:4502/system/console/jcrresolver`)来确定哪个资源解析程序处于活动状态。
...
有 [两个UI](/help/sites-authoring/select-ui.md) 在AEM中可用：
...
* **用于发送数据的URL** 配置用于跟踪页面统计信息的URL（如果跟踪器请求通过调度程序，则此URL至关重要）；例如，默认值为 `http://localhost:4502/libs/wcm/stats/tracker`.
