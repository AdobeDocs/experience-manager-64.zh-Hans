...
* **安装文件夹名称regexp** and **Max hierarchy depth of install folders** —— 指定在何处以及到哪个深度，将搜索存储库文件夹以查找要安装的资源。 当使用通配符时（如中所示）。&amp;ast;/install)将搜索所有相应的匹配项，例如 `/libs/sling/install` 和 `/libs/cq/core/install`。
...
* 使用本地安装（例如，使用）确 `http://localhost:4502/system/console/jcrresolver`定哪个资源解析程序处于活动状态。
...
创建新配置时，请勿更改工厂配置，而应使用此处提供的配置管理器为此组件创建新的工厂配置： **http://localhost:4502/system/console/configMgr/**。 代理配置在 **org.apache.http.proxyconfigurator中可用。**
...
AEM中 [有两个](/help/sites-authoring/select-ui.md) UI可用：
...
* **路径**&#x200B;此身份验证处理程序处于活动状态的路径。 如果此参数留空，则身份验证处理函数将被禁用。 例如，路径／导致身份验证处理程序用于整个存储库。
...
* **发送数据的URL** ，以配置用于跟踪页面统计信息的URL（如果跟踪器请求通过调度程序，则此URL至关重要）;例如，默认值为 `http://localhost:4502/libs/wcm/stats/tracker`。
