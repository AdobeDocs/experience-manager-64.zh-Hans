...
* **启用** （或禁用）默认演绎版，如 **HMTL**、 **纯文本**、 **** JSON **或XML**。
...
* **安装文件夹名** 称重新展 **开安装文件夹的最大层次结构深度**  — 指定在何处搜索要安装的资源库文件夹以及搜索到哪个深度。使用通配符时（如中所示）。&amp;ast;/install)将搜索所有适当的匹配项，例如`/libs/sling/install`和`/libs/cq/core/install`。
...
AEM中提供了[两个UI](/help/sites-authoring/select-ui.md) :
