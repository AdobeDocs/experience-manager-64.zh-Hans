...
在[就地升级](/help/sites-deploying/in-place-upgrade.md)之后，应执行以下活动以完成升级。 假定AEM已从6.4 jar启动，并且已部署升级的代码库。
...
* [Inspect the PreUpgradeBackup文件夹](#inspect-preupgradebackup-folder)
...
在使用目标版本jar开始AEM时，应仔细查看error.log。 应查看任何警告或错误。 通常，最好在日志开头查找问题。 日志中稍后出现的错误实际上可能是文件早期调用的根本原因的副作用。 如果出现重复的错误和警告，请参见下面的[分析升级问题](/help/sites-deploying/post-upgrade-checks-and-troubleshooting.md#analyzing-issues-with-upgrade)。
...
升级后，您应看到Oak版本已更新为&#x200B;**1.8.2**。 要验证Oak版本，请导航到OSGi控制台，并查看与Oak捆绑包关联的版本：Oak Core、Oak Commons、Oak Segment Tar。
...
AEM中的几个功能需要升级后执行其他步骤。 在[升级代码和自定义](/help/sites-deploying/upgrading-code-and-customizations.md)页上可以找到在AEM 6.4中迁移这些功能和步骤的完整列表。
...
发布环境完全升级和验证后，在“作者”环境上启用复制代理。 验证代理是否能够连接到相应的发布实例。 有关事件顺序的详细信息，请参阅[升级过程](/help/sites-deploying/upgrade-procedure.md)。
...
本节包含在升级到AEM 6.4过程中可能遇到的一些问题方案。

这些方案应有助于跟踪与升级相关问题的根本原因，并有助于确定项目或产品特定问题。

### 在升级{#dynamic-media-cloud-configuration}后重新创建Dynamic Media云配置

从较早版本升级到AEM 6.4后，从早期设置进行的Dynamic Media云配置可能无法从AEM 6.4 TouchUI访问。 要解决此问题，请使用CRXDE Lite删除之前的设置，然后新建Dynamic Media云配置。 另请参阅AEM 6.4](/help/sites-deploying/dynamicmedia-repository-restructuring-in-aem-6-4.md)中的[Dynamic Media存储库重构。
...
还建议将新AEM 6.4实例的捆绑列表与升级实例进行比较，以检测未升级的捆绑包。 这将提供`error.log`中要搜索内容的更近范围。
...
### 等{#missing-customizations-under-etc}下缺少自定义
