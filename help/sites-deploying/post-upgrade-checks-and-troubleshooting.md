...
使用目标版本jar开始AEM期间和之后，应仔细查看error.log。 应查看任何警告或错误。 一般而言，最好在日志的开头查找问题。 日志后面出现的错误实际上可能是根源的副作用，而根源在文件早期被调用。 如果出现重复的错误和警告，请参见下面的[分析升级问题](/help/sites-deploying/post-upgrade-checks-and-troubleshooting.md#analyzing-issues-with-upgrade)。
...
升级后，您应看到Oak版本已更新为&#x200B;**1.8.2**。 要验证Oak版本，请导航到OSGi控制台，并查看与Oak捆绑包相关的版本：Oak Core、Oak Commons、Oak Segment Tar。
...
AEM中的一些功能需要升级后执行其他步骤。 在[升级代码和自定义](/help/sites-deploying/upgrading-code-and-customizations.md)页上可以找到在AEM 6.4中迁移这些功能和步骤的完整列表。
...
发布环境完全升级和验证后，在创作环境上启用复制代理。 验证代理是否能够连接到相应的发布实例。 有关事件顺序的更多详细信息，请参阅[升级过程](/help/sites-deploying/upgrade-procedure.md)。
...
### 升级{#dynamic-media-cloud-configuration}后重新创建Dynamic Media云配置

从早期版本升级到AEM 6.4后，早期设置中的Dynamic Media云配置可能无法从AEM 6.4 TouchUI访问。 要解决此问题，请使用CRXDE Lite删除之前的设置，然后创建新的Dynamic Media云配置。 另请参见AEM 6.4](/help/sites-deploying/dynamicmedia-repository-restructuring-in-aem-6-4.md)中的[Dynamic Media存储库重组。
...
还建议将最新AEM 6.4实例的捆绑列表与升级实例进行比较，以检测未升级的捆绑包。 这将提供`error.log`中要搜索内容的更近范围。
...
### 在etc {#missing-customizations-under-etc}下缺少自定义项
