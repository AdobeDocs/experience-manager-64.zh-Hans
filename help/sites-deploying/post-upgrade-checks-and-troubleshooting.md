...
在[就地升级](/help/sites-deploying/in-place-upgrade.md)之后，应执行以下活动以完成升级。 假定AEM已从6.4 jar中启动，并且已部署已升级的代码库。
...
在使用目标版本jar启动AEM期间和之后，应仔细查看error.log。 应审核任何警告或错误。 通常最好在日志开头查找问题。 日志中稍后发生的错误实际上可能是文件早期调用的根本原因的副作用。 如果出现重复的错误和警告，请参阅下面的[分析升级问题](/help/sites-deploying/post-upgrade-checks-and-troubleshooting.md#analyzing-issues-with-upgrade)。
...
AEM中的几项功能在升级后需要执行其他步骤。 在[升级代码和自定义](/help/sites-deploying/upgrading-code-and-customizations.md)页面上，可以找到这些功能以及在AEM 6.4中迁移这些功能的完整列表和步骤。
...
完全升级和验证发布环境后，请在创作环境中启用复制代理。 验证代理是否能够连接到相应的发布实例。 有关事件顺序的更多详细信息，请参阅[升级过程](/help/sites-deploying/upgrade-procedure.md) 。
...
从早期版本升级到AEM 6.4后，从早期设置升级到Dynamic Media云配置可能无法从AEM 6.4触屏UI访问。 要解决此问题，请使用CRXDE Lite删除之前的设置，然后创建新的Dynamic Media云配置。 另请参阅AEM 6.4](/help/sites-deploying/dynamicmedia-repository-restructuring-in-aem-6-4.md)中的[Dynamic Media存储库重组。
...
还建议将新AEM 6.4实例的包列表与已升级的实例进行比较，以检测未升级的包。 这将在`error.log`中提供更详细的搜索内容。
