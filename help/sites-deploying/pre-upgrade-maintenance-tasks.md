...
升级过程能够很好地维护和合并存储库中的和路径下 `/apps` 的现有 `/libs` 内容和配置。 对于路径(包括 `/etc` Context Hub配置)所做的更改，通常需要在升级后重新应用这些更改。 虽然升级将备份其无法合并的任何更改，但我们建议在开始升级 `/var`之前手动备份这些更改。
...
OSGI `PreUpgradeTasksMBean` 组件预配置了一列表升级前维护任务，可以一次全部运行。 您可以按照以下过程配置任务:

1. 通过浏览到 `https://serveraddress:serverport/system/console/configMgr`

1. 搜索“预&#x200B;**升级任**&#x200B;务”，然后单击第一个匹配的组件。 组件的完整名称为 `com.adobe.aem.upgrade.prechecks.mbean.impl.PreUpgradeTasksMBeanImpl`
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>crx2</td> 
   <td>会进行标记和扫描。 对于共享数据存储，请删除此步骤<br /> ，然后手动运行或在执行前正确准备实例。</td> 
  </tr> 
  <tr> 
...
OSGI `PreUpgradeTasksMBeanImpl` 组件预配置了一列表升级前运行状况检查标记，在调用该方 `runAllPreUpgradeHealthChecks` 法时执行：

* **system** —— 花岗岩维护运行状况检查使用的标签

* **升级前** -这是一个自定义标记，可添加到升级前可以设置为运行的所有运行状况检查中

列表可编辑。 除了标记外 **，还可使** 用加号(+ **)和减号(-** )按钮添加更多自定义标记，或删除默认标记。

**MBean方法**

可以使用JMX控制台访问受管 [理的Bean功能](/help/sites-administering/jmx-console.md)。
...
1. 从“操作”部分选 **择任何** 方法，并在 **下面的窗** 口中选择“调用”。

以下是所有可用方法的列表 `PreUpgradeTasksMBeanImpl` :
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>ACTION_INFO</td> 
   <td>检查是否有任何升级前维护任务当前正在运行<br /> ，并返回包含当前正在运行的任务名称的阵列。</td> 
  </tr> 
  <tr> 
...
要禁用在的JAAS配置中定义的自 `repository.xml`定义模块，您需要修改配置以使用默认模 `LoginModule`块，如本例所示：
...
如果使用TarMK，则应在升级前执行脱机修订清理。 这将使存储库迁移步骤和后续升级任务执行得更快，并有助于确保在升级完成后可以成功执行在线修订清理。 有关运行脱机修订清除的信息，请参 [阅执行脱机修订清除](https://helpx.adobe.com/experience-manager/6-2/sites-deploying/storage-elements-in-aem-6.html#performing-offline-revision-cleanup)。
...
在对CRX3实例运行修订清理后，您应运行数据存储垃圾收集，以删除数据存储中任何未引用的blob。 有关说明，请参阅有关数据存 [储垃圾收集的文档](/help/sites-administering/data-store-garbage-collection.md)。
...
* 将实例与生产流量分离
* 创建导致问题的用户的备份。 您可以通过包管理器执行此操作。 有关详细信息，请 [参阅如何使用包](/help/sites-administering/package-manager.md)。
...
1. 升级数据库模式。 请查阅有关数据库类型的文档，以了解实现此目的需要使用哪些工具。
   ...
   有关Oak如何处理模式升级的更多信息，请 [参阅Apache网站的此页](https://jackrabbit.apache.org/oak/docs/nodestore/document/rdb-document-store.html#upgrade)。
