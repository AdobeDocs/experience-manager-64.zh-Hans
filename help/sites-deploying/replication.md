...
* [发布（激活）](/help/sites-authoring/publishing-pages.md#publishing-pages) 从创作到发布环境的内容。
* 显式刷新Dispatcher缓存中的内容。
* 将用户输入（例如，表单输入）从发布环境返回到创作环境（在创作环境的控制下）。

请求为 [已排队](/help/sites-deploying/osgi-configuration-settings.md) 转给相应的代理进行处理。

>[!NOTE]
>
>用户数据（用户、用户组和用户配置文件）不会在创作实例和发布实例之间复制。
>
>对于多个发布实例，用户数据在 [用户同步](/help/sites-administering/sync.md) 启用。
...
* 在“网站”选项卡中， [颜色状态指示器](/help/sites-authoring/publishing-pages.md#determining-publication-status) 为各个页面设置。
...
AEM [社区](/help/communities/overview.md) 从不对UGC使用复制。 Communities的部署需要UGC的公共存储(请参阅 [社区内容存储](/help/communities/working-with-srp.md))。

## 复制 — 开箱即用 {#replication-out-of-the-box}

标准安装AEM中包含的Geometrixx网站可用于说明复制。

要遵循此示例并使用所需的默认复制代理，请 [安装AEM](/help/sites-deploying/deploy.md) 与：
...
>请参阅 [监控复制代理](#monitoring-your-replication-agents).
...
* 调度程序刷新 — 用于管理调度程序缓存。 请参阅 [从创作环境中使Dispatcher缓存失效](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#invalidating-dispatcher-cache-from-the-authoring-environment) 和 [从发布实例中使调度程序缓存失效](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#invalidating-dispatcher-cache-from-a-publishing-instance) 以了解更多信息。

* [反向复制](#replicating-from-publish-to-author)  — 用于从发布复制到创作。 社区功能（如论坛、博客和评论）不使用反向复制。 由于未启用发件箱，因此会有效地禁用该功能。 使用反向复制需要自定义配置。

* 静态代理 — 这是一个“将节点的静态表示存储到文件系统中的代理”。 例如，使用默认设置时，内容页面和dam资产存储在 `/tmp`，以HTML或相应的资产格式。 请参阅 `Settings` 和 `Rules` 选项卡。 请求执行此操作，以便当直接从应用程序服务器请求页面时，可以看到内容。 这是一个专门的代理，在大多数情况下（可能）不是必需的。
...
>[!NOTE]
>
>当调度程序处理创作或发布实例的HTTP请求时，来自复制代理的HTTP请求必须包含PATH标头。 除了以下过程之外，您还必须将PATH标头添加到客户端标头的调度程序列表中。 (请参阅 [/clientheaders（客户端头）](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#specifying-the-http-headers-to-pass-through-clientheaders). [](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#specifying-the-http-headers-to-pass-through-clientheaders)

1. 访问 **工具** 选项卡。
1. 单击 **复制** （左窗格以打开文件夹）。
1. 双击 **作者代理** （左窗格或右窗格）。
1. 单击相应的代理名称（即链接）可显示有关该代理的详细信息。
1. 单击 **编辑** 要打开配置对话框，请执行以下操作：

   ![chlimage_1-145](assets/chlimage_1-145.png)

1. 提供的值应足以用于默认安装。 如果进行更改，请单击 **确定** 以保存它们(请参阅 [复制代理 — 配置参数](#replication-agents-configuration-parameters) ，以了解有关各个参数的更多详细信息)。
...
出于安全原因，大多数网络拓扑不允许连接 *从* “非军事区”(向不受信任的网络（如Internet）提供外部服务的子网络)。
...
>[!NOTE]
>
>对于AEM [社区](/help/communities/overview.md)，则发布实例上用户生成的内容不会使用复制。 请参阅 [社区内容存储](/help/communities/working-with-srp.md).
...
1. 单击 **创建** 创建新代理。
1. 双击新代理项以打开配置面板。
1. 单击 **编辑** - **代理设置** 对话框将打开 —  **序列化类型** 已定义为默认，则必须保持默认状态。
...
>[!NOTE]
>
>这可以与的使用结合使用 [代理用户Id](#settings) ，以选择要复制到单个发布环境的不同内容。 对于每个发布环境：
>
>1. 配置复制代理以复制到该发布环境。
>1. 配置用户帐户；具有读取将被复制到该特定发布环境的内容所需的访问权限。
>1. 将用户帐户分配为 **代理用户Id** 复制代理。
...
1. 单击 **编辑** - **代理设置** 对话框将打开：
   ...
   * 在 **运输** 选项卡：
   ...
   >[!NOTE]
   >
   >如果您在推荐的默认上下文以外的上下文中安装了AEM，则需要配置 [HTTP头](#extended) 在 **扩展** 选项卡。

1. 单击 **确定** 以保存更改。
1. 返回到 **工具** 选项卡，在此 **激活** the **调度程序刷新** 代理(**发布时的代理**)。

的 **调度程序刷新** 复制代理在创作时不活动。 您可以使用等效的URI在发布环境中访问同一页面；例如， `http://localhost:4503/etc/replication/agents.publish/flush.html`.
...
1. 按 **[!UICONTROL 工具]** 图标
1. 从左侧导航边栏中，转到 **[!UICONTROL 复制 — 创作代理]** 双击 **[!UICONTROL 默认代理]**.
...
有关疑难解答的详细信息，请阅读 [复制故障诊断](/help/sites-deploying/troubleshoot-rep.md) 页面。
