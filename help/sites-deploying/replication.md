...
复制代理是Adobe Experience Manager(AEM)的中心，作为用于：

* [将内容从作](/help/sites-authoring/publishing-pages.md#publishing-pages) 者发布（激活）到发布环境。
...
>[!NOTE]
>
>用户数据(用户、用户组和用户用户档案)不会在创作和发布实例之间复制。
>
>对于多个发布实例，启用[用户同步](/help/sites-administering/sync.md)时，用户数据是Sling分布。

## 正在从作者复制到发布{#replicating-from-author-to-publish}

复制到发布实例或调度程序需执行以下几个步骤：

* 作者要求发布（激活）某些内容；可以通过手动请求或预先配置的自动触发器来启动。
* 请求将传递给相应的默认复制代理；环境可以具有多个默认代理，这些代理将始终为此类操作选择。
* 复制代理将“打包”内容并将其置于复制队列中。
* 在“网站”选项卡中，为各个页面设置了[彩色状态指示符](/help/sites-authoring/publishing-pages.md#determining-publication-status)。
* 内容会从队列中提取，并使用配置的协议传输到发布环境;通常为HTTP。
* 发布环境的servlet接收请求并发布接收的内容；默认servlet为`http://localhost:4503/bin/receive`。
...
在某些情况下，需要一种称为反向复制的复制类型来将此环境返回到创作环境，从此数据将重新分发到其他发布。 出于安全考虑，必须严格控制发布到作者环境的任何通信。

反向复制使用发布环境中引用作者环境的代理。 此代理将数据放入发件箱。 此发件箱与创作环境中的复制侦听器匹配。 监听器会轮询发件箱以收集输入的任何数据，然后根据需要分发数据。 这可确保作者环境控制所有流量。

在其他情况下，例如对于“社区”功能（例如论坛、博客、评论和评论），使用复制很难在发布环境中输入用户生成的内容(UGC)的数量跨AEM实例高效同步。

AEM [ Communities](/help/communities/overview.md)从不对UGC使用复制。 相反，Communities的部署需要UGC的公用存储(请参阅[社区内容存储](/help/communities/working-with-srp.md))。

## 复制 — 开箱即用{#replication-out-of-the-box}

包含在AEM标准安装中的Geometrixx网站可用于说明复制。

要遵循此示例并使用默认复制代理，您需要[安装AEM](/help/sites-deploying/deploy.md)，具有：
...
代理将内容复制到默认发布实例。
   ...
   有关此（配置和日志）的详细信息，可从作者环境的“工具”控制台访问；或：
...
* 调度程序刷新 — 用于管理调度程序缓存。 有关详细信息，请参阅“创作”环境](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#invalidating-dispatcher-cache-from-the-authoring-environment)中的[ Invalidating Dispatcher Cache和Publishing Instance中的[ Invalidating Dispatcher Cache。](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#invalidating-dispatcher-cache-from-a-publishing-instance)

* [反向复制](#replicating-from-publish-to-author)  — 用于从发布复制到作者。反向复制不用于社区功能，如论坛、博客和评论。 由于未启用发件箱，因此会有效禁用它。 使用反向复制需要自定义配置。

* 静态代理 — 这是“将节点的静态表示存储到文件系统中的代理”。 例如，使用默认设置时，内容页面和dam资产会以HTML或相应资产格式存储在`/tmp`下。 有关配置，请参阅`Settings`和`Rules`选项卡。 请求它，以便直接从应用程序服务器请求页面时，可以看到内容。 这是一个专用代理，大多数情况下（可能）不需要此代理。
...
* **代理用户 ID**
   ...
   * 在发布环境上创建和写入内容
   ...
   此字段留空可使用系统用户帐户(sling中定义的帐户为管理员用户；默认情况下，此值为`admin`)。
   ...
   >[!CAUTION]
   >
   >对于发布环境上的代理，此帐户&#x200B;*必须*&#x200B;具有复制内容所需的创建/写访问权限。
...
* **接口**
   ...
   这将设置创建连接时使用的本地地址。 如果未设置，则使用默认地址。 这对于指定要在多宿主或群集系统上使用的接口很有用。

* **HTTP 方法**

   要使用的HTTP方法。

   对于调度程序刷新代理，这几乎总是GET，不应更改(POST是另一个可能的值)。
...
* **HTTP 头**
   ...
   * `CQ-Path:{path}`
   ...
   这些按钮（视情况而定）用于指示刷新手柄或路径时要使用的操作。 子参数是动态的：

   * `{action}` 指示复制操作
   * `{path}` 指示路径

   它们被与请求相关的路径/操作所取代，因此不需要“硬编码”：

   >[!NOTE]
   >
   >如果您在建议的默认上下文之外的上下文中安装了AEM，则需要在HTTP头中注册该上下文。 例如：
   >
...
* **修改**
   ...
   此处，修改页面时将自动触发此代理的复制。 这主要用于调度程序刷新代理，也用于反向复制。

* **在分发时**

   如果选中此项，代理将在修改时自动复制标记为要分发的任何内容。

* **到达开/关时间**

   这将在发生为页面定义的开始或结束时间时触发自动复制（以根据需要激活或取消激活页面）。 这主要用于调度程序刷新代理。

* **接收时**

   如果选中此项，代理将在每次收到复制事件时链式复制。
...
在创作环境的“工具”选项卡中，您可以配置驻留在创作环境（**作者**&#x200B;上的代理）或发布环境（**发布**&#x200B;上的代理）中的复制代理。 以下过程说明了作者环境的代理配置，但可用于两者。

>[!NOTE]
>
>当调度程序处理创作或发布实例的HTTP请求时，来自复制代理的HTTP请求必须包含PATH头。 除了以下过程之外，还必须将PATH头添加到客户端头的调度程序列表。 (请参阅[/clientheaders（客户端标头）](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#specifying-the-http-headers-to-pass-through-clientheaders)。 [](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#specifying-the-http-headers-to-pass-through-clientheaders)


1. 访问AEM中的&#x200B;**工具**&#x200B;选项卡。
1. 单击&#x200B;**复制**（左窗格可打开文件夹）。
1. 多次单击作者&#x200B;**上的**&#x200B;代理（左窗格或右窗格）。
1. 单击相应的代理名称（即链接）以显示有关该代理的详细信息。
1. 单击&#x200B;**编辑**&#x200B;打开配置对话框：

   ![chlimage_1-145](assets/chlimage_1-145.png)

1. 提供的值应足以用于默认安装。 如果进行更改，请单击&#x200B;**确定**&#x200B;以保存它们（有关各个参数的详细信息，请参阅[复制代理 — 配置参数](#replication-agents-configuration-parameters)）。

>[!NOTE]
>
>AEM的标准安装将`admin`指定为默认复制代理内传输凭据的用户。
>
...
出于安全原因，大多数网络拓扑不允许&#x200B;*从*“非军事区”（一种子网络，将外部服务公开给不受信任的网络，如Internet）连接。

由于发布环境通常位于DMZ中，要将内容返回到作者环境，必须从作者实例启动连接。 这可通过以下方式完成：

* 放置内容的发布环境中的&#x200B;*outbox*。
* 创作环境中的代理（发布），它定期轮询发布框以查找新内容。
...
**创作环境中的反向复制代** 理此组件用作活动组件，用于从发布环境的发件箱中收集信息：
...
安装后，已配置默认代理，以将内容复制到运行于localhost端口4503上的发布实例。
...
1. 单击&#x200B;**创建**&#x200B;以创建新代理。
1. 多次单击新代理项以打开配置面板。
1. 单击&#x200B;**编辑** — 将打开&#x200B;**代理设置**&#x200B;对话框 — **序列化类型**&#x200B;已定义为默认值，必须保持此状态。
...
然后，可以通过在创作环境中更新并发布页面来测试操作。

所有已配置为上述内容的发布实例上都将显示更新。

如果您遇到任何问题，可以检查创作实例上的日志。 根据所需的详细程度，您还可以使用上述的&#x200B;**代理设置**&#x200B;对话框将&#x200B;**日志级别**&#x200B;设置为`Debug`。

>[!NOTE]
>
>这可以与使用[代理用户ID](#settings)来选择不同内容以复制到单个发布环境结合使用。 对于每个发布环境:
>
>1. 配置复制代理以复制到该发布环境。
...
1. 打开AEM中的&#x200B;**工具**&#x200B;选项卡。
1. 单击&#x200B;**部署**。
1. 选择&#x200B;**复制**，然后选择&#x200B;**发布**&#x200B;上的代理。
1. 多次单击&#x200B;**调度程序刷新**&#x200B;项以打开概述。
1. 单击&#x200B;**编辑** — 将打开&#x200B;**代理设置**&#x200B;对话框：
   ...
   * 在&#x200B;**传输**&#x200B;选项卡中：
   ...
   >[!NOTE]
   >
   >如果您在建议的默认上下文之外的上下文中安装了AEM，则需要在&#x200B;**Extended**&#x200B;选项卡中配置[HTTP Headers](#extended)。

1. 单击&#x200B;**确定**&#x200B;以保存更改。
1. 返回至&#x200B;**工具**&#x200B;选项卡，从此处可以&#x200B;**激活**&#x200B;调度程序Flush **代理（**&#x200B;发布时的代理）**。**

**调度程序Flush**&#x200B;复制代理在创作时不活动。 您可以使用等效的URI在发布环境中访问同一页面；例如，`http://localhost:4503/etc/replication/agents.publish/flush.html`。

### 控制对复制代理{#controlling-access-to-replication-agents}的访问

对用于配置复制代理的页面的访问可以通过在`etc/replication`节点上使用用户和/或组页面权限来控制。
...
>[注意!]
>
>仅`/etc/replication`存储库位置支持创建复制代理。 为了正确处理关联的ACL，需要这样做。 在树的其他位置创建复制代理可能会导致未经授权的访问。
...
两个`agents`保存有关相应环境的配置信息，并且仅当该环境运行时才处于活动状态。 例如，`agents.publish`将仅用于发布环境。 以下屏幕截图显示了AEM WCM附带的创作环境中的发布代理：
...
* `[!UICONTROL Trigger Size]` -开始达到此大小限制时的批量复制（以MB为单位）。默认为5 MB。

## 其他资源 {#additional-resources}

有关疑难解答的详细信息，请阅读[复制疑难解答](/help/sites-deploying/troubleshoot-rep.md)页。
