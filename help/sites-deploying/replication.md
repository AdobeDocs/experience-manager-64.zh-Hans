...
* [将内容从作者](/help/sites-authoring/publishing-pages.md#publishing-pages) （激活）发布到发布环境。
...
* 在“网站”选项卡 [中，为各个页面](/help/sites-authoring/publishing-pages.md#determining-publication-status) 设置彩色状态指示符。
...
1. 单击 **创建** ，以创建新代理。
1. 多次-单击新代理项以打开配置面板。
1. 单 **击编辑** -将打 **开代理设置** 对话框——序列化类型 **已定义为** “默认”，但必须保留。
...
>[!NOTE]
>
>这可以与使用代理用户 [ID结合使用](#settings) ，以选择不同的内容，以便复制到单个发布环境。 对于每个发布环境:
>
>1. 配置复制代理以复制到该发布环境。
...
调度 **程序刷新** 复制代理在创作时不活动。 可以使用等效的URI在发布环境中访问同一页面； 例如 `http://localhost:4503/etc/replication/agents.publish/flush.html`,
