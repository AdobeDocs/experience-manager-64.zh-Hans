...
**AEM 6.4为“在** 线修订清除”过程 **的压缩阶段引入了两** 种新模式 **** :
...
AEM 6.4还在压缩过程中引入了更有效的内容重复数据删除机制，这进一步减少了存储库的磁盘空间占用。

以下两个图表展示了内部实验室测试的结果，这些测试结果说明与AEM 6.3相比，AEM 6.4中磁盘的平均执行时间和平均占用空间的减少：
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>TarMK的持久性格式将随AEM 6.4而改变。这些更改不需要主动迁移步骤。 现有存储库将进行滚动迁移，这对用户是透明的。 迁移过程是在AEM 6.4（或相关工具）首次访问存储库时启动的。</p> <p><strong>向AEM 6.4持久性格式的迁移启动后，存储库将无法恢复为以前的AEM 6.3持久性格式。</strong></p> </td> 
  </tr> 
 </tbody> 
</table>
