...
**AEM 6.4引** 入了 **“在** 线修订 **** 清理”流程的紧凑阶段的两个新模式：
...
下面两张图表显示了内部实验室测试的结果，这些测试说明与AEM 6.3相比，AEM 6.4中磁盘的平均执行时间和平均占用空间有所减少：
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>升级到AEM 6.4时，应该注意什么？</td> 
   <td><p>TarMK的持久性格式将随AEM 6.4而更改。这些更改不需要主动迁移步骤。 现有存储库将进行滚动迁移，这对用户是透明的。 迁移过程在AEM 6.4（或相关工具）首次访问存储库时启动。</p> <p><strong>迁移到AEM 6.4持久性格式后，存储库将无法恢复为之前的AEM 6.3持久性格式。</strong></p> </td> 
  </tr> 
 </tbody> 
</table>
