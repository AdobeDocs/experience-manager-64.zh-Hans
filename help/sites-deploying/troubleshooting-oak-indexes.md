...
强制关闭会导致AEM在重新启动后将异步索引挂起长达30分钟，并且通常需要另外15分钟才能完成第一次重新索引传递，总共大约45分钟(与 [初始检测](/help/sites-deploying/troubleshooting-oak-indexes.md#initial-detection) 45分钟)。 如果您怀疑在强制关闭后暂停了索引，则：
...
* 重新索引非常缓慢，直到日志文件中未报告与已遍历的节点数有关的显着进度。
