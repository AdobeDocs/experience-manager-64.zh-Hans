...
务必确保您运行的是受支持的操作系统、Java运行时、httpd和调度程序版本。 有关详细信息，请参阅[AEM 6.4技术要求页](/help/sites-deploying/technical-requirements.md)。 升级这些组件需要在您的项目计划中进行说明，并应在升级AEM之前完成。
...
AEM 6.4中的新增功能可在adobe.com[的AEM部分找到。 ](/help/release-notes/release-notes.md)请务必注意对组织中常用的UI或产品功能的任何更改。 在浏览新功能时，请注意任何对您的组织有价值的功能。 查看AEM 6.4中的变化后，为作者制定培训计划。 这可能涉及利用免费资源，如通过[Adobe数字学习服务](https://www.adobe.com/training.html)提供的帮助功能视频或正式培训。
...
AEM 6.4对存储库结构进行了更改，有助于使升级更加无缝。 这些更改包括根据Adobe或客户是否拥有该内容，将内容从/etc文件夹移出到包括/libs、/apps和/content在内的文件夹，从而限制在发布过程中覆盖内容的可能性。 在6.4升级时，存储库重组的方式不应要求进行代码更改，但建议在计划升级时查看AEM 6.4](/help/sites-deploying/repository-restructuring.md)中[存储库重组的详细信息。
...
初始评估后，接下来更全面的步骤可能是对测试实例执行升级并执行一些基本的烟雾测试。 Adobe也提供一些。 此外，[已弃用和已删除功能](/help/release-notes/deprecated-removed-features.md)的列表不仅应查看要升级到的版本，还应查看源版本和目标版本之间的任何版本。 例如，如果从AEM 6.2升级到6.4，则除了AEM 6.4的功能外，还应查看AEM 6.3已弃用和删除的功能。

![screen_shot_2018-04-04at120912](assets/screen_shot_2018-04-04at120912.png)

6.4中引入的图案检测器应能为您提供对大多数情况下在升级过程中预期结果的相当准确的估计。 但是，对于更复杂的自定义和部署，如果有不兼容的更改，您可以按照[执行就地升级](/help/sites-deploying/in-place-upgrade.md)中的说明将开发实例升级到AEM 6.4。 完成后，对此环境执行一些高级烟雾测试。 本练习的目标不是彻底完成测试用例清点并生成缺陷的正式清单，而是大致估计升级代码所需的工作量以实现6.4兼容性。 当与上一节中确定的[模式检测](/help/sites-deploying/pattern-detector.md)和架构更改相结合时，可以向项目管理团队提供粗略估计以规划升级。
...
我们为[升级代码和自定义](/help/sites-deploying/upgrading-code-and-customizations.md)提供了与AEM 6.4兼容的过程。在执行此迭代过程时，应根据需要对Runbook进行更改。 另请参阅AEM 6.4](/help/sites-deploying/backward-compatibility.md)中的[向后兼容性，了解在大多数情况下，自定义如何保持向后兼容，而无需在升级后立即进行开发。
