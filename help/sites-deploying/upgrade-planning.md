...
请务必确保您运行的是受支持的操作系统、Java运行时、httpd和Dispatcher版本。 有关更多信息，请参阅[AEM 6.4技术要求页面](/help/sites-deploying/technical-requirements.md)。 升级这些组件需要在您的项目计划中进行说明，并且应在升级AEM之前进行。
...
AEM 6.4的新增功能可在adobe.com](/help/release-notes/release-notes.md)的[AEM部分找到。 请务必注意对组织中常用的UI或产品功能所做的任何更改。 在您查看新增功能时，还应注意任何对贵组织有价值的功能。 查看AEM 6.4中的更改后，为您的作者制定培训计划。 这可能涉及利用免费提供的资源，如通过[Adobe数字学习服务](https://www.adobe.com/training.html)提供的helpx功能视频或正式培训。
...
AEM 6.4对存储库结构进行了更改，有助于使升级更加无缝。 更改涉及根据Adobe或客户是否拥有该内容，将内容从/etc文件夹移出到包括/libs、/apps和/content的文件夹，从而限制在发布期间覆盖内容的可能性。 存储库重组的完成方式是，在6.4升级时，它不应要求更改代码，不过建议在计划升级时查看AEM 6.4](/help/sites-deploying/repository-restructuring.md)中[存储库重组的详细信息。
...
初次评估后，更全面的下一步可能是对测试实例执行升级，并执行一些基本的烟雾测试。 Adobe还提供了一些。 此外，应不仅对要升级到的版本审查[已弃用和已删除功能](/help/release-notes/deprecated-removed-features.md)的列表，还应对源版本和目标版本之间的任何版本审查。 例如，如果从AEM 6.2升级到6.4，那么除了AEM 6.4的功能之外，还务必要查看已弃用和已删除的AEM 6.3功能。

![screen_shot_2018-04-04at120912](assets/screen_shot_2018-04-04at120912.png)

6.4中引入的模式检测器对于大多数情况，在升级过程中应提供相当准确的估计。 但是，对于更复杂的自定义和部署，如果您有不兼容的更改，则可以按照[执行就地升级](/help/sites-deploying/in-place-upgrade.md)中的说明将开发实例升级到AEM 6.4。 完成后，在此环境中执行一些高级烟雾测试。 本练习的目标不是详尽地完成测试用例清单并生成缺陷的正式清单，而是粗略地估计升级代码以实现6.4兼容性所需的工作量。 如果结合[模式检测](/help/sites-deploying/pattern-detector.md)和上一节中确定的体系结构变化，则可以向项目管理团队提供粗略估计，以规划升级。
...
我们为[升级代码和自定义](/help/sites-deploying/upgrading-code-and-customizations.md)提供了与AEM 6.4兼容的过程。在执行此迭代过程时，应根据需要对Runbook进行更改。 另请参阅AEM 6.4](/help/sites-deploying/backward-compatibility.md)中的[向后兼容性，以了解有关自定义如何在大多数情况下保持向后兼容，而无需在升级后立即进行开发的信息。
