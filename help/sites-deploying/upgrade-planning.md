...
请注意，可以直接从AEM版本6.0和最高6.4升级。运行5.6.x及更低版本的客户需要先升级到6.0或更高版本，建议使用6.0(SP3)。 此外，新的OAK Segment Tar格式现在用于自6.3起的Segment Node Store，并且即使对于6.0、6.1和6.2，也必须将存储库迁移到这种新格式。
...
务必确保您运行的是支持的操作系统、Java运行时、httpd和Dispatcher版本。 有关详细信息，请参阅 [AEM 6.4技术要求页](/help/sites-deploying/technical-requirements.md)。 升级这些组件需要在您的项目计划中说明，并且应在升级AEM之前完成。
...
AEM 6.4中的新增功能可在adobe.com [的AEM部分找到](/help/release-notes/release-notes.md)。 请务必注意对组织中常用的UI或产品功能所做的任何更改。 在浏览新增功能时，请注意任何对您的组织有价值的功能。 查看AEM 6.4中的更改后，为作者制定培训计划。 这可能涉及利用免费提供的资源，如通过 [Adobe数字学习服务提供的帮助功能视频或正式培训](https://www.adobe.com/training.html)。
...
查看AEM 6.4的技术要求，并确保您当前的硬件和软件已足够。 有关操作流程的潜在更改，请参阅以下文档：
...
AEM 6.4对存储库结构进行了更改，这有助于提高升级的无缝性。 这些更改包括根据Adobe或客户是否拥有该内容，将内容从/etc文件夹移出到包括/libs、/apps和/content在内的文件夹，从而限制在发布期间覆盖内容的可能性。 存储库重组的方式是，在6.4升级时不需要进行代码更改，但建议在计划升级时查看AEM 6.4中存储库重组的详细信息 [](/help/sites-deploying/repository-restructuring.md) 。
...
在初始评估后，下一步更全面的步骤可能是对测试实例执行升级并执行一些基本烟雾测试。 Adobe还提供了一些。 此外，不仅应查看 [要升级到的版本的已弃用和已删除功能列表](/help/release-notes/deprecated-removed-features.md) ，还应查看源版本和目标版本之间的任何版本的列表。 例如，如果从AEM 6.2升级到6.4，则除了AEM 6.4的功能之外，还应查看AEM 6.3已弃用和已删除的功能。

![screen_shot_2018-04-04at120912](assets/screen_shot_2018-04-04at120912.png)

6.4中引入的图案检测器应该能够相当准确地估计大多数情况下在升级期间的预期情况。 但是，对于更复杂的自定义和部署，如果您有不兼容的更改，则可以根据执行就地升级中的说明将开发实例 [升级到AEM 6.4](/help/sites-deploying/in-place-upgrade.md)。 完成后，在此环境中执行一些高级烟雾测试。 本练习的目标不是彻底完成测试用例清点并生成缺陷的正式清单，而是大致估计升级代码所需的工作量以实现6.4兼容性。 结合模式检 [测和上一节中确定的架构更改](/help/sites-deploying/pattern-detector.md) ，可以向项目管理团队提供粗略估计以规划升级。
...
我们提供了升级代 [码和自定义的过程](/help/sites-deploying/upgrading-code-and-customizations.md) ，以便与AEM 6.4兼容。执行此迭代过程时，应根据需要对操作手册进行更改。 另请参 [阅AEM 6.4中的向后兼容性](/help/sites-deploying/backward-compatibility.md) ，以了解自定义在大多数情况下如何保持向后兼容，而无需在升级后立即进行开发。
