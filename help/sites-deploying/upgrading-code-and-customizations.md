...
1. **模式检测** 器 — 如升级计划中所述并在本页中详细描述，运行模式检测器 [](/help/sites-deploying/pattern-detector.md) 以获取模式检测器报告，该报告包含除AEM目标版中不可用的API/捆绑包外，还需要解决的区域的详细信息。“模式检测”报告应当指示您的代码中存在任何不兼容现象，如果没有不兼容，则您的部署已兼容6.4，您仍可以选择使用6.4功能进行新开发，但您不需要它仅用于保持兼容性。 如果报告了不兼容项，则您可以选择a)以兼容模式运行，并推迟开发以获得新的6.4功能或兼容性，b)在升级后决定进行开发，然后转到步骤2。 有关详细信息，请参阅AEM 6.4](/help/sites-deploying/backward-compatibility.md)中的[向后兼容性。

1. **为6.4开发代码库**  — 为目标版本的代码库创建专用分支或存储库。使用升级前兼容性中的信息来规划要更新的代码区域。
1. **使用6.4 Uberjar**  — 更新代码库POM以指向6.4 uber jar编译代码。
1. **更新AEM自定义**  — 对AEM的任何自定义或扩展应更新/验证，以在6.4中运行并添加到6.4代码库。包括UI搜索Forms、资产自定义以及使用/mnt/overlay的任何内容

1. **部署到6.4环境**  — 应在开发/QA环境中启动AEM 6.4（作者+发布）的干净实例。应部署更新的代码库和具有代表性的内容示例（来自当前生产）。
1. **QA验证和错误修复** - QA应验证6.4的作者实例和发布实例上的应用程序。发现的任何错误都应被修复并提交到6.4代码库。根据需要重复开发周期，直到修复所有错误。

在继续升级之前，您应拥有一个稳定的应用程序代码库，该库已针对AEM的目标版本进行了彻底测试。 根据测试中的观察，可以有方法优化自定义代码。 这可能包括重构代码以避免遍历存储库、自定义索引以优化搜索或在JCR中使用无序节点等。

除了升级代码库和自定义以与新AEM版本一起使用的选项外，6.4还有助于使用[本页](/help/sites-deploying/backward-compatibility.md)中所述的向后兼容性功能更有效地管理自定义。
...
经典UI创作仍在AEM 6.4中可用，但现已弃用。 有关详细信息，请在[此处](/help/release-notes/deprecated-removed-features.md#pre-announcement-for-next-release)找到。 如果您的应用程序当前在经典UI作者环境上运行，则建议升级到AEM 6.4并继续使用经典UI。 然后，可以将迁移到触屏用户界面的计划为单独项目，以在多个开发周期中完成。 要在AEM 6.4中使用经典UI，需要将多个OSGi配置提交到代码库。 有关如何配置此项的详细信息，请在[此处](/help/sites-administering/enable-classic-ui.md)找到。

>[!NOTE]
>
>要帮助您摆脱经典UI并充分利用最新的AEM技术，请考虑利用[AEM现代化工具](/help/sites-developing/modernization-tools.md)来简化迁移。
...
要为现有资产生成资产ID，请在您将AEM实例升级为运行AEM 6.4时升级资产。要启用[资产分析功能](/help/assets/touch-ui-asset-insights.md)，此操作是必需的。 有关详细信息，请参阅[添加嵌入代码](/help/assets/touch-ui-using-page-tracker.md#adding-embed-code)。
...
ContextHub配置通过升级实现。 有关如何恢复现有ContextHub配置的说明，请在[此处](/help/sites-administering/contexthub-config.md#recovering-contexthub-configurations-after-upgrading)找到。

### 工作流自定义{#workflow-customizations}

更新开箱即用的修改工作流以添加或删除不需要的功能是一种常见做法。 DAM更新资产工作流是自定义的常见工作流。 应备份自定义实现所需的所有工作流并以版本控制存储，因为它们可能会在升级过程中被覆盖。
...
可编辑模板的结构在AEM 6.2和6.3之间发生了更改。如果您是从6.2或更低版本升级，并且如果您的站点内容是使用可编辑模板构建的，则需要使用[响应式节点清理工具](https://github.com/Adobe-Marketing-Cloud/aem-sites-template-migration)。 该工具用于在&#x200B;_**升级后运行**_&#x200B;以清理内容。 它需要同时在作者层和发布层上运行。

### CUG实施更改{#cug-implementation-changes}

已关闭的用户组的实施已发生重大变化，可解决先前版本的AEM中的性能和可伸缩性限制。 6.3中已弃用CUG的早期版本，并且仅在触屏UI中支持新实现。 如果从6.2或更低版本升级，则[此处](/help/sites-administering/closed-user-groups.md#upgrade-cug)可找到迁移到新CUG实现的说明。
