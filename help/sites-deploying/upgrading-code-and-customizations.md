...
1. **模式检测器**[](/help/sites-deploying/pattern-detector.md) -按照升级计划中的说明运行模式检测器，并在本页中进行详细介绍，以获取模式检测器报告，其中包含除AEM的Target版本中不可用的API/包外，还需要解决的区域的更多详细信息。 “模式检测”报告应指示代码中的任何不兼容性，如果没有，则您的部署已经兼容6.4，您仍然可以选择使用6.4功能进行新开发，但您不需要它来保持兼容性。 如果报告了不兼容性，则您可以选择a)以兼容性模式运行并推迟开发以获得新的6.4功能或兼容性，b)在升级后决定进行开发，然后转到步骤2。 请参阅AEM 6.4 [中的向后兼容性](/help/sites-deploying/backward-compatibility.md) ，了解更多详细信息。

1. **为6.4开发代码库** -为Target版本的代码库创建专用分支或存储库。 使用升级前兼容性中的信息来规划要更新的代码区域。
1. **使用6.4 Uber jar进行编译** -将代码库POM更新为指向6.4 uber jar并针对此编译代码。
1. **更新AEM自定义** -应更新／验证AEM的任何自定义或扩展，以便在6.4中工作，并将其添加到6.4代码库。 包括UI搜索表单、资产自定义，以及使用/mnt/overlay的任何内容

1. **部署到6.4环境** -应在开发/QA环境中启动AEM 6.4（作者+发布）的干净实例。 应部署更新的代码库和代表性的内容示例（来自当前生产）。
1. **QA验证和错误修复** - QA应验证6.4的作者实例和Publish实例上的应用程序。发现的所有错误应被修复并提交到6.4代码库。 根据需要重复开发周期，直到修复所有错误。

在继续升级之前，您应该拥有一个稳定的应用程序代码库，该库已针对目标版本的AEM进行了彻底测试。 根据测试中的观察结果，可以有方法优化自定义代码。 这可能包括重构代码以避免遍历存储库、自定义索引以优化搜索，或在JCR中使用无序节点等。

除了升级代码库和自定义以与新AEM版本一起使用的选项外，6.4还帮助使用本页所述的向后兼容性功能更有效地管理 [自定义](/help/sites-deploying/backward-compatibility.md)。

如上所述，如下图所示，在第一步中运行 [Pattern Detector](/help/sites-deploying/pattern-detector.md) ，将帮助您评估升级的整体复杂性，以及您是希望以兼容模式运行还是更新自定义以使用所有新AEM 6.4功能。 有关详细信息， [请参阅AEM 6.4中的向后兼容性](/help/sites-deploying/backward-compatibility.md) 。
...
AEM实施所需的所有代码和配置都应使用某种形式的版本控制进行管理。 应创建版本控制中的专用分支，以管理目标版本AEM中的代码库所需的任何更改。 将在此分支中管理针对AEM目标版本的代码库的迭代测试和后续错误修复。
...
在AEM 6.0之前的代 `SlingRepository.loginAdministrative()` 码库 `ResourceResolverFactory.getAdministrativeResourceResolver()` 中，使用管理会话的情况非常普遍。由于安全原因，这些方法的访问级别过于宽泛，因此已经弃用。 [在Sling的未来版本中，这些方法将被删除](https://sling.apache.org/documentation/the-sling-engine/service-authentication.html#deprecation-of-administrative-authentication)。 强烈建议重新构造任何代码以改用服务用户。 有关服务用户以及如 [何逐步退出管理会话的更多信息，请参阅此处](/help/sites-administering/security-service-users.md#how-to-phase-out-admin-sessions)。

### 查询和Oak索引 {#queries-and-oak-indexes}

在升级代码库时，任何对代码库的查询的使用都需要经过彻底的测试。 对于从Jackrabbit 2（AEM版本低于6.0）升级的客户，这一点尤其重要，因为Oak不会自动索引内容，并且可能需要创建自定义索引。 如果从AEM 6.x版本升级，开箱即用的Oak索引定义可能已更改，并可能影响现有查询。
...
经典UI创作仍在AEM 6.4中可用，但已弃用。 此处可找到更多 [信息](/help/release-notes/deprecated-removed-features.md#pre-announcement-for-next-release)。 如果应用程序当前在经典UI创作环境中运行，则建议升级到AEM 6.4并继续使用经典UI。 然后，可以将迁移到触屏UI计划为单独的项目，以在多个开发周期中完成。 要在AEM 6.4中使用经典UI，需要将多个OSGi配置提交到代码库。 有关如何配置此组件的更多详细信息，请在此 [处找到](/help/sites-administering/enable-classic-ui.md)。

>[!NOTE]
>
>要帮助您摆脱经典UI并利用最新的AEM技术，请考虑利用 [AEM Medranceization Tools](/help/sites-developing/modernization-tools.md) ，使迁移更轻松。
...
通常，通过在/libs下覆盖节点和／或文件（在/apps下包含其他节点），将AEM功能扩展到开箱即用。 应在版本控制中跟踪这些叠加，并针对AEM的目标版本进行测试。 如果文件（无论是JS、JSP、HTL）被覆盖，建议对已增强的功能留下注释，以便更轻松地在目标版本的AEM上进行回归测试。 有关一般叠加的更多信息，请访 [问](/help/sites-developing/overlays.md)。 有关特定AEM叠加的说明，请参阅下文。
...
要为现有资产生成资产ID，请在将AEM实例升级以运行AEM 6.4时升级资产。这是启用资产分析功 [能所必需的](/help/assets/touch-ui-asset-insights.md)。 有关详细信息，请参阅 [添加嵌入代码](/help/assets/touch-ui-using-page-tracker.md#adding-embed-code)。
...
更新开箱即用的修改工作流以添加或删除不需要的功能是一种常见做法。 DAM更新资产工作流是自定义的常见工作流。 自定义实施所需的所有工作流都应备份并存储在版本控制中，因为升级过程中可能会覆盖这些工作流。
...
可编辑模板的结构在AEM 6.2和6.3之间发生了更改。如果您是从6.2或更低版本升级，并且站点内容是使用可编辑的模板构建的，则需要使用响应式节 [点清理工具](https://github.com/Adobe-Marketing-Cloud/aem-sites-template-migration)。 该工具用于在升级 **_后运行_** ，以便清理内容。 它需要同时在作者层和发布层上运行。

### CUG实施更改 {#cug-implementation-changes}

已关闭的用户组的实施已发生重大更改，以解决AEM早期版本中的性能和可伸缩性限制。 CUG的先前版本在6.3中已弃用，新实现仅在触屏UI中受支持。 如果您是从6.2或更低版本升级，则此处可找到迁移到新CUG实施的说 [明](/help/sites-administering/closed-user-groups.md#upgrade-cug)。

## 测试过程 {#testing-procedure}

应为测试升级准备一个全面的测试计划。 测试升级后的代码库和应用程序首先需要在较低的环境中完成。 发现的所有错误应以迭代方式修复，直到代码库稳定，然后才应升级更高级别的环境。

### 测试升级过程 {#testing-the-upgrade-procedure}

如您的自定义运行手册中所述，应在开发和QA环境中测试此处所述的升级过程(请参阅 [规划您的升级](/help/sites-deploying/upgrade-planning.md))。 应重复升级过程，直到升级运行手册中记录了所有步骤并且升级过程平稳。
...
应创建涵盖上述实施测试区域的测试计划。 在很多情况下，按作者和发布任务列表分隔测试计划是有意义的。 在升级生产环境之前，应在开发、QA和Stage环境上执行此测试计划。 应在较低的环境中捕获测试结果和性能指标，以便在升级Stage和Production环境时进行比较。
