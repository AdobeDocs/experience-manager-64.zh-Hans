...
有关最新版本的更改和修复的列表，请参阅 [CRX2Oak发行说明](/help/release-notes/crx2oak.md)。

>[!NOTE]
>
>有关Apache Oak和AEM持久性的主要概念的更多信息，请 [参阅AEM平台简介](/help/sites-deploying/platform.md)。
...
支持使用外部Blob存储（通常称为数据存储）迁移存储库，这些支持采用不同的组合。 一个可能的迁移路径是从使用外部的CRX2存储库 `FileDataStore` 到使用外部的Oak存储库 `S3DataStore`。
...
>[!CAUTION]
>
>但是，Windows平台不支持内存映射操作。 因此，建议在Windows上执 **行迁移时添加** —disable-mmap参数。
...
如果数据需要在两个存储库之间复制，并且您的内容路径在两个实例上都不同，则可以在参数中定 `--merge-path` 义它。 一旦您这样做，CRX2Oak将仅将新节点复制到目标存储库，并将保留旧节点。
...
对于这些类型的情况，一个有用的特征是添加参 `--copy-versions` 数。 它可用于在迁移或复制存储库期间跳过版本节点。

您还可以通过添加来选择是否复制孤立版本 `--copy-orphaned-versions=true`。

这两个参数还支 `YYYY-MM-DD` 持日期格式，以防复制版本至迟于特定日期。
...
* `--ldap`: 将LDAP用户从CQ 5.x实例迁移到基于Oak的实例。 要使此功能正常工作，需要将Oak配置中的标识提供者命名为ldap。 For more information, see the [LDAP documentation](/help/sites-administering/ldap-config.md).

* `--ldap-config:` 结合使用多个LDAP服 `--ldap` 务器进行身份验证的CQ 5.x存储库的参数使用此参数。 您可以使用它指向CQ 5.x或 `ldap_login.conf` 配置 `jaas.conf` 文件。 格式为 `--ldapconfig=path/to/ldap_login.conf`。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>快速启动模式</td> 
   <td>运行CRX2Oak <strong>时，可以向命</strong> 令行添加日志级TRACE <strong></strong>或日志级调试选项。 在此模式下，日志会自动重定向 <strong>到upgrade.log文件</strong>。</td> 
  </tr> 
  <tr> 
   <td>独立模式</td> 
   <td><p>向CRX2Oak命 <strong>令行添加</strong> —跟踪选项，以在标准输出上显示TRACE事件(您需要使用重定向字符重定向日志： “&gt;”或“tee”命令，供以后检查)。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
迁移到MongoDB复制副本集时，请确保在与Mongo数据库 `WriteConcern` 的所有 `2` 连接上都将参数设置为。

可通过在连接字符串 `w=2` 的末尾添加参数来执行此操作，如下所示：
...
>[!NOTE]
>
>有关详细信息，请参阅MongoDB连接字符串文档中的写 [入问题](https://docs.mongodb.org/manual/reference/connection-string/#write-concern-options)。
