...
>[!NOTE]
>
>Client Context已被ContextHub在触屏UI中取代。 有关详细信息，请参阅[相关文档](/help/sites-developing/contexthub.md)。
...
* 客户端Web浏览器。
* 服务器（有关存储来自第三方源的信息，请参见[JSONP存储](/help/sites-administering/client-context.md)）
...
获取会话存储对象以读取或写入存储。 [`CQ_Analytics.ClientContextMgr`](/help/sites-developing/ccjsapi.md#cq-analytics-clientcontextmgr) 根据商店名称提供对商店的访问。获取后，使用[`CQ-Analytics.SessionStore`](/help/sites-developing/ccjsapi.md#cq-analytics-sessionstore)或[`CQ-Analytics.PersistedSessionStore`](/help/sites-developing/ccjsapi.md#cq-analytics-persistedsessionstore)的方法与存储数据交互。
