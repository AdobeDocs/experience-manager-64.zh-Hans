...
>[!NOTE]
>
>Client Context已在触屏UI中被ContextHub取代。 有关详细信息，请参阅[相关文档](/help/sites-developing/contexthub.md)。
...
* 客户端Web浏览器。
* 服务器（请参阅[JSONP存储](/help/sites-administering/client-context.md)以存储来自第三方源的信息）
...
获取会话存储对象以读取或写入存储数据。 [`CQ_Analytics.ClientContextMgr`](/help/sites-developing/ccjsapi.md#cq-analytics-clientcontextmgr) 提供对基于商店名称的商店的访问。获取后，使用[`CQ-Analytics.SessionStore`](/help/sites-developing/ccjsapi.md#cq-analytics-sessionstore)或[`CQ-Analytics.PersistedSessionStore`](/help/sites-developing/ccjsapi.md#cq-analytics-persistedsessionstore)的方法与存储数据进行交互。
...
1. 右键单击`/apps/myapp/contextstores/geoloc`节点，然后单击&#x200B;**创建 — >创建文件**。 指定名称init.js.jsp ，然后单击“确定”。
