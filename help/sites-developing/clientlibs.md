...
默认情况下， `cq:ClientLibraryFolder` 节点可置于 `/apps`, `/libs` 和 `/etc` 存储库的子树(这些默认值和其他设置可通过 **AdobeGraniteHTML库管理器** 面板 [系统控制台](http://localhost:4502/system/console/configMgr))。
...
>[!CAUTION]
>
>`<cq:includeClientLib>`自AEM 5.6以来，已弃用该函数，过去常用于包含客户端库。 [ `<ui:includeClientLib>`](/help/sites-developing/taglib.md#amp-lt-ui-includeclientlib) 应使用，如上所述。
...
>[!NOTE]
>
>客户端库文件夹下的静态资源必须位于名为 *资源*. 如果文件夹下没有静态资源（如图像） *资源*，则无法在发布实例上引用它。 示例如下：http://localhost:4503/etc.clientlibs/geometrixx/components/clientlibs/resources/example.gif
...
1. 转到Apache Felix配置管理器(位于 [http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr)
...
有关配置服务的信息，请参阅 [AEMHTML库管理器](/help/sites-deploying/osgi-configuration-settings.md).
