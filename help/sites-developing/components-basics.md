...
* 您是需要从头开始创建组件，还是可以从现有组件继承基础知识？
   ...
   * 没有必要再次发明轮子。
   * AEM提供了多种机制，允许您继承和扩展其他组件定义的详细信息，包括覆盖、叠加和 [Sling资源合并器](/help/sites-developing/sling-resource-merger.md).
...
* 我应该考虑哪些安全方面？
   ...
   * 请参阅 [安全检查列表 — 开发最佳实践](/help/sites-administering/security-checklist.md#development-best-practices) 以了解更多详细信息。
...
* **触屏优化UI**
   [标准用户界面](/help/sites-developing/touch-ui-concepts.md) 在AEM 5.6.0中作为预览版引入，并在6.x中进行了扩展。它基于Adobe Marketing Cloud的统一用户体验，使用 [Coral用户界面](/help/sites-developing/touch-ui-concepts.md#coral-ui) 和 [Granite用户界面](/help/sites-developing/touch-ui-concepts.md#granite-ui).

* **经典UI**
基于CQ 5.1中引入的ExtJS技术的用户界面。

请参阅 [面向客户的UI界面Recommendations](/help/sites-deploying/ui-recommendations.md) 以了解更多详细信息。
...
>[!NOTE]
>
>Adobe建议利用触屏优化UI从最新技术中受益。 [AEM现代化工具](modernization-tools.md) 可以更轻松地进行迁移。

### 内容逻辑和渲染标记  {#content-logic-and-rendering-markup}

建议将负责标记和渲染的代码与用于控制组件内容选择逻辑的代码分开。

这一理念得到了 [HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html)，一种模板语言，有意限制该语言，以确保使用真实的编程语言来定义底层业务逻辑。 可通过特定命令从HTL调用此（可选）逻辑。 此机制会突出显示为给定视图调用的代码，并在必要时，允许对同一组件的不同视图使用特定逻辑。

### HTL与JSP {#htl-vs-jsp}

HTL是AEM 6.0中引入的一种HTML模板语言。

是否使用的探讨 [HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html) 或JSP（Java服务器页面）开发您自己的组件时应该直截了当，因为HTL现在是AEM的推荐脚本语言。
...
* [开发组件](/help/sites-developing/developing-components-samples.md) （重点关注触屏优化UI）
...
* [使用包管理器](/help/sites-administering/package-manager.md) 将组件添加到包中，并将其移动到其他AEM实例。
...
* AEM标准组件的定义为 `cq:Component` 并具有关键元素：
   ...
   * jcr属性：
      ...
      jcr属性列表；这些是变量，虽然组件节点的基本结构、其属性和子节点由 `cq:Component` 定义
...
* **重要属性**:
   ...
   * 触屏 UI
      ...
      * 请参阅部分 [触屏UI中的组件图标](/help/sites-developing/components-basics.md#component-icon-in-touch-ui) 以了解详细信息。
...
1. `cq:icon`  — 指向 [Coral用户界面库](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/coral-ui/coralui3/Coral.Icon.html) 在组件浏览器中显示
...
的 [组件控制台](/help/sites-authoring/default-components-console.md#component-details) 显示特定组件的图标的定义方式。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>当前组件. 组件是节点类型 <code>cq:Component</code>.<br /> </td> 
  </tr> 
  <tr> 
   <td><code>componentGroup</code></td> 
   <td><code>String</code></td> 
   <td>可在组件浏览器（触屏优化UI）或Sidekick（经典UI）中选择组件的组。<br /> 值 <code>.hidden</code> 用于无法从UI中进行选择的组件，例如实际的段落系统。</td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><code>String</code></td> 
   <td>用于在组件没有对话框节点时覆盖大小写的对话框路径。<br /> </td> 
  </tr> 
  <tr> 
   <td> </td> 
   <td> </td> 
  ...
  <tr> 
   ...
   <td><code>String</code></td> 
   <td>如果已设置，则此属性将被用作单元格ID。 有关详细信息，请参阅知识库文章 <a href="https://helpx.adobe.com/experience-manager/kb/DesigneCellId.html">如何构建设计单元格ID</a>.<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><code>String</code></td> 
   <td>从组件浏览器或Sidekick添加组件时，要用作内容模板的节点的路径。 此路径必须是绝对路径，而不是相对于组件节点。<br /> 除非您希望重复使用其他位置已有可用的内容，否则不需要这样做， <code>cq:template</code> 足够了（见下文）。</td> 
  </tr> 
  <tr> 
...
* `cq:childEditConfig` ( `cq:EditConfig`) — 此选项可控制没有其自定义的子组件的可视化方面
...
>* 的 [AEM现代化工具](/help/sites-developing/modernization-tools.md) 还提供用于帮助您扩展/转换仅为经典UI定义对话框的组件。
>
...
* 触屏优化UI
   ...
   * `cq:dialog` ( `nt:unstructured`)节点：
      ...
      * 特定于触屏优化UI
      * 使用Granite UI组件进行定义
      * 拥有资产 `sling:resourceType`，作为标准Sling内容结构
      * 可以拥有资产 `helpPath` 定义在“帮助”图标(? 图标)。

         * 对于现成的组件，通常会引用文档中的页面。
         * 如果否 `helpPath` ，则会显示默认URL（文档概述页面）。
...
* 经典 UI
   ...
   * 您可以将对话框创建为 `cq:Dialog`，将提供单个选项卡 — 如在文本组件中一样，或者如果您需要多个选项卡，如在文本时间组件中一样，该对话框可定义为 `cq:TabPanel`.
   * a `cq:WidgetCollection` ( `items`)用于为任一输入字段( `cq:Widget`)或其他选项卡( `cq:Widget`)。 可以扩展此层次结构。
...
[设计模式下提供了设计对话框](/help/sites-authoring/default-components-designmode.md)，但并非所有组件(例如 **标题** 和 **图像** 都有设计对话框，而 **文本** 不会。
...
1. 将所需的组件添加到 `components` 模板定义的属性：
...
* **资源类型层次结构**
   ...
   用于使用属性扩展组件 `sling:resourceSuperType`. 这允许组件继承。 例如，文本组件将从标准组件继承各种属性。
...
* [ `cq:editConfig` 子节点](#configuring-with-cq-editconfig-child-nodes):
   ...
   * `cq:dropTargets` （节点类型） `nt:unstructured`):定义可以接受内容查找器资产中删除的放置目标列表
...
* 要查找 `cq:editConfig` 节点，例如 `cq:actions`，则可以在 **CRXDE Lite** 和使用以下XPath查询字符串进行搜索：

   `//element(cq:editConfig, cq:EditConfig)[@cq:actions]`

* 查找的子节点 `cq:editConfig`，例如，您可以搜索 `cq:dropTargets`，类型为 `cq:DropTargetConfig`;您可以在**CRXDE Lite**中使用查询工具，并使用以下XPath查询字符串进行搜索：
...
的 `cq:actions` 属性( `String array`)定义可对组件执行的一个或多个操作。 以下值可用于配置：
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>显示静态文本值 &lt;some text=""&gt;<br /> 仅在经典UI中可见。 触屏优化UI在上下文菜单中不显示操作，因此不适用。</td> 
  </tr> 
  <tr> 
   <td>-</td> 
  ...
  <tr> 
   ...
   <td>添加按钮以编辑组件。</td> 
    ...
    <td>添加按钮以编辑组件，并允许 <a href="/help/sites-authoring/annotations.md">批注</a>.</td>
   </tr>
  <tr> 
...
的 `cq:layout` 属性( `String`)定义如何在经典UI中编辑组件。 以下值可用：
...
该组件可以链接到编辑对话框。 的 `cq:dialogMode` 属性( `String`)定义组件对话框在经典UI中的打开方式。 以下值可用：
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><code>auto</code></td> 
   <td>如果组件宽度小于客户端 <code>CQ.themes.wcm.EditBase.INLINE_MINIMUM_WIDTH</code> 值时，对话框是浮动的，否则它是内联的。</td> 
  </tr> 
 </tbody> 
</table>
...
的 `cq:emptyText` 属性( `String`)定义不显示可视内容时显示的文本。 默认为： `Drag components or assets here`.

### cq:inherit {#cq-inherit}

的 `cq:inherit` 属性( `boolean`)定义是否从其继承的组件继承缺少的值。 默认为 `false`.

### dialogLayout {#dialoglayout}

的 `dialogLayout` 属性定义默认情况下应如何打开对话框。
...
的 `cq:dropTargets` 节点（节点类型） `nt:unstructured`)定义一个放置目标列表，以接受从内容查找器中拖动的资产中的放置。 它用作类型节点的集合 `cq:DropTargetConfig`.
...
类型的每个子节点 `cq:DropTargetConfig` 在组件中定义放置目标。 节点名称很重要，因为必须在JSP中使用它，如下所示，来生成分配给DOM元素的CSS类名称，该元素是有效的放置目标：
...
的 `<*drag and drop prefix*>` 由Java属性定义：
...
下载组件中提供了以下配置。 它会启用 `media` 组。 放置后，组件属性 `fileReference` 正在更新：
...
的 `cq:actionConfigs` 节点（节点类型） `nt:unstructured`)定义附加到由定义的列表的新操作列表 `cq:actions` 属性。 的每个子节点 `cq:actionConfigs` 通过定义小组件定义新操作。
...
* 名为的按钮 **管理评论** 运行处理程序函数的 `CQ_collab_forum_openCollabAdmin()`.
...
>[!NOTE]
>
>请参阅 [将新操作添加到组件工具栏](/help/sites-developing/customizing-page-authoring-touch.md#add-new-action-to-a-component-toolbar) 作为触屏UI的示例。

### cq:formParameters {#cq-formparameters}

的 `cq:formParameters` 节点（节点类型） `nt:unstructured`)定义添加到对话框表单的其他参数。 每个属性都会映射到一个表单参数。

以下配置添加了一个名为 `name`，使用值设置 `photos/primary` 对话框窗体：
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>(<code>boolean</code>)为True以启用组件的就地编辑。</td> 
  </tr> 
  <tr> 
...
>[!NOTE]
>
>的 `REFRESH_INSERTED` 和 `REFRESH_SELFMOVED` 处理程序仅在经典UI中可用。
...
>[!NOTE]
>
>对于嵌套的组件，对于定义为 `cq:listeners` 节点：
>
>* 对于嵌套的组件，以下属性的值 *必须* be `REFRESH_PAGE`:
>
>  * `aftermove`
>  * `aftercopy`
