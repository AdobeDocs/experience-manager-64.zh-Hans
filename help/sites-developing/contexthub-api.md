...
[ContextHub.Utils.Eventing](/help/sites-developing/contexthub-api.md#contexthub-utils-eventing)对象。 使用此对象绑定函数以存储事件。 有关默认值和初始化的信息，请参见[init(name,config)](/help/sites-developing/contexthub-api.md#init-name-config)。
...
ContextHub.Utils.Persistence对象。 有关默认值和初始化的信息，请参见[`init(name,config)`](/help/sites-developing/contexthub-api.md#init-name-config)。
...
* **选项** :（对象）传递给setItem函数的选项的可选对象。有关信息，请参阅[setItem(key,value,options)](/help/sites-developing/contexthub-api.md#setitem-key-value-options)的`options`参数。
...
删除密钥后，函数将触发`data`事件。 事件数据包括存储名称、已删除的键的名称、已删除的值、键的新值(null)以及“remove”的操作类型。
...
* **选项：** （对象）选项的对象。以下对象属性有效：
   ...
   * 静默：值`true`可防止触发`data`事件。 默认值为 `false`.
...
包含JSON数据的商店。 从外部JSONP服务或返回JSON数据的服务（可选）检索数据。 创建此类的实例时，使用[ `init`](/help/sites-developing/contexthub-api.md#init-name-config)函数指定服务详细信息。
...
* **override:** (Boolean)。值`true`将使现有服务配置被替换为属性`serviceConfig`。 值`false`将使现有服务配置属性与属性`serviceConfig`合并。
...
* **host:** （字符串）服务器名称或IP地址。
* **jsonp:** (Boolean)值true表示服务是JSONP服务，否则为false。如果为true，则{callback:&quot;ContextHub.Callbacks.*Object.name*}对象添加到service.params对象。

* **params:** （对象）URL参数表示为对象属性。参数名称是属性名称，参数值是属性值。
* **path:** （字符串）服务的路径。
* **port:** (Number)服务的端口号。
...
* **resolve:** （布尔值）确定是否在URL中包含已解析的参数。值`true`解析参数，而`false`解析参数不。
...
* **name:** （字符串）商店的名称。
* **config:** (Object)包含服务属性的对象。JSONPStore对象使用`service`对象的属性来构建JSONP服务的URL:
...
查询远程JSONP服务并缓存响应。 如果此函数上次调用后的时间小于`config.service.ttl`的值，则不调用服务，并且不更改缓存响应。 或者，您也可以强制调用服务。 调用[init](/help/sites-developing/contexthub-api.md#init-name-config)函数初始化存储时设置`config.service.ttl`属性。

完成事件时触发就绪查询。 如果未设置JSONP服务URL，则函数不执行任何操作。

**参数**

* **reload:** (Boolean)值true将删除缓存响应并强制调用JSONP服务。
...
ContextHub.Store.PersitedJSONPStore扩展了[ContextHub.Store.JSONPStore](/help/sites-developing/contexthub-api.md#contexthub-store-jsonpstore)，因此它继承了该类的所有函数。 但是，根据ContextHub持久性的配置，从JSONP服务检索的数据会被保留。 （请参阅[持久性模式](/help/sites-developing/ch-adding.md#persistence-modes)。）

## ContextHub.Store.PersistedStore {#contexthub-store-persistedstore}

ContextHub.Store.PersitedStore扩展了[ContextHub.Store.Core](/help/sites-developing/contexthub-api.md#contexthub-store-core)，因此它继承了该类的所有函数。 此存储中的数据会根据ContextHub持久性的配置进行保留。

## ContextHub.Store.SessionStore {#contexthub-store-sessionstore}

ContextHub.Store.SessionStore扩展了[ContextHub.Store.Core](/help/sites-developing/contexthub-api.md#contexthub-store-core)，因此它继承了该类的所有函数。 此存储中的数据将使用内存中的持久性（Javascript对象）进行保留。
...
向ContextHub注册UI模块渲染器。 注册呈示器后，它可用于[创建UI模块](/help/sites-administering/contexthub-config.md#adding-a-ui-module)。 当您是[扩展ContextHub.UI.BaseModuleRenderer](/help/sites-developing/ch-extend.md#creating-contexthub-ui-module-types)以创建自定义UI模块渲染器时，请使用此函数。

**参数**

* **moduleType:** (String)UI模块渲染器的标识符。如果渲染器已使用指定的值进行注册，则在注册此渲染器之前，将先取消注册现有渲染器。
* **renderer:** （字符串）呈现UI模块的类的名称。
* **dontRender:(** Boolean)设置为 `true` 阻止在注册呈示器后呈现ContextHub UI。默认值为 `false`.
...
Cookie值，如果找不到该键的Cookie，则为`null`。
...
* **过滤器：** 匹配cookie键的条件。支持以下类型：
   ...
   * 阵列：数组中的每个项都是过滤器。
   * RegExp对象：对象的测试函数用于匹配cookie键。
   * 函数：测试Cookie密钥的匹配项的函数。 如果测试确认匹配，该函数必须将cookie键作为参数并返回`true`。
...
* **选项：** 包含用于配置cookie属性的属性值的对象。有关信息，请参见` [setItem](/help/sites-developing/contexthub-api.md#setitem-key-value-options)`函数。 `expires`属性无效。
...
* **选项：** （可选）包含以下任意属性的对象，这些属性配置了cookie属性：
   ...
   * 过期：一个`date`或`number`值，它指定Cookie何时过期。 日期值指定到期的绝对时间。 数字（以天为单位）将到期时间设置为当前时间加数字。 默认值为 `undefined`.
...
* **选项：** 在 `options` 调用函数时使用的 `[removeItem](/help/sites-developing/contexthub-api.md#removeitem-key-options)` 参数。
...
允许您将函数绑定到ContextHub存储事件并取消绑定。 使用存储的[事件](/help/sites-developing/contexthub-api.md#eventing)属性访问ContextHub.Utils.事件对象。
...
* **name:** 要 [解除函](/help/sites-developing/contexthub-api.md#contexthub-utils-eventing) 数绑定的事件的名称。
...
* **name:** （字符串） [要将函](/help/sites-developing/contexthub-api.md#contexthub-utils-eventing) 数绑定到的事件的名称。

* **handler:** （函数）要绑定到事件的函数。
* **selector:** （字符串）绑定的唯一标识符。如果要使用`off`函数删除绑定，则需要选择器来标识绑定。

* **triggerForPastEvents:** (Boolean)指示是否应为过去发生的事件执行该处理函数。值`true`调用过去事件的处理函数。 值`false`将调用处理器以获取将来的事件。 默认值为 `true`.
...
* **data：要** 序列化的值或对象。此函数支持布尔值、数组、数字、字符串和日期值。
...
与键对应的值。 当密钥具有子密钥时，此函数返回一个复杂对象。 当键的值类型为`undefined`时，将返回`null`。
...
`string`值的数组，其中每个字符串是由斜杠划分的`key`部分。 表示已清理的密钥。 如果已清理的数组长度为零，则此函数返回`null`。
...
将键／值对添加到对象副本的数据树中。 有关数据树的信息，请参见[持久性](/help/sites-developing/contexthub.md#persistence)。

**参数**

* 树：数据对象。
* 键：要与要添加的值关联的键。 关键是数据树中项目的路径。 此函数调用`ContextHub.Utils.JSON.tree.sanitize`在添加密钥之前对其进行清理。
* value:要添加到数据树的值。
...
* **priority:** (Number)商店候选项的优先级。
* **applies:** （函数）用于评估当前环境中存储的可用性的函数。如果存储适用，该函数必须返回`true`，否则返回`false`。 默认值为返回true的函数：`function() {return true;}`
