...
使用ContextHub注册UI模块渲染器。 注册渲染器后，可将其用于[创建UI模块](/help/sites-administering/contexthub-config.md#adding-a-ui-module)。 当您正在[扩展ContextHub.UI.BaseModuleRenderer](/help/sites-developing/ch-extend.md#creating-contexthub-ui-module-types)时，使用此函数可创建自定义UI模块渲染器。
