...
* HTML(最好 [是HTML](https://dev.w3.org/html5/spec/Overview.html)5)
* 层叠样式表
* 可扩展标记语言(XML)
...
Adobe Experience Manager的技术栈基于Apache Felix [OSGI容器和Apache Sling](https://felix.apache.org/) Web框架，并嵌入一个基于Apache Jackrabbit [Apache Jackrabbit的](https://sling.apache.org/site/index.html)[](https://docs.adobe.com/content/docs/en/spec/jcr/2.0/index.html)[](https://jackrabbit.apache.org/jcr-api.html)Java Content Repository(JCR)。 您应熟悉这些单个项目，以及您要贡献的区域中使用的任何其他开放源代码组件（如Apache Lucene）。
...
在Java内容存储库中建立内容模型的方式需要一种完全不同于软件行业在关系世界中建立数据模型的惯例的思维方式。 内容管理JCR方式的新来者，必须读 [到大卫的模型： 内容建模指南](https://wiki.apache.org/jackrabbit/DavidsModel)。
...
由于REST提供了我们所做的许多工作的指导理念，您应认为精通RESTful设计的原则至关重要。 罗伊·菲尔丁的论文 [是开始的好地方](https://www.ics.uci.edu/~fielding/pubs/dissertation/rest_arch_style.htm)。

### Sling请求解决方案 {#sling-request-resolution}

要了解AEM的一个关键方面是传入请求如何与内容和应用程序行为相关、内容在内容存储库中的结构如何以及AEM在何处查找应用程序逻辑以处理请求。 了解Apache Sling URL [分解及其实施](https://sling.apache.org/site/url-decomposition.html) REST架构样式及其无状态、可缓存和分层系统约束的方式。
...
您做的任何事情都不应破坏客户的旧代码。 请考虑 `/libs` 仅包含可在升级过程中更新的产品代码。 存储 `/apps` 库的部分是项目代码，该部分 `/etc` 包含需要保留的自定义配置。 通常，不要覆盖中的任 `/apps`何内 `/content` 容， `/home`以及 升级后，旧项目代码、配置和内容应继续像升级前一样正常工作。
...
**群集和负载平衡** -为了提高生产环境的可用性并提高其性能，通常将多个作者实例和／或发布实例（发布到群集中）合并，方法是使这些实例对不同用户组可用，或在调度程序配置后对它们进行负载平衡。

还可以合并内容存储库的多个实例以创建高可 *用性* JCR解决方案，然后与AEM解决方案集成以最大限度地防止硬件和软件故障。 有关更 [多信息，请参](/help/sites-deploying/recommended-deploys.md#oak-cluster-with-mongomk-failover-for-high-availability-in-a-single-datacenter) 阅建议的部署。

**组件** -在AEM中，组件是一种对象类型，其实例通常可以通过从Sidekick中拖放来创建。 例如，随AEM一起提供的现成组件包括文本、标题、标记云、传送、图像和列表组件，这些组件在运行时都可从Sidekick获得。

**内容查找器** -在创作模式下，内容查找器是页面左侧的一个特殊面板（框架），它根据您在顶部选择的选项卡显示一列表图像、文档、Flash资产、页面、段落或存储库资源，您可以将这些资源从内容查找器拖放到您正在处理的页面（在右侧）。
...
本质上，JCR是一个节点和属性的系统，在该系统中，节点可以继承其他节点，所有内容都作为属性 *值存储*。 请注意，除了普通继承外，JCR还允许“mixin”节点的概念，这允许对多个继承进行建模。

JCR具有许多预定义的节点类型和属性类型，但通常键入系统相当灵活，并且（事实上）JCR的强项之一是它允许结构化和非结构化内容被同等轻松地存储／管理。 即，JCR可以容纳高度结构化的数据，但也可以容纳任意动态模式结构，而不受数据约束。

此处是用于JCR的Java API的JavaDoc [](http://jackrabbit.apache.org/jcr/jcr-api.html)。

在尝试阅读JavaDoc或JCR规范本身之前，您可能希望查看 [由Adobe Experience Services实](/help/sites-developing/the-basics.md#java-content-repository) 施的JCR的这一高级解释。
...
**Microkernel** —— 存储库中的每个工作区都可以单独配置为通过特定的Microkernel（一个管理数据读写的类）存储其数据。 同样，存储库范围的版本存储也可以独立配置为使用特定的微内核。 有许多不同的微内核可用，能够以各种文件格式或关系数据库存储数据。 （例如，存在MongoDB、DB2或Oracle的持久性管理器）AEM的默认微内核是TarMK（请参阅下面的更多内容）。
