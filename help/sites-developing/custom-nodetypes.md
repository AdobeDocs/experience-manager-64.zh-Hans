...
title: 自定义节点类型
seo-title: 自定义节点类型
description: AEM基于Sling，并且使用JCR存储库，该存储库提供两种类型的节点，但AEM还提供一系列自定义节点类型
seo-description: AEM基于Sling，并且使用JCR存储库，该存储库提供两种类型的节点，但AEM还提供一系列自定义节点类型
...
由于AEM基于Sling并使用JCR存储库，因此这两个存储库提供的节点类型可供使用：
...
* `@prop latitude` - latitude使用小数点编码为双精度
* `@prop longitude` - longitude encoded use decimal degrees
...
定义将节点标记为伪页面的混音类型。 这意味着它们可以适用于页面和WCM编辑支持。
...
* `@node thumbnail.png` -保存特征缩略图的文件。
* `@node workflows` -自动分配工作流配置。 配置将遵循以下结构：
   * `+ workflows`
      * `+ name1`
         * `- cq:path`
            * `- cq:workflowName`
* `@prop allowedParents` -用于确定模板路径的正则表达式模式，这些模板作为父模板允许。
* `@prop allowedChildren` -用于确定允许作为子模板的模板的路径的正则表达式模式。
* `@prop ranking` -在创建页面对话框中的模板列表中的位置。
...
* `@prop jcr:description` -组件说明。
* `@node dialog` -主对话框。
* `@prop dialogPath` -主对话框路径（对话框的替代路径）。
* `@node design_dialog` -设计对话框。
* `@prop cq:cellName` -设计单元格的名称。
* `@prop cq:isContainer` -指示这是否为容器组件。 这会强制使用子组件的单元格名称而不是路径名称。 例如，该组件 `parsys` 是一个容器组件。 如果未定义此值，则根据是否存在进行检查 `cq:childEditConfig`。
* `@prop cq:noDecoration` -如果为true，则在包含 `div` 此组件时不会绘制装饰标签。
* `@node cq:editConfig` -定义编辑栏参数的配置。
* `@node cq:childEditConfig` -子组件继承的编辑配置。
* `@node cq:htmlTag` -定义在包含组件时添加到“周围”标 `div` 签的其他标签属性。
* `@node icon.png`-包含特征图标的文件。
* `@node thumbnail.png` -保存特征缩略图的文件。
...
* `@node cq:infoProviders` -包含子节点，每个子节点都有 `className` 一个引用的属性 `PageInfoProvider`。
...
定义虚拟CQ组件。 这些组件当前仅用于新组件拖放向导。

* `@prop jcr:title` -此组件的标题。
* `@prop jcr:description` -此组件的说明。
* `@node cq:editConfig` -用于定义编辑栏参数的编辑配置。
...
定义要在编辑事件上执行的（客户端）监听器。 这些值必须引用有效的客户端监听器函数或包含预定义的快捷键：
...
* `@prop afterdelete` -删除组件后将触发。
* `@prop afterinsert` -将组件添加到此容器后将触发。
* `@prop afterremove` -从此容器删除组件后将触发。
* `@prop aftermove` -将组件移入此容器后将触发。
...
## 交付容器列表 {#delivery-container-list}
...
* `@prop * (string) copy` -与父节点一起版本化的其他属性。
...
节点类型包 `cq:contentPage` 含ContentBus内容页的属性和子节点定义。 仅当将此混音类型添加到类型的节点时， `cq:page`节点才会变为ContentBus内容页面。

中的项目 `cq:Cq4ContentPage` 有：

* `@prop cq:csd` -页面的ContentBus CSD。
* `@node cq:content` -页面内容。 如果页面节点处于“现有且无内容”或“已删除”状态，则此子节点不存在。
* `@node cq:attributes` -页面属性列表，以前称为版本标签。 此节点对于cq:contentPage类型是必填的。 当页面为节点版本时，属性节点版本化。
...
* `@prop source (String) mandatory` -数据源URI，此为必填项，且不得为空
* `@prop target (String)` -存储从数据源检索的数据的目标位置。 这是可选的，默认为cq:PollConfig节点。
* `@prop interval (Long)` -从数据源轮询新数据或更新数据的时间间隔（以秒为单位）。 这是可选的，默认为30分钟（1800秒）。
...
以小数点(DD)为单位定义地理位置的混音。

* `@prop latitude` - latitude使用小数度编码为双倍。
* `@prop longitude` -经度编码为使用小数度的双倍。
...
MailerService节点类型。 邮件发送者使用具有该混合的节点作为消息定义的根节点。
...
定义LiveRelationship混音。 主节点和从节点可以通过LiveRelathion实际链接。
...
定义LiveSync混音。 如果某个节点与作为从节点的主节点在LiveRelationship中相关，则该节点将标记为LiveSync。

* `@prop cq:master` - liveRelationship主节点的路径。
...
定义LiveSyncCancelled混音。 取消从节点的LiveSync行为，该行为可能由于其父节点之一而与LiveRelation关系相关。
...
任何用户／公共网站都可以标记cq:userContent中使用的内容（Web2.0样式）。
...
添加 `cq:userContent` 可由用户修改的子节点。 每个用户都有自己的子 `cq:userContent/<userid>` 节点，该子节点通常有mixin `cq:UserTaggable`。
