...
* [有关内容片段的更多信息](/help/assets/content-fragments.md) ，请 [使用内容片段创建和管理内容片段](/help/sites-authoring/content-fragments.md) ，以及使用页面创作。

* [管理资产](/help/assets/managing-assets-touch-ui.md) 、 [自定义和扩展资产](/help/assets/extending-assets.md) ，以了解有关标准资产的更多信息。

## 架构 {#architecture}

内容片 [段的基](/help/assets/content-fragments.md#constituent-parts-of-a-content-fragment) 本组成部分包括：
...
* 元数据和相关内容存储在下面 `jcr:content/metadata`

   除标题和说明外，标题和说明不被视为传统元数据并存储在 `jcr:content`
...
内容片段可以从AEM页面进行引用，就像任何其他资产类型一样。 AEM提供内 [**容片段&#x200B;**核心组件](https://helpx.adobe.com/experience-manager/core-components/using/content-fragment-component.html)-一个[组件，它允许您在页面上包含内容片段](/help/sites-authoring/content-fragments.md#adding-a-content-fragment-to-your-page)。 您还可以扩展此内容片**&#x200B;段核心组&#x200B;**件。
...
Web控制台中可以为OSGi捆绑 [DAM内容](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console)片段配置 **配置配置此参数**。

* **资源类型**

   可以提供 `sling:resourceTypes` 列表来定义用于渲染内容片段的组件以及应将后台处理应用到的位置。
