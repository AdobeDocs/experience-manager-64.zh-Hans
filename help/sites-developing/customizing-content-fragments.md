...
* 所有内容都存储在资 `jcr:content/data` 产的节点下：
   ...
   * 每个元素的数据作为具有元素名称的属性存储在相应的子节点中：
      ...
      例如，元素的内容作 `text` 为属性存储在 `text``jcr:content/data/master`
...
可以在 [Web控制台中为OSGi捆绑](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console)DAM内容片段配置 **此参数**。

* **资源类型**

   可以提供一 `sling:resourceTypes` 个列表来定义用于呈现内容片段的组件以及应将后台处理应用到的位置。

* **引用属性**

   可以配置属性列表，以指定对片段的引用存储于各个组件的位置。
