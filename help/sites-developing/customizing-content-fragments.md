...
* [创建和管理内容片](/help/assets/content-fragments.md) 段和使 [用内容片段进行页](/help/sites-authoring/content-fragments.md) 面创作，以了解有关内容片段的更多信息。

* [管理](/help/assets/managing-assets-touch-ui.md) 资产和 [自定义和扩](/help/assets/extending-assets.md) 展资产，以了解有关标准资产的更多信息。

## 架构 {#architecture}

内容片段的基本[组成部分](/help/assets/content-fragments.md#constituent-parts-of-a-content-fragment)为：
...
>[!CAUTION]
>
>[现在，建](/help/assets/content-fragments-models.md) 议使用内容片段模型创建所有片段。
>
...
* 内容片段模板：
   ...
   * 添加新变量等的函数必须相应更新片段。
   * [内容片](/help/sites-developing/content-fragment-templates.md) 段模板以不同于AEM生态系统中其他模板机制（如页面模板等）的方式进行操作。因此，应单独考虑这些问题。
   * 基于模板时，根据实际内容管理内容的MIME类型；这意味着每个元素和变量都可以具有不同的MIME类型。

## 与资产{#integration-with-assets}集成
...
* 元数据和相关内容存储在`jcr:content/metadata`下
   ...
   除标题和说明外，标题和说明不被视为传统元数据并存储在`jcr:content`上
...
有关详细信息，请参阅[内容片段——删除注意事项](/help/assets/content-fragments-delete.md)。
...
内容片段可以从AEM页面进行引用，就像任何其他资产类型一样。 AEM提供&#x200B;[**内容片段**&#x200B;核心组件](https://helpx.adobe.com/experience-manager/core-components/using/content-fragment-component.html) —— 一个[组件，允许您在页面](/help/sites-authoring/content-fragments.md#adding-a-content-fragment-to-your-page)中包含内容片段。 您还可以扩展此&#x200B;**内容片段**&#x200B;核心组件。

* 组件使用`fragmentPath`属性引用实际内容片段。 `fragmentPath`属性的处理方式与其他资产类型的类似属性相同；例如，当内容片段被移动到其他位置时。
...
此参数可在[Web控制台](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console)中为OSGi捆绑&#x200B;**DAM内容片段配置**&#x200B;配置。

* **资源类型**

   可以提供`sling:resourceTypes`的列表来定义用于渲染内容片段的组件以及应将后台处理应用到的组件。

* **引用属性**

   属性列表可以配置为指定对片段的引用存储于各个组件的位置。

>[!NOTE]
>
>属性和组件类型之间没有直接映射。
>
>AEM只需使用可在段落上找到的第一个属性。 因此，您应当谨慎选择属性。

![osgi-config](assets/osgi-config.png)

您仍必须遵循一些准则，以确保您的组件与内容片段后台处理兼容：

* 定义要呈现元素的属性名称必须为`element`或`elementNames`。

* 定义要呈现变体的属性名称必须是`variation`或`variationName`。

* 如果支持多个元素的输出（通过使用`elementNames`指定多个元素），则实际显示模式由属性`displayMode`定义：

   * 如果该值为`singleText`（并且只配置了一个元素），则元素将呈现为具有中间内容和布局支持等的文本。 这是仅渲染一个元素的片段的默认值。
   * 否则，将使用一种更简单的方法(可称为“表单视图”)，其中不支持中间内容，并且片段内容将“按原样”呈现。

* 如果为`displayMode` == `singleText`（隐式或显式）呈现片段，则将播放以下附加属性：
...
* **翻译**
   ...
   内容片段与[AEM转换工作流](/help/sites-administering/tc-manage.md)完全集成。 在建筑层面，这意味着：
   ...
   * 除了基于规则的路径之外，内容片段的不同语言版本之间没有进一步的联系；它们作为两个单独的片段处理，但UI提供了在语言变体之间导航的方法。
   >[!NOTE]
   >
   >AEM转换工作流可与`/content`一起使用：
   >
   >  * 由于内容片段模型位于`/conf`中，因此此类转换中不包含这些模型。 可以[将UI字符串国际化](/help/sites-developing/i18n-dev.md)。
   >  * 模板将被复制以创建片段，因此这是隐式的。


* **元数据架构**

   * 内容片段（重新）使用[元数据模式](/help/assets/metadata-schemas.md)，它可以用标准资产进行定义。
* CFM提供其自己的特定模式:
...
* `ContentFragment` 可适用于：
   ...
   * `Resource` -基础Sling资源；请注意，直接更 `Resource` 新基础需要重建对 `ContentFragment` 象。
   * `Asset` -表示 `Asset` 内容片段的DAM抽象；请注意，直接 `Asset` 更新需要重新构建对 `ContentFragment` 象。
...
* `filter.xml`
   ...
   内容片段管理的`filter.xml`已配置为不与资产核心内容包重叠。

## 编辑会话{#edit-sessions}

当用户在一个编辑器页面中打开内容片段时，将启动编辑会话。 当用户通过选择&#x200B;**保存**&#x200B;或&#x200B;**取消**&#x200B;离开编辑器时，编辑会话即结束。

### 要求{#requirements}

控制编辑会话的要求有：

* 编辑可跨多个视图（= HTML页）的内容片段应该是原子的。
* 编辑还应为&#x200B;*transactional*;在编辑会话结束时，必须提交（保存）或回滚（取消）更改。
...
* 内容更改
   ...
   * 只要用户更改内容且不存在编辑会话，就会创建新的编辑会话（请参阅[启动会话](#processes)）。
