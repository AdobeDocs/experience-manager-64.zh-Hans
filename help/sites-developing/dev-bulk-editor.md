...
批量编辑器显示时不 **显示根路径** 字段，因为hrp=true将隐藏该字段。 当参数hrp=false时，将显示字段（默认值）。

以下是批量编辑器列表参数的查询:

>[!NOTE]
>
>每个参数都可以有一个长名和一个短名。 例如，搜索根路径的长名 `rootPath`称是，短名称是 `rp`。 如果未定义长名称，则从请求中读取短名称。
...
产品列表组件允许用户显示和编辑数据表。 例如，您可以使用产品列表组件来表示目录中的产品。 该信息显示在标准HTML表中，并且任何编辑都在“编辑”对 **话框中执** 行，该对话框包含一个BulkEditor构件。 (此批量编辑器与/etc/importers/bulkeditor.html或通过“工具”菜单访问的编辑器完全相同。) 产品列表组件已针对特定的有限批量编辑器功能进行配置。 可以配置批量编辑器（或从批量编辑器派生的组件）的每个部分。
...
* 编辑对话框，在该对话框中可定义批量编辑器配置。 配置对话框以匹配组件的需求： 列以及对网格或搜索执行的可能操作。 有关 [所有配置属性](#bulk-editor-configuration-properties) ，请参阅批量编辑器配置属性。
...
如果复选框配置属性设置为true，则列的所有单元格将呈现为复选框。 复选框将 **true** 发送到服务器Save servlet，否 **则为** false。 在标题菜单中，还可以选 **择全部** 或 **选择无**。 如果所选标题是复选框列的标题，则这些选项将处于启用状态。
