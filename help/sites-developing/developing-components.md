...
* When [创作页面](/help/sites-authoring/default-components.md)，则作者可以通过这些组件编辑和配置内容。
   ...
   * 构建 [商务](/help/sites-administering/ecommerce.md) 例如，组件可以在站点中收集和渲染目录信息。

      请参阅 [发展电子商务](/help/sites-developing/ecommerce.md) 以了解更多信息。
...
的 [HTML模板语言(HTL)](https://helpx.adobe.com/experience-manager/htl/user-guide.html)，在AEM 6.0中引入，取代了JSP(JavaServer Pages)，成为首选和推荐的服务器端模板系统，用于HTML。 对于需要构建强大企业网站的Web开发人员，HTL有助于提高安全性和开发效率。
...
的 [WCM模式](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) 在切换到 **预览** 模式，即使页面未刷新。

对于呈现的对WCM模式敏感的组件，需要定义它们以专门刷新它们，然后依赖Cookie的值。

>[!NOTE]
>
>在触屏优化UI中，仅 `EDIT` 和 `PREVIEW` 用于 [WCM模式](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) cookie。
...
[Coral用户界面](https://helpx.adobe.com/cn/experience-manager/6-4/sites/developing/using/reference-materials/coral-ui/coralui3/index.html) 和 [Granite用户界面](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/index.html) 定义AEM的现代外观。

[Granite UI提供了大量基本组件（小组件）](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/index.html) 需要在创作环境中创建对话框。 必要时，您可以扩展此选择并创建自己的小组件。
...
>请参阅：
>
>* AEM Gem会议 [自定义对话框字段](https://experienceleague.adobe.com/docs/experience-manager-gems-events/gems/gems2015/aem-customizing-dialog-fields-in-touch-ui.html).
...
>[!NOTE]
>
>有关Granite UI的完整详细信息，请参阅 [Granite用户界面文档](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/index.html).
...
更具体地说，Granite UI提供了一系列字段组件，这些组件适合在对话框(或更一般地，在 [表单](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/foundation/form/index.html))。
...
要仅为组件对话框加载客户端库（即不会为其他组件加载），您需要设置属性 `extraClientlibs` 对话框的类别名称。 如果您的客户端库非常大，并且/或您的字段特定于该对话框，而在其他对话框中不需要，则建议使用此方法。
...
处理对话框字段中事件的方法现已完成 [自定义客户端库中的侦听器](#listeners-in-a-custom-client-library). 这是与以前的 [内容结构中的侦听器](#listeners-in-the-content-structure).
...
* 定义组件 [模板段落系统的属性](/help/sites-developing/components-basics.md#adding-your-component-to-the-paragraph-system).

## 配置段落系统以便拖动资产可创建组件实例 {#configuring-a-paragraph-system-so-that-dragging-an-asset-creates-a-component-instance}

AEM允许您在页面上配置段落系统，以便 [当用户将（相应）资产拖动到该页面的实例上时，将自动创建新组件的实例](/help/sites-authoring/editing-content.md) （而不是始终将空组件拖到页面中）。
...
* 提供一些 [HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html) 支持，模板语言旨在简化组件开发并提高安全性。
...
* HTL
   ...
   * 使用 [HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html) 不是强制的，但如果您的组件需要更新，则最好考虑 [从JSP迁移到HTL](/help/sites-developing/components-basics.md#htl-vs-jsp).
...
* 对话框
   ...
   * 的 [AEM现代化工具](/help/sites-developing/modernization-tools.md) 旨在帮助您扩展现有组件。
   ...
   * 自定义字段，有关更多信息，请参阅AEM Gems会议 [自定义对话框字段](https://experienceleague.adobe.com/docs/experience-manager-gems-events/gems/gems2015/aem-customizing-dialog-fields-in-touch-ui.html).
   ...
   * 使用JS侦听器，有关更多信息，请参阅 [处理字段事件](#handling-field-events) 和AEM Gem会议 [自定义对话框字段](https://experienceleague.adobe.com/docs/experience-manager-gems-events/gems/gems2015/aem-customizing-dialog-fields-in-touch-ui.html).
...
支持的Markdown与 [内容片段](/help/assets/content-fragments-markdown.md).
