...
随AEM 6.0引入的[HTML模板语言(HTL)](https://helpx.adobe.com/experience-manager/htl/user-guide.html)代替JSP(JavaServer Pages)作为HTML的首选和推荐的服务器端模板系统。 对于需要构建强大企业网站的Web开发人员，HTL有助于提高安全性和开发效率。
...
在切换到&#x200B;**预览**&#x200B;模式时，即使页面未刷新，也会设置[WCM模式](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) Cookie。

对于呈现对WCM模式敏感的组件，需要定义这些组件以具体刷新它们，然后依赖Cookie的值。

>[!NOTE]
>
>在触屏优化UI中，仅`EDIT`和`PREVIEW`值用于[ WCM Mode](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) cookie。
...
[Coral ](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/coral-ui/coralui3/index.html) UI和 [Granite ](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/index.html) UI定义AEM的现代外观。

[Granite UI提供了在创作环境上创建对](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/index.html) 话框所需的大量基本组件（构件）。必要时，您可以扩展此选择并创建您自己的Widget。
...
更具体地说，Granite UI提供了一系列适用于对话框（或更一般地，以[形式](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/foundation/form/index.html)）的字段组件。
...
要仅为您的组件对话框加载您的客户端库（即不为其他组件加载它），您需要将对话框的属性`extraClientlibs`设置为刚刚创建的客户端库的类别名。 如果您的客户端库很大，并且/或您的字段特定于该对话框，而在其他对话中不需要您，则建议这样做。
...
现在，对对话框字段的事件处理方法在自定义客户端库](#listeners-in-a-custom-client-library)中的[侦听器中完成。 这与在内容结构](#listeners-in-the-content-structure)中具有[侦听器的旧方法相比有所变化。
...
要实现此目的，您需要了解要与之交互的基础Widget库。 请参阅[Coral UI文档](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/coral-ui/coralui3/index.html)以确定要对哪个事件做出响应。 这与您过去使用ExtJS执行的过程非常相似：查找给定构件的文档页面，然后检查其事件 API的详细信息。
...
AEM优惠在您的页面上配置段落系统，以便当用户将（适当）资产拖动到该页面](/help/sites-authoring/editing-content.md)的实例时，会自动创建新组件的[实例（而不是总是将空组件拖动到页面）。
...
* HTL
   ...
   * 使用[HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html)并非强制性的，但如果您的组件需要更新，则最好考虑从JSP迁移到HTL](/help/sites-developing/components-basics.md#htl-vs-jsp)。[
