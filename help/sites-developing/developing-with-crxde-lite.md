...
1. 安装AEM。
1. 在您的浏览器中，输入https://`<host>`:`<port>`/crx/de. 默认情况下，这是 `http://localhost:4502/crx/de`。
...
通过CRXDE Lite，您可以在 [CND（紧凑命名空间和节点类型定义）表示法](http://jackrabbit.apache.org/jcr/node-type-notation.html).
