...
* **`RolloutConfig`** 保存列表 `LiveActions`，在触发时使用。 继 `LiveCopy` 承该 `RolloutConfig` 属性，并且结果会出现在中 `LiveRelationship`。
...
* 一个OSGI组件，用于实 [`com.day.cq.wcm.msm.api.LiveActionFactory`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory.html) 现接口并创建类的 `LiveAction` 实例。
...
方法的参 [`LiveActionFactory`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory.html)`.createAction` 数是对 `Resource` 象。 此对 `Resource` 象表示转 `cq:LiveSyncAction` 出配置中此实时操作的节点； 请参 [阅创建转出配置](/help/sites-administering/msm-sync.md#creating-a-rollout-configuration)。 与常用配置节点一样，您应将其调整为对 `ValueMap` 象：
...
* 表 [`Resource`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/Resource.html) 示Live Copy源的对象。
...
>[!NOTE]
>
>参 `Resource` 数可以是 `null` 或不适应 `Resources` 对象（如对象）的 `Node`[`NonExistingResource`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/NonExistingResource.html) 对象。
...
1. 重复上一步，创建文 `/apps/wcm/core` 件夹树。 在名为的核心中创 `sling:Folder` 建类型节点 `resources`。 <!-- ![chlimage_1-39](assets/chlimage_1-39.png) -->
