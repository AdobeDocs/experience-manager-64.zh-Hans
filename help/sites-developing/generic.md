...
* `cq:commerceProvider`属性还用于引用相应的商务工厂定义。
   ...
   * 例如，具有值geometrixx的`cq:commerceProvider`属性将与&#x200B;**Day CQ Commerce Factory for Outdoors**(`com.adobe.cq.commerce.hybris.impl.GeoCommerceServiceFactory`)的OSGi配置相关联，其中参数`commerceProvider`也具有值`geometrixx`。
   * 此处还可以配置其他属性（如果适用并可用）。
...
>
>* 对于通用实现，AEM从产品数据(`cq:productVariantAxes`)中的属性读取它
>
>
...
* 购物车归`CommerceSession:`所有
   ...
   * `CommerceSession`还将已触发的优惠券和促销应用到购物车。
...
* 促销活动:
   ...
   * ClientContext`SegmentMgr`解析区段，ClientContext`CartMgr`解析促销。 将触发每个受至少一个已解析区段约束的促销。
...
这样，`CommerceSession`负责检查凭证是否存在以及是否可以应用凭证。 这可能适用于仅在满足特定条件时才能应用的凭证；例如，当总购物车价格大于$100时)。 如果凭证因任何原因无法应用，`addVoucher`方法将引发异常。 此外，`CommerceSession`负责在添加／删除凭证后更新购物车的价格。
...
提供的`AbstractJcrCommerceSession`可以应用凭证。 类`getVouchers()`返回的凭证是`cq:Page`的实例，其中包含具有以下属性的jcr:content节点（等等）:
