...
这样，The `CommerceSession` 负责检查凭证是否存在以及是否可以应用它。 这可能适用于仅在满足特定条件时才能应用的凭证； 例如，当总购物车价格大于$100时)。 如果凭证因任何原因无法应用，则 `addVoucher` 方法将引发异常。 此外， `CommerceSession` The is responsibles for updating the cart&#39;s price(s)after a voucher is added / removed.
