...
Jackrabbit FileVault工具(VLT)是Apache Foundation开 [发的工具](https://www.apache.org/) ，它将Jackrabbit/AEM实例的内容映射到您的文件系统。 VLT工具具有与源控制系统客户端(如Subversion(SVN)客户端)类似的功能，提供正常的登记、注销和管理操作，以及可灵活呈现项目内容的配置选项。

从命令行运行VLT工具。 本文档介绍如何使用该工具，包括如何开始和获取帮助，以及所有命令和可用 [选项](#vlt-commands) 的 [列表](#vlt-global-options)。

## 概念和架构 {#concepts-and-architecture}

有关Filevault [工具概念和结构的全面概](https://jackrabbit.apache.org/filevault/overview.html) 述，请参阅官方Apache Jackrabbit Filevault文档中的“Filevault [Overview and Vault FS”（Filevault概述和Vault FS）页面](https://jackrabbit.apache.org/filevault/vaultfs.html)[](https://jackrabbit.apache.org/filevault/index.html) ，获取相关信息。
...
1. 从Maven项目库下载FileVault [存档文件。](https://repo1.maven.org/maven2/org/apache/jackrabbit/vault/vault-cli/)
   >[!NOTE]
   >
   >VLT工具的源可在GitHub [上使用。](https://github.com/apache/jackrabbit-filevault)
1. 解压存档。
1. 添 `<archive-dir>/vault-cli-<version>/bin` 加到环境 `PATH` 中，以便根据需要 `vlt` 访问命 `vlt.bat` 令文件或命令文件。 例如：
...
### 配置行字符结尾 {#configuring-the-end-of-line-character}

VLT会根据以下规则自动处理行尾(EOF):

* 在Windows结尾上签出的文件行 `CRLF`
* 在Linux/Unix上签出的文件行， `LF`
* 存储库的文件行以 `LF`

要确保VLT和SVN配置匹配，您应将属性 `svn:eol-style` 设置为 `native` 存储库中存储的文件的扩展名。 编辑svn设置并添加以下内容：
...
1. 通过键入以下内容(将端口号替换为4502和管理 **员密码** )，检查存储库：
...
在签出存储库并同步它后，应进行测试以确保所有内容都正常工作。 执行此操作的一个简单方法是编辑。jsp **文件** ，并查看提交更改后是否反映了您所做的更改。
...
例如，如果只想签出存储库的子树，则可 `/apps/geometrixx`以通过键入以下内容来签出：

```shell
vlt co http://localhost:4502/crx/-/jcr:root/apps/geometrixx geo
```

这样做会创建一个带有和目 `geo` 录的 `META-INF` 新导 `jcr_root` 出根目录，并将所有文件放 `/apps/geometrixx` 入中 `geo/jcr_root`。

### 执行已过滤的签出 {#performing-a-filtered-checkout}

如果您有现有的工作区过滤器，并且希望将其用于签出，则可以先创建目 `META-INF/vault` 录并将过滤器放在该目录，或者按如下方式在命令行中指定它：
...
导入本地文件系统(从 `<local-path>` 开始到电子仓库文件系统 `<uri>`)。 可以指定 `<jcr-path>` 为导入根。 如果 `--sync` 指定，导入的文件将自动置于保管库控制之下。
...
如果 `--show-update` 指定了，则根据远程版本检查每个文件。 第二个字母然后指定更新操作将执行什么操作。
...
>[!NOTE]
>
>在 `--exclude` 和参数之前，选项后面需要再加一个 `<src>` 选 `<dst>` 项。 例如：
>
>`vlt rcp -e ".*\.txt" -r`
...
该命 `vlt sync install` 令可用于自动安装保管库同步服务包和配置。

捆绑包安装在下 `/libs/crx/vault/install` 面，配置节点创建在 `/libs/crx/vault/com.day.jcr.sync.impl.VaultSyncServiceImpl`。 最初启用服务，但未配置同步根。
...
该命 `status` 令可用于显示有关正在运行的同步服务的信息。 &quot;
...
>[!NOTE]
>
>该命 `status` 令不从服务中提取任何实时数据，而是读取位于的配置 `/libs/crx/vault/com.day.jcr.sync.impl.VaultSyncServiceImpl`。

### 添加同步文件夹 {#adding-a-sync-folder}

该 `register` 命令用于添加要与配置同步的文件夹。
...
该命 `unregister` 令用于从配置中删除要同步的文件夹。
...
>[!NOTE]
>
>服务可以使用Web控制台或存储库中 `sling:OsgiConfig` 的节点(名 `com.day.jcr.sync.impl.VaultSyncServiceImpl`称为)进行配置。
>
...
文件 `.vlt-sync-config.properties` 允许您配置以下属性：

**禁用** “打开或关闭同步”。 默认情况下，此参数设置为false以允许同步。

**sync-once** 如果不为空，则下一次扫描将按给定方向同步文件夹，则将清除该参数。 支持以下两个值：
...
1. 使用vlt命令行签出您的存储库：
   ...
   >[!NOTE]
   >
   >您可以使用过滤器仅检出相应的路径。 有关信 [息，请参阅执行筛选](#performing-a-filtered-checkout) 的签出部分。
