...
请注意，翻译工具和您管理的词典用于以不同语言显示组件UI。 如果要翻译页面或用户生成的内容，请参 [阅多语言站点的翻译](/help/sites-administering/translation.md)[内容和用户生成内容的翻译](/help/communities/translate-ugc.md)。
...
1. 您现在可以单 **击** “确定”或查看目标词典。

   >[!NOTE]
   >
   >有关翻译项目的更多信息，请阅 [读管理翻译项目](/help/sites-administering/tc-manage.md)。

## 创建字典 {#creating-a-dictionary}

创建用于管理本地化的UI字符串的词典。 创建字典后，可以使用翻译工具管理它。

1. 使用CRXDE Lite，将新字典的根 `sling:Folder`节点()添加为包含语言定义的结构：
...
1. 在此根下添加所需的语言结构。 例如：
   ...
   >[!NOTE]
   >
   >这是Sling i18n模 [块的结构](https://sling.apache.org/site/internationalization-support.html)。
...
请注 [意，请求译文字符串的代码](/help/sites-developing/i18n-dev.md#including-variables-in-localized-sentences) ，会根据上下文为索引变量提供值。
...
在将英语字符串添加到词典后，可以添加该字符串的翻译。 您还可以 [导出字典](/help/sites-developing/i18n-translator.md#exporting-a-dictionary) ，以便由第三方翻译该字典。

1. 选 [择项目特定词典](#creating-a-dictionary) ，因为它指定包含翻译的存储库中的路径。 例如，选择字 **典** :
...
1. 要编辑某个字符串的译文，您可以执行以下任一操作：
   ...
   * 多次单击所需 **字符串的** “字符串 **”或“注释** ”字段以打开“编辑字符串” **对话框，根据需要编辑翻译，然后单****** 击确定以关闭对话框：

   ![chlimage_1-217](assets/chlimage_1-217.png)

1. 单击工 **具栏** 中的“保存”以提交更改。

   >[!NOTE]
   >
   >单击“重 **置并刷新** ”(而非“ **保存**”)可还原对先前文本所做的任何更改。
...
当您导出XLIFF文件并包含语言时，存储库中字典的节点结构必须包含该语言。 如果未包含该语言，则会出错。 例如，要导出法语XLIFF文件，字典文件夹必须包含名为的 `mix:language` 子节点 `fr`。 (请参 [阅创建字典](/help/sites-developing/i18n-translator.md#creating-a-dictionary)。)

请按照以下过程导出特定语言的XLIFF文件。

1. 打开翻译工具 `http://<host>:<port>/libs/cq/i18n/translator.html`
1. 使用“词典”下拉菜单选择要导出的词典。
1. 单击“导出”>“ *导出* 完整XX Xliff选项”, *其中* XX是双字母语言代码，如DE或FR。
...
1. 单击 **“Save All** inCRXDE Lite”并重新加载转换器。 将更新网格以显示定义的语言。

   >[!NOTE]
   >
   >翻译器只保存字典中实际存 [在的语言](#creating-a-dictionary) (即字典路径下的 `/apps/myProject/i18n`语言)。
   >
   >确保这些语言与网格中显示的语言相对应。

### 为作者提供语言 {#making-languages-available-to-authors}

为AEM实例新的语言定义词典后，您需要使其可供作者选择(例如，在“首选项”中 **使用**):

1. 要更改安全控制台首选项中可用语 **言的列表** ，请执 **行以下操** 作：
...
1. 要在“网站”控制台 **的** “首 **选项** ”中提供该语言，您需要在应用程序中进行以下更改：
...
各国都用同一种语言，比如美国，英国，澳大利亚都用英语。 这由一个代码表示，该代码同时指明语言和国家(如 `en_US`, `en_GB` 和) `en_AU`。

显示标记时使用默认国家／地区（例如，在语言副本对话框中），它们用于解析语言代码的国家／地区。

>[!NOTE]
>
>对于由上述转换器管理的本地化，只有准确的语言才能工作。 如果语言首选项下拉框 `en_uk`使用，则存储库中必 `en_uk` 须有一个词典。
...
1. 语言列表存储在：
   ...
   然后在那里更改或扩展列表。 语言 `defaultCountry` 节点上的属性(例如， `ja`)必须包含完整代码，如 `ja_jp`，该代码将 `jp` 定义为语言的默认国家 `ja`。
...
>[!NOTE]
>
>使用Dispatcher时，需要使缓存 [的页面失效](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html) ，以在呈现的组件字符串中包含新的字典字符串。
