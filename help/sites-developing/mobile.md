...
移动设备组根据设备功能提供移动设备的分段。 设备组提供了在创作实例上基于模拟器进行创作以及在发布实例上正确呈现内容所需的信息：作者向移动设备页面添加内容并将其发布后，即可在发布实例中请求该页面。 在该视图中，内容页面将使用配置的一个设备组呈现，而不是模拟器编辑视图。 设备组的选择基于 [移动设备检测](#client-side-device-detection). 然后，匹配设备组提供必要的样式信息。
...
AEM使用截断版本的 [沃夫尔](http://wurfl.sourceforge.net/index.php)™数据库，根据设备的用户代理查询设备功能，如屏幕分辨率或javascript支持。
...
1. 将以下属性添加到节点：
   ...
   * **regexp**:定义用户代理的正则表达式，例如：.&amp;ast;Mozilla。(&amp;a);iPad。&amp;ast;AppleWebKit。Safari(&amp;A)。&amp;ast;
...
>[!NOTE]
>
>有关更多详细信息，请阅读 [自定义BrowserMap](#using-browsermap-on-your-pages) 中。
