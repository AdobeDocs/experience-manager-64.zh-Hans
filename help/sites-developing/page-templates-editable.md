...
* 站点特定
   ...
   您为组织模板而创建的站点特定文件夹是使用具有管理员权限的用户帐户创建的。
...
* 模板类型允许您定义：
   ...
   * 建议为响应式网格定义断点，并在模板类型上设置移动模拟器。 这是可选的，因为配置也可以在单个模板上定义(请参阅模 [板类型和移动设备组](/help/sites-developing/page-templates-editable.md#template-type-and-mobile-device-groups))。
...
### 模板类型和移动设备组 {#template-type-and-mobile-device-groups}
...
1. **启用模板**
   ...
   * [从“模板](/help/sites-authoring/templates.md#enabling-and-allowing-a-template-template-author) ”控制台启 **用模板** 。
...
* 控制器- `/content/<my-site>/<my-page>`
   ...
   引用模板的生成页面。 内容控制整个过程。 根据定义，它可以访问相应的模板和组件。

* 配置 - `/conf/<my-folder>/settings/wcm/templates/<my-template>`

   模板 [和相关内容策略定义页面配置](#template-definitions) 。

* Model - OSGi捆绑套件

   OSGI包 [实现了该功能](/help/sites-deploying/osgi-configuration-settings.md) 。

* 查看 - `/apps/<my-site>/components`

   在创作和发布环境中，内容由组件呈 [现](/help/sites-developing/components.md)。
