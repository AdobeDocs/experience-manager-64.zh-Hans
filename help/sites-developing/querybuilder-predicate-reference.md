...
日期和时间( `YYYY-MM-DDTHH:mm:ss.SSSZ`)的格式，并允许部分表示，例如 `YYYY-MM-DD`或者，时间戳可以以UTC时区（UNIX时间格式）自1970年以来的毫秒数提供。
...
查找特定sling资源集合的成 [员项目](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/org/apache/sling/resource/collection/ResourceCollection.html)。
...
* **N_value**
   ...
   使 `1_value`用， `2_value`, ...检查多个值(默认情况下 `OR` 与if和= `AND` true结合)（自5.3起）
