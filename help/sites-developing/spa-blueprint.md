...
在上述实施中，项目组件在实际注册到组件映射存储之前以空虚功能进 [行扩展](/help/sites-developing/spa-blueprint.md#componentmapping) 。 这是通过封装和扩展 [ComponentMapping库来实现的](#componentmapping) ，以引入对配置对象 `EditConfig` 的支持：
...
这两个实体与路由的概念有关，但 [`ModelRouter`](/help/sites-developing/spa-routing.md) 它只负责使 [`PageModelManager](/help/sites-developing/spa-blueprint.md#pagemodelmanager) ，加载与当前应用程序状态同步的数据模型。
