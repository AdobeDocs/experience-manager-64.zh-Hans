...
通过处理该页面，`app.component.ts`将调用此处以简化版本列出的`main-content.component.ts`。
...
`MainComponent`将摄取页面模型的JSON表示形式，并处理内容以包装/装饰页面的每个元素。 有关`Page`的更多详细信息，请参阅文档[SPA Blueprint](/help/sites-developing/spa-blueprint.md)。
