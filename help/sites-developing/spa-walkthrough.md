...
1. 选择一个标题组件，然后注意工具栏的显示方式与任何其他组件的显示方式相同。 选择&#x200B;**[!UICONTROL 编辑]**。
...
1. 在发布服务器上或使用页面编辑器的“页面信息”菜单中的“ **[!UICONTROL 视图为已发布]****”选项加** 载We.Retail日志应用程序。
...
1. 如果尚未加载，请在发布服务器上加载We.Retail日志应用程序，或从页面编辑器的“页面信息” **[!UICONTROL 菜单中使用]** “ **视图为已发布** ”选项。
...
1. 在新选 `react.model.json` 项卡中加载。
   ...
   AEM SPA Editor利用 [AEM Content](/help/assets/content-fragments.md) Services将页面的整个内容作为JSON模型提供。
