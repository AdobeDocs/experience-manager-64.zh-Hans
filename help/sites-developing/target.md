...
# 为目标内容进行开发{#developing-for-targeted-content}

本节介绍有关开发组件以用于内容定位的主题。

* 有关与Adobe Target连接的信息，请参见[与Adobe Target集成](/help/sites-administering/target.md)。
...
## 在您的页面{#enabling-targeting-with-adobe-target-on-your-pages}中启用Adobe Target定位
...
此代码添加所需的分析javascript对象并加载与网站关联的云服务库。 对于目标服务，库通过`/libs/cq/analytics/components/testandtarget/headlibs.jsp`加载
...
>[!NOTE]
>
>仅支持随产品提供的`at.js`版本。 通过查看位于`/etc/clientlibs/foundation/testandtarget/atjs/source/at.js`的`at.js`文件，可以获得产品附带的`at.js`版本。
...
JSP会添加所需的分析javascript对象和对客户端javascript库的引用。 `testandtarget.js`文件包含`mbox.js`函数。 脚本生成的HTML与以下示例类似：
...
用于创建mbox的默认mbox.js文件位于`/etc/clientlibs/foundation/testandtarget/mbox/source/mbox.js`。 要使用客户mbox.js文件，请将该文件添加到目标云配置。 要添加文件，文件系统上必须有`mbox.js`文件。

例如，如果要使用[Marketing CloudID服务](https://docs.adobe.com/content/help/en/id-service/using/home.html)，您需要下载mbox.js，以便它包含`imsOrgID`变量的正确值，该值基于您的租户。 此变量是与Marketing CloudID服务集成时必需的。 有关信息，请参阅[Adobe Analytics作为Adobe Target](https://docs.adobe.com/content/help/en/target/using/integrate/a4t/a4t.html)和[实施](https://docs.adobe.com/content/help/en/target/using/integrate/a4t/before-implement.html)前的报告源。
...
您的目标配置包含一个自定义mbox.js文件，[页面标题部分](/help/sites-developing/target.md#the-head-section)中所需的代码会将该文件添加到客户端库框架，而不是对testandtarget.js库的引用。
...
例如，要禁用Geometrixx演示站点页面的标题组件的定位，请将属性添加到`/apps/geometrixx/components/title/cq:editConfig`节点。
...
要跟踪网站性能，请从订单确认页面向Adobe Target发送购买信息。 (请参阅Adobe Target文档中的[创建orderConfirmPage Mbox](https://docs.adobe.com/content/help/en/dtm/implementing/target/configure-target/mboxes/order-confirmation-mbox.html)。) Adobe Target将mbox数据识别为订单确认数据，当您的MBox名称为`orderConfirmPage`时，它使用以下特定参数名称：
...
每个顺序的每个参数的值都不同。 因此，您需要一个组件，它根据购买的属性生成代码。 CQ [电子商务集成框架](/help/sites-administering/ecommerce.md)允许您与产品目录集成并实施购物车和结帐页面。
...
* 根据Adobe TargetAPI的要求，添加类为`mboxDefault`的`div`元素。
...
### 内容定位{#client-libraries-for-content-targeting}的客户端库
