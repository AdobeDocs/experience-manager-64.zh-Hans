...
在AEM 5.6中，Adobe为创作环境引入了具有响应式设 [计的全新触屏](/help/sites-authoring/responsive-layout.md) 。 这与原始经典UI有很大不同，因为它设计用于在触控和桌面设备上运行。

此触屏优化UI现在是AEM的标准UI，它将取代经典UI。
...
它使用HTML词汇的扩展，以便作者能够表达他们构建交互式Web应用程序的意图。 这与WAI-ARIA和微格 [式类似](https://www.w3.org/TR/wai-aria/) , [也相似](http://microformats.org/)。
...
Granite UI基 [础组件提供构建任何UI所需的基本构建块](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/index.html) 。 其中包括：
...
Granite UI管 [理组件构建于基础组件之上](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/index.html) ，以提供任何管理应用程序都可以实现的通用构建块。 其中包括：
...
Coral UI(CUI)是Adobe针对触屏优化UI的可视样式的实现，旨在提供跨多个产品的用户体验的一致性。 Coral UI提供了采用创作环境中使用的可视样式所需的一切。
