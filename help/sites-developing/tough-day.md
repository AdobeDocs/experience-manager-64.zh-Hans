...
Tough Day 2是一个应用程序，它允许您强调测试AEM实例的限制。 它可以随默认测试套件一起运行，也可以根据测试需求进行配置。 您可以观 [看此录制](https://docs.adobe.com/ddc/en/gems/Toughday2---A-new-and-improved-stress-testing-and-benchmarking-tool.html) ，以呈现应用程序。
...
要运行套件测试，Tough Day 2将安装其默认内容包。 通过将参数设置为可以避免 `installsamplecontent` 这种情况 `false`发生，但请记住，您还应该更改要运行的测试的默认路径。 如果jar在运行时没有参数，则第2天将显示帮 [助信息](/help/sites-developing/tough-day.md#getting-help)。
...
| Max&amp;ast; | 测试执行的最长持续时间。 |
| 中值(&amp;A); | 计算所有测试执行的中值持续时间。 |
| 平均放大(&amp;A); | 计算所有测试执行的平均持续时间。 |
| StdDev&amp;ast; | 标准差。 |
| 90p&amp;ast; | 90百分点。 |
| 99p&amp;ast; | 99个百分点。 |
| 99.9p&amp;ast; | 99.9百分点。 |
