...
要优化高摄取负载，您可以定义 [工作流作为临时工作流](/help/sites-developing/workflows.md#transient-workflows).
...
要解决此问题，Adobe建议配置 **最大并行作业数** 将介于系统上处理器内核数量的一半到四分之三之间。 这应该允许系统有足够的容量在处理这些工作流时保持响应。

配置 **最大并行作业数**，您可以执行以下操作之一：

* 配置 **[OSGi配置](/help/sites-deploying/configuring-osgi.md)** 从AEM Web控制台中；表示 **队列：Granite工作流队列** ( **Apache Sling作业队列配置**)。

* 从 **Sling作业** AEM Web控制台的选项；表示 **作业队列配置：Granite工作流队列**，在 `http://localhost:4502/system/console/slingevent`.

此外，还为 **Granite工作流外部进程作业队列**. 它用于启动外部二进制文件的工作流进程，例如 **InDesign Server** 或 **图像Magick**.

### 配置单个作业队列 {#configure-individual-job-queues}

在某些情况下，配置单个作业队列以在单个作业的基础上控制并发线程或其他队列选项非常有用。 您可以从Web控制台通过 **Apache Sling作业队列配置** 工厂。 要查找要列出的相应主题，请执行工作流的模型，并在 **Sling作业** 控制台；例如，在 `http://localhost:4502/system/console/slingevent`.
...
默认情况下， **每周维护窗口** 具有 **工作流清除** 任务，但需要先配置该任务，然后才能运行。 要配置工作流清除，请新建 **AdobeGranite工作流清除配置** 必须在Web控制台中添加。
...
>[!NOTE]
>
>另请参阅 [AEM 6.4中的存储库重组](/help/sites-deploying/repository-restructuring.md).
...
* 旧版工作流设计（设计时和运行时）保留在以下路径下：
   ...
   >[!NOTE]
   >
   >如果编辑了这些设计 *使用AEM UI*，则详细信息将会复制到新位置。
...
* 在工作流流程中，如果 `WorkflowSession` 用于修改存储库，然后不显式保存会话 — 工作流将在会话完成后保存该会话。
* `Session.Save` 不应从工作流步骤中调用：

   * 建议修改工作流jcr会话；then `save` 工作流引擎在工作流完成执行后自动保存会话，因此不需要。
   * 不建议流程步骤创建自己的jcr会话。
...
### 最大限度地减少启动器的数量/范围 {#minimize-the-number-scope-of-launchers}

有一个侦听器负责所有 [工作流启动器](/help/sites-administering/workflows-starting.md#workflows-launchers) 注册：
...
### 启动器的配置增强 {#configuration-enhancements-for-launchers}
...
例如，一个工作流可对一组内容实施业务流程，然后激活该内容。 最好创建一个可激活每个节点的自定义工作流流程，而不是启动 **激活内容** 模型。 此方法将需要额外的开发工作，但与为每个激活启动单独的工作流实例相比，其执行效率更高。
...
此信息用于在单击 [**工作流信息** 的 **收件箱**](/help/sites-authoring/workflows-participating.md#opening-a-workflow-item-to-view-details-and-take-actions). 可以编辑现有的工作流模型以添加阶段。

### 激活页面处理步骤 {#activate-page-process-step}

的 **激活页面进程** 该步骤将为您激活页面，但不会自动查找任何引用的DAM资产并激活这些资产。
...
有许多系统工具可用于帮助监控、维护和疑难解答工作流。 以下所有示例URL均使用 `localhost:4502`，但应在任何创作实例( `<hostname>:<port>`)。
