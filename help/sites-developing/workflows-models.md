...
>[!CAUTION]
>
>有关经典UI的使用，请参阅 [AEM 6.3文档以供参考](https://helpx.adobe.com/experience-manager/6-3/sites-developing/workflows-models.html) 。
...
* 步骤、流 **[!UICONTROL 动开始]****[!UICONTROL 和流结束]**。
...
1. 打开“工 **[!UICONTROL 作流模型]** ”控制台；通过工 **[!UICONTROL 具]**、工 **[!UICONTROL 作流]**、 **[!UICONTROL 模型]** ，或者，例如：

   [http://localhost:4502/aem/workflow](http://localhost:4502/aem/workflow)

1. Select **[!UICONTROL Create]**, then **[!UICONTROL Create Model]**.
1. 将出 **[!UICONTROL 现“添加工作流模型]** ”(Add Workflow Model)对话框。 在选择完 **[!UICONTROL 成之前]** ，输入标题 **[!UICONTROL 和名称]** （可选） ****。
1. 新模型将列在“工作流模 **[!UICONTROL 型”控制台中]** 。
1. 选择新的工作流，然后使用 [**[!UICONTROL 编辑&#x200B;]**，打开它进行配置](#editing-a-workflow):
...
* 配置工作流属性 [，包括](#configuring-workflow-stages-that-show-workflow-progress)阶段 [,](#creating-a-transient-workflow) 无论工作流是临时的 [，还是使用多个资源](#configuring-a-workflow-for-multi-resource-support)

编辑 [**默认或旧版&#x200B;**（现成）工作流还有一个额外步骤，以确保在进行更改之前](#editing-a-default-or-legacy-workflow-for-the-first-time)[](/help/sites-developing/workflows-best-practices.md#locations-workflow-models)执行安全副本。

对工作流的更新完成后，必须使用 **[!UICONTROL Sync]****[!UICONTROL 生成运行时模型]**。 有关详细 [信息，请参阅同步您的工作流](#sync-your-workflow-generate-a-runtime-model) 。

### 同步工作流——生成运行时模型 {#sync-your-workflow-generate-a-runtime-model}

**Sync** （在编辑器工具栏中）生成一个运 [行时模型](/help/sites-developing/workflows.md#runtime-model)。 运行时模型是用户启动工作流时实际使用的模型。 如果不同步 **[!UICONTROL 更改]** ，则更改在运行时将不可用。

当您（或任何其他用户）对工作流进行任何更改时，您必须使用 **[!UICONTROL Sync]** ，以生成运行时模型，即使单个对话框（例如，对于步骤）具有自己的保存选项时也是如此。

当更改与运行时（已保存）模型同步时， **[!UICONTROL 将显示]** “同步”。

某些步骤包含必填字段和／或内置验证。 当这些条件不满足时，当您尝试同步模型时，将显示 **[!UICONTROL 错误]** 。 例如，当尚未为“参加者”步骤定义任何参 **[!UICONTROL 加者]** :
...
编辑模型时，可用步骤显示在“步骤”浏览器的各组 **[!UICONTROL 中]** 。 例如：
...
1. 打开现有的工作流模型进行编辑。 从“工作 **[!UICONTROL 流模型]** ”控制台中，选择所需的模型，然后选择“编 **[!UICONTROL 辑”]**。
1. 打开“步 **[!UICONTROL 骤]** ”浏览器；使用 **[!UICONTROL “切换侧面板]**”（位于顶部工具栏的最左侧）。 在此对话框中，您可以：

   * **[!UICONTROL 针对特定步骤进行筛选]** 。
   * 使用下拉选择器将选择限制为特定步骤组。
...
1. 将相应的步骤拖动到模型中的所需位置。
   ...
   将其添加到流后，您可以配 [置该步骤](#configuring-a-workflow-step)。
...
您可以 **使用** “步骤属性”对话框配置和自定义工作流 **[!UICONTROL 步骤的行为]** 。

1. 要打开步骤 **[!UICONTROL 的“步骤属性]** ”对话框，请执行以下任一操作：

   * 点按工作流模型中的步骤，然后从组件 **[!UICONTROL 工具栏中]** 选择配置。
   * 双击该步骤。
   >[!NOTE]
   >
   >有关随AEM一起安装的主要步骤组件的信息，请参阅工作 [流步骤参考](/help/sites-developing/workflows-step-ref.md)。

1. 根据需 **[!UICONTROL 要配置步骤]** “属性”;可用的属性取决于步骤类型，也可能有多个可用的选项卡。 例如，新工作流中 **[!UICONTROL 的默认参加者步骤]**，显示为 `Step 1`:
...
1. 打开工作流模型进行 [编辑](#editing-a-workflow)。
1. 从工 **[!UICONTROL 具栏中选择工作流模型]** “属性”。
1. 在对话框中，激活“临时工 **[!UICONTROL 作流]** ”（或根据需要取消激活）:

   ![wf-07](assets/wf-07.png)

1. 使用保存并关闭 **[!UICONTROL 确认更改]**;后跟 **[!UICONTROL Sync]** （编辑器工具栏）以生成运行时模型。
...
### 使工作流模型在触屏UI中可用 {#make-workflow-models-available-in-touchui}
...
1. 在对话框中，激活多资 **[!UICONTROL 源支持]** （或根据需要取消激活）:
...
要为工作流 **[!UICONTROL 定义阶段]** ，请执行以下操作：

1. 打开工作流模型进行编辑。
1. 从工 **[!UICONTROL 具栏中选择工作流模型]** “属性”。 然后打开“ **[!UICONTROL 阶段]** ”选项卡。
1. 添加（并定位）您所需的 **[!UICONTROL 阶段]**。 您可以为工作流模型定义任意数量的阶段。
...
1. 使用包管理器创建新 [包](/help/sites-administering/package-manager.md#package-manager):
   ...
   1. 通过工具、部署、包导航到包 **[!UICONTROL 管理器]********[!UICONTROL ,]**。
   1. 单击“ **[!UICONTROL 创建包”]**。
   1. 根据需 **[!UICONTROL 要指定包名称]**，以及任何其他详细信息。
   1. 单击&#x200B;**[!UICONTROL 确定]**。

1. 单击 **[!UICONTROL 新包工具栏上的]** “编辑”。

1. Open the **[!UICONTROL Filters]** tab.

1. 选择 **[!UICONTROL 添加过滤器]** ，并指定工作流模型设计的路 *径*:

   `/conf/global/settings/workflow/models/<*your-model-name*>`

   单击&#x200B;**[!UICONTROL 完成]**。

1. 选择 **[!UICONTROL 添加过滤器]** ，并指定运行时工作流模 *型的路径* :
...
1. 单击 **[!UICONTROL 保存]** ，以确认您的过滤器定义。
1. 从包定 **[!UICONTROL 义的工具栏中]** ，选择“构建”。
1. 从包工 **[!UICONTROL 具栏中]** ，选择“下载”。
...
1. 配置在 **[!UICONTROL 页面中显示的]** “表单开始”组件。
1. 使用 **[!UICONTROL 启动工作流]** ，从以下可用工作流中选择所需的工作流：
...
1. 定义有 **[!UICONTROL 效负荷]** ，并确认。

1. 根据需要执行操作，以便继续工作流。
1. 在工作流运行时监视日志文件。

您还可以将AEM配置为在日 **[!UICONTROL 志文件中显示]** DEBUG消息。 请参 [阅记录](/help/sites-deploying/configure-logging.md) ，了解详细信息，开发完成后，将“日志级别 **[!UICONTROL ”设置回“信]** 息” ****。
...
1. 删 `Step 1` 除（因为此示例的步骤类型错误）:
   ...
   * 单击该步骤，然后从组件工 **[!UICONTROL 具栏中]** ，选择删除。 确认该操作。

1. 从步骤浏 **[!UICONTROL 览器的“工作流]** ”选择中，将“参与者步骤 **[!UICONTROL ”拖动到工作流上，并将其放置在“流开始”和]** ** ****Flow结束**之间。
1. 要打开属性对话框，请执行以下操作之一：

   * 单击参加者步骤，然后从组件工 **[!UICONTROL 具栏中]** 选择配置。
   * 双击参加者步骤。

1. 在“公 **[!UICONTROL 用]** ”选项卡 `Validate Content` 中，输入“标 **[!UICONTROL 题]** ”和“说 **[!UICONTROL 明”]**。
1. 打开“用 **[!UICONTROL 户／组”选项卡]** :

   * Activate **[!UICONTROL Notify user via email]**.
   * 为“ `Administrator` 用 `admin`户／组”字 **[!UICONTROL 段选择(]** )。
   >[!NOTE]
   >
...
1. 将“或 **[!UICONTROL 拆分]** ”拖到工作流上，并将其放在“流结束” `Validate Content` 和“ **[!UICONTROL 流结束”之间]**。
1. 打开或 **[!UICONTROL 拆分]** ，进行配置。
1. 配置：

   * **[!UICONTROL 常见]**:选择 **[!UICONTROL 2个分支]**
   * **[!UICONTROL 分支1]**:选择 **[!UICONTROL 默认路由]**。
   * **[!UICONTROL 分支2]**:确保 **[!UICONTROL 未选择默认路由]** 。

1. 确认对 **[!UICONTROL OR拆分的更新]**。
1. 将“参 **[!UICONTROL 加者步骤]** ”拖到左侧分支，打开属性，指定以下值，然后确认更改：

   * **[!UICONTROL 标题]**: `Reject Publish Request`
   * **[!UICONTROL 用户／用户组]**:例如， `projects-administrators`
   * **[!UICONTROL 通过电子邮件通知用户]**:激活后，通过电子邮件通知用户。

1. 将处理 **[!UICONTROL 步骤拖到右侧分支]** ，打开属性，指定以下值，然后确认更改：

   * **[!UICONTROL 标题]**: `Publish Page as Requested`
   * **[!UICONTROL 流程]**:选择 `Activate Page`。 此过程将选定页面发布到发布者实例。
...
1. 将此工作流应用于您的页面，以便当用户移动到“完成 **** 验证内容 **[!UICONTROL ”步骤时，他们可以选择要按请求发布页面]** ，还是 ********“发布请求拒绝”。

   ![chlimage_1-182](assets/chlimage_1-182.png)

### 示例：为OR拆分定义规则 {#example-defining-a-rule-for-an-or-split}

**[!UICONTROL “或者拆分]** ”步骤允许您将条件处理路径引入工作流。
...
1. 编辑工作流，并将“ **[!UICONTROL OR拆分]** ”(OR Split)添加到模型。
1. 编辑OR拆分 **[!UICONTROL 的分支]** 1的 **[!UICONTROL 属性]**:

   * 将“值”(Value)设 **[!UICONTROL 置为]** ，将其定义为“默 **[!UICONTROL 认路由]** ”(Default Route) `true`。
   * 作 **[!UICONTROL 为规则]**，设置脚本的路径。 例如：
...
1. 编辑OR拆分的 **[!UICONTROL Branch 2]** 的 **[!UICONTROL 属性]**。

   * 作为 **[!UICONTROL 规则]**，将路径设置为其他脚本。 例如：

      `/apps/myapp/workflow/scripts/myscript2.ecma`

1. 在每个分支中设置各个步骤的属性。 确保已 **[!UICONTROL 设置用户／组]** 。
...
例如， **[!UICONTROL 请求激活]**。 此工作流用于在 **[!UICONTROL Sites]** 中发布页面，并在内容作者没有相应的复制权限时自动触发。 有关更 [多详细信息，请参阅自定义页面创作——自定义激活请求工作流](/help/sites-developing/customizing-page-authoring-touch.md#customizing-the-request-for-activation-workflow) 。
