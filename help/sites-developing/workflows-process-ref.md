...
AEM提供了几个可用于创建工作流模型的流程步骤。 也可以为内置步骤未涵盖的任务添加自定义流程步骤(请参 [阅创建工作流模型](/help/sites-developing/workflows-models.md))。
...
例如，如果工作流应用于AEM页面 *P* , *则随着工作流的前进，P会随步骤传递* ，每个步骤都可 *选地对P* 执行操作。
...
参数在工作流编辑器的“属性”窗 **格的“进程参数** ”属性 **中以单个字** 符串的形式输入。 对于下面描述的每个过程，参数字符串的格式以简单的EBNF语法进行描述。 例如，以下指示参数字符串由一个或多个逗号分隔的对组成，其中每对都由名称（即字符串）和值(以多次冒号分隔)组成：
...
传递给的会话 `WorkflowProcess` 由工作流进程服务的服务用户备份，该服务在存储库的根位置具有以下权限：
...
如果该权限集不足以满足您的 `WorkflowProcess` 实现，则必须使用具有所需权限的会话。
...
>
>权限现在定义为上述([权限](#permissions))。 同样，更新实现的推荐方法也是如此。
>
...
>
>
此操作将恢复为向实施提供管理员会话的旧行为， `WorkflowProcess` 并再次提供对整个存储库的无限制访问。
...
(绝 `AbsoluteTimeAutoAdvancer` 对时间自动提前器 ****)进程对AutoAdvancer的行为与AutoAdvancer相同，只是它在给定时间和日期超时，而不是在给定时间长度后超时。
...
该流 `AutoAdvancer` 程会自动将工作流前进到下一步。 如果有多个可能的下一步（例如，如果存在OR拆分），则此流程将沿默认路由推进 *工作流*，如果已指定，则不会进行工作流。
...
该 `ProcessAssembler` 过程在一个工作流步骤中顺序地执行多个子进程。 要使用 `ProcessAssembler`此类型，请在工作流中创建一个此类型的步骤，并设置其参数以指示要执行的子进程的名称和参数。
