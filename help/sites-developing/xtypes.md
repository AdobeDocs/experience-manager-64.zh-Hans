...
* box
   ...
   使用宽度和高度，将大小设置为框的任何[组件](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.Component)的基类。
...
* browsedialog
   ...
   浏览对话框允许用户浏览存储库以选择路径。 它通常通过[BrowseField](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.form.BrowseField)使用。
...
* bulkeditor
   ...
   必须将BulkEditor插入HTML表单（导入功能需要）。 这非常适用于[CQ.Dialog](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Dialog)。
...
* 容器
   ...
   任何[CQ.Ext.BoxComponent](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.BoxComponent)（可能包含其他组件）的基类。 容器处理容器项目的基本行为，即添加、插入和删除项目。
...
* contentfinder
   ...
   ContentFinder是一个专门的两列[Viewport](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.Viewport)，其中包含左侧的实际内容查找器和右侧的内容框架。
...
* contentfindertab
   ...
   ContentFinderTab是一个专用面板，提供了[CQ.wcm.ContentFinder](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.wcm.ContentFinder)的选项卡面板中使用的功能。 通常，其特征是搜索表单（查询框）和显示搜索的数据视图。
...
* customcontentpanel
   ...
   CustomContentPanel是一种特殊类型的面板，用于[CQ.Dialog](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Dialog):其内容将从中检索，并提交到与对话框中其他字段不同的URL。
...
* datefield
   ...
   提供具有[CQ.Ext.DatePicker](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.DatePicker)下拉列表的日期输入字段和自动日期验证。
...
* datepicker
   ...
   弹出式日期选取器。 [DateField](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.form.DateField)类使用此类以允许浏览和选择有效日期。
...
* 编辑栏
   ...
   尽管未在此列出，但EditBar具有[CQ.wcm.EditBase](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.wcm.EditBase)的所有成员。
...
* fieldset
   ...
   用于对[表单](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.form.FormPanel)中的项目进行分组的标准容器。...
...
* 群组存储
   ...
   提供按可用字段之一对记录进行分组的专用存储实现。 这通常与[CQ.Ext.grid.GroupingView](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.grid.GroupingView)一起使用来证明分组GridPanel的数据模型。
...
* htmleditor
   ...
   编辑器的工具栏按钮在[buttonTips](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.form.HtmlEditor)属性中定义了工具提示。
...
* listview
   ...
   CQ.Ext.list.ListView是[Grid](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.grid.GridPanel)类视图的快速且轻量化实现。
...
* lvcolumn
   ...
   该类封装要用于初始化[ListView](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.list.ListView)的列配置数据。
...
* 门
   ...
   所有需要与菜单相关的功能（如子菜单）且不是静态显示项的菜单项的基类。 Item通过添加特定于菜单的激活和点击处理扩展了[CQ.Ext.menu.BaseItem](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.menu.BaseItem)的基本功能。
...
* 面板
   ...
   面板通过从[CQ.Ext.Container](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.Container)继承而来。
...
* 密码
   ...
   密码类似于[CQ.Ext.form.TextField](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.form.TextField)，但保持其值为私有，允许用户输入敏感数据。
...
* pathfield
   ...
   PathField是为路径完成路径而设计的输入字段，还是一个用于打开[CQ.BrowseDialog](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.BrowseDialog)以浏览服务器存储库的按钮。 它还可以浏览页面段落以生成高级链接。
...
* 进度
   ...
   在手动模式下，您负责显示、更新（通过[updateProgress](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.ProgressBar)）并根据需要从自己的代码中清除进度条。 当您想要显示进度时，此方法最合适。
...
* propertygrid
   ...
   一种专门的网格实施，旨在模拟开发IDE中常见的传统属性网格。 网格中的每一行都表示某些对象的属性，并且数据将作为一组名称/值对存储在[CQ.Ext.grid.PropertyRecord](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.grid.PropertyRecord)s中。
...
* 快速提示
   ...
   @xtype quicktip A工具提示的专用工具提示类，可在标记中指定并由全局[CQ.Ext.QuickTips](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.QuickTips)实例自动管理。 有关其他使用详细信息和示例，请参阅QuickTips类标题。
...
* 放射组
   ...
   用于[CQ.Ext.form.Radio](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.form.Radio)控件的分组容器。
...
* 汇总计划
   ...
   转出计划提供了一个用于监视页面转出进度的对话框。 RolloutPlan由[CQ.wcm.msm.RolloutWizard](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.wcm.msm.RolloutWizard)启动。
...
* 滚水板
   ...
   转出向导提供了用于转出页面的向导。 RolloutWizard启动[CQ.wcm.msm.RolloutPlan](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.wcm.msm.RolloutPlan)。
...
* textbutton
   ...
   TextButton提供了一个具有[CQ.Ext.Button](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.Button)功能的文本链接。
...
* 触发器
   ...
   为可添加可单击触发器按钮的TextFields提供了便捷的包装器（默认情况下类似于组合框）。 触发器没有默认操作，因此您必须分配一个函数以通过覆盖[onTriggerClick](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Ext.form.TriggerField)来实施触发器点击处理程序。 您可以直接创建TriggerField，因为它呈现的方式与组合框完全一样。
